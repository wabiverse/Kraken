...
#define KRF_VARIATIONS_MAX 16

#define MAKE_DVAR_TAG(a, b, c, d) \
  (((uint32_t)a << 24u) | ((uint32_t)b << 16u) | ((uint32_t)c << 8u) | ((uint32_t)d))

#define KRF_VARIATION_AXIS_WEIGHT MAKE_DVAR_TAG('w', 'g', 'h', 't')  /* 'wght' weight axis. */
#define KRF_VARIATION_AXIS_SLANT MAKE_DVAR_TAG('s', 'l', 'n', 't')   /* 'slnt' slant axis. */
#define KRF_VARIATION_AXIS_WIDTH MAKE_DVAR_TAG('w', 'd', 't', 'h')   /* 'wdth' width axis. */
#define KRF_VARIATION_AXIS_SPACING MAKE_DVAR_TAG('s', 'p', 'a', 'c') /* 'spac' spacing axis. */
#define KRF_VARIATION_AXIS_OPTSIZE MAKE_DVAR_TAG('o', 'p', 's', 'z') /* 'opsz' optical size. */

/* -------------------------------------------------------------------- */
/** \name Sub-Pixel Offset & Utilities
 *
 * Free-type uses fixed point precision for sub-pixel offsets.
 * Utility functions here avoid exposing the details in the KRF API.
 * \{ */
...
KLI_INLINE int ft_pix_to_int(ft_pix v)
{
#ifdef USE_LEGACY_SPACING
  return (int)(v >> 6);
#else
...
KLI_INLINE int ft_pix_to_int_floor(ft_pix v)
{
  return (int)(v >> 6); /* No need for explicit floor as the bits are removed when shifting. */
}

KLI_INLINE int ft_pix_to_int_ceil(ft_pix v)
{
  return (int)(FT_PIX_CEIL(v) >> 6);
}

KLI_INLINE ft_pix ft_pix_from_int(int v)
{
  return v * 64;
}

KLI_INLINE ft_pix ft_pix_from_float(float v)
{
  return lroundf(v * 64.0f);
}

KLI_INLINE ft_pix ft_pix_round_advance(ft_pix v, ft_pix step)
{
...
#define KRF_BATCH_DRAW_LEN_MAX 2048 /* in glyph */

/** Number of characters in #GlyphCacheKRF.glyph_ascii_table. */
#define GLYPH_ASCII_TABLE_SIZE 128

/** Number of characters in #KerningCacheKRF.table. */
...
typedef struct BatchKRF {
  ...
  struct GlyphCacheKRF *glyph_cache;
} BatchKRF;

extern BatchKRF g_batch;

typedef struct KerningCacheKRF {
  /**
...
typedef struct GlyphCacheKRF {
  struct GlyphCacheKRF *next;
  struct GlyphCacheKRF *prev;
  ...
  /** Fast ascii lookup */
  struct GlyphKRF *glyph_ascii_table[GLYPH_ASCII_TABLE_SIZE];
...
typedef struct GlyphKRF {
  struct GlyphKRF *next;
  struct GlyphKRF *prev;
...
typedef struct FontKRF {
  ...
  /**
   * List of glyph caches (#GlyphCacheKRF) for this font for size, DPI, bold, italic.
   * Use krf_glyph_cache_acquire(font) and krf_glyph_cache_release(font) to access cache!
   */
  ListBase cache;

  /** Cache of unscaled kerning values. Will be NULL if font does not have kerning. */
  KerningCacheKRF *kerning_cache;
  ...
  FontBufInfoKRF buf_info;

  /** Mutex lock for glyph cache. */
  ThreadMutex glyph_cache_mutex;
} FontKRF;

typedef struct DirKRF {
  struct DirKRF *next;
  struct DirKRF *prev;
