 ...
 * \ingroup krf
 *
 * Workaround for win32 which needs to use KLI_fopen to access files.
 *
                                       ...
                                       unsigned long count)
{
  ...
  if (stream->pos != offset) {
    KLI_fseek(file, offset, SEEK_SET);
  }
...
static FT_Error FT_Stream_Open__win32_compat(FT_Stream stream, const char *filepathname)
{
  ...
  file = KLI_fopen(filepathname, "rb");
  if (!file) {
    fprintf(stderr,
            "FT_Stream_Open: "
            "could not open `%s'\n",
