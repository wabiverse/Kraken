 ...
 */
#define KRF_GAMMA_CORRECT_GLYPHS
...
static GlyphCacheKRF *krf_glyph_cache_find(const FontKRF *font, const float size)
{
  GlyphCacheKRF *gc = (GlyphCacheKRF *)font->cache.first;
  while (gc) {
    if (gc->size == size && (gc->bold == ((font->flags & KRF_BOLD) != 0)) &&
        (gc->italic == ((font->flags & KRF_ITALIC) != 0)) &&
...
static GlyphCacheKRF *krf_glyph_cache_new(FontKRF *font)
{
  GlyphCacheKRF *gc = (GlyphCacheKRF *)MEM_callocN(sizeof(GlyphCacheKRF), "krf_glyph_cache_new");

  gc->next = NULL;
  gc->prev = NULL;
  gc->size = font->size;
  gc->bold = ((font->flags & KRF_BOLD) != 0);
  gc->italic = ((font->flags & KRF_ITALIC) != 0);
  ...
  krf_ensure_size(font);

  /* Determine ideal fixed-width size for monospaced output. */
  FT_UInt gindex = krf_get_char_index(font, U'0');
  if (gindex && font->face) {
    FT_Fixed advance = 0;
  ...
  KLI_addhead(&font->cache, gc);
  return gc;
}

GlyphCacheKRF *krf_glyph_cache_acquire(FontKRF *font)
{
  KLI_mutex_lock(&font->glyph_cache_mutex);

  GlyphCacheKRF *gc = krf_glyph_cache_find(font, font->size);

  if (!gc) {
    gc = krf_glyph_cache_new(font);
  }

  return gc;
}

void krf_glyph_cache_release(FontKRF *font)
{
  KLI_mutex_unlock(&font->glyph_cache_mutex);
}

static void krf_glyph_cache_free(GlyphCacheKRF *gc)
{
  GlyphKRF *g;
  for (uint i = 0; i < ARRAY_SIZE(gc->bucket); i++) {
    while ((g = KLI_pophead(&gc->bucket[i]))) {
      krf_glyph_free(g);
    }
  }
  if (gc->texture) {
...
void krf_glyph_cache_clear(FontKRF *font)
{
  GlyphCacheKRF *gc;

  KLI_mutex_lock(&font->glyph_cache_mutex);

  while ((gc = KLI_pophead(&font->cache))) {
    krf_glyph_cache_free(gc);
  }
 ...
 */
static GlyphKRF *krf_glyph_cache_find_glyph(const GlyphCacheKRF *gc, uint charcode)
{
  if (charcode < GLYPH_ASCII_TABLE_SIZE) {
    return gc->glyph_ascii_table[charcode];
  }

  GlyphKRF *g = gc->bucket[krf_hash(charcode)].first;
  while (g) {
    if (g->c == charcode) {
      return g;
    }
...
#ifdef KRF_GAMMA_CORRECT_GLYPHS
 ...
 */
static uchar krf_glyph_gamma(uchar c)
{
  /* The following is `(char)(powf(c / 256.0f, 1.0f / 1.43f) * 256.0f)`. */
  static const uchar gamma[256] = {
    0,   5,   9,   11,  14,  16,  19,  21,  23,  25,  26,  28,  30,  32,  34,  35,  37,  38,  40,
    41,  43,  44,  46,  47,  49,  50,  52,  53,  54,  56,  57,  58,  60,  61,  62,  64,  65,  66,
    67,  69,  70,  71,  72,  73,  75,  76,  77,  78,  79,  80,  82,  83,  84,  85,  86,  87,  88,
    89,  91,  92,  93,  94,  95,  96,  97,  98,  99,  100, 101, 102, 103, 104, 105, 106, 107, 108,
    109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 124, 125, 126, 127,
    128, 129, 130, 131, 132, 133, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 143, 144,
    145, 146, 147, 148, 149, 150, 151, 151, 152, 153, 154, 155, 156, 157, 157, 158, 159, 160, 161,
    162, 163, 163, 164, 165, 166, 167, 168, 168, 169, 170, 171, 172, 173, 173, 174, 175, 176, 177,
    178, 178, 179, 180, 181, 182, 182, 183, 184, 185, 186, 186, 187, 188, 189, 190, 190, 191, 192,
    193, 194, 194, 195, 196, 197, 198, 198, 199, 200, 201, 201, 202, 203, 204, 205, 205, 206, 207,
    208, 208, 209, 210, 211, 211, 212, 213, 214, 214, 215, 216, 217, 217, 218, 219, 220, 220, 221,
    222, 223, 223, 224, 225, 226, 226, 227, 228, 229, 229, 230, 231, 231, 232, 233, 234, 234, 235,
    236, 237, 237, 238, 239, 239, 240, 241, 242, 242, 243, 244, 244, 245, 246, 247, 247, 248, 249,
    249, 250, 251, 251, 252, 253, 254, 254, 255};
  return gamma[c];
}

#endif /* KRF_GAMMA_CORRECT_GLYPHS */

/**
 * Add a rendered glyph to a cache.
 */
static GlyphKRF *krf_glyph_cache_add_glyph(FontKRF *font,
                                           GlyphCacheKRF *gc,
                                           FT_GlyphSlot glyph,
                                           uint charcode,
                                           FT_UInt glyph_index)
{
  GlyphKRF *g = (GlyphKRF *)MEM_callocN(sizeof(GlyphKRF), "krf_glyph_get");
  g->c = charcode;
  g->idx = glyph_index;
  ...
  if (buffer_size != 0) {
    if (font->flags & KRF_MONOCHROME) {
      ...
      for (int i = 0; i < buffer_size; i++) {
        glyph->bitmap.buffer[i] = krf_glyph_gamma(glyph->bitmap.buffer[i]);
      }
#endif /* KRF_GAMMA_CORRECT_GLYPHS */
    }
  ...
  const uint key = krf_hash(g->c);
  KLI_addhead(&(gc->bucket[key]), g);
  if (charcode < GLYPH_ASCII_TABLE_SIZE) {
 ...
 */
static const struct UnicodeBlock *krf_charcode_to_unicode_block(const uint charcode)
{
  if (charcode < 0x80) {
...
static int krf_charcode_to_coverage_bit(uint charcode)
{
  int coverage_bit = -1;
  const struct UnicodeBlock *block = krf_charcode_to_unicode_block(charcode);
  if (block) {
...
static bool krf_font_has_coverage_bit(const FontKRF *font, int coverage_bit)
{
  if (coverage_bit < 0) {
    return false;
  }
 ...
 */
static FT_UInt krf_glyph_index_from_charcode(FontKRF **font, const uint charcode)
{
  FT_UInt glyph_index = krf_get_char_index(*font, charcode);
  if (glyph_index) {
    return glyph_index;
  }
  ...
  FontKRF *last_resort = NULL;
  int coverage_bit = krf_charcode_to_coverage_bit(charcode);
  for (int i = 0; i < KRF_MAX_FONT; i++) {
    FontKRF *f = global_font[i];
    if (!f || f == *font || !(f->flags & KRF_DEFAULT)) {
      continue;
    }

    if (f->flags & KRF_LAST_RESORT) {
      last_resort = f;
      continue;
    }
    if (coverage_bit < 0 || krf_font_has_coverage_bit(f, coverage_bit)) {
      glyph_index = krf_get_char_index(f, charcode);
      if (glyph_index) {
        *font = f;
        return glyph_index;
      }
  ...
  if (last_resort) {
    glyph_index = krf_get_char_index(last_resort, charcode);
    if (glyph_index) {
      *font = last_resort;
      return glyph_index;
    }
 ...
 */
static FT_GlyphSlot krf_glyph_load(FontKRF *font, FT_UInt glyph_index)
{
  int load_flags;
  ...
  if (font->flags & KRF_MONOCHROME) {
    ...
    if (font->flags & KRF_HINTING_NONE) {
      load_flags |= FT_LOAD_TARGET_NORMAL | FT_LOAD_NO_HINTING;
    } else if (font->flags & KRF_HINTING_SLIGHT) {
      load_flags |= FT_LOAD_TARGET_LIGHT;
    } else if (font->flags & KRF_HINTING_FULL) {
 ...
 */
static bool krf_glyph_render_bitmap(FontKRF *font, FT_GlyphSlot glyph)
{
  const int render_mode = (font->flags & KRF_MONOCHROME) ? FT_RENDER_MODE_MONO :
...
{
  int axis_index;
  const FT_Var_Axis *axis = krf_var_axis_by_tag(font->variations, tag, &axis_index);
  if (axis && (axis_index < KRF_VARIATIONS_MAX)) {
    coords[axis_index] = krf_factor_to_coordinate(axis, factor);
    return true;
  }
  return false;
}
...
static bool krf_glyph_transform_weight(FT_GlyphSlot glyph, float factor, bool monospaced)
{
  if (glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
    ...
    const FontKRF *font = (FontKRF *)glyph->face->generic.data;
 ...
 */
static bool krf_glyph_transform_monospace(FT_GlyphSlot glyph, int width)
{
  if (glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
 ...
 */
static FT_GlyphSlot krf_glyph_render(FontKRF *settings_font,
                                     FontKRF *glyph_font,
                                     FT_UInt glyph_index,
                                     uint charcode,
                                     int fixed_width)
{
  if (glyph_font != settings_font) {
    krf_font_size(glyph_font, settings_font->size);
  }

  krf_ensure_size(glyph_font);
  ...
  float weight = (settings_font->flags & KRF_BOLD) ? 0.7f : settings_font->char_weight;
  ...
  float slant = (settings_font->flags & KRF_ITALIC) ? 0.375f : settings_font->char_slant;
  ...
  if (glyph_font->variations) {
    FT_Fixed coords[KRF_VARIATIONS_MAX];
    /* Load current design coordinates. */
    FT_Get_Var_Design_Coordinates(glyph_font->face, KRF_VARIATIONS_MAX, &coords[0]);
    /* Update design coordinates with new values. */
    weight_done = krf_glyph_set_variation_normalized(glyph_font,
                                                     coords,
                                                     KRF_VARIATION_AXIS_WEIGHT,
                                                     weight);
    slant_done = krf_glyph_set_variation_normalized(glyph_font,
                                                    coords,
                                                    KRF_VARIATION_AXIS_SLANT,
                                                    slant);
    width_done = krf_glyph_set_variation_normalized(glyph_font,
                                                    coords,
                                                    KRF_VARIATION_AXIS_WIDTH,
                                                    width);
    spacing_done = krf_glyph_set_variation_normalized(glyph_font,
                                                      coords,
                                                      KRF_VARIATION_AXIS_SPACING,
                                                      spacing);
    /* Optical size, if available, is set to current font size. */
    krf_glyph_set_variation_float(glyph_font,
                                  coords,
                                  KRF_VARIATION_AXIS_OPTSIZE,
                                  settings_font->size);
    /* Save updated design coordinates. */
    FT_Set_Var_Design_Coordinates(glyph_font->face, KRF_VARIATIONS_MAX, &coords[0]);
  }

  FT_GlyphSlot glyph = krf_glyph_load(glyph_font, glyph_index);
  if (!glyph) {
    return NULL;
  }

  if ((settings_font->flags & KRF_MONOSPACED) && (settings_font != glyph_font)) {
    krf_glyph_transform_monospace(glyph, KLI_wcwidth((char32_t)charcode) * fixed_width);
  }

  /* Fallback glyph transforms, but only if required and not yet done. */

  if (weight != 0.0f && !weight_done) {
    krf_glyph_transform_weight(glyph, weight, FT_IS_FIXED_WIDTH(glyph_font));
  }
  if (slant != 0.0f && !slant_done) {
    krf_glyph_transform_slant(glyph, slant);
  }
  if (width != 0.0f && !width_done) {
    krf_glyph_transform_width(glyph, width);
  }
  if (spacing != 0.0f && !spacing_done) {
    krf_glyph_transform_spacing(glyph, spacing);
  }

  if (krf_glyph_render_bitmap(glyph_font, glyph)) {
    return glyph;
  }
  return NULL;
}

GlyphKRF *krf_glyph_ensure(FontKRF *font, GlyphCacheKRF *gc, const uint charcode)
{
  GlyphKRF *g = krf_glyph_cache_find_glyph(gc, charcode);
  if (g) {
    return g;
  }

  /* Glyph might not come from the initial font. */
  FontKRF *font_with_glyph = font;
  FT_UInt glyph_index = krf_glyph_index_from_charcode(&font_with_glyph, charcode);

  if (!krf_ensure_face(font_with_glyph)) {
    return NULL;
  }

  FT_GlyphSlot glyph =
    krf_glyph_render(font, font_with_glyph, glyph_index, charcode, gc->fixed_width);

  if (glyph) {
    /* Save this glyph in the initial font's cache. */
    g = krf_glyph_cache_add_glyph(font, gc, glyph, charcode, glyph_index);
  }

  return g;
}

void krf_glyph_free(GlyphKRF *g)
{
  if (g->bitmap) {
    MEM_freeN(g->bitmap);
  }
...
static void krf_glyph_calc_rect(rcti *rect, GlyphKRF *g, const int x, const int y)
{
  rect->xmin = x + g->pos[0];
...
static void krf_glyph_calc_rect_test(rcti *rect, GlyphKRF *g, const int x, const int y)
{
  /* Intentionally check with `g->advance`, because this is the
   * width used by KRF_width. This allows that the text slightly
...
static void krf_glyph_calc_rect_shadow(rcti *rect,
                                       GlyphKRF *g,
                                       const int x,
                                       const int y,
                                       FontKRF *font)
{
  krf_glyph_calc_rect(rect, g, x + font->shadow_x, y + font->shadow_y);
}
...
static void krf_texture_draw(const uchar color[4],
                             ...
                             const int y2)
{
  ...
  *((int *)GPU_vertbuf_raw_step(&g_batch.offset_step)) = offset;
  ...
  if (g_batch.glyph_len == KRF_BATCH_DRAW_LEN_MAX) {
    krf_batch_draw();
  }
}

static void krf_texture5_draw(const uchar color_in[4],
                              const int glyph_size[2],
                              const int offset,
                              const int x1,
                              const int y1,
                              ...
                              const int y2)
{
  ...
  krf_texture_draw(color_in, glyph_size_flag, offset, x1, y1, x2, y2);
}

static void krf_texture3_draw(const uchar color_in[4],
                              const int glyph_size[2],
                              const int offset,
                              const int x1,
                              const int y1,
...
void krf_glyph_draw(FontKRF *font, GlyphCacheKRF *gc, GlyphKRF *g, const int x, const int y)
{
  ...
  if (font->flags & KRF_CLIPPING) {
    rcti rect_test;
    krf_glyph_calc_rect_test(&rect_test, g, x, y);
    KLI_rcti_translate(&rect_test, font->pos[0], font->pos[1]);

    if (!KLI_rcti_inside_rcti(&font->clip_rec, &rect_test)) {
      return;
    }
  }
  ...
  if (font->flags & KRF_SHADOW) {
    rcti rect_ofs;
    krf_glyph_calc_rect_shadow(&rect_ofs, g, x, y, font);

    if (font->shadow == 0) {
      krf_texture_draw(font->shadow_color,
                       g->dims,
                       g->offset,
                       rect_ofs.xmin,
                       rect_ofs.ymin,
      ...
      krf_texture3_draw(font->shadow_color,
                        g->dims,
                        g->offset,
                        rect_ofs.xmin,
                        rect_ofs.ymin,
      ...
      krf_texture5_draw(font->shadow_color,
                        g->dims,
                        g->offset,
                        rect_ofs.xmin,
                        rect_ofs.ymin,
  ...
  rcti rect;
  krf_glyph_calc_rect(&rect, g, x, y);

#if KRF_BLUR_ENABLE
  switch (font->blur) {
    case 3:
      krf_texture3_draw(font->color,
                        g->dims,
                        g->offset,
                        rect.xmin,
                        rect.ymin,
...
#else
  krf_texture_draw(font->color, g->dims, g->offset, rect.xmin, rect.ymin, rect.xmax, rect.ymax);
#endif
}
