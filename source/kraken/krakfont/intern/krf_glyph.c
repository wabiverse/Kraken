...
static GlyphCacheKRF *krf_glyph_cache_find(const FontKRF *font, const float size, uint dpi)
{
  GlyphCacheKRF *gc = (GlyphCacheKRF *)font->cache.first;
  while (gc) {
    if (gc->size == size && gc->dpi == dpi && (gc->bold == ((font->flags & KRF_BOLD) != 0)) &&
...
GlyphCacheKRF *krf_glyph_cache_acquire(FontKRF *font)
{
  ...
  GlyphCacheKRF *gc = krf_glyph_cache_find(font, font->size, font->dpi);
 ...
 */
static GlyphKRF *krf_glyph_cache_add_glyph(
    FontKRF *font, GlyphCacheKRF *gc, FT_GlyphSlot glyph, uint charcode, FT_UInt glyph_index)
{
                                     ...
                                     int fixed_width)
{
  if (glyph_font != settings_font) {
    krf_font_size(glyph_font, settings_font->size, settings_font->dpi);
  }
  ...
  if (glyph_font->variations) {
    ...
    weight_done = krf_glyph_set_variation_normalized(
        glyph_font, coords, KRF_VARIATION_AXIS_WEIGHT, weight);
    slant_done = krf_glyph_set_variation_normalized(
        glyph_font, coords, KRF_VARIATION_AXIS_SLANT, slant);
    width_done = krf_glyph_set_variation_normalized(
        glyph_font, coords, KRF_VARIATION_AXIS_WIDTH, width);
    spacing_done = krf_glyph_set_variation_normalized(
        glyph_font, coords, KRF_VARIATION_AXIS_SPACING, spacing);
    /* Optical size, if available, is set to current font size. */
    krf_glyph_set_variation_float(
        glyph_font, coords, KRF_VARIATION_AXIS_OPTSIZE, settings_font->size);
...
GlyphKRF *krf_glyph_ensure(FontKRF *font, GlyphCacheKRF *gc, const uint charcode)
{
  ...
  FT_GlyphSlot glyph = krf_glyph_render(
...
static void krf_glyph_calc_rect_shadow(
    rcti *rect, GlyphKRF *g, const int x, const int y, FontKRF *font)
{
