...
#define KRF_SAMPLE_LEN 5

typedef struct UnicodeSample
{
  char32_t sample[KRF_SAMPLE_LEN];
...
static const char32_t *krf_get_sample_text(FT_Face face)
{
...
bool KRF_thumb_preview(const char *filename,
                       unsigned char *buf,
                       int w,
                       int h,
                       int UNUSED(channels))
{
  ...
  const char32_t *codepoints = krf_get_sample_text(face);
  uint glyph_ids[KRF_SAMPLE_LEN] = {0};
  ...
  int width = 0;
  for (uint i = 0; i < KRF_SAMPLE_LEN && codepoints[i]; i++) {
    glyph_ids[i] = FT_Get_Char_Index(face, codepoints[i]);
    /* If sample glyph is not found, use another. */
    if (!glyph_ids[i]) {
      glyph_ids[i] = (uint)(face->num_glyphs / (KRF_SAMPLE_LEN + 1)) * (i + 1);
    }
  ...
  for (int i = 0; i < KRF_SAMPLE_LEN && glyph_ids[i]; i++) {
