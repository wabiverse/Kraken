 ...
 *
 * Also low level functions for managing \a FontKRF.
 */
...
static void (*krf_draw_cache_flush)(void) = NULL;

static ft_pix krf_font_height_max_ft_pix(struct FontKRF *font);
static ft_pix krf_font_width_max_ft_pix(struct FontKRF *font);
 ...
 */
static void krf_face_finalizer(void *object)
{
  FT_Face face = object;
  FontKRF *font = (FontKRF *)face->generic.data;
  font->face = NULL;
}
 ...
 */
static void krf_size_finalizer(void *object)
{
  FT_Size size = object;
  FontKRF *font = (FontKRF *)size->generic.data;
  font->ft_size = NULL;
}
...
uint krf_get_char_index(FontKRF *font, uint charcode)
{
  ...
  return krf_ensure_face(font) ? FT_Get_Char_Index(font->face, charcode) : 0;
}

/* Convert a FreeType 26.6 value representing an unscaled design size to fractional pixels. */
static ft_pix krf_unscaled_F26Dot6_to_pixels(FontKRF *font, FT_Pos value)
{
 ...
 * is on demand only, between #KRF_batch_draw_begin() and #KRF_batch_draw_end().
 */
static void krf_batch_draw_init(void)
{
...
static void krf_batch_draw_exit(void)
{
  GPU_BATCH_DISCARD_SAFE(g_batch.batch);
}

void krf_batch_draw_begin(FontKRF *font)
{
  ...
  const bool simple_shader = ((font->flags & (KRF_ROTATION | KRF_MATRIX | KRF_ASPECT)) == 0);
...
static GPUTexture *krf_batch_cache_texture_load(void)
{
  GlyphCacheKRF *gc = g_batch.glyph_cache;
  KLI_assert(gc);
  KLI_assert(gc->bitmap_len > 0);
...
void krf_batch_draw(void)
{
  ...
  if (krf_draw_cache_flush != NULL) {
    krf_draw_cache_flush();
  }

  GPUTexture *texture = krf_batch_cache_texture_load();
...
static void krf_batch_draw_end(void)
{
  if (!g_batch.active) {
    krf_batch_draw();
  }
    ...
    FontKRF *font, GlyphCacheKRF *gc, const char *str, size_t str_len, size_t *i_p)
{
  ...
  KLI_assert(charcode != KLI_UTF8_ERR);
  return krf_glyph_ensure(font, gc, charcode);
}

KLI_INLINE ft_pix krf_kerning(FontKRF *font, const GlyphKRF *g_prev, const GlyphKRF *g)
{
  ft_pix adjustment = 0;
  ...
  if (FT_HAS_KERNING(font) && g_prev) {
    ...
    if (delta.x != 0) {
      ...
      adjustment += krf_unscaled_F26Dot6_to_pixels(font, delta.x);
    }
  }
...
static void krf_font_draw_ex(FontKRF *font,
                             GlyphCacheKRF *gc,
                             const char *str,
                             ...
                             const ft_pix pen_y)
{
  ...
  krf_batch_draw_begin(font);

  while ((i < str_len) && str[i]) {
    g = krf_glyph_from_utf8_and_step(font, gc, str, str_len, &i);
    ...
    krf_glyph_draw(font, gc, g, ft_pix_to_int_floor(pen_x), ft_pix_to_int_floor(pen_y));
...
void krf_font_draw(FontKRF *font, const char *str, const size_t str_len, struct ResultKRF *r_info)
{
  GlyphCacheKRF *gc = krf_glyph_cache_acquire(font);
  krf_font_draw_ex(font, gc, str, str_len, r_info, 0);
  krf_glyph_cache_release(font);
}

int krf_font_draw_mono(FontKRF *font, const char *str, const size_t str_len, int cwidth)
{
  GlyphKRF *g;
  int col, columns = 0;
...
/* Sanity checks are done by KRF_draw_buffer() */
static void krf_font_draw_buffer_ex(FontKRF *font,
                                    GlyphCacheKRF *gc,
                                    const char *str,
                                    const size_t str_len,
                                    struct ResultKRF *r_info,
...
void krf_font_draw_buffer(FontKRF *font,
                          const char *str,
                          const size_t str_len,
                          struct ResultKRF *r_info)
{
  GlyphCacheKRF *gc = krf_glyph_cache_acquire(font);
  krf_font_draw_buffer_ex(font, gc, str, str_len, r_info, 0);
  krf_glyph_cache_release(font);
}
    ...
    FontKRF *font, const char *str, const size_t str_len, int width, int *r_width)
{
  ...
  for (i_prev = i = 0, width_new = pen_x = 0, g_prev = NULL; (i < str_len) && str[i];
    ...
    if (krf_font_width_to_strlen_glyph_process(font, g_prev, g, &pen_x, width_i)) {
      break;
    }
  }
    ...
    FontKRF *font, const char *str, const size_t str_len, int width, int *r_width)
{
  ...
  i = KLI_strnlen(str, str_len);
  s = KLI_str_find_prev_char_utf8(&str[i], str);
  i = (size_t)(s - str);
  s_prev = KLI_str_find_prev_char_utf8(s, str);
  i_prev = (size_t)(s_prev - str);

  i_tmp = i;
  g = krf_glyph_from_utf8_and_step(font, gc, str, str_len, &i_tmp);
  for (width_new = pen_x = 0; (s != NULL);
    ...
    if (s_prev != NULL) {
      i_tmp = i_prev;
      g_prev = krf_glyph_from_utf8_and_step(font, gc, str, str_len, &i_tmp);
      KLI_assert(i_tmp == i);
    }

    if (krf_font_width_to_strlen_glyph_process(font, g_prev, g, &pen_x, width)) {
      break;
    }
  }
...
static void krf_font_boundbox_ex(FontKRF *font,
                                 GlyphCacheKRF *gc,
                                 const char *str,
                                 const size_t str_len,
                                 rcti *box,
...
void krf_font_width_and_height(FontKRF *font,
                               const char *str,
                               const size_t str_len,
                               float *r_width,
                               float *r_height,
                               struct ResultKRF *r_info)
{
  ...
  *r_height = ((float)KLI_rcti_size_y(&box) * ya);
}

float krf_font_width(FontKRF *font,
                     const char *str,
                     const size_t str_len,
                     struct ResultKRF *r_info)
{
...
float krf_font_height(FontKRF *font,
                      const char *str,
                      const size_t str_len,
                      struct ResultKRF *r_info)
{
...
float krf_font_fixed_width(FontKRF *font)
{
  GlyphCacheKRF *gc = krf_glyph_cache_acquire(font);
                                ...
                                void *userdata)
{
  ...
  ft_pix line_height = krf_font_height_max_ft_pix(font);
...
/* krf_font_draw__wrap */
static void krf_font_draw__wrap_cb(FontKRF *font,
                                   GlyphCacheKRF *gc,
                                   const char *str,
                                   const size_t str_len,
                                   ft_pix pen_y,
...
{
  krf_font_draw_ex(font, gc, str, str_len, NULL, pen_y);
}
void krf_font_draw__wrap(FontKRF *font,
                         const char *str,
                         const size_t str_len,
                         struct ResultKRF *r_info)
{
  krf_font_wrap_apply(font, str, str_len, r_info, krf_font_draw__wrap_cb, NULL);
}

/* krf_font_boundbox__wrap */
static void krf_font_boundbox_wrap_cb(FontKRF *font,
                                      GlyphCacheKRF *gc,
                                      const char *str,
                                      const size_t str_len,
                                      ft_pix pen_y,
    ...
    FontKRF *font, const char *str, const size_t str_len, rcti *box, struct ResultKRF *r_info)
{
  ...
  krf_font_wrap_apply(font, str, str_len, r_info, krf_font_boundbox_wrap_cb, box);
}

/* krf_font_draw_buffer__wrap */
static void krf_font_draw_buffer__wrap_cb(FontKRF *font,
                                          GlyphCacheKRF *gc,
                                          const char *str,
                                          const size_t str_len,
                                          ft_pix pen_y,
                                          void *UNUSED(userdata))
{
  krf_font_draw_buffer_ex(font, gc, str, str_len, NULL, pen_y);
}
void krf_font_draw_buffer__wrap(FontKRF *font,
                                const char *str,
                                const size_t str_len,
                                struct ResultKRF *r_info)
{
...
static ft_pix krf_font_height_max_ft_pix(FontKRF *font)
{
  krf_ensure_size(font);
...
int krf_font_height_max(FontKRF *font)
{
  return ft_pix_to_int(krf_font_height_max_ft_pix(font));
}

static ft_pix krf_font_width_max_ft_pix(FontKRF *font)
{
  krf_ensure_size(font);
...
int krf_font_width_max(FontKRF *font)
{
  return ft_pix_to_int(krf_font_width_max_ft_pix(font));
}

int krf_font_descender(FontKRF *font)
{
  krf_ensure_size(font);
  return ft_pix_to_int((ft_pix)font->ft_size->metrics.descender);
}

int krf_font_ascender(FontKRF *font)
{
  krf_ensure_size(font);
  return ft_pix_to_int((ft_pix)font->ft_size->metrics.ascender);
}

char *krf_display_name(FontKRF *font)
{
  if (!krf_ensure_face(font) || !font->face->family_name) {
    return NULL;
  }
  return KLI_sprintfN("%s %s", font->face->family_name, font->face->style_name);
}
...
{
  krf_draw_cache_flush = cache_flush_fn;
}
...
static void krf_font_fill(FontKRF *font)
{
  font->aspect[0] = 1.0f;
  font->aspect[1] = 1.0f;
  font->aspect[2] = 1.0f;
  ...
  /* annoying bright color so we can see where to add KRF_color calls */
  font->color[0] = 255;
  font->color[1] = 255;
  font->color[2] = 0;
  font->color[3] = 255;
 ...
 */
bool krf_ensure_face(FontKRF *font)
{
  if (font->face) {
    return true;
  }

  if (font->flags & KRF_BAD_FONT) {
    return false;
  }
  ...
  if (font->filepath) {
    char *mfile = krf_dir_metrics_search(font->filepath);
    if (mfile) {
  ...
  if (FT_IS_FIXED_WIDTH(font)) {
    font->flags |= KRF_MONOSPACED;
  }

  if (FT_HAS_KERNING(font) && !font->kerning_cache) {
    /* Create kerning cache table and fill with value indicating "unset". */
    font->kerning_cache = MEM_mallocN(sizeof(KerningCacheKRF), __func__);
 ...
 */
FontKRF *krf_font_new_ex(const char *name,
                         const char *filepath,
                         const uchar *mem,
                         const size_t mem_size,
                         void *ft_library)
{
  FontKRF *font = (FontKRF *)MEM_callocN(sizeof(FontKRF), "krf_font_new");

  font->name = KLI_strdup(name);
  font->filepath = filepath ? KLI_strdup(filepath) : NULL;
  if (mem) {
    font->mem = (void *)mem;
  ...
  KLI_mutex_init(&font->glyph_cache_mutex);
  ...
  if (font->filepath) {
    ...
    for (int i = 0; i < (int)ARRAY_SIZE(static_face_details); i++) {
      KLI_split_file_part(font->filepath, filename, sizeof(filename));
  ...
  if (font->unicode_ranges[0] == 0xffffffffU && font->unicode_ranges[1] == 0xffffffffU &&
    ...
    font->flags |= KRF_LAST_RESORT;
  }

  return font;
}

FontKRF *krf_font_new(const char *name, const char *filepath)
{
  return krf_font_new_ex(name, filepath, NULL, 0, NULL);
}

FontKRF *krf_font_new_from_mem(const char *name, const uchar *mem, const size_t mem_size)
{
  return krf_font_new_ex(name, NULL, mem, mem_size, NULL);
}

void krf_font_attach_from_mem(FontKRF *font, const uchar *mem, const size_t mem_size)
{
  FT_Open_Args open;
...
void krf_font_free(FontKRF *font)
{
  ...
  KLI_mutex_end(&font->glyph_cache_mutex);
...
void krf_ensure_size(FontKRF *font)
{
  if (font->ft_size || !(font->flags & KRF_CACHED)) {
    return;
  }
