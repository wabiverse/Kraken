...
#ifndef __USD_USERDEF_TYPES_H__
#define __USD_USERDEF_TYPES_H__
...
#include "USD_texture_types.h" /* ColorBand */
// #include "USD_userdef_enums.h"
...
/* Themes; defines in `KIF_resource.h`. */
 ...
 * A theme.
 *
 * @note Currently only a single theme is ever used at once.
 * Different theme presets are stored as external files now.
 */
typedef struct kTheme
{
  struct kTheme *next, *prev;
  char name[32];
...
typedef struct kPathCompare
{
  struct kPathCompare *next, *prev;
  /** FILE_MAXDIR. */
  char path[768];
  char flag;
  char _pad0[7];
...
typedef struct kUserMenu
{
  struct kUserMenu *next, *prev;
  char space_type;
  char _pad0[7];
  char context[64];
  /* kUserMenuItem */
...
typedef struct kUserMenuItem
{
  struct kUserMenuItem *next, *prev;
  char ui_name[64];
  char type;
  char _pad0[7];
} kUserMenuItem;

typedef struct kUserMenuItem_Op
{
  kUserMenuItem item;
  char op_idname[64];
...
typedef struct kUserMenuItem_Menu
{
  kUserMenuItem item;
  char mt_idname[64];
} kUserMenuItem_Menu;

typedef struct kUserMenuItem_Prop
{
  kUserMenuItem item;
...
typedef struct kUserAssetLibrary
{
  struct kUserAssetLibrary *next, *prev;
...
typedef struct UserDef
{
   ...
   *
   * This supports the same layout as Kraken's scripts directory `release/scripts`.
   *
   ...
   * launch with a new version of Kraken. In this case setting the script path on top of
 ...
 * @note #eAutokey_Flag is used with a macro, search for lines like IS_AUTOKEY_FLAG(INSERTAVAIL).
 */
