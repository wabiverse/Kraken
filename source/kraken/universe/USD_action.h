...
#include "USD_userdef_types.h" /* ThemeWireColor */
...
typedef struct kMotionPathVert
{
 ...
 */
typedef struct kMotionPath
{
  /** Path samples. */
  kMotionPathVert *points;
...
typedef struct kAnimVizSettings
{
  /* General Settings ------------------------ */
...
/* kAnimVizSettings->path_viewflag */
...
/* kAnimVizSettings->path_bakeflag */
...
/* runtime */
typedef struct kPoseChannelDrawData
{
  float solid_color[4];
  float wire_color[4];
...
} kPoseChannelDrawData;

struct DualQuat;
struct Mat4;

typedef struct kPoseChannel_Runtime
{
 ...
 * PoseChannel
 *
 * A #kPoseChannel stores the results of Actions and transform information
 * with respect to the rest-position of #bArmature bones.
 */
typedef struct kPoseChannel
{
  struct kPoseChannel *next, *prev;
   ...
   *
   * \note This depends #kPoseChannel.custom being set and the #ARM_NO_CUSTOM flag being unset.
   */
  struct kPoseChannel *custom_tx;
  ...
  struct kPoseChannel *bbone_prev;
  struct kPoseChannel *bbone_next;

  /** Use for outliner. */
  void *temp;
  /** Runtime data for color and bbone segment matrix. */
  kPoseChannelDrawData *draw_data;

  /** Points to an original pose channel. */
  struct kPoseChannel *orig_pchan;

  /** Runtime data (keep last). */
  struct kPoseChannel_Runtime runtime;
} kPoseChannel;
...
/* kPose->iksolver and kPose->ikparam->iksolver */
...
/* header for all kPose->ikparam structures */
typedef struct kIKParam
{
  int iksolver;
} kIKParam;

/* kPose->ikparam when kPose->iksolver=1 */
typedef struct kItasc
{
  int iksolver;
  float precision;
  short numiter;
 ...
 *
 * This is also exploited for bone-groups. Bone-Groups are stored per kPose, and are used
 ...
 */
typedef struct kActionGroup
{
  struct kActionGroup *next, *prev;
...
typedef struct kAction
{
  ...
  ListBase curves;
  /** Groups of function-curves (kActionGroup). */
  ListBase groups;
