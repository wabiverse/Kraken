...
#pragma KRAKEN_REQUIRE(raytrace_lib.glsl)
#pragma KRAKEN_REQUIRE(ltc_lib.glsl)
...
float cubeFaceIndexPHOENIX(vec3 P)
{
  vec3 aP = abs(P);
...
vec2 cubeFaceCoordPHOENIX(vec3 P, float face, float scale)
{
  if (face < 2.0) {
...
vec2 cubeFaceCoordPHOENIX(vec3 P, float face, sampler2DArray tex)
{
  ...
  return cubeFaceCoordPHOENIX(P, face, scale);
}

vec2 cubeFaceCoordPHOENIX(vec3 P, float face, sampler2DArrayShadow tex)
{
  /* Scaling to compensate the 1px border around the face. */
  float cube_res = float(textureSize(tex, 0).x);
  float scale = (cube_res) / (cube_res + 1.0);
...
vec4 sample_cube(sampler2DArray tex, vec3 cubevec, float cube)
{
  ...
  float face = cubeFaceIndexPHOENIX(cubevec);
  vec2 uv = cubeFaceCoordPHOENIX(cubevec, face, tex);
...
float sample_cube_shadow(int shadow_id, vec3 P)
{
  ...
  float face = cubeFaceIndexPHOENIX(cubevec);
  vec2 coord = cubeFaceCoordPHOENIX(cubevec, face, shadowCubeTexture);
