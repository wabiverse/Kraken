...
struct ViewInfos
{
  ...
  /** @note vec3 array padded to vec4. */
...
KLI_STATIC_ASSERT_ALIGN(ViewInfos, 16)
...
KLI_STATIC_ASSERT_ALIGN(ObjectMatrices, 16)
...
#if !defined(GPU_SHADER) && defined(__cplusplus)
  void sync();
  void sync(const kraken::draw::ObjectRef ref, bool is_active_object);
#endif
};
KLI_STATIC_ASSERT_ALIGN(ObjectInfos, 16)
...
KLI_STATIC_ASSERT_ALIGN(ObjectBounds, 16)
...
KLI_STATIC_ASSERT_ALIGN(VolumeInfos, 16)
...
KLI_STATIC_ASSERT_ALIGN(CurvesInfos, 16)
...
#if !defined(GPU_SHADER) && defined(__cplusplus)
  bool sync(const kraken::draw::ObjectRef &ref, const GPUUniformAttr &attr);
#endif
};
#pragma pack(pop)
/** @note we only align to 4 bytes and fetch data manually so make sure
 * C++ compiler gives us the same size. */
KLI_STATIC_ASSERT_ALIGN(ObjectAttribute, 20)

/** @} */

/* -------------------------------------------------------------------- */
/** @name Indirect commands structures.
 * @{ */
...
KLI_STATIC_ASSERT_ALIGN(DrawCommand, 16)
...
KLI_STATIC_ASSERT_ALIGN(DispatchCommand, 16)
...
KLI_STATIC_ASSERT_ALIGN(DRWDebugPrintBuffer, 16)
...
KLI_STATIC_ASSERT_ALIGN(DRWDebugVert, 16)
