...
KLI_INLINE uint32_t DRW_handle_negative_scale_get(const DRWResourceHandle *handle)
{
  return (*handle & 0x80000000) != 0;
}

KLI_INLINE uint32_t DRW_handle_chunk_get(const DRWResourceHandle *handle)
{
  return (*handle & 0x7FFFFFFF) >> 9;
}

KLI_INLINE uint32_t DRW_handle_id_get(const DRWResourceHandle *handle)
{
  return (*handle & 0x000001FF);
}

KLI_INLINE void DRW_handle_increment(DRWResourceHandle *handle)
{
  *handle += 1;
}

KLI_INLINE void DRW_handle_negative_scale_enable(DRWResourceHandle *handle)
{
  *handle |= 0x80000000;
}

// KLI_INLINE void *DRW_memblock_elem_from_handle(struct KLI_memblock *memblock,
//                                                const DRWResourceHandle *handle)
// {
//   int elem = DRW_handle_id_get(handle);
//   int chunk = DRW_handle_chunk_get(handle);
//   return KLI_memblock_elem_get(memblock, chunk, elem);
// }
...
KLI_STATIC_ASSERT_ALIGN(DRWObjectMatrix, 16)
KLI_STATIC_ASSERT_ALIGN(DRWObjectInfos, 16)
...
#ifdef __LP64__
KLI_STATIC_ASSERT_ALIGN(DRWCommandChunk, 16);
#endif
...
typedef struct DRWData
{
  ...
  struct KLI_memblock *commands;
  struct KLI_memblock *commands_small;
  struct KLI_memblock *callbuffers;
  struct KLI_memblock *obmats;
  struct KLI_memblock *obinfos;
  struct KLI_memblock *cullstates;
  struct KLI_memblock *shgroups;
  struct KLI_memblock *uniforms;
  struct KLI_memblock *views;
  struct KLI_memblock *passes;
  struct KLI_memblock *images;
