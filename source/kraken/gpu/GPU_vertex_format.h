...
KLI_STATIC_ASSERT(GPU_VERT_ATTR_NAMES_BUF_LEN <= 256,
...
uint GPU_vertformat_attr_add(GPUVertFormat *,
                             const char *name,
                             GPUVertCompType,
                             uint comp_len,
                             GPUVertFetchMode);
...
KLI_INLINE const char *GPU_vertformat_attr_name_get(const GPUVertFormat *format,
...
KLI_INLINE int clampi(int x, int min_allowed, int max_allowed)
{
#if TRUST_NO_ONE
...
KLI_INLINE int gpu_convert_normalized_f32_to_i10(float x)
{
  int qx = x * 511.0f;
  return clampi(qx, SIGNED_INT_10_MIN, SIGNED_INT_10_MAX);
}

KLI_INLINE int gpu_convert_i16_to_i10(short x)
{
...
KLI_INLINE GPUPackedNormal GPU_normal_convert_i10_v3(const float data[3])
{
  GPUPackedNormal n = {
...
KLI_INLINE GPUPackedNormal GPU_normal_convert_i10_s3(const short data[3])
{
  GPUPackedNormal n = {
...
KLI_INLINE void GPU_normal_convert_v3(GPUNormal *gpu_normal,
                                      const float data[3],
