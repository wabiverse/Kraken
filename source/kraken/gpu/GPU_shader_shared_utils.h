 ...
 * We use the same vector and matrix types as Kraken C++. Some math functions are defined to use
...
#ifdef GPU_SHADER
#  define KLI_STATIC_ASSERT_ALIGN(type_, align_)
#  define KLI_STATIC_ASSERT_SIZE(type_, size_)
#  define static
#  define inline
#  define cosf cos
#  define sinf sin
...
#  ifdef __cplusplus
#    include "KLI_float4x4.hh"
#    include "KLI_math_vec_types.hh"
using kraken::float2;
using kraken::float3;
using kraken::float4;
using kraken::float4x4;
using kraken::int2;
using kraken::int3;
using kraken::int4;
using kraken::uint2;
...
using kraken::uint4;
using bool1 = int;
using bool2 = kraken::int2;
using bool3 = kraken::int3;
using bool4 = kraken::int4;
