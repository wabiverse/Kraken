...
#define CHECK(test_value, str, ofs, msg) \
  if ((test_value) == -1) {              \
     ...
     * - Enums needs to have underlying types specified to uint32_t to make them usable in
     * UBO/SSBO.
    ...
    void material_functions_parse(GPUFunctionDictionnary *g_functions)
    {
      ...
      auto function_parse = [&](const StringRef input,
        ...
        if (body_start != -1 && next_semicolon != -1 && body_start > next_semicolon) {
          ...
          KLI_assert_msg(false, "No prototypes allowed in node GLSL libraries.");
        }
      ...
      while (function_parse(input, cursor, func_return_type, func_name, func_args)) {
        ...
        if (!insert) {
          ...
          if (other_source != this) {
            ...
            print_error(input,
                        other_source->source.find(func_name),
...
#undef rfind_token
                          ...
                          const GPUFunctionDictionnary &g_functions)
    {
      ...
      while (true) {
        ...
        {
          pos = source.find("pragma KRAKEN_REQUIRE(", pos + 1);
          if (pos == -1) {
            return 0;
          }
          ...
          if (end == -1) {
            print_error(source, start, "Malformed KRAKEN_REQUIRE: Missing \")\" token");
            return 1;
          }
...
#undef SHADER_SOURCE
  ...
  KLI_assert_msg(errors == 0, "Dependency errors detected: Aborting");
...
GPUFunction *gpu_material_library_use_function(RSet *used_libraries, const char *name)
{
  GPUFunction *function = g_functions->lookup_default(name, nullptr);
  KLI_assert_msg(function != nullptr, "Requested function not in the function library");
  GPUSource *source = reinterpret_cast<GPUSource *>(function->source);
  KLI_rset_add(used_libraries, const_cast<char *>(source->filename.c_str()));
  return function;
}
