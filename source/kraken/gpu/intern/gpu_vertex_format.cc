...
void GPU_vertformat_multiload_enable(GPUVertFormat *format, int load_count)
{
  ...
  KLI_assert(load_count > 1 && load_count < 5);
  /* We need a packed format because of format->stride. */
  if (!format->packed) {
    VertexFormat_pack(format);
  }

  KLI_assert((format->name_len + 1) * load_count < GPU_VERT_FORMAT_MAX_NAMES);
  KLI_assert(format->attr_len * load_count <= GPU_VERT_ATTR_MAX_LEN);
  KLI_assert(format->name_offset * load_count < GPU_VERT_ATTR_NAMES_BUF_LEN);
  ...
  for (int i = 0; i < attr_len; i++, attr++) {
    ...
    for (int j = 1; j < load_count; j++) {
      char load_name[64];
      KLI_snprintf(load_name, sizeof(load_name), "%s%d", attr_name, j);
...
{
  KLI_assert(attr_id > -1 && attr_id < format->attr_len);
  GPUVertAttr *attr = &format->attrs[attr_id];
  char *attr_name = (char *)GPU_vertformat_attr_name_get(format, attr, 0);
  KLI_assert(strlen(attr_name) == strlen(new_name));
  int i = 0;
  while (attr_name[i] != '\0') {
...
void GPU_vertformat_safe_attr_name(const char *attr_name, char *r_safe_name, uint UNUSED(max_len))
{
  ...
  KLI_assert(GPU_MAX_SAFE_ATTR_NAME >= 12);
#if 0 /* For debugging */
...
void VertexFormat_texture_buffer_pack(GPUVertFormat *format)
{
   ...
   * to mis-align. */
  KLI_assert_msg(format->attr_len == 1,
