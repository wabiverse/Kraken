...
namespace kraken::gpu
{
  ...
  void Texture::attach_to(FrameBuffer *fb, GPUAttachmentType type)
  {
    ...
    KLI_assert_msg(0, "GPU: Error: Texture: Not enough attachment");
  }
  ...
  void Texture::detach_from(FrameBuffer *fb)
  {
    ...
    KLI_assert_msg(0, "GPU: Error: Texture: Framebuffer is not attached");
  }
...
GPUTexture *GPU_texture_create_from_vertbuf(const char *name, GPUVertBuf *vert)
{
  ...
  KLI_assert_msg(unwrap(vert)->extended_usage_ & GPU_USAGE_FLAG_BUFFER_TEXTURE_ONLY,
...
{
  KLI_assert(data != nullptr); /* Do not accept NULL as parameter. */
...
void GPU_texture_compare_mode(GPUTexture *tex_, bool use_compare)
{
  ...
  KLI_assert(!(use_compare) || (tex->format_flag_get() & GPU_FORMAT_DEPTH));
...
void GPU_texture_mipmap_mode(GPUTexture *tex_, bool use_mipmap, bool use_filter)
{
  Texture *tex = reinterpret_cast<Texture *>(tex_);
  /* Stencil and integer format does not support filtering. */
  KLI_assert(!(use_filter || use_mipmap) ||
             !(tex->format_flag_get() & (GPU_FORMAT_STENCIL | GPU_FORMAT_INTEGER)));
...
{
  KLI_assert(GPU_texture_stencil(tex) || !use_stencil);
...
{
  KLI_assert(py_ref == nullptr || unwrap(tex)->py_ref == nullptr);
