                       ...
                       eGPUBatchFlag owns_flag)
{
  ...
  KLI_assert((owns_flag & ~(GPU_BATCH_OWNS_VBO | GPU_BATCH_OWNS_INDEX)) == 0);
...
int GPU_batch_instbuf_add_ex(GPUBatch *batch, GPUVertBuf *insts, bool own_vbo)
{
  ...
  for (uint v = 0; v < GPU_BATCH_INST_VBO_MAX_LEN; v++) {
    if (batch->inst[v] == nullptr) {
      ...
      if (batch->inst[0]) {
        ...
        // KLI_assert(insts->vertex_len == batch->inst[0]->vertex_len);
      }
  ...
  KLI_assert_msg(0, "Not enough Instance VBO slot in batch");
  return -1;
}
...
int GPU_batch_vertbuf_add_ex(GPUBatch *batch, GPUVertBuf *verts, bool own_vbo)
{
  ...
  for (uint v = 0; v < GPU_BATCH_VBO_MAX_LEN; v++) {
    if (batch->verts[v] == nullptr) {
      ...
      if (batch->verts[0] != nullptr) {
        ...
        // KLI_assert(verts->vertex_len == batch->verts[0]->vertex_len);
      }
      batch->verts[v] = verts;
  ...
  /* we only make it this far if there is no room for another GPUVertBuf */
  KLI_assert_msg(0, "Not enough VBO slot in batch");
  return -1;
}
...
/* -------------------------------------------------------------------- */
/** @name Drawing / Drawcall functions
 * @{ */
...
{
  KLI_assert(batch->inst[0] == nullptr);
...
{
  KLI_assert(Context::get()->shader != nullptr);
  Batch *batch = static_cast<Batch *>(gpu_batch);
...
{
  KLI_assert(Context::get()->shader != nullptr);
  KLI_assert(indirect_buf != nullptr);
  Batch *batch = static_cast<Batch *>(gpu_batch);
