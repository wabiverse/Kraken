...
{
  KLI_assert(imm->prim_type == GPU_PRIM_NONE); /* Make sure we haven't already begun. */
  KLI_assert(vertex_count_makes_sense_for_primitive(vertex_len, prim_type));
...
void immEnd()
{
  KLI_assert(imm->prim_type != GPU_PRIM_NONE); /* Make sure we're between a Begin/End pair. */
  KLI_assert(imm->vertex_data || imm->batch);

  if (imm->strict_vertex_len) {
    KLI_assert(imm->vertex_idx == imm->vertex_len); /* With all vertices defined. */
  }
  else {
    KLI_assert(imm->vertex_idx <= imm->vertex_len);
    KLI_assert(imm->vertex_idx == 0 ||
...
static void setAttrValueBit(uint attr_id)
{
  ...
  KLI_assert(imm->unassigned_attr_bits & mask); /* not already set */
...
void immAttr1f(uint attr_id, float x)
{
  ...
  KLI_assert(attr_id < imm->vertex_format.attr_len);
  KLI_assert(attr->comp_type == GPU_COMP_F32);
  KLI_assert(attr->comp_len == 1);
  KLI_assert(imm->vertex_idx < imm->vertex_len);
  KLI_assert(imm->prim_type != GPU_PRIM_NONE); /* make sure we're between a Begin/End pair */
...
void immAttr1u(uint attr_id, uint x)
{
  GPUVertAttr *attr = &imm->vertex_format.attrs[attr_id];
  KLI_assert(attr_id < imm->vertex_format.attr_len);
  KLI_assert(attr->comp_type == GPU_COMP_U32);
  KLI_assert(attr->comp_len == 1);
  KLI_assert(imm->vertex_idx < imm->vertex_len);
  KLI_assert(imm->prim_type != GPU_PRIM_NONE); /* make sure we're between a Begin/End pair */
  setAttrValueBit(attr_id);
...
void immAttr2i(uint attr_id, int x, int y)
{
  GPUVertAttr *attr = &imm->vertex_format.attrs[attr_id];
  KLI_assert(attr_id < imm->vertex_format.attr_len);
  KLI_assert(attr->comp_type == GPU_COMP_I32);
  KLI_assert(attr->comp_len == 2);
  KLI_assert(imm->vertex_idx < imm->vertex_len);
  KLI_assert(imm->prim_type != GPU_PRIM_NONE); /* make sure we're between a Begin/End pair */
  setAttrValueBit(attr_id);
...
void immAttr2s(uint attr_id, short x, short y)
{
  GPUVertAttr *attr = &imm->vertex_format.attrs[attr_id];
  KLI_assert(attr_id < imm->vertex_format.attr_len);
  KLI_assert(attr->comp_type == GPU_COMP_I16);
  KLI_assert(attr->comp_len == 2);
  KLI_assert(imm->vertex_idx < imm->vertex_len);
  KLI_assert(imm->prim_type != GPU_PRIM_NONE); /* make sure we're between a Begin/End pair */
  setAttrValueBit(attr_id);
...
void immAttr3ub(uint attr_id, uchar r, uchar g, uchar b)
{
  GPUVertAttr *attr = &imm->vertex_format.attrs[attr_id];
  KLI_assert(attr_id < imm->vertex_format.attr_len);
  KLI_assert(attr->comp_type == GPU_COMP_U8);
  KLI_assert(attr->comp_len == 3);
  KLI_assert(imm->vertex_idx < imm->vertex_len);
  KLI_assert(imm->prim_type != GPU_PRIM_NONE); /* make sure we're between a Begin/End pair */
  setAttrValueBit(attr_id);
...
static void immEndVertex() /* and move on to the next vertex */
{
  ...
  if (imm->unassigned_attr_bits) {
    KLI_assert(imm->vertex_idx > 0); /* first vertex must have all attributes specified */
