...
static void gpu_viewport_textures_create(GPUViewport *viewport)
{
  ...
  if (viewport->color_render_tx[0] == NULL) {
    viewport->color_render_tx[0] =
      GPU_texture_create_2d("dtxl_color", UNPACK2(size), 1, GPU_RGBA16F, NULL);
    viewport->color_overlay_tx[0] =
      GPU_texture_create_2d("dtxl_color_overlay", UNPACK2(size), 1, GPU_SRGB8_A8, NULL);
  ...
  if ((viewport->flag & GPU_VIEWPORT_STEREO) != 0 && viewport->color_render_tx[1] == NULL) {
    viewport->color_render_tx[1] =
      GPU_texture_create_2d("dtxl_color_stereo", UNPACK2(size), 1, GPU_RGBA16F, NULL);
    viewport->color_overlay_tx[1] =
      GPU_texture_create_2d("dtxl_color_overlay_stereo", UNPACK2(size), 1, GPU_SRGB8_A8, NULL);
    if (GPU_clear_viewport_workaround()) {
  ...
  if (viewport->depth_tx == NULL) {
    viewport->depth_tx =
      GPU_texture_create_2d("dtxl_depth", UNPACK2(size), 1, GPU_DEPTH24_STENCIL8, NULL);
  }
...
void GPU_viewport_bind(GPUViewport *viewport, int view, const rcti *rect)
{
  ...
  rect_size[0] = KLI_rcti_size_x(rect) + 1;
  rect_size[1] = KLI_rcti_size_y(rect) + 1;
                                 ...
                                 float dither)
{
  ...
  if (view_settings->curve_mapping) {
    if (viewport->view_settings.curve_mapping) {
      if (view_settings->curve_mapping->changed_timestamp !=
        ...
        KKE_color_managed_view_settings_free(&viewport->view_settings);
      }
    }
  }
  ...
  KKE_color_managed_view_settings_copy(&viewport->view_settings, view_settings);
  /* Restore. */
  ...
  if (tmp_curve_mapping && viewport->view_settings.curve_mapping == NULL) {
    KKE_color_managed_view_settings_free(&viewport->view_settings);
    viewport->view_settings.curve_mapping = KKE_curvemapping_copy(tmp_curve_mapping);
  }

  KKE_color_managed_display_settings_copy(&viewport->display_settings, display_settings);
...
static GPUVertFormat *gpu_viewport_batch_format(void)
{
  if (g_viewport.format.attr_len == 0) {
    ...
    g_viewport.attr_id.pos =
      GPU_vertformat_attr_add(format, "pos", GPU_COMP_F32, 2, GPU_FETCH_FLOAT);
    g_viewport.attr_id.tex_coord =
                                        ...
                                        const rctf *rect_uv)
{
  ...
  const bool parameters_changed =
    (!KLI_rctf_compare(&viewport->batch.last_used_parameters.rect_pos, rect_pos, compare_limit) ||
     !KLI_rctf_compare(&viewport->batch.last_used_parameters.rect_uv, rect_uv, compare_limit));
                                    ...
                                    bool do_overlay_merge)
{
  ...
  KLI_rcti_sanitize(&sanitized_rect);

  KLI_assert(w == KLI_rcti_size_x(&sanitized_rect) + 1);
  KLI_assert(h == KLI_rcti_size_y(&sanitized_rect) + 1);
   ...
   * max values swapped). */
  if (KLI_rcti_size_x(rect) < 0) {
    SWAP(float, uv_rect.xmin, uv_rect.xmax);
  }
  if (KLI_rcti_size_y(rect) < 0) {
