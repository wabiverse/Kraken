...
namespace kraken::gpu
{
  ...
  VertBuf::~VertBuf()
  {
    ...
    KLI_assert(flag == GPU_VERTBUF_INVALID);
  }
  ...
  {
    KLI_assert(format.packed);
    /* Catch any unnecessary usage. */
    KLI_assert(vertex_alloc != vert_len || data == nullptr);
  ...
  {
    /* Catch any unnecessary usage. */
    KLI_assert(vertex_alloc != vert_len);
    vertex_len = vertex_alloc = vert_len;
...
void GPU_vertbuf_data_len_set(GPUVertBuf *verts_, uint v_len)
{
  ...
  KLI_assert(verts->data != nullptr); /* Only for dynamic data. */
  KLI_assert(v_len <= verts->vertex_alloc);
...
{
  VertBuf *verts = unwrap(verts_);
  KLI_assert(verts->get_usage_type() != GPU_USAGE_DEVICE_ONLY);
  const GPUVertFormat *format = &verts->format;
  const GPUVertAttr *a = &format->attrs[a_idx];
  KLI_assert(v_idx < verts->vertex_alloc);
  KLI_assert(a_idx < format->attr_len);
  KLI_assert(verts->data != nullptr);
...
void GPU_vertbuf_attr_fill_stride(GPUVertBuf *verts_, uint a_idx, uint stride, const void *data)
{
  ...
  if (format->attr_len == 1 && stride == format->stride) {
    ...
    for (uint v = 0; v < vertex_len; v++) {
      memcpy(verts->data + a->offset + v * format->stride,
