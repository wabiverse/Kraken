...
#define DEBUG_DEPENDENCIES 0
                         ...
                         GPULogParser *parser)
  {
    ...
    char *sources_combined = KLI_string_join_arrayN((const char **)sources.data(), sources.size());
    ...
    while ((line_end = strchr(log_line, '\n'))) {
      ...
      if ((log_item.cursor.row != -1) && (log_item.cursor.row != previous_location.row ||
        ...
        while ((src_line_end = strchr(src_line, '\n'))) {
          ...
          if (src_line_index >= log_item.cursor.row - DEBUG_CONTEXT_LINES) {
            KLI_dynstr_appendf(dynstr, "%5d | ", src_line_index);
            KLI_dynstr_nappend(dynstr, src_line, (src_line_end + 1) - src_line);
          }
        ...
        if (found_line_id) {
          if (log_item.cursor.row != previous_location.row) {
            ...
            KLI_dynstr_appendf(dynstr, line_prefix);
          }
          KLI_dynstr_nappend(dynstr, src_line, (src_line_end + 1) - src_line);
          /* Print char offset. */
          KLI_dynstr_appendf(dynstr, line_prefix);
      ...
      if (log_item.severity == Severity::Error) {
        KLI_dynstr_appendf(dynstr, "%s%s%s: ", err_col, "Error", info_col);
      } else if (log_item.severity == Severity::Error) {
        KLI_dynstr_appendf(dynstr, "%s%s%s: ", warn_col, "Warning", info_col);
      }
      /* Print the error itself. */
      KLI_dynstr_append(dynstr, info_col);
      KLI_dynstr_nappend(dynstr, log_line, (line_end + 1) - log_line);
      KLI_dynstr_append(dynstr, reset_col);
      /* Continue to next line. */
      log_line = line_end + 1;
    ...
    if (((LOG.type->flag & CLG_FLAG_USE) && (LOG.type->level >= 0)) ||
      ...
      const char *_str = KLI_dynstr_get_cstring(dynstr);
