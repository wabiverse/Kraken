...
static GPUVertFormat *preset_3d_format(void)
{
  if (g_presets_3d.format.attr_len == 0) {
    ...
    g_presets_3d.attr_id.pos =
      GPU_vertformat_attr_add(format, "pos", GPU_COMP_F32, 3, GPU_FETCH_FLOAT);
    g_presets_3d.attr_id.nor =
...
static GPUVertFormat *preset_2d_format(void)
{
  if (g_presets_2d.format.attr_len == 0) {
    ...
    g_presets_2d.attr_id.pos =
      GPU_vertformat_attr_add(format, "pos", GPU_COMP_F32, 2, GPU_FETCH_FLOAT);
    g_presets_2d.attr_id.col =
...
static GPUBatch *gpu_batch_sphere(int lat_res, int lon_res)
{
  ...
  KLI_assert(vbo_len == GPU_vertbuf_raw_used(&pos_step));
  KLI_assert(vbo_len == GPU_vertbuf_raw_used(&nor_step));
                                             ...
                                             const float width)
{
  ...
  if (!g_presets_2d.batch.panel_drag_widget) {
    g_presets_2d.batch.panel_drag_widget = gpu_batch_preset_panel_drag_widget(pixelsize,
                                                                              col_high,
                                                                              col_dark,
                                                                              width);
...
GPUBatch *GPU_batch_preset_quad(void)
{
  if (!g_presets_2d.batch.quad) {
    ...
    g_presets_2d.batch.quad = GPU_batch_create_ex(GPU_PRIM_TRI_STRIP,
                                                  vbo,
                                                  NULL,
                                                  GPU_BATCH_OWNS_VBO);
...
/* -------------------------------------------------------------------- */
/** @name Preset Registration Management
 * @{ */

void gpu_batch_presets_init(void)
{
  KLI_mutex_init(&g_presets_3d.mutex);
...
{
  KLI_mutex_lock(&g_presets_3d.mutex);
  KLI_addtail(&presets_list, KLI_genericNodeN(preset_batch));
  KLI_mutex_unlock(&g_presets_3d.mutex);
}
...
{
  LinkData *link;
  while ((link = KLI_pophead(&presets_list))) {
    GPUBatch *preset = link->data;
    GPU_batch_discard(preset);
    MEM_freeN(link);
  }

  KLI_mutex_end(&g_presets_3d.mutex);
}
