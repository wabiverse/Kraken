...
namespace kraken::gpu
{
  ...
  class ShaderInterface
  {
   ...
   public:
    ...
    {
      KLI_assert(builtin >= 0 && builtin < GPU_NUM_UNIFORMS);
    ...
    {
      KLI_assert(builtin >= 0 && builtin < GPU_NUM_UNIFORM_BLOCKS);
    ...
    {
      KLI_assert(builtin >= 0 && builtin < GPU_NUM_STORAGE_BUFFERS);
                                                  ...
                                                  uint32_t name_len) const
  {
    ...
    input->name_hash = KLI_hash_string(name);
  ...
  inline const ShaderInput *ShaderInterface::input_lookup(const ShaderInput *const inputs,
                                                          ...
                                                          const char *name) const
  {
    const uint name_hash = KLI_hash_string(name);
    ...
    for (int i = inputs_len - 1; i >= 0; i--) {
      if (inputs[i].name_hash == name_hash) {
         ...
         * as a real uniform. */
        KLI_assert(STREQ(name, name_buffer_ + inputs[i].name_offset));
        return inputs + i;
      }
    }
    return nullptr; /* not found */
