...
static const char *gpu_uniform_set_function_from_type(eNodeSocketDatatype type)
{
  switch (type) {
    ...
    case SOCK_RGBA:
      return "set_rgba";
    default:
      KLI_assert_msg(0, "No gpu function for non-supported eNodeSocketDatatype");
      return NULL;
  }
}
                                           ...
                                           const eNodeSocketInOut in_out)
{
  ...
  if (in_out == SOCK_IN) {
    socket = KLI_findlink(&node->inputs, index);
  } else {
    socket = KLI_findlink(&node->outputs, index);
  }

  KLI_assert(socket != NULL);
  KLI_assert(socket->in_out == in_out);
...
static void gpu_node_output(GPUNode *node, const eGPUType type, GPUNodeLink **link)
{
  ...
  KLI_addtail(&node->outputs, output);
}
...
{
  return KLI_rhash_new(uniform_attr_list_hash, uniform_attr_list_cmp, info);
}
...
void GPU_uniform_attr_list_copy(GPUUniformAttrList *dest, const GPUUniformAttrList *src)
{
  ...
  KLI_duplicatelist(&dest->list, &src->list);
}
...
void GPU_uniform_attr_list_free(GPUUniformAttrList *set)
{
  set->count = 0;
  set->hash_code = 0;
  KLI_freelistN(&set->list);
}

void gpu_node_graph_finalize_uniform_attrs(GPUNodeGraph *graph)
{
  GPUUniformAttrList *attrs = &graph->uniform_attrs;
  KLI_assert(attrs->count == KLI_listbase_count(&attrs->list));

  /* Sort the attributes by name to ensure a stable order. */
  KLI_listbase_sort(&attrs->list, uniform_attr_sort_cmp);
  ...
  {
    attr->id = next_id++;
    attrs->hash_code ^= KLI_rhashutil_uinthash(attr->hash_code + (1 << (attr->id + 1)));
  }
}
...
static char attr_prefix_get(GPUMaterialAttribute *attr)
{
  ...
  switch (attr->type) {
    ...
    case CD_HAIRLENGTH:
      return 'l';
    default:
      KLI_assert_msg(0, "GPUVertAttr Prefix type not found : This should not happen!");
      return '\0';
  }
}
                                                          ...
                                                          const bool is_default_color)
{
  ...
  if (attr == NULL) {
    ...
    KLI_addtail(&graph->attributes, attr);
  }
                                                            ...
                                                            bool use_dupli)
{
  ...
  if (attr == NULL && attrs->count < GPU_MAX_UNIFORM_ATTR) {
    ...
    attr->hash_code = KLI_rhashutil_strhash_p(attr->name) << 1 | (attr->use_dupli ? 0 : 1);
    attr->id = -1;
    KLI_addtail(&attrs->list, attr);
    attrs->count++;
  }
...
static GPULayerAttr *gpu_node_graph_add_layer_attribute(GPUNodeGraph *graph, const char *name)
{
  ...
  if (attr == NULL) {
    attr = MEM_callocN(sizeof(*attr), __func__);
    STRNCPY(attr->name, name);
    attr->hash_code = KLI_rhashutil_strhash_p(attr->name);
    KLI_addtail(attrs, attr);
  }
                                                      ...
                                                      eGPUSamplerState sampler_state)
{
  ...
  if (tex == NULL) {
    ...
    tex->sky = sky;
    tex->sampler_state = sampler_state;
    KLI_snprintf(tex->sampler_name, sizeof(tex->sampler_name), "samp%d", num_textures);
    if (ELEM(link_type, GPU_NODE_LINK_IMAGE_TILED, GPU_NODE_LINK_IMAGE_TILED_MAPPING)) {
      KLI_snprintf(tex->tiled_mapping_name,
                   sizeof(tex->tiled_mapping_name),
                   "tsamp%d",
                   num_textures);
    }
    KLI_addtail(&graph->textures, tex);
  }
                       ...
                       eGPUSamplerState sampler_state)
{
  ...
  link->texture =
    gpu_node_graph_add_texture(graph, ima, iuser, NULL, NULL, link->link_type, sampler_state);
  return link;
}
                           ...
                           eGPUSamplerState sampler_state)
{
  ...
  link->texture =
    gpu_node_graph_add_texture(graph, NULL, NULL, NULL, sky, link->link_type, sampler_state);
  return link;
}
...
GPUNodeLink *GPU_image_tiled_mapping(GPUMaterial *mat, Image *ima, ImageUser *iuser)
{
  ...
  link->texture =
    gpu_node_graph_add_texture(graph, ima, iuser, NULL, NULL, link->link_type, GPU_SAMPLER_MAX);
  return link;
}
...
void gpu_node_graph_free_nodes(GPUNodeGraph *graph)
{
  ...
  while ((node = KLI_pophead(&graph->nodes))) {
    gpu_node_free(node);
  }
...
{
  KLI_freelistN(&graph->outlink_aovs);
  KLI_freelistN(&graph->material_functions);
  KLI_freelistN(&graph->outlink_compositor);
  gpu_node_graph_free_nodes(graph);

  KLI_freelistN(&graph->textures);
  KLI_freelistN(&graph->attributes);
  GPU_uniform_attr_list_free(&graph->uniform_attrs);
  KLI_freelistN(&graph->layer_attrs);

  if (graph->used_libraries) {
    KLI_rset_free(graph->used_libraries, NULL);
...
void gpu_node_graph_prune_unused(GPUNodeGraph *graph)
{
  ...
  for (GPUMaterialAttribute *attr = graph->attributes.first, *next = NULL; attr; attr = next) {
    next = attr->next;
    if (attr->users == 0) {
      KLI_freelinkN(&graph->attributes, attr);
    }
  }
  ...
  for (GPUMaterialTexture *tex = graph->textures.first, *next = NULL; tex; tex = next) {
    next = tex->next;
    if (tex->users == 0) {
      KLI_freelinkN(&graph->textures, tex);
    }
  }
  ...
  {
    if (attr->users == 0) {
      KLI_freelinkN(&uattrs->list, attr);
      uattrs->count--;
    }
  }
  ...
  {
    if (attr->users == 0) {
      KLI_freelinkN(&graph->layer_attrs, attr);
    }
  }
}
