...
#  define GPU_SHADER_CREATE_INFO(_info) \
  ...
  enum class Type
  {
     ...
     * in the GL back-end, as implicit type conversions from packed normal attribute data to vec3
     * is supported. UCHAR/CHAR types are natively supported in Metal and can be used to avoid
  ...
  static inline Type to_type(const eGPUType type)
  {
    switch (type) {
      ...
      case GPU_MAT4:
        return Type::MAT4;
      default:
        KLI_assert_msg(0, "Error: Cannot convert eGPUType to shader::Type.");
        return Type::FLOAT;
    }
  }
...
#define TEST_VECTOR_EQUAL(a, b, _vector) \
  ...
  for (auto i : _vector.index_range()) { \
   ...
   public:
     ...
     * appended in the geometry shader IF AND ONLY IF the vertex_out interface instance name
     * matches the geometry_out interface instance name.
     */
