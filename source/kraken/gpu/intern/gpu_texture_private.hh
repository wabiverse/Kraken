...
#undef DEBUG_NAME_LEN
    ...
    inline size_t to_bytesize(eGPUTextureFormat format)
    {
      switch (format) {
        ...
        default:
          KLI_assert_msg(0, "Texture format incorrect or unsupported");
          return 0;
      }
    }
    ...
    inline size_t to_block_size(eGPUTextureFormat data_type)
    {
      switch (data_type) {
        ...
        case GPU_RGBA8_DXT5:
          return 16;
        default:
          KLI_assert_msg(0, "Texture format is not a compressed format");
          return 0;
      }
    }
    ...
    inline size_t to_bytesize(eGPUDataFormat data_format)
    {
      switch (data_format) {
        ...
        case GPU_DATA_2_10_10_10_REV:
          return 4;
        default:
          KLI_assert_msg(0, "Data format incorrect or unsupported");
          return 0;
      }
    }
    ...
    /* Ensure valid upload formats. With format conversion support, certain types can be extended
     * to allow upload from differing source formats. If these cases are added, amend accordingly.
     */
    inline bool validate_data_format_mtl(eGPUTextureFormat tex_format, eGPUDataFormat data_format)
    {
      switch (tex_format) {
        ...
        case GPU_RGB16F:
          return data_format == GPU_DATA_FLOAT;
        default:
          KLI_assert_msg(0, "Unrecognized data format");
          return data_format == GPU_DATA_FLOAT;
      }
    }
    ...
    static inline eGPUTextureFormat to_texture_format(const GPUVertFormat *format)
    {
      if (format->attr_len > 1 || format->attr_len == 0) {
        KLI_assert_msg(0, "Incorrect vertex format for buffer texture");
      ...
      KLI_assert_msg(0, "Unsupported vertex format for buffer texture");
      return GPU_DEPTH_COMPONENT24;
    }
