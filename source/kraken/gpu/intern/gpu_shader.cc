...
{
  KLI_strncpy(this->name, sh_name, sizeof(this->name));
}
...
static void standard_defines(Vector<const char *> &sources)
{
  ...
  switch (backend) {
    ...
    case GPU_BACKEND_METAL:
      ...
      break;
    default:
      KLI_assert(false && "Invalid GPU Backend Type");
      break;
  }
                                ...
                                const char *shname)
{
  ...
  KLI_assert(((fragcode != nullptr) && (vertcode != nullptr) && (computecode == nullptr)) ||
  ...
  if (tf_names != nullptr && tf_count > 0) {
    KLI_assert(tf_type != GPU_SHADER_TFB_NONE);
...
bool GPU_shader_create_info_check_error(const GPUShaderCreateInfo *_info, char r_error[128])
{
  ...
  KLI_strncpy(r_error, error.c_str(), 128);
  return false;
}
                                         ...
                                         const char *name)
{
  ...
  if (libcode == nullptr) {
    ...
    libcode = libcodecat = KLI_strdupcat(libcode, datatoc_gpu_shader_colorspace_lib_glsl);
  }
...
static const char *string_join_array_maybe_alloc(const char **str_arr, bool *r_is_alloc)
{
  ...
  *r_is_alloc = is_alloc;
  if (is_alloc) {
    return KLI_string_join_arrayN(str_arr, i);
  }
  ...
  int line)
{
  ...
  char name[64];
  KLI_snprintf(name, sizeof(name), "%s_%d", func, line);

  GPUShader *sh = GPU_shader_create(str_dst[0].str,
                                    str_dst[1].str,
                                    str_dst[2].str,
                                    nullptr,
                                    str_dst[3].str,
                                   ...
                                   int *r_type)
{
  ...
  KLI_strncpy(r_name, interface->input_name_get(attr), 256);
