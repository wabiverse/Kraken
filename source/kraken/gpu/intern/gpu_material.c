                                                ...
                                                float *row)
{
  ...
  KLI_assert(width == GPU_SKY_WIDTH);
  KLI_assert(height == GPU_SKY_HEIGHT);
...
const ListBase *GPU_material_layer_attributes(const GPUMaterial *material)
{
  ...
  return !KLI_listbase_is_empty(attrs) ? attrs : NULL;
}
...
KLI_STATIC_ASSERT_ALIGN(GPUSssKernelData, 16)
                                                   ...
                                                   GPUTexture **tex_profile)
{
  ...
  if (material->sss_dirty || (material->sss_samples != sample_len)) {
    ...
    material->sss_tex_profile =
      GPU_texture_create_1d("sss_tex_profile", 64, 1, GPU_RGBA16F, translucence_profile);
...
void GPU_material_add_output_link_aov(GPUMaterial *material, GPUNodeLink *link, int hash)
{
  ...
  aov_link->hash = hash;
  KLI_addtail(&material->graph.outlink_aovs, aov_link);
}
...
void GPU_material_add_output_link_composite(GPUMaterial *material, GPUNodeLink *link)
{
  ...
  KLI_addtail(&material->graph.outlink_compositor, compositor_link);
}
                                      ...
                                      GPUNodeLink **link)
{
  ...
  KLI_addtail(&material->graph.material_functions, func_link);
                                        ...
                                        void *thunk)
{
  ...
  mat->graph.used_libraries = KLI_rset_new(KLI_rhashutil_ptrhash,
                                           KLI_rhashutil_ptrcmp,
  ...
  // kNodeTree *localtree = ntreeLocalize(ntree);
  ...
  // KLI_assert(!localtree->id.py_instance); /* Or call #KKE_libblock_free_data_py. */
  // MEM_freeN(localtree);
...
void GPU_material_compile(GPUMaterial *mat)
{
  ...
  KLI_assert(ELEM(mat->status, GPU_MAT_QUEUED, GPU_MAT_CREATED));
  KLI_assert(mat->pass);
...
void GPU_materials_free(Main *kmain)
{
   ...
   * @TODO: Worlds.
   */
  // LISTBASE_FOREACH(World *, wo, &kmain->worlds)
  // {
                                         ...
                                         void *thunk)
{
  ...
  material->graph.used_libraries = KLI_rset_new(KLI_rhashutil_ptrhash,
                                                KLI_rhashutil_ptrcmp,
                                                "GPUNodeGraph.used_libraries");
  material->refcount = 1;
