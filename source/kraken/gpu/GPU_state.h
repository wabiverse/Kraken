...
/**
 * @note By convention, this is set as needed and not reset back to 1.0.
...
/**
 * @note Already pre-multiplied by `U.pixelsize`.
 */
...
void GPU_kgl_start(void);

/**
 * Just turn off the `kgl` safeguard system. Can be called even without #GPU_kgl_start.
 */
void GPU_kgl_end(void);
bool GPU_kgl_get(void);
