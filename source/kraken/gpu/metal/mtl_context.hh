...
namespace kraken::gpu
{
  ...
  class MTLRenderPassState
  {
   ...
   public:
    ...
    MTL::RenderPipelineState *bound_pso = nil;
    MTL::DepthStencilState *bound_ds_state = nil;
    uint last_used_stencil_ref_value = 0;
    MTL::ScissorRect last_scissor_rect;
    ...
    void bind_vertex_texture(MTL::Texture *tex, uint slot);
    void bind_fragment_texture(MTL::Texture *tex, uint slot);
    ...
    void bind_vertex_buffer(MTL::Buffer *buffer, uint buffer_offset, uint index);
    void bind_fragment_buffer(MTL::Buffer *buffer, uint buffer_offset, uint index);
  ...
  struct MTLContextDepthStencilState
  {
    ...
    MTL::CompareFunction depth_function;
    float depth_bias;
    ...
    uint stencil_ref;
    MTL::CompareFunction stencil_func;

    MTL::StencilOperation stencil_op_front_stencil_fail;
    MTL::StencilOperation stencil_op_front_depth_fail;
    MTL::StencilOperation stencil_op_front_depthstencil_pass;

    MTL::StencilOperation stencil_op_back_stencil_fail;
    MTL::StencilOperation stencil_op_back_depth_fail;
    MTL::StencilOperation stencil_op_back_depthstencil_pass;
    ...
    std::size_t hash() const
    {
      ...
      std::size_t stencilop_bitmask = ((std::size_t)this->stencil_op_front_stencil_fail) |
                                      ...
                                      ((std::size_t)this->stencil_op_front_depthstencil_pass
                                       << 6) |
  ...
  struct MTLContextTextureUtils
  {
    ...
    kraken::Map<DepthTextureUpdateRoutineSpecialisation, GPUShader *> depth_2d_update_shaders;
    ...
    template<typename T> void free_cached_pso_map(kraken::Map<T, MTL::ComputePipelineState *> &map)
    {
      for (typename kraken::Map<T, MTL::ComputePipelineState *>::MutableItem item : map.items()) {
        item.value->release();
        // item.value->release();
      }
      map.clear();
  ...
  struct MTLSamplerArray
  {
    ...
    bool operator==(const MTLSamplerArray &other) const
    {
      ...
      return (memcmp(this->mtl_sampler_flags,
                     ...
                     sizeof(gpu::MTLSamplerState) * this->num_samplers) == 0);
    }
  ...
  struct MTLContextGlobalShaderPipelineState
  {
     ...
     * modify these parameters. Certain values, tagged [PSO], are parameters which are required
     * to be passed into PSO creation, rather than dynamic state functions on the
     * RenderCommandEncoder.
     */

    /* Blending State. */
    MTL::ColorWriteMask color_write_mask;     /* [PSO] */
    bool blending_enabled;                    /* [PSO] */
    MTL::BlendOperation alpha_blend_op;       /* [PSO] */
    MTL::BlendOperation rgb_blend_op;         /* [PSO] */
    MTL::BlendFactor dest_alpha_blend_factor; /* [PSO] */
    MTL::BlendFactor dest_rgb_blend_factor;   /* [PSO] */
    MTL::BlendFactor src_alpha_blend_factor;  /* [PSO] */
    MTL::BlendFactor src_rgb_blend_factor;    /* [PSO] */
  ...
  class MTLCommandBufferManager
  {
   ...
   private:
    ...
    bool m_supports_render = false;

    /* CommandBuffer tracking. */
    MTL::CommandBuffer *m_active_command_buffer = nil;
    MTL::CommandBuffer *m_last_submitted_command_buffer = nil;
    ...
    MTL::RenderCommandEncoder *m_active_render_command_encoder = nil;
    MTL::BlitCommandEncoder *m_active_blit_command_encoder = nil;
    MTL::ComputeCommandEncoder *m_active_compute_command_encoder = nil;

    /* State associated with active RenderCommandEncoder. */
    MTLRenderPassState m_render_pass_state;
    MTLFrameBuffer *m_active_frame_buffer = nullptr;
    MTL::RenderPassDescriptor *m_active_pass_descriptor = nullptr;
   ...
   public:
    ...
    MTLCommandBufferManager(MTLContext &context)
      : m_context(context),
        m_render_pass_state(context, *this){};
    ...
    MTL::RenderCommandEncoder *get_active_render_command_encoder();
    MTL::BlitCommandEncoder *get_active_blit_command_encoder();
    MTL::ComputeCommandEncoder *get_active_compute_command_encoder();
    ...
    MTLRenderPassState &get_render_pass_state()
    {
      ...
      KLI_assert(this->is_inside_render_pass());
      return m_render_pass_state;
    }
    ...
    MTL::RenderCommandEncoder *ensure_begin_render_command_encoder(MTLFrameBuffer *ctx_framebuffer,
                                                                   bool force_begin,
                                                                   bool *new_pass);
    MTL::BlitCommandEncoder *ensure_begin_blit_encoder();
    MTL::ComputeCommandEncoder *ensure_begin_compute_encoder();
   ...
   private:
    ...
    MTL::CommandBuffer *ensure_begin();
  ...
  class MTLContext : public Context
  {
   ...
   public:
    ...
    MTLContext(void *anchor_window, void *anchor_context);
    ~MTLContext();
    ...
    MTL::RenderCommandEncoder *ensure_begin_render_pass();
    ...
    MTL::SamplerState *get_sampler_from_state(MTLSamplerState state);
    MTL::SamplerState *generate_sampler_from_state(MTLSamplerState state);
    MTL::SamplerState *get_default_sampler_state();
    ...
    bool ensure_render_pipeline_state(MTL::PrimitiveType prim_type);
    ...
    void ensure_texture_bindings(MTL::RenderCommandEncoder *rec,
                                 MTLShaderInterface *shader_interface,
                                 const MTLRenderPipelineStateInstance *pipeline_state_instance);
    void ensure_depth_stencil_state(MTL::PrimitiveType prim_type);

    MTL::Buffer *get_null_buffer();
    MTL::Buffer *get_null_attribute_buffer();
   ...
   private:
    ...
    /* Parent Context. */
    AnchorContextMetal *m_anchor_ctx;

    /* Render Passes and Frame-buffers. */
    MTL::Texture *m_default_fbo_mtltexture = nil;
    gpu::MTLTexture *m_default_fbo_gputexture = nullptr;

    /* Depth-stencil state cache. */
    kraken::Map<MTLContextDepthStencilState, MTL::DepthStencilState *> depth_stencil_state_cache;

    /* Compute and specialization caches. */
    MTLContextTextureUtils m_texture_utils;

    /* Texture Samplers. */
    /* Cache of generated #MTLSamplerState objects based on permutations of `eGPUSamplerState`.
     */
    MTL::SamplerState *m_sampler_state_cache[GPU_SAMPLER_MAX];
    MTL::SamplerState *m_default_sampler_state = nil;
    ...
    gpu::MTLBuffer *m_visibility_buffer = nullptr;
    bool m_visibility_is_dirty = false;

    /* Null buffers for empty/uninitialized bindings.
     * Null attribute buffer follows default attribute format of OpenGL Backend. */
    MTL::Buffer *m_null_buffer;           /* All zero's. */
    MTL::Buffer *m_null_attribute_buffer; /* Value float4(0.0,0.0,0.0,1.0). */

    /** Dummy Resources */
    /* Maximum of 32 texture types. Though most combinations invalid. */
    gpu::MTLTexture *m_dummy_textures[GPU_TEXTURE_BUFFER] = {nullptr};
    GPUVertFormat m_dummy_vertformat;
    GPUVertBuf *m_dummy_verts = nullptr;
  ...
  /* Anchor Context callback and present. */
