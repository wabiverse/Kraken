...
#define GPU_VAO_STATIC_LEN 64
  ...
  class MTLBatch : public Batch
  {
   ...
   public:
     ...
     * Otherwise, nil. */
    MTL::RenderCommandEncoder *bind(uint v_first, uint v_count, uint i_first, uint i_count);
    void unbind();
   ...
   private:
    ...
    MTL::Buffer *get_emulated_toplogy_buffer(GPUPrimType &in_out_prim_type, uint32_t &v_count);

    void prepare_vertex_descriptor_and_bindings(MTLVertBuf **buffers,
                                                int &num_buffers,
                                                int v_first,
                                                int v_count,
                                                int i_first,
