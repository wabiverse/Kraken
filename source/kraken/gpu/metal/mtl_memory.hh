...
namespace kraken::gpu
{
  ...
  class MTLBuffer
  {
   ...
   public:
    ...
    MTLBuffer(MTL::Device *device,
              uint64_t size,
              MTL::ResourceOptions options,
              uint alignment = 1);
    MTLBuffer(MTL::Buffer *external_buffer);
    ~MTLBuffer();

    /* Fetch information about backing MTLBuffer. */
    MTL::Buffer *get_metal_buffer() const;
    ...
    /* Debug. */
    void set_label(NS::String *str);

    /* Read properties. */
    MTL::ResourceOptions get_resource_options();
   ...
   * than individual buffer resource tracking reduces performance overhead.
   *
   ...
   * previously allocated MTLBuffer resources to be re-used. This allows us to save on the
   * expensive cost of memory allocation.
   */
  ...
  class MTLBufferPool
  {
   ...
   private:
     ...
     * the desired allocation size, we check it against `mtl_buffer_size_threshold_factor_`, which
     * defines what % larger than the original allocation the buffer can be.
     ...
     * - A size-ordered list (MultiSet) of allocated buffers is kept per MTL::ResourceOptions
   ...
   public:
    ...
    gpu::MTLBuffer *allocate_with_data(uint64_t size,
                                       bool cpu_visible,
   ...
   private:
    ...
    void ensure_buffer_pool(MTL::ResourceOptions options);
    void insert_buffer_into_pool(MTL::ResourceOptions options, gpu::MTLBuffer *buffer);
    void free();
  };
  ...
  class MTLScratchBufferManager
  {
   ...
   public:
    ...
    /* Maximum size of single scratch buffer allocation. When re-sizing, this is the maximum size
   ...
   public:
    ...
    MTLScratchBufferManager(MTLContext &context) : m_context(context){};
    ~MTLScratchBufferManager();
