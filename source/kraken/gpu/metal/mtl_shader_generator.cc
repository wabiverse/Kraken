...
namespace kraken::gpu
{
  ...
  static eMTLDataType to_mtl_type(Type type)
  {
    switch (type) {
      ...
      default: {
        KLI_assert_msg(false, "Unexpected data type");
      }
    }
    return MTL_DATATYPE_FLOAT;
  }
  ...
  static void extract_global_scope_constants(std::string &str, std::stringstream &global_scope_out)
  {
    ...
    for (char *c = current_str_begin; c < current_str_end - 6; c++) {
      ...
      if (*c == '}' || *c == ']' || *c == ')') {
        nested_bracket_depth--;
        KLI_assert(nested_bracket_depth >= 0);
        continue;
      }
  ...
  static bool extract_ssbo_pragma_info(const MTLShader *shader,
                                       ...
                                       MTL::PrimitiveType &out_prim_tye,
                                       uint32_t &out_num_output_verts)
  {
    ...
    if (in_vertex_src.find("#pragma USE_SSBO_VERTEX_FETCH") != std::string::npos) {
      uses_ssbo_fetch = std::regex_search(in_vertex_src,
                                          vertex_shader_ssbo_flags,
                                          use_ssbo_fetch_mode_find);
    }
    if (uses_ssbo_fetch) {
      ...
      if (str_output_primitive_type == "TriangleList") {
        out_prim_tye = MTL::PrimitiveTypeTriangle;
      } else if (str_output_primitive_type == "LineList") {
        out_prim_tye = MTL::PrimitiveTypeLine;
      } else {
      ...
      KLI_assert(out_num_output_verts > 0);
      return true;
    }
  ...
  {
    KLI_assert_msg(false, "Geometry shading unsupported by Metal");
    return "";
  }
  ...
  std::string MTLShader::compute_layout_declare(const ShaderCreateInfo &info) const
  {
    ...
    KLI_assert_msg(false, "Compute shaders unsupported by Metal");
    return "";
  }
  ...
  bool MTLShader::generate_msl_from_glsl(const shader::ShaderCreateInfo *info)
  {
    ...
    KLI_assert(m_shd_builder->m_glsl_vertex_source.size() > 0);
    if (!msl_iface.uses_transform_feedback) {
      KLI_assert(m_shd_builder->m_glsl_fragment_source.size() > 0);
    }
    ...
    if (m_transform_feedback_type != GPU_SHADER_TFB_NONE) {
      ...
      KLI_assert(m_tf_output_name_list.size() > 0);
    ...
    m_shd_builder->m_glsl_vertex_source = msl_defines_string + m_shd_builder->m_glsl_vertex_source;
    if (!msl_iface.uses_transform_feedback) {
      m_shd_builder->m_glsl_fragment_source = msl_defines_string +
                                              m_shd_builder->m_glsl_fragment_source;
    }
    ...
    MTL::PrimitiveType vertex_fetch_ssbo_output_prim_type = MTL::PrimitiveTypeTriangle;
    uint32_t vertex_fetch_ssbo_num_output_verts = 0;
    msl_iface.uses_ssbo_vertex_fetch_mode = extract_ssbo_pragma_info(
      this,
      msl_iface,
      m_shd_builder->m_glsl_vertex_source,
    ...
    m_shd_builder->m_glsl_vertex_source = std::regex_replace(m_shd_builder->m_glsl_vertex_source,
                                                             remove_excess_newlines,
                                                             "\n");
    m_shd_builder->m_glsl_vertex_source = std::regex_replace(m_shd_builder->m_glsl_vertex_source,
                                                             replace_mat3,
                                                             "MAT3(");
    replace_array_initializers_func(m_shd_builder->m_glsl_vertex_source);

    if (!msl_iface.uses_transform_feedback) {
      m_shd_builder->m_glsl_fragment_source =
        std::regex_replace(m_shd_builder->m_glsl_fragment_source, remove_excess_newlines, "\n");
      m_shd_builder->m_glsl_fragment_source =
        std::regex_replace(m_shd_builder->m_glsl_fragment_source, replace_mat3, "MAT3(");
      replace_array_initializers_func(m_shd_builder->m_glsl_fragment_source);
    }
    ...
    msl_iface.uses_gl_VertexID = bool(info->builtins_ & BuiltinBits::VERTEX_ID) ||
                                 m_shd_builder->m_glsl_vertex_source.find("gl_VertexID") !=
                                   std::string::npos;
    msl_iface.uses_gl_InstanceID = bool(info->builtins_ & BuiltinBits::INSTANCE_ID) ||
                                   m_shd_builder->m_glsl_vertex_source.find("gl_InstanceID") !=
                                     std::string::npos ||
                                   m_shd_builder->m_glsl_vertex_source.find("gpu_InstanceIndex") !=
                                     std::string::npos ||
    ...
    msl_iface.uses_gl_BaseInstanceARB =
      msl_iface.uses_gl_InstanceID ||
      m_shd_builder->m_glsl_vertex_source.find("gl_BaseInstanceARB") != std::string::npos ||
      m_shd_builder->m_glsl_vertex_source.find("gpu_BaseInstance") != std::string::npos;
    msl_iface.uses_gl_Position = m_shd_builder->m_glsl_vertex_source.find("gl_Position") !=
                                 std::string::npos;
    msl_iface.uses_gl_PointSize = m_shd_builder->m_glsl_vertex_source.find("gl_PointSize") !=
                                  std::string::npos;
    msl_iface.m_uses_mtl_array_index = m_shd_builder->m_glsl_vertex_source.find(
    ...
    if (!msl_iface.uses_transform_feedback) {
      ...
      msl_iface.uses_gl_PointCoord = bool(info->builtins_ & BuiltinBits::POINT_COORD) ||
                                     m_shd_builder->m_glsl_fragment_source.find("gl_PointCoord") !=
                                       std::string::npos;
      ...
      msl_iface.uses_gl_FrontFacing = bool(info->builtins_ & BuiltinBits::FRONT_FACING) ||
                                      m_shd_builder->m_glsl_fragment_source.find(
                                        "gl_FrontFacing") != std::string::npos;

      /* NOTE(Metal): If FragColor is not used, then we treat the first fragment output attachment
       * as the primary output. */
      msl_iface.uses_gl_FragColor = m_shd_builder->m_glsl_fragment_source.find("gl_FragColor") !=
                                    std::string::npos;
      ...
      msl_iface.uses_gl_FragDepth = m_shd_builder->m_glsl_fragment_source.find("gl_FragDepth") !=
                                    std::string::npos;
    ...
    std::string clip_search_str = m_shd_builder->m_glsl_vertex_source;
    ...
    m_shd_builder->m_glsl_vertex_source = std::regex_replace(m_shd_builder->m_glsl_vertex_source,
                                                             gl_clipdistance_find,
                                                             "gl_ClipDistance_$1");

    /* Replace 'out' attribute on function parameters with pass-by-reference. */
    replace_outvars(m_shd_builder->m_glsl_vertex_source);
    if (!msl_iface.uses_transform_feedback) {
      replace_outvars(m_shd_builder->m_glsl_fragment_source);
    }
    ...
    remove_multiline_comments_func(m_shd_builder->m_glsl_vertex_source);
    remove_singleline_comments_func(m_shd_builder->m_glsl_vertex_source);
    extract_global_scope_constants(m_shd_builder->m_glsl_vertex_source, ss_vertex);
#endif
    ...
    for (const StageInterfaceInfo *iface : info->vertex_out_interfaces_) {
      ...
      for (const StageInterfaceInfo::InOut &inout : iface->inouts) {
        ...
        int array_len = (is_array) ?
                          std::stoi(
                            std::regex_replace(arraystart, remove_non_numeric_characters, "")) :
                          0;
    ...
    if (msl_iface.m_uses_mtl_array_index) {
    ...
    ss_vertex << m_shd_builder->m_glsl_vertex_source << std::endl;
    ...
    if (msl_iface.uses_transform_feedback) {
      ss_vertex << msl_iface.generate_msl_vertex_transform_feedback_out_struct(
        ShaderStage::VERTEX);
    }
    ...
    if (!msl_iface.uses_transform_feedback) {
       ...
       * These cause excessive register pressure due to global arrays being instantiated
       * per-thread. NOTE: This is dependent on stripping out comments to remove false positives.
       */
      remove_multiline_comments_func(m_shd_builder->m_glsl_fragment_source);
      remove_singleline_comments_func(m_shd_builder->m_glsl_fragment_source);
      extract_global_scope_constants(m_shd_builder->m_glsl_fragment_source, ss_fragment);
#endif
      ...
      ss_fragment << m_shd_builder->m_glsl_fragment_source << std::endl;
    ...
    NS::FileManager *sharedFM = NS::FileManager::defaultManager();
    NS::URL *app_bundle_url = NS::Bundle::mainBundle()->bundleURL();
    NS::URL *shader_dir = app_bundle_url->URLByDeletingLastPathComponent()
                            ->URLByAppendingPathComponent(NS_STRING_("Shaders/"), YES);
    sharedFM->createDirectoryAtURL(shader_dir, YES, nil, nil);
    ...
    NS::String *msl_final_vert = NS_STRING_(ss_vertex.str().c_str());
    NS::String *msl_final_frag = (msl_iface.uses_transform_feedback) ?
                                   (NS_STRING_("")) :
                                   (NS_STRING_(ss_fragment.str().c_str()));
    ...
    this->set_vertex_function_name(
      NS_STRING_((std::string("vertex_function_entry_") + this->name).c_str())->retain());
    this->set_fragment_function_name(
      NS_STRING_((std::string("fragment_function_entry_") + this->name).c_str())->retain());
#else
    this->set_vertex_function_name(NS_STRING_("vertex_function_entry"));
    this->set_fragment_function_name(NS_STRING_("fragment_function_entry"));
#endif
    ...
    m_uses_mtl_array_index = msl_iface.m_uses_mtl_array_index;
    m_use_ssbo_vertex_fetch_mode = msl_iface.uses_ssbo_vertex_fetch_mode;
    if (msl_iface.uses_ssbo_vertex_fetch_mode) {
      m_ssbo_vertex_fetch_output_prim_type = vertex_fetch_ssbo_output_prim_type;
      m_ssbo_vertex_fetch_output_num_verts = vertex_fetch_ssbo_num_output_verts;
  ...
  {
    KLI_assert(m_use_ssbo_vertex_fetch_mode);
    ...
    KLI_assert_msg(this->uni_ssbo_input_prim_type_loc != -1,
                   "uni_ssbo_input_prim_type_loc uniform location invalid!");
    KLI_assert_msg(this->uni_ssbo_input_vert_count_loc != -1,
                   "uni_ssbo_input_vert_count_loc uniform location invalid!");
    KLI_assert_msg(this->uni_ssbo_uses_indexed_rendering != -1,
                   "uni_ssbo_uses_indexed_rendering uniform location invalid!");
    KLI_assert_msg(this->uni_ssbo_uses_index_mode_u16 != -1,
    ...
    for (int i = 0; i < mtl_interface->get_total_attributes(); i++) {
      ...
      /* SSBO-vertex-fetch Attribute data is passed via uniforms. here we need to extract the
       * uniform address for each attribute, and we can cache it for later use. */
      ShaderSSBOAttributeBinding &cached_ssbo_attr = m_cached_ssbo_attribute_bindings[i];
      ...
      KLI_assert(inp_unf_stride != nullptr);
      KLI_assert(inp_unf_offset != nullptr);
      KLI_assert(inp_unf_fetchmode != nullptr);
      KLI_assert(inp_unf_vbo_id != nullptr);
      KLI_assert(inp_unf_attr_type != nullptr);

      cached_ssbo_attr.uniform_stride = (inp_unf_stride != nullptr) ? inp_unf_stride->location :
                                                                      -1;
      cached_ssbo_attr.uniform_offset = (inp_unf_offset != nullptr) ? inp_unf_offset->location :
                                                                      -1;
      cached_ssbo_attr.uniform_fetchmode = (inp_unf_fetchmode != nullptr) ?
                                             inp_unf_fetchmode->location :
                                             -1;
      cached_ssbo_attr.uniform_vbo_id = (inp_unf_vbo_id != nullptr) ? inp_unf_vbo_id->location :
                                                                      -1;
      cached_ssbo_attr.uniform_attr_type = (inp_unf_attr_type != nullptr) ?
                                             inp_unf_attr_type->location :
                                             -1;

      KLI_assert(cached_ssbo_attr.uniform_offset != -1);
      KLI_assert(cached_ssbo_attr.uniform_stride != -1);
      KLI_assert(cached_ssbo_attr.uniform_fetchmode != -1);
      KLI_assert(cached_ssbo_attr.uniform_vbo_id != -1);
      KLI_assert(cached_ssbo_attr.uniform_attr_type != -1);
    }
  }
  ...
  void MSLGeneratorInterface::prepare_from_createinfo(const shader::ShaderCreateInfo *info)
  {
    ...
    /** Prepare Uniforms. */
    for (const shader::ShaderCreateInfo::PushConst &push_constant :
         create_info_->push_constants_) {
      ...
      for (const ShaderCreateInfo::Resource &res : resources) {
        ...
        switch (res.bind_type) {
          case shader::ShaderCreateInfo::Resource::BindType::SAMPLER: {
            ...
            KLI_assert(res.slot >= 0 && res.slot < MTL_MAX_TEXTURE_SLOTS);
            MSLTextureSampler msl_tex(ShaderStage::BOTH,
                                      res.sampler.type,
                                      res.sampler.name,
                                      access,
                                      res.slot);
          ...
          case shader::ShaderCreateInfo::Resource::BindType::UNIFORM_BUFFER: {
            MSLUniformBlock ubo;
            KLI_assert(res.uniformbuf.type_name.size() > 0);
            KLI_assert(res.uniformbuf.name.size() > 0);
            int64_t array_offset = res.uniformbuf.name.find_first_of("[");
    ...
    for (const shader::ShaderCreateInfo::FragOut &frag_out : create_info_->fragment_outputs_) {
      ...
      /* Validate input. */
      KLI_assert(frag_out.name.size() > 0);
      KLI_assert(frag_out.index >= 0);
  ...
  {
    KLI_assert(stage == ShaderStage::VERTEX || stage == ShaderStage::FRAGMENT);
  ...
  {
    KLI_assert(this->uses_ssbo_vertex_fetch_mode);
  ...
  std::string MSLGeneratorInterface::generate_msl_vertex_entry_stub()
  {
    ...
    if (this->uses_gl_InstanceID) {
      out << "vertex_shader_instance.gl_InstanceID = gl_InstanceID-gl_BaseInstanceARB;"
          << std::endl;
    }
                                                                 ...
                                                                 ShaderStage stage)
  {
    ...
    KLI_assert(this->texture_samplers.size() <= GPU_max_textures_vert());
    ...
    if (this->use_argument_buffer_for_samplers()) {
      ...
      KLI_assert(this->texture_samplers.size() <= MTL_MAX_DEFAULT_SAMPLERS);
      for (const MSLTextureSampler &tex : this->texture_samplers) {
        if (bool(tex.stage & stage)) {
  ...
  {
    KLI_assert(shader_stage == ShaderStage::VERTEX || shader_stage == ShaderStage::FRAGMENT);
    std::stringstream out;
  ...
  std::string MSLGeneratorInterface::generate_msl_vertex_out_struct(ShaderStage shader_stage)
  {
    ...
    if (this->uses_gl_Position) {
      ...
      if (!this->uses_transform_feedback) {
        ...
        KLI_assert(this->vertex_output_varyings.size() > 0);
        KLI_assert(this->vertex_output_varyings[0].type == "vec4");
    ...
    for (const MSLVertexOutputAttribute &v_out : this->vertex_output_varyings) {
      ...
      if (v_out.is_array) {
        ...
        if (is_matrix_type(v_out.type)) {
          KLI_assert(v_out.get_mtl_interpolation_qualifier() == " [[flat]]" &&
    ...
    for (int attribute = 0; attribute < this->vertex_input_attributes.size(); attribute++) {
      ...
      if (is_matrix_type(this->vertex_input_attributes[attribute].type)) {
        ...
        std::string attribute_conversion_func_name = get_attribute_conversion_function(
          &do_attribute_conversion_on_read,
  ...
  std::string MSLGeneratorInterface::generate_msl_vertex_output_population()
  {
    ...
    if (this->clip_distances.size() > 1) {
      for (int cd = 0; cd < this->clip_distances.size(); cd++) {
        out << "\toutput.clipdistance[" << cd << "] = vertex_shader_instance.gl_ClipDistance_"
            << cd << ";" << std::endl;
      }
    } else if (this->clip_distances.size() > 0) {
  ...
  std::string MSLGeneratorInterface::generate_msl_fragment_input_population()
  {
    ...
    for (int f_input = (this->uses_gl_Position) ? 0 : 1;
      ...
      for (int v_o = 0; v_o < this->vertex_output_varyings.size() && !exists_in_vertex_output;
           v_o++) {
        if (this->fragment_input_varyings[f_input].name ==
            this->vertex_output_varyings[v_o].name) {
    ...
    for (const MSLVertexInputAttribute &attr : vertex_input_attributes) {
      if (attr.layout_location >= 0) {
        ...
        for (uint32_t i = 1; i <= location_element_count; i++) {
          ...
          KLI_assert((used_locations & location_mask) == 0);
  ...
  MTLShaderInterface *MSLGeneratorInterface::bake_shader_interface(const char *name)
  {
    ...
    for (int attribute = 0; attribute < this->vertex_input_attributes.size(); attribute++) {
      ...
      if (is_matrix_type(this->vertex_input_attributes[attribute].type)) {
        ...
        for (int elem = 0;
           ...
           * shader interface, only the first one for the whole matrix, as we can pass whatever
           * data we want in this mode, and do not need to split attributes. */
    ...
    interface->add_push_constant_block(name_buffer_copystr(&interface->name_buffer_,
                                                           "PushConstantBlock",
                                                           name_buffer_size,
                                                           name_buffer_offset));
