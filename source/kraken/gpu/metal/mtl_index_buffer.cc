...
namespace kraken::gpu
{
  ...
  const uint32_t *MTLIndexBuf::read() const
  {
    ...
    KLI_assert(false && "Index buffer not ready to be read.");
    return nullptr;
  }
  ...
  void MTLIndexBuf::upload_data()
  {
...
#ifndef NDEBUG
      KLI_assert_msg(!mtlsrc->point_restarts_stripped_,
    ...
    if (ibo_ == nullptr && data_ != nullptr) {
      ...
      if (alloc_size_ == 0) {
        ...
        ibo_ = MTLContext::get_global_memory_manager().allocate_with_data(alloc_size_,
                                                                          true,
                                                                          data_);
        KLI_assert(ibo_);
        ibo_->set_label(NS_STRING_("Index Buffer"));
      }
  ...
  void MTLIndexBuf::update_sub(uint32_t start, uint32_t len, const void *data)
  {
    ...
    if (data_ != nullptr) {
      ...
      KLI_assert(start + len < this->size_get());
    ...
    MTL::Buffer *dest_buffer = ibo_->get_metal_buffer();
    KLI_assert(dest_buffer != nil);
  ...
  MTL::Buffer *MTLIndexBuf::get_index_buffer(GPUPrimType &in_out_primitive_type,
                                             uint32_t &in_out_v_count)
  {
    ...
    if (!should_optimize_or_emulate || is_subrange_ || !can_optimize_) {
      ...
      KLI_assert(this->optimized_ibo_ == nullptr);

      /* Return regular index buffer. */
      KLI_assert(this->ibo_ && this->ibo_->get_metal_buffer());
    ...
    if (optimized_ibo_ == nullptr) {
      ...
      switch (input_prim_type) {
        case GPU_PRIM_TRI_FAN: {
          ...
          KLI_assert(max_possible_verts > 0);
          ...
          if (index_type_ == GPU_INDEX_U16) {
            ...
            MutableSpan<uint16_t> output_data(
              static_cast<uint16_t *>(optimized_ibo_->get_host_ptr()),
              this->index_len_);
            emulated_v_count = populate_emulated_tri_fan_buf<uint16_t>(orig_data,
                                                                       output_data,
                                                                       this->index_len_);
          } else {
            ...
            MutableSpan<uint32_t> output_data(
              static_cast<uint32_t *>(optimized_ibo_->get_host_ptr()),
              this->index_len_);
            emulated_v_count = populate_emulated_tri_fan_buf<uint32_t>(orig_data,
                                                                       output_data,
                                                                       this->index_len_);
          }

          KLI_assert(emulated_v_count <= max_possible_verts);
        ...
        case GPU_PRIM_TRI_STRIP: {
          ...
          if (index_type_ == GPU_INDEX_U16) {
            ...
            MutableSpan<uint16_t> output_data(
              static_cast<uint16_t *>(optimized_ibo_->get_host_ptr()),
              this->index_len_);
            emulated_v_count = populate_optimized_tri_strip_buf<uint16_t>(orig_data,
                                                                          output_data,
                                                                          this->index_len_);
          } else {
            Span<uint32_t> orig_data(static_cast<const uint32_t *>(ibo_->get_host_ptr()),
            ...
            MutableSpan<uint32_t> output_data(
              static_cast<uint32_t *>(optimized_ibo_->get_host_ptr()),
              this->index_len_);
            emulated_v_count = populate_optimized_tri_strip_buf<uint32_t>(orig_data,
                                                                          output_data,
                                                                          this->index_len_);
          }
    ...
    for (uint i = 0; i < index_len_; i++) {
      if (uint_idx[i] == 0xFFFFFFFFu) {
        ...
        if (swap_index == -1) {
          ...
          index_len_--;
          KLI_assert(index_len_ <= swap_index);
        }
      }
    }
