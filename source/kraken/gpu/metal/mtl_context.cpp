...
namespace kraken
{
  ...
  namespace gpu
  {
    ...
    {
      return ANCHOR::GetCurrentContext() ? (MTLContext *)ANCHOR::GetIO().BackendRendererUserData :
                                           NULL;
    }
    ...
    void NewFrame(MTL::RenderPassDescriptor *desc)
    {
      ...
      ANCHOR_ASSERT(MTLBackend::getInstance().device != nullptr &&
                    "No Metal context. Did you call InitContext() ?");
      MTLBackend::getInstance().framebufferDescriptor =
        (new MTLBackendFramebufferDescriptor())->init(desc);
                    ...
                    size_t vertexBufferOffset)
    {
      ...
      // drawData->DisplayPos+data_data->DisplaySize (bottom right). DisplayMin is typically (0,0)
                  ...
                  MTL::RenderCommandEncoder *commandEncoder)
    {
      ...
      if (renderPipelineState == nil) {
        ...
        renderPipelineState = bd->newRenderPipelineState(bd->framebufferDescriptor,
                                                         commandBuffer->device());

        // Cache render pipeline state for later reuse
        bd->renderPipelineStateCache.insert(
          std::make_pair(bd->framebufferDescriptor, renderPipelineState));
      }
      ...
      MTLBackendBuffer *vertexBuffer = bd->dequeueReusableBuffer(vertexBufferLength,
                                                                 commandBuffer->device());
      MTLBackendBuffer *indexBuffer = bd->dequeueReusableBuffer(indexBufferLength,
                                                                commandBuffer->device());
      ...
      for (int n = 0; n < drawData->CmdListsCount; n++) {
        ...
        memcpy((char *)vertexBuffer->buffer->contents() + vertexBufferOffset,
               ...
               (size_t)cmd_list->VtxBuffer.Size * sizeof(AnchorDrawVert));
        memcpy((char *)indexBuffer->buffer->contents() + indexBufferOffset,
               cmd_list->IdxBuffer.Data,
               (size_t)cmd_list->IdxBuffer.Size * sizeof(AnchorDrawIdx));
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
          ...
          if (pcmd->UserCallback) {
            ...
            wabi::GfVec2f clip_min((pcmd->ClipRect[0] - clip_off[0]) * clip_scale[0],
                                   (pcmd->ClipRect[1] - clip_off[1]) * clip_scale[1]);
            wabi::GfVec2f clip_max((pcmd->ClipRect[2] - clip_off[0]) * clip_scale[0],
            ...
            if (clip_max[0] > fb_width) {
              clip_max[0] = (float)fb_width;
            }
            if (clip_max[1] > fb_height) {
              clip_max[1] = (float)fb_height;
            }
            ...
            MTL::ScissorRect scissorRect = {.x = NS::UInteger(clip_min[0]),
            ...
            commandEncoder->setVertexBufferOffset(
              (vertexBufferOffset + pcmd->VtxOffset * sizeof(AnchorDrawVert)),
              0);
            commandEncoder->drawIndexedPrimitives(
              MTL::PrimitiveTypeTriangle,
              pcmd->ElemCount,
              sizeof(AnchorDrawIdx) == 2 ? MTL::IndexTypeUInt16 : MTL::IndexTypeUInt32,
              indexBuffer->buffer,
              indexBufferOffset + pcmd->IdxOffset * sizeof(AnchorDrawIdx));
          }
        }
    ...
    bool CreateFonts(MTL::Device *device)
    {
      ...
      // In theory we could call GetTexDataAsAlpha8() and upload a 1-channel texture to save on
      // memory access bandwidth. However, using a shader designed for 1-channel texture would make
      // it less obvious to use the ImTextureID facility to render users own textures. You can make
      // that change in your implementation.
      ...
      texture->replaceRegion(MTL::Region::Make2D(0, 0, (NS::UInteger)width, (NS::UInteger)height),
                             0,
                             pixels,
                             (NS::UInteger)width * 4);
    ...
    bool CreateResources(MTL::Device *device)
    {
      ...
      MTLBackend::getInstance().depthStencilState = device->newDepthStencilState(
    ...
    MTLBackendFramebufferDescriptor *MTLBackendFramebufferDescriptor::init(
      MTL::RenderPassDescriptor *renderPassDescriptor)
    {
      if (MTL::RenderPassDescriptor::alloc()->init()) {
        sampleCount =
          renderPassDescriptor->colorAttachments()->object(0)->texture()->sampleCount();
        colorPixelFormat =
          renderPassDescriptor->colorAttachments()->object(0)->texture()->pixelFormat();
    ...
    MTLBackendBuffer *MTLBackend::dequeueReusableBuffer(NS::UInteger length, MTL::Device *device)
    {
      ...
      if (bestCandidate != nil) {
        bufferCache.erase(std::remove(bufferCache.begin(), bufferCache.end(), bestCandidate),
                          bufferCache.end());
    ...
    MTL::RenderPipelineState *MTLBackend::newRenderPipelineState(
      MTLBackendFramebufferDescriptor *descriptor,
      MTL::Device *device)
    {
      NS::Error *error = nil;
      ...
      if (vertexFunction == nil || fragmentFunction == nil) {
        ANCHOR_ASSERT((vertexFunction == nil || fragmentFunction == nil) &&
                      "Error: failed to find Metal shader functions in library.");
        return nil;
      }
      ...
      pipelineDescriptor->colorAttachments()->object(0)->setPixelFormat(
        framebufferDescriptor->colorPixelFormat);
      pipelineDescriptor->colorAttachments()->object(0)->setBlendingEnabled(YES);
      pipelineDescriptor->colorAttachments()->object(0)->setRgbBlendOperation(
      ...
      pipelineDescriptor->setStencilAttachmentPixelFormat(
        framebufferDescriptor->stencilPixelFormat);

      MTL::RenderPipelineState *renderPipelineState = device->newRenderPipelineState(
        pipelineDescriptor,
        &error);
      if (error != nil) {
