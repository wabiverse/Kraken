...
namespace kraken::gpu
{
  ...
  struct MSLVertexInputAttribute
  {
    ...
    {
      return (layout_location == right.layout_location && type == right.type &&
              name == right.name);
    }
  };
  ...
  struct MSLFragmentOutputAttribute
  {
    ...
    {
      return (layout_location == right.layout_location && type == right.type &&
              name == right.name && layout_index == right.layout_index);
    }
  };
  ...
  class MSLGeneratorInterface
  {
   ...
   public:
    ...
    kraken::Vector<MSLUniformBlock> uniform_blocks;
    kraken::Vector<MSLUniform> uniforms;
    kraken::Vector<MSLTextureSampler> texture_samplers;
    kraken::Vector<MSLVertexInputAttribute> vertex_input_attributes;
    kraken::Vector<MSLVertexOutputAttribute> vertex_output_varyings;
    ...
    kraken::Vector<MSLVertexOutputAttribute> fragment_input_varyings;
    kraken::Vector<MSLFragmentOutputAttribute> fragment_outputs;
    /* Transform feedback interface. */
    kraken::Vector<MSLVertexOutputAttribute> vertex_output_varyings_tf;
    /* Clip Distances. */
    kraken::Vector<std::string> clip_distances;
   ...
   public:
     ...
     * about vertex attribute bindings, in order to perform manual vertex assembly and
     * random-access vertex lookup throughout the bound VBOs.
     *
