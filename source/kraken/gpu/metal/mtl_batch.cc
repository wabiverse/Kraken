...
namespace kraken::gpu
{
    ...
    for (uint32_t a_idx = 0; a_idx < format->attr_len; a_idx++) {
      ...
      for (uint32_t n_idx = 0; n_idx < a->name_len; n_idx++) {
        ...
        if (input == nullptr || input->location == -1) {
          /* Vertex/instance buffers provided have attribute data for attributes which are not
           * needed by this particular shader. This shader only needs binding information for the
           * attributes has in the shader interface. */
          MTL_LOG_WARNING(
        ...
        if ((~attr_mask) & (1 << mtl_attr.location)) {
          ...
          if (!buffer_added) {
            ...
            desc.vertex_descriptor.buffer_layouts[buffer_index].step_function =
              (instanced) ? MTL::VertexStepFunctionPerInstance : MTL::VertexStepFunctionPerVertex;
          ...
          if (a->comp_len == 16 || a->comp_len == 12 || a->comp_len == 8) {
            ...
            if (active_shader_->get_uses_ssbo_vertex_fetch()) {
              ...
              if (a->comp_len == 16) {
                /* Debug safety checks. */
                KLI_assert_msg(mtl_attr.matrix_element_count == 4,
                               "mat4 type expected but there are fewer components");
                KLI_assert_msg(mtl_attr.size == 16, "Expecting subtype 'vec4' with 16 bytes");
                KLI_assert_msg(
                  mtl_attr.format == MTL::VertexFormatFloat4,
                  "Per-attribute vertex format MUST be float4 for an input type of 'mat4'");

                /* We have found the 'ROOT' attribute. A mat4 contains 4 consecutive float4
                 * attribute locations we must map to. */
              ...
              KLI_assert(desc.vertex_descriptor.num_attributes <= MTL_MAX_VERTEX_INPUT_ATTRIBUTES);
            }
          } else {
             ...
             * attribute read.
             *   - This handles cases such as conversion between types e.g. Integer to float
            ...
            if (can_use_internal_conversion) {
              ...
              KLI_assert(converted_format != MTL::VertexFormatInvalid);
            } else {
               ...
               * `format_conversion_mode` is assigned the kraken-specified fetch mode
               * (GPU_FETCH_*). This then controls how a given attribute is interpreted. The data
               * will be read as specified and then converted appropriately to the correct form.
               *
              ...
              MTL::VertexFormat converted_format;
              bool can_convert = mtl_vertex_format_resize(mtl_attr.format,
                                                          a->comp_len,
                                                          &converted_format);
              ...
              desc.vertex_descriptor.attributes[mtl_attr.location].format_conversion_mode =
                (GPUVertFetchMode)a->fetch_mode;
              KLI_assert(desc.vertex_descriptor.attributes[mtl_attr.location].format !=
            ...
            if (active_shader_->get_uses_ssbo_vertex_fetch()) {
              KLI_assert_msg(desc.vertex_descriptor.attributes[mtl_attr.location].format ==
                               mtl_attr.format,
             ...
             * this, it is possible that we may skip over certain attributes if they were not in
  ...
  MTL::RenderCommandEncoder *MTLBatch::bind(uint v_first, uint v_count, uint i_first, uint i_count)
  {
    ...
    if (!ctx) {
      KLI_assert_msg(false, "No context available for rendering.");
      return nil;
    }
    ...
    if (active_shader_ == nullptr || !active_shader_->is_valid()) {
       ...
       */
      KLI_assert_msg(false, "No valid Metal shader!");
      return nil;
    }
    ...
    prepare_vertex_descriptor_and_bindings(buffers,
                                           num_buffers,
                                           v_first,
                                           v_count,
                                           i_first,
    ...
    MTL::RenderCommandEncoder *rec = ctx->ensure_begin_render_pass();
    ...
    KLI_assert(!ctx->main_command_buffer.get_active_framebuffer()->get_dirty());
    ...
    if (G.debug & G_DEBUG_GPU) {
      rec->pushDebugGroup(
        NS_STRING_((std::string("batch_bind") + (this->elem ? "(indexed)" : "") +
                    "(shader: " + active_shader_->get_interface()->get_name() + ")")
                     .c_str()));
      rec->insertDebugSignpost(
        NS_STRING_((std::string("batch_bind") + (this->elem ? "(indexed)" : "") +
                    "(shader: " + active_shader_->get_interface()->get_name() + ")")
                     .c_str()));
    }
    ...
    if (uses_ssbo_fetch) {
      ...
      if (mtl_elem != nullptr) {
        ...
        KLI_assert(uniform_ssbo_index_mode_u16 != -1);
      ...
      MTL::Buffer *null_buffer = ctx->get_null_buffer();
      ...
      KLI_assert(uniform_ssbo_use_indexed != -1);
      ...
      KLI_assert(active_shader_->uni_ssbo_input_prim_type_loc != -1);
      KLI_assert(active_shader_->uni_ssbo_input_vert_count_loc != -1);
    ...
    for (int i = 0; i < num_buffers; i++) {
      ...
      MTL::Buffer *mtl_buffer = mtlvbo->get_metal_buffer();

      KLI_assert(mtl_buffer != nil);
  ...
  void MTLBatch::prepare_vertex_descriptor_and_bindings(MTLVertBuf **buffers,
                                                        int &num_buffers,
                                                        int v_first,
                                                        int v_count,
                                                        int i_first,
                                                        int i_count)
  {
    ...
    /* Here we populate the MTLContext vertex descriptor and resolve which buffers need to be
     * bound.
     */
    ...
    if (descriptor) {
      ...
      if (active_shader_->get_uses_ssbo_vertex_fetch()) {
        KLI_assert(desc.vertex_descriptor.uses_ssbo_vertex_fetch);
      ...
      /* NOTE: Attribute extraction order from buffer is the reverse of the OpenGL as we flag once
       * an attribute is found, rather than pre-setting the mask. */
      ...
      for (int v = 0; v < GPU_BATCH_INST_VBO_MAX_LEN; v++) {
        if (mtl_inst[v]) {
          MTL_LOG_INFO(" -- [Batch] Checking bindings for bound instance buffer %p\n",
                       mtl_inst[v]);
          int buffer_ind =
            this->prepare_vertex_binding(mtl_inst[v], desc, interface, attr_mask, true);
          if (buffer_ind >= 0) {
      ...
      for (int v = 0; v < GPU_BATCH_VBO_MAX_LEN; v++) {
        if (mtl_verts[v] != NULL) {
          ...
          int buffer_ind =
            this->prepare_vertex_binding(mtl_verts[v], desc, interface, attr_mask, false);
          if (buffer_ind >= 0) {
...
/* DEBUG: verify if our attribute bindings have been fully provided as expected. */
    ...
    if (attr_mask != 0) {
      for (uint16_t mask = 1, a = 0; a < 16; a++, mask <<= 1) {
        if (attr_mask & mask) {
           ...
           * 'MTLVertexFormatInvalid' in the vertex descriptor are bound to a NULL buffer during
           * PSO creation. */
          MTL_LOG_WARNING(
            "MTLBatch: Missing expected attribute '%s' at index '%d' for shader: %s\n",
          ...
          KLI_assert(desc.attributes[a].format == MTLVertexFormatInvalid);
        }
      }
    }
#endif
  ...
  void MTLBatch::draw_advanced(int v_first, int v_count, int i_first, int i_count)
  {
...
#if TRUST_NO_ONE
    KLI_assert(v_count > 0 && i_count > 0);
#endif

    /* Setup RenderPipelineState for batch. */
    MTLContext *ctx = reinterpret_cast<MTLContext *>(GPU_context_active_get());
    MTL::RenderCommandEncoder *rec = this->bind(v_first, v_count, i_first, i_count);
    if (rec == nil) {
      return;
    }
    ...
    if (active_shader_->get_uses_ssbo_vertex_fetch()) {
      ...
      KLI_assert_msg(
        mtl_vertex_count_fits_primitive_type(
          output_num_verts,
          active_shader_->get_ssbo_vertex_fetch_output_prim_type()),
    ...
    /* Perform regular draw. */
      ...
      if (mtl_needs_topology_emulation(this->prim_type)) {
        ...
        MTL::Buffer *generated_index_buffer = this->get_emulated_toplogy_buffer(emulated_prim_type,
                                                                                emulated_v_count);
        KLI_assert(generated_index_buffer != nil);

        MTL::PrimitiveType emulated_mtl_prim_type = gpu_prim_type_to_metal(emulated_prim_type);

        /* Temp: Disable culling for emulated primitive types.
         * TODO(Metal): Support face winding in topology buffer. */
        rec->setCullMode(MTL::CullModeNone);

        if (generated_index_buffer != nil) {
          KLI_assert(emulated_mtl_prim_type == MTL::PrimitiveTypeTriangle ||
                     emulated_mtl_prim_type == MTL::PrimitiveTypeLine);
          if (emulated_mtl_prim_type == MTL::PrimitiveTypeTriangle) {
            KLI_assert(emulated_v_count % 3 == 0);
          }
          if (emulated_mtl_prim_type == MTL::PrimitiveTypeLine) {
            KLI_assert(emulated_v_count % 2 == 0);
          }
    ...
    /* Perform indexed draw. */
      ...
      MTL::IndexType index_type = MTLIndexBuf::gpu_index_type_to_metal(mtl_elem->index_type_);
      ...
      KLI_assert_msg((v_first_ofs % index_size) == 0,
      ...
      MTL::Buffer *index_buffer = mtl_elem->get_index_buffer(final_prim_type, index_count);
      mtl_prim_type = gpu_prim_type_to_metal(final_prim_type);
      KLI_assert(index_buffer != nil);
      ...
      if (index_buffer != nil) {
        ...
        KLI_assert_msg(false, "Index buffer does not have backing Metal buffer");
      }
    }
  ...
  MTL::Buffer *MTLBatch::get_emulated_toplogy_buffer(GPUPrimType &in_out_prim_type,
                                                     uint32_t &in_out_v_count)
  {

    KLI_assert(in_out_v_count > 0);
    ...
    switch (input_prim_type) {
      ...
      case GPU_PRIM_LINES:
      case GPU_PRIM_TRIS:
        KLI_assert_msg(false, "Optimal primitive types should not reach here.");
        return nil;
        break;
      case GPU_PRIM_LINES_ADJ:
      case GPU_PRIM_TRIS_ADJ:
        KLI_assert_msg(false, "Adjacency primitive types should not reach here.");
        return nil;
        break;
      ...
      case GPU_PRIM_TRI_FAN:
        ...
        break;
      default:
        KLI_assert_msg(false, "Invalid primitive type.");
        return nil;
    }
    ...
    if (this->emulated_topology_buffer_ == nullptr) {
      ...
      switch (input_prim_type) {
        ...
        case GPU_PRIM_TRI_FAN:
          ...
          break;
        default:
          KLI_assert_msg(false, "Cannot generate optimized topology buffer for other types.");
          break;
      }
      ...
      switch (input_prim_type) {
        ...
        default:
          KLI_assert_msg(false, "Other primitive types do not require emulation.");
          return nil;
      }
