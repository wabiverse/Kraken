...
namespace kraken::gpu
{
  ...
  thread_local NS::AutoreleasePool *g_autoreleasepool = nil;
  ...
  Context *MTLBackend::context_alloc(void *anchor_window, void *anchor_context)
  {
    return new MTLContext(anchor_window, anchor_context);
  }
  ...
  void MTLBackend::render_begin()
  {
    ...
    KLI_assert(g_autoreleasepool_depth >= 0);
  ...
  void MTLBackend::render_end()
  {
    ...
    KLI_assert(g_autoreleasepool != nil);
    g_autoreleasepool_depth--;
    KLI_assert(g_autoreleasepool_depth >= 0);
  ...
  void MTLBackend::platform_init(MTLContext *ctx)
  {
    ...
    KLI_assert(ctx);
    MTL::Device *mtl_device = ctx->device;
    KLI_assert(device);

    NS::String *gpu_name = mtl_device->name();
    ...
    KLI_assert_msg(os == GPU_OS_MAC, "Platform must be macOS");
  ...
  bool supports_barycentric_whitelist(MTL::Device *device)
  {
    NS::String *gpu_name = device->name();
    KLI_assert(gpu_name->length());
    const char *vendor = gpu_name->utf8String();

    /* Verify GPU support. */
    bool supported_gpu = device->supportsFamily(MTL::GPUFamily::GPUFamilyMac2);
  ...
  bool MTLBackend::metal_is_supported()
  {
    ...
    if (supported_os_version) {
      ...
      if (forceIntel) {
        ...
        for (NS::UInteger dix = 0; dix < allDevices->count(); dix++) {
          MTL::Device *_device = allDevices->object<MTL::Device>(dix);
          if (_device->lowPower()) {
            device = _device;
          }
        }
      ...
      bool supports_argument_buffers_tier2 = (device->argumentBuffersSupport() ==
                                              MTL::ArgumentBuffersTier::ArgumentBuffersTier2);
      bool supports_barycentrics = device->supportsShaderBarycentricCoordinates() ||
                                   supports_barycentric_whitelist(device);
      bool supported_metal_version = device->supportsFamily(MTL::GPUFamily::GPUFamilyMac2);
  ...
  void MTLBackend::capabilities_init(MTLContext *ctx)
  {
    ...
    MTLBackend::capabilities.supports_argument_buffers_tier2 =
      (device->argumentBuffersSupport() == MTL::ArgumentBuffersTier::ArgumentBuffersTier2);
    MTLBackend::capabilities.supports_family_mac1 = device->supportsFamily(
      MTL::GPUFamily::GPUFamilyMac1);
    MTLBackend::capabilities.supports_family_mac2 = device->supportsFamily(
      MTL::GPUFamily::GPUFamilyMac2);
    MTLBackend::capabilities.supports_family_mac_catalyst1 = device->supportsFamily(
      MTL::GPUFamily::GPUFamilyMacCatalyst1);
    MTLBackend::capabilities.supports_family_mac_catalyst2 = device->supportsFamily(
      MTL::GPUFamily::GPUFamilyMacCatalyst2);

    /* Common Global Capabilities. */
    GCaps.max_texture_size = (device->supportsFamily(MTL::GPUFamily::GPUFamilyApple3) ||
    ...
    GCaps.max_textures = (MTLBackend::capabilities.supports_family_mac1) ?
                           128 :
                           ((device->supportsFamily(MTL::GPUFamily::GPUFamilyApple4)) ? 96 : 31);
    ...
    GCaps.shader_image_load_store_support = (device->supportsFamily(
                                               MTL::GPUFamily::GPUFamilyApple3) ||
                                             MTLBackend::capabilities.supports_family_mac1 ||
    ...
    if (GCaps.compute_shader_support) {
      ...
      uint max_threads_per_threadgroup_per_dim = (device->supportsFamily(
                                                    MTL::GPUFamily::GPUFamilyApple4) ||
                                                  MTLBackend::capabilities.supports_family_mac1) ?
                                                   1024 :
                                                   512;
