...
#include "KLI_function_ref.hh"
#include "KLI_string_ref.hh"
#include "KLI_vector.hh"

#include "UI_resources.h"

namespace kraken::nodes::geo_eval_log {
...
void context_path_add_generic(Vector<ContextPathItem> &path,
                              StructRNA &rna_type,
                              void *ptr,
                              const KIFIconID icon_override = ICON_NONE);
...
class DropTargetInterface {
 public:
   ...
   */
  virtual bool on_drop(kContext *C, const wmDrag &drag) const = 0;
};
 ...
 */
bool drop_target_apply_drop(kContext &C,
...
class uiListNameFilter {
 ...
 public:
  ...
  eUIListFilterResult operator()(const KrakenPRIM &itemptr,
                                 kraken::StringRefNull name,
                                 int index);
};

using uiListItemFilterFn = kraken::FunctionRef<eUIListFilterResult(
    const KrakenPRIM &itemptr, kraken::StringRefNull name, int index)>;
using uiListItemGetNameFn =
    kraken::FunctionRef<std::string(const KrakenPRIM &itemptr, int index)>;
 ...
 */
kraken::ui::AbstractGridView *UI_block_add_view(
    uiBlock &block,
    kraken::StringRef idname,
    std::unique_ptr<kraken::ui::AbstractGridView> grid_view);
kraken::ui::AbstractTreeView *UI_block_add_view(
    uiBlock &block,
    kraken::StringRef idname,
    std::unique_ptr<kraken::ui::AbstractTreeView> tree_view);
