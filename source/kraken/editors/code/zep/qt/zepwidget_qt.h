...
namespace Zep
{
  ...
  class ZepWidget_Qt : public QWidget, public IZepComponent
  {
   public:
...
#else
      NVec2f pixelScale(NVec2f(qRound(qApp->primaryScreen()->logicalDotsPerInchX() / 96.0f),
                               qRound(qApp->primaryScreen()->logicalDotsPerInchY() / 96.0f)));
#endif
    ...
    {
      m_spEditor->SetDisplayRegion(NVec2f(0.0f, 0.0f),
                                   NVec2f(pResize->size().width(), pResize->size().height()));
    }
