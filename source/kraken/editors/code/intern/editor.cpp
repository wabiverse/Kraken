...
namespace Zep
{
  ...
  ZepEditor::ZepEditor(ZepDisplay *pDisplay,
  ...
  ZepBuffer *ZepEditor::GetFileBuffer(const ZepPath &filePath, uint32_t fileFlags, bool create)
  {
    ...
    // Create buffer, try to load even if not present, the buffer represents the save path (it just
    // isn't saved yet)
  ...
  ZepWindow *ZepEditor::AddSearch()
  {
    ...
    pSearchBuffer->SetSyntax(std::make_shared<ZepSyntax>(*pSearchBuffer,
                                                         search_keywords,
                                                         search_identifiers,
  ...
  void ZepEditor::Display()
  {
    ...
    for (auto &tab : m_tabRegion->children) {
      ...
      // Tab text
      m_pDisplay->DrawChars(uiFont,
                            rc.topLeftPx + DPI_VEC2(NVec2f(textBorder, 0.0f)),
                            textCol,
                            (const uint8_t *)spTabRegionTab->name.c_str());
    }
