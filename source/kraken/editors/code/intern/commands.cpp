...
namespace Zep
{
  ...
  // Insert a string
  ZepCommand_Insert::ZepCommand_Insert(ZepBuffer &buffer,
                                       const GlyphIterator &start,
    ...
    : ZepCommand(buffer,
                 cursor,
                 cursorAfter.Valid() ? cursorAfter : (start.PeekByteOffset(long(str.size())))),
      m_startIndex(start),
      m_strInsert(str)
  {
    m_startIndex.Clamp();
  ...
  // Replace
  ZepCommand_ReplaceRange::ZepCommand_ReplaceRange(ZepBuffer &buffer,
                                                   ReplaceRangeMode currentMode,
  ...
  void ZepCommand_ReplaceRange::Undo()
  {
    if (m_startIndex != m_endIndex)
    {
      ...
      m_buffer.Replace(m_startIndex,
                       m_mode == ReplaceRangeMode::Fill ?
                         m_endIndex :
                         m_startIndex.PeekByteOffset((long)m_strReplace.length()),
                       m_changeRecord.strDeleted,
