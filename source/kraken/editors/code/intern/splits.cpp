...
namespace Zep
{
  ...
  void LayoutRegion(Region &region)
  {
    ...
    auto remaining = ((region.layoutType == RegionLayoutType::HBox) ? currentRect.Width() :
                                                                      currentRect.Height()) -
                     (totalFixedSize + padding);
    ...
    for (auto &rcChild : region.children) {
      ...
      if (region.layoutType == RegionLayoutType::VBox) {
        ...
        if (rcChild->flags & RegionFlags::Fixed) {
          rcChild->rect.bottomRightPx.y = rcChild->rect.Top() +
                                          SizeForLayout(region.layoutType, rcChild->fixed_size);
        ...
        if (rcChild->flags & RegionFlags::Fixed) {
          rcChild->rect.bottomRightPx.x = rcChild->rect.Left() +
                                          SizeForLayout(region.layoutType, rcChild->fixed_size);
      ...
      if (!(rcChild->flags & RegionFlags::Expanding) && rcChild->fixed_size.x != 0.0f &&
