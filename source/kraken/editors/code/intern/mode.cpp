...
namespace Zep
{
  ...
  {
    // Mode begin should always set this and we should always have a valid window associated with
    // the mode
...
#define COMPARE_STR(a, b) \
  ...
  void ZepMode::HandleMappedInput(const std::string &input)
  {
    ...
    // Figure out the command we have typed. foundCommand means that the command was interpreted
    // and understood. If spCommand is returned, then there is an atomic command operation that
    // needs to be done.
    ...
    if (spContext->foundCommand) {
      ...
      // Note: a command here is something that modifies the text.  It can be something like a
      // delete or a simple insert
      ...
      // Maybe all commands should handle the count?  What are the implications of that?  This bit
      // is a bit messy
      if (!(spContext->commandResult.flags & CommandResultFlags::HandledCount)) {
        ...
        for (int i = 1; i < spContext->keymap.TotalCount(); i++) {
          ...
          // Create a new 'inner' spContext-> for the next command, because we need to
          // re-initialize the command spContext-> for 'after' what just happened!
  ...
  bool ZepMode::GetCommand(CommandContext &context)
  {
    ...
    // Control
      ...
      auto pFound = buffer.FindNextMarker(GetCurrentWindow()->GetBufferCursor(),
                                          Direction::Forward,
        ...
        for (auto &p : searchPaths) {
          ...
          for (auto &priorityFolder : priorityFolders) {
            GetEditor().GetFileSystem().ScanDirectory(
              p,
              [&](const ZepPath &currentPath, bool &recurse) {
                recurse = false;
    ...
    // Moving between tabs
      ...
      auto target = context.buffer.WordMotion(context.bufferCursor,
                                              SearchType::Word,
                                              Direction::Forward);
      ...
      auto target = context.buffer.EndWordMotion(context.bufferCursor,
                                                 SearchType::Word,
                                                 Direction::Forward);
      GetCurrentWindow()->SetBufferCursor(target);
      return true;
      ...
      if (context.bufferCursor.Char() == '\n') {
        ...
        context.endRange = std::max(
          context.endRange,
          buffer.GetLinePos(context.endRange, LineLocation::LineFirstGraphChar));
      ...
      if (!context.keymap.captureChars.empty()) {
        GetCurrentWindow()->SetBufferCursor(
          context.buffer.FindOnLineMotion(bufferCursor,
                                          (const uint8_t *)&context.keymap.captureChars[0],
                                          Direction::Forward));
      ...
      GetCurrentWindow()->SetBufferCursor(
        context.buffer.FindOnLineMotion(bufferCursor,
                                        (const uint8_t *)m_lastFind.c_str(),
                                        m_lastFindDirection));
      return true;
      ...
      GlyphIterator appendItr = context.buffer.GetLinePos(bufferCursor,
                                                          LineLocation::LineLastNonCR);
  ...
  bool ZepMode::HandleExCommand(std::string strCommand)
  {
    ...
    if (m_lastKey == ExtKeys::RETURN) {
      ...
      if (pCommand) {
        ...
        if (strTok.size() > 1) {
          if (strTok[1] == "%") {
            pTab->AddWindow(&GetCurrentWindow()->GetBuffer(),
                            GetCurrentWindow(),
      ...
      if (m_currentCommand.length() > 0) {
        ...
        if (!searchString.empty()) {
          ...
          while (numMarkers < MaxMarkers) {
            auto found = buffer.Find(start,
                                     (uint8_t *)&searchString[0],
                                     (uint8_t *)&searchString[searchString.length()]);
            if (!found.Valid()) {
              break;
            }
