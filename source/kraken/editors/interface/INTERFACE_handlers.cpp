...
static void ui_multibut_states_create(uiBut *but_active, uiHandleButtonData *data)
{
  ...
  if ((but_active->stageprop && (LUXO_prop_flag(but_active->stageprop) & PROP_PROPORTIONAL)) ||
...
#  endif
    ...
    KLI_assert(active_back == nullptr);
...
static void ui_apply_but_func(kContext *C, uiBut *but)
{
  ...
  after->opcontext = static_cast<eWmOperatorContext>(but->opcontext);
...
{
  KLI_assert(callbacks->update_fn != nullptr);
...
#  endif
    ...
    if (ELEM(event->type, LEFTMOUSE, EVT_PADENTER, EVT_RETKEY) && event->val == KM_PRESS) {
      ...
      const uiBut *view_but = ui_view_item_find_mouse_over(data->region, event->mouse_pos);
      if (view_but) {
        ret = WM_UI_HANDLER_CONTINUE;
      }
                               ...
                               const wmEvent *event)
{
  ...
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    ...
    if (event->type == LEFTMOUSE && event->val == KM_PRESS) {
      ...
      if (ui_but_contains_point_px(but, but->active->region, event->mouse_pos) == false) {
        data->cancel = true;
...
static int ui_do_but_TAB(kContext *C,
                         ...
                         const wmEvent *event)
{
  const bool is_property = (but->stageprop != nullptr);
  ...
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    ...
    if (is_property && ELEM(prim_type, PROP_POINTER, PROP_STRING) && (but->custom_data != nullptr) &&
...
static uiButExtraOpIcon *ui_but_extra_operator_icon_mouse_over_get(uiBut *but,
                                                                   ...
                                                                   const wmEvent *event)
{
  if (KLI_listbase_is_empty(&but->extra_op_icons)) {
    return nullptr;
  }
...
static void ui_textedit_begin(kContext *C, uiBut *but, uiHandleButtonData *data)
{
  ...
  // if (ui_but_is_float(but) && !ui_but_is_unit(but) && !ui_but_anim_expression_get(but, nullptr, 0)
  // &&
  //     !no_zero_strip) {
  ...
  if (state == BUTTON_STATE_HIGHLIGHT) {
    ...
    if (ELEM(but->type, UI_BTYPE_BLOCK, UI_BTYPE_PULLDOWN, UI_BTYPE_POPOVER) ||
      ...
      if (data->used_mouse && !data->autoopentimer) {
        ...
        } else if (U.flag & USER_MENUOPENAUTO) {
  ...
  if (!(but->block->handle && but->block->handle->popup)) {
    if (button_modal_state(state)) {
      if (!button_modal_state(data->state)) {
        ...
        LISTBASE_FOREACH(wmEventHandlerUI *, mh, &data->window->modalhandlers)
        {
          handlers.push_back(mh);
        }

        WM_event_add_ui_handler(C, handlers, ui_handler_region_menu, nullptr, data, 0);
      }
    } else {
      if (button_modal_state(data->state)) {
        /* true = postpone free */
                                 ...
                                 uiButtonActivateType type)
{
  ...
  KLI_assert(ui_region_find_active_but(region) == nullptr);
                                            ...
                                            uiButtonActivateType activate_type)
{
  ...
  uiBut *labelbut = ui_but_find_mouse_over_ex(region, event->mouse_pos, true, false, nullptr, nullptr);
...
static void ui_but_copy(kContext *C, uiBut *but, const bool copy_array)
{
  ...
  const bool has_required_data = !(but->poin == nullptr && but->stagepoin.data == nullptr);
  ...
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    switch (event->type) {
      ...
      case MOUSEMOVE: {
        ...
        if ((!ui_block_is_menu(block) || ui_block_is_pie_menu(block)) &&
            !ui_but_contains_point_px(but, region, event->mouse_pos)) {
          exit = true;
      ...
      case TIMER: {
        ...
        if (event->customdata == data->autoopentimer) {
          ...
          if (ui_but_contains_point_px(but, region, event->mouse_pos) || but->active) {
            button_activate_state(C, but, BUTTON_STATE_MENU_OPEN);
          }
        }
    ...
    switch (event->type) {
      ...
      case MOUSEMOVE: {
        ...
        if (ui_but_contains_point_px(but, region, event->mouse_pos)) {

          /* Drag on a hold button (used in the toolbar) now opens it immediately. */
          if (data->hold_action_timer) {
            if (but->flag & UI_SELECT) {
              if (len_manhattan_v2v2_int(event->mouse_pos, event->prev_mouse_pos) <=
    ...
    switch (event->type) {
      case MOUSEMOVE: {
        ...
        if (data->menu && data->menu->region) {
          if (ui_region_contains_point_px(data->menu->region, event->mouse_pos)) {
            break;
          }
        }
...
static void ui_handle_button_return_submenu(kContext *C, const wmEvent *event, uiBut *but)
{
  ...
  if ((menu->menuretval & UI_RETURN_OK) || (menu->menuretval & UI_RETURN_CANCEL)) {
    ...
    if (event->type == MOUSEMOVE &&
        ui_but_contains_point_px(but, data->region, event->mouse_pos)) {
      button_activate_state(C, but, BUTTON_STATE_HIGHLIGHT);
    } else {
...
static void ui_block_interaction_end(kContext *C,
                                     ...
                                     uiBlockInteraction_Handle *interaction)
{
  KLI_assert(callbacks->end_fn != nullptr);
...
#ifdef USE_ALLSELECT
    {
      /* only Prim from this button is used */
      uiBut but_temp = *but;
  ...
  if (but) {
    ...
    if ((data->state == BUTTON_STATE_MENU_OPEN) &&
        ...
        (((data->menu->direction & (UI_DIR_LEFT | UI_DIR_RIGHT)) &&
          KLI_rctf_isect_rect_x(&but->rect, &but_other->rect, nullptr)) ||
         ((data->menu->direction & (UI_DIR_DOWN | UI_DIR_UP)) &&
          KLI_rctf_isect_rect_y(&but->rect, &but_other->rect, nullptr)))) {
...
static bool ui_prim_is_userdef(const KrakenPRIM *ptr, KrakenPROP *prop)
{
  if (!prop) {
    return false;
  }
