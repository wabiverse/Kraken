...
{
  return ui_text_icon_width_ex(layout,
                               name,
                               icon,
                               compact ? &ui_text_pad_compact : &ui_text_pad_default);
}
...
static void ui_item_size(uiItem *item, int *r_w, int *r_h)
{
  if (item->type == ITEM_BUTTON) {
    ...
    if (r_w) {
      *r_w = KLI_rctf_size_x(GfVec4f(bitem->but->rect.xmin,
                                     bitem->but->rect.xmax,
                                     bitem->but->rect.ymin,
                                     bitem->but->rect.ymax));
    }
    if (r_h) {
      *r_h = KLI_rctf_size_y(GfVec4f(bitem->but->rect.xmin,
                                     bitem->but->rect.xmax,
                                     bitem->but->rect.ymin,
                                     bitem->but->rect.ymax));
    }
...
static void ui_layer_but_cb(kContext *C, void *arg_but, void *arg_index)
{
  ...
  SdfValueTypeName arr_type = (prop->GetTypeName().GetScalarType() == TfToken()) ?
                                prop->GetTypeName().GetArrayType() :
                                prop->GetTypeName().GetScalarType();
                          ...
                          bool show_text)
{
  ...
  const PropertyType type = LUXO_prop_type(prop);
  const PropertySubType subtype = LUXO_prop_subtype(prop);
  ...
  if (type == PROP_BOOLEAN && ELEM(subtype, PROP_LAYER, PROP_LAYER_MEMBER)) {
    ...
    for (int b = 0; b < cols; b++) {
      ...
      for (int a = 0; a < colbuts; a++) {
        ...
        uiBut *but =
          uiDefAutoButR(block, ptr, prop, layer_num, "", icon, x + butw * a, y + buth, butw, buth);
      ...
      for (int a = 0; a < colbuts; a++) {
        ...
        uiBut *but =
          uiDefAutoButR(block, ptr, prop, layer_num, "", icon, x + butw * a, y, butw, buth);
        if (subtype == PROP_LAYER_MEMBER) {
          UI_but_func_set(but, ui_layer_but_cb, but, POINTER_FROM_INT(layer_num));
        }
      }
    ...
    totdim = (int)ptr->GetAttribute(prop->GetName()).GetTypeName().GetDimensions().size;
    if (totdim != 2) {
    ...
    if (ELEM(subtype, PROP_COLOR, PROP_COLOR_GAMMA) && !expand && ELEM(len, 3, 4)) {
      ...
      if (type == PROP_BOOLEAN &&
        ...
        // LUXO_prop_boolean_get_array(ptr, prop, boolarr);
      }

      const char *str_buf = show_text ? str : "";
      for (int a = 0; a < len; a++) {
        if (!icon_only && show_text) {
          // str[0] = LUXO_prop_array_item_char(prop, a);
        }
        if (boolarr) {
        ...
        uiBut *but =
          uiDefAutoButR(block, ptr, prop, a, str_buf, icon, 0, 0, width_item, UI_UNIT_Y);
        if (slider && but->type == UI_BTYPE_NUM) {
          uiButNumber *number_but = (uiButNumber *)but;
...
static void ui_item_enum_expand_handle(kContext *C, void *arg1, void *arg2)
{
  wmWindow *win = CTX_wm_window(C);
  ...
  if ((win->eventstate->modifier & KM_SHIFT) == 0) {
    ...
    // int current_value = LUXO_prop_enum_get(&but->stagepoin, but->stageprop);
    // if (!(current_value & enum_value)) {
    //   current_value = enum_value;
    // } else {
    //   current_value &= enum_value;
    // }
    // LUXO_prop_enum_set(&but->stagepoin, but->stageprop, current_value);
  }
}
                                          ...
                                          const bool is_first)
{
  ...
  if (prop->flag & PROP_ENUM_FLAG) {
    ...
    KLI_assert(ELEM(but->func, nullptr, block->func));
                                     ...
                                     const bool icon_only)
{
  ...
  if (radial) {
    // LUXO_prop_enum_items_gettexted_all(block->evil_C, ptr, prop, &item_array, nullptr, &free);
  } else {
    // LUXO_prop_enum_items_gettexted(block->evil_C, ptr, prop, &item_array, nullptr, &free);
  }
  ...
  for (auto &item : item_array) {
    ...
    if (!item->identifier.data()[0]) {
      const EnumPROP *next_item = item + 1;
    ...
    ui_item_enum_expand_elem_exec(layout,
                                  block,
                                  ptr,
                                  prop,
                                  uiname,
...
static void ui_item_enum_expand_tabs(uiLayout *layout,
                                     ...
                                     KrakenPRIM *ptr,
                                     KrakenPROP *prop,
                                     KrakenPRIM *ptr_highlight,
                                     KrakenPROP *prop_highlight,
                                     const char *uiname,
                                     const int h,
                                     const bool icon_only)
{
  ...
  for (uiBut *tab = last ? last->next : static_cast<uiBut *>(block->buttons.first); tab; tab = tab->next) {
  ...
  if (use_custom_highlight) {
    ...
    LUXO_prop_boolean_get_array(ptr_highlight, prop_highlight, highlight_array);
    int i = 0;
...
void UI_context_active_but_prop_get_filebrowser(const kContext *C,
                                                KrakenPRIM *r_ptr,
                                                KrakenPROP **r_prop,
                                                bool *r_is_undo,
                                                bool *r_is_userdef)
  ...
  LISTBASE_FOREACH (uiBlock *, block, &region->uiblocks) {
    LISTBASE_FOREACH (uiBut *, but, &block->buttons) {
      if (but && but->stagepoin.data) {
        if (LUXO_prop_type(but->stageprop) == PROP_STRING) {
          prevbut = but;
        }
      }
 ...
 */
static void ui_but_tip_from_enum_item(uiBut *but, const EnumPROP *item)
{
                             ...
                             KrakenPRIM *r_opptr)
{
  ...
  UI_but_func_hold_set(but, ui_item_menu_hold, KLI_strdup(menu_id));
}
                                        ...
                                        int retval)
{
  bool free;
  const EnumPROP *item;
  LUXO_prop_enum_items((kContext *)layout->root->block->evil_C, ptr, prop, &item, nullptr, &free);
                     ...
                     int value)
{
  ...
  KrakenPROP *prop = LUXO_prim_find_property(&ptr, propname);
  if (prop == nullptr) {
    TF_WARN("%s.%s not found", LUXO_prim_identifier(ptr.type).GetText(), propname);
    return;
  }
...
KLI_INLINE bool ui_layout_is_radial(const uiLayout *layout)
{
...
void uiItemsFullEnumO_items(uiLayout *layout,
                            ...
                            eWmOperatorContext context,
                            int flag,
                            const EnumPROP *item_array,
                            int totitem)
{
  const TfToken propname = LUXO_prop_identifier(prop);
  if (LUXO_prop_type(prop) != PROP_ENUM) {
    TF_WARN("%s.%s, not an enum type",
            LUXO_prim_identifier(ptr.type).GetText(),
            propname.GetText());
    return;
  }
  ...
  } else if ((uiLayoutGetLocalDir(layout) == UI_LAYOUT_HORIZONTAL) &&
  ...
  const EnumPROP *item = item_array;
  for (int i = 1; item->identifier.data() && !last_iter; i++, item++) {
    ...
    if (item->identifier.data()[0]) {
      ...
      LUXO_prop_enum_set(&tptr, prop, item->value);
                      ...
                      int flag)
{
  ...
  KLI_assert((prop == nullptr) || (LUXO_prop_type(prop) == PROP_ENUM));

  uiBlock *block = layout->root->block;
  if (prop && LUXO_prop_type(prop) == PROP_ENUM) {
    const EnumPROP *item_array = nullptr;
    int totitem;
    bool free;
    ...
    if (ui_layout_is_radial(layout)) {
       ...
       */
#if 0
      LUXO_prop_enum_items_gettexted_all(
          block->evil_C, &ptr, prop, &item_array, &totitem, &free);
#else
      //LUXO_prop_enum_items_gettexted(static_cast<bContext *>(block->evil_C), &ptr, prop, &item_array, &totitem, &free);
#endif
    } else {
      kContext *C = static_cast<kContext *>(block->evil_C);
      kContextStore *previous_ctx = CTX_store_get(C);
      CTX_store_set(C, layout->context);
      //LUXO_prop_enum_items_gettexted(C, &ptr, prop, &item_array, &totitem, &free);
  ...
  } else if (prop && LUXO_prop_type(prop) != PROP_ENUM) {
    TF_WARN("%s.%s, not an enum type", LUXO_prim_identifier(ptr.type).data(), propname);
    return;
  } else {
    TF_WARN("%s.%s not found", LUXO_prim_identifier(ptr.type).data(), propname);
    return;
  }
}
                        ...
                        const char *value_str)
{
  ...
  int value;
  if (item == nullptr /*|| LUXO_enum_value_from_id(item, value_str, &value) == 0*/) {
    if (free) {
      MEM_freeN((void *)item);
    }
    TF_WARN("%s.%s, enum %s not found",
            LUXO_prim_identifier(ptr.type).GetText(),
            propname,
            value_str);
    return;
                              ...
                              int *r_h)
{
  ...
  if (!name[0] && !icon_only) {
    if (ELEM(type, PROP_STRING, PROP_POINTER)) {
      ...
      const EnumPROP *item_array;
      bool free;
      // LUXO_prop_enum_items_gettexted(layout->root->block->evil_C,
      //                                   ptr,
      //                                   prop,
      //                                   &item_array,
      //                                   nullptr,
      ...
      for (const EnumPROP *item = item_array; item->identifier.data(); item++) {
        if (item->identifier.data()[0]) {
  ...
  if (!w) {
    if (type == PROP_ENUM && icon_only) {
      ...
      w = ui_text_icon_width(layout,
                             name,
                             icon,
                             ELEM(type, PROP_FLOAT, PROP_INT) ? false : compact);
    }
  }
  h = UI_UNIT_Y;
                 ...
                 int icon)
{
      ...
      if (type == PROP_BOOLEAN) {
        if ((is_array == false) || (index != LUXO_NO_INDEX)) {
          if (prop_flag & PROP_ICONS_CONSECUTIVE) {
            ...
            // icon = (LUXO_prop_boolean_get_index(ptr, prop, index)) ? ICON_CHECKBOX_HLT :
            //                                                             ICON_CHECKBOX_DEHLT;
          } else {
            // icon = (LUXO_prop_boolean_get(ptr, prop)) ? ICON_CHECKBOX_HLT :
            // ICON_CHECKBOX_DEHLT;
          }
        }
      } else if (type == PROP_ENUM) {
  ...
  if (use_prop_sep) {
    ...
    if ((name[0] == '\0') && !use_split_empty_name) {
      ...
      uiLayout *layout_split = uiLayoutSplit(layout_row ? layout_row : layout,
...
#ifdef UI_PROP_DECORATE
  if (ui_decorate.use_prop_decorate) {
    uiBut *but_decorate = ui_decorate.but ? ui_decorate.but->next : static_cast<uiBut *>(block->buttons.first);
    ...
    for (i = 0; i < ui_decorate.len && but_decorate; i++) {
      KrakenPRIM *ptr_dec = use_blank_decorator ? nullptr : &but_decorate->stagepoin;
      KrakenPROP *prop_dec = use_blank_decorator ? nullptr : but_decorate->stageprop;
             ...
             int icon)
{
  ...
  if (!prop) {
    ...
    TF_WARN("property not found: %s.%s", LUXO_prim_identifier(ptr->type).GetText(), propname);
    return;
  }
                              ...
                              const char *panel_type)
{
  ...
  if (but == nullptr) {
    const char *propname = LUXO_prop_identifier(prop).GetText();
                      ...
                      int value)
{
  if (LUXO_prop_type(prop) != PROP_ENUM) {
    const char *propname = LUXO_prop_identifier(prop).GetText();
    ui_item_disabled(layout, propname);
    TF_WARN("property not an enum: %s.%s", LUXO_prim_identifier(ptr->type).GetText(), propname);
    return;
  }
...
void uiItemEnumR_string_prop(uiLayout *layout,
                             ...
                             int icon)
{
  if (UNLIKELY(LUXO_prop_type(prop) != PROP_ENUM)) {
    const char *propname = LUXO_prop_identifier(prop).GetText();
    ui_item_disabled(layout, propname);
    TF_WARN("not an enum property: %s.%s", LUXO_prim_identifier(ptr->type).GetText(), propname);
    return;
  }
  ...
  for (int a = 0; item[a].identifier.data(); a++) {
    ...
    if (item[a].value == ivalue) {
      const char *item_name = name ? name : CTX_IFACE_(KLI_strdup(prop->GetDisplayName().c_str()));
                        ...
                        int icon)
{
  ...
  if (UNLIKELY(prop == nullptr)) {
    ui_item_disabled(layout, propname);
    TF_WARN("enum property not found: %s.%s", LUXO_prim_identifier(ptr->type).GetText(), propname);
    return;
  }
...
void uiItemsEnumR(uiLayout *layout, struct KrakenPRIM *ptr, const char *propname)
{
  ...
  const EnumPROP *item;
  bool free;
  //LUXO_prop_enum_items_gettexted(static_cast<bContext *>(block->evil_C), ptr, prop, &item, &totitem, &free);
...
static void search_id_collection(KrakenPRIM *ptype, KrakenPRIM *r_ptr, KrakenPROP **r_prop)
{
  ...
  *r_prop = nullptr;
  ...
  // LUXO_PRIM_BEGIN(r_ptr, iprop)
  // {
  //   /* if it's a collection and has same pointer type, we've got it */
  //   if (LUXO_prop_type(iprop) == PROP_COLLECTION) {
  //     KrakenPRIM *sprim = LUXO_prop_pointer_type(r_ptr, iprop);
                         ...
                         const bool results_are_suggestions)
{
  ...
  // KrakenPRIM sptr;
  // if (!searchprop) {
  //   if (LUXO_prop_type(prop) == PROP_STRING) {
  //     has_search_fn = (prop->flag != 0);
  //   }
  //   if (LUXO_prop_type(prop) == PROP_POINTER) {
  //     // KrakenPRIM *ptype = LUXO_prop_pointer_type(ptr, prop);
  ...
  //    * else branch below so the button was disabled. Now we have a searchprop, so it can be
  //    enabled
  //    * again. */
                         ...
                         bool results_are_suggestions)
{
  ...
  if (!ELEM(type, PROP_POINTER, PROP_STRING, PROP_ENUM)) {
    TF_WARN("Property %s.%s must be a pointer, string or enum",
            LUXO_prim_identifier(ptr->type).GetText(),
            LUXO_prop_identifier(prop).GetText());
    return;
  }
  if (LUXO_prop_type(searchprop) != PROP_COLLECTION) {
    TF_WARN("search collection property is not a collection type: %s.%s",
            LUXO_prim_identifier(searchptr->type).GetText(),
            LUXO_prop_identifier(searchprop).GetText());
    return;
  }
  ...
  if (icon == ICON_NONE) {
    ...
    if (type == PROP_POINTER) {
      // icontype = LUXO_prop_pointer_type(ptr, prop);
    } else {
      // icontype = LUXO_prop_pointer_type(searchptr, searchprop);
    }
                    ...
                    int icon)
{
  ...
  KrakenPROP *searchprop = LUXO_prim_find_property(searchptr, searchpropname);
  if (!searchprop) {
...
void uiItemDecoratorR_prop(uiLayout *layout, KrakenPRIM *ptr, KrakenPROP *prop, int index)
{
  uiBlock *block = layout->root->block;
...
void uiItemDecoratorR(uiLayout *layout, KrakenPRIM *ptr, const char *propname, int index)
{
  KrakenPROP *prop = nullptr;

  if (ptr && propname) {
    /* validate arguments */
    prop = LUXO_prim_find_property(ptr, propname);
    if (!prop) {
      ui_item_disabled(layout, propname);
      TF_WARN("property not found: %s.%s", LUXO_prim_identifier(ptr->type).GetText(), propname);
      return;
                            ...
                            int icon)
{
  ...
  uiBut *but =
    ui_item_menu(layout, name, icon, ui_item_paneltype_func, pt, nullptr, pt->description, true);
...
static uiBut *uiItemL_(uiLayout *layout, const char *name, int icon)
{
  ...
  if (icon && name[0]) {
    ...
    but =
      uiDefIconBut(block, UI_BTYPE_LABEL, 0, icon, 0, 0, w, UI_UNIT_Y, nullptr, 0.0, 0.0, 0, 0, nullptr);
  } else {
    but = uiDefBut(block, UI_BTYPE_LABEL, 0, name, 0, 0, w, UI_UNIT_Y, nullptr, 0.0, 0.0, 0, 0, nullptr);
  }
...
void uiItemLDrag(uiLayout *layout, KrakenPRIM *ptr, const char *name, int icon)
{
  uiBut *but = uiItemL_(layout, name, icon);
                             ...
                             KrakenPRIM *r_opptr)
{
  ...
  uiBut *but = ui_item_menu(layout, name, icon, menu_item_enum_opname_menu, nullptr, lvl, nullptr, true);
  ...
  if (r_opptr) {
    ...
    KLI_assert(but->opptr->data == nullptr);
                          ...
                          int icon)
{
  ...
  MenuItemLevel *lvl = (MenuItemLevel *)MEM_callocN(sizeof(MenuItemLevel), "MenuItemLevel");
  lvl->stagepoin = *ptr;
  KLI_strncpy(lvl->propname, LUXO_prop_identifier(prop).GetText(), sizeof(lvl->propname));
  lvl->opcontext = layout->root->opcontext;
...
static void ui_litem_estimate_row(uiLayout *litem)
{
  ...
  for (auto &item : litem->items) {
    ui_item_size(&item->item, &itemw, &itemh);

    min_size_flag = min_size_flag && (item->item.flag & UI_ITEM_FIXED_SIZE);
...
static void ui_litem_layout_row(uiLayout *litem)
{
  ...
  do {
    ...
    for (auto &item : litem->items) {
      if (item->item.flag & UI_ITEM_AUTO_FIXED_SIZE) {
        continue;
      }
      ...
      if (w - lastw > 0) {
        neww =
          ui_item_fit(itemw, x, totw, w - lastw, !item->item.next, litem->alignment, &extra_pixel);
      } else {
      ...
      bool min_flag = item->item.flag & UI_ITEM_FIXED_SIZE;
      /* ignore min flag for rows with right or center alignment */
      if (item->item.type != ITEM_BUTTON &&
          ELEM(litem->alignment, UI_LAYOUT_ALIGN_RIGHT, UI_LAYOUT_ALIGN_CENTER) &&
          litem->alignment == UI_LAYOUT_ALIGN_EXPAND && item->item.flag & UI_ITEM_FIXED_SIZE) {
        min_flag = false;
      }

      if ((neww < minw || min_flag) && w != 0) {
        /* fixed size */
        item->item.flag |= UI_ITEM_AUTO_FIXED_SIZE;
        if (item->item.type != ITEM_BUTTON && item->item.flag & UI_ITEM_FIXED_SIZE) {
          minw = itemw;
        }
        fixedw += minw;
        flag = 1;
  ...
  for (auto &item : litem->items) {
    ...
    /* position item */
    ui_item_position(&item->item, x + offset, y - itemh, itemw, itemh);
...
static void ui_litem_estimate_column(uiLayout *litem, bool is_box)
{
  ...
  for (auto &item : litem->items) {
    ...
    if (item->item.next && (!is_box || item != litem->items.front())) {
      litem->h += litem->space;
    }
  }
...
static void ui_litem_layout_column(uiLayout *litem, bool is_box, bool is_menu)
{
  ...
  for (auto &item : litem->items) {
    ...
    y -= itemh;
    ui_item_position(&item->item, x, y, is_menu ? itemw : litem->w, itemh);
    ...
    if (is_box) {
      item->item.flag |= UI_ITEM_BOX_ITEM;
    }
  }
...
static bool ui_item_is_radial_displayable(uiItem *item)
{
  ...
  if ((item->type == ITEM_BUTTON) && (item->type == UI_BTYPE_LABEL)) {
    return false;
  }
...
static void ui_litem_layout_radial(uiLayout *litem)
{
  ...
  for (auto &UNUSED(item) : litem->items) {
    totitems++;
  }
  ...
  for (auto &item : litem->items) {
    ...
    if (ui_item_is_radial_displayable(&item->item)) {
      RadialDirection dir;
      float vec[2];
      float factor[2];
      ...
      if (item->item.type == ITEM_BUTTON) {
        item->but->pie_dir = dir;
        /* scale the buttons */
        item->but->rect.ymax *= 1.5f;
        /* add a little bit more here to include number */
        item->but->rect.xmax += 1.5f * UI_UNIT_X;
        /* enable drawing as pie item if supported by widget */
        if (ui_item_is_radial_drawable(item)) {
          item->but->emboss = UI_EMBOSS_RADIAL;
          item->but->drawflag |= UI_BUT_ICON_LEFT;
        }
      }
...
static void ui_litem_layout_root_radial(uiLayout *litem)
{
  ...
  uiButtonItem *item = litem->items.front();
...
static void ui_litem_layout_column_flow(uiLayout *litem)
{
  ...
  for (auto &item : litem->items) {
    ...
    y -= itemh;
    emy -= itemh;
    ui_item_position(&item->item, x, y, itemw, itemh);
...
static void ui_litem_grid_flow_compute(std::vector<uiButtonItem *> items,
                                       ...
                                       UILayoutGridFlowOutput *results)
{
  ...
  float *avg_w = nullptr, *totweight_w = nullptr;
  int *max_h = nullptr;

  KLI_assert(
    parameters->tot_columns != 0 ||
    (results->cos_x_array == nullptr && results->widths_array == nullptr && results->tot_w == nullptr));
  KLI_assert(
    parameters->tot_rows != 0 ||
    (results->cos_y_array == nullptr && results->heights_array == nullptr && results->tot_h == nullptr));
  ...
  for (auto &item : items) {
    int item_w, item_h;
    ui_item_size(&item->item, &item_w, &item_h);
...
static void ui_litem_estimate_grid_flow(uiLayout *litem)
{
  ...
  /* Estimate average needed width and height per item. */
  {
    ...
    UILayoutGridFlowOutput uiout = {
      .tot_items = &gflow->tot_items,
      .global_avg_w = &avg_w,
      .global_max_h = &max_h,
    };

    ui_litem_grid_flow_compute(litem->items, &uiflow, &uiout);
...
static void ui_litem_layout_grid_flow(uiLayout *litem)
{
  ...
  KLI_assert(gflow->tot_columns > 0);
  KLI_assert(gflow->tot_rows > 0);
  ...
  for (auto &item : litem->items) {
    ...
    ui_item_position(&item->item, cos_x[col], cos_y[row], item_w, item_h);
...
static void ui_litem_estimate_absolute(uiLayout *litem)
{
  ...
  for (auto &item : litem->items) {
    ...
    ui_item_offset(&item->item, &itemx, &itemy);
    ui_item_size(&item->item, &itemw, &itemh);
...
static void ui_litem_layout_absolute(uiLayout *litem)
{
  ...
  for (auto &item : litem->items) {
    ...
    ui_item_position(&item->item, x + itemx - minx, y + itemy - miny, itemw, itemh);
  }
...
static void ui_litem_layout_split(uiLayout *litem)
{
  ...
  for (auto &item : litem->items) {
    int itemh;
    ui_item_size(&item->item, nullptr, &itemh);

    ui_item_position(&item->item, x, y - itemh, colw, itemh);
    x += colw;
...
static void ui_litem_layout_overlap(uiLayout *litem)
{
  ...
  for (auto &item : litem->items) {
    int itemw, itemh;
    ui_item_size(&item->item, &itemw, &itemh);
    ui_item_position(&item->item, x, y - itemh, litem->w, itemh);
...
static uiLayoutItemBx *ui_layout_box(uiLayout *layout, int type)
{
  ...
  box->roundbox = uiDefBut(layout->root->block, type, 0, "", 0, 0, 0, 0, nullptr, 0.0, 0.0, 0, 0, "");
...
void ui_layout_list_set_labels_active(uiLayout *layout)
{
  for (auto &item : layout->items) {
    if (item->item.type != ITEM_BUTTON) {
      ui_layout_list_set_labels_active(item->but->layout);
    } else if (item->but->flag & UI_BUT_LIST_ITEM) {
      UI_but_flag_enable(item->but, UI_SELECT);
    }
  }
}
...
static bool block_search_panel_label_matches(const uiBlock *block, const char *search_string)
{
  if ((block->panel != nullptr) && (block->panel->type != nullptr)) {
    if (KLI_strcasestr(block->panel->type->label, search_string)) {
      return true;
    }
  }
  return false;
...
static bool button_matches_search_filter(uiBut *but, const char *search_filter)
{
  ...
  if (KLI_strcasestr(but->str, search_filter)) {
    return true;
  }

  if (but->optype != nullptr) {
    if (KLI_strcasestr(but->optype->name, search_filter)) {
      return true;
    }
  }
  ...
  if (but->stageprop != nullptr) {
    ...
    if (but->type == UI_BTYPE_MENU) {
      ...
      int items_len;
      const EnumPROP *items_array = nullptr;
      bool free;
      // LUXO_prop_enum_items_gettexted(nullptr, ptr, enum_prop, &items_array, &items_len, &free);
      ...
      for (int i = 0; i < items_len; i++) {
        ...
        if (KLI_strcasestr(items_array[i].name, search_filter)) {
          return true;
        }
      }
      if (free) {
        MEM_freeN((EnumPROP *)items_array);
      }
    }
  }
...
bool UI_block_apply_search_filter(uiBlock *block, const char *search_filter)
{
  ...
  if (panel != nullptr && panel->type->flag & PANEL_TYPE_NO_SEARCH) {
    ...
    KLI_assert(block->panel->type != nullptr);
...
static void ui_item_scale(uiLayout *litem, const float scale[2])
{
  ...
  for (auto &item : litem->items) {
    ...
    ui_item_size(&item->item, &w, &h);
    ui_item_offset(&item->item, &x, &y);
    ...
    ui_item_position(&item->item, x, y, w, h);
  }
}
...
static void ui_item_align(uiLayout *litem, short nr)
{
  for (auto &item : litem->items) {
    ...
    } else if (item->item.type == ITEM_LAYOUT_ABSOLUTE) {
      /* pass */
    } else if (item->item.type == ITEM_LAYOUT_OVERLAP) {
      /* pass */
    } else if (item->item.type == ITEM_LAYOUT_BOX) {
...
static void ui_item_flag(uiLayout *litem, int flag)
{
  for (auto &item : litem->items) {
    if (item->item.type == ITEM_BUTTON) {
      ...
      ui_item_flag(litem, flag);
    }
  }
}

static void ui_item_layout(uiLayout *layout)
{
  if (layout->item.type != ITEM_BUTTON) {
    ...
    if (layout->align) {
      ui_item_align(layout, ++layout->root->block->alignnr);
    }
    if (!layout->active) {
      ui_item_flag(layout, UI_BUT_INACTIVE);
    }
    if (!layout->enabled) {
      ui_item_flag(layout, UI_BUT_DISABLED);
    }

    switch (layout->item.type) {
      case ITEM_LAYOUT_COLUMN:
        ui_litem_layout_column(layout, false, false);
        break;
      case ITEM_LAYOUT_COLUMN_FLOW:
        ui_litem_layout_column_flow(layout);
        break;
      case ITEM_LAYOUT_GRID_FLOW:
        ui_litem_layout_grid_flow(layout);
        break;
      case ITEM_LAYOUT_ROW:
        ui_litem_layout_row(layout);
        break;
      case ITEM_LAYOUT_BOX:
        ui_litem_layout_box(layout);
        break;
      case ITEM_LAYOUT_ROOT:
        ui_litem_layout_root(layout);
        break;
      case ITEM_LAYOUT_ABSOLUTE:
        ui_litem_layout_absolute(layout);
        break;
      case ITEM_LAYOUT_SPLIT:
        ui_litem_layout_split(layout);
        break;
      case ITEM_LAYOUT_OVERLAP:
        ui_litem_layout_overlap(layout);
        break;
      case ITEM_LAYOUT_RADIAL:
        ui_litem_layout_radial(layout);
        break;
      default:
        break;
    }
    ...
    for (auto &subitem : layout->items) {
      if (layout->item.flag & UI_ITEM_BOX_ITEM) {
        subitem->item.flag |= UI_ITEM_BOX_ITEM;
      }
    ...
    if (layout->item.flag & UI_ITEM_BOX_ITEM) {
      for (auto &subitem : layout->items) {
        subitem->but->drawflag |= UI_BUT_BOX_ITEM;
      }
    }
  }
}
                          ...
                          const uiStyle *style)
{
  ...
  root->layout = layout;
  KLI_addtail(&block->layouts, root);
...
void ui_layout_add_but(uiLayout *layout, uiBut *but)
{
  ...
  if (layout->child_items_layout) {
    layout->child_items_layout->items.push_back(bitem);
  } else {
    layout->items.push_back(bitem);
  }
  but->layout = layout;
...
{
  const std::vector<uiButtonItem *> child_list = layout->child_items_layout ?
  ...
  for (auto &item : child_list) {
    if (item->item.type == ITEM_BUTTON) {
      ...
      uiButtonItem *nested_item = ui_layout_find_button_item(layout->child_items_layout, but);
      if (nested_item) {
        return nested_item;
      }
    }
...
void UI_block_layout_resolve(uiBlock *block, int *r_x, int *r_y)
{
  ...
  KLI_listbase_clear(&block->layouts);
...
{
  return !KLI_listbase_is_empty(&block->layouts);
}
                            ...
                            uiFreeArgFunc free_arg)
{
  ...
  for (auto &item : layout->items) {
    ...
    if (item->item.type == ITEM_BUTTON) {
      if (item->but->type == UI_BTYPE_DECORATOR) {
        continue;
      }
      UI_but_func_tooltip_set(item->but, func, arg, free_arg);
    } else {
      uiLayoutSetTooltipFunc(layout, func, arg, copy_arg, free_arg);
    }
  }
...
void uiLayoutSetContextFromBut(uiLayout *layout, uiBut *but)
{
  ...
  if (but->stagepoin.data && but->stageprop) {
    ...
    KrakenPRIM ptr_prop;
    LUXO_pointer_create(nullptr, &PRIM_KrakenPROP, but->stageprop, &ptr_prop);
    uiLayoutSetContextPointer(layout, "button_prop", &ptr_prop);
    uiLayoutSetContextPointer(layout, "button_pointer", &but->stagepoin);
  }
}

wmOperatorType *UI_but_operatortype_get_from_enum_menu(uiBut *but, KrakenPROP **r_prop)
{
  if (r_prop != nullptr) {
    *r_prop = nullptr;
  }
  ...
  if (but->menu_create_func == menu_item_enum_opname_menu) {
    ...
    if ((ot != nullptr) && (r_prop != nullptr)) {
      *r_prop = MEM_new<KrakenPROP>(__func__, ot->prim->GetAttribute(TfToken(lvl->propname)));
    }
    return ot;
  }
  return nullptr;
...
{
  for (auto &subitem : layout->items) {
    if (subitem->item.type == ITEM_BUTTON) {
      if (!(subitem->but->flag & UI_HIDDEN) && STREQ(subitem->but->str, pt->label)) {
        return true;
      }
    } else {
      if (ui_layout_has_panel_label(layout->child_items_layout, pt)) {
        return true;
      }
    }
  }
...
static void ui_paneltype_draw_impl(kContext *C, PanelType *pt, uiLayout *layout, bool show_header)
{
  ...
  uiButtonItem *last_item = layout->items.back();
  ...
  pt->draw(C, panel);
  panel->layout = nullptr;
  KLI_assert(panel->runtime.custom_data_ptr == nullptr);
...
static void ui_layout_introspect_button(DynStr *ds, uiButtonItem *bitem)
{
...

  {
    ...
    if (ot) {
      char *opstr =
        WM_operator_pystring_ex((kContext *)but->block->evil_C, nullptr, false, true, ot, nullptr);
      KLI_dynstr_appendf(ds, "'operator':'''%s''', ", opstr ? opstr : "");
      KLI_dynstr_appendf(ds,
                         "'property':'''%s''', ",
                         prop ? LUXO_prop_identifier(prop).GetText() : "");
      MEM_freeN(opstr);
    }
  }
...
#define CASE_ITEM(id)                   \
  case id: {                            \
    ...
    KLI_dynstr_append(ds, "'type': '"); \
    /* Skip 'ITEM_'. */                 \
    KLI_dynstr_append(ds, id_str + 5);  \
    KLI_dynstr_append(ds, "', ");       \
    break;                              \
  }                                     \
    ((void)0)
...
#undef CASE_ITEM
      ...
      switch (item->item.type) {
        case ITEM_BUTTON:
          ui_layout_introspect_button(ds, item);
          break;
        default:
...
uiLayout *uiItemsAlertBox(uiBlock *block, const int size, const eAlertIcon icon)
{
  ...
  uiLayout *block_layout =
    UI_block_layout(block, UI_LAYOUT_VERTICAL, UI_LAYOUT_PANEL, 0, 0, dialog_width, 0, 0, style);
