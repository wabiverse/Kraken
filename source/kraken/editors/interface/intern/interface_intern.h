...
#define LUXO_NO_INDEX -1
#define LUXO_ENUM_VALUE -2
...
/** Popover width (multiplied by #UI_WIDGET_UNIT) */
...
extern void ui_hsvcircle_vals_from_pos(const rcti *rect,
                                       float mx,
                                       float my,
                                       float *r_val_rad,
                                       float *r_val_dist);
 ...
 */
extern void ui_hsvcircle_pos_from_vals(const ColorPicker *cpicker,
                                       const rcti *rect,
                                       const float *hsv,
                                       float *xpos,
                                       float *ypos);
extern void ui_hsvcube_pos_from_vals(const struct uiButHSVCube *hsv_but,
                                     const rcti *rect,
                                     const float *hsv,
                                     float *xp,
                                     float *yp);
...
extern bool ui_but_string_eval_number(struct kContext *C,
                                      const uiBut *but,
                                      const char *str,
 ...
 */
extern void ui_but_active_string_clear_and_exit(struct kContext *C, uiBut *but) ATTR_NONNULL();
/**
 ...
 */
extern void ui_but_set_string_interactive(struct kContext *C, uiBut *but, const char *value);
...
extern void ui_but_luxo_menu_convert_to_panel_type(struct uiBut *but, const char *panel_type);
extern void ui_but_luxo_menu_convert_to_menu_type(struct uiBut *but, const char *menu_type);
...
extern bool ui_but_is_luxo_valid(uiBut *but) ATTR_WARN_UNUSED_RESULT;
/**
...
struct uiPopupBlockHandle
{
  ...
  bool popup;
  void (*popup_func)(struct kContext *C, void *arg, int event);
  void (*cancel_func)(struct kContext *C, void *arg);
  void *popup_arg;
...
uiBlock *ui_block_func_COLOR(struct kContext *C, uiPopupBlockHandle *handle, void *arg_but);
 ...
 */
struct ARegion *ui_searchbox_create_generic(struct kContext *C,
                                            struct ARegion *butregion,
                                            uiButSearch *search_but);
struct ARegion *ui_searchbox_create_operator(struct kContext *C,
                                             struct ARegion *butregion,
                                             uiButSearch *search_but);
struct ARegion *ui_searchbox_create_menu(struct kContext *C,
                                         struct ARegion *butregion,
                                         uiButSearch *search_but);
 ...
 */
void ui_searchbox_update(struct kContext *C, struct ARegion *region, uiBut *but, bool reset);
int ui_searchbox_autocomplete(struct kContext *C, struct ARegion *region, uiBut *but, char *str);
bool ui_searchbox_event(struct kContext *C,
                        struct ARegion *region,
                        uiBut *but,
                        struct ARegion *butregion,
...
void ui_searchbox_free(struct kContext *C, struct ARegion *region);
/**
 ...
 */
uiBlock *ui_popup_block_refresh(struct kContext *C,
                                uiPopupBlockHandle *handle,
                                struct ARegion *butregion,
                                uiBut *but);
...
uiPopupBlockHandle *ui_popup_block_create(struct kContext *C,
                                          ...
                                          void *arg,
                                          uiFreeArgFunc arg_free);
uiPopupBlockHandle *ui_popup_menu_create(struct kContext *C,
                                         struct ARegion *butregion,
                                         uiBut *but,
...
uiPopupBlockHandle *ui_popover_panel_create(struct kContext *C,
                                            struct ARegion *butregion,
                                            uiBut *but,
                                            uiMenuCreateFunc menu_func,
                                            void *arg);
...
void ui_popup_block_free(struct kContext *C, uiPopupBlockHandle *handle);
...
extern void ui_draw_dropshadow(const rctf *rct,
                               float radius,
                               float aspect,
                               float alpha,
                               int select);
 ...
 */
extern void ui_but_activate_over(struct kContext *C, struct ARegion *region, uiBut *but);
extern void ui_but_execute_begin(struct kContext *C,
                                 struct ARegion *region,
                                 uiBut *but,
                                 void **active_back);
extern void ui_but_execute_end(struct kContext *C,
                               struct ARegion *region,
                               uiBut *but,
                               void *active_back);
extern void ui_but_active_free(const struct kContext *C, uiBut *but);
/**
 ...
 */
extern void ui_but_update_view_for_active(const struct kContext *C, const uiBlock *block);
...
void ui_but_drag_start(struct kContext *C, uiBut *but);
...
void ui_but_anim_copy_driver(struct kContext *C);
void ui_but_anim_paste_driver(struct kContext *C);
/**
 ...
 */
struct ARegion *ui_screen_region_find_mouse_over_ex(struct kScreen *screen, const int xy[2])
  ATTR_NONNULL(1, 2);
struct ARegion *ui_screen_region_find_mouse_over(struct kScreen *screen,
                                                 const struct wmEvent *event);

/* interface_context_menu.c */

bool ui_popup_context_menu_for_button(struct kContext *C, uiBut *but, const struct wmEvent *event);
/**
 * menu to show when right clicking on the panel header
 */
void ui_popup_context_menu_for_panel(struct kContext *C,
                                     struct ARegion *region,
                                     struct Panel *panel);
...
/**
 * For use with #ui_luxo_collection_search_update_fn.
 */
...
} uiLUXOCollectionSearch;
void ui_luxo_collection_search_update_fn(const struct kContext *C,
                                         void *arg,
                                         const char *str,
                                         uiSearchItems *items,
                                         bool is_first);
...
bool ui_jump_to_target_button_poll(struct kContext *C);
...
struct ID *ui_template_id_liboverride_hierarchy_make(struct kContext *C,
                                                     struct Main *bmain,
                                                     struct ID *owner_id,
                                                     struct ID *id,
