...
static struct kThemeState g_theme_state = {
  NULL,
  SPACE_VIEW3D,
  RGN_TYPE_WINDOW,
};
...
const uchar *UI_ThemeGetColorPtr(kTheme *ktheme, int spacetype, int colorid)
{
  ...
  KLI_assert(KLI_findindex(&U.themes, theme_active) != -1);
...
void UI_theme_init_default(void)
{
  ...
  kTheme *ktheme = KLI_findstring(&U.themes, "Default", offsetof(kTheme, name));
  if (ktheme == NULL) {
    ktheme = MEM_callocN(sizeof(kTheme), __func__);
