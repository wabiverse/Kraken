...
static void panel_set_expansion_from_list_data(const kContext *C, Panel *panel);
static int get_panel_real_size_y(const Panel *panel);
static void panel_activate_state(const kContext *C, Panel *panel, const uiHandlePanelState state);
...
static Panel *panel_add_instanced(ARegion *region,
                                  ...
                                  KrakenPRIM *custom_data)
{
  Panel *panel = MEM_cnew<Panel>(__func__);
  panel->type = panel_type;
  KLI_strncpy(panel->panelname, panel_type->idname.data(), sizeof(panel->panelname));
  ...
  KLI_addtail(panels, panel);

  return panel;
}

Panel *UI_panel_add_instanced(const kContext *C,
                              ARegion *region,
                              ListBase *panels,
                              ...
                              KrakenPRIM *custom_data)
{
  ...
  PanelType *panel_type = static_cast<PanelType *>(
    KLI_findstring(&region_type->paneltypes, panel_idname, offsetof(PanelType, idname)));
 ...
 */
static void panel_delete(const kContext *C, ARegion *region, ListBase *panels, Panel *panel)
{
  ...
  KLI_freelistN(&panel->children);

  KLI_remlink(panels, panel);
  if (panel->activedata) {
...
void UI_panels_free_instanced(const kContext *C, ARegion *region)
{
                                ...
                                uiListPanelIDFromDataFunc panel_idname_func)
{
  ...
  if (data == nullptr) {
    ...
    data_len = KLI_listbase_count(data);
  ...
  LISTBASE_FOREACH(Panel *, panel, &region->panels)
  {
    if (panel->type != nullptr && panel->type->flag & PANEL_TYPE_INSTANCED) {
      ...
      if (!STREQ(panel_idname, panel->type->idname.data())) {
        return false;
      }
...
static void reorder_instanced_panel_list(kContext *C, ARegion *region, Panel *drag_panel)
{
    ...
    if (panel->type) {
      if (panel->type->flag & PANEL_TYPE_INSTANCED) {
        if (panel_type_context_poll(region, panel->type, context)) {
          if (panel == drag_panel) {
            KLI_assert(start_index == -1); /* This panel should only appear once. */
  ...
  KLI_assert(start_index != -1); /* The drag panel should definitely be in the list. */
...
static void panel_set_expansion_from_list_data(const kContext *C, Panel *panel)
{
  KLI_assert(panel->type != nullptr);
  KLI_assert(panel->type->flag & PANEL_TYPE_INSTANCED);
 ...
 */
static void region_panels_set_expansion_from_list_data(const kContext *C, ARegion *region)
{
  LISTBASE_FOREACH(Panel *, panel, &region->panels)
  {
 ...
 */
static void set_panels_list_data_expand_flag(const kContext *C, const ARegion *region)
{
  LISTBASE_FOREACH(Panel *, panel, &region->panels)
  {
    PanelType *panel_type = panel->type;
...
static bool panel_custom_data_active_get(const Panel *panel)
{
  ...
  KLI_assert(UI_panel_is_active(panel));
  KLI_assert(panel->type != nullptr);

  if (panel->type->active_property[0] != '\0') {
    KrakenPRIM *ptr = UI_panel_custom_data_get(panel);
    if (ptr != nullptr && ptr->IsValid()) {
      return FormFactory(ptr->GetAttribute(TfToken(panel->type->active_property)));
    }
  }
...
static bool panel_type_context_poll(ARegion *region,
                                    ...
                                    const char *context)
{
  if (!KLI_listbase_is_empty(&region->panels_category)) {
...
{
  const char *idname = pt->idname.data();
...
{
  Panel *panel_last;
  const char *drawname = CTX_IFACE_(pt->label);
  const char *idname = pt->idname.data();
  ...
  if (newpanel) {
    ...
    panel->type = pt;
    KLI_strncpy(panel->panelname, idname, sizeof(panel->panelname));
  ...
  KLI_strncpy(panel->drawname, drawname, sizeof(panel->drawname));
  ...
  for (panel_last = static_cast<Panel *>(lb->first); panel_last; panel_last = panel_last->next) {
    if (panel_last->runtime_flag & PANEL_LAST_ADDED) {
      KLI_remlink(lb, panel);
      KLI_insertlinkafter(lb, panel_last, panel);
      break;
    }
  }
 ...
 */
static void panel_set_expansion_from_search_filter_recursive(const kContext *C,
                                                             Panel *panel,
 ...
 */
static void region_panels_set_expansion_from_search_filter(const kContext *C,
                                                           ARegion *region,
                                                           const bool use_search_closed)
{
  LISTBASE_FOREACH(Panel *, panel, &region->panels)
...
static void panel_remove_invisible_layouts_recursive(Panel *panel, const Panel *parent_panel)
{
  ...
  {
    if (child_panel->runtime_flag & PANEL_ACTIVE) {
      KLI_assert(child_panel->runtime.block != nullptr);
...
static void region_panels_remove_invisible_layouts(ARegion *region)
{
  ...
  {
    if (panel->runtime_flag & PANEL_ACTIVE) {
      KLI_assert(panel->runtime.block != nullptr);
...
void UI_panels_draw(const kContext *C, ARegion *region)
{
                                  ...
                                  uchar r_color[4])
{
  if (!show_background) {
    ...
    const uiWidgetColors *wcol = &ktheme->tui.wcol_menu_back;
                                        ...
                                        const rcti *header_rect)
{
  ...
  const kTheme *ktheme = UI_GetTheme();
  const float aspect = panel->runtime.block->aspect;
  const float radius = (ktheme->tui.panel_roundness * U.widget_unit * 0.5f) / aspect;
                                       ...
                                       const bool region_search_filter_active)
{
  ...
  const int header_height = KLI_rcti_size_y(header_rect);
  ...
  /* Draw collapse icon. */
  {
    const float size_y = KLI_rcti_size_y(&widget_rect);
  ...
  if (!is_subpanel && show_background) {
    ...
    GPUBatch *batch = GPU_batch_preset_panel_drag_widget(U.pixelsize,
                                                         color_high,
                                                         color_dark,
                                                         drag_widget_size);
                                        ...
                                        const rcti *header_rect)
{
  ...
  if (is_open || panel->type->flag & PANEL_TYPE_NO_HEADER) {
    ...
    // UI_draw_roundbox_corner_set(is_open ? UI_CNR_BOTTOM_RIGHT | UI_CNR_BOTTOM_LEFT :
    // UI_CNR_ALL);
...
void UI_panel_category_draw_all(ARegion *region, const char *category_id_active)
{
  ...
  kTheme *ktheme = UI_GetTheme();
  const float tab_curve_radius = ktheme->tui.wcol_tab.roundness * U.widget_unit * zoom;
  ...
  KRF_enable(fontid, KRF_ROTATION);
  KRF_rotation(fontid, M_PI_2);
  ui_fontscale(&fstyle_points, aspect);
  KRF_size(fontid, fstyle_points * U.dpi_fac);
  ...
  if ((1 << region->regiontype) & RGN_TYPE_HAS_CATEGORY_MASK) {
    KLI_assert(UI_panel_category_is_visible(region));
  }
  ...
  //   const int category_width = KRF_width(fontid, category_id_draw, KRF_DRAW_STR_DUMMY_MAX);
  ...
  if (y_ofs > KLI_rcti_size_y(&v2d->mask)) {
    scaletabs = float(KLI_rcti_size_y(&v2d->mask)) / float(y_ofs);
  ...
  //     const char *category_id = pc_dyn->idname;
  //     const char *category_id_draw = IFACE_(category_id);
  //     const int category_width = KLI_rcti_size_y(rct) - (tab_v_pad_text * 2);
  //     size_t category_draw_len = KRF_DRAW_STR_DUMMY_MAX;
  // #if 0
  //     int category_width = KRF_width(fontid, category_id_draw, KRF_DRAW_STR_DUMMY_MAX);
  // #endif
  ...
  //       pos =
  //         GPU_vertformat_attr_add(immVertexFormat(), "pos", GPU_COMP_I32, 2,
  ...
  //     if (do_scaletabs) {
  //       category_draw_len =
  //         KRF_width_to_strlen(fontid, category_id_draw, category_draw_len, category_width,
  //         nullptr);
  //     }

  //     KRF_position(fontid, rct->xmax - text_v_ofs, rct->ymin + tab_v_pad_text, 0.0f);
  //     KRF_color3ubv(fontid, is_active ? theme_col_text_hi : theme_col_text);
  //     KRF_draw(fontid, category_id_draw, category_draw_len);
  ...
  KRF_disable(fontid, KRF_ROTATION);
}
...
static void ui_do_animate(kContext *C, Panel *panel)
{
  uiHandlePanelData *data = static_cast<uiHandlePanelData *>(panel->activedata);
...
void UI_panels_begin(const kContext * /*C*/, ARegion *region)
{
...
void UI_panels_end(const kContext *C, ARegion *region, int *r_x, int *r_y)
{
...
static void ui_do_drag(const kContext *C, const wmEvent *event, Panel *panel)
{
  uiHandlePanelData *data = static_cast<uiHandlePanelData *>(panel->activedata);
  ARegion *region = CTX_wm_region(C);
  ...
  const int y = clamp_i(event->mouse_pos[1], coords[0], coords[1] + DRAG_REGION_PAD);
  ...
  dy *= KLI_rctf_size_y(&region->v2d.cur) / float(KLI_rcti_size_y(&from_gf));
...
static void ui_panel_drag_collapse_handler_remove(kContext * /*C*/, void *userdata)
{
  uiPanelDragCollapseHandle *dragcol_data = static_cast<uiPanelDragCollapseHandle *>(userdata);
  MEM_freeN(dragcol_data);
}

static void ui_panel_drag_collapse(const kContext *C,
                                   ...
                                   const int xy_dst[2])
{
  ...
  for (auto &block : region->uiblocks) {
    ...
    if (KLI_rctf_isect_segment(&rect, xy_a_block, xy_b_block)) {
 ...
 */
static int ui_panel_drag_collapse_handler(kContext *C, const wmEvent *event, void *userdata)
{
  ...
  switch (event->type) {
    case MOUSEMOVE:
      ui_panel_drag_collapse(C, dragcol_data, event->mouse_pos);
...
static void ui_panel_drag_collapse_handler_add(const kContext *C, const bool was_open)
{
  wmWindow *win = CTX_wm_window(C);
  ...
  copy_v2_v2_int(dragcol_data->xy_init, event->mouse_pos);

  std::vector<wmEventHandlerUI *> handlers;
  LISTBASE_FOREACH(wmEventHandlerUI *, eh, &win->modalhandlers)
  {
 ...
 */
static void ui_handle_panel_header(const kContext *C,
                                   const uiBlock *block,
                                   const int mx,
                                   ...
                                   const bool shift)
{
  ...
  KLI_assert(panel->type != nullptr);
  KLI_assert(!(panel->type->flag & PANEL_TYPE_NO_HEADER));
  ...
  if (ELEM(event_type, EVT_RETKEY, EVT_PADENTER, EVT_AKEY) || mx < expansion_area_xmax) {
    if (ctrl && !is_subpanel) {
      ...
      if (UI_panel_is_closed(panel) || KLI_listbase_is_empty(&panel->children)) {
        ...
        KLI_assert(first_child != nullptr);
...
PanelCategoryDyn *UI_panel_category_find(const ARegion *region, const char *idname)
{
  ...
  //   KLI_findstring(&region->panels_category, idname, offsetof(PanelCategoryDyn, idname)));
}

PanelCategoryStack *UI_panel_category_active_find(ARegion *region, const char *idname)
{
  return nullptr;
  // return static_cast<PanelCategoryStack *>(
  // KLI_findstring(&region->panels_category_active, idname, offsetof(PanelCategoryStack,
  // idname)));
}
...
static void ui_panel_category_active_set(ARegion *region, const char *idname, bool fallback)
{
  ...
  //     pc_act_next = pc_act->next;
  //     if (!KLI_findstring(&region->type->paneltypes,
  //                         pc_act->idname,
  //                         offsetof(PanelType, category))) {
  //       KLI_remlink(lb, pc_act);
  //       MEM_freeN(pc_act);
  //     }
  //   }
...
static PanelCategoryDyn *panel_categories_find_mouse_over(ARegion *region, const wmEvent *event)
{
  ...
  // {
  //   if (KLI_rcti_isect_pt(&ptd->rect, event->mval[0], event->mval[1])) {
  //     return ptd;
  //   }
  // }
...
void UI_panel_category_add(ARegion *region, const char *name)
{
  ...
  // KLI_addtail(&region->panels_category, pc_dyn);

  // KLI_strncpy(pc_dyn->idname, name, sizeof(pc_dyn->idname));
...
int ui_handler_panel_region(kContext *C,
                            const wmEvent *event,
                            ARegion *region,
                            const uiBut *active_but)
{
  ...
  if (UI_view2d_mouse_in_scrollers(region, &region->v2d, event->mouse_pos)) {
    return WM_UI_HANDLER_CONTINUE;
  }
...
static void ui_panel_custom_data_set_recursive(Panel *panel, KrakenPRIM *custom_data)
{
  panel->runtime.custom_data_ptr = custom_data;
...
void UI_panel_context_pointer_set(Panel *panel, const char *name, KrakenPRIM *ptr)
{
  uiLayoutSetContextPointer(panel->layout, name, ptr);
  panel->runtime.context = uiLayoutGetContextStore(panel->layout);
}

void UI_panel_custom_data_set(Panel *panel, KrakenPRIM *custom_data)
{
  KLI_assert(panel->type != nullptr);
...
KrakenPRIM *UI_panel_custom_data_get(const Panel *panel)
{
  return panel->runtime.custom_data_ptr;
}

KrakenPRIM *UI_region_panel_custom_data_under_cursor(const kContext *C, const wmEvent *event)
{
  ARegion *region = CTX_wm_region(C);
...
static int ui_handler_panel(kContext *C, const wmEvent *event, void *userdata)
{
...
static void ui_handler_remove_panel(kContext *C, void *userdata)
{
  Panel *panel = static_cast<Panel *>(userdata);
...
static void panel_handle_data_ensure(const kContext *C,
                                     wmWindow *win,
                                     ...
                                     const uiHandlePanelState state)
{
  if (panel->activedata == nullptr) {
    ...
    WM_event_add_ui_handler(C, handlers, ui_handler_panel, ui_handler_remove_panel, panel, 0);
  }
  ...
  data->state = state;
  data->startx = win->eventstate->mouse_pos[0];
  data->starty = win->eventstate->mouse_pos[1];
...
static void panel_activate_state(const kContext *C, Panel *panel, const uiHandlePanelState state)
{
  ...
  if (state == PANEL_STATE_DRAG) {
    ...
    wmOperatorType *ot = WM_operatortype_find(TfToken("VIEW2D_OT_edge_pan"));
