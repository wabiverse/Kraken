...
/* be sure to keep 'kThemeState' in sync */
static kThemeState g_theme_state = {
  nullptr,
  SPACE_VIEW3D,
  RGN_TYPE_WINDOW,
};
...
static bool ED_region_is_overlap(int spacetype, int regiontype)
{
...
const uchar *UI_ThemeGetColorPtr(kTheme *ktheme, int spacetype, int colorid)
{
  ...
  /* ensure we're not getting a color after running KKE_kraken_userdef_free */
  KLI_assert(KLI_findindex(&U.themes, g_theme_state.theme) != -1);
  KLI_assert(colorid != TH_UNDEFINED);
    ...
    if (colorid < TH_THEMEUI) {
      ...
      switch (spacetype) {
        case SPACE_PROPERTIES:
          ts = &ktheme->space_properties;
          break;
        case SPACE_VIEW3D:
          ts = &ktheme->space_view3d;
          break;
        case SPACE_GRAPH:
          ts = &ktheme->space_graph;
          break;
        case SPACE_FILE:
          ts = &ktheme->space_file;
          break;
        case SPACE_NLA:
          ts = &ktheme->space_nla;
          break;
        case SPACE_ACTION:
          ts = &ktheme->space_action;
          break;
        case SPACE_SEQ:
          ts = &ktheme->space_sequencer;
          break;
        case SPACE_IMAGE:
          ts = &ktheme->space_image;
          break;
        case SPACE_TEXT:
          ts = &ktheme->space_text;
          break;
        case SPACE_OUTLINER:
          ts = &ktheme->space_outliner;
          break;
        case SPACE_INFO:
          ts = &ktheme->space_info;
          break;
        case SPACE_USERPREF:
          ts = &ktheme->space_preferences;
          break;
        case SPACE_CONSOLE:
          ts = &ktheme->space_console;
          break;
        case SPACE_NODE:
          ts = &ktheme->space_node;
          break;
        case SPACE_CLIP:
          ts = &ktheme->space_clip;
          break;
        case SPACE_TOPBAR:
          ts = &ktheme->space_topbar;
          break;
        case SPACE_STATUSBAR:
          ts = &ktheme->space_statusbar;
          break;
        case SPACE_SPREADSHEET:
          ts = &ktheme->space_spreadsheet;
          break;
        default:
          ts = &ktheme->space_view3d;
          break;
      ...
      switch (colorid) {
        ...
        case TH_WIDGET_EMBOSS:
          cp = ktheme->tui.widget_emboss;
          break;

        case TH_EDITOR_OUTLINE:
          cp = ktheme->tui.editor_outline;
          break;
        case TH_WIDGET_TEXT_CURSOR:
          cp = ktheme->tui.widget_text_cursor;
          break;
        case TH_WIDGET_TEXT_SELECTION:
          cp = ktheme->tui.wcol_text.item;
          break;
        case TH_WIDGET_TEXT_HIGHLIGHT:
          cp = ktheme->tui.wcol_text.text_sel;
          break;

        case TH_TRANSPARENT_CHECKER_PRIMARY:
          cp = ktheme->tui.transparent_checker_primary;
          break;
        case TH_TRANSPARENT_CHECKER_SECONDARY:
          cp = ktheme->tui.transparent_checker_secondary;
          break;
        case TH_TRANSPARENT_CHECKER_SIZE:
          cp = &ktheme->tui.transparent_checker_size;
          break;
        ...
        case TH_GIZMO_HI:
          cp = ktheme->tui.gizmo_hi;
          break;
        case TH_GIZMO_PRIMARY:
          cp = ktheme->tui.gizmo_primary;
          break;
        case TH_GIZMO_SECONDARY:
          cp = ktheme->tui.gizmo_secondary;
          break;
        case TH_GIZMO_VIEW_ALIGN:
          cp = ktheme->tui.gizmo_view_align;
          break;
        case TH_GIZMO_A:
          cp = ktheme->tui.gizmo_a;
          break;
        case TH_GIZMO_B:
          cp = ktheme->tui.gizmo_b;
          break;

        case TH_ICON_SCENE:
          cp = ktheme->tui.icon_scene;
          break;
        case TH_ICON_COLLECTION:
          cp = ktheme->tui.icon_collection;
          break;
        case TH_ICON_OBJECT:
          cp = ktheme->tui.icon_object;
          break;
        case TH_ICON_OBJECT_DATA:
          cp = ktheme->tui.icon_object_data;
          break;
        case TH_ICON_MODIFIER:
          cp = ktheme->tui.icon_modifier;
          break;
        case TH_ICON_SHADING:
          cp = ktheme->tui.icon_shading;
          break;
        case TH_ICON_FOLDER:
          cp = ktheme->tui.icon_folder;
          break;
        case TH_ICON_FUND: {
        ...
        case TH_SCROLL_TEXT:
          cp = ktheme->tui.wcol_scroll.text;
          break;
...
void UI_theme_init_default()
{
  ...
  kTheme *ktheme = static_cast<kTheme *>(KLI_findstring(&U.themes, "Default", offsetof(kTheme, name)));
  if (ktheme == nullptr) {
    ktheme = MEM_cnew<kTheme>(__func__);
    KLI_addtail(&U.themes, ktheme);
  }

  UI_SetTheme(0, 0); /* make sure the global used in this file is set */

  const int active_theme_area = ktheme->active_theme_area;
  memcpy(ktheme, &U_theme_default, sizeof(*ktheme));
  ktheme->active_theme_area = active_theme_area;
}
...
void UI_SetTheme(int spacetype, int regionid)
{
  if (spacetype) {
    ...
    g_theme_state.theme = static_cast<kTheme *>(U.themes.first);
...
{
  return static_cast<kTheme *>(U.themes.first);
}

void UI_Theme_Store(struct kThemeState *theme_state)
{
  *theme_state = g_theme_state;
}
void UI_Theme_Restore(struct kThemeState *theme_state)
{
...
bool UI_GetIconThemeColor4ubv(int colorid, uchar col[4])
{
  ...
  } else if (!((g_theme_state.spacetype == SPACE_OUTLINER && g_theme_state.regionid == RGN_TYPE_WINDOW) ||
               (g_theme_state.spacetype == SPACE_PROPERTIES && g_theme_state.regionid == RGN_TYPE_NAV_BAR) ||
...
void UI_GetColorPtrBlendShade3ubv(const uchar cp1[3],
                                  const uchar cp2[3],
                                  uchar col[3],
                                  float fac,
                                  int offset)
...
int UI_ThemeMenuShadowWidth(void)
{
  ...
  return (int)(ktheme->tui.menu_shadow_width * U.dpi_fac);
}
