...
typedef struct IDTypeInfo
{
   ...
   * `KKE_lib_id_make_local_generic()` is enough.
   */
  IDTypeMakeLocalFunction make_local;

  /**
   * Called by `KKE_library_foreach_ID_link()` to apply a callback over all other ID usages (ID
...
/* Those are defined in the respective KKE files. */
...
/* General helpers. */
const struct IDTypeInfo *KKE_idtype_get_info_from_idcode(short id_code);
const struct IDTypeInfo *KKE_idtype_get_info_from_id(const struct ID *id);
...
#define KKE_idtype_idcode_is_localizable KKE_idtype_idcode_is_linkable
 ...
 */
uint64_t KKE_idtype_idcode_to_idfilter(short idcode);
/**
 * Convert an \a idfilter into an \a idcode (e.g. #FILTER_ID_OB -> #ID_OB).
 */
short KKE_idtype_idcode_from_idfilter(uint64_t idfilter);

/**
 * Convert an \a idcode into an index (e.g. #ID_OB -> #INDEX_ID_OB).
 */
int KKE_idtype_idcode_to_index(short idcode);
/**
 * Get an \a idcode from an index (e.g. #INDEX_ID_OB -> #ID_OB).
 */
short KKE_idtype_idcode_from_index(int index);
 ...
 * XXX Ideally those would rather belong to #KKE_lib_id, but using callback function pointers makes
