...
namespace kraken::kke::cryptomatte
{
   ...
   *
   * Cryptomatte passes are formatted with a trailing number for storing multiple samples that
   * belong to the same cryptomatte layer. This function would remove the trailing numbers to
   * determine the cryptomatte layer name.
   *
   * # Example
   *
   * A render_pass_name could be 'View Layer.CryptoMaterial02'. The cryptomatte layer would be
   * 'View Layer.CryptoMaterial'.
   *
  ...
  struct CryptomatteHash
  {
    ...
    {
      hash = KLI_hash_mm3((const unsigned char *)name, name_len, 0);
    }

    static CryptomatteHash from_hex_encoded(kraken::StringRef hex_encoded);
  ...
  {
    kraken::Map<std::string, CryptomatteHash> hashes;

#ifdef WITH_CXX_GUARDEDALLOC
    MEM_CXX_CLASS_ALLOC_FUNCS("cryptomatte:CryptomatteLayer")
#endif

    static std::unique_ptr<CryptomatteLayer> read_from_manifest(kraken::StringRefNull manifest);
    uint32_t add_ID(const struct ID &id);
    void add_hash(kraken::StringRef name, CryptomatteHash cryptomatte_hash);
  ...
  struct CryptomatteStampDataCallbackData
  {
    ...
    kraken::Map<std::string, std::string> hash_to_layer_name;
     ...
     */
    static kraken::StringRef extract_layer_hash(kraken::StringRefNull key);
  ...
  CryptomatteLayer *KKE_cryptomatte_layer_get(CryptomatteSession &session,
