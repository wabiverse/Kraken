...
namespace AnchorGraphEditor
{
  ...
  struct Options
  {
    ...
    AnchorU32 mNodeBorderColor{
      ANCHOR_COL32(100, 100, 100, 0)};  // node border color when it's not selected
    ...
    AnchorU32 mDefaultSlotColor{
      ANCHOR_COL32(128,
                   128,
                   128,
                   255)};  // when no color is provided in node template, use this value
    AnchorU32 mFrameFocus{
      ANCHOR_COL32(64, 128, 255, 255)};    // rectangle border when graph editor has focus
  ...
  struct Delegate
  {
    ...
    virtual void CustomDraw(AnchorDrawList *drawList,
                            AnchorBBox rectangle,
