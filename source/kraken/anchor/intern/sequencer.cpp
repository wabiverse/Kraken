...
bool Sequencer(SequenceInterface *sequence, int *currentFrame, bool *expanded, int *selectedEntry, int *firstFrame, int sequenceOptions)
{
  ...
  if (expanded && !*expanded)
  {
    ...
    draw_list->AddRectFilled(canvas_pos, GfVec2f(canvas_size[0] + canvas_pos[0], canvas_pos[1] + ItemHeight), 0xFF3D3837, 0);
    char tmps[512];
  ...
  else
  {
    ...
    GfVec2f childFrameSize(canvas_size[0], canvas_size[1] - 8.f - headerSize[1] - (hasScrollBar ? scrollBarSize[1] : 0));
    ...
    // current frame top
    AnchorBBox topRect(GfVec2f(canvas_pos[0] + legendWidth, canvas_pos[1]), GfVec2f(canvas_pos[0] + canvas_size[0], canvas_pos[1] + ItemHeight));
    ...
    {
      if (SequencerAddDelButton(draw_list, GfVec2f(canvas_pos[0] + legendWidth - ItemHeight, canvas_pos[1] + 2), true) && io.MouseReleased[0])
    ...
    auto drawLine = [&](int i, int regionHeight) {
      ...
      int px = (int)canvas_pos[0] + int(i * framePixelWidth) + legendWidth - int(firstFrameUsed * framePixelWidth);
      int tiretStart = baseIndex ? 4 : (halfIndex ? 10 : 14);
      int tiretEnd = baseIndex ? regionHeight : ItemHeight;

      if (px <= (canvas_size[0] + canvas_pos[0]) && px >= (canvas_pos[0] + legendWidth))
      {
        draw_list->AddLine(GfVec2f((float)px, canvas_pos[1] + (float)tiretStart), GfVec2f((float)px, canvas_pos[1] + (float)tiretEnd - 1), 0xFF606060, 1);

        draw_list->AddLine(GfVec2f((float)px, canvas_pos[1] + (float)ItemHeight), GfVec2f((float)px, canvas_pos[1] + (float)regionHeight - 1), 0x30606060, 1);
      }
    ...
    /*
                  draw_list->AddLine(canvas_pos, GfVec2f(canvas_pos[0], canvas_pos[1] + controlHeight), 0xFF000000, 1);
                  draw_list->AddLine(GfVec2f(canvas_pos[0], canvas_pos[1] + ItemHeight), GfVec2f(canvas_size[0], canvas_pos[1] + ItemHeight), 0xFF000000, 1);
                  */
    // clip content
    ...
    for (int i = 0; i < sequenceCount; i++)
    {
      ...
      {
        bool overDel = SequencerAddDelButton(draw_list, GfVec2f(contentMin[0] + legendWidth - ItemHeight + 2 - 10, tpos[1] + 2), false);
        if (overDel && io.MouseReleased[0])
          delEntry = i;

        bool overDup = SequencerAddDelButton(draw_list, GfVec2f(contentMin[0] + legendWidth - ItemHeight - ItemHeight + 2 - 10, tpos[1] + 2), true);
    ...
    for (int i = 0; i < sequenceCount; i++)
    {
      ...
      GfVec2f pos = GfVec2f(contentMin[0] + legendWidth, contentMin[1] + ItemHeight * i + 1 + customHeight);
      GfVec2f sz = GfVec2f(canvas_size[0] + canvas_pos[0], pos[1] + ItemHeight - 1 + localCustomHeight);
      if (!popupOpened && cy >= pos[1] && cy < pos[1] + (ItemHeight + localCustomHeight) && movingEntry == -1 && cx > contentMin[0] && cx < contentMin[0] + canvas_size[0])
      {
        col += 0x80201008;
        pos[0] -= legendWidth;
      }
    ...
    if (selected)
    {
      ...
      for (int i = 0; i < *selectedEntry; i++)
        customHeight += sequence->GetCustomHeight(i);
      ;
      draw_list->AddRectFilled(GfVec2f(contentMin[0], contentMin[1] + ItemHeight * *selectedEntry + customHeight), GfVec2f(contentMin[0] + canvas_size[0], contentMin[1] + ItemHeight * (*selectedEntry + 1) + customHeight), 0x801080FF, 1.f);
    }
    ...
    for (int i = 0; i < sequenceCount; i++)
    {
      ...
      GfVec2f pos = GfVec2f(contentMin[0] + legendWidth - firstFrameUsed * framePixelWidth, contentMin[1] + ItemHeight * i + 1 + customHeight);
      ...
      GfVec2f slotP3(pos[0] + *end * framePixelWidth + framePixelWidth, pos[1] + ItemHeight - 2 + localCustomHeight);
      ...
      AnchorBBox rects[3] = {AnchorBBox(slotP1, GfVec2f(slotP1[0] + framePixelWidth / 2, slotP2[1])), AnchorBBox(GfVec2f(slotP2[0] - framePixelWidth / 2, slotP1[1]), slotP2), AnchorBBox(slotP1, slotP2)};
      ...
      if (localCustomHeight > 0)
      {
        ...
        AnchorBBox customRect(rp + GfVec2f(legendWidth - (firstFrameUsed - sequence->GetFrameMin() - 0.5f) * framePixelWidth, float(ItemHeight)),
                              rp + GfVec2f(legendWidth + (sequence->GetFrameMax() - firstFrameUsed - 0.5f + 2.f) * framePixelWidth, float(localCustomHeight + ItemHeight)));
        AnchorBBox clippingRect(rp + GfVec2f(float(legendWidth), float(ItemHeight)), rp + GfVec2f(canvas_size[0], float(localCustomHeight + ItemHeight)));

        AnchorBBox legendRect(rp + GfVec2f(0.f, float(ItemHeight)), rp + GfVec2f(float(legendWidth), float(localCustomHeight)));
        AnchorBBox legendClippingRect(canvas_pos + GfVec2f(0.f, float(ItemHeight)), canvas_pos + GfVec2f(float(legendWidth), float(localCustomHeight + ItemHeight)));
      ...
      else
      {
        ...
        AnchorBBox customRect(rp + GfVec2f(legendWidth - (firstFrameUsed - sequence->GetFrameMin() - 0.5f) * framePixelWidth, float(0.f)),
                              rp + GfVec2f(legendWidth + (sequence->GetFrameMax() - firstFrameUsed - 0.5f + 2.f) * framePixelWidth, float(ItemHeight)));
        AnchorBBox clippingRect(rp + GfVec2f(float(legendWidth), float(0.f)), rp + GfVec2f(canvas_size[0], float(ItemHeight)));
    ...
    if (currentFrame && firstFrame && *currentFrame >= *firstFrame && *currentFrame <= sequence->GetFrameMax())
    {
      ...
      float cursorOffset = contentMin[0] + legendWidth + (*currentFrame - firstFrameUsed) * framePixelWidth + framePixelWidth / 2 - cursorWidth * 0.5f;
      draw_list->AddLine(GfVec2f(cursorOffset, canvas_pos[1]), GfVec2f(cursorOffset, contentMax[1]), 0xA02A2AFF, cursorWidth);
      char tmps[512];
    ...
    {
      AnchorBBox rectCopy(GfVec2f(contentMin[0] + 100, canvas_pos[1] + 2), GfVec2f(contentMin[0] + 100 + 30, canvas_pos[1] + ItemHeight - 2));
      bool inRectCopy = rectCopy.Contains(io.MousePos);
      unsigned int copyColor = inRectCopy ? 0xFF1080FF : 0xFF000000;
      draw_list->AddText(rectCopy.Min, copyColor, "Copy");

      AnchorBBox rectPaste(GfVec2f(contentMin[0] + 140, canvas_pos[1] + 2), GfVec2f(contentMin[0] + 140 + 30, canvas_pos[1] + ItemHeight - 2));
    ...
    if (hasScrollBar)
    {
      ...
      float startFrameOffset = ((float)(firstFrameUsed - sequence->GetFrameMin()) / (float)frameCount) * (canvas_size[0] - legendWidth);
      ...
      GfVec2f scrollBarD(scrollBarMin[0] + legendWidth + barWidthInPixels + startFrameOffset, scrollBarMax[1] - 2);
      ...
      if (sizingRBar)
      {
        ...
        else
        {
          ...
          {
            framePixelWidthTarget = framePixelWidth = (canvas_size[0] - legendWidth) / float(sequence->GetFrameMax() - *firstFrame);
          }
        }
      }
      else if (sizingLBar)
      ...
      else
      {
        if (MovingScrollBar)
        {
          ...
          else
          {
            ...
            *firstFrame = int((io.MousePos[0] - panningViewSource[0]) / framesPerPixelInBar) - panningViewFrame;
  ...
  if (expanded)
  {
    bool overExpanded = SequencerAddDelButton(draw_list, GfVec2f(canvas_pos[0] + 2, canvas_pos[1] + 2), !*expanded);
