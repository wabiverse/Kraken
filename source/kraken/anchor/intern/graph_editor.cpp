...
static GfVec2f GetInputSlotPos(Delegate &delegate, const Node &node, SlotIndex slotIndex, float factor)
{
  ...
  return GfVec2f(node.mRect.Min[0] * factor,
                 node.mRect.Min[1] * factor + Size[1] * ((float)slotIndex + 1) / ((float)InputsCount + 1) + 8.f);
}
...
static GfVec2f GetOutputSlotPos(Delegate &delegate, const Node &node, SlotIndex slotIndex, float factor)
{
  ...
  return GfVec2f(node.mRect.Min[0] * factor + Size[0],
                 node.mRect.Min[1] * factor + Size[1] * ((float)slotIndex + 1) / ((float)OutputsCount + 1) + 8.f);
}
...
static void FitNodes(Delegate &delegate, ViewState &viewState, const GfVec2f viewSize, bool selectedNodesOnly)
{
  ...
  viewState.mPosition = GfVec2f(-nodeCenter[0], -nodeCenter[1]) + (viewSize * 0.5f) / viewState.mFactorTarget;
}
...
static void HandleQuadSelection(Delegate &delegate, AnchorDrawList *drawList, const GfVec2f offset, const float factor, AnchorBBox contentRect, const Options &options)
{
                              ...
                              const bool inMinimap)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    for (SlotIndex slotIndex = 0; slotIndex < slotCount[i]; slotIndex++)
    {
      ...
      GfVec2f textPos =
        p + GfVec2f(-options.mNodeSlotRadius * (i ? -1.f : 1.f) * (overCon ? 3.f : 2.f) - (i ? 0 : textSize[0]),
                    -textSize[1] / 2);
      ...
      else
      {
        ...
        {
          drawList->AddText(io.FontDefault, 14, textPos + GfVec2f(2, 2), ANCHOR_COL32(0, 0, 0, 255), conText);
    ...
    if (closestConn != -1)
    {
      ...
      hoverSlot = true;
      drawList->AddCircleFilled(closestPos, options.mNodeSlotRadius * options.mNodeSlotHoverFactor * 0.75f, ANCHOR_COL32(0, 0, 0, 200));
      drawList->AddCircleFilled(closestPos, options.mNodeSlotRadius * options.mNodeSlotHoverFactor, slotColor);
      drawList->AddText(io.FontDefault, 16, closestTextPos + GfVec2f(1, 1), ANCHOR_COL32(0, 0, 0, 255), conText);
...
static void DrawGrid(AnchorDrawList *drawList, GfVec2f windowPos, const ViewState &viewState, const GfVec2f canvasSize, AnchorU32 gridColor, AnchorU32 gridColor2, float gridSize)
{
  ...
  for (float x = fmodf(viewState.mPosition[0] * viewState.mFactor, gridSpace); x < canvasSize[0]; x += gridSpace, divx++)
  {
    bool tenth = !(divx % 10);
    drawList->AddLine(GfVec2f(x, 0.0f) + windowPos, GfVec2f(x, canvasSize[1]) + windowPos, tenth ? gridColor2 : gridColor);
  }
  for (float y = fmodf(viewState.mPosition[1] * viewState.mFactor, gridSpace); y < canvasSize[1]; y += gridSpace, divy++)
  {
    bool tenth = !(divy % 10);
    drawList->AddLine(GfVec2f(0.0f, y) + windowPos, GfVec2f(canvasSize[0], y) + windowPos, tenth ? gridColor2 : gridColor);
  }
}
                     ...
                     const AnchorBBox &viewPort)
{
  ...
  AnchorBBox customDrawRect(nodeRectangleMin + GfVec2f(options.mRounding, 20 + options.mRounding), nodeRectangleMax - GfVec2f(options.mRounding, options.mRounding));
...
bool DrawMiniMap(AnchorDrawList *drawList, Delegate &delegate, ViewState &viewState, const Options &options, const GfVec2f windowPos, const GfVec2f canvasSize)
{
  ...
  drawList->AddRectFilled(minScreen, maxScreen, ANCHOR_COL32(30, 30, 30, 200), 3, AnchorDrawFlags_RoundCornersAll);
  ...
  drawList->AddRectFilled(viewMinScreen, viewMaxScreen, ANCHOR_COL32(255, 255, 255, 32), 1, AnchorDrawFlags_RoundCornersAll);
  drawList->AddRect(viewMinScreen, viewMaxScreen, ANCHOR_COL32(255, 255, 255, 128), 1, AnchorDrawFlags_RoundCornersAll);
  ...
  if (mouseInMinimap && io.MouseClicked[0])
  {
    ...
    const GfVec2f worldPosCenter = GfVec2f(AnchorLerp(mapmin[0], mapmax[0], clickedRatio[0]), AnchorLerp(mapmin[1], mapmax[1], clickedRatio[1]));
