...
namespace AnchorGizmo
{
  ...
  static const AnchorU32 directionColor[3] = {ANCHOR_COL32(0xAA, 0, 0, 0xFF),
                                              ANCHOR_COL32(0, 0xAA, 0, 0xFF),
                                              ANCHOR_COL32(0, 0, 0xAA, 0XFF)};

  // Alpha: 100%: FF, 87%: DE, 70%: B3, 54%: 8A, 50%: 80, 38%: 61, 12%: 1F
  static const AnchorU32 planeColor[3] = {ANCHOR_COL32(0xAA, 0, 0, 0x61),
                                          ANCHOR_COL32(0, 0xAA, 0, 0x61),
                                          ANCHOR_COL32(0, 0, 0xAA, 0x61)};
                                       ...
                                       OPERATION operation)
  {
    ...
    for (unsigned int axisIndex = 0; axisIndex < numAxes; ++axisIndex) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        bool overSmallAnchor = AnchorLengthSqr(midBound - io.MousePos) <=
                               (AnchorBigRadius * AnchorBigRadius);
  ...
  static int GetRotateType(OPERATION op)
  {
    ...
    vec_t deltaScreen = {io.MousePos[0] - gContext.mScreenSquareCenter[0],
                         io.MousePos[1] - gContext.mScreenSquareCenter[1],
                         0.f,
                         0.f};
  ...
  static bool HandleTranslation(float *matrix,
                                float *deltaMatrix,
                                OPERATION op,
                                int &type,
                                const float *snap)
