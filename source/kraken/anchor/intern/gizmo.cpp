...
static GfVec2f worldToPos(const vec_t &worldPos, const matrix_t &mat, GfVec2f position = GfVec2f(gContext.mX, gContext.mY), GfVec2f size = GfVec2f(gContext.mWidth, gContext.mHeight))
{
  vec_t trans;
...
static void DrawRotationGizmo(OPERATION op, int type)
{
  ...
  for (int axis = 0; axis < 3; axis++)
  {
    ...
    float radiusAxis = sqrtf((AnchorLengthSqr(worldToPos(gContext.mModel.v.position, gContext.mViewProjection) - circlePos[0])));
...
static void DrawScaleGizmo(OPERATION op, int type)
{
  ...
  for (unsigned int i = 0; i < 3; i++)
  {
    ...
    if (belowAxisLimit)
    {
      ...
      GfVec2f worldDirSSpaceNoScale = worldToPos(dirAxis * markerScale * gContext.mScreenFactor, gContext.mMVP);
      GfVec2f worldDirSSpace = worldToPos((dirAxis * markerScale * scaleDisplay[i]) * gContext.mScreenFactor, gContext.mMVP);
...
static void HandleAndDrawLocalBounds(const float *bounds, matrix_t *matrix, const float *snapValues, OPERATION operation)
{
  ...
  for (unsigned int axisIndex = 0; axisIndex < numAxes; ++axisIndex)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      bool overBigAnchor = AnchorLengthSqr(worldBound1 - io.MousePos) <= (AnchorBigRadius * AnchorBigRadius);
      bool overSmallAnchor = AnchorLengthSqr(midBound - io.MousePos) <= (AnchorBigRadius * AnchorBigRadius);
...
static int GetScaleType(OPERATION op)
{
  ...
  for (unsigned int i = 0; i < 3 && type == MT_NONE; i++)
  {
    ...
    const GfVec2f axisStartOnScreen = worldToPos(gContext.mModel.v.position + dirAxis * gContext.mScreenFactor * startOffset, gContext.mViewProjection);
    const GfVec2f axisEndOnScreen = worldToPos(gContext.mModel.v.position + dirAxis * gContext.mScreenFactor * endOffset, gContext.mViewProjection);
...
static int GetMoveType(OPERATION op, vec_t *gizmoHitProportion)
{
  ...
  for (unsigned int i = 0; i < 3 && type == MT_NONE; i++)
  {
    ...
    const GfVec2f axisStartOnScreen = worldToPos(gContext.mModel.v.position + dirAxis * gContext.mScreenFactor * 0.1f, gContext.mViewProjection) - GfVec2f(gContext.mX, gContext.mY);
    const GfVec2f axisEndOnScreen = worldToPos(gContext.mModel.v.position + dirAxis * gContext.mScreenFactor, gContext.mViewProjection) - GfVec2f(gContext.mX, gContext.mY);
...
void ViewManipulate(float *view, float length, GfVec2f position, GfVec2f size, AnchorU32 backgroundColor)
{
  ...
  // panels
  static const GfVec2f panelPosition[9] = {GfVec2f(0.75f, 0.75f), GfVec2f(0.25f, 0.75f), GfVec2f(0.f, 0.75f), GfVec2f(0.75f, 0.25f), GfVec2f(0.25f, 0.25f), GfVec2f(0.f, 0.25f), GfVec2f(0.75f, 0.f), GfVec2f(0.25f, 0.f), GfVec2f(0.f, 0.f)};

  static const GfVec2f panelSize[9] = {GfVec2f(0.25f, 0.25f), GfVec2f(0.5f, 0.25f), GfVec2f(0.25f, 0.25f), GfVec2f(0.25f, 0.5f), GfVec2f(0.5f, 0.5f), GfVec2f(0.25f, 0.5f), GfVec2f(0.25f, 0.25f), GfVec2f(0.5f, 0.25f), GfVec2f(0.25f, 0.25f)};
