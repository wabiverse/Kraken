...
int Edit(Delegate &delegate, const GfVec2f &size, unsigned int id, const AnchorBBox *clippingRect, std::vector<EditPoint> *selectedPoints)
{
  ...
  auto pointToRange = [&](GfVec2f pt) { return (pt - d_min) / range; };
  auto rangeToPoint = [&](GfVec2f pt) { return GfVec2f(pt * range) + d_min; };

  draw_list->AddLine(GfVec2f(GfVec2f(-1.f, -d_min[1] / range[1]) * viewSize) + offset, GfVec2f(GfVec2f(1.f, -d_min[1] / range[1]) * viewSize) + offset, 0xFF000000, 1.5f);
    ...
    for (size_t p = 0; p < ptCount - 1; p++)
    {
      ...
      if (curveType == CurveSmooth || curveType == CurveLinear)
      {
        ...
        for (size_t substep = 0; substep < subStepCount - 1; substep++)
        {
          ...
          const GfVec2f pos1 = GfVec2f(GfVec2f(sp1[0], AnchorLerp(p1[1], p2[1], rt1)) * viewSize) + offset;
          const GfVec2f pos2 = GfVec2f(GfVec2f(sp2[0], AnchorLerp(p1[1], p2[1], rt2)) * viewSize) + offset;

          if (distance(io.MousePos[0], io.MousePos[1], pos1[0], pos1[1], pos2[0], pos2[1]) < 8.f && !scrollingV)
          {
  ...
  if (overSelectedPoint && io.MouseDown[0])
  {
    if ((fabsf(io.MouseDelta[0]) > 0.f || fabsf(io.MouseDelta[1]) > 0.f) && !selection.empty())
    {
      ...
      {
        const GfVec2f mouseRange = pointToRange(originalPoints[originalIndex]) + GfVec2f(io.MousePos - mousePosOrigin);
  ...
  if (movingCurve != -1)
  {
    ...
    if (ptCount >= 1)
    {
      for (size_t p = 0; p < ptCount; p++)
      {
        delegate.EditPoint(movingCurve, int(p), GfVec2f(rangeToPoint(pointToRange(originalPoints[p]) + GfVec2f(io.MousePos - mousePosOrigin)) * sizeOfPixel));
      }
      ret = 1;
    }
    if (!io.MouseDown[0])
