...
namespace ImCurveEdit
{
   ...
   static GfVec2f operator*(const GfVec2f& a, const float b) {
      return GfVec2f(a.x * b, a.y * b);
   }
#endif
   ...
   static int DrawPoint(AnchorDrawList* draw_list, GfVec2f pos, const GfVec2f size, const GfVec2f offset, bool edited)
   {
      int ret = 0;
      AnchorIO& io = Anchor::GetIO();

      static const GfVec2f localOffsets[4] = { GfVec2f(1,0), GfVec2f(0,1), GfVec2f(-1,0), GfVec2f(0,-1) };
      GfVec2f offsets[4];
      for (int i = 0; i < 4; i++)
      {
      ...
      const GfVec2f center = pos * size + offset;
      const AnchorRect anchor(center - GfVec2f(5, 5), center + GfVec2f(5, 5));
   ...
   int Edit(Delegate& delegate, const GfVec2f& size, unsigned int id, const AnchorRect* clippingRect, ImVector<EditPoint>* selectedPoints)
   {
      ...
      const GfVec2f offset = Anchor::GetCursorScreenPos() + GfVec2f(0.f, size.y);
      const GfVec2f ssize(size.x, -size.y);
      const AnchorRect container(offset + GfVec2f(0.f, ssize.y), offset + GfVec2f(ssize.x, 0.f));
      GfVec2f& min = delegate.GetMin();
      GfVec2f& max = delegate.GetMax();
      ...
      if (container.Contains(io.MousePos))
      {
         if (fabsf(io.MouseWheel) > FLT_EPSILON)
         {
            ...
            float ratioY = AnchorLerp(min.y, max.y, r);
      ...
      GfVec2f range = max - min + GfVec2f(1.f, 0.f);  // +1 because of inclusive last frame

      const GfVec2f viewSize(size.x, -size.y);
      const GfVec2f sizeOfPixel = GfVec2f(1.f, 1.f) / viewSize;
      ...
      auto pointToRange = [&](GfVec2f pt) { return (pt - min) / range; };
      auto rangeToPoint = [&](GfVec2f pt) { return (pt * range) + min; };

      draw_list->AddLine(GfVec2f(-1.f, -min.y / range.y) * viewSize + offset, GfVec2f(1.f, -min.y / range.y) * viewSize + offset, 0xFF000000, 1.5f);
         ...
         for (size_t p = 0; p < ptCount - 1; p++)
         {
            ...
            if (curveType == CurveSmooth || curveType == CurveLinear)
            {
               ...
               for (size_t substep = 0; substep < subStepCount - 1; substep++)
               {
                  ...
                  const GfVec2f pos1 = GfVec2f(sp1.x, AnchorLerp(p1.y, p2.y, rt1)) * viewSize + offset;
                  const GfVec2f pos2 = GfVec2f(sp2.x, AnchorLerp(p1.y, p2.y, rt2)) * viewSize + offset;
            ...
            {
               GfVec2f dp1 = p1 * viewSize + offset;
               GfVec2f dp2 = GfVec2f(p2.x, p1.y) * viewSize + offset;
               GfVec2f dp3 = p2 * viewSize + offset;
      ...
      if (overSelectedPoint && io.MouseDown[0])
      {
         if ((fabsf(io.MouseDelta.x) > 0.f || fabsf(io.MouseDelta.y) > 0.f) && !selection.empty())
         {
            ...
            {
               const GfVec2f p = rangeToPoint(pointToRange(originalPoints[originalIndex]) + (io.MousePos - mousePosOrigin) * sizeOfPixel);
      ...
      if (movingCurve == -1 && overCurve != -1 && Anchor::IsMouseClicked(0) && selection.empty() && !selectingQuad)
      {
      ...
      if (selectingQuad)
      {
         ...
         const AnchorRect selectionQuad(bmin, bmax);
         if (!io.MouseDown[0])
         {
            if (!io.KeyShift)
               selection.clear();
            ...
            for (size_t c = 0; c < curveCount; c++)
            {
               ...
               for (size_t p = 0; p < ptCount; p++)
               {
                  const GfVec2f center = pointToRange(pts[p]) * viewSize + offset;
      ...
      if (!overCurveOrPoint && Anchor::IsMouseClicked(0) && !selectingQuad && movingCurve == -1 && !overSelectedPoint && container.Contains(io.MousePos))
      {
         selectingQuad = true;
