                            ...
                            const bool use_curve_mapping)
{
  ...
  if (!textures.uniforms.empty()) {
    ...
    for (OCIO_GPUUniform &uniform : textures.uniforms) {
      ...
      kraken::StringRef suffix = kraken::StringRefNull("xyzw").substr(0, vec_len);
    ...
    textures.uniforms_buffer = GPU_uniformbuf_create_ex(ubo_size,
                                                        ubo_data_buf,
                                                        "OCIO_LutParameters");
                          ...
                          int index)
{
  ...
  shader_desc
    ->getTexture(index, texture_name, sampler_name, width, height, channel, interpolation);
                                  ...
                                  OCIO_CurveMappingSettings *curve_mapping_settings)
{
  ...
  GPU_texture_update_sub(curvemap.texture,
                         GPU_DATA_FLOAT,
                         pixels,
                         UNPACK3(offset),
                         UNPACK3(extent));
  ...
  OCIO_CurveMappingSettings *curve_mapping_settings)
{
   ...
   */
  OCIO_ConstProcessorRcPtr *processor_to_scene_linear =
    OCIO_configGetProcessorWithNames(config, input, ROLE_SCENE_LINEAR);
  OCIO_ConstProcessorRcPtr *processor_to_display =
    OCIO_createDisplayProcessor(config, ROLE_SCENE_LINEAR, view, display, look, 1.0f, 1.0f, false);
  ...
  if (processor_to_scene_linear && processor_to_display) {
    ...
    if (createGPUTextures(display_shader.textures,
                                    ...
                                    const bool use_overlay)
{
  ...
  OCIO_GPUDisplayShader &display_shader =
    getGPUDisplayShader(config, input, view, display, look, curve_mapping_settings);
