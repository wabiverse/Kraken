...
{
  if (outF) {
    KLI_bicubic_interpolation_fl(in->rect_float, outF, in->x, in->y, 4, u, v);
  } else {
    KLI_bicubic_interpolation_char((uchar *)in->rect, outI, in->x, in->y, 4, u, v);
  }
}
...
{
  KLI_assert(outF);
  KLI_assert(in->rect_float);
  KLI_bilinear_interpolation_fl(in->rect_float, outF, in->x, in->y, 4, u, v);
}
                                       ...
                                       float v)
{
  KLI_assert(outI);
  KLI_assert(in->rect);
  KLI_bilinear_interpolation_char((uchar *)in->rect, outI, in->x, in->y, 4, u, v);
}
...
/* -------------------------------------------------------------------- */
/** @name Threaded Image Processing
 * @{ */

static void processor_apply_func(TaskPool *__restrict pool, void *taskdata)
{
  void (*do_thread)(void *) = (void (*)(void *))KLI_task_pool_user_data(pool);
  do_thread(taskdata);
}
  ...
  void *(do_thread)(void *))
{
  ...
  task_pool = KLI_task_pool_create(do_thread, TASK_PRIORITY_HIGH);
  ...
  for (i = 0; i < total_tasks; i++) {
    ...
    KLI_task_pool_push(task_pool, processor_apply_func, handle, false, NULL);

    start_line += lines_per_task;
  }

  /* work and wait until tasks are done */
  KLI_task_pool_work_and_wait(task_pool);

  /* Free memory. */
  MEM_freeN(handles);
  KLI_task_pool_free(task_pool);
}
                                            ...
                                            void *custom_data)
{
  ...
  KLI_parallel_range_settings_defaults(&settings);
  KLI_task_parallel_range(0, total_scanlines, &data, processor_apply_parallel, &settings);
}
