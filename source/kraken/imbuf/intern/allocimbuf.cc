...
{
  KLI_spin_init(&refcounter_spin);
}

void imb_refcounter_lock_exit(void)
{
  KLI_spin_end(&refcounter_spin);
}
...
{
  KLI_spin_init(&mmap_spin);
}

void imb_mmap_lock_exit(void)
{
  KLI_spin_end(&mmap_spin);
}

void imb_mmap_lock(void)
{
  KLI_spin_lock(&mmap_spin);
}

void imb_mmap_unlock(void)
{
  KLI_spin_unlock(&mmap_spin);
}
#endif
...
void IMB_freeImBuf(ImBuf *ibuf)
{
  ...
  KLI_spin_unlock(&refcounter_spin);

  if (needs_free) {
    /* Include this check here as the path may be manipulated after creation. */
    KLI_assert_msg(!(ibuf->filepath[0] == '/' && ibuf->filepath[1] == '/'),
                   "'.usd' relative \"//\" must not be used in ImBuf!");
...
bool addzbufImBuf(ImBuf *ibuf)
{
  ...
  if ((ibuf->zbuf = static_cast<int *>(imb_alloc_pixels(ibuf->x, ibuf->y, 1, sizeof(uint), __func__))))
  {
    ibuf->mall |= IB_zbuf;
    ibuf->flags |= IB_zbuf;
    return true;
...
bool addzbuffloatImBuf(ImBuf *ibuf)
{
  ...
  if ((ibuf->zbuf_float = static_cast<float *>(imb_alloc_pixels(ibuf->x, ibuf->y, 1, sizeof(float), __func__))))
  {
    ibuf->mall |= IB_zbuffloat;
...
bool imb_addrectImBuf(ImBuf *ibuf)
{
  ...
  if ((ibuf->rect = static_cast<uint *>(imb_alloc_pixels(ibuf->x, ibuf->y, 4, sizeof(uchar), __func__))))
  {
    ibuf->mall |= IB_rect;
    ibuf->flags |= IB_rect;
    if (ibuf->planes > 32) {
...
struct ImBuf *IMB_allocFromBufferOwn(uint *rect, float *rectf, uint w, uint h, uint channels)
{
  ...
  if (rectf) {
    KLI_assert(MEM_allocN_len(rectf) == sizeof(float[4]) * w * h);
  ...
  if (rect) {
    KLI_assert(MEM_allocN_len(rect) == sizeof(uchar[4]) * w * h);
    ibuf->rect = rect;
...
struct ImBuf *IMB_allocFromBuffer(const uint *rect, const float *rectf, uint w, uint h, uint channels)
{
  ImBuf *ibuf = nullptr;
