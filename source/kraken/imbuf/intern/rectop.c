...
static void rect_crop_4bytes(void **buf_p, const int size_src[2], const rcti *crop)
{
  ...
  const int size_dst[2] = {
    KLI_rcti_size_x(crop) + 1,
    KLI_rcti_size_y(crop) + 1,
  };
  uint *src = *buf_p;
...
void IMB_rect_crop(ImBuf *ibuf, const rcti *crop)
{
  ...
  KLI_assert(size_dst[0] > 0 && size_dst[1] > 0);
  KLI_assert(crop->xmin >= 0 && crop->ymin >= 0);
  KLI_assert(crop->xmax < ibuf->x && crop->ymax < ibuf->y);
                ...
                if (mask > *dmr) {
                  ...
                  *dmr = mask;
                  ...
                  if (mode == IMB_BLEND_INTERPOLATE) {
                    mask_src[3] = src[3];
                    blend_color_interpolate_byte((uchar *)dr,
                                                 (uchar *) or
                                                 ,
                                                 mask_src,
                                                 mask / 65535.0f);
                       ...
                       struct ColorManagedDisplay *display)
{
  ...
  buf_rectfill_area((uchar *)ibuf->rect,
                    ibuf->rect_float,
                    ibuf->x,
                    ibuf->y,
                    col,
