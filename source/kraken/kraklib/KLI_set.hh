 ...
 * Gadget Vault.
 *
 * A `kraken::Set<Key>` is an unordered container for unique elements of type `Key`. It is
 ...
 *
 * In most cases, your default choice for a hash set in Blender should be `kraken::Set`.
 *
 * kraken::Set is implemented using open addressing in a slot array with a power-of-two size.
 ...
 *
 * A rudimentary benchmark can be found in KLI_set_test.cc. The results of that benchmark are
 * there as well. The numbers show that in this specific case kraken::Set outperforms
 ...
 * - The probing strategy can be customized. See KLI_probing_stragies.hh for details.
 * - The slot type can be customized. See KLI_set_slots.hh for details.
 ...
 *   interface as kraken::Set. This is useful for bench-marking.
 *
...
namespace kraken
{
  ...
  template<
     ...
     * for an additional variable. Also see KLI_set_slots.hh.
     */
...
#define SET_SLOT_PROBING_END() SLOT_PROBING_END()
   ...
   public:
    ...
    /**
     * Add a new key to the set. This invokes undefined behavior when the key is in the set
    ...
    /**
     * Add a key to the set. If the key exists in the set already, nothing is done. The return
     * value is true if the key was newly added to the set.
     *
    ...
    /**
     * Returns the key that is stored in the set that compares equal to the given key. If the key
     * is not in the set, the given default value is returned instead.
     */
    ...
    /**
     * Returns the key in the set that compares equal to the given key. If it does not exist, the
     * key is newly added.
     */
    ...
    /**
     * Deletes the key from the set. Returns true when the key did exist beforehand, otherwise
     * false.
     *
    ...
    /**
     * Deletes the key from the set. This invokes undefined behavior when the key is not in the
     * map.
     */
     ...
     * while iterating over the set. However, after this method has been called, the removed
     * element must not be accessed anymore.
     */
    ...
    /**
     * Creates a new slot array and reinserts all keys inside of that. This method can be used to
     * get rid of removed slots. Also this is useful for benchmarking the grow function.
     */
    void rehash()
    {
    ...
    /**
     * Potentially resize the set such that it can hold the specified number of keys without
   ...
   private:
    ...
    KLI_NOINLINE void realloc_and_reinsert(const int64_t min_usable_slots)
    {
    ...
    /**
     * In some cases when exceptions are thrown, it's best to just reset the entire container to
     * make sure that invariants are maintained. This should happen very rarely in practice.
     */
  ...
  /**
   * A wrapper for std::unordered_set with the API of kraken::Set. This can be used for
   * benchmarking.
   */
  template<typename Key> class StdUnorderedSetWrapper
  {
   private:
    ...
    using SetType = std::unordered_set<Key, kraken::DefaultHash<Key>>;
    SetType set_;
  ...
  template<typename Key,
           ...
           typename Slot = typename DefaultSetSlot<Key>::type>
  using RawSet =
    Set<Key, InlineBufferCapacity, ProbingStrategy, Hash, IsEqual, Slot, RawAllocator>;
