 ...
 *
 * A `kraken::Map<Key, Value>` is an unordered associative container that stores key-value pairs.
 ...
 *
 * Your default choice for a hash map in Blender should be `kraken::Map`.
 *
 * kraken::Map is implemented using open addressing in a slot array with a power-of-two size.
 ...
 *
 * A rudimentary benchmark can be found in KLI_map_test.cc. The results of that benchmark are there
 * as well. The numbers show that in this specific case kraken::Map outperforms std::unordered_map
 ...
 * - The hash function can be customized. See KLI_hash.hh for details.
 * - The probing strategy can be customized. See KLI_probing_strategies.hh for details.
 * - The slot type can be customized. See KLI_map_slots.hh for details.
 ...
 *   interface as kraken::Map. This is useful for benchmarking.
 */
...
#include "KLI_hash.hh"
#include "KLI_hash_tables.hh"
#include "KLI_map_slots.hh"
#include "KLI_probing_strategies.hh"
...
namespace kraken
{
  ...
  template<
    ...
    /**
     * The strategy used to deal with collisions. They are defined in KLI_probing_strategies.hh.
     */
    typename ProbingStrategy = DefaultProbingStrategy,
    /**
     * The hash function used to hash the keys. There is a default for many types. See KLI_hash.hh
     ...
     * footprint for some types. Slot types are defined in KLI_map_slots.hh
     */
  ...
  class Map
  {
   ...
   private:
    ...
    /**
     * The number of slots minus one. This is a bit mask that can be used to turn any integer into
     * a valid slot index efficiently.
     */
    uint64_t slot_mask_;

    /** This is called to hash incoming keys. */
    KLI_NO_UNIQUE_ADDRESS Hash hash_;

    /** This is called to check equality of two keys. */
    KLI_NO_UNIQUE_ADDRESS IsEqual is_equal_;
...
#undef LOAD_FACTOR
    ...
    /**
     * This is the array that contains the actual slots. There is always at least one empty slot
     * and the size of the array is a power of two.
     */
    SlotArray slots_;
...
#define MAP_SLOT_PROBING_END() SLOT_PROBING_END()
   ...
   public:
    ...
    {
      this->add_new__impl(std::forward<ForwardKey>(key),
                          hash_(key),
                          std::forward<ForwardValue>(value)...);
    }
    ...
    {
      return this->add__impl(std::forward<ForwardKey>(key),
                             hash_(key),
                             std::forward<ForwardValue>(value)...);
    }
    ...
    {
      return this->add_overwrite__impl(std::forward<ForwardKey>(key),
                                       hash_(key),
                                       std::forward<ForwardValue>(value)...);
    }
    ...
    /**
     * Deletes the key-value-pair with the given key. Returns true when the key was contained and
     * is now removed, otherwise false.
     *
    ...
    /**
     * Deletes the key-value-pair with the given key. This invokes undefined behavior when the key
     * is not in the map.
     */
    ...
    /**
     * Get the value that corresponds to the given key and remove it from the map. If the key is
     * not in the map, return the given default value instead.
     */
     ...
     * multiple lookups
     *
     * When the key did not yet exist in the map, the create_value function is called. Otherwise
     * the modify_value function is called.
     *
     * Both functions are expected to take a single parameter of type `Value *`. In create_value,
     * this pointer will point to uninitialized memory that has to be initialized by the function.
     * In modify_value, it will point to an already initialized value.
     *
    ...
    auto add_or_modify_as(ForwardKey &&key,
                          ...
                          const ModifyValueF &modify_value) -> decltype(create_value(nullptr))
    {
      return this->add_or_modify__impl(std::forward<ForwardKey>(key),
                                       create_value,
                                       modify_value,
                                       hash_(key));
    }
    ...
    /**
     * Returns a reference to the value corresponding to the given key. If the key is not in the
     * map, a new key-value-pair is added and a reference to the value in the map is returned.
     */
    ...
    {
      return this->lookup_or_add__impl(std::forward<ForwardKey>(key),
                                       hash_(key),
                                       std::forward<ForwardValue>(value)...);
    }
    ...
    struct BaseIterator
    {
     ...
     protected:
      ...
      /* We could have separate base iterators for const and non-const iterators, but that would
       * add more complexity than benefits right now. */
      Slot *slots_;
     ...
     public:
      ...
      {
        KLI_assert(a.slots_ == b.slots_);
        KLI_assert(a.total_slots_ == b.total_slots_);
    ...
    /**
     * Allows writing a range-for loop that iterates over all keys. The iterator is invalidated,
    ...
    /**
     * Print common statistics like size and collision count. This is useful for debugging
     * purposes.
     */
   ...
   private:
    ...
    KLI_NOINLINE void realloc_and_reinsert(int64_t min_usable_slots)
    {
      int64_t total_slots, usable_slots;
      max_load_factor_.compute_total_and_usable_slots(SlotArray::inline_buffer_capacity(),
                                                      min_usable_slots,
                                                      &total_slots,
                                                      &usable_slots);
      KLI_assert(total_slots >= 1);
    ...
    {
      KLI_assert(!this->contains_as(key));
                             ...
                             uint64_t hash) -> decltype(create_value(nullptr))
    {
      ...
      KLI_STATIC_ASSERT((std::is_same_v<CreateReturnT, ModifyReturnT>),
    ...
    template<typename ForwardKey, typename CreateValueF>
    Value &lookup_or_add_cb__impl(ForwardKey &&key,
                                  const CreateValueF &create_value,
                                  uint64_t hash)
    {
      this->ensure_can_add();
    ...
    void ensure_can_add()
    {
      if (occupied_and_removed_slots_ >= usable_slots_) {
        ...
        KLI_assert(occupied_and_removed_slots_ < usable_slots_);
      }
    }
  };
  ...
  /**
   * A wrapper for std::unordered_map with the API of kraken::Map. This can be used for
   * benchmarking.
   */
  template<typename Key, typename Value> class StdUnorderedMapWrapper
  {
   private:
    ...
    using MapType = std::unordered_map<Key, Value, kraken::DefaultHash<Key>>;
    MapType map_;
