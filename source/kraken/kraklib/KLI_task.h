...
void KLI_task_scheduler_init(void);
void KLI_task_scheduler_exit(void);
int KLI_task_scheduler_num_threads(void);
 ...
 */
TaskPool *KLI_task_pool_create(void *userdata, eTaskPriority priority);

/**
 * Background: always run tasks in a background thread, never immediately
 * execute them. For running background jobs.
 */
TaskPool *KLI_task_pool_create_background(void *userdata, eTaskPriority priority);

/**
 * Background Serial: run tasks one after the other in the background,
 * without parallelization between the tasks.
 */
TaskPool *KLI_task_pool_create_background_serial(void *userdata, eTaskPriority priority);
 ...
 */
TaskPool *KLI_task_pool_create_suspended(void *userdata, eTaskPriority priority);

/**
 * No threads: immediately executes tasks on the same thread. For debugging.
 */
TaskPool *KLI_task_pool_create_no_threads(void *userdata);

void KLI_task_pool_free(TaskPool *pool);

void KLI_task_pool_push(TaskPool *pool,
                        TaskRunFunction run,
                        void *taskdata,
                        bool free_taskdata,
 ...
 */
void KLI_task_pool_work_and_wait(TaskPool *pool);
/**
 * Cancel all tasks, keep worker threads running.
 */
void KLI_task_pool_cancel(TaskPool *pool);
 ...
 */
bool KLI_task_pool_current_canceled(TaskPool *pool);

/**
 * Optional `userdata` pointer to pass along to run function.
 */
void *KLI_task_pool_user_data(TaskPool *pool);

/**
 * Optional mutex to use from run function.
 */
ThreadMutex *KLI_task_pool_user_mutex(TaskPool *pool);
...
typedef struct TaskParallelSettings
{
   ...
   * KLI_task_parallel_range() by itself will always use threading when range
...
KLI_INLINE void KLI_parallel_range_settings_defaults(TaskParallelSettings *settings);

void KLI_task_parallel_range(int start,
                             int stop,
                             void *userdata,
 ...
 */
void KLI_task_parallel_iterator(void *userdata,
 ...
 */
void KLI_task_parallel_listbase(struct ListBase *listbase,
                                void *userdata,
                                TaskParallelIteratorFunc func,
                                const TaskParallelSettings *settings);
 ...
 *
 * \param mempool: The iterable KLI_mempool to loop over.
 * \param userdata: Common userdata passed to all instances of \a func.
 * \param func: Callback function.
 * \param settings: See public API doc of TaskParallelSettings for description of all settings.
 *
 * \note There is no static scheduling here.
 */
void KLI_task_parallel_mempool(struct KLI_mempool *mempool,
                               void *userdata,
...
KLI_INLINE void KLI_parallel_mempool_settings_defaults(TaskParallelSettings *settings)
{
  memset(settings, 0, sizeof(*settings));
  settings->use_threading = true;
}
 ...
 */
int KLI_task_parallel_thread_id(const TaskParallelTLS *tls);
 ...
 *
 * \code{.c}
 * TaskGraph *task_graph = KLI_task_graph_create();
 * TaskNode *root = KLI_task_graph_node_create(task_graph, root_exec, NULL, NULL);
 * TaskNode *node_1 = KLI_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 * TaskNode *node_2 = KLI_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 * TaskNode *node_3 = KLI_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 * TaskNode *node_4 = KLI_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 *
 * KLI_task_graph_edge_create(root, node_1);
 * KLI_task_graph_edge_create(root, node_2);
 * KLI_task_graph_edge_create(node_2, node_3);
 * KLI_task_graph_edge_create(node_2, node_4);
 * \endcode
 *
 ...
 *
 *    `KLI_task_graph_node_push_work(root);`
 *
 ...
 *
 *    `KLI_task_graph_free(task_graph);`
 *
 ...
 * Task data is freed when calling #KLI_task_graph_free.
 *
 * \code{.c}
 * MyData *task_data = MEM_callocN(sizeof(MyData), __func__);
 * TaskNode *root = KLI_task_graph_node_create(task_graph, root_exec, task_data, MEM_freeN);
 * TaskNode *node_1 = KLI_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * TaskNode *node_2 = KLI_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * TaskNode *node_3 = KLI_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * TaskNode *node_4 = KLI_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * \endcode
 * \{ */
...
struct TaskGraph *KLI_task_graph_create(void);
void KLI_task_graph_work_and_wait(struct TaskGraph *task_graph);
void KLI_task_graph_free(struct TaskGraph *task_graph);
struct TaskNode *KLI_task_graph_node_create(struct TaskGraph *task_graph,
                                            TaskGraphNodeRunFunction run,
                                            void *user_data,
                                            TaskGraphNodeFreeFunction free_func);
bool KLI_task_graph_node_push_work(struct TaskNode *task_node);
void KLI_task_graph_edge_create(struct TaskNode *from_node, struct TaskNode *to_node);
