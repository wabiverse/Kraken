...
static void sigbus_handler(int sig, siginfo_t *siginfo, void *ptr)
{
  ...
  LISTBASE_FOREACH(LinkData *, link, &error_handler.open_mmaps)
  {
    ...
    if (error_addr >= file->memory && error_addr < file->memory + file->length) {
      ...
      const void *mapped_memory = mmap(file->memory,
                                       file->length,
                                       PROT_READ,
...
static void sigbus_handler_add(KLI_mmap_file *file)
{
  KLI_addtail(&error_handler.open_mmaps, KLI_genericNodeN(file));
}

/* Removes a file from the list that the error handler checks. */
static void sigbus_handler_remove(KLI_mmap_file *file)
{
  LinkData *link = KLI_findptr(&error_handler.open_mmaps, file, offsetof(LinkData, data));
  KLI_freelinkN(&error_handler.open_mmaps, link);
}
#endif

KLI_mmap_file *KLI_mmap_open(int fd)
{
  void *memory, *handle = NULL;
  size_t length = KLI_lseek(fd, 0, SEEK_END);
  ...
  /* Now that the mapping was successful, allocate memory and set up the KLI_mmap_file. */
  KLI_mmap_file *file = MEM_callocN(sizeof(KLI_mmap_file), __func__);
  file->memory = memory;
  file->handle = handle;
  file->length = length;
...
void *KLI_mmap_get_pointer(KLI_mmap_file *file)
{
  return file->memory;
}

void KLI_mmap_free(KLI_mmap_file *file)
{
#ifndef WIN32
