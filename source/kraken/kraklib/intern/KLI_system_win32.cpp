...
static bool kli_private_symbols_loaded()
{
...
static void kli_windows_get_module_name(LPVOID address, PCHAR buffer, size_t size)
{
  HMODULE mod;
  buffer[0] = 0;
  if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (const char *)address, &mod)) {
    if (GetModuleFileName(mod, (LPSTR)buffer, size)) {
      PathStripPath((LPSTR)buffer);
    }
  }
}
...
static void kli_load_symbols()
{
  ...
  if (GetModuleFileNameA(NULL, pdb_file, sizeof(pdb_file))) {
    ...
    /* append kraken.pdb */
    PathAppendA(pdb_file, "kraken.pdb");
    if (PathFileExistsA(pdb_file)) {
      ...
      if (mod) {
        ...
        if (GetFileAttributesExA(pdb_file, GetFileExInfoStandard, &file_data)) {
          /**
           * SymInitialize will try to load symbols on its own,
           * so we first must unload whatever it did trying to
           * help. */
...
static bool KLI_windows_system_backtrace_run_trace(FILE *fp, HANDLE hThread, PCONTEXT context)
{
  ...
  while (true) {
    if (StackWalk64(IMAGE_FILE_MACHINE_AMD64,
      ...
      if (frame.AddrPC.Offset) {
        ...
        kli_windows_get_module_name((LPVOID)frame.AddrPC.Offset, module, sizeof(module));
...
static void kli_windows_system_backtrace_exception_record(FILE *fp, PEXCEPTION_RECORD record)
{
  char module[MAX_PATH];
  ...
  if (record->ExceptionRecord) {
    fprintf(fp, "Nested ");
    kli_windows_system_backtrace_exception_record(fp, record->ExceptionRecord);
  }
  fprintf(fp, "\n\n");
}

static bool kli_windows_system_backtrace_stack_thread(FILE *fp, HANDLE hThread)
{
  CONTEXT context = {0};
  ...
  return KLI_windows_system_backtrace_run_trace(fp, hThread, &context);
}

static bool KLI_windows_system_backtrace_stack(FILE *fp)
{
...
static void kli_windows_system_backtrace_threads(FILE *fp)
{
  fprintf(fp, "Threads:\n");
  ...
  do {
    if (te32.th32OwnerProcessID == GetCurrentProcessId()) {
      if (GetCurrentThreadId() != te32.th32ThreadID) {
        ...
        kli_windows_system_backtrace_stack_thread(fp, ht);
        CloseHandle(ht);
      }
    }
...
static void kli_windows_get_module_version(const char *file, char *buffer, size_t buffersize)
{
  buffer[0] = 0;
  DWORD verHandle = 0;
  UINT size = 0;
...
static void kli_windows_system_backtrace_modules(FILE *fp)
{
  ...
  do {
    if (me32.th32ProcessID == GetCurrentProcessId()) {
      ...
      kli_windows_get_module_version((const char*)me32.szExePath, version, sizeof(version));
...
void KLI_system_backtrace(FILE *fp)
{
  ...
  if (current_exception) {
    kli_windows_system_backtrace_exception_record(fp, current_exception->ExceptionRecord);
  }
  if (KLI_windows_system_backtrace_stack(fp)) {
    /**
     * When the kraken symbols are missing the stack traces
     * will be unreliable so only run if the previous step
     * completed successfully. */
    kli_windows_system_backtrace_threads(fp);
  }
