...
char *KLI_cpu_brand_string(void)
{
  char buf[49] = {0};
  int result[4] = {0};
...
int KLI_cpu_support_sse41(void)
{
  int result[4], num;
  __cpuid(result, 0);
  num = result[0];
...
void KLI_hostname_get(char *buffer, size_t bufsize)
{
  ...
  if (gethostname(buffer, bufsize - 1) < 0) {
    KLI_strncpy(buffer, "-unknown-", bufsize);
  }
...
size_t KLI_system_memory_max_in_megabytes(void)
{
...
int KLI_system_memory_max_in_megabytes_int(void)
{
  const size_t limit_megabytes = KLI_system_memory_max_in_megabytes();
