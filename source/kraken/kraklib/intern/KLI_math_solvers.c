    ...
    const float *a, const float *b, const float *c, const float *d, float *r_x, const int count)
{
  ...
  double *c1 = (double *)malloc(bytes * 2);
  double *d1 = c1 + count;
    ...
    const float *a, const float *b, const float *c, const float *d, float *r_x, const int count)
{
  ...
  if (count == 2) {
    ...
    return KLI_tridiagonal_solve(a2, b, c2, d, r_x, count);
  }
  ...
  if (a0 == 0.0f && cN == 0.0f) {
    return KLI_tridiagonal_solve(a, b, c, d, r_x, count);
  }

  size_t bytes = sizeof(float) * (unsigned)count;
  float *tmp = (float *)malloc(bytes * 2);
  float *b2 = tmp + count;
  ...
  bool success = KLI_tridiagonal_solve(a, b2, c, tmp, tmp, count) &&
                 KLI_tridiagonal_solve(a, b2, c, d, r_x, count);
