...
double KLI_dir_free_space(const char *dir)
{
#ifdef WIN32
...
size_t KLI_file_descriptor_size(int file)
{
  KLI_stat_t st;
  if ((file < 0) || (KLI_fstat(file, &st) == -1)) {
    return -1;
  }
  return st.st_size;
}

size_t KLI_file_size(const char *path)
{
  KLI_stat_t stats;
  if (KLI_stat(path, &stats) == -1) {
    return -1;
  }
...
#  ifdef WIN32
  ...
  if (KLI_path_extension_check(path, ".lnk")) {
...
#ifndef __APPLE__
bool KLI_file_alias_target(const char *filepath,
...
{
#  ifdef WIN32
  if (!KLI_path_extension_check(filepath, ".lnk")) {
    return false;
  }
...
{
  KLI_stat_t st;
  if (KLI_fstat(fileno(fp), &st) == -1) {
    return NULL;
  }
  if (S_ISDIR(st.st_mode)) {
    return NULL;
  }
  if (KLI_fseek(fp, 0L, SEEK_END) == -1) {
    return NULL;
  }
  /* Don't use the 'st_size' because it may be the symlink. */
  const long int filelen = KLI_ftell(fp);
  if (filelen == -1) {
    return NULL;
  }
  if (KLI_fseek(fp, 0L, SEEK_SET) == -1) {
    return NULL;
  }
...
void *KLI_file_read_text_as_mem_with_newline_as_nil(const char *filepath,
                                                    ...
                                                    size_t *r_size)
{
  char *mem = KLI_file_read_text_as_mem(filepath, pad_bytes, r_size);
  if (mem != NULL) {
...
LinkNode *KLI_file_read_as_lines(const char *filepath)
{
  FILE *fp = KLI_fopen(filepath, "r");
  ...
  KLI_fseek(fp, 0, SEEK_END);
  size = (size_t)KLI_ftell(fp);
  KLI_fseek(fp, 0, SEEK_SET);
  ...
  if (buf) {
    ...
    for (i = 0; i <= size; i++) {
      if (i == size || buf[i] == '\n') {
        char *line = KLI_strdupn(&buf[last], i - last);
        KLI_linklist_append(&lines, line);
        last = i + 1;
      }
    }
...
void KLI_file_free_lines(LinkNode *lines)
{
  KLI_linklist_freeN(lines);
}

bool KLI_file_older(const char *file1, const char *file2)
{
#ifdef WIN32
