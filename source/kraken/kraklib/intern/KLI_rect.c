...
/* avoid including KLI_math */
static void unit_m4(float m[4][4]);

bool KLI_rcti_is_empty(const rcti *rect)
{
  return ((rect->xmax <= rect->xmin) || (rect->ymax <= rect->ymin));
}

bool KLI_rctf_is_empty(const rctf *rect)
{
  return ((rect->xmax <= rect->xmin) || (rect->ymax <= rect->ymin));
}

bool KLI_rcti_isect_x(const rcti *rect, const int x)
{
  if (x < rect->xmin) {
    return false;
  }
...
bool KLI_rcti_isect_y(const rcti *rect, const int y)
{
  if (y < rect->ymin) {
    return false;
  }
...
bool KLI_rctf_isect_x(const rctf *rect, const float x)
{
  if (x < rect->xmin) {
    return false;
  }
...
bool KLI_rctf_isect_y(const rctf *rect, const float y)
{
  if (y < rect->ymin) {
    return false;
  }
...
bool KLI_rctf_isect_pt(const rctf *rect, const float x, const float y)
{
  if (x < rect->xmin) {
    return false;
  }
...
int KLI_rcti_length_x(const rcti *rect, const int x)
{
  if (x < rect->xmin) {
    return rect->xmin - x;
  }
...
int KLI_rcti_length_y(const rcti *rect, const int y)
{
  if (y < rect->ymin) {
    return rect->ymin - y;
  }
...
float KLI_rctf_length_x(const rctf *rect, const float x)
{
  if (x < rect->xmin) {
    return rect->xmin - x;
  }
...
float KLI_rctf_length_y(const rctf *rect, const float y)
{
  if (y < rect->ymin) {
    return rect->ymin - y;
  }
...
bool KLI_rcti_isect_segment(const rcti *rect, const int s1[2], const int s2[2])
{
  ...
  if (KLI_rcti_isect_pt_v(rect, s1) || KLI_rcti_isect_pt_v(rect, s2)) {
    return true;
  }
...
bool KLI_rcti_isect_circle(const rcti *rect, const float xy[2], const float radius)
{
  float dx, dy;
...
bool KLI_rctf_isect_circle(const rctf *rect, const float xy[2], const float radius)
{
  float dx, dy;
...
void KLI_rcti_union(rcti *rct_a, const rcti *rct_b)
{
  if (rct_a->xmin > rct_b->xmin) {
    rct_a->xmin = rct_b->xmin;
  }
...
void KLI_rctf_init(rctf *rect, float xmin, float xmax, float ymin, float ymax)
{
  rect->xmin = xmin;
  rect->xmax = xmax;
  rect->ymin = ymin;
  ...
  KLI_rctf_sanitize(rect);
}

void KLI_rcti_init(rcti *rect, int xmin, int xmax, int ymin, int ymax)
{
  rect->xmin = xmin;
  rect->xmax = xmax;
  rect->ymin = ymin;
  ...
  KLI_rcti_sanitize(rect);
}

bool KLI_rctf_is_valid(const rctf *rect)
{
  return (rect->xmin <= rect->xmax) && (rect->ymin <= rect->ymax);
}

bool KLI_rcti_is_valid(const rcti *rect)
{
  return (rect->xmin <= rect->xmax) && (rect->ymin <= rect->ymax);
}

void KLI_rctf_sanitize(rctf *rect)
{
  ...
  KLI_assert(KLI_rctf_is_valid(rect));
}

void KLI_rcti_sanitize(rcti *rect)
{
  if (rect->xmin > rect->xmax) {
  ...
  KLI_assert(KLI_rcti_is_valid(rect));
}

void KLI_rctf_init_pt_radius(rctf *rect, const float xy[2], float size)
{
  rect->xmin = xy[0] - size;
  rect->xmax = xy[0] + size;
  rect->ymin = xy[1] - size;
...
void KLI_rcti_init_pt_radius(rcti *rect, const int xy[2], int size)
{
  rect->xmin = xy[0] - size;
  rect->xmax = xy[0] + size;
  rect->ymin = xy[1] - size;
...
void KLI_rcti_init_minmax(rcti *rect)
{
...
void KLI_rcti_do_minmax_v(rcti *rect, const int xy[2])
{
  if (xy[0] < rect->xmin) {
    rect->xmin = xy[0];
  }
...
void KLI_rcti_do_minmax_rcti(rcti *rect, const rcti *other)
{
...
void KLI_rctf_do_minmax_v(rctf *rect, const float xy[2])
{
  if (xy[0] < rect->xmin) {
    rect->xmin = xy[0];
  }
...
void KLI_rctf_transform_calc_m4_pivot_min_ex(
    const rctf *dst, const rctf *src, float matrix[4][4], uint x, uint y)
{
  ...
  matrix[x][x] = KLI_rctf_size_x(src) / KLI_rctf_size_x(dst);
  matrix[y][y] = KLI_rctf_size_y(src) / KLI_rctf_size_y(dst);
...
{
  KLI_rctf_transform_calc_m4_pivot_min_ex(dst, src, matrix, 0, 1);
}
...
void KLI_rcti_recenter(rcti *rect, int x, int y)
{
  const int dx = x - KLI_rcti_cent_x(rect);
  const int dy = y - KLI_rcti_cent_y(rect);
  KLI_rcti_translate(rect, dx, dy);
}
void KLI_rctf_recenter(rctf *rect, float x, float y)
{
  const float dx = x - KLI_rctf_cent_x(rect);
  const float dy = y - KLI_rctf_cent_y(rect);
  KLI_rctf_translate(rect, dx, dy);
}

void KLI_rcti_resize_x(rcti *rect, int x)
{
  rect->xmin = KLI_rcti_cent_x(rect) - (x / 2);
  rect->xmax = rect->xmin + x;
}

void KLI_rcti_resize_y(rcti *rect, int y)
{
  rect->ymin = KLI_rcti_cent_y(rect) - (y / 2);
  rect->ymax = rect->ymin + y;
}

void KLI_rcti_resize(rcti *rect, int x, int y)
{
  rect->xmin = KLI_rcti_cent_x(rect) - (x / 2);
  rect->ymin = KLI_rcti_cent_y(rect) - (y / 2);
  rect->xmax = rect->xmin + x;
...
void KLI_rcti_pad(rcti *rect, int pad_x, int pad_y)
{
  rect->xmin -= pad_x;
  rect->ymin -= pad_y;
  rect->xmax += pad_x;
...
void KLI_rctf_pad(rctf *rect, float pad_x, float pad_y)
{
  rect->xmin -= pad_x;
  rect->ymin -= pad_y;
  rect->xmax += pad_x;
...
void KLI_rctf_resize_x(rctf *rect, float x)
{
  rect->xmin = KLI_rctf_cent_x(rect) - (x * 0.5f);
  rect->xmax = rect->xmin + x;
}

void KLI_rctf_resize_y(rctf *rect, float y)
{
  rect->ymin = KLI_rctf_cent_y(rect) - (y * 0.5f);
  rect->ymax = rect->ymin + y;
}

void KLI_rctf_resize(rctf *rect, float x, float y)
{
  rect->xmin = KLI_rctf_cent_x(rect) - (x * 0.5f);
  rect->ymin = KLI_rctf_cent_y(rect) - (y * 0.5f);
  rect->xmax = rect->xmin + x;
...
void KLI_rcti_scale(rcti *rect, const float scale)
{
  const int cent_x = KLI_rcti_cent_x(rect);
  const int cent_y = KLI_rcti_cent_y(rect);
  const int size_x_half = KLI_rcti_size_x(rect) * (scale * 0.5f);
  const int size_y_half = KLI_rcti_size_y(rect) * (scale * 0.5f);
...
void KLI_rctf_scale(rctf *rect, const float scale)
{
  const float cent_x = KLI_rctf_cent_x(rect);
  const float cent_y = KLI_rctf_cent_y(rect);
  const float size_x_half = KLI_rctf_size_x(rect) * (scale * 0.5f);
  const float size_y_half = KLI_rctf_size_y(rect) * (scale * 0.5f);
  rect->xmin = cent_x - size_x_half;
  rect->ymin = cent_y - size_y_half;
  rect->xmax = cent_x + size_x_half;
  rect->ymax = cent_y + size_y_half;
...
void KLI_rctf_pad_y(rctf *rect,
                    const float boundary_size,
                    const float pad_min,
                    const float pad_max)
{
  KLI_assert(pad_max >= 0.0f);
  KLI_assert(pad_min >= 0.0f);
  KLI_assert(boundary_size > 0.0f);
  ...
  float total_extend = KLI_rctf_size_y(rect) * total_pad / (boundary_size - total_pad);
  rect->ymax += total_extend * (pad_max / total_pad);
  rect->ymin -= total_extend * (pad_min / total_pad);
}

void KLI_rctf_interp(rctf *rect, const rctf *rect_a, const rctf *rect_b, const float fac)
{
...
/* KLI_rcti_interp() not needed yet */

bool KLI_rctf_clamp_pt_v(const rctf *rect, float xy[2])
{
  bool changed = false;
  if (xy[0] < rect->xmin) {
    xy[0] = rect->xmin;
...
bool KLI_rcti_clamp_pt_v(const rcti *rect, int xy[2])
{
  bool changed = false;
  if (xy[0] < rect->xmin) {
    xy[0] = rect->xmin;
...
bool KLI_rctf_clamp(rctf *rect, const rctf *rect_bounds, float r_xy[2])
{
  bool changed = false;
...
bool KLI_rctf_compare(const rctf *rect_a, const rctf *rect_b, const float limit)
{
...
bool KLI_rcti_compare(const rcti *rect_a, const rcti *rect_b)
{
...
bool KLI_rcti_isect_rect_x(const rcti *src1, const rcti *src2, int range_x[2])
{
...
bool KLI_rcti_isect_rect_y(const rcti *src1, const rcti *src2, int range_y[2])
{
...
void KLI_rcti_rctf_copy(rcti *dst, const rctf *src)
{
  ...
  dst->xmax = dst->xmin + floorf(KLI_rctf_size_x(src) + 0.5f);
  dst->ymin = floorf(src->ymin + 0.5f);
  dst->ymax = dst->ymin + floorf(KLI_rctf_size_y(src) + 0.5f);
}
...
void KLI_rcti_rctf_copy_round(rcti *dst, const rctf *src)
{
  dst->xmin = floorf(src->xmin + 0.5f);
...
void print_rctf(const char *str, const rctf *rect)
{
  printf("%s: xmin %.8f, xmax %.8f, ymin %.8f, ymax %.8f (%.12fx%.12f)\n",
         ...
         rect->ymin,
         rect->ymax,
         KLI_rctf_size_x(rect),
         KLI_rctf_size_y(rect));
}
...
void print_rcti(const char *str, const rcti *rect)
{
  printf("%s: xmin %d, xmax %d, ymin %d, ymax %d (%dx%d)\n",
         ...
         rect->ymin,
         rect->ymax,
         KLI_rcti_size_x(rect),
         KLI_rcti_size_y(rect));
}
...
void KLI_rctf_rotate_expand(rctf *dst, const rctf *src, const float angle)
{
  const float mat2[2] = {sinf(angle), cosf(angle)};
  const float cent[2] = {KLI_rctf_cent_x(src), KLI_rctf_cent_y(src)};
