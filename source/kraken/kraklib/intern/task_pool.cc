...
static void tbb_task_pool_create(TaskPool *pool, eTaskPriority priority)
{
  if (pool->type == TASK_POOL_TBB_SUSPENDED) {
    ...
    pool->suspended_mempool = KLI_mempool_create(sizeof(Task), 512, 512, KLI_MEMPOOL_ALLOW_ITER);
  }
...
static void tbb_task_pool_run(TaskPool *pool, Task &&task)
{
  if (pool->is_suspended) {
    ...
    Task *task_mem = (Task *)KLI_mempool_alloc(pool->suspended_mempool);
...
static void tbb_task_pool_work_and_wait(TaskPool *pool)
{
  ...
  if (pool->suspended_mempool) {
    ...
    KLI_mempool_iternew(pool->suspended_mempool, &iter);
    while (Task *task = (Task *)KLI_mempool_iterstep(&iter)) {
      tbb_task_pool_run(pool, std::move(*task));
    }

    KLI_mempool_clear(pool->suspended_mempool);
  }
...
static void tbb_task_pool_free(TaskPool *pool)
{
  ...
  if (pool->suspended_mempool) {
    KLI_mempool_destroy(pool->suspended_mempool);
  }
}
...
static void *background_task_run(void *userdata)
{
  ...
  while (Task *task = (Task *)KLI_thread_queue_pop(pool->background_queue)) {
    (*task)();
    task->~Task();
    MEM_freeN(task);
  }
...
{
  pool->background_queue = KLI_thread_queue_init();
  KLI_threadpool_init(&pool->background_threads, background_task_run, 1);
}
...
static void background_task_pool_run(TaskPool *pool, Task &&task)
{
  ...
  new (task_mem) Task(std::move(task));
  KLI_thread_queue_push(pool->background_queue, task_mem);

  if (KLI_available_threads(&pool->background_threads)) {
    KLI_threadpool_insert(&pool->background_threads, pool);
  }
}
...
static void background_task_pool_work_and_wait(TaskPool *pool)
{
  ...
  KLI_thread_queue_nowait(pool->background_queue);
  KLI_thread_queue_wait_finish(pool->background_queue);
  KLI_threadpool_clear(&pool->background_threads);
}
...
static void background_task_pool_cancel(TaskPool *pool)
{
  ...
  KLI_threadpool_remove(&pool->background_threads, pool);
...
static void background_task_pool_free(TaskPool *pool)
{
  ...
  KLI_threadpool_end(&pool->background_threads);
  KLI_thread_queue_free(pool->background_queue);
}
...
{
  const bool use_threads = KLI_task_scheduler_num_threads() > 1 && type != TASK_POOL_NO_THREADS;
  ...
  KLI_mutex_init(&pool->user_mutex);
 ...
 * In multi-threaded context, there is no differences with #KLI_task_pool_create(),
 * but in single-threaded case it is ensured to have at least one worker thread to run on
 * (i.e. you don't have to call #KLI_task_pool_work_and_wait
...
/**
 * Similar to KLI_task_pool_create() but does not schedule any tasks for execution
 * for until KLI_task_pool_work_and_wait() is called. This helps reducing threading
...
bool KLI_task_pool_current_canceled(TaskPool *pool)
{
  ...
  KLI_assert_msg(0, "KLI_task_pool_canceled: Control flow should not come here!");
  return false;
}
