...
MINLINE int compare_ff_relative(float a, float b, const float max_diff, const int max_ulps)
{
  ...
  KLI_assert(sizeof(float) == sizeof(int));
  KLI_assert(max_ulps < (1 << 22));
 ...
 *
 * NOTE: Are to be called ONLY from inside `#ifdef KLI_HAVE_SSE2` !!!
 */
 ...
 */
MALWAYS_INLINE __m128 _kli_math_fastpow(const int exp, const int e2coeff, const __m128 arg)
{
  __m128 ret;
...
MALWAYS_INLINE __m128 _kli_math_improve_5throot_solution(const __m128 old_result, const __m128 x)
{
...
MALWAYS_INLINE __m128 _kli_math_fastpow24(const __m128 arg)
{
  ...
  __m128 x = _kli_math_fastpow(0x3F4CCCCD, 0x4F55A7FB, arg);
  __m128 arg2 = _mm_mul_ps(arg, arg);
  __m128 arg4 = _mm_mul_ps(arg2, arg2);
  /* error max = 0.018        avg = 0.0031    |avg| = 0.0031 */
  x = _kli_math_improve_5throot_solution(x, arg4);
...
MALWAYS_INLINE __m128 _kli_math_rsqrt(__m128 in)
{
...
MALWAYS_INLINE __m128 _kli_math_fastpow512(const __m128 arg)
{
  /* 5/12 is too small, so compute the 4th root of 20/12 instead.
   * 20/12 = 5/3 = 1 + 2/3 = 2 - 1/3. 2/3 is a suitable argument for fastpow.
   * weighting coefficient: a^-1/2 = 2 a; a = 2^-2/3
   */
  __m128 xf = _kli_math_fastpow(0x3f2aaaab, 0x5eb504f3, arg);
  __m128 xover = _mm_mul_ps(arg, xf);
  __m128 xfm1 = _kli_math_rsqrt(xf);
  __m128 x2 = _mm_mul_ps(arg, arg);
  __m128 xunder = _mm_mul_ps(x2, xfm1);
  /* sqrt2 * over + 2 * sqrt2 * under */
  __m128 xavg = _mm_mul_ps(_mm_set1_ps(1.0f / (3.0f * 0.629960524947437f) * 0.999852f),
                           _mm_add_ps(xover, xunder));
  xavg = _mm_mul_ps(xavg, _kli_math_rsqrt(xavg));
  xavg = _mm_mul_ps(xavg, _kli_math_rsqrt(xavg));
  return xavg;
}

MALWAYS_INLINE __m128 _kli_math_blend_sse(const __m128 mask, const __m128 a, const __m128 b)
{
