...
DynStr *KLI_dynstr_new_memarena(void)
{
  DynStr *ds = MEM_mallocN(sizeof(*ds), "DynStr");
  ds->elems = ds->last = NULL;
  ds->curlen = 0;
  ds->memarena = KLI_memarena_new(KLI_MEMARENA_STD_BUFSIZE, __func__);

  return ds;
}

KLI_INLINE void *dynstr_alloc(DynStr *__restrict ds, size_t size)
{
  return ds->memarena ? KLI_memarena_alloc(ds->memarena, size) : malloc(size);
}

void KLI_dynstr_append(DynStr *__restrict ds, const char *cstr)
{
...
void KLI_dynstr_nappend(DynStr *__restrict ds, const char *cstr, int len)
{
  DynStrElem *dse = dynstr_alloc(ds, sizeof(*dse));
  int cstrlen = KLI_strnlen(cstr, len);
...
void KLI_dynstr_vappendf(DynStr *__restrict ds, const char *__restrict format, va_list args)
{
  ...
  while (1) {
    ...
    if (len == sizeof(fixedmessage)) {
      ...
      message = MEM_callocN(sizeof(char) * len, "KLI_dynstr_appendf");
    }
    ...
    if (retval == -1) {
      ...
      len *= 2;
      if (len > maxlen) {
        fprintf(stderr, "KLI_dynstr_append text too long or format error.\n");
        break;
      }
    } else if (retval >= len) {
...
void KLI_dynstr_appendf(DynStr *__restrict ds, const char *__restrict format, ...)
{
  ...
  /* note that it's tempting to just call KLI_dynstr_vappendf here
...
int KLI_dynstr_get_len(const DynStr *ds)
{
  return ds->curlen;
}

void KLI_dynstr_get_cstring_ex(const DynStr *__restrict ds, char *__restrict rets)
{
  char *s;
  ...
  KLI_assert((s - rets) == ds->curlen);
  rets[ds->curlen] = '\0';
}

char *KLI_dynstr_get_cstring(const DynStr *ds)
{
  char *rets = MEM_mallocN(ds->curlen + 1, "dynstr_cstring");
  KLI_dynstr_get_cstring_ex(ds, rets);
  return rets;
}

void KLI_dynstr_clear(DynStr *ds)
{
  if (ds->memarena) {
    KLI_memarena_clear(ds->memarena);
  } else {
...
void KLI_dynstr_free(DynStr *ds)
{
  if (ds->memarena) {
    KLI_memarena_free(ds->memarena);
  } else {
    KLI_dynstr_clear(ds);
  }
