...
#ifndef __KLI_MATH_COLOR_INLINE_C__
#  define __KLI_MATH_COLOR_INLINE_C__
...
extern float KLI_color_from_srgb_table[256];
extern unsigned short KLI_color_to_srgb_table[0x10000];
...
#  ifdef __BIG_ENDIAN__
  return KLI_color_to_srgb_table[tmp.us[0]];
#  else
  return KLI_color_to_srgb_table[tmp.us[1]];
#  endif
}
...
{
  linear[0] = KLI_color_from_srgb_table[srgb[0]];
  linear[1] = KLI_color_from_srgb_table[srgb[1]];
  linear[2] = KLI_color_from_srgb_table[srgb[2]];
...
#endif /* __KLI_MATH_COLOR_INLINE_C__ */
