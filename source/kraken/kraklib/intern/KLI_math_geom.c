                         ...
                         const float b2[2])
{
  ...
  if (min_dist_sq == dist_sq1) {
    ...
    *r_lambda_b = 0.0f;
    ...
    KLI_assert(min_dist_sq == dist_sq4);
...
bool isect_ray_ray_epsilon_v3(const float ray_origin_a[3],
                              ...
                              float *r_lambda_b)
{
  KLI_assert(r_lambda_a || r_lambda_b);
  float n[3];
    ...
    const float p[3], const float v1[3], const float v2[3], const float v3[3], float r_isect_co[3])
{
  if (isect_point_tri_prism_v3(p, v1, v2, v3)) {
    ...
    KLI_assert(len_squared_v3(no) != 0.0f);
...
{
  KLI_ASSERT_UNIT_V3(normal);

  copy_v3_v3(r_mat[2], normal);
  ortho_basis_v3v3_v3(r_mat[0], r_mat[1], r_mat[2]);

  KLI_ASSERT_UNIT_V3(r_mat[0]);
  KLI_ASSERT_UNIT_V3(r_mat[1]);

  transpose_m3(r_mat);

  KLI_assert(!is_negative_m3(r_mat));
  KLI_assert((fabsf(dot_m3_v3_row_z(r_mat, normal) - 1.0f) < KLI_ASSERT_UNIT_EPSILON) ||
             is_zero_v3(normal));
}
...
void axis_dominant_v3_to_m3_negate(float r_mat[3][3], const float normal[3])
{
  ...
  KLI_assert(!is_negative_m3(r_mat));
  KLI_assert((dot_m3_v3_row_z(r_mat, normal) < KLI_ASSERT_UNIT_EPSILON) || is_zero_v3(normal));
}
...
int interp_sparse_array(float *array, const int list_size, const float skipval)
{
  ...
  float *array_up = calloc((size_t)list_size, sizeof(float));
  float *array_down = calloc((size_t)list_size, sizeof(float));

  int *ofs_tot_up = calloc((size_t)list_size, sizeof(int));
  int *ofs_tot_down = calloc((size_t)list_size, sizeof(int));
...
{
  KLI_ASSERT_UNIT_V3(tan_l);
  KLI_ASSERT_UNIT_V3(tan_r);
