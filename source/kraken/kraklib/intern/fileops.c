...
bool KLI_file_is_writable(const char *filepath)
{
  bool writable;
  if (KLI_access(filepath, W_OK) == 0) {
    ...
    KLI_split_dirfile(filepath, parent, NULL, sizeof(parent), 0);
#ifdef WIN32
    /* windows does not have X_OK */
    writable = KLI_access(parent, W_OK) == 0;
#else
    writable = KLI_access(parent, X_OK | W_OK) == 0;
#endif
  }
  return writable;
}

bool KLI_file_touch(const char *file)
{
  FILE *f = KLI_fopen(file, "r+b");
...
#  else
  {
    char short_name[256];
    KLI_get_short_name(short_name, filepath);
...
static bool delete_recursive(const char *dir)
{
  ...
  i = filelist_num = KLI_filelist_dir_contents(dir, &filelist);
  fl = filelist;
  while (i--) {
  ...
  KLI_filelist_free(filelist, filelist_num);
 ...
 */
int KLI_delete_soft(const char *file, const char **error_message)
{
  int err;
...
/* Not used anywhere! */
#  if 0
int KLI_move(const char *file, const char *to)
{
  ...
  KLI_strncpy(str, to, sizeof(str));
  /* points 'to' to a directory ? */
  if (KLI_path_slash_rfind(str) == (str + strlen(str) - 1)) {
    if (KLI_path_slash_rfind(file) != NULL) {
      strcat(str, KLI_path_slash_rfind(file) + 1);
    }
  }
...
int KLI_copy(const char *file, const char *to)
{
  char str[MAXPATHLEN + 12];
  int err;
...
#  if 0
int KLI_create_symlink(const char *file, const char *to)
{
...
int KLI_rename(const char *from, const char *to)
{
  if (!KLI_exists(from)) {
    return 0;
  }
  ...
  if (KLI_exists(to) && KLI_strcasecmp(from, to)) {
    if (KLI_delete(to, false, false)) {
      return 1;
    }
  }
...
#  ifdef __HAIKU__
      {
        ...
        KLI_path_join(filepath, sizeof(filepath), startfrom, dirent->d_name, NULL);
...
static const char *check_destination(const char *file, const char *to)
{
  ...
  if (!stat(to, &st)) {
    if (S_ISDIR(st.st_mode)) {
      ...
      filename = KLI_path_slash_rfind(str);
      ...
      KLI_join_dirfile(path, len + 1, to, filename);
