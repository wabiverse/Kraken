...
void rgb_to_yuv(float r, float g, float b, float *r_y, float *r_u, float *r_v, int colorspace)
{
  ...
  switch (colorspace) {
    case KLI_YUV_ITU_BT601:
      y = 0.299f * r + 0.587f * g + 0.114f * b;
      u = -0.147f * r - 0.289f * g + 0.436f * b;
      v = 0.615f * r - 0.515f * g - 0.100f * b;
      break;
    case KLI_YUV_ITU_BT709:
    default:
      KLI_assert(colorspace == KLI_YUV_ITU_BT709);
...
void ycc_to_rgb(float y, float cb, float cr, float *r_r, float *r_g, float *r_b, int colorspace)
{
   ...
   * KLI_YCC_ITU_BT601 y 16.0 cr 16.0 -> r -0.7009. */
...
float KLI_color_from_srgb_table[256];
ushort KLI_color_to_srgb_table[0x10000];
...
void KLI_init_srgb_conversion(void)
{
  ...
  for (i = 0; i < 0x10000; i++) {
    ...
    if (f <= 0) {
      KLI_color_to_srgb_table[i] = 0;
    } else if (f < 255) {
      KLI_color_to_srgb_table[i] = (ushort)(f * 0x100 + 0.5f);
    } else {
      KLI_color_to_srgb_table[i] = 0xff00;
    }
  }
  ...
  for (b = 0; b <= 255; b++) {
    ...
    KLI_color_from_srgb_table[b] = f;
    i = hipart(f);
    /* replace entries so byte->float->byte does not change the data: */
    KLI_color_to_srgb_table[i] = (ushort)(b * 0x100);
  }
}
