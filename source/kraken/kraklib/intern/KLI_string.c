...
char *KLI_strncpy_ensure_pad(char *__restrict dst,
                             ...
                             size_t maxncpy)
{
  ...
  if (src[0] == '\0') {
    ...
    srclen = KLI_strnlen(src, maxncpy);
...
char *KLI_sprintfN(const char *__restrict format, ...)
{
  DynStr *ds;
  va_list arg;
  char *n;
  ...
  ds = KLI_dynstr_new();
  KLI_dynstr_vappendf(ds, format, arg);
  n = KLI_dynstr_get_cstring(ds);
  KLI_dynstr_free(ds);
...
KLI_INLINE bool str_unescape_pair(char c_next, char *r_out)
{
...
size_t KLI_str_unescape_ex(char *__restrict dst,
                           const char *__restrict src,
                           const size_t src_maxncpy,
                           /* Additional arguments to #KLI_str_unescape */
...
const char *KLI_str_escape_find_quote(const char *str)
{
  bool escape = false;
...
bool KLI_str_quoted_substr_range(const char *__restrict str,
                                 ...
                                 int *__restrict r_end)
{
  ...
  KLI_assert_msg(prefix[prefix_len - 1] != '"',
  ...
  str_start += 1;
  const char *str_end = KLI_str_escape_find_quote(str_start);
 ...
 * happened to use a fixed size buffer, so only #KLI_str_quoted_substr is needed. */
#if 0
/**
 ...
 */
char *KLI_str_quoted_substrN(const char *__restrict str, const char *__restrict prefix)
{
  int start_match_ofs, end_match_ofs;
  if (!KLI_str_quoted_substr_range(str, prefix, &start_match_ofs, &end_match_ofs)) {
    return NULL;
  }
  const size_t escaped_len = (size_t)(end_match_ofs - start_match_ofs);
  char *result = malloc(sizeof(char) * (escaped_len + 1));
  const size_t unescaped_len = KLI_str_unescape(result, str + start_match_ofs, escaped_len);
...
bool KLI_str_quoted_substr(const char *__restrict str,
                           const char *__restrict prefix,
                           char *result,
                           size_t result_maxlen)
{
  ...
  const size_t escaped_len = (size_t)(end_match_ofs - start_match_ofs);
  bool is_complete;
  KLI_str_unescape_ex(result, str + start_match_ofs, escaped_len, result_maxlen, &is_complete);
...
char *KLI_str_replaceN(const char *__restrict str,
                       ...
                       const char *__restrict substr_new)
{
  ...
  while ((match = strstr(str, substr_old))) {
    ...
    if (str != match) {
      ...
      KLI_dynstr_nappend(ds, str, (match - str));
    ...
    KLI_dynstr_append(ds, substr_new);
  ...
  if (ds) {
    ...
    KLI_dynstr_append(ds, str);

    /* Convert to new c-string (MEM_malloc'd), and free the buffer. */
    str_new = KLI_dynstr_get_cstring(ds);
    KLI_dynstr_free(ds);
...
bool KLI_str_replace_table_exact(char *string,
                                 ...
                                 int replace_table_len)
{
  for (int i = 0; i < replace_table_len; i++) {
    if (STREQ(string, replace_table[i][0])) {
      KLI_strncpy(string, replace_table[i][1], string_len);
      return true;
    }
  }
  return false;
...
int KLI_strcaseeq(const char *a, const char *b)
{
  return (KLI_strcasecmp(a, b) == 0);
}
...
int KLI_string_max_possible_word_count(const int str_len)
{
  return (str_len / 2) + 1;
}

bool KLI_string_has_word_prefix(const char *haystack, const char *needle, size_t needle_len)
{
  const char *match = KLI_strncasestr(haystack, needle, needle_len);
  if (match) {
    if ((match == haystack) || (*(match - 1) == ' ') || ispunct(*(match - 1))) {
      return true;
    }
    return KLI_string_has_word_prefix(match + 1, needle, needle_len);
  }
  return false;
}

bool KLI_string_all_words_matched(const char *name,
                                  const char *str,
                                  int (*words)[2],
                                  const int words_len)
{
  ...
  for (index = 0; index < words_len; index++) {
    if (!KLI_string_has_word_prefix(name, str + words[index][0], (size_t)words[index][1])) {
      break;
    }
  }
...
char *KLI_strncasestr(const char *s, const char *find, size_t len)
{
  char c, sc;
  ...
  if ((c = *find++) != 0) {
    ...
    if (len > 1) {
      ...
      } while (KLI_strncasecmp(s, find, len - 1) != 0);
    }
    else {
      {
        do {
...
int KLI_strcmp_ignore_pad(const char *str1, const char *str2, const char pad)
{
...
int KLI_str_index_in_array_n(const char *__restrict str,
...
int KLI_str_index_in_array(const char *__restrict str, const char **__restrict str_array)
{
  int index;
  const char **str_iter = str_array;
...
bool KLI_str_startswith(const char *__restrict str, const char *__restrict start)
{
...
bool KLI_strn_endswith(const char *__restrict str, const char *__restrict end, size_t slength)
{
...
bool KLI_str_endswith(const char *__restrict str, const char *__restrict end)
{
  const size_t slength = strlen(str);
  return KLI_strn_endswith(str, end, slength);
}
...
char KLI_tolower_ascii(const char c)
{
  return (c >= 'A' && c <= 'Z') ? c + ('a' - 'A') : c;
}

char KLI_toupper_ascii(const char c)
{
  return (c >= 'a' && c <= 'z') ? c - ('a' - 'A') : c;
}

void KLI_str_tolower_ascii(char *str, const size_t len)
{
  size_t i;

  for (i = 0; (i < len) && str[i]; i++) {
    str[i] = KLI_tolower_ascii(str[i]);
  }
}

void KLI_str_toupper_ascii(char *str, const size_t len)
{
  size_t i;

  for (i = 0; (i < len) && str[i]; i++) {
    str[i] = KLI_toupper_ascii(str[i]);
  }
}
...
int KLI_str_rstrip_float_zero(char *str, const char pad)
{
  char *p = strchr(str, '.');
  int totstrip = 0;
  if (p) {
...
size_t KLI_str_partition(const char *str, const char delim[], const char **sep, const char **suf)
{
  return KLI_str_partition_ex(str, NULL, delim, sep, suf, false);
}

size_t KLI_str_rpartition(const char *str, const char delim[], const char **sep, const char **suf)
{
  return KLI_str_partition_ex(str, NULL, delim, sep, suf, true);
}

size_t KLI_str_partition_ex(const char *str,
                            const char *end,
                            const char delim[],
                            const char **sep,
                            const char **suf,
...
int KLI_string_find_split_words(
...
static size_t KLI_str_format_int_grouped_ex(char src[16], char dst[16], int num_len)
{
  char *p_src = src;
  char *p_dst = dst;
...
size_t KLI_str_format_int_grouped(char dst[16], int num)
{
  char src[16];
  int num_len = sprintf(src, "%d", num);

  return KLI_str_format_int_grouped_ex(src, dst, num_len);
}

size_t KLI_str_format_uint64_grouped(char dst[16], uint64_t num)
{
...
void KLI_str_format_byte_unit(char dst[15], long long int bytes, const bool base_10)
{
  ...
  KLI_STATIC_ASSERT(ARRAY_SIZE(units_base_2) == ARRAY_SIZE(units_base_10), "array size mismatch");
  ...
  size_t len = KLI_snprintf_rlen(dst, dst_len, "%.*f", decimals, bytes_converted);
  len -= (size_t)KLI_str_rstrip_float_zero(dst, '\0');
  dst[len++] = ' ';
  KLI_strncpy(dst + len, base_10 ? units_base_10[order] : units_base_2[order], dst_len - len);
}

void KLI_str_format_decimal_unit(char dst[7], int number_to_format)
{
  ...
  KLI_snprintf(dst, dst_len, "%.*f%s", decimals, number_to_format_converted, units[order]);
}
