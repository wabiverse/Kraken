 ...
 * - #KLI_SPACE_TRANSFORM_SETUP(&data,  ob1, ob2)
 *
 * After that the following calls can be used:
 * - Converts a coordinate in ob1 space to the corresponding ob2 space:
 *   #KLI_space_transform_apply(&data, co);
 * - Converts a coordinate in ob2 space to the corresponding ob1 space:
 *   #KLI_space_transform_invert(&data, co);
 *
 * Same concept as #KLI_space_transform_apply and #KLI_space_transform_invert,
 * but no is normalized after conversion (and not translated at all!):
 * - #KLI_space_transform_apply_normal(&data, no);
 * - #KLI_space_transform_invert_normal(&data, no);
 */
 ...
 */
void KLI_space_transform_from_matrices(struct SpaceTransform *data,
 ...
 */
void KLI_space_transform_global_from_matrices(struct SpaceTransform *data,
                                              const float local[4][4],
                                              const float target[4][4]);
void KLI_space_transform_apply(const struct SpaceTransform *data, float co[3]);
void KLI_space_transform_invert(const struct SpaceTransform *data, float co[3]);
void KLI_space_transform_apply_normal(const struct SpaceTransform *data, float no[3]);
void KLI_space_transform_invert_normal(const struct SpaceTransform *data, float no[3]);

#define KLI_SPACE_TRANSFORM_SETUP(data, local, target) \
  KLI_space_transform_from_matrices((data), (local)->obmat, (target)->obmat)
