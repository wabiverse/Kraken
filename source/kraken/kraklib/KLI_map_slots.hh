 ...
 * Gadget Vault.
 *
 * This file contains slot types that are supposed to be used with kraken::Map.
 *
...
namespace kraken
{
  ...
  template<typename Key, typename Value> class SimpleMapSlot
  {
   ...
   public:
     ...
     * key and value have to be made as well.
     */
    ...
    /**
     * The move constructor has to copy the state. If the other slot was occupied, the key and
     * value from the other have to moved as well. The other slot stays in the state it was in
     * before. Its optionally stored key and value remain in a moved-from state.
     */
    ...
    /**
     * Returns true if the slot is empty, i.e. it does not contain a key and is not in removed
     * state.
     */
    bool is_empty() const
    {
    ...
    /**
     * Returns the hash of the currently stored key. In this simple map slot implementation, we
     * just computed the hash here. Other implementations might store the hash in the slot instead.
     */
