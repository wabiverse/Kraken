...
namespace kraken
{
   ...
   *
   * Those should eventually be de-duplicated with functions in KLI_math_base.h.
   * \{ */
  ...
  {
    KLI_assert(x >= 0);
    return (is_power_of_2_constexpr(int(x))) ? log2_floor_constexpr(x) :
  ...
  {
    return power_of_2_max_constexpr(ceil_division_by_fraction(min_usable_slots,
  ...
  /** \name Load Factor
   *
   * This is an abstraction for a fractional load factor. The hash table using this class is
   * assumed to use arrays with a size that is a power of two.
   *
   * \{ */
  ...
  class LoadFactor
  {
   ...
   public:
    ...
    {
      KLI_assert(numerator > 0);
      KLI_assert(numerator < denominator);
    }
    ...
    {
      KLI_assert(is_power_of_2_i(int(min_total_slots)));
      ...
      const int64_t usable_slots = floor_multiplication_with_fraction(total_slots,
                                                                      numerator_,
                                                                      denominator_);
      KLI_assert(min_usable_slots <= usable_slots);
   ...
   *
   * One solution to deal with this problem is to embed the state information in the key. That
   * means, two values of the key type are selected to indicate whether the slot is empty or
   * removed.
   *
   ...
   *
   * Those specific values are used, because with them a single comparison is enough to check
   * whether a slot is occupied. The keys `0x0000...0000` and `0x0000...0001` also satisfy this
   * constraint. However, nullptr is much more likely to be used as valid key.
   */
   ...
   *
   * A utility class that makes it easier for hash table implementations to provide statistics to
   * the developer. These statistics can be helpful when trying to figure out why a hash table is
   * slow.
   *
  ...
  class HashTableStats
  {
   ...
   public:
      ...
      for (const auto &key : keys) {
        ...
        if (keys_by_collision_count_.size() <= collisions) {
          keys_by_collision_count_.append_n_times(0,
                                                  collisions - keys_by_collision_count_.size() +
                                                    1);
        }
    ...
    void print(StringRef name = "")
    {
      ...
      KLI_str_format_byte_unit(memory_size_str, size_in_bytes_, true);
