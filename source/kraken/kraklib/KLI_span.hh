 ...
 * \ingroup bli
 *
 * An `kraken::Span<T>` references an array that is owned by someone else. It is just a
 ...
 *
 * There is also `kraken::MutableSpan<T>`. It is mostly the same as Span, but allows the
 * array to be modified.
 *
 * A (Mutable)Span can refer to data owned by many different data structures including
 * kraken::Vector, kraken::Array, kraken::VectorSet, std::vector, std::array, std::string,
 * std::initializer_list and c-style array.
 *
 * `kraken::Span` is very similar to `std::span` (C++20). However, there are a few differences:
 * - `kraken::Span` is const by default. This is to avoid making things mutable when they don't
 *   have to be. To get a non-const span, you need to use `kraken::MutableSpan`. Below is a list
 *   of const-behavior-equivalent pairs of data structures:
 *   - std::span<int>                <==>  kraken::MutableSpan<int>
 *   - std::span<const int>          <==>  kraken::Span<int>
 *   - std::span<int *>              <==>  kraken::MutableSpan<int *>
 *   - std::span<const int *>        <==>  kraken::MutableSpan<const int *>
 *   - std::span<int * const>        <==>  kraken::Span<int *>
 *   - std::span<const int * const>  <==>  kraken::Span<const int *>
 * - `kraken::Span` always has a dynamic extent, while `std::span` can have a size that is
 *   determined at compile time. I did not have a use case for that yet. If we need it, we can
 *   decide to add this functionality to `kraken::Span` or introduce a new type like
 *   `kraken::FixedSpan<T, N>`.
 *
 * `kraken::Span<T>` should be your default choice when you have to pass a read-only array
 ...
 *
 * `kraken::MutableSpan<T>` can be used when a function is supposed to return an array, the
...
template<typename T> class Span {
 ...
 public:
  ...
  template<typename U, KLI_ENABLE_IF((is_span_convertible_pointer_v<U, T>))>
  ...
  {
    KLI_assert((size_ * sizeof(T)) % sizeof(NewT) == 0);
...
/** This is defined here, because in `KLI_index_range.hh` `Span` is not yet defined. */
