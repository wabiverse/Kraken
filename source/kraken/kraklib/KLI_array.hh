 ...
 * Gadget Vault.
 *
 * A `kraken::Array<T>` is a container for a fixed size array the size of which is NOT known at
 * compile time.
 *
 * If the size is known at compile time, `std::array<T, N>` should be used instead.
 *
 * kraken::Array should usually be used instead of kraken::Vector whenever the number of elements
 * is known at construction time. Note however, that kraken::Array will default construct all
 ...
 *
 * kraken::Array supports small object optimization to improve performance when the size turns out
