 ...
 * implementations like kraken::Set and kraken::Map. A probing strategy produces a sequence of
...
namespace kraken
{
  ...
  /**
   * The simplest probing strategy. It's bad in most cases, because it produces clusters in the
   * hash table, which result in many collisions. However, if the hash function is very good or the
   * hash table is small, this strategy might even work best.
   */
   ...
   *
   * In the first few steps, this strategy can have good cache performance. It largely depends on
   * how many keys fit into a cache line in the hash table.
   */
   ...
   *
   * It is very fast when the original hash value is good. If there are collisions, more bits of
   * the hash value are taken into account.
   *
   * LinearSteps: Can be set to something larger than 1 for improved cache performance in some
   * cases. PreShuffle: When true, the initial call to next() will be done to the constructor. This
   * can help when the hash function has put little information into the lower bits.
   */
