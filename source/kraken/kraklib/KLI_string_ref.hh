 ...
 * \ingroup bli
 *
 * A `kraken::StringRef` references a const char array owned by someone else. It is just a pointer
 ...
 * null-terminated strings, `kraken::StringRefNull` can be used. It is essentially the same as
 ...
 *
 * kraken::StringRef vs. std::string_view:
...
constexpr StringRef StringRefBase::trim(StringRef characters_to_remove) const
{
   ...
   * been `not_found`. */
  KLI_assert_msg(find_end != not_found,
...
{
  KLI_assert(static_cast<int64_t>(strlen(str)) == size);
}
...
{
  KLI_assert(str != nullptr);
  KLI_assert(data_[size_] == '\0');
}
...
{
  KLI_assert(this->startswith(prefix));
...
{
  KLI_assert(begin <= one_after_end);
}
