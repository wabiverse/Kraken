 ...
 * Gadget Vault.
 *
 * A specialization of `kraken::DefaultHash<T>` provides a hash function for values of type T.
 ...
 *
 * Hash table implementations like kraken::Set support heterogeneous key lookups. That means that
 ...
 *   either global or KLI namespace.
 *
 *     template<> struct kraken::DefaultHash<TheType> {
...
namespace kraken
{
   ...
   * on the type itself. Implementing a `hash()` method on a type is often significantly easier
   * than specializing #DefaultHash.
   *
  ...
  template<typename T> struct DefaultHash
  {
    ...
    {
      /* Try calling the static `T::hash_as(value)` function with the given value. The returned
       * hash should be "compatible" with `T::hash()`. Usually that means that if `value` is
       * converted to `T` its hash does not change. */
      /* If this results in a compiler error, no hash function for the heterogeneous lookup has
       * been found. */
...
#define TRIVIAL_DEFAULT_INT_HASH(TYPE)    \
  ...
  /**
   * We cannot make any assumptions about the distribution of keys, so use a trivial hash function
   * by default. The default probing strategy is designed to take all bits of the hash into account
  ...
  {
    /**
     * Take a #StringRef as parameter to support heterogeneous lookups in hash table
     * implementations when std::string is used as key.
     */
