 ...
 * Gadget Vault.
 *
 * This file contains different slot types that are supposed to be used with kraken::Set.
 *
...
namespace kraken
{
  ...
  template<typename Key> class SimpleSetSlot
  {
   ...
   public:
    ...
    /**
     * The copy constructor has to copy the state. If the other slot was occupied, a copy of the
     * key has to be made as well.
     */
    ...
    /**
     * Return true if the slot is empty, i.e. it does not contain a key and is not in removed
     * state.
     */
    bool is_empty() const
    {
    ...
    /**
     * Change the state of this slot from occupied to removed. The key has to be destructed as
     * well.
     */
    void remove()
    {
  ...
  /**
   * An IntrusiveSetSlot uses two special values of the key to indicate whether the slot is empty
   * or removed. This saves some memory in all cases and is more efficient in many cases. The
   * KeyInfo type indicates which specific values are used. An example for a KeyInfo implementation
   * is PointerKeyInfo.
   *
  ...
  template<typename Key, typename KeyInfo> class IntrusiveSetSlot
  {
   ...
   public:
    ...
    IntrusiveSetSlot(IntrusiveSetSlot &&other) noexcept(
      std::is_nothrow_move_constructible_v<Key>) = default;
    ...
    bool contains(const ForwardKey &key,
                  ...
                  const uint64_t UNUSED(hash)) const
    {
      KLI_assert(KeyInfo::is_not_empty_or_removed(key));
  ...
  /**
   * Use SimpleSetSlot by default, because it is the smallest slot type that works for all key
   * types.
   */
  template<typename Key> struct DefaultSetSlot
  {
