...
#ifndef NDEBUG
#  define KLI_ASSERT_UNIT(v) \
    { \
      const float _test_unit = length_squared(v); \
      KLI_assert(!(std::abs(_test_unit - 1.0f) >= KLI_ASSERT_UNIT_EPSILON) || \
                 !(std::abs(_test_unit) >= KLI_ASSERT_UNIT_EPSILON)); \
    } \
    (void)0
#else
#  define KLI_ASSERT_UNIT(v) (void)(v)
#endif
...
template<typename T, int Size, KLI_ENABLE_IF((is_math_float_type<T>))>
 ...
 */
template<typename T, int Size, KLI_ENABLE_IF((is_math_integral_type<T>))>
...
template<typename T, typename FactorT, int Size, KLI_ENABLE_IF((is_math_float_type<FactorT>))>
