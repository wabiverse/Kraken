...
/**
 * @file
 * @ingroup Kraken Vector.
 *
 * A `kraken::Vector<T>` is a dynamically growing contiguous array for values of type T. It is
 ...
 * allocation is done. As a consequence, iterators are invalidated when a kraken::Vector is moved
 * (iterators of std::vector remain valid when the vector is moved).
 *
 * `kraken::Vector` should be your default choice for a vector data structure in Kraken.
 */
...
namespace kraken
{
  ...
  class Vector
  {
   ...
   private:
    ...
    KLI_NO_UNIQUE_ADDRESS Allocator allocator_;

    /** A placeholder buffer that will remain uninitialized until it is used. */
    KLI_NO_UNIQUE_ADDRESS TypedBuffer<T, InlineBufferCapacity> inline_buffer_;
...
#  define UPDATE_VECTOR_SIZE(ptr) ((void)0)
   ...
   public:
    ...
    template<typename U, size_t N, KLI_ENABLE_IF((std::is_convertible_v<U, T>))>
    ...
    template<typename InputIt,
             ...
             KLI_ENABLE_IF((!std::is_convertible_v<InputIt, int>))>
    ...
    /**
     * Create a copy of another vector. The other vector will not be changed. If the other vector
     * has less than InlineBufferCapacity elements, no allocation will be made.
     */
    ...
    /**
     * Steal the elements from another vector. This does not do an allocation. The other vector
     * will have zero elements afterwards.
     */
    template<int64_t OtherInlineBufferCapacity>
    ...
    template<typename U, KLI_ENABLE_IF((is_span_convertible_pointer_v<T, U>))>
    ...
    {
      KLI_assert(end_ + n <= capacity_end_);
      end_ += n;
      UPDATE_VECTOR_SIZE(this);
    }
     ...
     * operation when the vector is large, but can be very cheap when it is known that the vector
     * is small.
     */
    ...
    {
      KLI_assert(amount >= 0);
      KLI_assert(begin_ + amount <= capacity_end_);
     ...
     * where n is the number of values that have to be moved. Undefined behavior is invoked when
     * the insert position is out of bounds.
     */
    ...
    {
      KLI_assert(insert_index >= 0);
      KLI_assert(insert_index <= this->size());
    ...
    int64_t size() const
    {
      ...
      KLI_assert(debug_size_ == current_size);
      return current_size;
    }
    ...
    /**
     * Destructs the last element and decreases the size by one. This invokes undefined behavior
     * when the vector is empty.
     */
    void remove_last()
    {
      KLI_assert(!this->is_empty());
      end_--;
      end_->~T();
      UPDATE_VECTOR_SIZE(this);
    }
    ...
    /**
     * Finds the first occurrence of the value, removes it and copies the last element to the hole
     * in the vector. This takes O(n) time.
     */
     ...
     * front. This takes O(n) time. If the order is not important, remove_and_reorder should be
     * used instead.
     *
    ...
    {
      const int64_t old_size = this->size();
      KLI_assert(start_index >= 0);
      KLI_assert(amount >= 0);
      KLI_assert(start_index + amount <= old_size);
    ...
    void print_stats(StringRef name = "") const
    {
      ...
      KLI_str_format_byte_unit(memory_size_str, sizeof(*this), true);
   ...
   private:
    ...
    KLI_NOINLINE void realloc_to_at_least(const int64_t min_capacity)
    {
      if (this->capacity() >= min_capacity) {
        return;
      }

      /* At least double the size of the previous allocation. Otherwise consecutive calls to grow
       * can cause a reallocation every time even though min_capacity only increments. */
...
#undef UPDATE_VECTOR_SIZE
  ...
  /**
   * Same as a normal Vector, but does not use Kraken's guarded allocator. This is useful when
