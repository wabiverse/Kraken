 ...
 */
void KLI_setenv(const char *env, const char *val) ATTR_NONNULL(1);
/**
 ...
 * (not used anywhere).
 */
void KLI_setenv_if_new(const char *env, const char *val) ATTR_NONNULL(1);
/**
 ...
 */
const char *KLI_getenv(const char *env) ATTR_NONNULL(1) ATTR_WARN_UNUSED_RESULT;
 ...
 */
void KLI_split_dirfile(const char *string, char *dir, char *file, size_t dirlen, size_t filelen);
/**
 * Copies the parent directory part of string into `dir`, max length `dirlen`.
 */
void KLI_split_dir_part(const char *string, char *dir, size_t dirlen);
/**
 * Copies the leaf filename part of string into `file`, max length `filelen`.
 */
void KLI_split_file_part(const char *string, char *file, size_t filelen);

/**
 * Returns a pointer to the last extension (e.g. the position of the last period).
 * Returns NULL if there is no extension.
 */
const char *KLI_path_extension(const char *filepath) ATTR_NONNULL();

/**
 * Append a filename to a dir, ensuring slash separates.
 */
void KLI_path_append(char *__restrict dst, size_t maxlen, const char *__restrict file)
  ATTR_NONNULL();
/**
 * Simple appending of filename to dir, does not check for valid path!
 * Puts result into `dst`, which may be same area as `dir`.
 *
 * \note Consider using #KLI_path_join for more general path joining
 ...
 */
size_t KLI_path_join(char *__restrict dst, size_t dst_len, const char *path, ...)
...
bool KLI_path_contains(const char *container_path,
 ...
 */
const char *KLI_path_slash_find(const char *string) ATTR_NONNULL() ATTR_WARN_UNUSED_RESULT;
/**
 ...
 */
void KLI_path_slash_native(char *path) ATTR_NONNULL();

#ifdef _WIN32
bool KLI_path_program_extensions_add_win32(char *name, size_t maxlen);
#endif
/**
 * Search for a binary (executable)
 */
bool KLI_path_program_search(char *fullname, size_t maxlen, const char *name);
 ...
 */
bool KLI_path_extension_glob_validate(char *ext_fnmatch) ATTR_NONNULL();
/**
 * Removes any existing extension on the end of \a path and appends \a ext.
 * \return false if there was no room.
 */
bool KLI_path_extension_replace(char *path, size_t maxlen, const char *ext) ATTR_NONNULL();
/**
 * Strip's trailing '.'s and adds the extension only when needed
 */
bool KLI_path_extension_ensure(char *path, size_t maxlen, const char *ext) ATTR_NONNULL();
/**
 ...
 */
bool KLI_path_filename_ensure(char *filepath, size_t maxlen, const char *filename) ATTR_NONNULL();
/**
 ...
 */
int KLI_path_sequence_decode(const char *string,
                             char *head,
                             char *tail,
 ...
 *
 * \note Same as #KLI_path_normalize but adds a trailing slash.
 */
void KLI_path_normalize_dir(const char *relabase, char *dir) ATTR_NONNULL(2);
 ...
 */
bool KLI_filename_make_safe_ex(char *fname, bool allow_tokens) ATTR_NONNULL(1);
bool KLI_filename_make_safe(char *fname) ATTR_NONNULL(1);
 ...
 */
bool KLI_path_make_safe(char *path) ATTR_NONNULL(1);
 ...
 */
bool KLI_path_abs(char *path, const char *basepath) ATTR_NONNULL();
/**
 * Replaces "#" character sequence in last slash-separated component of `path`
 * with frame as decimal integer, with leading zeroes as necessary, to make digits.
 */
bool KLI_path_frame(char *path, int frame, int digits) ATTR_NONNULL();
/**
 * Replaces "#" character sequence in last slash-separated component of `path`
 * with sta and end as decimal integers, with leading zeroes as necessary, to make digits
 * digits each, with a hyphen in-between.
 */
bool KLI_path_frame_range(char *path, int sta, int end, int digits) ATTR_NONNULL();
/**
 * Get the frame from a filename formatted by kraken's frame scheme
 */
bool KLI_path_frame_get(char *path, int *r_frame, int *r_digits_len) ATTR_NONNULL();
/**
 ...
 */
void KLI_path_frame_strip(char *path, char *r_ext) ATTR_NONNULL();
/**
 * Check if we have '#' chars, usable for #KLI_path_frame, #KLI_path_frame_range
 */
bool KLI_path_frame_check_chars(const char *path) ATTR_NONNULL(1) ATTR_WARN_UNUSED_RESULT;
/**
 * Checks for a relative path (ignoring Blender's "//") prefix
 * (unlike `!KLI_path_is_rel(path)`).
 * When false, #KLI_path_abs_from_cwd would expand the absolute path.
 */
bool KLI_path_is_abs_from_cwd(const char *path) ATTR_NONNULL(1) ATTR_WARN_UNUSED_RESULT;
/**
 ...
 * In most cases #KLI_path_abs should be used instead.
 */
bool KLI_path_abs_from_cwd(char *path, size_t maxlen) ATTR_NONNULL();
/**
 * Replaces `file` with a relative version (prefixed by "//") such that #KLI_path_abs, given
 * the same `relfile`, will convert it back to its original value.
 */
void KLI_path_rel(char *file, const char *relfile) ATTR_NONNULL();
 ...
 */
bool KLI_path_is_unc(const char *path) ATTR_NONNULL(1) ATTR_WARN_UNUSED_RESULT;

/**
 * Creates a display string from path to be used menus and the user interface.
 * Like `bpy.path.display_name()`.
 */
void KLI_path_to_display_name(char *display_name, int maxlen, const char *name) ATTR_NONNULL();

#if defined(WIN32)
void KLI_path_normalize_unc_16(wchar_t *path_16);
void KLI_path_normalize_unc(char *path_16, int maxlen);
#endif
...
/**
 * Returns the result of #KLI_path_cmp with both paths normalized and slashes made native.
 *
 * \note #KLI_path_cmp is used for Blender's internal logic to consider paths to be the same
 * #KLI_path_cmp_normalized may be used in when handling other kinds of paths
