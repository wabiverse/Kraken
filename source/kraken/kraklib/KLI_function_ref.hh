...
namespace kraken
{
  ...
  template<typename Ret, typename... Params> class FunctionRef<Ret(Params...)>
  {
   private:
     ...
     * any other callable.
     *
     * The value does not need to be initialized because it is not used unless `callback_` is set
     * as well, in which case it will be initialized as well.
     *
   ...
   public:
     ...
     *
     * This invokes undefined behavior if the `FunctionRef` does not reference a function
     * currently.
     */
    Ret operator()(Params... params) const
    {
      KLI_assert(callback_ != nullptr);
    ...
    operator bool() const
    {
      /* Just checking `callback_` is enough to determine if the `FunctionRef` is in a state that
       * it can be called in. */
