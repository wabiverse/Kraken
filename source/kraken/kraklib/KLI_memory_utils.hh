...
namespace kraken
{
  ...
  /**
   * Call the default constructor on n consecutive elements. For trivially constructible types,
   * this does nothing.
   *
  ...
  template<size_t Size, size_t Alignment> class AlignedBuffer
  {
    ...
    KLI_NO_UNIQUE_ADDRESS BufferType buffer_;
  ...
  template<typename T, int64_t Size = 1> class TypedBuffer
  {
   private:
    ...
    KLI_NO_UNIQUE_ADDRESS AlignedBuffer<sizeof(T) * (size_t)Size, alignof(T)> buffer_;
  ...
  /**
   * This can be used by container constructors. A parameter of this type should be used to
   * indicate that the constructor does not construct the elements.
   */
   ...
   * It assumes that the container has an exception-safe move-constructor and a noexcept
   * constructor tagged with the NoExceptConstructor tag.
   */
  template<typename Container>
...
#define KLI_SCOPED_DEFER_NAME1(a, b) a##b
#define KLI_SCOPED_DEFER_NAME2(a, b) KLI_SCOPED_DEFER_NAME1(a, b)
#define KLI_SCOPED_DEFER_NAME(a) KLI_SCOPED_DEFER_NAME2(_scoped_defer_##a##_, __LINE__)
 ...
 */
#define KLI_SCOPED_DEFER(function_to_defer)                                                       \
  auto KLI_SCOPED_DEFER_NAME(func) = (function_to_defer);                                         \
  kraken::detail::ScopedDeferHelper<decltype(KLI_SCOPED_DEFER_NAME(func))> KLI_SCOPED_DEFER_NAME( \
    helper){std::move(KLI_SCOPED_DEFER_NAME(func))};
