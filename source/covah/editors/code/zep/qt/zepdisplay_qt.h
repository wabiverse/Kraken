...
class ZepDisplay_Qt : public ZepDisplay {
 public:
                 ...
                 const uint8_t *text_end) const override
  {
    ...
    // Note: this logic works, but is suspect; I had trouble getting the font to align with where
    // it should be placed!
    // m_pPainter->drawText(p0.x(), p0.y() + font.GetPixelHeight() - qtFont.Descent() + 1,
    // QString::fromUtf8((char*)text_begin, text_end - text_begin));

    m_pPainter->drawText(p0.x(),
                         p0.y() - qtFont.Descent() + GetPixelScale().y * 1,
                         m_pPainter->viewport().width() - p0.x(),
                         m_pPainter->viewport().height() - p0.y(),
