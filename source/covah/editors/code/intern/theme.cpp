...
void ZepTheme::SetDarkTheme()
{
  ...
  m_colors[ThemeColor::LineNumberBackground] = m_colors[ThemeColor::Background] +
  ...
  m_colors[ThemeColor::CursorNormal]     = NVec4f(
      130.0f / 255.0f, 140.0f / 255.0f, 230.0f / 255.0f, 1.0f);
...
{
  return ThemeColor((uint32_t)ThemeColor::UniqueColor0 +
                    (uint32_t)(index % (uint32_t)ThemeColor::UniqueColorLast));
}
