...
/*void ZepMode_Standard::AddKeyPress(uint32_t key, uint32_t modifierKeys)
{
    ...
    // after the cursor!  It depends on which direction the user moved when they started the
selection if (m_currentMode == EditorMode::Visual)
    {
        if (begin_shift)
        {
        ...
        {
            m_visualEnd = GetCurrentWindow()->GetBufferCursor(); //
buffer.LocationFromOffsetByChars(GetCurrentWindow()->GetBufferCursor(), -1);
        }
    }
    ...
    {
        // Grab it
        std::string str = std::string(buffer.GetWorkingBuffer().begin() + startOffset,
buffer.GetWorkingBuffer().begin() + endOffset); GetEditor().GetRegister('"').text = str;
    ...
    if (op == CommandOperation::Insert)
    {
        ...
        if (m_currentMode == EditorMode::Visual)
        {
            ...
            auto cmd = std::make_shared<ZepCommand_DeleteRange>(buffer, startOffset, endOffset,
GetCurrentWindow()->GetBufferCursor()); if (!ch.empty())
            {
        ...
        if (!ch.empty())
        {
            auto cmd = std::make_shared<ZepCommand_Insert>(buffer, startOffset, ch,
GetCurrentWindow()->GetBufferCursor()); if (boundary)
            {
                cmd->SetFlags(CommandFlags::GroupBoundary);
            }
            AddCommand(std::static_pointer_cast<ZepCommand>(cmd));
    ...
    else if (op == CommandOperation::Delete)
    {
        // Delete
        auto cmd = std::make_shared<ZepCommand_DeleteRange>(buffer, startOffset, endOffset,
GetCurrentWindow()->GetBufferCursor()); AddCommand(std::static_pointer_cast<ZepCommand>(cmd));
        return_to_insert = true;
    }
