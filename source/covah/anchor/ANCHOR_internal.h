...
struct ANCHOR_API ImRect
{
  ...
  ImRect()
    : Min(0.0f, 0.0f),
      Max(0.0f, 0.0f)
  {}
  ImRect(const wabi::GfVec2f &min, const wabi::GfVec2f &max)
    : Min(min),
      Max(max)
  {}
  ImRect(const wabi::GfVec4f &v)
    : Min(v[0], v[1]),
      Max(v[2], v[3])
  {}
...
template<typename T>
struct ANCHOR_API ImChunkStream
{
...
enum ANCHOR_TreeNodeFlagsPrivate_
{
  ANCHOR_TreeNodeFlags_ClipLabelForTrailingButton = 1 << 20
};
...
enum ANCHOR_LayoutType_
{
  ANCHOR_LayoutType_Horizontal = 0,
  ANCHOR_LayoutType_Vertical = 1
};
...
{
  AnchorU8 Data[8];  // Can fit any data up to ANCHOR_DataType_COUNT
};
...
struct ANCHOR_Context
{
  ...
  AnchorU64 ActiveIdUsingKeyInputMask;  // Active widget will want to read those key inputs. When
                                        // we grow the ANCHOR_Key enum we'll need to either to
...
typedef AnchorS8 ANCHOR_TableColumnIdx;
typedef AnchorU8 ANCHOR_TableDrawChannelIdx;
...
ANCHOR_API void RenderNavHighlight(
  const ImRect &bb,
  ANCHOR_ID id,
  ANCHOR_NavHighlightFlags flags = ANCHOR_NavHighlightFlags_TypeDefault);  // Navigation highlight
