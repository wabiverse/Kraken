...
string ArchGetFileName(FILE *file)
{
  ...
  string result;
  char buf[PATH_MAX];
  ssize_t r = readlink(ArchStringPrintf("/proc/self/fd/%d", fileno(file)).c_str(),
                       buf,
                       sizeof(buf));
  if (r != -1) {
...
string ArchMakeTmpFileName(const string &prefix, const string &suffix)
{
  ...
  if (n == 1)
    ...
    return ArchStringPrintf("%s/%s.%d.%d%s",
                            tmpDir.c_str(),
                            prefix.c_str(),
                            pid,
                            n,
...
bool ArchQueryMappedMemoryResidency(void const *addr, size_t len, unsigned char *pageMap)
{
  ...
  int ret = mincore(reinterpret_cast<caddr_t>(const_cast<void *>(addr)),
                    len,
