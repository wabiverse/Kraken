 ...
 */
#ifndef WABI_BASE_WORK_ARENA_DISPATCHER_H
#define WABI_BASE_WORK_ARENA_DISPATCHER_H
...
class WorkArenaDispatcher {
 public:
  ...
  template<class Callable, class... Args> inline void Run(Callable &&c, Args &&...args)
  {
    _arena->execute(_MakeRunner(
        &_dispatcher, std::bind(std::forward<Callable>(c), std::forward<Args>(args)...)));
  }
 ...
 private:
  ...
  template<class Fn> struct _Runner {
  ...
  template<class Fn>
  _Runner<typename std::remove_reference<Fn>::type> _MakeRunner(WorkDispatcher *wd, Fn &&fn)
  {
    return _Runner<typename std::remove_reference<Fn>::type>(wd, std::forward<Fn>(fn));
  }
...
#endif  // WABI_BASE_WORK_ARENA_DISPATCHER_H
