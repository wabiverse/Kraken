...
#include "pixarDoubleConversion/double-conversion.h"
...
vector<string> TfMatchedStringTokenize(const string &source,
                                       char openDelimiter,
                                       char closeDelimiter,
...
// distance(b1, e1).
static std::pair<char const *, char const *> Mismatch(const char *b1,
                                                      const char *e1,
                                                      const char *b2)
{
...
bool TfDictionaryLessThan::_LessImpl(const string &lstr, const string &rstr) const
{
  ...
  std::tie(lcur, rcur) = Mismatch(lstr.c_str(),
                                  lstr.c_str() + std::min(lstr.size(), rstr.size()),
                                  rstr.c_str());
...
void Tf_ApplyDoubleToStringConverter(float val, char *buffer, int bufferSize)
{
  ...
  wabi_double_conversion::StringBuilder builder(buffer, bufferSize);
