 ...
 */
#ifndef WABI_BASE_TF_STL_H
#define WABI_BASE_TF_STL_H
...
template<size_t N> class TfGet {
 public:
  ...
  template<class PairOrTuple> constexpr auto &operator()(PairOrTuple &p) const
  {
    return std::get<N>(p);
  }

  template<class PairOrTuple> constexpr const auto &operator()(const PairOrTuple &p) const
  {
    return std::get<N>(p);
  }
