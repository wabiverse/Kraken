...
//
#ifndef WABI_BASE_TF_PY_ENUM_H
#define WABI_BASE_TF_PY_ENUM_H
 ...
 private:
  ...
  struct _EnumFromPython
  {
    ...
    static void *convertible(PyObject *obj)
    {
      ...
      if (boost::is_same<T, TfEnum>::value || (boost::is_integral<T>::value && !boost::is_enum<T>::value))
...
PyObject *Tf_PyEnumRegistry::_EnumToPython<T>::convert(T const &t)
{
  ...
  return boost::python::incref(Tf_PyEnumRegistry::GetInstance()._enumsToObjects[e]);
}
 ...
 private:
  ...
  void _ExportValues(bool cleanNames, _EnumPyClassType &enumClass)
  {
    ...
    TF_FOR_ALL (name, names)
    {
      ...
      std::string cleanedName = cleanNames ? Tf_PyCleanEnumName(*name) : *name;
