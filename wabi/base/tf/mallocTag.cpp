...
// #1   Tf_MallocGlobalData::_CaptureMallocStack(Tf_MallocPathNode const*, void const*, unsigned
// long) #2   TfMallocTag::_MallocWrapper(unsigned long, void const*)
...
{
  Tf_MallocBlockInfo() : blockSize(0), pathNodeIndex(0) {}

  Tf_MallocBlockInfo(size_t size, uint32_t index) : blockSize(size), pathNodeIndex(index) {}
...
static_assert(sizeof(Tf_MallocBlockInfo) == 8, "Unexpected size for Tf_MallocBlockInfo");
#endif
...
struct Tf_MallocGlobalData
{
  ...
  {
    return Tf_GetOrCreateCallSite(&_callSiteTable, name, &_captureCallSiteCount);
  }
  ...
  void _CaptureMallocStack(const Tf_MallocPathNode *node, const void *ptr, size_t size);
  void _ReleaseMallocStack(const Tf_MallocPathNode *node, const void *ptr);
  ...
  typedef TfHashMap<const void *, TfMallocTag::CallStackInfo, TfHash> _CallStackTableType;
  ...
  typedef TfHashMap<const void *, Tf_MallocBlockInfo, TfHash> _PathNodeTableType;
...
struct Tf_MallocPathNode
{
  ...
  void _BuildTree(TfMallocTag::CallTree::PathNode *node, bool skipRepeated);
...
inline bool Tf_MallocGlobalData::_RegisterPathNode(Tf_MallocPathNode *pathNode)
{
...
inline bool Tf_MallocGlobalData::_RegisterPathNodeForBlock(Tf_MallocPathNode *pathNode,
                                                           void *block,
                                                           size_t blockSize)
{
...
void Tf_MallocGlobalData::_ReleaseMallocStack(const Tf_MallocPathNode *node, const void *ptr)
{
  if (node->_callSite->_trace) {
...
static inline void _ExtractIndexAndGetSize(void *ptr, size_t *size, uint32_t *index)
{
...
static inline void _StoreIndexAndGetSize(void *ptr, size_t *size, uint32_t index)
{
  // Get the control word.
...
static inline void _ExtractIndexAndGetSize(void *, size_t *, uint32_t *)
{
  TF_FATAL_ERROR("Attempting to use Malloc Tags on unsupported platform");
}

static inline void _StoreIndexAndGetSize(void *, size_t *, uint32_t)
{
  TF_FATAL_ERROR("Attempting to use Malloc Tags on unsupported platform");
}
 ...
 */
inline bool TfMallocTag::_ShouldNotTag(TfMallocTag::_ThreadData **tptr, _Tagging *statePtr)
{
...
void *TfMallocTag::_ReallocWrapper(void *oldPtr, size_t nBytes, const void *)
{
  ...
  void *newPtr = NULL;
  {
    ...
    // Update malloc global data with bookkeeping information. This has to
    // happen while the mutex is held.
    if (_mallocGlobalData->_RegisterPathNodeForBlock(newNode, newPtr, blockSize)) {

      _mallocGlobalData->_CaptureMallocStack(newNode, newPtr, blockSize);
      ...
      _mallocGlobalData->_RunDebugHookForNode(newNode, newPtr, blockSize);
    }
...
void *TfMallocTag::_MallocWrapper_ptmalloc(size_t nBytes, const void *)
{
  void *ptr = _mallocHook.Malloc(nBytes);
...
void *TfMallocTag::_ReallocWrapper_ptmalloc(void *oldPtr, size_t nBytes, const void *)
{
  /*
   * If ptr is NULL, we want to make sure we don't double count,
   * because a call to _mallocHook.Realloc(ptr, nBytes) could call
...
void *TfMallocTag::_MemalignWrapper_ptmalloc(size_t alignment, size_t nBytes, const void *)
{
  void *ptr = _mallocHook.Memalign(alignment, nBytes);
...
void TfMallocTag::_FreeWrapper_ptmalloc(void *ptr, const void *)
{
  if (!ptr)
    return;
...
bool TfMallocTag::GetCallTree(CallTree *tree, bool skipRepeated)
{
  ...
  if (Tf_MallocGlobalData *gd = _mallocGlobalData) {
    ...
    TF_FOR_ALL (csi, callSiteTable) {
      CallTree::CallSite cs = {csi->second->_name, static_cast<size_t>(csi->second->_totalBytes)};
...
void TfMallocTag::_SetTagging(_Tagging status)
{
  TfMallocTag::Tls::Find()->_tagState = status;
}

TfMallocTag::_Tagging TfMallocTag::_GetTagging()
{
...
void TfMallocTag::Auto::_Begin(const string &name)
{
  _Begin(name.c_str());
}

void TfMallocTag::Auto::_Begin(const char *name)
{
  if (!name || !name[0])
    return;
...
void TfMallocTag::Pop(const char *name)
{
  if (!TfMallocTag::_doTagging)
    return;
                              ...
                              const std::string *rootName = nullptr)
{
  ...
  out << TfStringPrintf("%13s B %13s B %7ld samples    ",
                        _GetAsCommaSeparatedString(node.nBytes).c_str(),
                        _GetAsCommaSeparatedString(node.nBytesDirect).c_str(),
                        node.nAllocations);
...
static void _ReportCapturedMallocStacks(std::ostream &out,
                                                   ...
                                                   size_t maxPrintedNodes) const
{
  ...
  if (setting == TREE || setting == BOTH) {
    ...
    size_t reportedMem =
      _PrintMallocNode(&rpt, this->root, 0, 0, 0, printedNodes, maxPrintedNodes);
