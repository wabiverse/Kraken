...
namespace
{
  ...
  template<class T>
  constexpr
    typename std::enable_if<GfIsGfRange<T>::value && T::dimension == 1, Vt_PyShape<1>>::type
    Vt_GetElementShapeImpl(T *)
  {
    return {2};
  }
  ...
  template<class T>
  constexpr
    typename std::enable_if<GfIsGfRange<T>::value && T::dimension != 1, Vt_PyShape<2>>::type
    Vt_GetElementShapeImpl(T *)
  {
  ...
  bool Vt_ArrayFromBuffer(TfPyObjWrapper const &obj, VtArray<T> *out, string *errPtr = nullptr)
  {
    ...
    auto numItems = std::accumulate(view.shape, view.shape + view.ndim, 1, multiply);
...
template<class T>
boost::optional<VtArray<T>> VtArrayFromPyBuffer(TfPyObjWrapper const &obj, std::string *err)
{
  VtArray<T> array;
