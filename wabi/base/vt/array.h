 ...
 */
#ifndef WABI_BASE_VT_ARRAY_H
#define WABI_BASE_VT_ARRAY_H
...
template<typename ELEM> class VtArray : public Vt_ArrayBase {
 public:
  ...
  /// Reference type.
  typedef typename PointerIterator<ElementType>::reference reference;
  /// Const reference type.
  typedef typename PointerIterator<const ElementType>::reference const_reference;
  /// Pointer type.
  ...
  typename std::enable_if<!std::is_integral<ForwardIter>::value>::type assign(ForwardIter first,
                                                                              ForwardIter last)
  {
    struct _Copier {
 ...
 private:
  ...
  {
    return !_data || (ARCH_LIKELY(!_foreignSource) && _GetNativeRefCount(_data) == 1);
  }
  ...
  value_type *_AllocateNew(size_t capacity)
  {
    ...
    void *data = malloc(sizeof(_ControlBlock) + capacity * sizeof(value_type));
