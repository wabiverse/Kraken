...
} const &other){{{LIST("_data[%(i)s] = other[%(i)s];", sep = '\n    ')}}} { % endfor % }
{
  % endif %
}
...
GF_API bool GfOrthogonalizeBasis({{VEC}} * tx,
                                 {{VEC}} * ty,
                                 {{VEC}} * tz,
                                 bool normalize,
...
#endif  // WABI_BASE_GF_{{ UPPER(VEC)[2:] }}_H
