...
#include <ostream>
  ...
  WABI_NAMESPACE_BEGIN
    ...
    TF_REGISTRY_FUNCTION(TfType){TfType::Define<{{MAT}}>();
}
...
{ % macro DIAGONAL(dim, i) % } {{'{'}} {%- for ROW in range(dim) %
}
{
  % -if loop.first %
}
{{'{'}} { % else % } {{'{' | indent(i, true)}} { % endif % } {%- for COL in range(dim) %
}
{
  % -if ROW == COL - %
}
...
::{{MAT}}(const std::vector<std::vector<{{S}}>> &v)
{
  {
    {
      SCL
...
{ % endfor % } { % block customConstructors % } { % endblock customConstructors % }
...
::SetDiagonal(const GfVec{{SUFFIX}} & v)
{
  {
    {
      MATRIX(fmt = "_mtx[%(i)s][%(j)s] = 0.0;",
...
::GetTranspose() const
{
  ...
  {
    {
      MATRIX("transpose._mtx[%(j)s][%(i)s] = _mtx[%(i)s][%(j)s];", sep = "\n    ", indent = 4)
    }
  }
...
operator-(const {{MAT}} & m)
{
  return {{MAT}}({{MATRIX("-m._mtx[%(i)s][%(j)s]", indent = 19)}});
}
  ...
  const {{MAT}} & m)
{
  ...
  f({% for COL in range(DIM) %
  } float({{LIST("vec[%%(i)s] * m._mtx[%%(i)s][%(COL)s]" % {'COL' : COL}, sep = " + ")}}) {
    % -if not loop.last %
  } {{",\n"}} { % endif % } { % endfor % });
}
  ...
  } f &vec)
{
  ...
  f({% for ROW in range(DIM) %
  } float({{LIST("vec[%%(i)s] * m._mtx[%(ROW)s][%%(i)s]" % {'ROW' : ROW}, sep = " + ")}}) {
    % -if not loop.last %
  } {{",\n"}} { % endif % } { % endfor % });
}
{ % endif - % }
