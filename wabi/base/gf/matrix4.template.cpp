...
::{{MAT}}(const GfRotation &rotate, const GfVec3{{SCL[0]}} & translate)
{
...
::{{MAT}}(const GfMatrix3{{SCL[0]}} & rotmx, const GfVec3{{SCL[0]}} & translate)
{
  SetTransform(rotmx, translate);
}
{ % endblock customConstructors % }

{ % block customFunctions % } {{MAT}} &
{
  {
    MAT
  }
}
::SetTransform(const GfRotation &rotate, const GfVec3{{SCL[0]}} & translate)
{
  SetRotate(rotate);
...
::SetTransform(const GfMatrix3{{SCL[0]}} & rotate, const GfVec3{{SCL[0]}} & translate)
{
  SetRotate(rotate);
  return SetTranslateOnly(translate);
}
...
::GetInverse(double *detPtr, double eps) const
{
  {
    {
      SCL
  ...
  if (GfAbs(det) > eps) {
    ...
    static_cast<float>({{t}}){ % -else % } {{t}} { % -endif % } { % endmacro % }
...
::Orthonormalize(bool issueWarning)
{
...
::GetOrthonormalized(bool issueWarning) const
{
  {
    {
      MAT
...
{ % block customXformFunctions % }  // Leaves the [3][3] element as 1
{{MAT}} &
{
  {
    MAT
...
::SetRotate(const GfQuat{{SCL[0]}} & rot)
{
  SetRotateOnly(rot);
...
::SetRotateOnly(const GfQuat{{SCL[0]}} & rot)
{
...
::SetRotate(const GfRotation &rot)
{
  SetRotateOnly(rot);
...
::SetRotateOnly(const GfRotation &rot)
{
...
::SetRotate(const GfMatrix3{{SCL[0]}} & mx3)
{
  _mtx[0][0] = mx3[0][0];
  _mtx[0][1] = mx3[0][1];
  _mtx[0][2] = mx3[0][2];
...
::SetRotateOnly(const GfMatrix3{{SCL[0]}} & mx3)
{
  _mtx[0][0] = mx3[0][0];
  _mtx[0][1] = mx3[0][1];
  _mtx[0][2] = mx3[0][2];
...
::SetTranslate(const GfVec3{{SCL[0]}} & t)
{
  _mtx[0][0] = 1.0;
  _mtx[0][1] = 0.0;
  _mtx[0][2] = 0.0;
...
::SetTranslateOnly(const GfVec3{{SCL[0]}} & t)
{
  _mtx[3][0] = t[0];
  _mtx[3][1] = t[1];
  _mtx[3][2] = t[2];
...
::SetLookAt(const GfVec3{{SCL[0]}} & eyePoint,
            ...
            const GfVec3{{SCL[0]}} & upDirection)
{
  ...
  view = (centerPoint - eyePoint).GetNormalized();
  ...
  right = GfCross(view, upDirection).GetNormalized();
...
::SetLookAt(const GfVec3{{SCL[0]}} & eyePoint, const GfRotation &orientation)
{
...
::Factor({{MAT}} * r, GfVec3{{SCL[0]}} * s, {{MAT}} * u, GfVec3{{SCL[0]}} * t, {{MAT}} * p, {
  {
    SCL
  }
} eps) const
{
  ...
  r->Set(eigenvectors[0][0],
         ...
         0.0,
         1.0);
  { % else % } GfMatrix4d rTmp(eigenvectors[0][0],
                               eigenvectors[0][1],
                               eigenvectors[0][2],
                               0.0,
                               eigenvectors[1][0],
...
::_Jacobi3(GfVec3d *eigenvalues, GfVec3d eigenvectors[3]) const
{
  // This was adapted from the (open source) Open Inventor
  // SbMatrix::Jacobi3().
...
::RemoveScaleShear() const
{
  ...
  scaleOrientMat, factoredRotMat, perspMat;
  GfVec3
  {
    {
      SCL[0]
                    ...
                    const GfVec3{{SCL[0]}} & axis2) const
{
  ...
  GfVec3{{SCL[0]}} { % endif - % }(ExtractRotation().Decompose(axis0, axis1, axis2));
}
...
{
  return GfMatrix3{{SCL[0]}}(_mtx[0][0],
                             _mtx[0][1],
                             _mtx[0][2],
