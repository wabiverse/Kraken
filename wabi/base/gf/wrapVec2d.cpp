...
static double NormalizeHelper(GfVec2d *vec, double eps = GF_MIN_VECTOR_LENGTH)
{
...
static int __len__(const GfVec2d &self)
{
  return 2;
}
...
struct FromPythonTuple {
  FromPythonTuple()
  {
    converter::registry::push_back(&_convertible, &_construct, boost::python::type_id<GfVec2d>());
  }
 ...
 private:
  ...
  {
    typedef double Scalar;
    void *storage = ((converter::rvalue_from_python_storage<GfVec2d> *)data)->storage.bytes;
    new (storage) GfVec2d(_SequenceGetItem(obj_ptr, 0), _SequenceGetItem(obj_ptr, 1));
...
void wrapVec2d()
{
  ...
  to_python_converter<std::vector<GfVec2d>, TfPySequenceToPython<std::vector<GfVec2d>>>();
  ...
  TfPyContainerConversions::from_python_sequence<std::vector<GfVec2d>,
