...
::OrthogonalizeBasis({{VEC}} * tx, {{VEC}} * ty, {{VEC}} * tz, const bool normalize, double eps)
{
...
::BuildOrthonormalFrame({{VEC}} * v1, {{VEC}} * v2, {
  {
    SCL
  }
} eps) const
 ...
 */
bool GfOrthogonalizeBasis({{VEC}} * tx, {{VEC}} * ty, {{VEC}} * tz, bool normalize, double eps)
{
  {
    {
      VEC
...
GfSlerp(double alpha, const {{VEC}} & v0, const {{VEC}} & v1)
{
  ...
  if (fabs(sinAngle) < 0.00001) {
    ...
    v = v0 * cos(alpha * M_PI) + vX * sin(alpha * M_PI);
    return v;
  }
