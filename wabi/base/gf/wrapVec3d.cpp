...
namespace
{
  ...
  static string __repr__(GfVec3d const &self)
  {
    string elems;
    for (size_t i = 0; i < self.dimension; ++i)
      elems += (i ? ", " : "") + TfPyRepr(self[i]);

    return TF_PY_REPR_PREFIX + "Vec3d(" + elems + ")";
  }
  ...
  static double NormalizeHelper(GfVec3d *vec, double eps = GF_MIN_VECTOR_LENGTH)
  {
  ...
  static bool OrthogonalizeBasisHelper(GfVec3d *v1,
                                       GfVec3d *v2,
                                       GfVec3d *v3,
                                       bool normalize,
                                       double eps = GF_MIN_ORTHO_TOLERANCE)
  {
    return GfVec3d::OrthogonalizeBasis(v1, v2, v3, normalize, eps);
  }
  BOOST_PYTHON_FUNCTION_OVERLOADS(OrthogonalizeBasis_overloads, OrthogonalizeBasisHelper, 4, 5)

  static tuple BuildOrthonormalFrameHelper(const GfVec3d &self, double eps = GF_MIN_VECTOR_LENGTH)
  {
    GfVec3d v1, v2;
  ...
  static int __len__(const GfVec3d &self)
  {
    return 3;
  }

  // Implements __getitem__ for a single index
  static double __getitem__(const GfVec3d &self, int index)
  {
    return self[normalizeIndex(index)];
  }

  // Implements __getitem__ for a slice
  static list __getslice__(const GfVec3d &self, slice indices)
  {
    list result;

    const double *begin = self.data();
    const double *end = begin + 3;
  ...
  static void __setitem__(GfVec3d &self, int index, double value)
  {
  ...
  static void __setslice__(GfVec3d &self, slice indices, object values)
  {
    ...
    // Make sure that all items can be extracted before changing the GfVec3d.
    //
  ...
  static bool __contains__(const GfVec3d &self, double value)
  {
...
#if PY_MAJOR_VERSION == 2
  static GfVec3d __truediv__(const GfVec3d &self, double value)
  {
    return self / value;
  }

  static GfVec3d __itruediv__(GfVec3d &self, double value)
  {
    return self /= value;
  }
#endif
  ...
  struct FromPythonTuple
  {
    FromPythonTuple()
    {
      converter::registry::push_back(&_convertible, &_construct, boost::python::type_id<GfVec3d>());
    }
   ...
   private:
    ...
    {
      typedef double Scalar;
      void *storage = ((converter::rvalue_from_python_storage<GfVec3d> *)data)->storage.bytes;
      new (storage)
        GfVec3d(_SequenceGetItem(obj_ptr, 0), _SequenceGetItem(obj_ptr, 1), _SequenceGetItem(obj_ptr, 2));
  ...
  {
    static boost::python::tuple getinitargs(const GfVec3d &v)
    {
...
void wrapVec3d()
{
  ...
  class_<GfVec3d> cls("Vec3d", no_init);
  cls
  ...
  to_python_converter<std::vector<GfVec3d>, TfPySequenceToPython<std::vector<GfVec3d>>>();
  ...
  // Allow conversion of lists of GfVec3d to std::vector<GfVec3d>
  TfPyContainerConversions::from_python_sequence<std::vector<GfVec3d>,
