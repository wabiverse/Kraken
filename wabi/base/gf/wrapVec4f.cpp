...
static float NormalizeHelper(GfVec4f *vec, float eps = GF_MIN_VECTOR_LENGTH)
{
...
static int __len__(const GfVec4f &self)
{
  return 4;
}
...
struct FromPythonTuple
{
  FromPythonTuple()
  {
    converter::registry::push_back(&_convertible, &_construct, boost::python::type_id<GfVec4f>());
  }
 ...
 private:
  ...
  {
    typedef float Scalar;
    void *storage = ((converter::rvalue_from_python_storage<GfVec4f> *)data)->storage.bytes;
    new (storage) GfVec4f(_SequenceGetItem(obj_ptr, 0),
...
void wrapVec4f()
{
  ...
  to_python_converter<std::vector<GfVec4f>, TfPySequenceToPython<std::vector<GfVec4f>>>();
  ...
  TfPyContainerConversions::from_python_sequence<std::vector<GfVec4f>,
