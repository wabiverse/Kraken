...
#include "wabi/base/gf/matrixData.h"
...
#include <vector>
  ...
  { % endfor % } { % block forwardDeclarations % } { % endblock % }
  ...
  }
  {
   public:
    ...
    { % endfor % } { % block customConstructors % } { % endblock % } {
      % if SCL == 'double' %
    }
    /// This explicit constructor converts a "float" matrix to a "double" matrix.
    GF_API
    explicit {{MAT}}(const class GfMatrix {
      {
        DIM
      }
    } f &m);
    ...
    GF_API{{MAT}} & SetDiagonal(const GfVec{{SUFFIX}} &);
    ...
    /// {{ SCL }} values so that standard indexing (such as <c>m[0][1]</c>)
    /// works correctly.
    const {{SCL}} *operator[](int i) const
    {
      return _mtx[i];
    }

    /// Hash.
    friend inline size_t hash_value({
      {
        MAT
      }
    } const &m)
    ...
    GetInverse(double *det = NULL, double eps = 0) const;

    /// Returns the determinant of the matrix.
    GF_API
    double GetDeterminant() const;

    { % block customFunctions % } { % endblock customFunctions % }
    ...
    operator*(const {{MAT}} & m1, double d)
    {
      {
        {
          MAT
    ...
    operator*(double d, const {{MAT}} & m)
    {
      return m * d;
    }

    /// Adds matrix \e m to this matrix.
    GF_API{{MAT}} &operator+=(const {{MAT}} & m);
    ...
    operator+(const {{MAT}} & m1, const {{MAT}} & m2)
    {
      {
        {
          MAT
    ...
    operator*(const {{MAT}} & m, const GfVec{{SUFFIX}} & vec)
    {
      return GfVec{{SUFFIX}}({%- for ROW in range(DIM) -%
    ...
    operator*(const GfVec{{SUFFIX}} & vec, const {{MAT}} & m)
    {
      return GfVec{{SUFFIX}}({%- for COL in range(DIM) -%
...
#endif  // WABI_BASE_GF_{{ UPPER(MAT)[2:] }}_H
