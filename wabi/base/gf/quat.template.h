...
#include "wabi/base/gf/vec3{{ SUFFIX }}.h"
#include "wabi/wabi.h"
{
  % if SCL == 'GfHalf' - %
}
...
GfDot(const {{QUAT}} & q1, const {{QUAT}} & q2);
...
}
{
 public:
  ...
  const GfVec3{{SUFFIX}} & GetImaginary() const
  {
    return _imaginary;
  }
  ...
  } eps = GF_MIN_VECTOR_LENGTH) const
  {
    {
      {
        QUAT
  ...
  Transform(const GfVec3{{SUFFIX}} & point) const;
  ...
  GF_API{{QUAT}} &operator*=(const {{QUAT}} & q);
...
#endif  // WABI_BASE_GF_{{ UPPER(QUAT)[2:] }}_H
