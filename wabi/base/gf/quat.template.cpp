...
#include "wabi/base/gf/quat{{ SUFFIX }}.h"
#include "wabi/base/gf/vec4{{ SUFFIX }}.h"
...
::Transform(const GfVec3{{SUFFIX}} & point) const
{
  ...
  tmpDot = GfDot(_imaginary, _imaginary);
  {
    {
      SCL
    }
...
::operator*=(const {{QUAT}} & q)
{
  ...
  i(r1 * i2[0] + r2 * i1[0] + (i1[1] * i2[2] - i1[2] * i2[1]),
...
GfSlerp(double alpha, const {{QUAT}} & q0, const {{QUAT}} & q1)
{
  ...
  if (1.0 - cosTheta > 0.00001)
  {
    ...
    theta = acos(cosTheta), sinTheta = sin(theta);
