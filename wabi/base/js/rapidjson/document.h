...
template<typename Encoding, typename Allocator = MemoryPoolAllocator<>> class GenericValue
{
  ...
  */
  ...
  {
    static const uint16_t defaultFlags[7] = {kNullFlag,
                                             kFalseFlag,
                                             kTrueFlag,
                                             kObjectFlag,
                                             kArrayFlag,
  ...
  */
  ...
  /*! \tparam T Either \ref Type, \c int, \c unsigned, \c int64_t, \c uint64_t, \c double, \c true,
   * \c false
   */
  template<typename T>
  ...
  /*! \pre IsObject() == true
     ...
     operator[](SizeType)) \note In version 0.1x, if the member is not found, this function returns
     a null value. This makes issue 7. Since 0.2, if the name is not correct, it will assert. If
     user is unsure whether a member exists, user should use HasMember() first. A better approach
     is to use FindMember(). \note Linear time complexity.
  */
  template<typename T>
  RAPIDJSON_DISABLEIF_RETURN(
  ...
  */
  ...
  /*!
     ...
     Whether a member with that name exists. \note It is better to use FindMember() directly if you
     need the obtain the value as well. \note Linear time complexity.
  */
  template<typename SourceAllocator>
  ...
  */
  ...
  /*! \param name A string value as name of member.
     ...
     before. Commonly use GenericDocument::GetAllocator(). \return The value itself for fluent API.
  ...
  GenericValue &AddMember(GenericValue &name, GenericValue &value, Allocator &allocator)
  {
    ...
    if (o.size >= o.capacity) {
      if (o.capacity == 0) {
        ...
        SetMembersPointer(
          reinterpret_cast<Member *>(allocator.Realloc(GetMembersPointer(),
  ...
  */
  ...
  /*! \param pos iterator to the member to remove
     ...
     returned. \note This function preserves the relative order of the remaining object members. If
     you do not need this, use the more efficient \ref RemoveMember(MemberIterator). \note Linear
     time complexity.
  */
  ...
  */
  ...
  /*! \tparam T Either \ref Type, \c int, \c unsigned, \c int64_t, \c uint64_t
      ...
      \param allocator    Allocator for reallocating memory. It must be the same one as used
     before. Commonly use GenericDocument::GetAllocator(). \pre IsArray() == true \return The value
  ...
  */
   ...
   * check whether the converison is lossless.
   */
