...
namespace
{
  ...
  template<class T> bool IsHolding(const _Any &any)
  {
    return boost::any_cast<T>(&any) != 0;
  }
  template<class T> T Get(const _Any &any)
  {
  ...
  {
    IndenterScope(Indenter &indenter) : indenter(indenter)
    {
      indenter.Start();
    }
    ~IndenterScope()
...
template<class T> static void _CheckArrayOf(const JsValue &value)
{
  ...
  indent << "array.size = " << array.size() << ", expArray.size = " << expArray.size()
         << std::endl;
...
static void _CheckArray(const _AnyVector &array, const JsArray &expArray)
{
  IndenterScope scope(*indenter);
  ...
  for (size_t i = 0; i < array.size(); ++i) {
    ...
    switch (expArray[i].GetType()) {
      ...
      case JsValue::IntType: {
        ...
        TF_AXIOM(
          (IsHolding<int64_t>(array[i]) && Get<int64_t>(array[i]) == expArray[i].GetInt()) ||
      ...
      default:
        TF_FATAL_ERROR("Unknown type in test array");
    }
  }
}
...
int main(int argc, char const *argv[])
{
  ...
  for (const auto &p : aObject) {
    ...
    indent << "key " << p.first << " typeid is " << (ti ? ArchGetDemangled(*ti) : "nil")
           << std::endl;
    ...
    if (p.first == "Array") {
      ...
      _CheckArray(Get<_AnyVector>(p.second), object[p.first].Get<JsArray>());
