...
HgiTextureDesc const &HgiTexture::GetDescriptor() const
{
  return _descriptor;
}
...
size_t HgiTexture::_GetByteSizeOfResource(const HgiTextureDesc &descriptor)
{
  ...
  return mipInfo.byteOffset + descriptor.layerCount * mipInfo.byteSizePerLayer;
}
...
{
  return lhs.debugName == rhs.debugName && lhs.usage == rhs.usage && lhs.format == rhs.format &&
...
void HgiTextureView::SetViewTexture(HgiTextureHandle const &handle)
{
  _viewTexture = handle;
}

HgiTextureHandle const &HgiTextureView::GetViewTexture() const
{
  return _viewTexture;
}
