...
#include "wabi/imaging/hioOpenVDB/vdbAssetInterface.h"
...
namespace
{
  ...
  openvdb::GridPtrVecPtr _ReadVDBGridsFromAssetBuffer(const std::shared_ptr<ArAsset> &asset)
  {
    if (!asset) {
      return nullptr;
    }
...
{
  std::shared_ptr<ArAsset> asset = ArGetResolver().OpenAsset(ArResolvedPath(assetPath));
  ...
  if (HioOpenVDBArAssetInterface *const vdbAsset = dynamic_cast<HioOpenVDBArAssetInterface *>(
        asset.get())) {
    TRACE_FUNCTION_SCOPE(
      "Reading VDB grids from "
    ...
    if (openvdb::GridPtrVecPtr grids = _ReadVDBGridsFromAssetBuffer(asset)) {
