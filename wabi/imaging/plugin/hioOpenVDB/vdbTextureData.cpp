...
class HioOpenVDB_TextureDataFactory final : public HioFieldTextureDataFactoryBase {
 protected:
  ...
  {
    return std::make_shared<HioOpenVDB_TextureData>(filePath, fieldName, targetMemory);
  }
};
...
const GfBBox3d &HioOpenVDB_TextureData::GetBoundingBox() const
{
  return _boundingBox;
}

HioFormat HioOpenVDB_TextureData::GetFormat() const
{
  return _format;
}

int HioOpenVDB_TextureData::ResizedWidth() const
{
  return _resizedWidth;
}

int HioOpenVDB_TextureData::ResizedHeight() const
{
  return _resizedHeight;
}

int HioOpenVDB_TextureData::ResizedDepth() const
{
  return _resizedDepth;
}

bool HioOpenVDB_TextureData::HasRawBuffer() const
{
...
bool HioOpenVDB_TextureData::Read()
{
  ...
  int bytesPerPixel;
  gridHolder->GetMetadata(&bytesPerPixel, &_format);
  ...
  if (nativeSize > _targetMemory && _targetMemory > 0) {
    ...
    TF_DEBUG(HIOOPENVDB_DEBUG_TEXTURE).Msg("[VdbTextureData] Scaling by factor %f\n", scale);

    // Apply voxel scaling to grid transform
    gridTransform = GfMatrix4d(GfVec4d(scale, scale, scale, 1.0)) * gridTransform;
  ...
  const openvdb::CoordBBox &treeBoundingBox = _denseGrid->GetTreeBoundingBox();
