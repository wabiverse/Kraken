...
#include "wabi/imaging/plugin/rmanArgsParser/rmanArgsParser.h"
...
const NdrTokenVec &RmanArgsParserPlugin::GetDiscoveryTypes() const
{
  static const NdrTokenVec discoveryTypes = {_tokens->discoveryType};
  return discoveryTypes;
}

const TfToken &RmanArgsParserPlugin::GetSourceType() const
{
  return _tokens->sourceType;
}

NdrNodeUniquePtr RmanArgsParserPlugin::Parse(const NdrNodeDiscoveryResult &discoveryResult)
{
  xml_document doc;
  ...
  return SdrShaderNodeUniquePtr(new SdrShaderNode(discoveryResult.identifier,
...
std::string RmanArgsParserPlugin::_GetDsoPathFromArgsPath(const std::string &argsPath)
{
  // We assume:
  ...
  if (pathElts.size() < 3 || !TfStringEndsWith(argsPath, argsExt) || pathElts[pathElts.size() - 2] != "Args")
  {
    TF_WARN(
  ...
  std::string pluginFileName = TfStringReplace(pathElts.back(), argsExt, dsoExt);
  pathElts.pop_back();
...
SdrShaderPropertyUniquePtr RmanArgsParserPlugin::_ParseChildElem(const SdrShaderRepresentation &shaderRep,
                                                                 bool isOutput,
                                                                 ...
                                                                 const std::string &parentPage) const
{
  ...
  while (attrChild)
  {
    ...
    if (EQUALS(helpStr, attrChild.name()))
    {
      ...
      attrChild.print(helpStream, /*indent=*/"\t", pugi::format_default | pugi::format_no_escapes);
  ...
  if (validConnectionTypes.size() || hasTagAttr)
  {
    ...
    attributes.emplace(SdrPropertyMetadata->ValidConnectionTypes,
  ...
  return _CreateProperty(shaderRep, isOutput, attributes, validConnectionTypes, options);
}

void RmanArgsParserPlugin::_Parse(SdrShaderRepresentation &shaderRep,
                                  xml_node parent,
  ...
  while (childElement)
  {
    ...
    else if (EQUALS(typeTagStr, childElement.name()))
    {
      ...
      if (attrChild && EQUALS(tagStr, attrChild.name()))
      {
        ...
        TF_DEBUG(NDR_PARSING).Msg("Deprecated 'typeTag' on shader [%s]", shaderRep.name.c_str());
      }
    }
...
std::tuple<TfToken, size_t> RmanArgsParserPlugin::_GetTypeName(const NdrTokenMap &attributes) const
{
  // Determine arraySize
  // -------------------------------------------------------------------------
...
VtValue RmanArgsParserPlugin::_GetVtValue(const std::string &stringValue,
                                          TfToken &type,
                                          size_t arraySize,
                                          const NdrTokenMap &metadata) const
{
  ...
  // -------------------------------------------------------------------------
  int isDynamicArray = IsTruthy(SdrPropertyMetadata->IsDynamicArray, metadata);
...
void RmanArgsParserPlugin::_OutputDeprecationWarning(const TfToken &attrName,
                                                     const SdrShaderRepresentation &shaderRep,
                                                     const TfToken &propName) const
{
  TF_DEBUG(NDR_PARSING)
    .Msg("Deprecated '%s' attribute on shader [%s] on property [%s]",
         attrName.GetText(),
         shaderRep.name.c_str(),
         propName.GetText());
}

SdrShaderPropertyUniquePtr RmanArgsParserPlugin::_CreateProperty(const SdrShaderRepresentation &shaderRep,
                                                                 bool isOutput,
                                                                 NdrTokenMap &attributes,
                                                                 NdrStringVec &validConnectionTypes,
                                                                 NdrOptionVec &options) const
{
  TfToken propName = _Get(attributes, _xmlAttributeNames->nameAttr, TfToken("NAME UNSPECIFIED"));
  ...
  if (attributes.count(_xmlAttributeNames->tagAttr))
  {
    ...
    attributes.insert(
      {SdrPropertyMetadata->ValidConnectionTypes, attributes.at(_xmlAttributeNames->tagAttr)});
  ...
  // -------------------------------------------------------------------------
  const VtValue defaultValue = attributes.count(_xmlAttributeNames->defaultAttr) ?
  ...
  return SdrShaderPropertyUniquePtr(new SdrShaderProperty(propName,
                                                          typeName,
                                                          defaultValue,
                                                          isOutput,
                                                          arraySize,
...
NdrStringVec RmanArgsParserPlugin::_GetAttributeValuesFromChildren(xml_node parent,
...
std::string RmanArgsParserPlugin::_Get(const NdrTokenMap &map,
                                       const TfToken &key,
...
TfToken RmanArgsParserPlugin::_Get(const NdrTokenMap &map, const TfToken &key, TfToken defaultValue) const
{
  typename NdrTokenMap::const_iterator it = map.find(key);
...
int RmanArgsParserPlugin::_Get(const NdrTokenMap &map, const TfToken &key, int defaultValue) const
{
  typename NdrTokenMap::const_iterator it = map.find(key);
...
float RmanArgsParserPlugin::_Get(const NdrTokenMap &map, const TfToken &key, float defaultValue) const
{
  typename NdrTokenMap::const_iterator it = map.find(key);
