...
void HdxPrmanRenderBuffer::_Deallocate()
{
...
/*static*/
size_t HdxPrmanRenderBuffer::_GetBufferSize(GfVec2i const &dims, HdFormat format)
{
  return dims[0] * dims[1] * HdDataSizeOfFormat(format);
}

/*virtual*/
bool HdxPrmanRenderBuffer::Allocate(GfVec3i const &dimensions, HdFormat format, bool multiSampled)
{
  _Deallocate();
...
template<typename T>
static void _ConvertPixel(HdFormat dstFormat, uint8_t *dst, HdFormat srcFormat, uint8_t const *src)
{
...
void HdxPrmanRenderBuffer::Blit(HdFormat format, int width, int height, uint8_t const *data)
{
