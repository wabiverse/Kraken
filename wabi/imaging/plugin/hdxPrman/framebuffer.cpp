...
static PtDspyError HydraDspyImageOpen(PtDspyImageHandle *handle_p,
...
static PtDspyError HydraDspyImageData(PtDspyImageHandle handle,
                                      int xmin,
                                      int xmax_plusone,
                                      int ymin,
                                      int ymax_plusone,
      ...
      for (HdxPrmanFramebuffer::HdPrmanAovIt it = buf->aovs.begin(); it != buf->aovs.end(); ++it) {
        ...
        if (it->format == HdFormatInt32) {
          ...
          if (it->name == HdAovTokens->depth) {
            ...
            aovData[0]           = data_f32[dataIdx++] + (alphaInv)*clear[0];  // R
...
static PtDspyError HydraDspyImageClose(PtDspyImageHandle handle)
{
  TF_UNUSED(handle);
  return PkDspyErrorNone;
}

static PtDspyError HydraDspyImageQuery(PtDspyImageHandle handle,
...
HdxPrmanFramebuffer *HdxPrmanFramebuffer::GetByID(int32_t id)
{
  _BufferRegistry &registry = *_bufferRegistry;
  {
    std::lock_guard<std::mutex> lock(registry.mutex);
...
void HdxPrmanFramebuffer::AddAov(TfToken aovName, HdFormat format, VtValue clearValue)
{
  HdPrmanAov aov;
  aov.name       = aovName;
  aov.format     = format;
...
void HdxPrmanFramebuffer::Clear()
{
  int size = w * h;
...
void HdxPrmanFramebuffer::Register(RixContext *ctx)
{
  assert(ctx);
