...
void HdxPrmanRenderDelegate::_Initialize(std::shared_ptr<HdPrman_Context> context)
{
  ...
  _resourceRegistry = std::make_shared<HdxPrman_ResourceRegistry>(_interactiveContext);
}
...
static TfTokenVector _PushBack(TfTokenVector const &vec, TfToken elem)
{
...
const TfTokenVector &HdxPrmanRenderDelegate::GetSupportedBprimTypes() const
{
  static TfTokenVector types = _PushBack(HdPrmanRenderDelegate::GetSupportedBprimTypes(),
...
bool HdxPrmanRenderDelegate::IsStopSupported() const
{
  return true;
}

bool HdxPrmanRenderDelegate::Stop()
{
  _interactiveContext->StopRender();
  return true;
}
bool HdxPrmanRenderDelegate::Restart()
{
