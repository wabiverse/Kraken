...
void HdxPrman_InteractiveContext::Begin(HdRenderDelegate *renderDelegate)
{
  ...
  // Options
  {
    ...
    SetOptionsFromRenderSettings(static_cast<HdPrmanRenderDelegate *>(renderDelegate), _options);
  ...
  {
    // Light shader
    riley::ShadingNode lightNode{riley::ShadingNode::k_Light,  // type
    ...
    ConvertCategoriesToAttributes(fallbackLightId, VtArray<TfToken>(), _fallbackLightAttrs);
    _fallbackLightAttrs.SetString(RixStr.k_grouping_membership, us_default);
    _fallbackLightAttrs.SetString(RixStr.k_identifier_name, RtUString(fallbackLightId.GetText()));
  ...
  fallbackMaterial = riley::MaterialId::k_InvalidId;
  {
    ...
    pxrPrimvar_node.params.SetColor(us_defaultColor, RtColorRGB(0.5, 0.5, 0.5));
    ...
    fallbackMaterial = riley->CreateMaterial(&materialNodes[0], materialNodes.size());
  }
  fallbackVolumeMaterial = riley::MaterialId::k_InvalidId;
  {
    ...
    fallbackVolumeMaterial = riley->CreateMaterial(materialNodes.data(), materialNodes.size());
  }
}
...
void HdxPrman_InteractiveContext::SetFallbackLightsEnabled(bool enabled)
{
  ...
  riley->ModifyLightInstance(riley::GeometryMasterId::k_InvalidId,  // no group
                             _fallbackLight,
...
bool HdxPrman_InteractiveContext::CreateDisplays(const HdRenderPassAovBindingVector &aovBindings)
{
  ...
  framebuffer.dspyId = riley->CreateDisplay(rtid,
  ...
  renderViews.clear();
  renderViews.push_back(riley::RenderView{rtid, integratorId, cameraId});
