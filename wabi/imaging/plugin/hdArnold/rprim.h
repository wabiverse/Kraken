...
class HdArnoldRprim : public HydraType
{
 public:
  ...
  const HdArnoldShape &GetShape() const
  {
    return _shape;
  }
  ...
  ///
  /// @return Pointer to the Arnold Node.
  const AtNode *GetArnoldNode() const
  {
    return _shape.GetShape();
  }
  ...
  HdArnoldRenderDelegate *GetRenderDelegate()
  {
    return _renderDelegate;
  }
  /// Syncs internal data and arnold state with hydra.
  HDARNOLD_API
  void SyncShape(HdDirtyBits dirtyBits,
  ...
  HDARNOLD_API
  void SetShapeVisibility(uint8_t visibility)
  {
  ...
  uint8_t GetShapeVisibility() const
  {
    return _shape.GetVisibility();
  }
