...
HdArnoldShape::HdArnoldShape(const AtString &shapeType,
...
void HdArnoldShape::Sync(HdRprim *rprim,
...
{
  // Hydra prim IDs are starting from zero, and growing with the number of primitives, so it's safe
  // to directly cast. However, prim ID 0 is valid in hydra (the default value for the id buffer in
  // arnold), so we have to to offset them by one, so we can use the the 0 prim id to detect
  // background pixels reliably both in CPU and GPU backend mode. Later, we'll subtract 1 from the
  // id in the driver.

  AiNodeSetUInt(_shape, str::id, static_cast<unsigned int>(primId) + 1);
}

void HdArnoldShape::_SyncInstances(HdDirtyBits dirtyBits,
                                   HdArnoldRenderDelegate *renderDelegate,
                                   HdSceneDelegate *sceneDelegate,
                                   HdArnoldRenderParamInterrupt &param,
                                   const SdfPath &id,
                                   ...
                                   bool force)
{
  ...
  // TODO(pal) : If the instancer is created without any instances, or it doesn't have any
  // instances, we might end
