    ...
    for (const auto &primvar : GetDelegate()->GetPrimvarDescriptors(id, HdInterpolationInstance)) {
      ...
      if (primvar.name == _tokens->instanceTransform) {
        ...
        HdArnoldInsertPrimvar(_primvars,
                              ...
                              GetDelegate()->Get(id, primvar.name));
      }
    }
  }
                                                  ...
                                                  HdArnoldSampledMatrixArrayType &sampleArray)
{
  ...
  // Similarly to the HdPrman render delegate, we take a look at the sampled values, and take the
  // one with the most samples and use its time range.
  // TODO(pal): Improve this further by using the widest time range and calculate sample count
  // based on that.
