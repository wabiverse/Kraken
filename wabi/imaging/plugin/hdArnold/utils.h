...
/// Converts constant scope primvars to built-in parameters. When the attribute holds an array, the
/// first element will be used.
///
/// If @param visibility is not a nullptr, the visibility calculation will store the value in the
/// pointed uint8_t instead of setting it on the node.
///
...
void HdArnoldSetVertexPrimvar(AtNode *node,
                              ...
                              HdSceneDelegate *sceneDelegate,
                              const HdPrimvarDescriptor &primvarDesc);
/// Sets a Face-Varying scope Primvar on an Arnold node from a Hydra Primitive. If @p vertexCounts
/// is not a nullptr and it is not empty, it is used to reverse the order of the generated face
/// vertex indices, to support left handed topologies. The total sum of the @p vertexCounts array
/// is expected to be the same as the number values stored in the primvar if @p vertexCountSum is
/// not provided.
///
/// @param node Pointer to an Arnold Node.
/// @param name Name of the primvar.
/// @param role Role of the primvar.
/// @param value Value of the primvar.
/// @param vertexCounts Optional pointer to the VtIntArray holding the face vertex counts for the
/// mesh.
...
/// Generates the idxs array for flattened USD values. When @p vertexCounts is not nullptr and not
/// empty, the the indices are reversed per polygon. The sum of the values stored in @p
/// vertexCounts is expected to match
/// @p numIdxs if @p vertexCountSum is not provided.
///
/// @param numIdxs Number of face vertex indices to generate.
/// @param vertexCounts Optional VtArrayInt pointer to the face vertex counts of the mesh or
/// nullptr.
/// @param vertexCountSum Optional size_t with sum of the vertexCounts.
...
/// Insert a primvar into a primvar map. Add a new entry if the primvar is not part of the map,
...
/// Get the non-computed primvars and ignoring the points primvar. If multiple position keys are
/// used, the function does not query the value of the normals.
///
/// @param delegate Pointer to the Hydra Scene Delegate.
/// @param id Path to the Hyra Primitive.
/// @param dirtyBits Dirty bits of what has changed for the current sync.
...
/// @param arnoldSubsets Output parameter containing the path to all the materials for each
