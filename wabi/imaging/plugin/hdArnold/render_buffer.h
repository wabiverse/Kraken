...
class HdArnoldRenderBuffer : public HdRenderBuffer
{
 public:
  ...
  HDARNOLD_API
  void WriteBucket(unsigned int bucketXO,
  ...
  /// Return wether or not the buffer has any updates. The function also resets the internal
  /// counter tracking the if there has been any updates.
  ///
  /// @return True if the buffer has any updates, false otherwise.
  bool HasUpdates()
  {
    return _hasUpdates.exchange(false, std::memory_order_acq_rel);
  }
