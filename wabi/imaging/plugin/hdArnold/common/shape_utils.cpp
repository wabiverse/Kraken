...
void ArnoldUsdReadCreases(AtNode *node,
...
/// Sets radius attribute on an Arnold shape from a VtValue holding VtFloatArray. We expect this to
/// be a width value, so a (*0.5) function will be applied to the values.
///
...
void ArnoldUsdCurvesData::SetRadiusFromValue(AtNode *node, const VtValue &value)
{
  ...
  if (value.IsHolding<VtFloatArray>()) {
    ...
    std::transform(values.begin(), values.end(), out, [](const float w) -> float {
      return w * 0.5f;
    });
    AiArrayUnmap(arr);
