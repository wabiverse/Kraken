...
class HdArnoldRenderDelegate final : public HdRenderDelegate
{
 public:
  ...
  ///
  /// This function can be used to skip calling the render function in HdRenderPass, so a sync step
  /// will be enforced before the next iteration.
  ///
  ...
  const DelegateRenderProducts &GetDelegateRenderProducts() const
  {
    return _delegateRenderProducts;
  }
  ...
  /// Use this to trigger forced updates on shapes, in cases where it's uncertain if the shape has
  /// it's material id updated.
  ///
