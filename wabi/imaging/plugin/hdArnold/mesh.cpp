...
namespace
{
  ...
  {
    inline static unsigned int f(AtNode *node, const StorageType &data, const AtString &arnoldName)
    {
      return false;
    }
  };
  ...
  struct _ConvertValueToArnoldParameter<UsdType, ArnoldType, VtValue>
  {
    inline static unsigned int f(AtNode *node, const VtValue &value, const AtString &arnoldName)
    {
      ...
      // Data comes in as flattened and in these cases the memory layout of the USD data matches
      // the memory layout of the Arnold data.
  ...
  {
    inline static unsigned int f(AtNode *node,
                                 const HdArnoldSampledPrimvarType &samples,
  ...
  template<typename UsdType, unsigned ArnoldType, typename StorageType>
  inline void _ConvertVertexPrimvarToBuiltin(AtNode *node,
                                             const StorageType &data,
                                             const AtString &arnoldName,
                                             const AtString &arnoldIndexName)
  {
    // We are receiving per vertex data, the way to support this is in arnold to use the values and
    // copy the vertex ids to the new ids for the given value.
    if (_ConvertValueToArnoldParameter<UsdType, ArnoldType, StorageType>::f(node,
                                                                            data,
                                                                            arnoldName) == 0) {
      return;
    }
  ...
  template<typename UsdType, unsigned ArnoldType, typename StorageType>
  inline void _ConvertFaceVaryingPrimvarToBuiltin(AtNode *node,
                                                  const StorageType &data,
                                                  const AtString &arnoldName,
                                                  const AtString &arnoldIndexName,
                                                  ...
                                                  const size_t *vertexCountSum = nullptr)
  {
    const auto numValues =
      _ConvertValueToArnoldParameter<UsdType, ArnoldType, StorageType>::f(node, data, arnoldName);
    if (numValues == 0) {
      return;
    }
    AiNodeSetArray(node,
                   arnoldIndexName,
                   HdArnoldGenerateIdxs(numValues, vertexCounts, vertexCountSum));
  }
                        ...
                        const TfToken &reprToken)
{
  ...
  const auto dirtyPrimvars = HdArnoldGetComputedPrimvars(sceneDelegate,
                                                         id,
                                                         *dirtyBits,
                                                         _primvars) ||
  ...
  if (_primvars.count(HdTokens->points) != 0) {
    ...
    param.Interrupt();
    _numberOfPositionKeys = HdArnoldSetPositionFromPrimvar(GetArnoldNode(),
                                                           id,
                                                           sceneDelegate,
                                                           str::vlist);
  }
  ...
  if (dirtyTopology) {
    ...
    _vertexCountSum = 0;
    // We are manually calculating the sum of the vertex counts here, because we are filtering for
    // negative values from the vertex indices array.
    if (isLeftHanded) {
      for (auto i = decltype(numFaces){0}; i < numFaces; ++i) {
        ...
        for (auto vertex = decltype(vertexCount){0}; vertex < vertexCount; vertex += 1) {
          vidxs[_vertexCountSum + vertexCount - vertex - 1] = static_cast<uint32_t>(
            vertexIndices[_vertexCountSum + vertex]);
        }
        _vertexCountSum += vertexCount;
      }
    } else {
      // We still need _vertexCountSum as it is used to validate primvars.
      // We are manually calculating the sum of the vertex counts here, because we are filtering
      // for negative values from the vertex indices array.
      std::transform(_vertexCounts.begin(),
                     _vertexCounts.end(),
                     nsides,
                     [&](const int i) -> uint32_t {
                       if (Ai_unlikely(i <= 0)) {
                         return 0;
                       }
      ...
      std::transform(vertexIndices.begin(),
                     vertexIndices.end(),
                     vidxs,
                     [](const int i) -> uint32_t {
    ...
    AiNodeSetArray(GetArnoldNode(),
                   str::shidxs,
                   HdArnoldGetShidxs(topology.GetGeomSubsets(), numFaces, _subsets));
  }
  ...
  if (HdChangeTracker::IsDisplayStyleDirty(*dirtyBits, id)) {
    ...
    AiNodeSetByte(GetArnoldNode(),
                  str::subdiv_iterations,
  ...
  if (HdChangeTracker::IsSubdivTagsDirty(*dirtyBits, id)) {
    ...
    ArnoldUsdReadCreases(GetArnoldNode(),
  ...
  auto assignMaterials = [&]() {
    ...
    // If there has been a change in data, we already detached materials and the two arrays are
    // different.
  ...
  if (dirtyPrimvars) {
    ...
    for (auto &primvar : _primvars) {
      ...
      if (desc.interpolation == HdInterpolationConstant) {
        HdArnoldSetConstantPrimvar(GetArnoldNode(),
                                   primvar.first,
                                   desc.role,
                                   desc.value,
                                   &visibility);
        ...
        if (primvar.first == _tokens->st || primvar.first == _tokens->uv) {
          _ConvertVertexPrimvarToBuiltin<GfVec2f, AI_TYPE_VECTOR2>(GetArnoldNode(),
                                                                   desc.value,
                                                                   str::uvlist,
                                                                   str::uvidxs);
          ...
          if (desc.value.IsEmpty()) {
            ...
            _ConvertVertexPrimvarToBuiltin<GfVec3f, AI_TYPE_VECTOR>(GetArnoldNode(),
                                                                    sample,
                                                                    str::nlist,
                                                                    str::nidxs);
          } else {
          ...
          // If we get to points here, it's a computed primvar, so we need to use a different
          // function.
        ...
        if (primvar.first == _tokens->st || primvar.first == _tokens->uv) {
          _ConvertFaceVaryingPrimvarToBuiltin<GfVec2f, AI_TYPE_VECTOR2>(GetArnoldNode(),
                                                                        desc.value,
                                                                        str::uvlist,
                                                                        str::uvidxs,
                                                                        &_vertexCounts,
          ...
          if (desc.value.IsEmpty()) {
            ...
            sceneDelegate->SamplePrimvar(id, primvar.first, &sample);
            sample.count = _numberOfPositionKeys;
            _ConvertFaceVaryingPrimvarToBuiltin<GfVec3f, AI_TYPE_VECTOR>(GetArnoldNode(),
                                                                         sample,
                                                                         str::nlist,
                                                                         str::nidxs,
                                                                         &_vertexCounts,
          ...
          HdArnoldSetFaceVaryingPrimvar(GetArnoldNode(),
                                        primvar.first,
                                        desc.role,
                                        desc.value,
                                        &_vertexCounts,
...
bool HdArnoldMesh::_IsVolume() const
{
  return AiNodeGetFlt(GetArnoldNode(), str::step_size) > 0.0f;
}
