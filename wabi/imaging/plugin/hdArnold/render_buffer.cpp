...
namespace
{
  ...
  // xo, xe, yo, ye is already clamped against width and height and we checked corner cases when
  // the bucket is empty.
...
bool HdArnoldRenderBuffer::IsMapped() const
{
  return false;
}
...
void HdArnoldRenderBuffer::WriteBucket(unsigned int bucketXO,
                                       unsigned int bucketYO,
                                       unsigned int bucketWidth,
                                       unsigned int bucketHeight,
                                       HdFormat format,
                                       const void *bucketData)
{
  if (!_SupportedComponentFormat(format)) {
  ...
  // The simplest case is when the component format and the count matches, we can copy per line in
  // this case. If the the component format matches, but the count is different, we are copying as
  // much data as we can
  ...
  if (componentFormat == inComponentFormat) {
    ...
    if (inComponentCount == componentCount) {
      ...
      const auto copiedDataSize = std::min(inComponentCount, componentCount) *
                                  HdDataSizeOfFormat(componentFormat);
