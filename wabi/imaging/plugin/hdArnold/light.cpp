...
TF_DEFINE_PRIVATE_TOKENS(
  ...
  std::vector<ParamDesc> distantParams = {
    {"angle", UsdLuxTokens->inputsAngle}
  };
  ...
  auto spotLightSync = [](AtNode *light,
                          AtNode **filter,
                          const AtNodeEntry *nentry,
                          const SdfPath &id,
    ...
    const auto hdAngle = sceneDelegate
                           ->GetLightParamValue(id, UsdLuxTokens->inputsShapingConeAngle)
    ...
    auto getBarndoor = [&](const TfToken &name) -> float {
      const auto barndoor =
        AiClamp(sceneDelegate->GetLightParamValue(id, name).GetWithDefault(0.0f), 0.0f, 1.0f);
    ...
    auto createBarndoor = [&]() {
      *filter = AiNode(renderDelegate->GetUniverse(), str::barndoor);
    };
    if (hasBarndoor) {
  ...
  auto pointLightSync = [](AtNode *light,
                           AtNode **filter,
                           const AtNodeEntry *nentry,
                           const SdfPath &id,
                           HdSceneDelegate *sceneDelegate,
    ...
    const auto treatAsPointValue = sceneDelegate->GetLightParamValue(id,
  ...
  auto photometricLightSync = [](AtNode *light,
                                 AtNode **filter,
                                 const AtNodeEntry *nentry,
                                 const SdfPath &id,
                                 HdSceneDelegate *sceneDelegate,
  ...
  auto distantLightSync = [](AtNode *light,
                             AtNode **filter,
                             const AtNodeEntry *nentry,
                             const SdfPath &id,
                             HdSceneDelegate *sceneDelegate,
  ...
  auto diskLightSync = [](AtNode *light,
                          AtNode **filter,
                          const AtNodeEntry *nentry,
                          const SdfPath &id,
                          HdSceneDelegate *sceneDelegate,
  ...
  auto rectLightSync = [](AtNode *light,
                          AtNode **filter,
                          const AtNodeEntry *nentry,
                          const SdfPath &id,
                          HdSceneDelegate *sceneDelegate,
  ...
  auto cylinderLightSync = [](AtNode *light,
                              AtNode **filter,
                              const AtNodeEntry *nentry,
                              const SdfPath &id,
                              HdSceneDelegate *sceneDelegate,
  ...
  auto domeLightSync = [](AtNode *light,
                          AtNode **filter,
                          const AtNodeEntry *nentry,
                          const SdfPath &id,
                          HdSceneDelegate *sceneDelegate,
  ...
  class HdArnoldGenericLight : public HdLight
  {
   public:
    ...
    HdArnoldGenericLight(HdArnoldRenderDelegate *delegate,
                         const SdfPath &id,
                         const AtString &arnoldType,
                         const SyncParams &sync,
  ...
  HdArnoldGenericLight::HdArnoldGenericLight(HdArnoldRenderDelegate *delegate,
                                             const SdfPath &id,
                                             const AtString &arnoldType,
  ...
  void HdArnoldGenericLight::Sync(HdSceneDelegate *sceneDelegate,
                                  HdRenderParam *renderParam,
                                  HdDirtyBits *dirtyBits)
  {
    ...
    if (*dirtyBits & HdLight::DirtyParams) {
      // If the params have changed, we need to see if any of the shaping parameters were applied
      // to the sphere light.
      ...
      *dirtyBits |= HdLight::DirtyTransform;
      ...
      for (const auto &primvar :
        ...
        ConvertPrimvarToBuiltinParameter(
          _light,
          primvar.name,
          sceneDelegate->Get(id, TfToken{TfStringPrintf("primvars:%s", primvar.name.GetText())}));
      }
      ...
      if (filtersValue.IsHolding<SdfPathVector>()) {
        ...
        if (filters.empty()) {
          ...
          AiNodeSetArray(_light,
                         str::filters,
                         AiArrayConvert(filters.size(), 1, AI_TYPE_NODE, filters.data()));
        }
      }
    }
  ...
  AtNode *HdArnoldGenericLight::GetLightNode() const
  {
    return _light;
  }
