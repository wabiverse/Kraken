...
class HdArnoldRenderParam final : public HdRenderParam {
 public:
  ...
  ///
  /// @param needsRestart Whether or not changes are applied to the scene and we need to restart
  /// rendering.
  /// @param clearStatus Clears the internal failure status. Set it to false when no scene data
  /// changed, that could
  ...
  HDARNOLD_API
  void Pause();
  /// Resumes an already paused render, does nothing if no render is running, or the render is not
  /// paused.
  HDARNOLD_API
  void Resume();
