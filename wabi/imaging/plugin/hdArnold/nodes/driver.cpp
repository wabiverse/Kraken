...
node_update
{
  ...
  data->renderBuffers = static_cast<HdArnoldRenderBufferStorage *>(
    AiNodeGetPtr(node, str::aov_pointer));
  // TODO(pal): Clear AOVs.
}
...
driver_process_bucket
{
  ...
  while (AiOutputIteratorGetNext(iterator, &outputName, &pixelType, &bucketData)) {
    if (pixelType == AI_TYPE_VECTOR && strcmp(outputName, "P") == 0) {
      ...
      buckets.emplace_back(strcmp(outputName, "RGBA") == 0 ? HdAovTokens->color :
                                                             TfToken{outputName},
                           pixelType,
                           bucketData);
    }
