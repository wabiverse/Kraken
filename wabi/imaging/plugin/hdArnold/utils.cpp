...
namespace
{
  ...
  auto nodeSetBoolFromInt = [](AtNode *node, const AtString paramName, int v) {
  ...
  auto nodeSetStrFromAssetPath =
    [](AtNode *node, const AtString paramName, const SdfAssetPath &v) {
      AiNodeSetStr(node,
                   paramName,
                   v.GetResolvedPath().empty() ? AtString(v.GetAssetPath().c_str()) :
                                                 AtString(v.GetResolvedPath().c_str()));
    };
  ...
  inline bool _Declare(AtNode *node,
                       const TfToken &name,
                       const TfToken &scope,
                       const TfToken &type)
  {
    if (AiNodeEntryLookUpParameter(AiNodeGetNodeEntry(node), AtString(name.GetText())) !=
        nullptr) {
      TF_DEBUG(HDARNOLD_PRIMVARS)
        .Msg(
          "Unable to translate %s primvar for %s due to a name collision with a built-in "
          "parameter",
          name.GetText(),
          AiNodeGetName(node));
      return false;
    ...
    return AiNodeDeclare(
      node,
      AtString(name.GetText()),
      AtString(TfStringPrintf("%s %s", scope.GetText(), type.GetText()).c_str()));
  }

  template<typename T>
  inline uint32_t _ConvertArray(AtNode *node,
                                const AtString &name,
                                uint8_t arnoldType,
                                const VtValue &value)
  {
  ...
  inline uint32_t _ConvertArrayTyped(AtNode *node,
                                     const AtString &name,
                                     uint8_t arnoldType,
                                     const VtValue &value)
  {
  ...
  template<typename TO, typename FROM>
  inline uint32_t _ConvertArrayTuple(AtNode *node,
                                     const AtString &name,
                                     uint8_t arnoldType,
                                     const VtValue &value)
  {
  ...
  template<typename T> inline void _ConvertToString(AtString &to, const T &from)
  {
    to = {};
  }

  template<> inline void _ConvertToString<std::string>(AtString &to, const std::string &from)
  {
    to = AtString{from.c_str()};
  }

  template<> inline void _ConvertToString<TfToken>(AtString &to, const TfToken &from)
  {
    to = AtString{from.GetText()};
  }

  template<> inline void _ConvertToString<SdfAssetPath>(AtString &to, const SdfAssetPath &from)
  {
    to = AtString{from.GetResolvedPath().empty() ? from.GetAssetPath().c_str() :
                                                   from.GetResolvedPath().c_str()};
  }

  // Anything but string should be trivially copyable.
  template<typename T> AtArray *_ArrayConvert(const VtArray<T> &v, uint8_t arnoldType)
  {
  ...
  template<typename TO, typename FROM>
  inline uint32_t _DeclareAndConvertArrayTyped(AtNode *node,
                                               const TfToken &name,
                                               const TfToken &scope,
                                               const TfToken &type,
                                               uint8_t arnoldType,
                                               const VtValue &value,
                                               bool isConstant,
                                               void (*f)(AtNode *, const AtString, FROM))
  {
  ...
  template<typename TO, typename FROM>
  inline uint32_t _DeclareAndConvertArrayTuple(AtNode *node,
                                               const TfToken &name,
                                               const TfToken &scope,
                                               const TfToken &type,
                                               uint8_t arnoldType,
                                               ...
                                               void (*f)(AtNode *, const AtString, FROM))
  {
    ...
    auto *arr = AiArrayAllocate(v.size(), 1, arnoldType);
    std::transform(reinterpret_cast<const typename CFROM::ScalarType *>(v.data()),
                   reinterpret_cast<const typename CFROM::ScalarType *>(v.data()) +
                     v.size() * CFROM::dimension,
                   reinterpret_cast<TO *>(AiArrayMap(arr)),
                   [](const typename CFROM::ScalarType &from) -> TO {
                     return static_cast<TO>(from);
                   });
    AiArrayUnmap(arr);
    AiNodeSetArray(node, AtString(name.GetText()), arr);
  ...
  template<typename T>
  inline uint32_t _DeclareAndConvertArray(AtNode *node,
                                          const TfToken &name,
                                          const TfToken &scope,
                                          const TfToken &type,
                                          uint8_t arnoldType,
                                          const VtValue &value,
                                          bool isConstant,
                                          void (*f)(AtNode *, const AtString, T))
  {
    // We are removing const and reference from the type. When using std::string or SdfAssetPath,
    // we want to use a function pointer with const& type, because we'll be providing our own
    // lambda to do the conversion, and we don't want to copy complex types. For other cases,
    // Arnold functions are receiving types by their value. We can't use a template to
    // automatically deduct the type of the functions, because the AiNodeSet functions have
    // overrides for both const char* and AtString in their second parameter, so we are forcing the
    // deduction using the function pointer.
  ...
  template<typename TO, typename FROM>
  inline void _DeclareAndConvertInstanceArrayTyped(AtNode *node,
                                                   const TfToken &name,
                                                   const TfToken &type,
                                                   uint8_t arnoldType,
                                                   const VtValue &value,
  ...
  template<typename TO, typename FROM>
  inline void _DeclareAndConvertInstanceArrayTuple(AtNode *node,
                                                   const TfToken &name,
                                                   const TfToken &type,
                                                   uint8_t arnoldType,
                                                   const VtValue &value,
                                                   const VtIntArray &indices)
  {
    ...
    for (auto id = decltype(numIndices){0}; id < numIndices; id += 1) {
      const auto index = indices[id];
      if (Ai_likely(index >= 0 && static_cast<size_t>(index) < numValues)) {
        std::transform(data + index * CFROM::dimension,
                       data + (index + 1) * CFROM::dimension,
                       mapped + id * CFROM::dimension,
                       [](const typename CFROM::ScalarType &from) -> TO {
                         return static_cast<TO>(from);
                       });
      } else {
  ...
  template<typename T>
  inline void _DeclareAndConvertInstanceArray(AtNode *node,
                                              const TfToken &name,
                                              const TfToken &type,
                                              uint8_t arnoldType,
                                              const VtValue &value,
  ...
  inline uint32_t _DeclareAndAssignFromArray(AtNode *node,
                                             const TfToken &name,
                                             const TfToken &scope,
                                             const VtValue &value,
                                             bool isColor,
                                             bool isConstant = false)
  {
    if (value.IsHolding<VtBoolArray>()) {
      return _DeclareAndConvertArray<bool>(node,
                                           name,
                                           scope,
                                           _tokens->BOOL,
                                           AI_TYPE_BOOLEAN,
      ...
      return _DeclareAndConvertArray<VtUCharArray::value_type>(node,
                                                               name,
                                                               scope,
                                                               _tokens->BYTE,
                                                               AI_TYPE_BYTE,
      ...
      return _DeclareAndConvertArray<unsigned int>(node,
                                                   name,
                                                   scope,
                                                   _tokens->UINT,
                                                   AI_TYPE_UINT,
      ...
      return _DeclareAndConvertArray<int>(node,
                                          name,
                                          scope,
                                          _tokens->INT,
                                          AI_TYPE_INT,
      ...
      return _DeclareAndConvertArray<float>(node,
                                            name,
                                            scope,
                                            _tokens->FLOAT,
                                            AI_TYPE_FLOAT,
      ...
      return _DeclareAndConvertArray<const GfVec2f &>(node,
                                                      name,
                                                      scope,
                                                      _tokens->VECTOR2,
                                                      AI_TYPE_VECTOR2,
      ...
      if (isColor) {
        return _DeclareAndConvertArray<const GfVec3f &>(node,
                                                        name,
                                                        scope,
                                                        _tokens->RGB,
                                                        AI_TYPE_RGB,
      ...
      return _DeclareAndConvertArray<const GfVec4f &>(node,
                                                      name,
                                                      scope,
                                                      _tokens->RGBA,
                                                      AI_TYPE_RGBA,
      ...
      return _DeclareAndConvertArray<const std::string &>(node,
                                                          name,
                                                          scope,
                                                          _tokens->STRING,
                                                          AI_TYPE_STRING,
                                                          value,
                                                          isConstant,
                                                          nodeSetStrFromStdStr);
    } else if (value.IsHolding<VtTokenArray>()) {
      return _DeclareAndConvertArray<TfToken>(node,
                                              name,
                                              scope,
                                              _tokens->STRING,
                                              AI_TYPE_STRING,
      ...
      return _DeclareAndConvertArray<const SdfAssetPath &>(node,
                                                           name,
                                                           scope,
                                                           _tokens->STRING,
                                                           AI_TYPE_STRING,
                                                           value,
                                                           isConstant,
                                                           nodeSetStrFromAssetPath);
    } else if (value.IsHolding<VtArray<GfHalf>>()) {  // HALF types
      return _DeclareAndConvertArrayTyped<float, GfHalf>(node,
                                                         name,
                                                         scope,
                                                         _tokens->FLOAT,
                                                         AI_TYPE_FLOAT,
      ...
      return _DeclareAndConvertArrayTuple<float, const GfVec2h &>(node,
                                                                  name,
                                                                  scope,
                                                                  _tokens->VECTOR2,
                                                                  AI_TYPE_VECTOR2,
                                                                  value,
                                                                  isConstant,
                                                                  nodeSetVec2FromVec2h);
    } else if (value.IsHolding<VtArray<GfVec3h>>()) {
      if (isColor) {
        return _DeclareAndConvertArrayTuple<float, const GfVec3h &>(node,
                                                                    name,
                                                                    scope,
                                                                    _tokens->RGB,
                                                                    AI_TYPE_RGB,
      ...
      return _DeclareAndConvertArrayTuple<float, const GfVec4h &>(node,
                                                                  name,
                                                                  scope,
                                                                  _tokens->RGBA,
                                                                  AI_TYPE_RGBA,
                                                                  value,
                                                                  isConstant,
                                                                  nodeSetRGBAFromVec4h);
    } else if (value.IsHolding<VtArray<double>>()) {  // double types
      return _DeclareAndConvertArrayTyped<float, double>(node,
                                                         name,
                                                         scope,
                                                         _tokens->FLOAT,
                                                         AI_TYPE_FLOAT,
                                                         value,
                                                         isConstant,
                                                         nodeSetFltFromDouble);
    } else if (value.IsHolding<VtArray<GfVec2d>>()) {
      return _DeclareAndConvertArrayTuple<float, const GfVec2d &>(node,
                                                                  name,
                                                                  scope,
                                                                  _tokens->VECTOR2,
                                                                  AI_TYPE_VECTOR2,
                                                                  value,
                                                                  isConstant,
                                                                  nodeSetVec2FromVec2d);
    } else if (value.IsHolding<VtArray<GfVec3d>>()) {
      if (isColor) {
        return _DeclareAndConvertArrayTuple<float, const GfVec3d &>(node,
                                                                    name,
                                                                    scope,
                                                                    _tokens->RGB,
                                                                    AI_TYPE_RGB,
      ...
      return _DeclareAndConvertArrayTuple<float, const GfVec4d &>(node,
                                                                  name,
                                                                  scope,
                                                                  _tokens->RGBA,
                                                                  AI_TYPE_RGBA,
                                                                  value,
                                                                  isConstant,
                                                                  nodeSetRGBAFromVec4d);
    }
    return 0;
  }

  inline void _DeclareAndAssignConstant(AtNode *node,
                                        const TfToken &name,
                                        const VtValue &value,
                                        bool isColor = false)
  {
  ...
  inline void _DeclareAndAssignInstancePrimvar(AtNode *node,
                                               const TfToken &name,
                                               const VtValue &value,
                                               bool isColor,
                                               const VtIntArray &indices)
  {
    if (value.IsHolding<VtBoolArray>()) {
      _DeclareAndConvertInstanceArray<bool>(node,
                                            name,
                                            _tokens->BOOL,
                                            AI_TYPE_BOOLEAN,
                                            value,
      ...
      _DeclareAndConvertInstanceArray<VtUCharArray::value_type>(node,
                                                                name,
                                                                _tokens->BYTE,
                                                                AI_TYPE_BYTE,
                                                                value,
      ...
      _DeclareAndConvertInstanceArray<unsigned int>(node,
                                                    name,
                                                    _tokens->UINT,
                                                    AI_TYPE_UINT,
                                                    value,
      ...
      _DeclareAndConvertInstanceArray<float>(node,
                                             name,
                                             _tokens->FLOAT,
                                             AI_TYPE_FLOAT,
                                             value,
      ...
      _DeclareAndConvertInstanceArray<const GfVec2f &>(node,
                                                       name,
                                                       _tokens->VECTOR2,
                                                       AI_TYPE_VECTOR2,
                                                       value,
      ...
      if (isColor) {
        _DeclareAndConvertInstanceArray<const GfVec3f &>(node,
                                                         name,
                                                         _tokens->RGB,
                                                         AI_TYPE_RGB,
                                                         value,
      ...
      _DeclareAndConvertInstanceArray<const GfVec4f &>(node,
                                                       name,
                                                       _tokens->RGBA,
                                                       AI_TYPE_RGBA,
                                                       value,
      ...
      _DeclareAndConvertInstanceArray<const std::string &>(node,
                                                           name,
                                                           _tokens->STRING,
                                                           AI_TYPE_STRING,
                                                           value,
      ...
      _DeclareAndConvertInstanceArray<TfToken>(node,
                                               name,
                                               _tokens->STRING,
                                               AI_TYPE_STRING,
                                               value,
      ...
      _DeclareAndConvertInstanceArray<const SdfAssetPath &>(node,
                                                            name,
                                                            _tokens->STRING,
                                                            AI_TYPE_STRING,
                                                            value,
      ...
      _DeclareAndConvertInstanceArrayTyped<float, GfHalf>(node,
                                                          name,
                                                          _tokens->FLOAT,
                                                          AI_TYPE_FLOAT,
                                                          value,
      ...
      _DeclareAndConvertInstanceArrayTuple<float, GfVec2h>(node,
                                                           name,
                                                           _tokens->VECTOR2,
                                                           AI_TYPE_VECTOR2,
                                                           value,
      ...
      if (isColor) {
        _DeclareAndConvertInstanceArrayTuple<float, GfVec3h>(node,
                                                             name,
                                                             _tokens->RGB,
                                                             AI_TYPE_RGB,
                                                             value,
      ...
      _DeclareAndConvertInstanceArrayTuple<float, GfVec4h>(node,
                                                           name,
                                                           _tokens->RGBA,
                                                           AI_TYPE_RGBA,
                                                           value,
      ...
      _DeclareAndConvertInstanceArrayTyped<float, double>(node,
                                                          name,
                                                          _tokens->FLOAT,
                                                          AI_TYPE_FLOAT,
                                                          value,
      ...
      _DeclareAndConvertInstanceArrayTuple<float, GfVec2d>(node,
                                                           name,
                                                           _tokens->VECTOR2,
                                                           AI_TYPE_VECTOR2,
                                                           value,
      ...
      if (isColor) {
        _DeclareAndConvertInstanceArrayTuple<float, GfVec3d>(node,
                                                             name,
                                                             _tokens->RGB,
                                                             AI_TYPE_RGB,
                                                             value,
      ...
      _DeclareAndConvertInstanceArrayTuple<float, GfVec4d>(node,
                                                           name,
                                                           _tokens->RGBA,
                                                           AI_TYPE_RGBA,
                                                           value,
  ...
  inline bool _CharStartsWithToken(const char *c, const TfToken &t)
  {
    return strncmp(c, t.GetText(), t.size()) == 0;
  }
  ...
  inline void _SetRayFlag(AtNode *node,
  ...
  // We are using function pointers instead of template arguments to deduct the function type,
  // because Arnold's AiNodeSetXXX functions have overrides in the form of, void (*) (AtNode*,
  // const char*, T v) and void (*) (AtNode*, AtString, T v), so the compiler is unable to deduct
  // which function to use. Using function pointers to force deduction is the easiest way, yet
  // lambdas are still inlined. This way, we can still use AiNodeSetXXX functions where possible,
  // and we only need to create a handful of functions to wrap the more complex type conversions.
  template<typename T>
  inline bool _SetFromValueOrArray(AtNode *node,
                                   const AtString &paramName,
                                   const VtValue &value,
                                   void (*f)(AtNode *, const AtString, T))
  {
...
void HdArnoldSetTransform(AtNode *node, HdSceneDelegate *sceneDelegate, const SdfPath &id)
{
  ...
  // We expect the samples to be sorted, and we reset motion start and motion end if there is only
  // one sample. This might be an [] in older USD versions, so not using standard container
  // accessors.
  if (xf.count > 1) {
...
void HdArnoldSetTransform(const std::vector<AtNode *> &nodes,
...
void HdArnoldSetParameter(AtNode *node, const AtParamEntry *pentry, const VtValue &value)
{
  ...
  if (paramType == AI_TYPE_ARRAY) {
    ...
    switch (arrayType) {
      ...
      case AI_TYPE_RGBA:
        ...
        break;
      default:
        AiMsgError("Unsupported array parameter %s.%s",
                   AiNodeGetName(node),
  ...
  switch (paramType) {
    case AI_TYPE_BYTE:
      _SetFromValueOrArray<int, unsigned char, long>(node,
                                                     paramName,
                                                     value,
                                                     nodeSetByteFromInt,
                                                     nodeSetByteFromUChar,
                                                     nodeSetByteFromLong);
      break;
    case AI_TYPE_INT:
    ...
    case AI_TYPE_USHORT:
      ...
      break;
    case AI_TYPE_BOOLEAN:
      _SetFromValueOrArray<bool, int, long>(node,
                                            paramName,
                                            value,
                                            AiNodeSetBool,
                                            nodeSetBoolFromInt,
    ...
    case AI_TYPE_FLOAT:
    case AI_TYPE_HALF:
      _SetFromValueOrArray<float, GfHalf, double>(node,
                                                  paramName,
                                                  value,
                                                  AiNodeSetFlt,
                                                  nodeSetFltFromHalf,
      ...
      break;
    case AI_TYPE_RGB:
      _SetFromValueOrArray<const GfVec3f &, const GfVec3h &, const GfVec3d &>(node,
                                                                              paramName,
                                                                              value,
                                                                              nodeSetRGBFromVec3,
                                                                              nodeSetRGBFromVec3h,
    ...
    case AI_TYPE_MATRIX:
      ...
      break;
    case AI_TYPE_ENUM:
      _SetFromValueOrArray<int, long, TfToken, const std::string &>(node,
                                                                    paramName,
                                                                    value,
                                                                    AiNodeSetInt,
                                                                    nodeSetIntFromLong,
    ...
    default:
      AiMsgError("Unsupported parameter %s.%s",
                 AiNodeGetName(node),
                 AiParamGetName(pentry).c_str());
  }
}

bool ConvertPrimvarToBuiltinParameter(AtNode *node,
                                      const TfToken &name,
                                      const VtValue &value,
                                      uint8_t *visibility)
{
...
void HdArnoldSetConstantPrimvar(AtNode *node,
                                const TfToken &name,
                                const TfToken &role,
                                const VtValue &value,
                                uint8_t *visibility)
...
void HdArnoldSetConstantPrimvar(AtNode *node,
                                ...
                                uint8_t *visibility)
{
  HdArnoldSetConstantPrimvar(node,
                             primvarDesc.name,
                             primvarDesc.role,
                             sceneDelegate->Get(id, primvarDesc.name),
                             visibility);
}
...
void HdArnoldSetUniformPrimvar(AtNode *node,
                               ...
                               const VtValue &value)
{
  _DeclareAndAssignFromArray(node,
                             name,
                             _tokens->uniform,
                             value,
...
void HdArnoldSetFaceVaryingPrimvar(AtNode *node,
                                   const TfToken &name,
                                   const TfToken &role,
                                   const VtValue &value,
...
{
  const auto numElements = _DeclareAndAssignFromArray(node,
                                                      name,
                                                      _tokens->indexed,
                                                      value,
                                                      role == HdPrimvarRoleTokens->color);
  ...
  AiNodeSetArray(node,
                 AtString(TfStringPrintf("%sidxs", name.GetText()).c_str()),
...
void HdArnoldSetFaceVaryingPrimvar(AtNode *node,
                                   ...
                                   const size_t *vertexCountSum)
{
  HdArnoldSetFaceVaryingPrimvar(node,
                                primvarDesc.name,
                                primvarDesc.role,
                                sceneDelegate->Get(id, primvarDesc.name),
                                vertexCounts,
...
void HdArnoldSetInstancePrimvar(AtNode *node,
                                const TfToken &name,
                                const TfToken &role,
                                const VtIntArray &indices,
                                const VtValue &value)
{
  _DeclareAndAssignInstancePrimvar(node,
                                   TfToken{TfStringPrintf("instance_%s", name.GetText())},
                                   value,
                                   role == HdPrimvarRoleTokens->color,
                                   indices);
}

size_t HdArnoldSetPositionFromPrimvar(AtNode *node,
                                      const SdfPath &id,
                                      HdSceneDelegate *sceneDelegate,
                                      const AtString &paramName)
{
...
void HdArnoldSetRadiusFromPrimvar(AtNode *node, const SdfPath &id, HdSceneDelegate *sceneDelegate)
{
  ...
  auto convertWidth = [](const float w) -> float {
    return w * 0.5f;
  };
...
AtArray *HdArnoldGenerateIdxs(unsigned int numIdxs,
                              const VtIntArray *vertexCounts,
                              const size_t *vertexCountSum)
{
...
void HdArnoldInsertPrimvar(HdArnoldPrimvarMap &primvars,
                           const TfToken &name,
                           const TfToken &role,
...
bool HdArnoldGetComputedPrimvars(HdSceneDelegate *delegate,
                                 const SdfPath &id,
                                 ...
                                 const std::vector<HdInterpolation> *interpolations)
{
  ...
  for (auto interpolation :
       (interpolations == nullptr ? primvarInterpolations : *interpolations)) {
  ...
  for (const auto &primvar : dirtyPrimvars) {
    ...
    changed = true;
    HdArnoldInsertPrimvar(primvars,
                          primvar.name,
                          primvar.role,
...
void HdArnoldGetPrimvars(HdSceneDelegate *delegate,
                         const SdfPath &id,
                         HdDirtyBits dirtyBits,
                         bool multiplePositionKeys,
                         HdArnoldPrimvarMap &primvars,
                         const std::vector<HdInterpolation> *interpolations)
{
  for (auto interpolation :
