...
namespace
{
  ...
  AtNode *_CreateFilter(HdArnoldRenderDelegate *renderDelegate,
                        const HdAovSettingsMap &aovSettings)
  {
    ...
    const auto filterType = _GetOptionalSetting(aovSettings,
                                                _tokens->aovSettingFilter,
                                                std::string{});
    if (filterType.empty()) {
      return nullptr;
    }
    ...
    _renderDelegate(renderDelegate)
{
  ...
  const auto beautyString = TfStringPrintf("RGBA RGBA %s %s",
                                           AiNodeGetName(_defaultFilter),
                                           AiNodeGetName(_mainDriver));
  const auto positionString = TfStringPrintf("P VECTOR %s %s",
                                             AiNodeGetName(_closestFilter),
                                             AiNodeGetName(_mainDriver));
  const auto idString = TfStringPrintf("ID UINT %s %s",
                                       AiNodeGetName(_closestFilter),
                                       AiNodeGetName(_mainDriver));
                                  ...
                                  const TfTokenVector &renderTags)
{
  ...
  if (useOwnedCamera) {
    ...
    if (currentUniverseCamera != currentCamera) {
      renderParam->Interrupt();
      AiNodeSetPtr(AiUniverseGetOptions(_renderDelegate->GetUniverse()),
                   str::camera,
                   currentCamera);
    }
  }
    ...
    for (auto &buffer : storage) {
      if (buffer.second.buffer != nullptr) {
        buffer.second.buffer
          ->WriteBucket(0, 0, _width, _height, HdFormatUNorm8Vec4, zeroData.data());
      }
    }
  };
    ...
    if (_RenderBuffersChanged(aovBindings) ||
      ...
      for (const auto &binding : aovBindings) {
        ...
        const auto *filterGeoName = buffer.filter != nullptr ? AiNodeGetName(buffer.filter) :
                                                               closestName;
        const auto sourceType = _GetOptionalSetting<TfToken>(binding.aovSettings,
                                                             _tokens->sourceType,
                                                             _tokens->raw);
        const auto sourceName = _GetOptionalSetting<std::string>(binding.aovSettings,
                                                                 _tokens->sourceName,
                                                                 binding.aovName.GetString());
      ...
      if (!delegateRenderProducts.empty() && _deepProducts.empty()) {
        ...
        for (const auto &product : delegateRenderProducts) {
          ...
          for (const auto &renderVar : product.renderVars) {
            ...
            *tolerance = _GetOptionalSetting<float>(renderVar.settings,
                                                    _tokens->tolerance,
                                                    defaultTolerance);
            *enableFiltering = _GetOptionalSetting<bool>(renderVar.settings,
                                                         _tokens->enableFiltering,
                                                         defaultEnableFiltering);
            *halfPrecision = _GetOptionalSetting<bool>(renderVar.settings,
                                                       _tokens->halfPrecision,
                                                       defaultHalfPrecision);
            ...
            if (isRaw && renderVar.sourceName == HdAovTokens->color) {
              ...
              const auto format = _GetOptionalSetting<TfToken>(
                renderVar.settings,
                _tokens->dataType,
                _GetTokenFromHdFormat(renderVar.format));
              const auto arnoldTypes = _GetArnoldAOVTypeFromTokenType(format);
      ...
      if (!outputs.empty()) {
        AiNodeSetArray(_renderDelegate->GetOptions(),
                       str::outputs,
                       AiArrayConvert(static_cast<uint32_t>(outputs.size()),
                                      1,
                                      AI_TYPE_STRING,
                                      outputs.data()));
      }
...
#    endif
                                   color);
      _fullscreenShader.SetTexture(_tokens->depth,
                                   _width,
                                   _height,
                                   HdFormatFloat32,
                                   reinterpret_cast<uint8_t *>(depth));
