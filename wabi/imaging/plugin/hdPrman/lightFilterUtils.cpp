...
bool HdPrmanLightFilterPopulateParams(riley::ShadingNode *filter,
                                      SdfPath &filterPath,
                                      TfToken filterType,
                                      ...
                                      RtUString lightTypeName)
{
  ...
  if (filterType == _tokens->PxrIntMultLightFilter) {
    ...
    pval = sceneDelegate->GetLightParamValue(filterPath, TfToken("colorSaturation"));
    if (pval.IsHolding<float>()) {
    ...
    if (pval.IsHolding<std::vector<GfVec3f>>()) {
      ...
      filter->params.SetColorArray(
        RtUString("colorRamp_Colors"), reinterpret_cast<const RtColorRGB *>(&v[0]), v.size());
    }
  ...
  if (genCoordSys) {
    ...
    if (genParentShader)
      filter->params.SetString(RtUString("__lightFilterParentShader"), lightTypeName);
  }
