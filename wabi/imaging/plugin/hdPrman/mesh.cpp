  ...
  *geomSubsets = topology.GetGeomSubsets();
  if (!geomSubsets->empty())
  {
    ...
    if (numUnusedFaces)
    {
      ...
      size_t count = 0;
      for (size_t i = 0;
           i < faceMask.size() && count < numUnusedFaces;
           ++i)
      {
        if (faceMask[i])
        {
