...
#include "wabi/imaging/plugin/hdPrman/matfiltConvertPreviewMaterial.h"
...
TfTokenVector const &HdPrmanMaterial::GetShaderSourceTypes()
{
  return *_sourceTypes;
}
...
{
  *_filterChain = {MatfiltConvertPreviewMaterial,
...
MatfiltFilterChain HdPrmanMaterial::GetFilterChain()
{
  return *_filterChain;
}

void HdPrmanMaterial::SetFilterChain(MatfiltFilterChain const &chain)
{
  *_filterChain = chain;
}
...
static VtArray<GfVec3f> _ConvertToVec3fArray(const VtArray<GfVec3d> &v)
{
  VtArray<GfVec3f> out;
  out.resize(v.size());
...
static bool _ConvertNodes(HdMaterialNetwork2 const &network,
                          ...
                          _PathSet *visitedNodes)
{
  ...
  for (const auto &param : node.parameters) {
    ...
    if (propType.IsEmpty()) {
      ...
      TF_DEBUG(HDPRMAN_MATERIALS)
        .Msg(
          "Unknown shader entry field type for "
...
static void _ConvertHdMaterialNetwork2ToRman(HdPrman_Context *context,
                                             SdfPath const &id,
                                             ...
                                             riley::DisplacementId *displacementId)
{
  ...
  for (auto const &terminal : network.terminals) {
    _PathSet visitedNodes;
    if (_ConvertNodes(network, terminal.second.upstreamNode, &nodes, &visitedNodes)) {
      if (terminal.first == HdMaterialTerminalTokens->surface ||
        ...
        if (*materialId == riley::MaterialId::k_InvalidId) {
          *materialId = riley->CreateMaterial(&nodes[0], nodes.size());
        }
        else {
        ...
        if (*displacementId == riley::DisplacementId::k_InvalidId) {
          *displacementId = riley->CreateDisplacement(&nodes[0], nodes.size());
        }
        else {
          riley->ModifyDisplacement(*displacementId, &nodes[0], nodes.size());
        }
        if (*displacementId == riley::DisplacementId::k_InvalidId) {
...
/* virtual */
void HdPrmanMaterial::Sync(HdSceneDelegate *sceneDelegate,
...
/* virtual */
HdDirtyBits HdPrmanMaterial::GetInitialDirtyBitsMask() const
{
  return HdChangeTracker::AllDirty;
}

bool HdPrmanMaterial::IsValid() const
{
