...
void HdPrman_Context::IncrementLightLinkCount(TfToken const &name)
{
  std::lock_guard<std::mutex> lock(_lightLinkMutex);
  ++_lightLinkRefs[name];
}

void HdPrman_Context::DecrementLightLinkCount(TfToken const &name)
{
  std::lock_guard<std::mutex> lock(_lightLinkMutex);
...
bool HdPrman_Context::IsLightLinkUsed(TfToken const &name)
{
  std::lock_guard<std::mutex> lock(_lightLinkMutex);
  return _lightLinkRefs.find(name) != _lightLinkRefs.end();
}

bool HdPrman_Context::IsShutterInstantaneous() const
{
  return _instantaneousShutter;
}

void HdPrman_Context::SetInstantaneousShutter(bool instantaneousShutter)
{
  _instantaneousShutter = instantaneousShutter;
}

void HdPrman_Context::IncrementLightFilterCount(TfToken const &name)
{
  std::lock_guard<std::mutex> lock(_lightFilterMutex);
  ++_lightFilterRefs[name];
}

void HdPrman_Context::DecrementLightFilterCount(TfToken const &name)
{
  std::lock_guard<std::mutex> lock(_lightFilterMutex);
...
bool HdPrman_Context::IsLightFilterUsed(TfToken const &name)
{
  std::lock_guard<std::mutex> lock(_lightFilterMutex);
                                                                    ...
                                                                    HdDirtyBits dirtyBits)
{
  ...
  compPrimvars = sceneDelegate->GetExtComputationPrimvarDescriptors(id, interp);
...
static bool _IsMasterAttribute(TfToken const &primvarName)
{
  ...
  if (paramType == _ParamTypePrimvar)
  {
    ...
    if (!computedPrimvars.empty())
    {
      ...
      for (auto const &compPrimvar : computedPrimvars)
      {
        ...
        if (!_SetParamValue(name, val, detail, compPrimvar.role, params))
        {
...
void HdPrman_Context::ReleaseCoordSysBindings(SdfPath const &id)
{
  std::lock_guard<std::mutex> lock(_coordSysMutex);
                                                   ...
                                                   RtParamList &options)
{
  ...
  for (auto const &entry : renderSettings)
  {
    ...
    if (hasRiPrefix)
    {
      ...
      _SetParamValue(riName, val, RtDetailType::k_constant, TfToken(), options);
    }
    else
    {
                                                            ...
                                                            RtParamList &params)
{
  ...
  for (auto const &entry : renderSettings)
  {
    ...
    if (hasRiPrefix)
    {
      ...
      _SetParamValue(riName, val, RtDetailType::k_constant, TfToken(), params);
    }
  }
}
