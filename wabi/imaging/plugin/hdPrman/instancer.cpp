...
void HdPrmanInstancer::_SyncPrimvars(HdSceneDelegate *delegate, HdDirtyBits dirtyBits)
{
  HD_TRACE_FUNCTION();
  ...
  HdTimeSampleArray<VtMatrix4dArray, HDPRMAN_MAX_TIME_SAMPLES> *sa)
{
  ...
  delegate->SamplePrimvar(instancerId, HdInstancerTokens->instanceTransform, &boxedInstanceXforms);
  delegate->SamplePrimvar(instancerId, HdInstancerTokens->translate, &boxedTranslates);
  delegate->SamplePrimvar(instancerId, HdInstancerTokens->scale, &boxedScales);
  delegate->SamplePrimvar(instancerId, HdInstancerTokens->rotate, &boxedRotates);
...
void HdPrmanInstancer::GetInstancePrimvars(SdfPath const &prototypeId,
