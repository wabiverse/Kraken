...
class HioOIIO_Image : public HioImage
{
 public:
  ...
  bool GetSamplerMetadata(HioAddressDimension pname, HioAddressMode *param) const override;
...
static HioFormat _GetHioFormatFromImageData(unsigned int nchannels, TypeDesc typedesc, bool isSRGB)
{
  switch (nchannels) {
    case 1:
...
static TypeDesc _GetOIIOBaseType(HioFormat hioFormat)
{
  switch (hioFormat) {
...
static std::string _TranslateMetadataKey(std::string const &metadataKey, bool *convertMatrixTypes)
{
  if (metadataKey == "NP") {
...
static void _SetAttribute(ImageSpec *spec, std::string const &metadataKey, VtValue const &value)
{
  ...
  if (value.IsHolding<std::string>()) {
    spec->attribute(key,
                    TypeDesc(TypeDesc::STRING, TypeDesc::SCALAR),
    ...
    if (convertMatrixTypes) {
      ...
      spec->attribute(key,
                      TypeDesc(TypeDesc::DOUBLE, TypeDesc::MATRIX44),
...
/* virtual */
int HioOIIO_Image::GetWidth() const
{
  return _imagespec.width;
}

/* virtual */
int HioOIIO_Image::GetHeight() const
{
  return _imagespec.height;
}

/* virtual */
HioFormat HioOIIO_Image::GetFormat() const
{
  return _GetHioFormatFromImageData(_imagespec.nchannels, _imagespec.format, IsColorSpaceSRGB());
}

/* virtual */
int HioOIIO_Image::GetBytesPerPixel() const
{
  return _imagespec.pixel_bytes();
}

/* virtual */
bool HioOIIO_Image::IsColorSpaceSRGB() const
{
...
/* virtual */
bool HioOIIO_Image::GetSamplerMetadata(HioAddressDimension pname, HioAddressMode *param) const
{
  switch (pname) {
...
/* virtual */
int HioOIIO_Image::GetNumMipLevels() const
{
  // XXX Add support for mip counting
  return 1;
}

std::string HioOIIO_Image::_GetFilenameExtension() const
{
                                               ...
                                               const ImageSpec &config) const
{
  ...
  std::unique_ptr<ImageInput> imageInput(ImageInput::open(inputFilename, &config));
...
/* virtual */
bool HioOIIO_Image::_OpenForReading(std::string const &filename,
                                    int subimage,
                                    int mip,
                                    HioImage::SourceColorSpace sourceColorSpace,
                                    bool suppressErrors)
...
/* virtual */
bool HioOIIO_Image::Read(StorageSpec const &storage)
{
  return ReadCropped(0, 0, 0, 0, storage);
}

/* virtual */
bool HioOIIO_Image::ReadCropped(int const cropTop,
                                int const cropBottom,
                                int const cropLeft,
                                int const cropRight,
                                StorageSpec const &storage)
{
  ...
  if (!image->get_pixels(ROI(0, storage.width, 0, storage.height, 0, 1), type, storage.data)) {
#else
  if (!image->get_pixels(0, storage.width, 0, storage.height, 0, 1, type, storage.data)) {
#endif
