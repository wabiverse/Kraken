...
ccl::TypeDesc HdBbAttributeSource::GetTypeDesc(const HdType &type)
{
...
ccl::TypeDesc HdBbAttributeSource::GetTypeDesc(const TfToken &role)
{
...
const TfToken &HdBbAttributeSource::GetRole(const ccl::TypeDesc &type_desc)
{
...
HdType HdBbAttributeSource::GetType(const ccl::TypeDesc &type_desc)
{
...
ccl::TypeDesc HdBbAttributeSource::GetTypeDesc(const HdType &type, const TfToken &role)
{
...
bool HdBbAttributeSource::IsHoldingFloat(const VtValue &value)
{
...
bool HdBbAttributeSource::CanCastToFloat(const VtValue &value)
{
  // unsupported Matrix3 and Matrix4
  return value.CanCast<float>() || value.CanCast<GfVec2f>() || value.CanCast<GfVec3f>() ||
         value.CanCast<GfVec4f>() || value.CanCast<VtFloatArray>() ||
         value.CanCast<VtVec2fArray>() || value.CanCast<VtVec3fArray>() ||
         value.CanCast<VtVec4fArray>();
}

bool HdBbAttributeSource::_CheckBuffersValid() const
{
  ...
  if (!TF_VERIFY(
          element != ccl::AttributeElement::ATTR_ELEMENT_NONE,
...
bool HdBbAttributeSource::_CheckBuffersSize() const
{
  const VtValue &value                 = m_value;
  const ccl::AttributeElement &element = GetAttributeElement();
...
bool HdBbAttributeSource::_CheckBuffersType() const
{
  const VtValue &value = m_value;
...
bool HdBbAttributeSource::_CheckValid() const
{
...
VtValue HdBbAttributeSource::UncheckedCastToFloat(const VtValue &input_value)
{
...
void HdBbAttributeSource::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  if (specs)
    specs->emplace_back(GetRole(m_type_desc), GetTupleType());
}

const void *HdBbAttributeSource::GetData() const
{
...
HdTupleType HdBbAttributeSource::GetTupleType() const
{
  return GetTupleType(GetSourceTypeDesc());
}

HdTupleType HdBbAttributeSource::GetTupleType(const ccl::TypeDesc &type_desc)
{
...
HdBbAttributeSource::HdBbAttributeSource(TfToken name,
                                         const TfToken &role,
                                         const VtValue &value,
...
HdBbAttributeSource::HdBbAttributeSource(const VtValue &value,
                                         ccl::AttributeSet *attribs,
                                         ccl::AttributeStandard std)
    : HdBbAttributeSource(TfToken{ccl::Attribute::standard_name(std)},
                          GetRole(attribs->geometry->standard_type(std)),
                          value,
                          attribs,
                          attribs->geometry->standard_element(std),
                          attribs->geometry->standard_type(std))
{}

HdCyclesPointCloudAttributeSource::HdCyclesPointCloudAttributeSource(
    TfToken name,
    const TfToken &role,
    const VtValue &value,
    ccl::PointCloud *pc,
    ...
    : HdBbAttributeSource(std::move(name),
                          ...
                          value,
                          &pc->attributes,
                          interpolation_to_pointcloud_element(interpolation),
