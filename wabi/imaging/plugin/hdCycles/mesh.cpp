...
HdCyclesMesh::HdCyclesMesh(HdCyclesRenderDelegate *renderDelegate, const SdfPath &id)
  : HdMesh(id),
...
void HdCyclesMesh::_AddUVSet(const TfToken &name,
                             const VtValue &uvs,
                             ccl::Scene *scene,
                             HdInterpolation interpolation)
{
  ...
  {
    VtValue refined_value = refiner->RefineConstantData(
      name, HdPrimvarRoleTokens->textureCoordinate, uvs_value);
  ...
  {
    VtValue refined_value = refiner->RefineUniformData(
      name, HdPrimvarRoleTokens->textureCoordinate, uvs_value);
  ...
  {
    VtValue refined_value = refiner->RefineVertexData(
      name, HdPrimvarRoleTokens->textureCoordinate, uvs_value);
  ...
  {
    VtValue refined_value = refiner->RefineVaryingData(
      name, HdPrimvarRoleTokens->textureCoordinate, uvs_value);
    if (refined_value.GetArraySize() != m_cyclesMesh->verts.size())
    {
  ...
  {
    VtValue refined_value = refiner->RefineFaceVaryingData(
      name, HdPrimvarRoleTokens->textureCoordinate, uvs_value);
...
void HdCyclesMesh::_PopulateColors(const TfToken &name,
                                   const TfToken &role,
                                   const VtValue &data,
                                   ccl::Scene *scene,
                                   HdInterpolation interpolation,
                                   const SdfPath &id)
{
  ...
  m_object_source->CreateAttributeSource<HdBbMeshAttributeSource>(
    name, role, data, m_cyclesMesh, interpolation, m_topology);
}

void HdCyclesMesh::_PopulateNormals(HdSceneDelegate *sceneDelegate, const SdfPath &id)
{
  // Normals are tricky in Hd. When subdivisionSchema is being set to != none,
  // then Hd interface suppresses loading any normals as 'normals' or 'primvar:normals' from
  // primvars. It happens because Hd assumes that normals are going to come from the limit surface
  // of Osd. Also, dirty normals bit is never set. To evaluate this function, whenever points
  // change, we force normal bits to be dirty in the PropagateDirtyBits.
  ...
  if (interpolation == HdInterpolationConstant)
  {
    ...
    VtValue refined_value = refiner->RefineConstantData(
      HdTokens->normals, HdPrimvarRoleTokens->normal, normals_value);
    if (refined_value.GetArraySize() != 1)
    {
  ...
  else if (interpolation == HdInterpolationVertex || interpolation == HdInterpolationVarying)
  {
    ...
    if (interpolation == HdInterpolationVertex)
    {
      refined_value = refiner->RefineVertexData(
        HdTokens->normals, HdPrimvarRoleTokens->normal, normals_value);
    }
    else
    {
      refined_value = refiner->RefineVaryingData(
        HdTokens->normals, HdPrimvarRoleTokens->normal, normals_value);
    }
...
ccl::Mesh *HdCyclesMesh::_CreateCyclesMesh()
{
...
ccl::Object *HdCyclesMesh::_CreateCyclesObject()
{
...
void HdCyclesMesh::_PopulateMotion(HdSceneDelegate *sceneDelegate, const SdfPath &id)
{
  ...
  for (unsigned int i = 0; i < numSamples; ++i)
  {
    ...
    VtValue refined_points_value = refiner->RefineVertexData(
      HdTokens->points, HdPrimvarRoleTokens->point, values[i]);
...
void HdCyclesMesh::_PopulateTopology(HdSceneDelegate *sceneDelegate, const SdfPath &id)
{
...
void HdCyclesMesh::_PopulateMaterials(HdSceneDelegate *sceneDelegate,
                                      HdCyclesRenderParam *renderParam,
                                      ccl::Shader *default_surface,
                                      const SdfPath &id)
{
  // Any topology change will mark MaterialId as dirty and automatically trigger material
  // discovery. During topology population process, material id for each face is set to 0. That
  // means default shader must be always present under 0 index in the shader table. Object material
  // discovery overrides the default shader. SubSet material discovery appends to the shaders table
  // and overrides materials ids for subset faces. This behaviour is to cover a corner case, where
  // there is no object material, but there is a sub set, that does not assign materials to all
  // faces.
...
void HdCyclesMesh::_PopulateObjectMaterial(HdSceneDelegate *sceneDelegate, const SdfPath &id)
{
...
void HdCyclesMesh::_PopulateSubSetsMaterials(HdSceneDelegate *sceneDelegate, const SdfPath &id)
{
  ...
  const HdCyclesMeshRefiner *refiner = m_topology->GetRefiner();
  VtValue refined_value = refiner->RefineUniformData(
    HdTokens->materialParams, HdPrimvarRoleTokens->none, VtValue{face_materials});
...
void HdCyclesMesh::_PopulatePrimvars(HdSceneDelegate *sceneDelegate,
                                     ccl::Scene *scene,
                                     const SdfPath &id,
                                     HdDirtyBits *dirtyBits)
{
  ...
  for (auto &interpolation_description : primvars_desc)
  {
    for (const HdPrimvarDescriptor &description : interpolation_description.second)
    {
      ...
      {
        m_object_source->CreateAttributeSource<HdBbMeshAttributeSource>(
          description.name, description.role, value, m_cyclesMesh, description.interpolation, m_topology);
      }
...
void HdCyclesMesh::_PopulateVertices(HdSceneDelegate *sceneDelegate,
                                     const SdfPath &id,
                                     HdDirtyBits *dirtyBits)
{
  ...
  VtVec3fArray points;
  VtValue refined_points_value = refiner->RefineVertexData(
    HdTokens->points, HdPrimvarRoleTokens->point, points_value);
  if (refined_points_value.IsHolding<VtVec3fArray>())
  {
  ...
  if (refiner->IsSubdivided())
  {
    ...
    VtFloat3Array refined_vertices{
      &foreign_data_source, m_cyclesMesh->verts.data(), m_cyclesMesh->verts.size(), false};
...
void HdCyclesMesh::_FinishMesh(ccl::Scene *scene)
{
...
HdDirtyBits HdCyclesMesh::_PropagateDirtyBits(HdDirtyBits bits) const
{
  ...
  if (HdChangeTracker::IsPrimvarDirty(
        bits, GetId(), HdCyclesTokens->primvarsCyclesMeshSubdivisionMaxLevel))
  {
...
void HdCyclesMesh::Sync(HdSceneDelegate *sceneDelegate,
                        ...
                        TfToken const &reprToken)
{
  ...
  for (auto &primvarDescsEntry : primvarDescsPerInterpolation)
  {
    for (auto &pv : primvarDescsEntry.second)
    {
      ...
      assetName = _HdCyclesGetMeshParam<std::string>(
        pv, dirtyBits, id, this, sceneDelegate, HdCyclesTokens->primvarsCyclesObjectAssetName, assetName);
...
void HdCyclesMesh::_UpdateObject(ccl::Scene *scene,
