...
HdCyclesPoints::HdCyclesPoints(HdCyclesRenderDelegate *renderDelegate, const SdfPath &id)
    : HdPoints(id),
...
void HdCyclesPoints::_InitRepr(TfToken const &reprToken, HdDirtyBits *dirtyBits)
{}
...
HdDirtyBits HdCyclesPoints::_PropagateDirtyBits(HdDirtyBits bits) const
{
  ...
  */
  if (HdChangeTracker::IsPrimvarDirty(bits, GetId(), HdCyclesTokens->cyclesObjectPointStyle)) {
    bits |= HdChangeTracker::DirtyPoints;
  }
  return bits;
}

void HdCyclesPoints::Finalize(HdRenderParam *renderParam)
{}

void HdCyclesPoints::_InitializeNewCyclesPointCloud()
{
  static const HdCyclesConfig &config = HdCyclesConfig::GetInstance();
...
void HdCyclesPoints::_ReadObjectFlags(HdSceneDelegate *sceneDelegate,
                                      const SdfPath &id,
                                      HdDirtyBits *dirtyBits)
{
  ...
  for (auto &primvarDescsEntry : primvarDescsPerInterpolation) {
    for (auto &pv : primvarDescsEntry.second) {
      ...
      m_cyclesObject->is_shadow_catcher = _HdCyclesGetPointsParam<bool>(
          pv,
          dirtyBits,
          id,
          this,
      ...
      m_cyclesObject->pass_id = _HdCyclesGetPointsParam<bool>(
          pv,
          dirtyBits,
          id,
          this,
      ...
      m_cyclesObject->use_holdout = _HdCyclesGetPointsParam<bool>(
          pv,
          dirtyBits,
          id,
          this,
...
void HdCyclesPoints::_PopulatePoints(HdSceneDelegate *sceneDelegate,
                                     const SdfPath &id,
                                     bool styleHasChanged,
                                     bool &sizeHasChanged)
{
  assert(m_cyclesPointCloud);
...
void HdCyclesPoints::_PopulateWidths(HdSceneDelegate *sceneDelegate,
                                     const SdfPath &id,
                                     ...
                                     VtValue value_)
{
  ...
  if (interpolation != HdInterpolationVertex && interpolation != HdInterpolationConstant) {
    TF_WARN("Point cloud %s has widths has no supported interpolation %s",
            id.GetText(),
...
void HdCyclesPoints::_PopulateColors(HdSceneDelegate *sceneDelegate,
                                     const SdfPath &id,
                                     const HdInterpolation &interpolation,
                                     VtValue value_)
{
  ...
  if (interpolation != HdInterpolationVertex && interpolation != HdInterpolationConstant) {
    TF_WARN("Point cloud %s has colors with no supported interpolation %s",
            id.GetText(),
            _HdInterpolationStr(interpolation));
    return;
  }
...
/*
    ...
    won't exist anymore.
*/
void HdCyclesPoints::_PopulateOpacities(HdSceneDelegate *sceneDelegate,
                                        const SdfPath &id,
                                        const HdInterpolation &interpolation,
                                        VtValue value_)
{
  ...
  if (interpolation != HdInterpolationVertex && interpolation != HdInterpolationConstant) {
    TF_WARN("Point cloud %s has opacities has no supported interpolation %s",
            id.GetText(),
            _HdInterpolationStr(interpolation));
    return;
  }
...
*/
void HdCyclesPoints::_PopulateNormals(HdSceneDelegate *sceneDelegate,
                                      const SdfPath &id,
                                      const HdInterpolation &interpolation,
                                      VtValue value_)
{
  ...
  if (interpolation != HdInterpolationVertex && interpolation != HdInterpolationConstant) {
    TF_WARN("Point cloud %s has normals with no supported interpolation %s",
            id.GetText(),
            _HdInterpolationStr(interpolation));
    return;
  }
...
void HdCyclesPoints::_PopulateVelocities(HdSceneDelegate *sceneDelegate,
                                         const SdfPath &id,
                                         const HdInterpolation &interpolation,
                                         VtValue value_)
{
  ...
  if (interpolation != HdInterpolationVertex && interpolation != HdInterpolationConstant) {
    TF_WARN("Point cloud %s has velocities with not supported interpolation %s",
            id.GetText(),
            _HdInterpolationStr(interpolation));
    return;
  }
...
void HdCyclesPoints::_PopulateAccelerations(HdSceneDelegate *sceneDelegate,
                                            const SdfPath &id,
                                            const HdInterpolation &interpolation,
                                            VtValue value_)
{
  ...
  if (interpolation != HdInterpolationVertex && interpolation != HdInterpolationConstant) {
    TF_WARN("Point cloud %s has accelerations with not supported interpolation %s",
            id.GetText(),
            _HdInterpolationStr(interpolation));
  }
...
void HdCyclesPoints::_PopulateGenerated(ccl::Scene *scene, const SdfPath &id)
{
...
void HdCyclesPoints::_PopulateMaterial(HdSceneDelegate *sceneDelegate,
...
void HdCyclesPoints::_PopulateObjectMaterial(HdSceneDelegate *sceneDelegate, const SdfPath &id)
{
...
void HdCyclesPoints::_UpdateObject(ccl::Scene *scene,
...
void HdCyclesPoints::Sync(HdSceneDelegate *sceneDelegate,
                          ...
                          TfToken const &reprSelector)
{
  ...
  if (HdChangeTracker::IsPrimvarDirty(
          *dirtyBits, id, HdCyclesTokens->cyclesObjectPointResolution)) {
    needsRebuildBVH = true;

    HdTimeSampleArray<VtValue, 1> xf;
    sceneDelegate->SamplePrimvar(id, HdCyclesTokens->cyclesObjectPointResolution, &xf);
    if (xf.count > 0) {
      const int &resolutions = xf.values[0].Get<int>();
      m_pointResolution      = std::max(resolutions, 10);
      TF_WARN(
          "Point resolution for geometry %s is currently unused, please create an issue for this!",
          id.GetText());
    }
  }
  ...
  for (auto &interpolation_description : primvars_desc) {
    for (const HdPrimvarDescriptor &description : interpolation_description.second) {
      ...
      if (description.name == HdCyclesTokens->cyclesObjectPointStyle) {
        if (value.IsEmpty()) {
          ...
          if (point_style == HdCyclesTokens->sphere) {
            ...
            TF_WARN("Unrecognized point style %s for point cloud %s",
                    point_style.GetText(),
                    id.GetText());
          }
          needsRebuildBVH = true;
...
bool HdCyclesPoints::IsValid() const
{
  return true;
}

void HdCyclesPoints::_CheckIntegrity(HdCyclesRenderParam *param)
{
  assert(m_cyclesPointCloud);
