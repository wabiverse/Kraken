  ...
  std::array<HdCyclesAov, 27> DefaultAovs = {
    {
     ...
     {"DiffInd",
       ccl::PASS_DIFFUSE_INDIRECT,
       HdCyclesAovTokens->DiffInd,
       HdFormatFloat32Vec3,
       true},
     {"DiffCol", ccl::PASS_DIFFUSE_COLOR, HdCyclesAovTokens->DiffCol, HdFormatFloat32Vec3, true},

     {"GlossDir",
       ccl::PASS_GLOSSY_DIRECT,
       HdCyclesAovTokens->GlossDir,
       HdFormatFloat32Vec3,
       true},
     {"GlossInd",
  ...
  std::array<HdCyclesAov, 2> DenoiseAovs = {
    {
     {"DenoiseNormal",
       ccl::PASS_NONE,
       HdCyclesAovTokens->DenoiseNormal,
       HdFormatFloat32Vec3,
       true},
     {"DenoiseAlbedo",
       ccl::PASS_NONE,
       HdCyclesAovTokens->DenoiseAlbedo,
       HdFormatFloat32Vec3,
       true},
     }
  };
...
void HdCyclesRenderParam::_InitializeDefaults()
{
...
float HdCyclesRenderParam::GetProgress()
{
  return m_renderProgress;
}

bool HdCyclesRenderParam::IsConverged()
{
  return GetProgress() >= 1.0f;
}

void HdCyclesRenderParam::_SessionUpdateCallback()
{
...
*/
bool HdCyclesRenderParam::Initialize(HdRenderSettingsMap const &settingsMap)
{
  // -- Delegate
...
void HdCyclesRenderParam::_UpdateDelegateFromConfig(bool a_forceInit)
{
  static const HdCyclesConfig &config = HdCyclesConfig::GetInstance();
  (void)config;
}
...
bool HdCyclesRenderParam::_HandleDelegateRenderSetting(const TfToken &key, const VtValue &value)
{
  bool delegate_updated = false;

  if (key == HdCyclesTokens->cyclesUseSquareSamples) {
...
void HdCyclesRenderParam::_UpdateSessionFromConfig(bool a_forceInit)
{
  static const HdCyclesConfig &config = HdCyclesConfig::GetInstance();
...
bool HdCyclesRenderParam::_HandleSessionRenderSetting(const TfToken &key, const VtValue &value)
{
  ccl::SessionParams *sessionParams = &m_sessionParams;
  ...
  /*if (key == HdCyclesTokens->cyclesBackground) {
  ...
  if (key == HdCyclesTokens->cyclesProgressiveRefine) {
    sessionParams->progressive_refine =
      _HdCyclesGetVtValue<bool>(value, sessionParams->progressive_refine, &session_updated);
  }

  if (key == HdCyclesTokens->cyclesProgressive) {
    sessionParams->progressive = _HdCyclesGetVtValue<bool>(value,
  ...
  if (key == HdCyclesTokens->cyclesProgressiveUpdateTimeout) {
    sessionParams->progressive_update_timeout = static_cast<double>(
      _HdCyclesGetVtValue<float>(value,
                                 static_cast<float>(sessionParams->progressive_update_timeout),
                                 &session_updated));
  }

  if (key == HdCyclesTokens->cyclesExperimental) {
    sessionParams->experimental = _HdCyclesGetVtValue<bool>(value,
  ...
  if (key == HdCyclesTokens->cyclesTileSize) {
    if (value.IsHolding<GfVec2i>()) {
      sessionParams->tile_size = vec2i_to_int2(
        _HdCyclesGetVtValue<GfVec2i>(value,
      ...
      sessionParams->tile_size = vec2f_to_int2(
        _HdCyclesGetVtValue<GfVec2f>(value,
                                     int2_to_vec2f(sessionParams->tile_size),
                                     &session_updated));
      TF_WARN(
  ...
  TfToken tileOrder;
  if (key == HdCyclesTokens->cyclesTileOrder) {
  ...
  if (key == HdCyclesTokens->cyclesStartResolution) {
    sessionParams->start_resolution = _HdCyclesGetVtValue<int>(value,
                                                               sessionParams->start_resolution,
                                                               &session_updated);
  }

  if (key == HdCyclesTokens->cyclesPixelSize) {
    sessionParams->pixel_size = _HdCyclesGetVtValue<int>(value,
                                                         sessionParams->pixel_size,
                                                         &session_updated);
  }

  if (key == HdCyclesTokens->cyclesAdaptiveSampling) {
    sessionParams->adaptive_sampling = _HdCyclesGetVtValue<bool>(value,
  ...
  if (key == HdCyclesTokens->cyclesDisplayBufferLinear) {
    sessionParams->display_buffer_linear =
      _HdCyclesGetVtValue<bool>(value, sessionParams->display_buffer_linear, &session_updated);
  }
  ...
  if (key == HdCyclesTokens->cyclesDenoiseUse) {
    denoisingParams.use = _HdCyclesGetVtValue<bool>(value,
                                                    denoisingParams.use,
                                                    &denoising_updated);
  }

  if (key == HdCyclesTokens->cyclesDenoiseStorePasses) {
    denoisingParams.store_passes = _HdCyclesGetVtValue<bool>(value,
                                                             denoisingParams.store_passes,
                                                             &denoising_updated);
  }
...
void HdCyclesRenderParam::_UpdateSceneFromConfig(bool a_forceInit)
{
  static const HdCyclesConfig &config = HdCyclesConfig::GetInstance();
...
bool HdCyclesRenderParam::_HandleSceneRenderSetting(const TfToken &key, const VtValue &value)
{
  // -- Scene
  ...
  if (key == HdCyclesTokens->cyclesShadingSystem) {
    TfToken shading_system = _HdCyclesGetVtValue<TfToken>(value,
                                                          HdCyclesTokens->svm,
                                                          &scene_updated);
  ...
  if (key == HdCyclesTokens->cyclesBvhType) {
    TfToken bvh_type = _HdCyclesGetVtValue<TfToken>(value,
                                                    HdCyclesTokens->bvh_dynamic,
                                                    &scene_updated);
  ...
  if (key == HdCyclesTokens->cyclesUseBvhSpatialSplit) {
    sceneParams->use_bvh_spatial_split =
      _HdCyclesGetVtValue<bool>(value, sceneParams->use_bvh_spatial_split, &scene_updated);
  }

  if (key == HdCyclesTokens->cyclesUseBvhUnalignedNodes) {
    sceneParams->use_bvh_unaligned_nodes =
      _HdCyclesGetVtValue<bool>(value, sceneParams->use_bvh_unaligned_nodes, &scene_updated);
  }

  if (key == HdCyclesTokens->cyclesNumBvhTimeSteps) {
    sceneParams->num_bvh_time_steps = _HdCyclesGetVtValue<int>(value,
                                                               sceneParams->num_bvh_time_steps,
                                                               &scene_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureUseCache) {
    sceneParams->texture.use_cache = _HdCyclesGetVtValue<bool>(value,
                                                               sceneParams->texture.use_cache,
                                                               &texture_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureCacheSize) {
    sceneParams->texture.cache_size = _HdCyclesGetVtValue<int>(value,
                                                               sceneParams->texture.cache_size,
                                                               &texture_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureTileSize) {
    sceneParams->texture.tile_size = _HdCyclesGetVtValue<int>(value,
                                                              sceneParams->texture.tile_size,
                                                              &texture_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureDiffuseBlur) {
    sceneParams->texture.diffuse_blur =
      _HdCyclesGetVtValue<float>(value, sceneParams->texture.diffuse_blur, &texture_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureGlossyBlur) {
    sceneParams->texture.glossy_blur = _HdCyclesGetVtValue<float>(value,
                                                                  sceneParams->texture.glossy_blur,
                                                                  &texture_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureAutoConvert) {
    sceneParams->texture.auto_convert =
      _HdCyclesGetVtValue<bool>(value, sceneParams->texture.auto_convert, &texture_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureAcceptUnmipped) {
    sceneParams->texture.accept_unmipped =
      _HdCyclesGetVtValue<bool>(value, sceneParams->texture.accept_unmipped, &texture_updated);
  }

  if (key == HdCyclesTokens->cyclesTextureAcceptUntiled) {
    sceneParams->texture.accept_untiled =
      _HdCyclesGetVtValue<bool>(value, sceneParams->texture.accept_untiled, &texture_updated);
  }
  if (key == HdCyclesTokens->cyclesTextureAutoTile) {
    sceneParams->texture.auto_tile = _HdCyclesGetVtValue<bool>(value,
                                                               sceneParams->texture.auto_tile,
                                                               &texture_updated);
  }
  if (key == HdCyclesTokens->cyclesTextureAutoMip) {
    sceneParams->texture.auto_mip = _HdCyclesGetVtValue<bool>(value,
                                                              sceneParams->texture.auto_mip,
                                                              &texture_updated);
  }
  ...
  if (key == HdCyclesTokens->cyclesTextureMaxSize) {
    sceneParams->texture_limit = _HdCyclesGetVtValue<int>(value,
...
void HdCyclesRenderParam::_UpdateIntegratorFromConfig(bool a_forceInit)
{
  if (!m_cyclesScene)
    return;
...
bool HdCyclesRenderParam::_HandleIntegratorRenderSetting(const TfToken &key, const VtValue &value)
{
  // -- Integrator Settings
  ...
  if (key == HdCyclesTokens->cyclesIntegratorMinBounce) {
    integrator->min_bounce = _HdCyclesGetVtValue<int>(value,
                                                      integrator->min_bounce,
                                                      &integrator_updated);
  }

  if (key == HdCyclesTokens->cyclesIntegratorMaxBounce) {
    integrator->max_bounce = _HdCyclesGetVtValue<int>(value,
                                                      integrator->max_bounce,
                                                      &integrator_updated);
  }

  if (key == HdCyclesTokens->cyclesIntegratorMethod) {
    TfToken integratorMethod = _HdCyclesGetVtValue<TfToken>(value,
                                                            HdCyclesTokens->path,
                                                            &method_updated);
  ...
  if (key == HdCyclesTokens->cyclesIntegratorSamplingMethod) {
    TfToken defaultPattern = HdCyclesTokens->sobol;
    ...
    TfToken samplingMethod = _HdCyclesGetVtValue<TfToken>(value,
                                                          defaultPattern,
                                                          &integrator_updated);
  ...
  if (key == HdCyclesTokens->cyclesIntegratorAaSamples) {
    ...
    integrator->aa_samples = _HdCyclesGetVtValue<int>(value,
  ...
  if (key == HdCyclesTokens->cyclesIntegratorAdaptiveMinSamples) {
    bool sample_updated = false;
    integrator->adaptive_min_samples = _HdCyclesGetVtValue<int>(value,
                                                                integrator->adaptive_min_samples,
                                                                &sample_updated);

    if (sample_updated) {
      if (m_useSquareSamples) {
        integrator->adaptive_min_samples = std::min(integrator->adaptive_min_samples *
                                                      integrator->adaptive_min_samples,
                                                    INT_MAX);
      }
      integrator_updated = true;
  ...
  if (key == HdCyclesTokens->cyclesIntegratorDiffuseSamples) {
    bool sample_updated = false;
    integrator->diffuse_samples = _HdCyclesGetVtValue<int>(value,
  ...
  if (key == HdCyclesTokens->cyclesIntegratorGlossySamples) {
    bool sample_updated = false;
    integrator->glossy_samples = _HdCyclesGetVtValue<int>(value,
  ...
  if (key == HdCyclesTokens->cyclesIntegratorTransmissionSamples) {
    bool sample_updated = false;
    integrator->transmission_samples = _HdCyclesGetVtValue<int>(value,
                                                                integrator->transmission_samples,
                                                                &sample_updated);
  ...
  if (key == HdCyclesTokens->cyclesIntegratorAoSamples) {
    bool sample_updated = false;
    integrator->ao_samples = _HdCyclesGetVtValue<int>(value,
  ...
  if (key == HdCyclesTokens->cyclesIntegratorMeshLightSamples) {
    bool sample_updated = false;
    integrator->mesh_light_samples = _HdCyclesGetVtValue<int>(value,
                                                              integrator->mesh_light_samples,
                                                              &sample_updated);
  ...
  if (key == HdCyclesTokens->cyclesIntegratorSubsurfaceSamples) {
    bool sample_updated = false;
    integrator->subsurface_samples = _HdCyclesGetVtValue<int>(value,
                                                              integrator->subsurface_samples,
                                                              &sample_updated);
  ...
  if (key == HdCyclesTokens->cyclesIntegratorVolumeSamples) {
    bool sample_updated = false;
    integrator->volume_samples = _HdCyclesGetVtValue<int>(value,
  ...
  if (key == HdCyclesTokens->cyclesIntegratorStartSample) {
    integrator->start_sample = _HdCyclesGetVtValue<int>(value,
  ...
  if (key == HdCyclesTokens->cyclesIntegratorCausticsRefractive) {
    integrator->caustics_refractive = _HdCyclesGetVtValue<bool>(value,
  ...
  if (key == HdCyclesTokens->cyclesIntegratorFilterGlossy) {
    integrator->filter_glossy = _HdCyclesGetVtValue<float>(value,
                                                           integrator->filter_glossy,
                                                           &integrator_updated);
  }

  if (key == HdCyclesTokens->cyclesIntegratorSampleClampDirect) {
    integrator->sample_clamp_direct = _HdCyclesGetVtValue<float>(value,
                                                                 integrator->sample_clamp_direct,
                                                                 &integrator_updated);
  }
  if (key == HdCyclesTokens->cyclesIntegratorSampleClampIndirect) {
    integrator->sample_clamp_indirect =
      _HdCyclesGetVtValue<float>(value, integrator->sample_clamp_indirect, &integrator_updated);
  }

  if (key == HdCyclesTokens->cyclesIntegratorMotionBlur) {
    integrator->motion_blur = _HdCyclesGetVtValue<bool>(value,
                                                        integrator->motion_blur,
                                                        &integrator_updated);
  }

  if (key == HdCyclesTokens->cyclesIntegratorSampleAllLightsDirect) {
    integrator->sample_all_lights_direct =
      _HdCyclesGetVtValue<bool>(value, integrator->sample_all_lights_direct, &integrator_updated);
  }
  ...
  if (key == HdCyclesTokens->cyclesIntegratorLightSamplingThreshold) {
    integrator->light_sampling_threshold =
      _HdCyclesGetVtValue<float>(value, integrator->light_sampling_threshold, &integrator_updated);
  }
...
void HdCyclesRenderParam::_UpdateFilmFromConfig(bool a_forceInit)
{
  if (!m_cyclesScene)
    return;
...
bool HdCyclesRenderParam::_HandleFilmRenderSetting(const TfToken &key, const VtValue &value)
{
  // -- Film Settings
  ...
  if (key == HdCyclesTokens->cyclesFilmPassAlphaThreshold) {
    film->pass_alpha_threshold = _HdCyclesGetVtValue<float>(value,
  ...
  if (key == HdCyclesTokens->cyclesFilmFilterType) {
    TfToken filter = _HdCyclesGetVtValue<TfToken>(value, HdCyclesTokens->box, &film_updated);
  ...
  if (key == HdCyclesTokens->cyclesFilmMistStart) {
    film->mist_start = _HdCyclesGetVtValue<float>(value, film->mist_start, &film_updated, false);
  }

  if (key == HdCyclesTokens->cyclesFilmMistDepth) {
  ...
  if (key == HdCyclesTokens->cyclesFilmUseLightVisibility) {
    film->use_light_visibility = _HdCyclesGetVtValue<bool>(value,
  ...
  if (key == HdCyclesTokens->cyclesFilmUseAdaptiveSampling) {
    film->use_adaptive_sampling = _HdCyclesGetVtValue<bool>(value,
...
void HdCyclesRenderParam::_UpdateBackgroundFromConfig(bool a_forceInit)
{
  if (!m_cyclesScene)
    return;
...
bool HdCyclesRenderParam::_HandleBackgroundRenderSetting(const TfToken &key, const VtValue &value)
{
  // -- Background Settings
  ...
  if (key == HdCyclesTokens->cyclesBackgroundAoFactor) {
    background->ao_factor = _HdCyclesGetVtValue<float>(value,
                                                       background->ao_factor,
                                                       &background_updated);
  }
  if (key == HdCyclesTokens->cyclesBackgroundAoDistance) {
    background->ao_distance = _HdCyclesGetVtValue<float>(value,
                                                         background->ao_distance,
                                                         &background_updated);
  }

  if (key == HdCyclesTokens->cyclesBackgroundUseShader) {
    background->use_shader = _HdCyclesGetVtValue<bool>(value,
  ...
  if (key == HdCyclesTokens->cyclesBackgroundTransparent) {
    background->transparent = _HdCyclesGetVtValue<bool>(value,
                                                        background->transparent,
                                                        &background_updated);
  }

  if (key == HdCyclesTokens->cyclesBackgroundTransparentGlass) {
    background->transparent_glass = _HdCyclesGetVtValue<bool>(value,
  ...
  if (key == HdCyclesTokens->cyclesBackgroundVolumeStepSize) {
    background->volume_step_size = _HdCyclesGetVtValue<float>(value,
...
void HdCyclesRenderParam::_HandlePasses()
{
...
bool HdCyclesRenderParam::SetRenderSetting(const TfToken &key, const VtValue &value)
{
...
bool HdCyclesRenderParam::_CreateSession()
{
  ...
  m_cyclesSession->write_render_tile_cb = std::bind(&HdCyclesRenderParam::_WriteRenderTile,
                                                    this,
                                                    ccl::_1);
  m_cyclesSession->update_render_tile_cb = std::bind(&HdCyclesRenderParam::_UpdateRenderTile,
                                                     this,
                                                     ccl::_1,
                                                     ccl::_2);
...
void HdCyclesRenderParam::_WriteRenderTile(ccl::RenderTile &rtile)
{
  // No session, exit out
  if (!m_cyclesSession)
    return;
...
void HdCyclesRenderParam::_UpdateRenderTile(ccl::RenderTile &rtile, bool highlight)
{
  if (m_cyclesSession->params.progressive_refine)
    _WriteRenderTile(rtile);
}

bool HdCyclesRenderParam::_CreateScene()
{
  static const HdCyclesConfig &config = HdCyclesConfig::GetInstance();
...
void HdCyclesRenderParam::StartRender()
{
  _CyclesStart();
}

void HdCyclesRenderParam::StopRender()
{
  _CyclesExit();
}

// Deprecate? This isnt used... Also doesnt work
void HdCyclesRenderParam::RestartRender()
{
  StopRender();
  Initialize({});
  StartRender();
...
void HdCyclesRenderParam::PauseRender()
{
  if (m_cyclesSession)
    m_cyclesSession->set_pause(true);
}

void HdCyclesRenderParam::ResumeRender()
{
  if (m_cyclesSession)
    m_cyclesSession->set_pause(false);
}

void HdCyclesRenderParam::Interrupt(bool a_forceUpdate)
{
  m_shouldUpdate = true;
  PauseRender();
}

void HdCyclesRenderParam::CommitResources()
{
...
void HdCyclesRenderParam::SetBackgroundShader(ccl::Shader *a_shader, bool a_emissive)
{
  if (a_shader)
...
bool HdCyclesRenderParam::SetDeviceType(ccl::DeviceType a_deviceType, ccl::SessionParams &params)
{
...
void HdCyclesRenderParam::_CyclesStart()
{
  m_cyclesSession->start();
}

void HdCyclesRenderParam::_CyclesExit()
{
  m_cyclesSession->set_pause(true);
...
void HdCyclesRenderParam::CyclesReset(bool a_forceUpdate)
{
...
void HdCyclesRenderParam::DirectReset()
{
  m_cyclesSession->reset(m_bufferParams, m_cyclesSession->params.samples);
}

void HdCyclesRenderParam::UpdateShadersTag(ccl::vector<ccl::Shader *> &shaders)
{
...
void HdCyclesRenderParam::AddShader(ccl::Shader *shader)
{
  if (!m_cyclesScene) {
...
void HdCyclesRenderParam::AddLight(ccl::Light *light)
{
  if (!m_cyclesScene) {
...
void HdCyclesRenderParam::AddObject(ccl::Object *object)
{
  if (!m_cyclesScene) {
...
void HdCyclesRenderParam::AddGeometry(ccl::Geometry *geometry)
{
  if (!m_cyclesScene) {
    TF_WARN("Couldn't add geometry to scene. Scene is null.");
    return;
...
void HdCyclesRenderParam::RemoveShader(ccl::Shader *shader)
{
...
void HdCyclesRenderParam::RemoveLight(ccl::Light *light)
{
...
void HdCyclesRenderParam::RemoveObject(ccl::Object *object)
{
...
void HdCyclesRenderParam::RemoveGeometry(ccl::Geometry *geometry)
{
...
void HdCyclesRenderParam::AddShaderSafe(ccl::Shader *shader)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  AddShader(shader);
}

void HdCyclesRenderParam::AddLightSafe(ccl::Light *light)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  AddLight(light);
}

void HdCyclesRenderParam::AddObjectSafe(ccl::Object *object)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  AddObject(object);
}

void HdCyclesRenderParam::AddGeometrySafe(ccl::Geometry *geometry)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  AddGeometry(geometry);
}

void HdCyclesRenderParam::RemoveShaderSafe(ccl::Shader *shader)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  RemoveShader(shader);
}

void HdCyclesRenderParam::RemoveLightSafe(ccl::Light *light)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  RemoveLight(light);
}

void HdCyclesRenderParam::RemoveObjectSafe(ccl::Object *object)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  RemoveObject(object);
}

void HdCyclesRenderParam::RemoveGeometrySafe(ccl::Geometry *geometry)
{
  ccl::thread_scoped_lock lock{m_cyclesScene->mutex};
  RemoveGeometry(geometry);
}

VtDictionary HdCyclesRenderParam::GetRenderStats() const
{
 ...
 // - Solaris, husk specific
  ...
  // Currently these don't update properly. It is unclear if we need to tag
  // renderstats as dynamic. Maybe our VtValues need to live longer?
...
void HdCyclesRenderParam::SetAovBindings(HdRenderPassAovBindingVector const &a_aovs)
{
  ...
  for (const HdRenderPassAovBinding &aov : m_aovs) {
    ...
    for (HdCyclesAov &cyclesAov : DefaultAovs) {
      if (sourceName == cyclesAov.token) {
        ...
        ccl::Pass::add(cyclesAov.type,
                       m_bufferParams.passes,
                       cyclesAov.name.c_str(),
                       cyclesAov.filter);
        continue;
      }
    }
  ...
  // Ordering matters
  if (cryptoObject) {
    film->cryptomatte_passes = static_cast<ccl::CryptomatteType>(film->cryptomatte_passes |
                                                                 ccl::CRYPT_OBJECT);
    for (int i = 0; i < cryptoObject; ++i) {
      ccl::Pass::add(ccl::PASS_CRYPTOMATTE,
                     m_bufferParams.passes,
    ...
    if (n_comps_cycles <= n_comps_hd) {
      ...
      switch (rb->GetFormat()) {
        case HdFormatFloat16:
          pixels_type = ccl::RenderBuffers::ComponentType::Float16;
          break;
        case HdFormatFloat16Vec3:
          pixels_type = ccl::RenderBuffers::ComponentType::Float16x3;
          break;
        case HdFormatFloat16Vec4:
          pixels_type = ccl::RenderBuffers::ComponentType::Float16x4;
          break;
        case HdFormatFloat32:
          pixels_type = ccl::RenderBuffers::ComponentType::Float32;
          break;
        case HdFormatFloat32Vec3:
          pixels_type = ccl::RenderBuffers::ComponentType::Float32x3;
          break;
        case HdFormatFloat32Vec4:
          pixels_type = ccl::RenderBuffers::ComponentType::Float32x4;
          break;
        case HdFormatInt32:
          pixels_type = ccl::RenderBuffers::ComponentType::Int32;
          break;
        default:
          assert(false);
          break;
      ...
      if (pixels_type == ccl::RenderBuffers::ComponentType::None) {
        TF_WARN("Unsupported component type %d for aov %s ",
                static_cast<int>(rb->GetFormat()),
                aov.aovName.GetText());
        rb->Unmap();
        return;
      }
