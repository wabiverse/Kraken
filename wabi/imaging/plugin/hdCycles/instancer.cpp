...
VtMatrix4dArray HdCyclesInstancer::ComputeTransforms(SdfPath const &prototypeId)
{
  Sync();
...
namespace
{
  ...
  template<typename Op, typename T>
  void ApplyTransform(HdTimeSampleArray<VtValue, HD_CYCLES_MOTION_STEPS> const &samples,
                      VtIntArray const &instanceIndices,
                      float time,
                      GfMatrix4d *transforms)
  {
    ...
    if (i == 0) {
      ...
      return ApplyTransform<Op, T>(samples.values[static_cast<size_type>(samples.count) - 1],
                                   instanceIndices,
                                   transforms);
