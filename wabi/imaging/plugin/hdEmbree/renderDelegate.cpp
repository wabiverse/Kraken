 ...
 */
#include "wabi/imaging/plugin/hdEmbree/renderDelegate.h"

#include "wabi/imaging/plugin/hdEmbree/config.h"
#include "wabi/imaging/plugin/hdEmbree/instancer.h"
#include "wabi/imaging/plugin/hdEmbree/renderParam.h"
#include "wabi/imaging/plugin/hdEmbree/renderPass.h"
...
static void _RenderCallback(HdEmbreeRenderer *renderer, HdRenderThread *renderThread)
{
  renderer->Clear();
...
void HdEmbreeRenderDelegate::_Initialize()
{
...
HdRenderSettingDescriptorList HdEmbreeRenderDelegate::GetRenderSettingDescriptors() const
{
  return _settingDescriptors;
}

HdRenderParam *HdEmbreeRenderDelegate::GetRenderParam() const
{
  return _renderParam.get();
}

void HdEmbreeRenderDelegate::CommitResources(HdChangeTracker *tracker)
{}

TfTokenVector const &HdEmbreeRenderDelegate::GetSupportedRprimTypes() const
{
  return SUPPORTED_RPRIM_TYPES;
}

TfTokenVector const &HdEmbreeRenderDelegate::GetSupportedSprimTypes() const
{
  return SUPPORTED_SPRIM_TYPES;
}

TfTokenVector const &HdEmbreeRenderDelegate::GetSupportedBprimTypes() const
{
  return SUPPORTED_BPRIM_TYPES;
}

HdResourceRegistrySharedPtr HdEmbreeRenderDelegate::GetResourceRegistry() const
{
...
VtDictionary HdEmbreeRenderDelegate::GetRenderStats() const
{
  VtDictionary stats;
  stats[HdPerfTokens->numCompletedSamples.GetString()] = _renderer.GetCompletedSamples();
  return stats;
}

bool HdEmbreeRenderDelegate::IsPauseSupported() const
{
  return true;
}

bool HdEmbreeRenderDelegate::Pause()
{
  _renderThread.PauseRender();
  return true;
}

bool HdEmbreeRenderDelegate::Resume()
{
  _renderThread.ResumeRender();
  return true;
}

HdRenderPassSharedPtr HdEmbreeRenderDelegate::CreateRenderPass(HdRenderIndex *index,
...
void HdEmbreeRenderDelegate::DestroyInstancer(HdInstancer *instancer)
{
  delete instancer;
}
...
void HdEmbreeRenderDelegate::DestroyRprim(HdRprim *rPrim)
{
  delete rPrim;
}
...
HdSprim *HdEmbreeRenderDelegate::CreateFallbackSprim(TfToken const &typeId)
{
...
void HdEmbreeRenderDelegate::DestroySprim(HdSprim *sPrim)
{
  delete sPrim;
}
...
void HdEmbreeRenderDelegate::DestroyBprim(HdBprim *bPrim)
{
  delete bPrim;
}
