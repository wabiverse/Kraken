...
#include "wabi/imaging/plugin/hdEmbree/config.h"
#include "wabi/imaging/plugin/hdEmbree/context.h"
#include "wabi/imaging/plugin/hdEmbree/mesh.h"
...
void HdEmbreeRenderer::SetScene(RTCScene scene)
{
  _scene = scene;
}

void HdEmbreeRenderer::SetSamplesToConvergence(int samplesToConvergence)
{
  _samplesToConvergence = samplesToConvergence;
}

void HdEmbreeRenderer::SetAmbientOcclusionSamples(int ambientOcclusionSamples)
{
  _ambientOcclusionSamples = ambientOcclusionSamples;
}

void HdEmbreeRenderer::SetEnableSceneColors(bool enableSceneColors)
{
  _enableSceneColors = enableSceneColors;
}

void HdEmbreeRenderer::SetDataWindow(const GfRect2i &dataWindow)
{
...
void HdEmbreeRenderer::SetCamera(const GfMatrix4d &viewMatrix, const GfMatrix4d &projMatrix)
{
...
void HdEmbreeRenderer::SetAovBindings(HdRenderPassAovBindingVector const &aovBindings)
{
...
bool HdEmbreeRenderer::_ValidateAovBindings()
{
...
/* static */
GfVec4f HdEmbreeRenderer::_GetClearColor(VtValue const &clearValue)
{
...
void HdEmbreeRenderer::Clear()
{
  ...
  for (size_t i = 0; i < _aovBindings.size(); ++i) {
    ...
    HdEmbreeRenderBuffer *rb = static_cast<HdEmbreeRenderBuffer *>(_aovBindings[i].renderBuffer);
...
int HdEmbreeRenderer::GetCompletedSamples() const
{
...
void HdEmbreeRenderer::Render(HdRenderThread *renderThread)
{
  ...
  for (size_t i = 0; i < _aovBindings.size(); ++i) {
    ...
    // render buffer.
    //
    static_cast<HdEmbreeRenderBuffer *>(_aovBindings[i].renderBuffer)->Map();
    ...
    if (i == 0) {
      ...
      if (_width != _aovBindings[i].renderBuffer->GetWidth() ||
          _height != _aovBindings[i].renderBuffer->GetHeight()) {
        TF_CODING_ERROR("Embree render buffers have inconsistent sizes");
      }
    }
  }
  ...
  for (int i = 0; i < _samplesToConvergence; ++i) {
    ...
    const unsigned int numTilesX = (_dataWindow.GetWidth() + tileSize - 1) / tileSize;
    const unsigned int numTilesY = (_dataWindow.GetHeight() + tileSize - 1) / tileSize;

    // Render by scheduling square tiles of the sample buffer in a parallel
    // for loop.
    WorkParallelForN(numTilesX * numTilesY,
                     std::bind(&HdEmbreeRenderer::_RenderTiles,
                               this,
...
void HdEmbreeRenderer::_RenderTiles(HdRenderThread *renderThread, size_t tileStart, size_t tileEnd)
{
  ...
  for (unsigned int tile = tileStart; tile < tileEnd; ++tile) {
    ...
    for (unsigned int y = y0; y < y1; ++y) {
      for (unsigned int x = x0; x < x1; ++x) {
        ...
        const GfVec3f ndc(2 * ((x + jitter[0] - minX) / w) - 1, 2 * ((y + jitter[1] - minY) / h) - 1, -1);
...
static void _PopulateRay(RTCRay *ray, GfVec3f const &origin, GfVec3f const &dir, float nearest)
{
  ray->org_x = origin[0];
  ray->org_y = origin[1];
  ray->org_z = origin[2];
...
static void _PopulateRayHit(RTCRayHit *rayHit, GfVec3f const &origin, GfVec3f const &dir, float nearest)
{
...
static GfVec3f _CosineWeightedDirection(GfVec2f const &uniform_float)
{
  GfVec3f dir;
...
void HdEmbreeRenderer::_TraceRay(unsigned int x,
                                 unsigned int y,
                                 ...
                                 std::default_random_engine &random)
{
  ...
  for (size_t i = 0; i < _aovBindings.size(); ++i) {
    HdEmbreeRenderBuffer *renderBuffer = static_cast<HdEmbreeRenderBuffer *>(_aovBindings[i].renderBuffer);
...
bool HdEmbreeRenderer::_ComputeId(RTCRayHit const &rayHit, TfToken const &idType, int32_t *id)
{
...
bool HdEmbreeRenderer::_ComputeDepth(RTCRayHit const &rayHit, float *depth, bool clip)
{
  if (rayHit.hit.geomID == RTC_INVALID_GEOMETRY_ID) {
    return false;
  }
...
bool HdEmbreeRenderer::_ComputeNormal(RTCRayHit const &rayHit, GfVec3f *normal, bool eye)
{
  if (rayHit.hit.geomID == RTC_INVALID_GEOMETRY_ID) {
    return false;
  }
...
bool HdEmbreeRenderer::_ComputePrimvar(RTCRayHit const &rayHit, TfToken const &primvar, GfVec3f *value)
{
  if (rayHit.hit.geomID == RTC_INVALID_GEOMETRY_ID) {
    return false;
  }
...
GfVec4f HdEmbreeRenderer::_ComputeColor(RTCRayHit const &rayHit,
                                        std::default_random_engine &random,
                                        GfVec4f const &clearColor)
{
  ...
  if (_enableSceneColors) {
    ...
    if (it != prototypeContext->primvarMap.end()) {
      it->second->Sample(rayHit.hit.primID, rayHit.hit.u, rayHit.hit.v, &color);
    }
  }
  ...
  float aoLightIntensity = _ComputeAmbientOcclusion(hitPos, normal, random);
...
float HdEmbreeRenderer::_ComputeAmbientOcclusion(GfVec3f const &position,
