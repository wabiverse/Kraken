...
void HdEmbreeMesh::Finalize(HdRenderParam *renderParam)
{
...
HdDirtyBits HdEmbreeMesh::GetInitialDirtyBitsMask() const
{
...
HdDirtyBits HdEmbreeMesh::_PropagateDirtyBits(HdDirtyBits bits) const
{
  return bits;
}

void HdEmbreeMesh::_InitRepr(TfToken const &reprToken, HdDirtyBits *dirtyBits)
{
  TF_UNUSED(dirtyBits);
...
void HdEmbreeMesh::Sync(HdSceneDelegate *sceneDelegate,
                        ...
                        TfToken const &reprToken)
{
  ...
  HdEmbreeRenderParam *embreeRenderParam = static_cast<HdEmbreeRenderParam *>(renderParam);
...
RTCGeometry HdEmbreeMesh::_CreateEmbreeSubdivMesh(RTCScene scene, RTCDevice device)
{
  ...
  if (numEdgeCreases > 0) {
    ...
    for (size_t i = 0; i < creaseLengths.size(); ++i) {
      ...
      for (int j = 0; j < numEdges; ++j) {
        ...
        embreeCreaseWeights[embreeEdgeIndex] = weightPerCrease ? creaseWeights[i] :
...
RTCGeometry HdEmbreeMesh::_CreateEmbreeTriangleMesh(RTCScene scene, RTCDevice device)
{
...
void HdEmbreeMesh::_UpdatePrimvarSources(HdSceneDelegate *sceneDelegate, HdDirtyBits dirtyBits)
{
  HD_TRACE_FUNCTION();
...
TfTokenVector HdEmbreeMesh::_UpdateComputedPrimvarSources(HdSceneDelegate *sceneDelegate,
                                                          HdDirtyBits dirtyBits)
{
  HD_TRACE_FUNCTION();
...
void HdEmbreeMesh::_CreatePrimvarSampler(TfToken const &name,
                                         VtValue const &data,
...
void HdEmbreeMesh::_PopulateRtMesh(HdSceneDelegate *sceneDelegate,
                                   RTCScene scene,
                                   RTCDevice device,
                                   HdDirtyBits *dirtyBits,
                                   HdMeshReprDesc const &desc)
{
  ...
  if (HdChangeTracker::IsTopologyDirty(*dirtyBits, id) || doRefine != _refined) {
    ...
    _GetPrototypeContext()->primitiveParams = (_refined ? _trianglePrimitiveParams : VtIntArray());
...
HdEmbreePrototypeContext *HdEmbreeMesh::_GetPrototypeContext()
{
  return static_cast<HdEmbreePrototypeContext *>(rtcGetGeometryUserData(_geometry));
}

HdEmbreeInstanceContext *HdEmbreeMesh::_GetInstanceContext(RTCScene scene, size_t i)
{
