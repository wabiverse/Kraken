 ...
 * \param out_context - the pointer to rif_context object which will be created if the function
 * performs successfully. \return RIF_SUCCESS - if the rif_image_filter object is created
 * successfully. Otherwise, it returns one of the following errors: \return
 * RIF_ERROR_INVALID_API_VERSION - if API version is unsupported. \return
 * RIF_ERROR_INVALID_PARAMETER - if \p out_context is nullptr. \return RIF_ERROR_INVALID_CONTEXT -
 * if context creation failed. \return RIF_ERROR_INTERNAL_ERROR - if an internal error occurs. In
 * such case, please send a bug report to this email address AMDRadeonProDeveloper@amd.com
 */
extern RIF_API_ENTRY rif_int rifCreateContext(rif_uint64 api_version,
 ...
 * \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one of the
 * following errors: \return RIF_ERROR_INVALID_PARAMETER - if \p name or \p vendor is nullptr.
 * \return RIF_ERROR_INVALID_CONTEXT - if getting of the device info failed.
 */
extern RIF_API_ENTRY rif_int rifGetDeviceInfo(rif_backend_api_type backend_api_type,
                                              rif_int device_id,
                                              rif_device_info device_info,
                                              size_t size,
                                              void *data,
                                              size_t *size_ret);
 ...
 * Creates the new rif_image object with defined by \p image_desc format associated with
 * rif_context.
 *
 * \param[in] context - a valid rif_context object.
 * \param[in] image_desc - the description of image size and image pixels format.
 * \param[out] out_image - the pointer to rif_image object which will be created if the function
 * performs successfully. \param[in] data the pointer to host data storage. \return RIF_SUCCESS -
 * if the rif_image object is created successfully. Otherwise, it returns one of the following
 * errors: \return RIF_ERROR_INVALID_CONTEXT - if \p context is not a valid rif_context. \return
 * RIF_ERROR_INVALID_PARAMETER - if \p out_image is nullptr. \return
 * RIF_ERROR_UNSUPPORTED_IMAGE_FORMAT - if image format defined by \p image_desc is currently
 * unsupported.
 */
extern RIF_API_ENTRY rif_int rifContextCreateImage(rif_context context,
 ...
 * \param[out] size_ret - the size of the data stored in \p data.
 * \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one of the
 * following errors: \return RIF_ERROR_INVALID_IMAGE - if \p image is not a valid rif_image.
 * \return RIF_ERROR_INVALID_PARAMETER - if \p size_ret or data is nullptr or if size is less than
...
/*!
 * \brief rifImageMap
 *  Maps the rif_image pixels data into the host address space and returns a pointer to this mapped
 * region.
 *
 * \param[in] image - a valid rif_image object.
 ...
 * \RIF_ERROR_INTERNAL_ERROR - if an internal error occurs. Sends a bug report if such an error
 * occurs.
 */
 ...
 *
 * \param[in] context - a valid rif_context object.
 * \param[out] command_queue - the pointer to rif_command_queue object which will be created if
 * function performs successfully. \return RIF_SUCCESS - if the command_queue object is created
 * successfully. Otherwise, it returns one of the following errors: \return
 * RIF_ERROR_INVALID_CONTEXT - if \p context is not a valid rif_context. \return
 * RIF_ERROR_INVALID_PARAMETER - if \p command_queue is nullptr.
 */
extern RIF_API_ENTRY rif_int rifContextCreateCommandQueue(rif_context context,
                                                          rif_command_queue *command_queue);

/*!
 * \brief rifContextExecuteCommandQueue
 * Starts the execution of the command_queue object associated with context \p context. All the
 * filters in queue are not guarantied to complete execution by the time this function returns. See
 * rifFlushQueue, rifSyncronizeQueue.
 *
 * \param[in] context - a valid rif_context object.
 * \param[in] command_queue - the pointer to rif_command_queue object to execute.
 * \param[in] executionFinishedCallbackFunction - the user callback function which will be called
 * after command queue executution started. executionFinishedCallbackFunction can be a nullptr.
 * \param[in] data - the user data which will be passed to executionFinishedCallbackFunction.
 * \param[out] time - if not nullptr, will activate profiling and will store execution time. Must
 * be initialized to 0 by user. \return RIF_SUCCESS - if the command queue is executed
 * successfully. Otherwise, it returns one of the following errors: \return
 * RIF_ERROR_INVALID_CONTEXT - if \p context is not a valid rif_context object. \return
 * RIF_ERROR_INVALID_QUEUE - if \p command_queue is not a valid rif_command_queue object. \return
 * RIF_ERROR_INTERNAL_ERROR - if an internal error occurs. Send a bug report if such an error
 * occurs.
 */
extern RIF_API_ENTRY rif_int
rifContextExecuteCommandQueue(rif_context context,
                              rif_command_queue command_queue,
 ...
 *
 * \param[in] context - a valid rif_context object.
 * \param[in] type - the filter type that defines the conversion to be performed. For example,
 * RIF_IMAGE_FILTER_TONE_MAP. or RIF_IMAGE_FILTER_GAMMA_CORRECTION. Each type of filter corresponds
 * to a set of parameters with a certain string names and certain data types. \param[out]
 * out_effect - the pointer to rif_image_filter object which will be created if function performs
 * successfully. \return RIF_SUCCESS - if the rif_image_filter object is created successfully.
 * Otherwise, it returns one of the following errors: \return RIF_ERROR_INVALID_CONTEXT - if \p
 * context is not a valid rif_context. \return RIF_ERROR_INVALID_PARAMETER - if \p out_effect is
 * nullptr \return RIF_ERROR_UNIMPLEMENTED - if filter with type \p type is not implemented at the
 * moment. \return RIF_ERROR_UNSUPPORTED - if filter with type \p type is not supported.
 */
extern RIF_API_ENTRY rif_int rifContextCreateImageFilter(rif_context context,
                                                         rif_image_filter_type type,
                                                         rif_image_filter *out_effect);

/*!
 * \brief rifCommandQueueAttachImageFilter
 * Attaches the filter \p image_filter to command queue \p command_queue and set the input and
 * output rif_image object of the filter.
 *
 ...
 * \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one of the
 * following errors: \return RIF_ERROR_INVALID_QUEUE - if \p command_queue is not a valid
 * rif_command_queue object. \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter object. \return RIF_ERROR_INVALID_IMAGE - if \p input_image or \p output_image
 * is not a valid rif_image object. \return RIF_ERROR_INTERNAL_ERROR - if an internal error occurs.
 * Sends a bug report if such an error occurs.
 */
extern RIF_API_ENTRY rif_int rifCommandQueueAttachImageFilter(rif_command_queue command_queue,
 ...
 * Attaches the filter \p image_filter to command queue \p command_queue and set the rectangle to
 * be processed, input and output rif_image object of the filter.
 *
 * \param[in] command_queue - a valid rif_command_queue object.
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] input_image - a valid rif_image object.
 * \param[in] output_image - a valid rif_image object.
 * \param[in] x - X coordinate of the left most pixel of the rectangle to be processed. Must be a
 * multiple of 8. \param[in] y - Y coordinate of the top most pixel of the rectangle to be
 * processed. Must be a multiple of 8. \param[in] w - width of the rectangle to be processed. Must
 * be a multiple of 8. \param[in] h - height of the rectangle to be processed. Must be a multiple
 * of 8. \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one
 * of the following errors: \return RIF_ERROR_INVALID_QUEUE - if \p command_queue is not a valid
 * rif_command_queue object. \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter object. \return RIF_ERROR_INVALID_IMAGE - if \p input_image or \p output_image
 * is not a valid rif_image object. \return RIF_ERROR_INTERNAL_ERROR - if an internal error occurs.
 * Sends a bug report if such an error occurs. \return RIF_ERROR_INVALID_PARAMETER - if either x,
 * y, w, or h is not a multiple of 8.
 */
extern RIF_API_ENTRY rif_int rifCommandQueueAttachImageFilterRect(rif_command_queue command_queue,
                                                                  rif_image_filter image_filter,
                                                                  rif_image input_image,
                                                                  rif_image output_image,
                                                                  rif_uint x,
 ...
 * following errors: \return RIF_ERROR_INVALID_QUEUE - if \p command_queue is not a valid
 * rif_command_queue object. \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter object. \return RIF_ERROR_INVALID_PARAMETER - if \p image_filter is not
 * attached to \p command_queue.
 */
extern RIF_API_ENTRY rif_int rifCommandQueueDetachImageFilter(rif_command_queue command_queue,
                                                              rif_image_filter image_filter);

/*!
 * \brief rifImageFilterSetParameter1u
 * Used to set the unsigned integer parameter value for a specified by \p name parameter of the
 * filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 ...
 * \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one of the
 * following errors: \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type unsigned int. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter1u(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_uint x);

/*!
 * \brief rifImageFilterSetParameter2u
 * Used to set the two-component unsigned integer vector parameter for a specified by \p name
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type two-component unsigned int vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter2u(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_uint x,
                                                          rif_uint y);
 ...
 * Used to set the tree-component unsigned integer vector parameter for a specified by \p name
 * parameter of the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type tree-component unsigned int vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter3u(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_uint x,
                                                          rif_uint y,
                                                          rif_uint z);
 ...
 * Used to set the four-component unsigned integer vector parameter for a specified by \p name
 * parameter of the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type four-component unsigned int vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter4u(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_uint x,
                                                          rif_uint y,
                                                          rif_uint z,
 ...
 * Used to set the unsigned int 16-component vector parameter values for a specified by \p name
 * parameter of the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is of type 8-component unsigned int vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter8u(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_uint *val);
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is of type 16-component unsigned int vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter16u(rif_image_filter image_filter,
                                                           rif_char const *name,
                                                           rif_uint *val);
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type int. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter1i(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_int x);

/*!
 * \brief rifImageFilterSetParameter2i
 * Used to set the two-component integer vector parameter for a specified by \p name parameter of
 * the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * filter parameter \p name is not of type unsigned int. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter2i(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_int x,
                                                          rif_int y);
 ...
 * Used to set the tree-component integer vector parameter for a specified by \p name parameter of
 * the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type tree-component int vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter3i(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_int x,
                                                          rif_int y,
                                                          rif_int z);
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type four-component int vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter4i(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_int x,
                                                          rif_int y,
                                                          rif_int z,
 ...
 * Used to set the int 16-component vector parameter values for a specified by \p name parameter of
 * the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is of type 8-component int vector. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter8i(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_int *val);
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is of type 16-component int vector. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter16i(rif_image_filter image_filter,
                                                           rif_char const *name,
                                                           rif_int *val);
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type float. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter1f(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_float x);
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type two-component float vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter2f(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_float x,
                                                          rif_float y);
 ...
 * Used to set the float three-component vector parameter values for a specified by \p name
 * parameter of the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type three-component float vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter3f(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_float x,
                                                          rif_float y,
                                                          rif_float z);
 ...
 * Used to set the float four-component vector parameter values for a specified by \p name
 * parameter of the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type four-component float vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter4f(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_float x,
                                                          rif_float y,
                                                          rif_float z,
 ...
 * Used to set the float 8-component vector parameter values for a specified by \p name parameter
 * of the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type 8-component float vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter8f(rif_image_filter image_filter,
                                                          rif_char const *name,
                                                          rif_float *val);

/*!
 * \brief rifImageFilterSetParameter16f
 * Used to set the float 16-component vector parameter values for a specified by \p name parameter
 * of the filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type 16-component float vector. \see
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameter16f(rif_image_filter image_filter,
                                                           rif_char const *name,
                                                           rif_float *val);
 ...
 * rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameterString(rif_image_filter image_filter,
                                                              rif_char const *name,
                                                              rif_char const *val);
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type image. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameterImage(rif_image_filter image_filter,
                                                             rif_char const *name,
                                                             rif_image img);
 ...
 * following errors: \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterClearParameterImage(rif_image_filter image_filter,
                                                               rif_char const *name);

/*!
 * \brief rifImageFilterSetParameterFloatArray
 * Used to set the float array parameters values for a specified by \p name parameter of the
 * filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type float array. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameterFloatArray(rif_image_filter image_filter,
                                                                  rif_char const *name,
                                                                  rif_float *arr,
                                                                  rif_uint num);
 ...
 * Used to set the array of images parameter values for a specified by \p name parameter of the
 * filter.
 *
 * \param[in] image_filter - a valid rif_image_filter object.
 * \param[in] name - the name of the filter parameter which value will be changed.
 ...
 * rif_image_filter. \return RIF_ERROR_INVALID_FILTER_ARGUMENT_NAME - if parameter with name \p
 * name is not associated with this type of filter. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if
 * filter parameter \p name is not of type image array. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetParameterImageArray(rif_image_filter image_filter,
                                                                  rif_char const *name,
                                                                  rif_image *arr,
                                                                  rif_uint num);
 ...
 * \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one of the
 * following errors: \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter. \see rifContextCreateImageFilter
 */
extern RIF_API_ENTRY rif_int rifImageFilterSetComputeType(rif_image_filter image_filter,
 ...
 * \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one of the
 * following errors: \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter. \return RIF_ERROR_INVALID_PARAMETER - if \p size_ret is nullptr or if size_net
 * is less than the requested image info. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if filter_info
 * type is not supported.
 */
extern RIF_API_ENTRY rif_int rifImageFilterGetInfo(rif_image_filter image_filter,
 ...
 * following errors: \return RIF_ERROR_INVALID_FILTER - if \p image_filter is not a valid
 * rif_image_filter. \return RIF_ERROR_INVALID_PARAMETER - if \p size_ret is nullptr or if size_net
 * is less than the requested image info. \return RIF_ERROR_INVALID_PARAMETER_TYPE - if param_info
 * type is not supported.
 */
extern RIF_API_ENTRY rif_int rifParameterGetInfo(rif_image_filter image_filter,
                                                 rif_uint paramIdx,
 ...
 * \param[in] size - size of memory allocated for requested context info (incl. terminating zero
 * for strings). \param[out] data - memory pointer to write requested info to. \param[out] size_ret
 * - the size of the data stored in \p data \return RIF_SUCCESS - if the function is executed
 * successfully. Otherwise, it returns one of the following errors: \return
 * RIF_ERROR_INVALID_CONTEXT - if \p context is not a valid rif_context. \return
 * RIF_ERROR_INVALID_PARAMETER - if \p data and \p size_ret are both NULL, or value of \p
 * context_info is invalid, or \p size bytes in not enough to store requested info.
 */
extern RIF_API_ENTRY rif_int rifContextGetInfo(rif_context context,
 ...
 * \return RIF_SUCCESS - if the function is executed successfully. Otherwise, it returns one of the
 * following errors: \return RIF_ERROR_INVALID_CONTEXT - if \p context is not a valid rif_context.
 * \return RIF_ERROR_INVALID_PARAMETER - if is value of \p context_info is invalid.
 */
extern RIF_API_ENTRY rif_int rifContextSetInfo(rif_context context,
                                               rif_context_info context_info,
                                               const void *data);
