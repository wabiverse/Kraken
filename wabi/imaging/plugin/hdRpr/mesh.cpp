                     ...
                     TfToken const &reprName)
{
  ...
  if (HdChangeTracker::IsPrimvarDirty(*dirtyBits, id, HdTokens->normals)) {
    ...
    m_authoredNormals = HdRprSamplePrimvar(id,
                                           HdTokens->normals,
                                           sceneDelegate,
  ...
  if (newMesh) {
    ...
    if (m_geomSubsets.empty()) {
      if (auto rprMesh = rprApi->CreateMesh(m_pointSamples,
                                            m_faceVertexIndices,
                                            m_normalSamples,
                                            m_normalIndices,
                                            m_uvSamples,
      ...
      for (auto it = m_geomSubsets.begin(); it != m_geomSubsets.end();) {
        ...
        if (!m_uvIndices.empty()) {
          ...
          std::fill(uvIndexRemapping.begin(),
                    uvIndexRemapping.begin() + m_uvSamples.front().size(),
                    -1);
        }
        ...
        if (auto rprMesh = rprApi->CreateMesh(subsetPointSamples,
                                              subsetIndexes,
                                              subsetNormalSamples,
                                              subsetNormalIndices,
                                              subsetUvSamples,
  ...
  if (!m_rprMeshes.empty()) {
      ...
      auto getMeshMaterial =
        [sceneDelegate, rprApi, dirtyBits, &primvarDescsPerInterpolation, this](
          ...
          if (material && material->GetRprMaterialObject()) {
            ...
            HdRprFillPrimvarDescsPerInterpolation(sceneDelegate,
                                                  GetId(),
                                                  &primvarDescsPerInterpolation);
            return GetFallbackMaterial(sceneDelegate,
                                       rprApi,
                                       *dirtyBits,
                                       primvarDescsPerInterpolation);
          }
        ...
        if (newNumInstances == 0) {
          ...
          for (int i = 0; i < m_rprMeshes.size(); ++i) {
            ...
            for (int j = 0; j < newNumInstances; ++j) {
              rprApi->SetTransform(meshInstances[j],
                                   instanceTransforms.count,
                                   instanceTransforms.times.data(),
                                   combinedTransforms[j].data());
            }
    ...
    if (updateTransform) {
      for (auto &rprMesh : m_rprMeshes) {
        rprApi->SetTransform(rprMesh,
                             m_transformSamples.count,
                             m_transformSamples.times.data(),
                             m_transformSamples.values.data());
      }
    }
  }
