...
template<typename T>
bool HdRprGetConstantPrimvar(TfToken const &name,
                             HdSceneDelegate *sceneDelegate,
                             SdfPath const &id,
                             T *out_value)
{
  ...
  TF_WARN("[%s] %s: unexpected type. Expected %s but actual type is %s",
          id.GetText(),
          name.GetText(),
          typeid(T).name(),
          typeName.c_str());
...
template<typename T>
bool HdRprSamplePrimvar(SdfPath const &id,
                        TfToken const &key,
                        HdSceneDelegate *sceneDelegate,
                        ...
                        VtArray<T> *sampleValuesPtr)
{
  ...
  size_t authoredSampleCount = sceneDelegate->SamplePrimvar(id,
                                                            key,
                                                            maxSampleCount,
                                                            sampleTimes.data(),
                                                            sampleVtValues.data());
  ...
  for (size_t i = 0; i < sampleVtValues.size(); ++i) {
    if (sampleVtValues[i].IsHolding<T>()) {
      ...
      TF_RUNTIME_ERROR("[%s] Failed to sample %s primvar data: unexpected underlying type - %s",
                       id.GetText(),
                       key.GetText(),
                       sampleVtValues[i].GetTypeName().c_str());
      return false;
    }
  }
...
inline void HdRprGetPrimvarIndices(HdInterpolation interpolation,
