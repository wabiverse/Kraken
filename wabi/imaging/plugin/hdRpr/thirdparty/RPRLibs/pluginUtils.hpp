...
// Finds grid with given name in vdb file and reads it into provided data container; returns error
// otherwise RET - bool - true if data was read succesfully RET - string - error description OUT -
// VDBGrid - vdb grid data ready to be passed to RPR IN  - openvdb::io::File - vdb file to be read
// (file must be opened) IN  - string - name of the grid to be read
template<typename GridValueT>
std::tuple<bool, std::string> ReadFileGridToVDBGrid(VDBGrid<GridValueT> &outGridData,
...
// IN - Grid bbox - can be different from current grid bbox (this is to support case with different
// grids having different sizes)
template<typename GridValueT>
bool ProcessVDBGrid(VDBGrid<GridValueT> &outGrid,
                    const openvdb::GridBase *baseGrid,
                    const openvdb::CoordBBox &bbox);

// Reads only names of the grids
// The idea is that there could be large number of grids, and we want to proceed only up to 3 at a
// time so there is no need to read all of the grids However we need to grab the names of the grids
// into UI RET - bool - true if data was read succesfully RET - string - error description IN -
