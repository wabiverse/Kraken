...
const TfTokenVector HdRprDelegate::SUPPORTED_SPRIM_TYPES = {HdPrimTypeTokens->camera,
...
HdRprDelegate::~HdRprDelegate()
{
  ...
  // Config singleton persists in memory after delegate destruction, therefore version must be
  // invalidated
  HdRprConfig *config;
...
HdInstancer *HdRprDelegate::CreateInstancer(HdSceneDelegate *delegate,
                                            SdfPath const &id HDRPR_INSTANCER_ID_ARG_DECL)
{
...
{
  WABI_INTERNAL_NS::HdRprConfig *config;
  auto configInstanceLock = WABI_INTERNAL_NS::HdRprConfig::GetInstance(&config);
  config->SetRenderDevice(WABI_INTERNAL_NS::TfToken(renderDevice));
}
...
void HdRprSetRenderQuality(const char *quality)
{
  WABI_INTERNAL_NS::HdRprConfig *config;
  auto configInstanceLock = WABI_INTERNAL_NS::HdRprConfig::GetInstance(&config);
  config->SetRenderQuality(WABI_INTERNAL_NS::TfToken(quality));
}
