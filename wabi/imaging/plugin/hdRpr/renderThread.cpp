...
void HdRprRenderThread::StopRender()
{
  if (IsRendering())
  {
    ...
    // In case rendering thread currently inside of some sort of rendering task that could be
    // stopped call stopCallback to speed up return from renderCallback
    m_stopCallback();
    std::unique_lock<std::mutex> lock(m_requestedStateMutex);
    m_requestedState = StateIdle;
