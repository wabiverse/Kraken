...
namespace rif
{
  ...
  namespace
  {
      ...
      : Context(modelPath)
    {
      ...
      rpr_status status = rprContext->GetInfo(rpr::ContextInfo(RPR_CL_CONTEXT),
      ...
      status = rprContext->GetInfo(rpr::ContextInfo(RPR_CL_DEVICE),
      ...
      RIF_ERROR_CHECK_THROW(rifCreateContextFromOpenClContext(RIF_API_VERSION,
                                                              clContext,
                                                              clDevice,
                                                              clCommandQueue,
                                                              path.data(),
...
#undef GPU
      ...
      : Context(modelPath)
    {
      ...
      rpr_status status = rprContext->GetInfo(RPR_CONTEXT_CREATION_FLAGS,
    ...
    std::unique_ptr<Image> ContextMetal::CreateImage(HdRprApiFramebuffer *rprFrameBuffer)
    {
      ...
      auto status = rprFrameBuffer->GetRprObject()->GetInfo(RPR_FRAMEBUFFER_FORMAT,
                                                            sizeof(fbFormat),
                                                            &fbFormat,
                                                            nullptr);
      if (status != RPR_SUCCESS)
...
#undef GPU
  ...
  std::unique_ptr<Context> Context::Create(rpr::Context *rprContext,
                                           ...
                                           std::string const &modelPath)
  {
    ...
    rpr_creation_flags contextFlags = 0;
    if (RPR_ERROR_CHECK(rprContext->GetInfo(RPR_CONTEXT_CREATION_FLAGS,
                                            sizeof(rpr_creation_flags),
                                            &contextFlags,
                                            nullptr),
  ...
  {
    rif_image rifImage = nullptr;
    RIF_ERROR_CHECK_THROW(rifContextCreateImage(m_context, &desc, nullptr, &rifImage),
                          "Failed to create RIF image");
    return std::unique_ptr<Image>(new Image(rifImage));
  }
  ...
  rif_image_filter Context::CreateImageFilter(rif_image_filter_type type)
  {
    ...
    RIF_ERROR_CHECK_THROW(rifContextCreateImageFilter(m_context, type, &outFilter),
