...
struct LoaderContext
{
  ...
  class ValueConverter
  {
   public:
    ...
    virtual bool Convert(float color[4], LoaderContext *context) const
    {
      return false;
    }

    /// Used to convert color value computed in run-time
    virtual std::unique_ptr<BaseConverterNode> GetConversionNode(LoaderContext *context) const
    {
      return nullptr;
    }
  };
...
struct Node
{
  ...
  virtual rpr_status Connect(std::string const &upstreamOutput,
                             Node *downstreamNode,
                             mx::TypedElement *downstreamElement,
                             LoaderContext *context) = 0;

  /// Set input of the current (downstream) node to the output of upstream node
  virtual rpr_status SetInput(mx::TypedElement *downstreamElement,
                              mx::Element *upstreamElement,
  ...
  /// at the end of MaterialX processing we collect them into one linear array that is returned to
  /// the user.
  ///
...
struct RprNode : public Node
{
  ...
  rpr_status Connect(std::string const &upstreamOutput,
                     Node *downstreamNode,
                     mx::TypedElement *downstreamElement,
  ...
  rpr_status SetInput(mx::TypedElement *downstreamElement,
                      rpr_material_node_input downstreamRprId,
                      mx::ValueElement *upstreamValueElement,
...
struct MtlxNodeGraphNode : public Node
{
  ...
  MtlxNodeGraphNode(mx::ConstGraphElementPtr const &mtlxGraph,
...
mx::OutputPtr GetOutput(mx::InterfaceElement const *interfaceElement,
                        mx::PortElement *portElement,
                        LoaderContext *context)
{
  ...
  if (interfaceElement->getType() == mx::MULTI_OUTPUT_TYPE_STRING)
  {
    ...
    if (targetOutputName.empty())
    {
      LOG_ERROR(context,
                ...
                interfaceElement->asString().c_str());
      return nullptr;
    }
...
bool LoaderContext::ConnectToGlobalOutput(T *input, Node *node)
{
  ...
  // The output attribute may point to the output of some free-standing node graph or free-standing
  // output
  //
      ...
      std::unique_ptr<BaseConverterNode> GetConversionNode(LoaderContext *context) const override
      {
        struct SrgbConversionNode : public BaseConverterNode
        {
          ...
          SrgbConversionNode(LoaderContext *context)
          {
            ...
            // set RPR_MATERIAL_INPUT_COLOR0 to the input color
            rprMaterialNodeSetInputFByKey(nodes[kIsAboveBreak],
                                          RPR_MATERIAL_INPUT_COLOR1,
                                          kSrgbBreakPnt[0],
                                          kSrgbBreakPnt[1],
                                          kSrgbBreakPnt[2],
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        LOG(context,
            "Unsupported node: %s (%s)",
            mtlxNode->getName().c_str(),
            mtlxNode->getCategory().c_str());
      }
    }
  }
...
{
  return outputType == "color3" || outputType == "color2" || outputType == "vector3" ||
...
rpr_status RprWrapNode::SetInput(mx::TypedElement *downstreamElement,
                                 mx::Element *upstreamElement,
                                 rpr_material_node upstreamRprNode,
                                 LoaderContext *context)
{
...
rpr_status MtlxNodeGraphNode::Connect(std::string const &upstreamOutput,
                                      Node *downstreamNode,
                                      mx::TypedElement *downstreamElement,
                                      LoaderContext *context)
{
...
rpr_status MtlxNodeGraphNode::SetInput(mx::TypedElement *downstreamElement,
                                       mx::Element *upstreamElement,
                                       rpr_material_node upstreamRprNode,
                                       LoaderContext *context)
{
  ...
  LOG_ERROR(context,
            "failed to set %s input for %s: no such interface socket",
...
rpr_status RprNode::Connect(std::string const &upstreamOutput,
                            Node *downstreamNode,
                            mx::TypedElement *downstreamElement,
                            LoaderContext *context)
{
...
rpr_status RprNode::SetInput(mx::TypedElement *downstreamElement,
                             mx::Element *upstreamElement,
                             rpr_material_node upstreamRprNode,
                             LoaderContext *context)
{
...
bool GetInputF(mx::TypedElement *downstreamElement,
               mx::ValueElement *upstreamElement,
                                 ...
                                 LoaderContext *context)
{
  ...
  if (isFileDirty)
  {
    ...
    resolvedFilepath = context->ResolveFile(fileValueElement->getActiveFilePrefix() +
                                            fileValueElement->getValueString());
...
rpr_status RprImageNode::SetInput(mx::TypedElement *downstreamElement,
                                  mx::ValueElement *upstreamValueElement,
                                  LoaderContext *context)
{
...
void ForEachOutput(mx::ElementPtr const &element, F &&func)
{
  ...
  //                 if (mx::ElementPtr shaderNodeElement =
  //                 element->getParent()->getChild(shader->getNodeName())) {
...
//     // We are not using looks for what they are designed (the assignment of material to
//     geometry)
...
RPRMtlxLoader::Result RPRMtlxLoader::Load(MaterialX::Document const *mtlxDocument,
                                          ...
                                          rpr_material_system rprMatSys)
{
  ...
  class MtlxRenderableElements
  {
   public:
    ...
    Element const &Get(RPRMtlxLoader::OutputType type) const
    {
  ...
  //         entry.second.node = std::make_unique<MtlxNodeGraphNode>(nodeGraph, requiredOutputs,
  //         &ctx);
  ...
  //                         LOG(&ctx, "Bindinput %s: %s (%s)", bindInput->getName().c_str(),
  //                         valueStr.c_str(), type.c_str());
  ...
  //                 if (auto nodeGraphOutput = GetOutput(nodeGraphNode->mtlxGraph.get(),
  //                 output.get(), &ctx)) {
  //                     if (auto subNode =
  //                     nodeGraphNode->GetSubNode(nodeGraphOutput->getNodeName(), &ctx)) {
  ...
  //                     rprOutput->Connect(mx::EMPTY_STRING, entry.second.wrapNode.get(), nullptr,
  //                     &ctx); rprOutput = entry.second.wrapNode.get();
  //                 }
  //             }
