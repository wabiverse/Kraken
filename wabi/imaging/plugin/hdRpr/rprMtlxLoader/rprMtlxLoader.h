...
class RPRMtlxLoader
{
 public:
  ...
  void SetSceneDistanceUnit(std::string const &unit)
  {
  ...
  struct Result
  {
    ...
    struct ImageNode
    {
      ...
      /// The most common example is applying gamma to rpr_image, where gamma value is taken from
      /// the file's metadata. disableRprImageColorspace is true when mtlx loader generates its own
      /// conversion nodes.
  ...
  /// \p renderableElements may be null, in this case, loader will use first available ones in the
  /// mtlxDocument. To disable a particular output type element, pass empty string. If any
  /// renderable element (non-empty string) does not exist in mtlxDocument, the loader will
  /// auto-select a renderable element with a corresponding output type.
  ///
  ...
  Result Load(MaterialX::Document const *mtlxDocument,
