...
void HdRprApiFramebuffer::Clear(float r, float g, float b, float a)
{
  ...
  // value of singlesampled AOV (any ID AOV, worldCoordinate, etc) is always equals to `clearValue
  // + renderedValue`
...
bool HdRprApiFramebuffer::GetData(void *dstBuffer, size_t dstBufferSize)
{
  ...
  return !RPR_ERROR_CHECK(m_rprFb->GetInfo(RPR_FRAMEBUFFER_DATA, size, dstBuffer, nullptr),
                          "Failed to get framebuffer data");
}
