...
TF_DEFINE_PRIVATE_TOKENS(HdRprVolumeTokens,
                         (color)(density)(temperature)(scatteringColor)(transmissionColor)(emissionColor)(anisotropy)(multipleScattering)(normalize)(bias)(gain)(scale)(ramp)(blackbodyMode)(physical)(artistic));
...
- scale - float - 1.0 - scale to be applied to value before lookup table. `fieldColor = LUT(scale *
fieldValue)`
...
- blackbodyMode - TfToken - auto:
    * physical mode: temperature interpreted as pure physical values - the temperature in Kelvins.
The temperature will be normalized by some max temperature and used as an emissive lookup. The
emission lookup table will be filled with blackbody colors in the range [0, maxTemperature].
...
namespace
{
  ...
  HdRprApi::VolumeMaterialParameters ParseVolumeMaterialParameters(HdSceneDelegate *sceneDelegate,
                                                                   SdfPath const &volumeId)
  {
    ...
    params.scatteringColor = sceneDelegate->Get(volumeId, HdRprVolumeTokens->scatteringColor)
                               .GetWithDefault(params.scatteringColor);
    params.transmissionColor = sceneDelegate->Get(volumeId, HdRprVolumeTokens->transmissionColor)
                                 .GetWithDefault(params.transmissionColor);
    params.emissionColor = sceneDelegate->Get(volumeId, HdRprVolumeTokens->emissionColor)
                             .GetWithDefault(params.emissionColor);
    params.density =
      sceneDelegate->Get(volumeId, HdRprVolumeTokens->density).GetWithDefault(params.density);
    params.anisotropy = sceneDelegate->Get(volumeId, HdRprVolumeTokens->anisotropy)
                          .GetWithDefault(params.anisotropy);
    params.multipleScattering = sceneDelegate->Get(volumeId, HdRprVolumeTokens->multipleScattering)
                                  .GetWithDefault(params.multipleScattering);
    return params;
  }
  ...
  template<typename T>
  bool ParseGridParameter(TfToken const &name,
  ...
  GridParameters ParseGridParameters(HdSceneDelegate *sceneDelegate, SdfPath const &fieldId)
  {
    ...
    if (ParseGridParameter(HdRprVolumeTokens->normalize,
                           sceneDelegate,
                           fieldId,
                           &params.normalize))
      params.authoredParamsMask |= GridParameters::kNormalizeAuthored;
    if (ParseGridParameter(HdRprVolumeTokens->bias, sceneDelegate, fieldId, &params.bias))
      params.authoredParamsMask |= GridParameters::kBiasAuthored;
    if (ParseGridParameter(HdRprVolumeTokens->gain, sceneDelegate, fieldId, &params.gain))
      params.authoredParamsMask |= GridParameters::kGainAuthored;
    if (ParseGridParameter(HdRprVolumeTokens->scale, sceneDelegate, fieldId, &params.scale))
      params.authoredParamsMask |= GridParameters::kScaleAuthored;
    if (ParseGridParameter(HdRprVolumeTokens->ramp, sceneDelegate, fieldId, &params.ramp))
      params.authoredParamsMask |= GridParameters::kRampAuthored;
    return params;
  }
  ...
  void ParseOpenvdbMetadata(GridInfo *grid)
  {
    auto isAllParametersParsed = [](GridInfo *grid) {
      ...
      static constexpr auto metadataParameters = GridParameters::kRampAuthored |
    ...
    try {
      ...
      for (auto it = metadata->beginMeta();
           it != metadata->endMeta() && !isAllParametersParsed(grid);
           ++it) {
        if (it->first == cdrampMd) {
          ...
          try {
            ...
            if (root["colortype"] == "RGB") {
              ...
              // Houdini's ramp is defined as parameter-color pair (the parameter is in [0; 1]
              // range) Here we convert arbitrarily distributed color ramp to a linear ramp
              ...
              for (int i = 0; i < kLookupTableGranularityLevel; ++i) {
                ...
                ramp.push_back(HdResampleRawTimeSamples(t,
                                                        parameters.size(),
                                                        parameters.data(),
                                                        colors.data()));
              }
      ...
      TF_RUNTIME_ERROR("Failed to parse openvdb metadata \"%s\": %s",
                       grid->filepath.c_str(),
                       e.what());
    }
  }
                       ...
                       TfToken const &reprName)
{
  ...
  if (*dirtyBits & HdChangeTracker::DirtyTopology) {
    ...
    auto getVdbGrid = [&](SdfPath const &fieldId,
      ...
      auto fieldName =
        sceneDelegate->Get(fieldId, UsdVolTokens->fieldName).GetWithDefault(TfToken());
        ...
        if (targetInfo->vdbGrid) {
          ...
          if (fieldSubscription == m_fieldSubscriptions.end()) {
            activeFieldSubscriptions.emplace(
              desc.fieldId,
              rprRenderParam->SubscribeVolumeForFieldUpdates(this, desc.fieldId));
          } else {
            // Reuse the old one
    ...
    if (densityGridInfo.vdbGrid && emissionGrid) {
      ...
      if (densityGridInfo.vdbGrid->transform() != emissionGrid->transform())
        TF_RUNTIME_ERROR(
          "[Node: %s]: density grid and temperature grid have different transform. Taking "
          "transform of density grid",
          GetId().GetName().c_str());
    }

    openvdb::Vec3d voxelSize = densityGrid ? densityGrid->voxelSize() : emissionGrid->voxelSize();
    openvdb::math::Transform gridTransform = densityGrid ? densityGrid->transform() :
                                                           emissionGrid->transform();
    openvdb::CoordBBox activeVoxelsBB;
    if (densityGrid)
      activeVoxelsBB.expand(densityGrid->evalActiveVoxelBoundingBox());
    if (emissionGrid)
      activeVoxelsBB.expand(emissionGrid->evalActiveVoxelBoundingBox());
    if (albedoGrid)
      activeVoxelsBB.expand(albedoGrid->evalActiveVoxelBoundingBox());
    ...
    if (densityGrid) {
      ...
      if (densityGridInfo.params.ramp.empty()) {
        if ((densityGridInfo.params.authoredParamsMask & GridParameters::kNormalizeAuthored) ==
            0) {
    ...
    GfVec3f gridBBLow((float)(gridMin.x() - voxelSize[0] / 2),
                      (float)(gridMin.y() - voxelSize[1] / 2),
                      (float)(gridMin.z() - voxelSize[2] / 2));
    ...
    m_rprVolume = rprApi->CreateVolume(densityGridData.coords,
                                       densityGridData.values,
                                       densityGridInfo.params.ramp,
                                       densityGridInfo.params.scale,
                                       albedoGridData.coords,
                                       albedoGridData.values,
                                       albedoGridInfo.params.ramp,
                                       albedoGridInfo.params.scale,
                                       emissionGridData.coords,
                                       emissionGridData.values,
                                       emissionGridInfo.params.ramp,
                                       emissionGridInfo.params.scale,
                                       GfVec3i(activeVoxelsBBSize.asPointer()),
                                       voxelSizeGf,
                                       gridBBLow,
                                       volumeMaterialParams);
...
{
  int mask = HdChangeTracker::Clean | HdChangeTracker::DirtyTopology |
...
void HdRprVolume::_InitRepr(TfToken const &reprName, HdDirtyBits *dirtyBits)
{
  TF_UNUSED(reprName);
  TF_UNUSED(dirtyBits);
