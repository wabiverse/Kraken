                       ...
                       TfToken const &reprSelector)
{
  ...
  bool dirtyDisplayColors = HdChangeTracker::IsPrimvarDirty(*dirtyBits,
                                                            id,
                                                            HdTokens->displayColor);
  if (dirtyDisplayColors) {
    HdRprFillPrimvarDescsPerInterpolation(sceneDelegate, id, &primvarDescsPerInterpolation);
    if (HdRprIsPrimvarExists(HdTokens->displayColor,
                             primvarDescsPerInterpolation,
                             &m_colorsInterpolation)) {
...
void HdRprPoints::_InitRepr(TfToken const &reprName, HdDirtyBits *dirtyBits)
{
  TF_UNUSED(reprName);
  TF_UNUSED(dirtyBits);
