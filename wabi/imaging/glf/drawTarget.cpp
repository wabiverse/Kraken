...
GlfDrawTargetRefPtr GlfDrawTarget::New(GfVec2i const &size, bool requestMSAA)
{
...
GlfDrawTargetRefPtr GlfDrawTarget::New(GlfDrawTargetPtr const &drawtarget)
{
...
GlfDrawTarget::~GlfDrawTarget()
{
  ...
  if (_framebuffer) {
    TF_VERIFY(glIsFramebuffer(_framebuffer), "Tried to free invalid framebuffer");
  ...
  if (_framebufferMS) {
    TF_VERIFY(glIsFramebuffer(_framebufferMS), "Tried to free invalid multisampled framebuffer");
...
void GlfDrawTarget::DeleteAttachment(std::string const &name)
{
  AttachmentsMap &attachments = _GetAttachments();
  AttachmentsMap::iterator it = attachments.find(name);
...
GlfDrawTarget::AttachmentRefPtr GlfDrawTarget::GetAttachment(std::string const &name)
{
  AttachmentsMap &attachments = _GetAttachments();
  AttachmentsMap::iterator it = attachments.find(name);
...
void GlfDrawTarget::ClearAttachments()
{
  _DeleteAttachments();
}

void GlfDrawTarget::CloneAttachments(GlfDrawTargetPtr const &drawtarget)
{
  if (!drawtarget) {
...
GlfDrawTarget::AttachmentsMap const &GlfDrawTarget::GetAttachments() const
{
  return _GetAttachments();
}

GlfDrawTarget::AttachmentsMap &GlfDrawTarget::_GetAttachments() const
{
  TF_VERIFY(_attachmentsPtr, "DrawTarget has uninitialized attachments map.");

  return _attachmentsPtr->attachments;
}

void GlfDrawTarget::SetSize(GfVec2i size)
{
  if (size == _size) {
    return;
  }
...
void GlfDrawTarget::_DeleteAttachments()
{
...
void GlfDrawTarget::_GenFrameBuffer()
{
  ...
  if (HasMSAA()) {
    ...
    TF_VERIFY(glIsFramebuffer(_framebufferMS), "Failed to allocate multisampled framebuffer");
  }

  // Create non-multisampled framebuffer
  glGenFramebuffers(1, &_framebuffer);
  glBindFramebuffer(GL_FRAMEBUFFER, _framebuffer);
  TF_VERIFY(glIsFramebuffer(_framebuffer), "Failed to allocate framebuffer");

  _RestoreBindingState();
}

GLuint GlfDrawTarget::GetFramebufferId() const
{
  return _framebuffer;
}

GLuint GlfDrawTarget::GetFramebufferMSId() const
{
...
void GlfDrawTarget::_BindAttachment(GlfDrawTarget::AttachmentRefPtr const &a)
{
  ...
  if (a->GetFormat() == GL_DEPTH_COMPONENT) {
    ...
    TF_VERIFY(attach < _GetMaxAttachments(), "Exceeding number of Attachments available ");
  ...
  // Regular framebuffer
  glBindFramebuffer(GL_FRAMEBUFFER, _framebuffer);
  glFramebufferTexture2D(GL_FRAMEBUFFER, attachment, GL_TEXTURE_2D, id, /*level*/ 0);
...
void GlfDrawTarget::_SaveBindingState()
{
  glGetIntegerv(GL_READ_FRAMEBUFFER_BINDING, (GLint *)&_unbindRestoreReadFB);
  glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING, (GLint *)&_unbindRestoreDrawFB);
}

void GlfDrawTarget::_RestoreBindingState()
{
  glBindFramebuffer(GL_READ_FRAMEBUFFER, _unbindRestoreReadFB);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, _unbindRestoreDrawFB);
}

void GlfDrawTarget::Bind()
{
  if (++_bindDepth != 1) {
    return;
  }
...
bool GlfDrawTarget::IsBound() const
{
  return (_bindDepth > 0);
}

void GlfDrawTarget::Unbind()
{
  if (--_bindDepth != 0) {
    return;
  }
...
/* static */
void GlfDrawTarget::Resolve(const std::vector<GlfDrawTarget *> &drawTargets)
{
  GLF_GROUP_FUNCTION();
...
void GlfDrawTarget::TouchContents()
{
...
bool GlfDrawTarget::IsValid(std::string *reason)
{
  return _Validate(reason);
}

bool GlfDrawTarget::_Validate(std::string *reason)
{
  if (!_framebuffer) {
    return false;
  }
...
bool GlfDrawTarget::WriteToFile(std::string const &name,
                                ...
                                GfMatrix4d const &projectionMatrix)
{
  ...
  if (it == attachments.end()) {
    TF_CODING_ERROR("\"" + name +
                    "\" is not a valid variable name for this"
                    " DrawTarget");
    return false;
  }
  ...
  int nelems = GlfGetNumElements(a->GetFormat()), elemsize = GlfGetElementSize(a->GetType()),
      stride = _size[0] * nelems * elemsize, bufsize = _size[1] * stride;
...
GlfDrawTarget::AttachmentRefPtr GlfDrawTarget::Attachment::New(int glIndex,
                                                               GLenum format,
                                                               GLenum type,
...
{
  return TfCreateRefPtr(new Attachment(glIndex, format, type, internalFormat, size, numSamples));
}

GlfDrawTarget::Attachment::Attachment(int glIndex,
                                      GLenum format,
                                      GLenum type,
                                      GLenum internalFormat,
                                      GfVec2i size,
...
// we assume that the framebuffer is currently bound !
void GlfDrawTarget::Attachment::_GenTexture()
{
  ...
  size_t baseImageSize = (size_t)(bytePerPixel * numChannel * _size[0] * _size[1]);
...
void GlfDrawTarget::Attachment::_DeleteTexture()
{
  if (_textureName) {
...
void GlfDrawTarget::Attachment::ResizeTexture(const GfVec2i &size)
{
  _size = size;
...
/* virtual */
GlfTexture::BindingVector GlfDrawTarget::Attachment::GetBindings(TfToken const &identifier,
                                                                 GLuint samplerName)
{
...
/* virtual */
VtDictionary GlfDrawTarget::Attachment::GetTextureInfo(bool forceLoad)
{
  TF_UNUSED(forceLoad);
...
void GlfDrawTarget::Attachment::TouchContents()
{
  _UpdateContentsID();
}
