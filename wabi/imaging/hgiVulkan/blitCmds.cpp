...
HgiVulkanBlitCmds::HgiVulkanBlitCmds(HgiVulkan *hgi) : _hgi(hgi), _commandBuffer(nullptr)
{
...
void HgiVulkanBlitCmds::PushDebugGroup(const char *label)
{
  _CreateCommandBuffer();
  HgiVulkanBeginLabel(_hgi->GetPrimaryDevice(), _commandBuffer, label);
}

void HgiVulkanBlitCmds::PopDebugGroup()
{
  _CreateCommandBuffer();
  HgiVulkanEndLabel(_hgi->GetPrimaryDevice(), _commandBuffer);
}

void HgiVulkanBlitCmds::CopyTextureGpuToCpu(HgiTextureGpuToCpuOp const &copyOp)
{
  _CreateCommandBuffer();

  HgiVulkanTexture *srcTexture = static_cast<HgiVulkanTexture *>(copyOp.gpuSourceTexture.Get());

  if (!TF_VERIFY(srcTexture && srcTexture->GetImage(), "Invalid texture handle")) {
    return;
  }
  ...
  vkCmdCopyImageToBuffer(_commandBuffer->GetVulkanCommandBuffer(),
                         srcTexture->GetImage(),
  ...
  _commandBuffer->AddCompletedHandler([dst, src, byteSize] { memcpy(dst, src, byteSize); });
}

void HgiVulkanBlitCmds::CopyTextureCpuToGpu(HgiTextureCpuToGpuOp const &copyOp)
{
  _CreateCommandBuffer();
  ...
  if (!dstTexture->IsCPUStagingAddress(copyOp.cpuSourceBuffer)) {
    ...
    if (mipInfos.size() > copyOp.mipLevel) {
      ...
      uint8_t *dst = static_cast<uint8_t *>(dstTexture->GetCPUStagingAddress());
...
void HgiVulkanBlitCmds::CopyBufferGpuToGpu(HgiBufferGpuToGpuOp const &copyOp)
{
  _CreateCommandBuffer();
...
void HgiVulkanBlitCmds::CopyBufferCpuToGpu(HgiBufferCpuToGpuOp const &copyOp)
{
  _CreateCommandBuffer();
  ...
  HgiVulkanBuffer *buffer = static_cast<HgiVulkanBuffer *>(copyOp.gpuDestinationBuffer.Get());
  ...
  if (!buffer->IsCPUStagingAddress(copyOp.cpuSourceBuffer) ||
    ...
    uint8_t *src = ((uint8_t *)copyOp.cpuSourceBuffer) + copyOp.sourceByteOffset;
...
void HgiVulkanBlitCmds::CopyBufferGpuToCpu(HgiBufferGpuToCpuOp const &copyOp)
{
  _CreateCommandBuffer();
  ...
  // Copy to cpu buffer when cmd buffer has been executed
  _commandBuffer->AddCompletedHandler([dst, src, size] { memcpy(dst, src, size); });
}

void HgiVulkanBlitCmds::CopyTextureToBuffer(HgiTextureToBufferOp const &copyOp)
{
  TF_CODING_ERROR("Missing Implementation");
}

void HgiVulkanBlitCmds::CopyBufferToTexture(HgiBufferToTextureOp const &copyOp)
{
  TF_CODING_ERROR("Missing Implementation");
}

void HgiVulkanBlitCmds::GenerateMipMaps(HgiTextureHandle const &texture)
{
  _CreateCommandBuffer();
  ...
  vkGetPhysicalDeviceFormatProperties(device->GetVulkanPhysicalDevice(), format, &formatProps);
...
void HgiVulkanBlitCmds::MemoryBarrier(HgiMemoryBarrier barrier)
{
  _CreateCommandBuffer();
  _commandBuffer->MemoryBarrier(barrier);
}

HgiVulkanCommandBuffer *HgiVulkanBlitCmds::GetCommandBuffer()
{
  return _commandBuffer;
}

bool HgiVulkanBlitCmds::_Submit(Hgi *hgi, HgiSubmitWaitType wait)
{
  if (!_commandBuffer) {
    return false;
  }
