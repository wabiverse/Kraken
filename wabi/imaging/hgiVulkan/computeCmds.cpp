...
void HgiVulkanComputeCmds::PushDebugGroup(const char *label)
{
  _CreateCommandBuffer();
  HgiVulkanBeginLabel(_hgi->GetPrimaryDevice(), _commandBuffer, label);
}

void HgiVulkanComputeCmds::PopDebugGroup()
{
  _CreateCommandBuffer();
  HgiVulkanEndLabel(_hgi->GetPrimaryDevice(), _commandBuffer);
}

void HgiVulkanComputeCmds::BindPipeline(HgiComputePipelineHandle pipeline)
{
  _CreateCommandBuffer();

  HgiVulkanComputePipeline *pso = static_cast<HgiVulkanComputePipeline *>(pipeline.Get());
...
void HgiVulkanComputeCmds::BindResources(HgiResourceBindingsHandle res)
{
  _CreateCommandBuffer();
...
void HgiVulkanComputeCmds::SetConstantValues(HgiComputePipelineHandle pipeline,
                                             uint32_t bindIndex,
                                             uint32_t byteSize,
                                             const void *data)
{
...
void HgiVulkanComputeCmds::Dispatch(int dimX, int dimY)
{
  _CreateCommandBuffer();
  _BindResources();
  ...
  const VkPhysicalDeviceLimits limits = _hgi->GetCapabilities()->vkDeviceProperties.limits;
  const GfVec3i maxNumWorkGroups = GfVec3i(limits.maxComputeWorkGroupCount[0],
  ...
  vkCmdDispatch(_commandBuffer->GetVulkanCommandBuffer(),
                (uint32_t)numWorkGroupsX,
                (uint32_t)numWorkGroupsY,
                1);
}

bool HgiVulkanComputeCmds::_Submit(Hgi *hgi, HgiSubmitWaitType wait)
{
  if (!_commandBuffer) {
    return false;
  }
...
void HgiVulkanComputeCmds::_BindResources()
{
  if (!_pipelineLayout) {
    return;
  }
...
void HgiVulkanComputeCmds::MemoryBarrier(HgiMemoryBarrier barrier)
{
  _CreateCommandBuffer();
