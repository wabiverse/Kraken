...
#include "wabi/imaging/hgiVulkan/shaderProgram.h"
    ...
    _vkPipelineLayout(nullptr)
{
  ...
  TF_VERIFY(vkCreateComputePipelines(_device->GetVulkanDevice(),
...
void HgiVulkanComputePipeline::BindPipeline(VkCommandBuffer cb)
{
  vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_COMPUTE, _vkPipeline);
}

VkPipelineLayout HgiVulkanComputePipeline::GetVulkanPipelineLayout() const
{
  return _vkPipelineLayout;
}

HgiVulkanDevice *HgiVulkanComputePipeline::GetDevice() const
{
  return _device;
}

uint64_t &HgiVulkanComputePipeline::GetInflightBits()
{
