    ...
    switch (p_node->op) {
      ...
      case SpvOpTypeStruct: {
        ...
        for (; word_index < p_node->word_count; ++word_index, ++member_index) {
          ...
          result =
            ParseType(p_parser, p_member_node, p_member_decorations, p_module, p_member_type);
          if (result != SPV_REFLECT_RESULT_SUCCESS) {
            break;
          }
          // This looks wrong
                                                     ...
                                                     SpvReflectBlockVariable *p_var)
{
  ...
  if (IsNotNull(p_type->members) && (p_type->member_count > 0)) {
    ...
    for (uint32_t member_index = 0; member_index < p_type->member_count; ++member_index) {
      ...
      if (is_struct) {
        SpvReflectResult result = ParseDescriptorBlockVariable(p_parser,
                                                               p_module,
                                                               p_member_type,
                                                               p_member_var);
        if (result != SPV_REFLECT_RESULT_SUCCESS) {
    ...
    switch (p_member_type->op) {
      ...
      case SpvOpTypeArray: {
        ...
        bool is_struct = (p_member_type->type_flags & SPV_REFLECT_TYPE_FLAG_STRUCT) ==
                         SPV_REFLECT_TYPE_FLAG_STRUCT;
        if (is_struct) {
          SpvReflectResult result = ParseDescriptorBlockVariableSizes(p_parser,
                                                                      p_module,
                                                                      false,
                                                                      true,
                                                                      is_parent_rta,
                                          ...
                                          uint32_t depth)
{
  ...
  for (size_t i = 0; i < p_func->callee_count; ++i) {
    SpvReflectResult result =
      TraverseCallGraph(p_parser, p_func->callee_ptrs[i], p_func_count, p_func_ids, depth + 1);
    if (result != SPV_REFLECT_RESULT_SUCCESS) {
      return result;
    }
  }
...
static SpvReflectResult ParsePushConstantBlocks(Parser *p_parser, SpvReflectShaderModule *p_module)
{
  ...
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    ...
    result =
      ParseDescriptorBlockVariableSizes(p_parser, p_module, true, false, false, p_push_constant);
    if (result != SPV_REFLECT_RESULT_SUCCESS) {
      return result;
    }
