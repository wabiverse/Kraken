...
bool Hd_VertexAdjacency::BuildAdjacencyTable(HdMeshTopology const *topology)
{
  // compute adjacency
  ...
  HD_PERF_COUNTER_ADD(HdPerfTokens->adjacencyBufSize, numEntries * sizeof(int));
...
bool Hd_AdjacencyBuilderComputation::_CheckValid() const
{
  return true;
}
...
void Hd_AdjacencyBufferSource::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  specs->emplace_back(HdTokens->adjacency, HdTupleType{HdTypeInt32, 1});
}

bool Hd_AdjacencyBufferSource::_CheckValid() const
{
  return true;
}
