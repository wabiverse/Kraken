...
static bool _DirtyRprimIdsFilterPredicate(const SdfPath &rprimID, const void *predicateParam)
{
  ...
  if (mask == HdChangeTracker::Clean || bits & mask) {
    ...
    const TfToken &primRenderTag = renderIndex->UpdateRenderTag(rprimID, bits);
...
SdfPathVector const &HdDirtyList::GetDirtyRprims()
{
  HD_TRACE_FUNCTION();
  ...
  // any of its prims.
  const unsigned int currentSceneStateVersion = _GetChangeTracker().GetSceneStateVersion();
  ...
  if (_sceneStateVersion == currentSceneStateVersion && !_pruneDirtyList && !_rebuildDirtyList) {
    ...
    TF_DEBUG(HD_DIRTY_LIST)
      .Msg("DirtyList: Scene (%d) state version and filters unchanged.\n", _sceneStateVersion);
...
static std::ostream &operator<<(std::ostream &os, HdReprSelectorVector const &reprs)
{
  os << "[";
...
void HdDirtyList::UpdateRenderTagsAndReprSelectors(TfTokenVector const &tags,
                                                   HdReprSelectorVector const &reprs)
{
  ...
  bool trackedReprsChanged = false;
  {
    ...
    unsigned int currentRprimIndexVersion = _GetChangeTracker().GetRprimIndexVersion();
    unsigned int currentRprimRenderTagVersion = _GetChangeTracker().GetRenderTagVersion();
    if (trackedRenderTagsChanged || _rprimIndexVersion != currentRprimIndexVersion ||
      ...
      if (TfDebug::IsEnabled(HD_DIRTY_LIST)) {
        std::stringstream ss;
        ss << "Resetting tracked reprs in dirty list from " << _trackedReprs << " to " << reprs
           << "\n";
...
HdChangeTracker &HdDirtyList::_GetChangeTracker() const
{
  return _renderIndex.GetChangeTracker();
}

void HdDirtyList::_UpdateDirtyIdsIfNeeded()
{
  ...
  // GetDirtyRprims().
  const unsigned int currentRprimIndexVersion = _GetChangeTracker().GetRprimIndexVersion();
  const unsigned int currentRprimRenderTagVersion = _GetChangeTracker().GetRenderTagVersion();
  const unsigned int currentVaryingStateVersion = _GetChangeTracker().GetVaryingStateVersion();
  const bool gatherAllRprims = _rebuildDirtyList ||
                               _rprimIndexVersion != currentRprimIndexVersion ||
                               _rprimRenderTagVersion != currentRprimRenderTagVersion;

  const bool gatherVaryingRprims = _pruneDirtyList ||
  ...
  // just the varying ones.
  {
    if (gatherAllRprims) {
      ...
      TF_DEBUG(HD_DIRTY_LIST)
        .Msg(
          "DirtyList: varying state version "
          "(%d -> %d)\n",
  ...
  HdPrimGather gather;
  gather.PredicatedFilter(rprimPaths,
                          includePaths,
                          excludePaths,
                          _DirtyRprimIdsFilterPredicate,
                          &filterParam,
  ...
  if (mask == HdChangeTracker::Clean) {
    ...
    for (const SdfPath &dirtyRprimId : _dirtyIds) {
      _GetChangeTracker().MarkRprimDirty(dirtyRprimId, HdChangeTracker::InitRepr);
    }
  }
