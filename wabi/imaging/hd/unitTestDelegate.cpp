...
void HdUnitTestDelegate::SetVisibility(bool vis)
{
  _visibility = vis;
  TF_FOR_ALL (it, _meshes) {
    GetRenderIndex().GetChangeTracker().MarkRprimDirty(it->first,
...
void HdUnitTestDelegate::AddMesh(SdfPath const &id)
{
                                        ...
                                        SdfPath const &instancerId)
{
  ...
  if (!normals.empty()) {
    _primvars[id].emplace_back(_Primvar(HdTokens->normals,
                                        VtValue(normals),
...
void HdUnitTestDelegate::AddInstancer(SdfPath const &id,
...
void HdUnitTestDelegate::AddPrimvar(SdfPath const &id,
                                    TfToken const &name,
                                    VtValue const &value,
...
void HdUnitTestDelegate::UpdatePrimvarValue(SdfPath const &id,
                                            TfToken const &name,
                                            VtValue const &value,
                                            VtIntArray const &indices)
{
...
void HdUnitTestDelegate::RemovePrimvar(SdfPath const &id, TfToken const &name)
{
  _Primvars::iterator pvIt;
  if (_FindPrimvar(id, name, &pvIt)) {
...
void HdUnitTestDelegate::BindMaterial(SdfPath const &rprimId, SdfPath const &materialId)
{
  _materialBindings[rprimId] = materialId;
}

void HdUnitTestDelegate::RebindMaterial(SdfPath const &rprimId, SdfPath const &materialId)
{
...
void HdUnitTestDelegate::HideRprim(SdfPath const &id)
{
  _hiddenRprims.insert(id);
  MarkRprimDirty(id, HdChangeTracker::DirtyRenderTag);
}

void HdUnitTestDelegate::UnhideRprim(SdfPath const &id)
{
  _hiddenRprims.erase(id);
  MarkRprimDirty(id, HdChangeTracker::DirtyRenderTag);
}

void HdUnitTestDelegate::SetReprSelector(SdfPath const &id, HdReprSelector const &reprSelector)
{
...
void HdUnitTestDelegate::SetVisibility(SdfPath const &id, bool vis)
{
  _visibilities[id] = vis;
  GetRenderIndex().GetChangeTracker().MarkRprimDirty(id, HdChangeTracker::DirtyVisibility);
}
...
void HdUnitTestDelegate::UpdatePositions(SdfPath const &id, float time)
{
  if (_meshes.find(id) != _meshes.end()) {
...
void HdUnitTestDelegate::UpdateInstancerPrimvars(float time)
{
  // update instancers
  TF_FOR_ALL (it, _instancers) {
    for (size_t i = 0; i < it->second.rotate.size(); ++i) {
      ...
      GfVec4f quat(q.GetReal(), q.GetImaginary()[0], q.GetImaginary()[1], q.GetImaginary()[2]);
...
void HdUnitTestDelegate::UpdateInstancerPrototypes(float time)
{
...
void HdUnitTestDelegate::UpdateCamera(SdfPath const &id, TfToken const &key, VtValue value)
{
...
void HdUnitTestDelegate::UpdateTask(SdfPath const &id, TfToken const &key, VtValue value)
{
...
/*virtual*/
TfToken HdUnitTestDelegate::GetRenderTag(SdfPath const &id)
{
  HD_TRACE_FUNCTION();
...
HdMeshTopology HdUnitTestDelegate::GetMeshTopology(SdfPath const &id)
{
  ...
  return HdMeshTopology(mesh.scheme, mesh.orientation, mesh.numVerts, mesh.verts, mesh.holes);
}

/*virtual*/
HdBasisCurvesTopology HdUnitTestDelegate::GetBasisCurvesTopology(SdfPath const &id)
{
  HD_TRACE_FUNCTION();
...
/*virtual*/
PxOsdSubdivTags HdUnitTestDelegate::GetSubdivTags(SdfPath const &id)
{
  HD_TRACE_FUNCTION();
...
/*virtual*/
GfRange3d HdUnitTestDelegate::GetExtent(SdfPath const &id)
{
  HD_TRACE_FUNCTION();
...
/*virtual*/
bool HdUnitTestDelegate::GetDoubleSided(SdfPath const &id)
{
  if (_meshes.find(id) != _meshes.end()) {
...
/*virtual*/
HdDisplayStyle HdUnitTestDelegate::GetDisplayStyle(SdfPath const &id)
{
...
/*virtual*/
SdfPath HdUnitTestDelegate::GetMaterialId(SdfPath const &rprimId)
{
  SdfPath materialId;
...
/*virtual*/
SdfPath HdUnitTestDelegate::GetInstancerId(SdfPath const &primId)
{
  SdfPath instancerId;
...
/*virtual*/
VtValue HdUnitTestDelegate::GetMaterialResource(SdfPath const &materialId)
{
...
/*virtual*/
VtValue HdUnitTestDelegate::GetCameraParamValue(SdfPath const &cameraId, TfToken const &paramName)
{
...
/*virtual*/
GfMatrix4d HdUnitTestDelegate::GetTransform(SdfPath const &id)
{
  HD_TRACE_FUNCTION();
...
/*virtual*/
bool HdUnitTestDelegate::GetVisible(SdfPath const &id)
{
  HD_TRACE_FUNCTION();
...
/*virtual*/
VtValue HdUnitTestDelegate::Get(SdfPath const &id, TfToken const &key)
{
  HD_TRACE_FUNCTION();
...
/* virtual */
HdReprSelector HdUnitTestDelegate::GetReprSelector(SdfPath const &id)
{
  HD_TRACE_FUNCTION();
...
/* virtual */
HdPrimvarDescriptorVector HdUnitTestDelegate::GetPrimvarDescriptors(SdfPath const &id,
  ...
  if (cit != _primvars.end()) {
    ...
    for (auto const &pv : pvs) {
      if (pv.interp == interpolation) {
        primvars.emplace_back(pv.name, pv.interp, pv.role, !pv.indices.empty());
      }
    }
  }
...
void HdUnitTestDelegate::AddCube(SdfPath const &id,
                                 GfMatrix4f const &transform,
                                 bool guide,
                                 SdfPath const &instancerId,
                                 TfToken const &scheme)
...
void HdUnitTestDelegate::AddPolygons(SdfPath const &id,
                                     GfMatrix4f const &transform,
...
void HdUnitTestDelegate::AddFaceVaryingPolygons(SdfPath const &id,
                                                GfMatrix4f const &transform,
                                                SdfPath const &instancerId)
{
  int numVerts[] = {3, 4, 5};
  ...
  VtVec3fArray colorArray = {GfVec3f(1, 0, 0),
                             GfVec3f(1, 0.3, 0),
                             GfVec3f(1, 1, 0),
                             GfVec3f(0, 1, 0),
                             GfVec3f(0, 1, 1),
...
void HdUnitTestDelegate::AddGrid(SdfPath const &id,
                                 int nx,
                                 int ny,
                                 GfMatrix4f const &transform,
                                 bool rightHanded,
...
void HdUnitTestDelegate::AddGridWithCustomColor(SdfPath const &id,
                                                int nx,
                                                int ny,
                                                GfMatrix4f const &transform,
                                                VtValue const &color,
...
void HdUnitTestDelegate::AddGridWithFaceColor(SdfPath const &id,
                                              int nx,
                                              int ny,
                                              GfMatrix4f const &transform,
                                              bool rightHanded,
...
void HdUnitTestDelegate::AddGridWithVertexColor(SdfPath const &id,
                                                int nx,
                                                int ny,
                                                GfMatrix4f const &transform,
                                                bool rightHanded,
...
void HdUnitTestDelegate::AddGridWithFaceVaryingColor(SdfPath const &id,
                                                     int nx,
                                                     int ny,
                                                     GfMatrix4f const &transform,
                                                     bool rightHanded,
...
void HdUnitTestDelegate::AddCurves(SdfPath const &id,
                                   TfToken const &type,
                                   TfToken const &basis,
                                   GfMatrix4f const &transform,
                                   HdInterpolation colorInterp,
                                   ...
                                   SdfPath const &instancerId)
{
  ...
  if (authoredNormals) {
    GfVec3f normals[] = {GfVec3f(.0f, -.7f, .7f),
                         GfVec3f(.0f, .0f, 1.0f),
                         GfVec3f(.0f, .7f, .7f),
                         GfVec3f(.7f, .7f, .0f),
                         GfVec3f(.0f, .0f, 1.0f),
...
void HdUnitTestDelegate::Remove(SdfPath const &id)
{
  GetRenderIndex().RemoveRprim(id);
}

void HdUnitTestDelegate::Clear()
{
  GetRenderIndex().Clear();
}

void HdUnitTestDelegate::MarkRprimDirty(SdfPath path, HdDirtyBits flag)
{
  GetRenderIndex().GetChangeTracker().MarkRprimDirty(path, flag);
}

GfVec3f HdUnitTestDelegate::PopulateBasicTestSet()
{
  GfMatrix4d dmat;
  double xPos = 0.0;
  ...
  {
    dmat.SetTranslate(GfVec3d(xPos, -3.0, 0.0));
    AddPolygons(SdfPath("/nonquads1"), GfMatrix4f(dmat), HdInterpolationConstant);

    dmat.SetTranslate(GfVec3d(xPos, 0.0, 0.0));
    AddPolygons(SdfPath("/nonquads2"), GfMatrix4f(dmat), HdInterpolationUniform);

    dmat.SetTranslate(GfVec3d(xPos, 3.0, 0.0));
    AddPolygons(SdfPath("/nonquads3"), GfMatrix4f(dmat), HdInterpolationVertex);

    dmat.SetTranslate(GfVec3d(xPos, 6.0, 0.0));
    AddPolygons(SdfPath("/nonquads4"), GfMatrix4f(dmat), HdInterpolationFaceVarying);
  ...
  // more grids (backface, single sided)
  {
    ...
    AddGridWithFaceColor(SdfPath("/grid5"), 10, 10, GfMatrix4f(dmat));

    // inverse Z
    dmat.SetScale(GfVec3d(1, 1, -1));
    dmat.SetTranslateOnly(GfVec3d(xPos, 3.0, 0.0));
    AddGridWithVertexColor(SdfPath("/grid6"), 10, 10, GfMatrix4f(dmat));

    // left-handed
    dmat.SetTranslate(GfVec3d(xPos, 6.0, 0.0));
    AddGridWithFaceVaryingColor(SdfPath("/grid7"),
                                3,
                                3,
                                GfMatrix4f(dmat),
                                /*rightHanded=*/false);
  ...
  // more grids (backface, double sided)
  {
    ...
    dmat.SetScale(GfVec3d(-1, 1, 1));
    dmat.SetTranslateOnly(GfVec3d(xPos, 0.0, 0.0));
    AddGridWithFaceColor(SdfPath("/grid9"),
                         10,
                         10,
                         GfMatrix4f(dmat),
                         /*rightHanded=*/true,
    ...
    dmat.SetScale(GfVec3d(1, 1, -1));
    dmat.SetTranslateOnly(GfVec3d(xPos, 3.0, 0.0));
    AddGridWithVertexColor(SdfPath("/grid10"),
                           10,
                           10,
                           GfMatrix4f(dmat),
                           /*rightHanded=*/true,
    ...
    // left-handed
    dmat.SetTranslate(GfVec3d(xPos, 6.0, 0.0));
    AddGridWithFaceVaryingColor(SdfPath("/grid11"),
                                3,
                                3,
                                GfMatrix4f(dmat),
                                /*righthanded=*/false,
  ...
  {
    dmat.SetTranslate(GfVec3d(xPos, -3.0, 0.0));
    AddPoints(SdfPath("/points1"),
              GfMatrix4f(dmat),
              HdInterpolationConstant,
              HdInterpolationConstant);

    dmat.SetTranslate(GfVec3d(xPos, 0.0, 0.0));
    AddPoints(SdfPath("/points2"),
              GfMatrix4f(dmat),
              HdInterpolationVertex,
              HdInterpolationConstant);
...
GfVec3f HdUnitTestDelegate::PopulateInvalidPrimsSet()
{
  // empty mesh
...
VtValue HdUnitTestDelegate::_GetPrimvarValue(SdfPath const &id, TfToken const &name)
{
  _Primvars::iterator pvIt;
  if (_FindPrimvar(id, name, &pvIt)) {
    return pvIt->value;
                                      ...
                                      _Primvars::iterator *pvIt)
{
  ...
  HdUnitTestDelegate::_Primvars::iterator it2 = std::find_if(primvars.begin(),
                                                             primvars.end(),
                                                             [&name](const _Primvar &x) {
                                                               return x.name == name;
                                                             });
