...
template<typename T>
static bool _FanTriangulate(T *dst, T const *src, int offset, int index, int size, bool flip)
{
  // overrun check
...
static bool _FanTriangulate(GfVec3i *dst,
                            int const *src,
                            int offset,
                            int index,
                            int size,
...
void HdMeshUtil::ComputeTriangleIndices(VtVec3iArray *indices,
...
template<typename T>
static void _TriangulateFaceVarying(SdfPath const &id,
  ...
  for (int i = 0, v = 0; i < (int)faceVertexCounts.size(); ++i) {
    ...
    if (nVerts < 3) {
      ...
      for (int j = 0; j < nVerts - 2; ++j) {
        if (!_FanTriangulate(&results[dstIndex], source, v, j, numElements, flip)) {
          invalidTopology = true;
        }
...
bool HdMeshUtil::ComputeTriangulatedFaceVaryingPrimvar(void const *source,
                                                       int numElements,
                                                       HdType dataType,
                                                       VtValue *triangulated)
{
  ...
  VtIntArray const &holeFaces = (_topology->GetRefineLevel() > 0) ? VtIntArray() :
...
/*static*/ int HdMeshUtil::ComputeNumQuads(VtIntArray const &numVerts,
                                           VtIntArray const &holeFaces,
...
void HdMeshUtil::ComputeQuadInfo(HdQuadInfo *quadInfo)
{
  HD_TRACE_FUNCTION();
  ...
  for (int i = 0; i < numFaces; ++i) {
    ...
    if (holeIndex < numHoleFaces && holeFacesPtr[holeIndex] == i) {
      // skip hole faces.
      vertIndex += nv;
      ++holeIndex;
      continue;
                                    ...
                                    VtVec4iArray *quadsEdgeIndices /*=nullptr*/)
{
  ...
  for (int i = 0, qv = 0, v = 0; i < numFaces; ++i) {
    ...
    if (nv == 4) {
      ...
      (*primitiveParams)[qv] = GfVec2i(EncodeCoarseFaceParam(i, /*edgeFlag=*/0), qv);
      ...
      for (int j = 0; j < nv; ++j) {
        ...
        (*primitiveParams)[qv] = GfVec2i(EncodeCoarseFaceParam(i, edgeFlag), qv);
...
template<typename T>
static void _Quadrangulate(SdfPath const &id,
                           void const *sourceUntyped,
                           int numElements,
                           HdQuadInfo const *qi,
...
bool HdMeshUtil::ComputeQuadrangulatedPrimvar(HdQuadInfo const *qi,
                                              void const *source,
                                              int numElements,
                                              HdType dataType,
                                              VtValue *quadrangulated)
{
  ...
  switch (dataType) {
    ...
    case HdTypeDoubleVec4:
      ...
      break;
    default:
      TF_CODING_ERROR("Unsupported points type for quadrangulation [%s]", _id.GetText());
      return false;
  }
...
template<typename T>
static void _QuadrangulateFaceVarying(SdfPath const &id,
                                      VtIntArray const &faceVertexCounts,
                                      VtIntArray const &holeFaces,
                                      bool flip,
                                      void const *sourceUntyped,
  ...
  for (int i = 0, v = 0; i < (int)faceVertexCounts.size(); ++i) {
    ...
    if (nVerts < 3) {
      ...
      for (int j = 0; j < nVerts; ++j) {
        results[dstIndex++] = source[v + j];
        // mid edge
        results[dstIndex++] = (source[v + j] + source[v + (j + 1) % nVerts]) * 0.5;
        // center
        results[dstIndex++] = center;
        // mid edge
        results[dstIndex++] = (source[v + j] + source[v + (j + nVerts - 1) % nVerts]) * 0.5;
      }
    }
    v += nVerts;
  }
...
bool HdMeshUtil::ComputeQuadrangulatedFaceVaryingPrimvar(void const *source,
                                                         int numElements,
                                                         HdType dataType,
                                                         VtValue *quadrangulated)
{
...
/*static*/ HdMeshUtil::EdgeMap HdMeshUtil::ComputeAuthoredEdgeMap(HdMeshTopology const *topology,
                                                                  bool skipHoles /*=false*/)
{
  ...
  for (int i = 0, v = 0; i < numFaces; ++i) {
    ...
    else if (skipHoles && holeIndex < numHoleFaces && holeFacesPtr[holeIndex] == i) {
      // Skip hole faces.
      ++holeIndex;
    }
    else {
...
/*static*/ HdMeshUtil::ReverseEdgeMap HdMeshUtil::ComputeReverseEdgeMap(const EdgeMap &edgeMap)
{
  HD_TRACE_FUNCTION();
  ReverseEdgeMap res;
...
/*static*/ std::pair<bool, GfVec2i> HdMeshUtil::GetVertexIndicesForEdge(
...
/*static*/ std::pair<bool, int> HdMeshUtil::GetAuthoredEdgeID(HdMeshTopology const *topology,
                                                              GfVec2i edge)
{
