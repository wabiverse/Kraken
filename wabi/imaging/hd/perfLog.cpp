...
{
  static bool isEnabledPerfLog = TfGetenv("HD_ENABLE_PERFLOG", "0") == "1";
...
void HdPerfLog::AddCacheHit(TfToken const &name, SdfPath const &id, TfToken const &tag)
{
...
size_t HdPerfLog::GetCacheHits(TfToken const &name)
{
  _Lock lock(_mutex);
  if (HdPerfLog::_CacheEntry *value = TfMapLookupPtr(_cacheMap, name))
...
size_t HdPerfLog::GetCacheMisses(TfToken const &name)
{
  _Lock lock(_mutex);
  if (HdPerfLog::_CacheEntry *value = TfMapLookupPtr(_cacheMap, name))
...
TfTokenVector HdPerfLog::GetCacheNames()
{
  _Lock lock(_mutex);
  TfTokenVector names;
...
TfTokenVector HdPerfLog::GetCounterNames()
{
  _Lock lock(_mutex);
  TfTokenVector names;
...
void HdPerfLog::IncrementCounter(TfToken const &name)
{
  if (ARCH_LIKELY(!_enabled))
    return;
  _Lock lock(_mutex);
...
void HdPerfLog::DecrementCounter(TfToken const &name)
{
  if (ARCH_LIKELY(!_enabled))
    return;
  _Lock lock(_mutex);
...
void HdPerfLog::SetCounter(TfToken const &name, double value)
{
  if (ARCH_LIKELY(!_enabled))
    return;
  _Lock lock(_mutex);
...
void HdPerfLog::AddCounter(TfToken const &name, double value)
{
  if (ARCH_LIKELY(!_enabled))
    return;
  _Lock lock(_mutex);
...
void HdPerfLog::SubtractCounter(TfToken const &name, double value)
{
  if (ARCH_LIKELY(!_enabled))
    return;
  _Lock lock(_mutex);
...
double HdPerfLog::GetCounter(TfToken const &name)
{
  _Lock lock(_mutex);
  return TfMapLookupByValue(_counterMap, name, 0.0);
}

void HdPerfLog::ResetCounters()
{
  if (ARCH_LIKELY(!_enabled))
    return;
  _Lock lock(_mutex);
...
void HdPerfLog::AddResourceRegistry(HdResourceRegistry *const resourceRegistry)
{
