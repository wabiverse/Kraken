...
bool HdBasisCurvesTopology::operator==(HdBasisCurvesTopology const &other) const
{
  HD_TRACE_FUNCTION();

  // no need to compare _adajency and _quadInfo
  return (_curveType == other._curveType && _curveBasis == other._curveBasis &&
          _curveWrap == other._curveWrap && _curveVertexCounts == other._curveVertexCounts &&
          _curveIndices == other._curveIndices && _invisiblePoints == other._invisiblePoints &&
...
HdTopology::ID HdBasisCurvesTopology::ComputeHash() const
{
  HD_TRACE_FUNCTION();
...
size_t HdBasisCurvesTopology::CalculateNeededNumberOfControlPoints() const
{
  size_t numVerts = 0;
