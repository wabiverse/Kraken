...
GfMatrix4d HdCamera::GetProjectionMatrix() const
{
  ...
  cam.SetProjection(GetProjection() == HdCamera::Orthographic ? GfCamera::Orthographic :
                                                                GfCamera::Perspective);
  cam.SetHorizontalAperture(GetHorizontalAperture() / GfCamera::APERTURE_UNIT);
  cam.SetVerticalAperture(GetVerticalAperture() / GfCamera::APERTURE_UNIT);
  cam.SetHorizontalApertureOffset(GetHorizontalApertureOffset() / GfCamera::APERTURE_UNIT);
  cam.SetVerticalApertureOffset(GetVerticalApertureOffset() / GfCamera::APERTURE_UNIT);
  cam.SetFocalLength(GetFocalLength() / GfCamera::FOCAL_LENGTH_UNIT);
  cam.SetClippingRange(GetClippingRange());

  return cam.GetFrustum().ComputeProjectionMatrix();
}

/* virtual */
HdDirtyBits HdCamera::GetInitialDirtyBitsMask() const
{
  return AllDirty;
}
