...
class HdVtBufferSource final : public HdBufferSource {
 public:
  ...
  HD_API
  HdVtBufferSource(TfToken const &name, VtValue const &value, int arraySize = 1);
  ...
  /// \param arraySize indicates how many values are provided per element.
  HD_API
  HdVtBufferSource(TfToken const &name, VtArray<GfMatrix4d> const &matrices, int arraySize = 1);
...
#endif  // WABI_IMAGING_HD_VT_BUFFER_SOURCE_H
