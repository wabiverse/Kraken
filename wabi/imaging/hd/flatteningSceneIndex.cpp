...
HdFlatteningSceneIndex::HdFlatteningSceneIndex(const HdSceneIndexBaseRefPtr &inputScene)
...
HdSceneIndexPrim HdFlatteningSceneIndex::GetPrim(const SdfPath &primPath) const
{
...
SdfPathVector HdFlatteningSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
...
void HdFlatteningSceneIndex::_PrimsAdded(const HdSceneIndexBase &sender,
                                         const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
    ...
    auto iterBoolPair = _prims.insert({entry.primPath, {HdSceneIndexPrim()}});
    ...
    if (prim.dataSource) {
      _PrimLevelWrappingDataSource::Cast(prim.dataSource)->UpdateInputDataSource(dataSource);
    } else {
      prim.dataSource = _PrimLevelWrappingDataSource::New(*this, entry.primPath, dataSource);
    }
...
void HdFlatteningSceneIndex::_PrimsRemoved(const HdSceneIndexBase &sender,
...
void HdFlatteningSceneIndex::_PrimsDirtied(const HdSceneIndexBase &sender,
                                           const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : entries) {
    ...
    if (entry.dirtyLocators.Intersects(HdPurposeSchema::GetDefaultLocator())) {
...
void HdFlatteningSceneIndex::_DirtyHierarchy(
  ...
  HdSceneIndexObserver::DirtiedPrimEntries *dirtyEntries)
{
  ...
  for (; it != startEndIt.second;) {
    ...
    if (_PrimLevelWrappingDataSourceHandle dataSource = _PrimLevelWrappingDataSource::Cast(
...
HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::_PrimLevelWrappingDataSource(
...
void HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::UpdateInputDataSource(
  HdContainerDataSourceHandle inputDataSource)
{
  _inputDataSource = inputDataSource;
}

bool HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::PrimDirtied(
...
bool HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::Has(const TfToken &name)
{
...
TfTokenVector HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::GetNames()
{
...
HdDataSourceBaseHandle HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::Get(
  const TfToken &name)
{
  if (name == HdXformSchemaTokens->xform) {
    return _GetXform();
...
HdDataSourceBaseHandle HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::_GetPurpose()
{
  HdContainerDataSourceHandle computedPurposeDataSource = HdContainerDataSource::AtomicLoad(
    _computedPurposeDataSource);

  if (computedPurposeDataSource) {
    return computedPurposeDataSource;
  }

  HdPurposeSchema inputPurpose = HdPurposeSchema::GetFromParent(_inputDataSource);
  ...
  if (inputPurpose) {
    ...
    if (_primPath.GetPathElementCount()) {
      ...
      if (it != _sceneIndex._prims.end()) {
        parentPurpose = HdPurposeSchema::GetFromParent(it->second.prim.dataSource);
      }
    }
  ...
  HdContainerDataSource::AtomicStore(_computedPurposeDataSource, computedPurposeDataSource);

  return computedPurposeDataSource;
}

HdDataSourceBaseHandle HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::_GetVis()
{
  HdContainerDataSourceHandle computedVisDataSource = HdContainerDataSource::AtomicLoad(
    _computedVisDataSource);

  if (computedVisDataSource) {
    return computedVisDataSource;
  }

  HdVisibilitySchema inputVis = HdVisibilitySchema::GetFromParent(_inputDataSource);
  ...
  if (inputVis) {
    ...
    if (_primPath.GetPathElementCount()) {
      ...
      const auto it = _sceneIndex._prims.find(parentPath);
      if (it != _sceneIndex._prims.end()) {
        parentVis = HdVisibilitySchema::GetFromParent(it->second.prim.dataSource);
      }
    }
  ...
  HdContainerDataSource::AtomicStore(_computedVisDataSource, computedVisDataSource);

  return computedVisDataSource;
}

HdDataSourceBaseHandle HdFlatteningSceneIndex::_PrimLevelWrappingDataSource::_GetXform()
{
  HdContainerDataSourceHandle computedXformDataSource = HdContainerDataSource::AtomicLoad(
    _computedXformDataSource);
  ...
  HdXformSchema inputXform = HdXformSchema::GetFromParent(_inputDataSource);

  // If this xform is fully composed, early out.
  if (inputXform) {
    HdBoolDataSourceHandle resetXformStack = inputXform.GetResetXformStack();
    if (resetXformStack && resetXformStack->GetTypedValue(0.0f)) {
      ...
      HdContainerDataSource::AtomicStore(_computedXformDataSource, computedXformDataSource);
  ...
  if (_primPath.GetPathElementCount()) {
    ...
    const auto it = _sceneIndex._prims.find(parentPath);
    if (it != _sceneIndex._prims.end()) {
      parentXform = HdXformSchema::GetFromParent(it->second.prim.dataSource);
    }
  }
  ...
  HdMatrixDataSourceHandle parentMatrixDataSource = parentXform ? parentXform.GetMatrix() :
                                                                  nullptr;
  HdMatrixDataSourceHandle inputMatrixDataSource = inputXform ? inputXform.GetMatrix() : nullptr;

  if (inputMatrixDataSource && parentMatrixDataSource) {
    GfMatrix4d parentMatrix = parentMatrixDataSource->GetTypedValue(0.0f);
    GfMatrix4d inputMatrix = inputMatrixDataSource->GetTypedValue(0.0f);
