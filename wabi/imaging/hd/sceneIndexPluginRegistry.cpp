...
#include "wabi/imaging/hd/sceneIndexPlugin.h"
...
HdSceneIndexPluginRegistry &HdSceneIndexPluginRegistry::GetInstance()
{
...
HdSceneIndexPlugin *HdSceneIndexPluginRegistry::_GetSceneIndexPlugin(const TfToken &pluginId)
{
  return static_cast<HdSceneIndexPlugin *>(GetPlugin(pluginId));
}

HdSceneIndexBaseRefPtr HdSceneIndexPluginRegistry::AppendSceneIndex(
  const TfToken &sceneIndexPluginId,
  const HdSceneIndexBaseRefPtr &inputScene,
  const HdContainerDataSourceHandle &inputArgs)
{
  if (HdSceneIndexPlugin *plugin = _GetSceneIndexPlugin(sceneIndexPluginId)) {
    HdSceneIndexBaseRefPtr result = plugin->AppendSceneIndex(inputScene, inputArgs);
...
HdSceneIndexBaseRefPtr HdSceneIndexPluginRegistry::_AppendForPhases(
  const HdSceneIndexBaseRefPtr &inputScene,
  ...
  const HdContainerDataSourceHandle &argsUnderlay)
{
  ...
  for (const auto &phasesPair : phasesMap) {
    for (const _Entry &entry : phasesPair.second) {
      ...
      result = AppendSceneIndex(entry.sceneIndexPluginId, result, args);
    }
  }
  return result;
}
...
{
  const JsValue &loadWithRendererValue = plugRegistry.GetDataFromPluginMetaData(
    pluginType,
    "loadWithRenderer");
  ...
  if (loadWithRendererValue.GetType() == JsValue::StringType) {
    ...
    for (const std::string &s : loadWithRendererValue.GetArrayOf<std::string>()) {
      _preloadsForRenderer[s].push_back(TfToken(pluginType.GetTypeName().c_str()));
    }
  }
}

HdSceneIndexBaseRefPtr HdSceneIndexPluginRegistry::AppendSceneIndicesForRenderer(
  ...
  const HdSceneIndexBaseRefPtr &inputScene)
{
  ...
  HdContainerDataSourceHandle underlayArgs = HdRetainedContainerDataSource::New(
    HdSceneIndexPluginRegistryTokens->rendererDisplayName,
    HdRetainedTypedSampledDataSource<std::string>::New(rendererDisplayName));
...
void HdSceneIndexPluginRegistry::RegisterSceneIndexForRenderer(
  const std::string &rendererDisplayName,
  const TfToken &sceneIndexPluginId,
  const HdContainerDataSourceHandle &inputArgs,
...
{
  _EntryList &entryList = _sceneIndicesForRenderers[rendererDisplayName][insertionPhase];
