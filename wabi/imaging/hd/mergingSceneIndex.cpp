...
HdMergingSceneIndex::HdMergingSceneIndex() : _observer(this) {}

void HdMergingSceneIndex::AddInputScene(const HdSceneIndexBaseRefPtr &inputScene,
...
void HdMergingSceneIndex::RemoveInputScene(const HdSceneIndexBaseRefPtr &sceneIndex)
{
  for (_InputEntries::iterator it = _inputs.begin(); it != _inputs.end(); ++it) {
    if (sceneIndex == it->sceneIndex) {
      ...
      while (!removalTestQueue.empty()) {
        ...
        if (!prim.dataSource && GetChildPrimPaths(path).empty()) {
...
std::vector<HdSceneIndexBaseRefPtr> HdMergingSceneIndex::GetInputScenes() const
{
...
HdSceneIndexPrim HdMergingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  TRACE_FUNCTION();
  ...
  switch (contributingDataSources.size()) {
    ...
    case 1:
      ...
      break;
    default:
      result.dataSource = HdOverlayContainerDataSource::New(contributingDataSources.size(),
                                                            contributingDataSources.data());
  };

  return result;
}

SdfPathVector HdMergingSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
  TRACE_FUNCTION();

  TfDenseHashSet<SdfPath, SdfPath::Hash, std::equal_to<SdfPath>, 32> childPaths;

  for (const _InputEntry &entry : _inputs) {
    if (primPath.HasPrefix(entry.sceneRoot)) {

      for (const SdfPath &childPath : entry.sceneIndex->GetChildPrimPaths(primPath)) {
...
void HdMergingSceneIndex::_PrimsAdded(const HdSceneIndexBase &sender,
...
void HdMergingSceneIndex::_PrimsRemoved(const HdSceneIndexBase &sender,
  ...
  for (const HdSceneIndexObserver::RemovedPrimEntry &entry : entries) {
    ...
    for (const _InputEntry &inputEntry : _inputs) {
      ...
      if (inputEntry.sceneIndex->GetPrim(entry.primPath).dataSource ||
          !inputEntry.sceneIndex->GetChildPrimPaths(entry.primPath).empty()) {
    ...
    if (primFullyRemoved) {
      ...
      addedEntries.emplace_back(entry.primPath, GetPrim(entry.primPath).primType);
    }
  }
...
void HdMergingSceneIndex::_PrimsDirtied(const HdSceneIndexBase &sender,
...
void HdMergingSceneIndex::_Observer::PrimsAdded(const HdSceneIndexBase &sender,
                                                const AddedPrimEntries &entries)
{
  _owner->_PrimsAdded(sender, entries);
}

void HdMergingSceneIndex::_Observer::PrimsRemoved(const HdSceneIndexBase &sender,
                                                  const RemovedPrimEntries &entries)
{
  _owner->_PrimsRemoved(sender, entries);
}

void HdMergingSceneIndex::_Observer::PrimsDirtied(const HdSceneIndexBase &sender,
