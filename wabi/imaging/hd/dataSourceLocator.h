...
//
#ifndef WABI_IMAGING_HD_DATASOURCELOCATOR_H
#define WABI_IMAGING_HD_DATASOURCELOCATOR_H
...
class HdDataSourceLocator
{
 public:
  ...
  HD_API
  HdDataSourceLocator(const TfToken &t1, const TfToken &t2, const TfToken &t3, const TfToken &t4);
  HD_API
  HdDataSourceLocator(const TfToken &t1,
                      const TfToken &t2,
                      const TfToken &t3,
                      const TfToken &t4,
                      const TfToken &t5);
  ...
  ///
  HD_API
  HdDataSourceLocator ReplacePrefix(const HdDataSourceLocator &oldPrefix,
...
inline size_t HdDataSourceLocator::Hash() const
{
  return TfHash()(*this);
}
...
class HdDataSourceLocatorSet
{
 ...
 public:
  ...
  HD_API
  HdDataSourceLocatorSet(const std::initializer_list<const HdDataSourceLocator> &l);
 ...
 private:
  ...
  void _InsertAndDeleteSuffixes(_Locators::iterator *position, const HdDataSourceLocator &locator);
...
#endif  // WABI_IMAGING_HD_DATASOURCELOCATOR_H
