...
HdDataSourceBaseHandle HdContainerDataSource::Get(const HdContainerDataSourceHandle &container,
...
void HdDebugPrintDataSource(std::ostream &s, HdDataSourceBaseHandle dataSource, int level)
{
  if (HdContainerDataSourceHandle handle = HdContainerDataSource::Cast(dataSource)) {
  ...
  } else if (HdVectorDataSourceHandle handle = HdVectorDataSource::Cast(dataSource)) {
    size_t nElements = handle->GetNumElements();
    for (size_t i = 0; i < nElements; ++i) {
      s << std::string(level, '\t') << "[" << i << "]\n";
      HdDebugPrintDataSource(s, handle->GetElement(i), level + 1);
    }
  } else if (HdSampledDataSourceHandle handle = HdSampledDataSource::Cast(dataSource)) {
...
void HdDebugPrintDataSource(HdDataSourceBaseHandle dataSource, int level)
{
