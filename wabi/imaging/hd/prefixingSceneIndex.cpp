...
#include "wabi/imaging/hd/dataSourceTypeDefs.h"
...
namespace
{
  ...
  class Hd_PrefixingSceneIndexPathDataSource : public HdTypedSampledDataSource<SdfPath>
  {
   public:

    HD_DECLARE_DATASOURCE(Hd_PrefixingSceneIndexPathDataSource)

    Hd_PrefixingSceneIndexPathDataSource(const SdfPath &prefix,
    ...
    {
      if (_inputDataSource) {
        return _inputDataSource->GetContributingSampleTimesForInterval(startTime,
                                                                       endTime,
                                                                       outSampleTimes);
      }
  ...
  class Hd_PrefixingSceneIndexContainerDataSource : public HdContainerDataSource
  {
   public:
    ...
    Hd_PrefixingSceneIndexContainerDataSource(const SdfPath &prefix,
    ...
    HdDataSourceBaseHandle Get(const TfToken &name) override
    {
      ...
      if (HdDataSourceBaseHandle childSource = _inputDataSource->Get(name)) {

        if (auto childContainer = HdContainerDataSource::Cast(childSource)) {
          return New(_prefix, std::move(childContainer));
        }

        if (auto childPathDataSource = HdTypedSampledDataSource<SdfPath>::Cast(childSource)) {

          return Hd_PrefixingSceneIndexPathDataSource::New(_prefix, childPathDataSource);
        }
...
HdPrefixingSceneIndex::HdPrefixingSceneIndex(const HdSceneIndexBaseRefPtr &inputScene,
                                             const SdfPath &prefix)
  : HdSingleInputFilteringSceneIndexBase(inputScene),
    _prefix(prefix)
{}

HdSceneIndexPrim HdPrefixingSceneIndex::GetPrim(const SdfPath &primPath) const
{
  if (!primPath.HasPrefix(_prefix)) {
    return {TfToken(), nullptr};
  }

  HdSceneIndexPrim prim = _GetInputSceneIndex()->GetPrim(_RemovePathPrefix(primPath));

  if (prim.dataSource) {
    prim.dataSource = Hd_PrefixingSceneIndexContainerDataSource::New(_prefix, prim.dataSource);
  }

  return prim;
}

SdfPathVector HdPrefixingSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
  // In the case that primPath has our prefix, we just strip out that
  // prefix and let the input scene index handle it.
  if (primPath.HasPrefix(_prefix)) {
    SdfPathVector result = _GetInputSceneIndex()->GetChildPrimPaths(_RemovePathPrefix(primPath));

    for (SdfPath &path : result) {
      path = _prefix.AppendPath(path.MakeRelativePath(SdfPath::AbsoluteRootPath()));
    }
...
void HdPrefixingSceneIndex::_PrimsAdded(const HdSceneIndexBase &sender,
                                        const HdSceneIndexObserver::AddedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
    prefixedEntries.emplace_back(_AddPathPrefix(entry.primPath), entry.primType);
  }

  _SendPrimsAdded(prefixedEntries);
}

void HdPrefixingSceneIndex::_PrimsRemoved(const HdSceneIndexBase &sender,
...
void HdPrefixingSceneIndex::_PrimsDirtied(const HdSceneIndexBase &sender,
                                          const HdSceneIndexObserver::DirtiedPrimEntries &entries)
{
  ...
  for (const HdSceneIndexObserver::DirtiedPrimEntry &entry : entries) {
    prefixedEntries.emplace_back(_AddPathPrefix(entry.primPath), entry.dirtyLocators);
  }

  _SendPrimsDirtied(prefixedEntries);
}

inline SdfPath HdPrefixingSceneIndex::_AddPathPrefix(const SdfPath &primPath) const
{
  return primPath.ReplacePrefix(SdfPath::AbsoluteRootPath(), _prefix);
}

inline SdfPath HdPrefixingSceneIndex::_RemovePathPrefix(const SdfPath &primPath) const
{
