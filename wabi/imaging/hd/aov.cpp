...
std::ostream &operator<<(std::ostream &out, const HdRenderPassAovBinding &desc)
{
...
bool operator==(const HdRenderPassAovBinding &lhs, const HdRenderPassAovBinding &rhs)
{
  return lhs.aovName == rhs.aovName && lhs.renderBuffer == rhs.renderBuffer &&
         lhs.renderBufferId == rhs.renderBufferId && lhs.clearValue == rhs.clearValue &&
...
bool HdAovHasDepthStencilSemantic(TfToken const &aovName)
{
  ...
  return TfStringEndsWith(TfStringToLower(aovName.GetString()),
