...
class HdPerfLog
{
 public:
  ...
  HD_API
  void AddCacheHit(TfToken const &name, SdfPath const &id, TfToken const &tag = TfToken());

  /// Tracks a cache miss for the named cache, the id and tag are reported
  /// when debug logging is enabled.
  HD_API
  void AddCacheMiss(TfToken const &name, SdfPath const &id, TfToken const &tag = TfToken());
...
#endif  // WABI_IMAGING_HD_PERF_LOG_H
