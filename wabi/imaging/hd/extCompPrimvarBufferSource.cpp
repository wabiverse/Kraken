...
TfToken const &HdExtCompPrimvarBufferSource::GetName() const
{
  return _primvarName;
}

void HdExtCompPrimvarBufferSource::GetBufferSpecs(HdBufferSpecVector *specs) const
{
...
template<class HashState> void TfHashAppend(HashState &h, HdExtCompPrimvarBufferSource const &bs)
{
  ...
  h.Append(bs._source->GetName(), bs._primvarName);
}

size_t HdExtCompPrimvarBufferSource::ComputeHash() const
{
  return TfHash()(*this);
}
...
void const *HdExtCompPrimvarBufferSource::GetData() const
{
  return _rawDataPtr;
}

HdTupleType HdExtCompPrimvarBufferSource::GetTupleType() const
{
  return _tupleType;
}

size_t HdExtCompPrimvarBufferSource::GetNumElements() const
{
  return _source->GetNumElements();
}

bool HdExtCompPrimvarBufferSource::_CheckValid() const
{
  return (_source && (_sourceOutputIdx != HdExtCompCpuComputation::INVALID_OUTPUT_INDEX) &&
