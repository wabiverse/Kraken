...
//
#ifndef WABI_IMAGING_HD_EXT_COMPUTATION_UTILS_H
#define WABI_IMAGING_HD_EXT_COMPUTATION_UTILS_H
...
class HdExtComputationUtils
{
 ...
 private:
  ...
  HD_API
  static void _LimitTimeSamples(size_t maxSampleCount, std::vector<double> *times);
  ...
  HD_API
  static bool _InvokeComputation(HdSceneDelegate &sceneDelegate,
  ...
  SampledValueStore<CAPACITY> *computedPrimvarValueStore)
{
  ...
  ComputationDependencyMap cdm = _GenerateDependencyMap(compPrimvars, sceneDelegate);
  ...
  // Execution
  SampledValueStore<CAPACITY> valueStore;
  _ExecuteSampledComputations<CAPACITY>(sortedComputations,
                                        sceneDelegate,
                                        maxSampleCount,
                                        &valueStore);
  ...
  SampledValueStore<CAPACITY> *valueStore)
{
  ...
  for (auto const &comp : computations) {
    ...
    for (double t : times) {
      ...
      if (!_InvokeComputation(*sceneDelegate,
                              *comp,
...
#endif  // WABI_IMAGING_HD_EXT_COMPUTATION_UTILS_H
