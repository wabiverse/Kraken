...
/*static*/
void HdDirtyBitsTranslator::RprimDirtyBitsToLocatorSet(TfToken const &primType,
...
/* static */
void HdDirtyBitsTranslator::SprimDirtyBitsToLocatorSet(TfToken const &primType,
                                                       const HdDirtyBits bits,
                                                       HdDataSourceLocatorSet *set)
{
  if (ARCH_UNLIKELY(set == nullptr)) {
  ...
  if (primType == HdPrimTypeTokens->material) {
    ...
    if (bits & (HdCamera::DirtyParams | HdCamera::DirtyClipPlanes | HdCamera::DirtyWindowPolicy)) {
    ...
    if (bits & (HdLight::DirtyParams | HdLight::DirtyShadowParams | HdLight::DirtyCollection)) {
    ...
    const static HdDataSourceLocator locator(HdPrimTypeTokens->drawTarget);
    if (bits) {
      set->append(locator);
    }
    ...
    if (bits & (HdExtComputation::DirtyInputDesc | HdExtComputation::DirtySceneInput)) {
...
/*static*/
void HdDirtyBitsTranslator::InstancerDirtyBitsToLocatorSet(TfToken const &primType,
                                                           const HdDirtyBits bits,
                                                           HdDataSourceLocatorSet *set)
{
  if (ARCH_UNLIKELY(set == nullptr)) {
...
/*static*/
void HdDirtyBitsTranslator::BprimDirtyBitsToLocatorSet(TfToken const &primType,
                                                       const HdDirtyBits bits,
                                                       HdDataSourceLocatorSet *set)
{
  if (ARCH_UNLIKELY(set == nullptr)) {
...
static bool _FindLocator(HdDataSourceLocator const &locator,
...
/*static*/
HdDirtyBits HdDirtyBitsTranslator::RprimLocatorSetToDirtyBits(TfToken const &primType,
                                                              HdDataSourceLocatorSet const &set)
{
  ...
  if (_FindLocator(HdLegacyDisplayStyleSchema::GetDefaultLocator(), end, &it, false)) {
    if (HdLegacyDisplayStyleSchema::GetDefaultLocator().HasPrefix(*it)) {
      bits |= HdChangeTracker::DirtyDisplayStyle | HdChangeTracker::DirtyCullStyle |
              HdChangeTracker::DirtyRepr;
    } else {
      do {
        if (it->HasPrefix(HdLegacyDisplayStyleSchema::GetCullStyleLocator())) {
          bits |= HdChangeTracker::DirtyCullStyle;
        } else if (it->HasPrefix(HdLegacyDisplayStyleSchema::GetReprSelectorLocator())) {
          bits |= HdChangeTracker::DirtyRepr;
        } else {
          bits |= HdChangeTracker::DirtyDisplayStyle;
        }
        ++it;
      } while (it != end && it->Intersects(HdLegacyDisplayStyleSchema::GetDefaultLocator()));
    }
  }
  ...
  if (_FindLocator(HdExtComputationPrimvarsSchema::GetDefaultLocator(), end, &it)) {
  ...
  if (_FindLocator(HdPrimvarsSchema::GetDefaultLocator(), end, &it, false)) {
    ...
    if (HdPrimvarsSchema::GetDefaultLocator().HasPrefix(*it)) {
      bits |= HdChangeTracker::DirtyPrimvar | HdChangeTracker::DirtyNormals |
              HdChangeTracker::DirtyPoints | HdChangeTracker::DirtyWidths;
    } else {
      do {
        if (it->HasPrefix(HdPrimvarsSchema::GetNormalsLocator())) {
          bits |= HdChangeTracker::DirtyNormals;
        } else if (it->HasPrefix(HdPrimvarsSchema::GetPointsLocator())) {
          bits |= HdChangeTracker::DirtyPoints;
        } else if (it->HasPrefix(HdPrimvarsSchema::GetWidthsLocator())) {
          bits |= HdChangeTracker::DirtyWidths;
        } else {
          bits |= HdChangeTracker::DirtyPrimvar;
        }
        ++it;
      } while (it != end && it->Intersects(HdPrimvarsSchema::GetDefaultLocator()));
    }
  }
...
/*static*/
HdDirtyBits HdDirtyBitsTranslator::SprimLocatorSetToDirtyBits(TfToken const &primType,
                                                              HdDataSourceLocatorSet const &set)
{
  HdDataSourceLocatorSet::const_iterator it = set.begin();
  ...
  if (primType == HdPrimTypeTokens->material) {
    ...
    if (_FindLocator(HdCameraSchema::GetDefaultLocator(), end, &it)) {
      bits |= HdCamera::DirtyWindowPolicy | HdCamera::DirtyClipPlanes | HdCamera::DirtyParams;
    }
    if (_FindLocator(HdXformSchema::GetDefaultLocator(), end, &it)) {
    ...
    if (_FindLocator(HdExtComputationSchema::GetDefaultLocator(), end, &it, false)) {
      if (HdExtComputationSchema::GetDefaultLocator().HasPrefix(*it)) {
        bits |= HdExtComputation::DirtyDispatchCount | HdExtComputation::DirtyElementCount |
                HdExtComputation::DirtyKernel | HdExtComputation::DirtyInputDesc |
                HdExtComputation::DirtySceneInput | HdExtComputation::DirtyOutputDesc;
      } else {
        do {
          if (it->HasPrefix(HdExtComputationSchema::GetDispatchCountLocator())) {
            bits |= HdExtComputation::DirtyDispatchCount;
          }
          if (it->HasPrefix(HdExtComputationSchema::GetElementCountLocator())) {
            bits |= HdExtComputation::DirtyElementCount;
          }
          if (it->HasPrefix(HdExtComputationSchema::GetGlslKernelLocator())) {
            bits |= HdExtComputation::DirtyKernel;
          }
          if (it->HasPrefix(HdExtComputationSchema::GetInputValuesLocator()) ||
              it->HasPrefix(HdExtComputationSchema::GetInputComputationsLocator())) {
            bits |= HdExtComputation::DirtyInputDesc | HdExtComputation::DirtySceneInput;
          }
          if (it->HasPrefix(HdExtComputationSchema::GetOutputsLocator())) {
            bits |= HdExtComputation::DirtyOutputDesc;
          }
          ++it;
        } while (it != end && it->Intersects(HdExtComputationSchema::GetDefaultLocator()));
      }
    }
  } else {
    // unknown prim type, use AllDirty for anything
...
/*static*/
HdDirtyBits HdDirtyBitsTranslator::InstancerLocatorSetToDirtyBits(
...
/*static*/
HdDirtyBits HdDirtyBitsTranslator::BprimLocatorSetToDirtyBits(TfToken const &primType,
                                                              HdDataSourceLocatorSet const &set)
{
  HdDataSourceLocatorSet::const_iterator it = set.begin();
