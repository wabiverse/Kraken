...
void HdRenderIndex::SyncAll(HdTaskSharedPtrVector *tasks, HdTaskContext *taskContext)
{
...

  {
    ...
    WorkParallelForN(
        syncMap.size(),
        std::bind(
            &_Worker::Process, std::ref(worker), std::placeholders::_1, std::placeholders::_2));
  }
      ...
      if (!TfDebug::IsEnabled(HD_DISABLE_MULTITHREADED_RPRIM_SYNC) &&
        ...
        dispatcher.Run([&r, workerState]() {
          WorkParallelForN(
              r.rprims.size(),
              std::bind(
                  &_SyncRPrims::Sync, workerState, std::placeholders::_1, std::placeholders::_2));
        });
      }
      else {
...
void HdRenderIndex::_GatherRenderTags(const HdTaskSharedPtrVector *tasks)
{
  ...
  for (size_t taskNum = 0; taskNum < numTasks; ++taskNum) {
    ...
    currentRenderTags.insert(
        currentRenderTags.end(), taskRenderTags.begin(), taskRenderTags.end());
  }
