...
HdContainerDataSourceEditor::_NodeSharedPtr HdContainerDataSourceEditor::_GetNode(
...
HdContainerDataSourceEditor &HdContainerDataSourceEditor::Set(
  const HdDataSourceLocator &locator,
  const HdDataSourceBaseHandle &dataSource)
{
  ...
  if (_initialContainer && (!dataSource || HdContainerDataSource::Cast(dataSource))) {
...
namespace
{
  bool _LongerLocatorIsLess(const HdDataSourceLocator &a, const HdDataSourceLocator &b)
  {
...
HdContainerDataSourceHandle HdContainerDataSourceEditor::Finish()
{
  if (_initialContainer) {
    ...
    if (_directContainerSets.empty()) {
      ...
      return HdOverlayContainerDataSource::New(TfArraySize(containers), containers);
    } else {

      static const HdBlockDataSourceHandle block = HdBlockDataSource::New();
      ...
      std::sort(_directContainerSets.begin(), _directContainerSets.end(), _LongerLocatorIsLess);
...
HdContainerDataSourceHandle HdContainerDataSourceEditor::_FinishWithNoInitialContainer()
{
  return _NodeContainerDataSource::New(_root);
}


HdContainerDataSourceEditor::_NodeContainerDataSource::_NodeContainerDataSource(
  _NodeSharedPtr node)
  : _node(node)
{}

bool HdContainerDataSourceEditor::_NodeContainerDataSource::Has(const TfToken &name)
{
  if (_node) {
...
TfTokenVector HdContainerDataSourceEditor::_NodeContainerDataSource::GetNames()
{
...
HdDataSourceBaseHandle HdContainerDataSourceEditor::_NodeContainerDataSource::Get(
  const TfToken &name)
{
  if (_node) {
    const auto it = _node->entries.find(name);
