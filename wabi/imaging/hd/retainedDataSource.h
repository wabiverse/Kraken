...
//
#ifndef WABI_IMAGING_HD_RETAINEDDATASOURCE_H
#define WABI_IMAGING_HD_RETAINEDDATASOURCE_H
...
class HdRetainedContainerDataSource : public HdContainerDataSource
{
 public:
  ...
  HD_API
  static Handle New(size_t count, const TfToken *names, const HdDataSourceBaseHandle *values);

  HD_API
  static Handle New(const TfToken &name1, const HdDataSourceBaseHandle &value1);

  HD_API
  static Handle New(const TfToken &name1,
...
class HdRetainedSampledDataSource : public HdSampledDataSource
{
 public:
  ...
  HdRetainedSampledDataSource(VtValue value) : _value(value) {}
...
/// semantics.
///
template<typename T> class HdRetainedTypedSampledDataSource : public HdTypedSampledDataSource<T>
{
 public:
  ...
  HdRetainedTypedSampledDataSource(const T &value) : _value(value) {}
  ...
  static typename HdRetainedTypedSampledDataSource<T>::Handle New(const T &value);
...
template<typename T>
typename HdRetainedTypedSampledDataSource<T>::Handle HdRetainedTypedSampledDataSource<T>::New(
  const T &value)
{
...
template<>
HdRetainedTypedSampledDataSource<bool>::Handle HdRetainedTypedSampledDataSource<bool>::New(
  const bool &value);
...
class HdRetainedTypedMultisampledDataSource : public HdTypedSampledDataSource<T>
{
 public:
  ...
  HdRetainedTypedMultisampledDataSource(size_t count,
...
template<typename T>
bool HdRetainedTypedMultisampledDataSource<T>::GetContributingSampleTimesForInterval(
  HdSampledDataSource::Time startTime,
  HdSampledDataSource::Time endTime,
...
template<typename T>
T HdRetainedTypedMultisampledDataSource<T>::GetTypedValue(HdSampledDataSource::Time shutterOffset)
{
  ...
  for (size_t i = 0, e = _sampledValues.count(); i < e; ++i) {
    ...
    if (sampleTime > shutterOffset) {
      ...
      if (i < 1) {
        ...
        const HdSampledDataSource::Time delta = sampleTime - shutterOffset;
        ...
        // tie, use the earlier.
        const HdSampledDataSource::Time previousDelta = shutterOffset - _sampledValues[i - 1];
...
class HdRetainedSmallVectorDataSource : public HdVectorDataSource
{
 public:
  ...
  HD_API
  HdRetainedSmallVectorDataSource(size_t count, HdDataSourceBaseHandle *values);
...
HD_API
HdSampledDataSourceHandle HdCreateTypedRetainedDataSource(VtValue const &v);

WABI_NAMESPACE_END

#endif  // WABI_IMAGING_HD_RETAINEDDATASOURCE_H
