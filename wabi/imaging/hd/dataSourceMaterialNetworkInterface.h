...
#include "wabi/imaging/hd/containerDataSourceEditor.h"
#include "wabi/imaging/hd/materialNetworkInterface.h"
...
class HdDataSourceMaterialNetworkInterface : public HdMaterialNetworkInterface
{
 public:
  ...
  HD_API
  TfTokenVector GetAuthoredNodeParameterNames(const TfToken &nodeName) const override;
  ...
  HD_API
  InputConnectionVector GetNodeInputConnection(const TfToken &nodeName,
  ...
  HD_API
  void SetNodeType(const TfToken &nodeName, const TfToken &nodeType) override;
  ...
  HD_API
  void SetNodeInputConnection(const TfToken &nodeName,
                              const TfToken &inputName,
                              const InputConnectionVector &connections) override;

  HD_API
  void DeleteNodeInputConnection(const TfToken &nodeName, const TfToken &inputName) override;

  HD_API
  TfTokenVector GetTerminalNames() const override;

  HD_API
  InputConnectionResult GetTerminalConnection(const TfToken &terminalName) const override;

  HD_API
  void DeleteTerminal(const TfToken &terminalName) override;

  HD_API
  void SetTerminalConnection(const TfToken &terminalName,
 ...
 private:
  ...
  using _OverrideMap = std::unordered_map<HdDataSourceLocator, HdDataSourceBaseHandle, TfHash>;

  using _TokenSet = std::unordered_set<TfToken, TfHash>;

  void _SetOverride(const HdDataSourceLocator &loc, const HdDataSourceBaseHandle &ds);
  ...
  HdContainerDataSourceHandle _GetNode(const TfToken &nodeName) const;
  HdContainerDataSourceHandle _GetNodeParameters(const TfToken &nodeName) const;
  HdContainerDataSourceHandle _GetNodeConnections(const TfToken &nodeName) const;
