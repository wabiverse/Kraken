...
HD_API
inline GfQuatf HdResampleNeighbors(float alpha, const GfQuatf &v0, const GfQuatf &v1)
{
                                                  ...
                                                  const VtIntArray *is)
{
  ...
  if (i == 0) {
    ...
    return std::pair<T, VtIntArray>(HdResampleNeighbors(alpha, vs[i - 1], vs[i]), is[i - 1]);
  }
}
...
#endif  // WABI_IMAGING_HD_TIME_SAMPLE_ARRAY_H
