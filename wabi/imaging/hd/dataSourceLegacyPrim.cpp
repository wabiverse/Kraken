...
//
#include "wabi/imaging/hd/dataSourceLegacyPrim.h"
...
#include "wabi/imaging/hd/basisCurvesSchema.h"
#include "wabi/imaging/hd/basisCurvesTopologySchema.h"
#include "wabi/imaging/hd/cameraSchema.h"
#include "wabi/imaging/hd/categoriesSchema.h"
#include "wabi/imaging/hd/coordSysBindingSchema.h"
#include "wabi/imaging/hd/extComputationInputComputationSchema.h"
#include "wabi/imaging/hd/extComputationOutputSchema.h"
#include "wabi/imaging/hd/extComputationPrimvarSchema.h"
#include "wabi/imaging/hd/extComputationPrimvarsSchema.h"
#include "wabi/imaging/hd/extComputationSchema.h"
#include "wabi/imaging/hd/extentSchema.h"
#include "wabi/imaging/hd/geomSubsetSchema.h"
#include "wabi/imaging/hd/geomSubsetsSchema.h"
#include "wabi/imaging/hd/instanceCategoriesSchema.h"
#include "wabi/imaging/hd/instancedBySchema.h"
#include "wabi/imaging/hd/instancerTopologySchema.h"
#include "wabi/imaging/hd/instanceSchema.h"
#include "wabi/imaging/hd/legacyDisplayStyleSchema.h"
#include "wabi/imaging/hd/lightSchema.h"
#include "wabi/imaging/hd/materialBindingSchema.h"
#include "wabi/imaging/hd/materialConnectionSchema.h"
#include "wabi/imaging/hd/materialNetworkSchema.h"
#include "wabi/imaging/hd/materialNodeSchema.h"
#include "wabi/imaging/hd/materialSchema.h"
#include "wabi/imaging/hd/meshSchema.h"
#include "wabi/imaging/hd/meshTopologySchema.h"
#include "wabi/imaging/hd/primvarSchema.h"
#include "wabi/imaging/hd/primvarsSchema.h"
#include "wabi/imaging/hd/purposeSchema.h"
#include "wabi/imaging/hd/renderBufferSchema.h"
#include "wabi/imaging/hd/subdivisionTagsSchema.h"
#include "wabi/imaging/hd/visibilitySchema.h"
#include "wabi/imaging/hd/volumeFieldBindingSchema.h"
#include "wabi/imaging/hd/volumeFieldSchema.h"
#include "wabi/imaging/hd/xformSchema.h"
...
namespace
{
  ...
  class Hd_DataSourceLegacyPrimvarValue : public HdSampledDataSource
  {
   public:
    ...
    Hd_DataSourceLegacyPrimvarValue(const TfToken &primvarName,
    ...
    VtValue GetValue(Time shutterOffset) override
    {
      if (shutterOffset == 0) {
        ...
        //      query fails
        float sampleTimes;
        _sceneDelegate->SamplePrimvar(_primId, _primvarName, 1, &sampleTimes, &result);
  ...
  class Hd_DataSourceLegacyIndexedPrimvarValue : public HdSampledDataSource
  {
   public:
    ...
    Hd_DataSourceLegacyIndexedPrimvarValue(const TfToken &primvarName,
                                           const SdfPath &primId,
                                           HdSceneDelegate *sceneDelegate)
      : _primvarName(primvarName),
        _primId(primId),
    ...
    VtValue GetValue(Time shutterOffset) override
    {
      if (shutterOffset == 0) {
        ...
        return _sceneDelegate->GetIndexedPrimvar(_primId, _primvarName, &indices);

      } else {
        _GetTimeSamples();
        std::pair<VtValue, VtIntArray> pv = _timeSamples.ResampleIndexed(shutterOffset);
        return pv.first;
      }
    }
   ...
   private:
    ...
    {
      if (_timeSamples.count == 0) {
        _sceneDelegate->SampleIndexedPrimvar(_primId, _primvarName, &_timeSamples);
      }
    }
  ...
  class Hd_DataSourceLegacyPrimvarIndices : public HdTypedSampledDataSource<VtArray<int>>
  {
   public:

    HD_DECLARE_DATASOURCE(Hd_DataSourceLegacyPrimvarIndices);

    Hd_DataSourceLegacyPrimvarIndices(const TfToken &primvarName,
                                      const SdfPath &primId,
                                      HdSceneDelegate *sceneDelegate)
      : _primvarName(primvarName),
        _primId(primId),
    ...
    VtIntArray GetTypedValue(Time shutterOffset) override
    {
      if (shutterOffset == 0) {
        VtIntArray indices(0);
        VtValue value = _sceneDelegate->GetIndexedPrimvar(_primId, _primvarName, &indices);
        return indices;
      } else {
        _GetTimeSamples();
        std::pair<VtValue, VtIntArray> pv = _timeSamples.ResampleIndexed(shutterOffset);
  ...
  class Hd_DataSourceLegacyPrimvarsContainer : public HdContainerDataSource
  {
   public:
    ...
    Hd_DataSourceLegacyPrimvarsContainer(const SdfPath &primId, HdSceneDelegate *sceneDelegate)
      : _primId(primId),
        _sceneDelegate(sceneDelegate)
    {
      TF_VERIFY(_sceneDelegate);
    ...
    void AddDesc(const TfToken &name,
      ...
      if ((*it).second.indexed) {
        return HdPrimvarSchema::Builder()
          .SetIndexedPrimvarValue(
            Hd_DataSourceLegacyIndexedPrimvarValue::New(name, _primId, _sceneDelegate))
          .SetIndices(Hd_DataSourceLegacyPrimvarIndices::New(name, _primId, _sceneDelegate))
          .SetInterpolation(
            HdPrimvarSchema::BuildInterpolationDataSource((*it).second.interpolation))
          .SetRole(HdPrimvarSchema::BuildRoleDataSource((*it).second.role))
          .Build();
      } else {
        return HdPrimvarSchema::Builder()
          .SetPrimvarValue(Hd_DataSourceLegacyPrimvarValue::New(name, _primId, _sceneDelegate))
          .SetInterpolation(
            HdPrimvarSchema::BuildInterpolationDataSource((*it).second.interpolation))
          .SetRole(HdPrimvarSchema::BuildRoleDataSource((*it).second.role))
          .Build();
   ...
   private:
    ...
    using _EntryMap =
      TfDenseHashMap<TfToken, _Entry, TfToken::HashFunctor, std::equal_to<TfToken>, 32>;
  ...
  class Hd_DataSourceLegacyMatrixValue : public HdMatrixDataSource
  {
   public:
    ...
    Hd_DataSourceLegacyMatrixValue(const TfToken &type,
                                   const SdfPath &primId,
                                   HdSceneDelegate *sceneDelegate)
      : _type(type),
        _primId(primId),
   ...
   private:
    ...
    void _GetTimeSamples()
    {
      if (_timeSamples.count == 0) {
        if (_type == HdPrimTypeTokens->instancer) {
          _sceneDelegate->SampleInstancerTransform(_primId, &_timeSamples);
        } else {
  ...
  using Hd_MeshTopologyStoreSharedPtr = std::shared_ptr<class Hd_MeshTopologyStore>;
  ...
  class Hd_MeshTopologyStore
  {
   public:
    ...
    HdMeshTopologySharedPtr Get()
    {
      ...
      mt = std::make_shared<HdMeshTopology>(_sceneDelegate->GetMeshTopology(_id));
    ...
    template<typename T> class MemberDataSource : public HdTypedSampledDataSource<T>
    {
     public:
      ...
      MemberDataSource(const Hd_MeshTopologyStoreSharedPtr &mts) : _mts(mts) {}
...
#define DEFINE_MESH_TOPOLOGY_ACCESSOR_DATASOURCE(N, T, A)                  \
  ...
  {                                                                        \
   public:                                                                 \
    ...
    HD_DECLARE_DATASOURCE(N);                                              \
                                                                           \
    N(const Hd_MeshTopologyStoreSharedPtr &mts) : MemberDataSource(mts) {} \
                                                                           \
    ...
    DEFINE_MESH_TOPOLOGY_ACCESSOR_DATASOURCE(FaceVertexCountsDataSource,
                                             VtArray<int>,
                                             GetFaceVertexCounts);
    DEFINE_MESH_TOPOLOGY_ACCESSOR_DATASOURCE(FaceVertexIndicesDataSource,
                                             VtArray<int>,
                                             GetFaceVertexIndices);
    DEFINE_MESH_TOPOLOGY_ACCESSOR_DATASOURCE(HoleIndicesDataSource, VtArray<int>, GetHoleIndices);
    DEFINE_MESH_TOPOLOGY_ACCESSOR_DATASOURCE(OrientationDataSource, TfToken, GetOrientation);
    DEFINE_MESH_TOPOLOGY_ACCESSOR_DATASOURCE(SubdivisionSchemeDataSource, TfToken, GetScheme);
  ...
  class Hd_DataSourceMeshTopology : public HdContainerDataSource
  {
   public:
    ...
    Hd_DataSourceMeshTopology(const Hd_MeshTopologyStoreSharedPtr &mts) : _mts(mts) {}
  ...
  class Hd_DataSourceMesh : public HdContainerDataSource
  {
    ...
    HdDataSourceBaseHandle Get(const TfToken &name) override
    {
      ...
      if (name == HdMeshSchemaTokens->subdivisionTags) {
        ...
        return HdSubdivisionTagsSchema::BuildRetained(
          HdRetainedTypedSampledDataSource<TfToken>::New(t.GetFaceVaryingInterpolationRule()),
          HdRetainedTypedSampledDataSource<TfToken>::New(t.GetVertexInterpolationRule()),
          HdRetainedTypedSampledDataSource<TfToken>::New(t.GetTriangleSubdivision()),
          HdRetainedTypedSampledDataSource<VtIntArray>::New(t.GetCornerIndices()),
          HdRetainedTypedSampledDataSource<VtFloatArray>::New(t.GetCornerWeights()),
          HdRetainedTypedSampledDataSource<VtIntArray>::New(t.GetCreaseIndices()),
          HdRetainedTypedSampledDataSource<VtIntArray>::New(t.GetCreaseLengths()),
          HdRetainedTypedSampledDataSource<VtFloatArray>::New(t.GetCreaseWeights()));
      }

      if (name == HdMeshSchemaTokens->subdivisionScheme) {
        return Hd_MeshTopologyStore::SubdivisionSchemeDataSource::New(_GetMeshTopologyStore());
      }

      if (name == HdMeshSchemaTokens->doubleSided) {
        return HdRetainedTypedSampledDataSource<bool>::New(_sceneDelegate->GetDoubleSided(_id));
      }
   ...
   private:
    ...
    HdDataSourceBaseHandle _BuildGeomSubsets()
    {
      ...
      const HdGeomSubsets &gs = _GetMeshTopologyStore()->Get()->GetGeomSubsets();
      for (const HdGeomSubset &geomSubset : gs) {
        HdDataSourceBaseHandle materialIdDs = HdRetainedTypedSampledDataSource<SdfPath>::New(
        ...
        HdContainerDataSourceHandle containers[2] = {
          HdGeomSubsetSchema::BuildRetained(
            HdGeomSubsetSchema::BuildTypeDataSource(HdGeomSubsetSchemaTokens->typeFaceSet),
            HdRetainedTypedSampledDataSource<VtIntArray>::New(geomSubset.indices)),

          HdRetainedContainerDataSource::New(
            HdMaterialBindingSchemaTokens->materialBinding,
            HdMaterialBindingSchema::BuildRetained(1, &t, &materialIdDs))};

        names.push_back(TfToken(geomSubset.id.GetText()));
        values.push_back(HdOverlayContainerDataSource::New(2, containers));
      }

      static const TfToken invisibleFacesToken("__invisibleFaces");
      static const TfToken invisiblePointsToken("__invisiblePoints");

      VtIntArray invisibleFaces = _GetMeshTopologyStore()->Get()->GetInvisibleFaces();
      if (!invisibleFaces.empty()) {
        HdContainerDataSourceHandle containers[2] = {
          HdGeomSubsetSchema::BuildRetained(
            HdGeomSubsetSchema::BuildTypeDataSource(HdGeomSubsetSchemaTokens->typeFaceSet),
            HdRetainedTypedSampledDataSource<VtIntArray>::New(invisibleFaces)),
          HdRetainedContainerDataSource::New(
            HdVisibilitySchemaTokens->visibility,
            HdVisibilitySchema::BuildRetained(HdRetainedTypedSampledDataSource<bool>::New(false))),
        };
      ...
      VtIntArray invisiblePoints = _GetMeshTopologyStore()->Get()->GetInvisiblePoints();
      if (!invisiblePoints.empty()) {
        HdContainerDataSourceHandle containers[2] = {
          HdGeomSubsetSchema::BuildRetained(
            HdGeomSubsetSchema::BuildTypeDataSource(HdGeomSubsetSchemaTokens->typePointSet),
            HdRetainedTypedSampledDataSource<VtIntArray>::New(invisiblePoints)),
          HdRetainedContainerDataSource::New(
            HdVisibilitySchemaTokens->visibility,
            HdVisibilitySchema::BuildRetained(HdRetainedTypedSampledDataSource<bool>::New(false))),
        };
    ...
    {
      Hd_MeshTopologyStoreSharedPtr mts = std::atomic_load(&_meshTopologyStore);
      if (mts) {
        return mts;
      }
  ...
  using Hd_BasisCurvesTopologyStoreSharedPtr = std::shared_ptr<class Hd_BasisCurvesTopologyStore>;
  ...
  class Hd_BasisCurvesTopologyStore
  {
   public:
    ...
    Hd_BasisCurvesTopologyStore(const SdfPath &id, HdSceneDelegate *sceneDelegate)
      : _id(id),
        _sceneDelegate(sceneDelegate)
    {}
    ...
    {
      HdBasisCurvesTopologySharedPtr bct = std::atomic_load(&_basisCurvesTopology);
      if (bct) {
        return bct;
      }

      bct = std::make_shared<HdBasisCurvesTopology>(_sceneDelegate->GetBasisCurvesTopology(_id));
    ...
    template<typename T> class MemberDataSource : public HdTypedSampledDataSource<T>
    {
     public:
      ...
      MemberDataSource(const Hd_BasisCurvesTopologyStoreSharedPtr &bcts) : _bcts(bcts) {}
...
#define DEFINE_BASISCURVES_TOPOLOGY_ACCESSOR_DATASOURCE(N, T, A)                    \
  ...
  {                                                                                 \
   public:                                                                          \
    ...
    HD_DECLARE_DATASOURCE(N);                                                       \
                                                                                    \
    N(const Hd_BasisCurvesTopologyStoreSharedPtr &bcts) : MemberDataSource(bcts) {} \
                                                                                    \
    T GetTypedValue(HdSampledDataSource::Time shutterOffset) override               \
    {                                                                               \
      return _bcts->Get()->A();                                                     \
    ...
    DEFINE_BASISCURVES_TOPOLOGY_ACCESSOR_DATASOURCE(CurveTypeDataSource, TfToken, GetCurveType);
    DEFINE_BASISCURVES_TOPOLOGY_ACCESSOR_DATASOURCE(CurveWrapDataSource, TfToken, GetCurveWrap);
    DEFINE_BASISCURVES_TOPOLOGY_ACCESSOR_DATASOURCE(CurveBasisDataSource, TfToken, GetCurveBasis);
    DEFINE_BASISCURVES_TOPOLOGY_ACCESSOR_DATASOURCE(CurveVertexCountsDataSource,
                                                    VtArray<int>,
                                                    GetCurveVertexCounts);
    DEFINE_BASISCURVES_TOPOLOGY_ACCESSOR_DATASOURCE(CurveIndicesDataSource,
                                                    VtArray<int>,
                                                    GetCurveIndices);
  ...
  class Hd_DataSourceBasisCurvesTopology : public HdContainerDataSource
  {
   public:
    ...
    {
      if (name == HdBasisCurvesTopologySchemaTokens->curveVertexCounts) {
        return Hd_BasisCurvesTopologyStore::CurveVertexCountsDataSource::New(_bcts);
      }
      if (name == HdBasisCurvesTopologySchemaTokens->curveIndices) {
        return Hd_BasisCurvesTopologyStore::CurveIndicesDataSource::New(_bcts);
      }
      if (name == HdBasisCurvesTopologySchemaTokens->basis) {
        return Hd_BasisCurvesTopologyStore::CurveBasisDataSource::New(_bcts);
      }
      if (name == HdBasisCurvesTopologySchemaTokens->type) {
        return Hd_BasisCurvesTopologyStore::CurveTypeDataSource::New(_bcts);
      }
      if (name == HdBasisCurvesTopologySchemaTokens->wrap) {
        return Hd_BasisCurvesTopologyStore::CurveWrapDataSource::New(_bcts);
      }
      return nullptr;
    }
  ...
  class Hd_DataSourceBasisCurves : public HdContainerDataSource
  {
   public:
    ...
    {
      if (name == HdBasisCurvesSchemaTokens->topology) {
        return Hd_DataSourceBasisCurvesTopology::New(_GetBasisCurvesTopologyStore());
      }
   ...
   private:
    ...
    HdDataSourceBaseHandle _BuildGeomSubsets()
    {
      ...
      VtIntArray invisibleCurves = _GetBasisCurvesTopologyStore()->Get()->GetInvisibleCurves();

      if (!invisibleCurves.empty()) {
        HdContainerDataSourceHandle containers[2] = {
          HdGeomSubsetSchema::BuildRetained(
            HdGeomSubsetSchema::BuildTypeDataSource(HdGeomSubsetSchemaTokens->typeCurveSet),
            HdRetainedTypedSampledDataSource<VtIntArray>::New(invisibleCurves)),
          HdRetainedContainerDataSource::New(
            HdVisibilitySchemaTokens->visibility,
            HdVisibilitySchema::BuildRetained(HdRetainedTypedSampledDataSource<bool>::New(false))),
        };
      ...
      VtIntArray invisiblePoints = _GetBasisCurvesTopologyStore()->Get()->GetInvisiblePoints();
    ...
    {
      Hd_BasisCurvesTopologyStoreSharedPtr bcts = std::atomic_load(&_basisCurvesTopologyStore);
      if (bcts) {
        return bcts;
      }

      bcts = std::make_shared<Hd_BasisCurvesTopologyStore>(_id, _sceneDelegate);
  ...
  class Hd_TypedDataSourceLegacyCameraParamValue : public HdTypedSampledDataSource<T>
  {
   public:
    ...
    Hd_TypedDataSourceLegacyCameraParamValue(const SdfPath &id,
                                             const TfToken &key,
                                             HdSceneDelegate *sceneDelegate)
      : _id(id),
        _key(key),
    ...
    bool GetContributingSampleTimesForInterval(
      ...
      std::vector<HdSampledDataSource::Time> *outSampleTimes) override
    {
      return Hd_DataSourceLegacyPrimvarValue::New(_key, _id, _sceneDelegate)
        ->GetContributingSampleTimesForInterval(startTime, endTime, outSampleTimes);
    }
    ...
    T GetTypedValue(HdSampledDataSource::Time shutterOffset) override
    {
      ...
      if (shutterOffset == 0.0f) {
        ...
        v =
          Hd_DataSourceLegacyPrimvarValue::New(_key, _id, _sceneDelegate)->GetValue(shutterOffset);
      }
  ...
  class Hd_DataSourceLegacyCameraParamValue : public HdSampledDataSource
  {
   public:
    ...
    Hd_DataSourceLegacyCameraParamValue(const SdfPath &id,
                                        const TfToken &key,
                                        HdSceneDelegate *sceneDelegate)
      : _id(id),
        _key(key),
    ...
    HdDataSourceBaseHandle Get(const TfToken &name) override
    {
      ...
      if (name == HdCameraSchemaTokens->projection) {
        ...
        return HdRetainedTypedSampledDataSource<TfToken>::New(
          proj == HdCamera::Perspective ? HdCameraSchemaTokens->perspective :
        ...
        return HdRetainedTypedSampledDataSource<CameraUtilConformWindowPolicy>::New(wp);
        ...
        return HdRetainedTypedSampledDataSource<std::vector<GfVec4d>>::New(cp);
      } else if (std::find(HdCameraSchemaTokens->allTokens.begin(),
                           HdCameraSchemaTokens->allTokens.end(),
                           name) != HdCameraSchemaTokens->allTokens.end()) {
        // all remaining HdCameraSchema members are floats and should
        // be returned as a typed data source for schema conformance.
        return Hd_TypedDataSourceLegacyCameraParamValue<float>::New(_id, name, _sceneDelegate);
      } else {
        return Hd_DataSourceLegacyCameraParamValue::New(_id, name, _sceneDelegate);
      }
    }
  ...
  class Hd_DataSourceLight : public HdContainerDataSource
  {
   ...
   private:
    ...
    bool _UseGet(const TfToken &name) const
    {
      ...
      if (name == HdLightTokens->params || name == HdLightTokens->shadowParams ||
  ...
  class Hd_InstanceCategoriesVectorDataSource : public HdVectorDataSource
  {
   public:
    ...
    Hd_InstanceCategoriesVectorDataSource(const SdfPath &id, HdSceneDelegate *sceneDelegate)
      : _id(id),
        _sceneDelegate(sceneDelegate),
        _checked(false)
    {
   ...
   private:
    ...
    void _FillArray()
    {
      ...
      std::vector<VtArray<TfToken>> values = _sceneDelegate->GetInstanceCategories(_id);
      if (!values.empty()) {
        _values.reserve(values.size());
        for (const VtArray<TfToken> &value : values) {
          // TODO, deduplicate by address or value
          _values.push_back(HdCategoriesSchema::BuildRetained(value.size(),
                                                              const_cast<TfToken *>(value.data()),
                                                              0,
                                                              nullptr));
        }
      }
  ...
  class Hd_InstancerTopologyDataSource : public HdContainerDataSource
  {
   public:
    ...
    Hd_InstancerTopologyDataSource(const SdfPath &id, HdSceneDelegate *sceneDelegate)
      : _id(id),
        _sceneDelegate(sceneDelegate)
    {
      TF_VERIFY(_sceneDelegate);
    ...
    {
      if (name == HdInstancerTopologySchemaTokens->prototypes) {
        return HdRetainedTypedSampledDataSource<VtArray<SdfPath>>::New(_protos);
      } else if (name == HdInstancerTopologySchemaTokens->instanceIndices) {
        return Hd_InstanceIndicesDataSource::New(_id, _sceneDelegate, _protos);
      } else {
        return nullptr;
      }
    }
   ...
   private:
    ...
    class Hd_InstanceIndicesDataSource : public HdVectorDataSource
    {
     public:
      ...
      Hd_InstanceIndicesDataSource(const SdfPath &id,
                                   HdSceneDelegate *sceneDelegate,
      ...
      HdDataSourceBaseHandle GetElement(size_t element) override
      {
        ...
        VtIntArray instanceIndices = _sceneDelegate->GetInstanceIndices(_id, _protos[element]);

        return HdRetainedTypedSampledDataSource<VtIntArray>::New(instanceIndices);
      }
  ...
  class Hd_DisplayStyleDataSource : public HdContainerDataSource
  {
   public:
    ...
    Hd_DisplayStyleDataSource(HdSceneDelegate *sceneDelegate, const SdfPath &id)
      : _sceneDelegate(sceneDelegate),
        _id(id),
    ...
    HdDataSourceBaseHandle Get(const TfToken &name) override
    {
      if (name == HdLegacyDisplayStyleSchemaTokens->refineLevel) {
        ...
        return (_displayStyle.refineLevel != 0) ?
                 HdRetainedTypedSampledDataSource<int>::New(_displayStyle.refineLevel) :
                 nullptr;
        ...
        return HdRetainedTypedSampledDataSource<bool>::New(_displayStyle.flatShadingEnabled);
      } else if (name == HdLegacyDisplayStyleSchemaTokens->displacementEnabled) {
        if (!_displayStyleRead) {
          _displayStyle = _sceneDelegate->GetDisplayStyle(_id);
          _displayStyleRead = true;
        }
        return HdRetainedTypedSampledDataSource<bool>::New(_displayStyle.displacementEnabled);
        ...
        return HdRetainedTypedSampledDataSource<bool>::New(_displayStyle.pointsShadingEnabled);
      } else if (name == HdLegacyDisplayStyleSchemaTokens->materialIsFinal) {
        if (!_displayStyleRead) {
          _displayStyle = _sceneDelegate->GetDisplayStyle(_id);
          _displayStyleRead = true;
        }
        return HdRetainedTypedSampledDataSource<bool>::New(_displayStyle.materialIsFinal);
      } else if (name == HdLegacyDisplayStyleSchemaTokens->shadingStyle) {
        TfToken shadingStyle = _sceneDelegate->GetShadingStyle(_id).GetWithDefault<TfToken>();
        ...
        if (!empty) {
          ...
          reprSelectorDs = HdRetainedTypedSampledDataSource<VtArray<TfToken>>::New(array);
        }
  ...
  class Hd_GenericGetSampledDataSource : public HdSampledDataSource
  {
   public:
    ...
    Hd_GenericGetSampledDataSource(HdSceneDelegate *sceneDelegate,
                                   const SdfPath &id,
                                   const TfToken &key)
      : _sceneDelegate(sceneDelegate),
        _id(id),
  ...
  class Hd_LegacyDrawTargetContainerDataSource : public HdContainerDataSource
  {
   public:
    ...
    Hd_LegacyDrawTargetContainerDataSource(HdSceneDelegate *sceneDelegate, const SdfPath &id)
      : _sceneDelegate(sceneDelegate),
        _id(id)
    {}
    ...
    bool Has(const TfToken &name) override
    {
      return std::find(_drawTargetTokens->allTokens.begin(),
                       ...
                       name) != _drawTargetTokens->allTokens.end();
    }
  ...
  class Hd_DataSourceLegacyExtComputationPrimvarsContainer : public HdContainerDataSource
  {
   public:

    HD_DECLARE_DATASOURCE(Hd_DataSourceLegacyExtComputationPrimvarsContainer);

    Hd_DataSourceLegacyExtComputationPrimvarsContainer(const SdfPath &primId,
                                                       HdSceneDelegate *sceneDelegate)
      : _primId(primId),
        _sceneDelegate(sceneDelegate)
    {
    ...
    HdDataSourceBaseHandle Get(const TfToken &name) override
    {
      ...
      return HdExtComputationPrimvarSchema::BuildRetained(
        HdExtComputationPrimvarSchema::BuildInterpolationDataSource(it->second.interpolation),
        HdExtComputationPrimvarSchema::BuildRoleDataSource(it->second.role),
        HdRetainedTypedSampledDataSource<SdfPath>::New(it->second.sourceComputation),
        HdRetainedTypedSampledDataSource<TfToken>::New(it->second.sourceComputationOutputName),
        HdRetainedTypedSampledDataSource<HdTupleType>::New(it->second.valueType));
    }
  ...
  HD_DECLARE_DATASOURCE_HANDLES(Hd_DataSourceLegacyExtComputationPrimvarsContainer);
  ...
  class Hd_DataSourceLegacyExtComputationInput : public HdSampledDataSource
  {
   public:
    ...
    Hd_DataSourceLegacyExtComputationInput(const TfToken &name,
                                           const SdfPath &id,
                                           HdSceneDelegate *sceneDelegate)
      : _name(name),
        _id(id),
   ...
   private:
    ...
    {
      if (_timeSamples.count == 0) {
        _sceneDelegate->SampleExtComputationInput(_id, _name, &_timeSamples);
      }
    }
  ...
  class Hd_DataSourceLegacyExtComputationInputValues : public HdContainerDataSource
  {
   public:

    HD_DECLARE_DATASOURCE(Hd_DataSourceLegacyExtComputationInputValues);

    Hd_DataSourceLegacyExtComputationInputValues(const SdfPath &id, HdSceneDelegate *sceneDelegate)
      : _id(id),
        _sceneDelegate(sceneDelegate)
    {}
    ...
    HdDataSourceBaseHandle Get(const TfToken &name) override
    {
      return Hd_DataSourceLegacyExtComputationInput::New(name, _id, _sceneDelegate);
    }
  ...
  class Hd_DataSourceLegacyExtComputation : public HdContainerDataSource
  {
   public:
    ...
    Hd_DataSourceLegacyExtComputation(const SdfPath &id, HdSceneDelegate *sceneDelegate)
      : _id(id),
        _sceneDelegate(sceneDelegate)
    {}
    ...
    {
      if (name == HdExtComputationSchemaTokens->inputValues) {
        return Hd_DataSourceLegacyExtComputationInputValues::New(_id, _sceneDelegate);
        ...
        for (const auto &desc : descs) {
          out.push_back(HdExtComputationInputComputationSchema::BuildRetained(
            HdRetainedTypedSampledDataSource<TfToken>::New(desc.name),
            HdRetainedTypedSampledDataSource<SdfPath>::New(desc.sourceComputationId),
            HdRetainedTypedSampledDataSource<TfToken>::New(desc.sourceComputationOutputName)));
        }
        ...
        for (const auto &desc : descs) {
          out.push_back(HdExtComputationOutputSchema::BuildRetained(
            HdRetainedTypedSampledDataSource<TfToken>::New(desc.name),
            HdRetainedTypedSampledDataSource<HdTupleType>::New(desc.valueType)));
        }
        return HdRetainedSmallVectorDataSource::New(out.size(), out.data());
        ...
        return HdExtComputationCallbackDataSource::New(_id, _sceneDelegate);
      } else if (name == HdExtComputationSchemaTokens->dispatchCount) {
        size_t dispatchCount = 0;
        VtValue vDispatch = _sceneDelegate->GetExtComputationInput(_id, HdTokens->dispatchCount);
        ...
        VtValue vElement = _sceneDelegate->GetExtComputationInput(_id, HdTokens->elementCount);
  ...
  class Hd_DataSourceRenderBuffer : public HdContainerDataSource
  {
   public:
    ...
    Hd_DataSourceRenderBuffer(HdSceneDelegate *sceneDelegate, const SdfPath &id)
      : _sceneDelegate(sceneDelegate),
        _id(id)
    {
      TF_VERIFY(_sceneDelegate);
    ...
    {
      if (name == HdRenderBufferSchemaTokens->dimensions) {
        return HdRetainedTypedSampledDataSource<GfVec3i>::New(rb.dimensions);
      } else if (name == HdRenderBufferSchemaTokens->format) {
        return HdRetainedTypedSampledDataSource<HdFormat>::New(rb.format);
      } else if (name == HdRenderBufferSchemaTokens->multiSampled) {
        return HdRetainedTypedSampledDataSource<bool>::New(rb.multiSampled);
      } else {
        return HdSampledDataSourceHandle(
          Hd_GenericGetSampledDataSource::New(_sceneDelegate, _id, name));
      }
...
void HdExtComputationCallbackDataSource::Invoke(HdExtComputationContext *context)
{
...
HdDataSourceLegacyPrim::HdDataSourceLegacyPrim(SdfPath id,
                                               TfToken type,
                                               HdSceneDelegate *sceneDelegate)
  : _id(id),
    _type(type),
...
const HdDataSourceLocatorSet &HdDataSourceLegacyPrim::GetCachedLocators()
{
...
static bool _IsTypeLightLike(const TfToken &type)
{
...
bool HdDataSourceLegacyPrim::Has(const TfToken &name)
{
  ...
  if (HdPrimTypeIsGprim(_type)) {
    if (name == HdPrimvarsSchemaTokens->primvars ||
        ...
        name == HdPurposeSchemaTokens->purpose || name == HdVisibilitySchemaTokens->visibility ||
        name == HdInstancedBySchemaTokens->instancedBy ||
        name == HdCategoriesSchemaTokens->categories || name == HdXformSchemaTokens->xform ||
  ...
  if (HdPrimTypeIsLight(_type)) {
    if (name == HdMaterialSchemaTokens->material || name == HdXformSchemaTokens->xform ||
  ...
  if (_IsTypeLightLike(_type)) {
    if (name == HdLightSchemaTokens->light || name == HdMaterialSchemaTokens->material) {
      return true;
    }
  }
  ...
  if (_type == HdPrimTypeTokens->instancer) {
    if (name == HdXformSchemaTokens->xform || name == HdInstancedBySchemaTokens->instancedBy ||
  ...
  if (_type == HdPrimTypeTokens->camera) {
    if (name == HdCameraSchemaTokens->camera || name == HdXformSchemaTokens->xform) {
      return true;
    }
  }
...
static bool _ConvertHdMaterialNetworkToHdDataSources(const HdMaterialNetworkMap &hdNetworkMap,
                                                     HdContainerDataSourceHandle *result)
{
  ...
  for (auto const &iter : hdNetworkMap.map) {
    ...
    for (const HdMaterialNode &node : hdNetwork.nodes) {
      ...
      for (const auto &p : node.parameters) {
        ...
        paramsValues.push_back(HdRetainedTypedSampledDataSource<VtValue>::New(p.second));
      }

      // Accumulate array connections to the same input
      TfDenseHashMap<TfToken, TfSmallVector<HdDataSourceBaseHandle, 8>, TfToken::HashFunctor>
        connectionsMap;
      ...
      for (const HdMaterialRelationship &rel : hdNetwork.relationships) {
        if (rel.outputId == node.path) {
          ...
          HdDataSourceBaseHandle c = HdMaterialConnectionSchema::BuildRetained(
      ...
      nodeValues.push_back(HdMaterialNodeSchema::BuildRetained(
        HdRetainedContainerDataSource::New(paramsNames.size(),
                                           paramsNames.data(),
                                           paramsValues.data()),
        HdRetainedContainerDataSource::New(cNames.size(), cNames.data(), cValues.data()),
        HdRetainedTypedSampledDataSource<TfToken>::New(node.identifier)));
    }
    ...
    terminalsValues.push_back(HdMaterialConnectionSchema::BuildRetained(
      ...
      HdRetainedTypedSampledDataSource<TfToken>::New(terminalsNames.back())));
  }
  ...
  HdContainerDataSourceHandle terminalsDefaultContext = HdRetainedContainerDataSource::New(
    terminalsNames.size(),
    terminalsNames.data(),
    terminalsValues.data());

  // Create the material network, potentially one per network selector
  HdDataSourceBaseHandle network = HdMaterialNetworkSchema::BuildRetained(nodesDefaultContext,
                                                                          terminalsDefaultContext);

  TfToken defaultContext = HdMaterialSchemaTokens->universalRenderContext;
  *result = HdMaterialSchema::BuildRetained(1, &defaultContext, &network);

  return true;
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetPrimvarsDataSource()
{
  ...
  for (size_t interpolation = HdInterpolationConstant; interpolation < HdInterpolationCount;
    ...
    TfToken interpolationToken = _InterpolationAsToken((HdInterpolation)interpolation);

    for (const auto &primvarDesc : v) {
      if (!primvarsDs) {
        primvarsDs = Hd_DataSourceLegacyPrimvarsContainer::New(_id, _sceneDelegate);
      }
      primvarsDs->AddDesc(primvarDesc.name,
                          interpolationToken,
                          primvarDesc.role,
                          primvarDesc.indexed);
    }
...
HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetExtComputationPrimvarsDataSource()
{
  ...
  for (size_t interpolation = HdInterpolationConstant; interpolation < HdInterpolationCount;
    ...
    HdExtComputationPrimvarDescriptorVector v =
      _sceneDelegate->GetExtComputationPrimvarDescriptors(_id, (HdInterpolation)interpolation);

    TfToken interpolationToken = _InterpolationAsToken((HdInterpolation)interpolation);

    for (const auto &primvarDesc : v) {
      if (!primvarsDs) {
        primvarsDs = Hd_DataSourceLegacyExtComputationPrimvarsContainer::New(_id, _sceneDelegate);
      }
      primvarsDs->AddDesc(primvarDesc.name,
                          interpolationToken,
                          primvarDesc.role,
...
HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetMaterialBindingDataSource()
{
  SdfPath path = _sceneDelegate->GetMaterialId(_id);
  if (path.IsEmpty()) {
    return nullptr;
  }
  HdDataSourceBaseHandle bindingPath = HdRetainedTypedSampledDataSource<SdfPath>::New(path);

  TfToken t = HdMaterialBindingSchemaTokens->allPurpose;
  HdContainerDataSourceHandle binding = HdMaterialBindingSchema::BuildRetained(1,
                                                                               &t,
                                                                               &bindingPath);
  return binding;
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetXformDataSource()
{
  HdContainerDataSourceHandle t = HdXformSchema::BuildRetained(
...
HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetMaterialDataSource()
{
  ...
  HdMaterialNetworkMap hdNetworkMap = materialContainer.UncheckedGet<HdMaterialNetworkMap>();
...
HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetDisplayStyleDataSource()
{
  return Hd_DisplayStyleDataSource::New(_sceneDelegate, _id);
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetInstancedByDataSource()
{
  ...
  return HdInstancedBySchema::BuildRetained(
    HdRetainedTypedSampledDataSource<VtArray<SdfPath>>::New(VtArray<SdfPath>({instancerId})));
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetInstancerTopologyDataSource()
{
  TRACE_FUNCTION();

  HdContainerDataSourceHandle instancerTopology = HdContainerDataSource::AtomicLoad(
    _instancerTopology);
  ...
  instancerTopology = Hd_InstancerTopologyDataSource::New(_id, _sceneDelegate);

  HdContainerDataSource::AtomicStore(_instancerTopology, instancerTopology);

  return instancerTopology;
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetVolumeFieldBindingDataSource()
{
  HdVolumeFieldDescriptorVector volumeFields = _sceneDelegate->GetVolumeFieldDescriptors(_id);
  ...
  for (HdVolumeFieldDescriptor const &desc : volumeFields) {
    ...
    paths.push_back(HdRetainedTypedSampledDataSource<SdfPath>::New(desc.fieldId));
  }
  return HdVolumeFieldBindingSchema::BuildRetained(names.size(), names.data(), paths.data());
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetCoordSysBindingDataSource()
{
  HdIdVectorSharedPtr coordSysBindings = _sceneDelegate->GetCoordSysBindings(_id);
  ...
  for (SdfPath const &path : *coordSysBindings) {
    ...
    paths.push_back(HdRetainedTypedSampledDataSource<SdfPath>::New(path));
  }
  return HdCoordSysBindingSchema::BuildRetained(names.size(), names.data(), paths.data());
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetVisibilityDataSource()
{
  bool vis = _sceneDelegate->GetVisible(_id);
  if (vis) {
    static const HdContainerDataSourceHandle visOn = HdVisibilitySchema::BuildRetained(
      HdRetainedTypedSampledDataSource<bool>::New(true));
    return visOn;
  } else {
    static const HdContainerDataSourceHandle visOff = HdVisibilitySchema::BuildRetained(
      HdRetainedTypedSampledDataSource<bool>::New(false));
    return visOff;
  }
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetPurposeDataSource()
{
  TfToken purpose = _sceneDelegate->GetRenderTag(_id);
  return HdPurposeSchema::BuildRetained(HdRetainedTypedSampledDataSource<TfToken>::New(purpose));
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetExtentDataSource()
{
...
HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetCategoriesDataSource()
{
  VtArray<TfToken> categories = _sceneDelegate->GetCategories(_id);
  if (!categories.empty()) {
    return HdCategoriesSchema::BuildRetained(categories.size(), categories.data(), 0, nullptr);
  }
  return nullptr;
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetInstanceCategoriesDataSource()
{
  return HdInstanceCategoriesSchema::BuildRetained(
    HdVectorDataSourceHandle(Hd_InstanceCategoriesVectorDataSource::New(_id, _sceneDelegate)));
}

HdDataSourceBaseHandle HdDataSourceLegacyPrim::_GetPrmanParamsDataSource()
{
  ...
  for (const TfToken &dictName : namesValue.UncheckedGet<TfTokenVector>()) {
    ...
    std::map<TfToken, VtValue> dict = dictValue.UncheckedGet<std::map<TfToken, VtValue>>();
    ...
    for (const auto &valuePair : dict) {
      ...
      values.push_back(HdRetainedSampledDataSource::New(valuePair.second));
    }

    dictKeys.push_back(dictName);
    dicts.push_back(
      HdRetainedContainerDataSource::New(valueKeys.size(), valueKeys.data(), values.data()));
  }


  return HdRetainedContainerDataSource::New(dictKeys.size(), dictKeys.data(), dicts.data());
}
...
HdDataSourceBaseHandle HdDataSourceLegacyPrim::Get(const TfToken &name)
{
  if (name == HdMeshSchemaTokens->mesh) {
  ...
  } else if (name == HdExtComputationPrimvarsSchemaTokens->extComputationPrimvars) {
    ...
    return HdRetainedTypedSampledDataSource<HdSceneDelegate *>::New(_sceneDelegate);
