...
void HdPrimDataSourceOverlayCache::HandlePrimsAdded(
  ...
  const HdSceneIndexBaseRefPtr &source)
{
  ...
  for (const auto &entry : entries) {
    ...
    HdContainerDataSourceHandle inputDataSource = source->GetPrim(entry.primPath).dataSource;

    auto iterBoolPair = _cache.insert({entry.primPath, HdSceneIndexPrim()});
    ...
    // otherwise, create it.
    if (prim.dataSource) {
      _HdPrimDataSourceOverlay::Cast(prim.dataSource)->UpdateInputDataSource(inputDataSource);
    } else {
      prim.dataSource = _HdPrimDataSourceOverlay::New(inputDataSource,
                                                      parentOverlay,
                                                      shared_from_this());
    }
  }
...
void HdPrimDataSourceOverlayCache::HandlePrimsRemoved(
...
void HdPrimDataSourceOverlayCache::HandlePrimsDirtied(
  ...
  HdSceneIndexObserver::DirtiedPrimEntries *additionalDirtied)
{
  for (const auto &entry : entries) {
    ...
    if (it != _cache.end()) {
      _HdPrimDataSourceOverlay::Cast(it->second.dataSource)->PrimDirtied(dirtyAttributes);
    }
...
void HdPrimDataSourceOverlayCache::_HdPrimDataSourceOverlay::UpdateInputDataSource(
  HdContainerDataSourceHandle inputDataSource)
{
  _inputDataSource = inputDataSource;
  _overlayMap.clear();
...
void HdPrimDataSourceOverlayCache::_HdPrimDataSourceOverlay::PrimDirtied(
...
bool HdPrimDataSourceOverlayCache::_HdPrimDataSourceOverlay::Has(const TfToken &name)
{
...
TfTokenVector HdPrimDataSourceOverlayCache::_HdPrimDataSourceOverlay::GetNames()
{
  TRACE_FUNCTION();
...
HdDataSourceBaseHandle HdPrimDataSourceOverlayCache::_HdPrimDataSourceOverlay::Get(
  const TfToken &name)
{
  if (ARCH_UNLIKELY(!_inputDataSource)) {
    return nullptr;
  ...
  if (TF_VERIFY(cache)) {
    ...
    if (topoIt != cache->_overlayTopology.end()) {
      ...
      HdDataSourceBaseHandle ds = cache->_ComputeOverlayDataSource(name,
                                                                   _inputDataSource,
