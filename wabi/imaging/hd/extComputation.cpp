...
TF_DEFINE_ENV_SETTING(HD_ENABLE_SHARED_EXT_COMPUTATION_DATA,
                      1,
                      "Enable sharing of ext computation data buffers");

/* static */
bool HdExtComputation::_IsEnabledSharedExtComputationData()
{
  static bool enabled = (TfGetEnvSetting(HD_ENABLE_SHARED_EXT_COMPUTATION_DATA) == 1);
  return enabled;
}
...
void HdExtComputation::_Sync(HdSceneDelegate *sceneDelegate,
                             HdRenderParam *renderParam,
                             HdDirtyBits *dirtyBits)
{
  HD_TRACE_FUNCTION();
...
HdDirtyBits HdExtComputation::GetInitialDirtyBitsMask() const
{
  return AllDirty;
}

size_t HdExtComputation::GetDispatchCount() const
{
  return (_dispatchCount > 0 ? _dispatchCount : _elementCount);
}

TfTokenVector HdExtComputation::GetOutputNames() const
{
...
bool HdExtComputation::IsInputAggregation() const
{
