...
const HdDataSourceLocator &HdDataSourceLocator::EmptyLocator()
{
...
HdDataSourceLocator::HdDataSourceLocator(const TfToken &t1)
{
...
HdDataSourceLocator::HdDataSourceLocator(const TfToken &t1, const TfToken &t2) : _tokens{t1, t2} {}

HdDataSourceLocator::HdDataSourceLocator(const TfToken &t1, const TfToken &t2, const TfToken &t3)
  : _tokens{t1, t2, t3}
{}
...
size_t HdDataSourceLocator::GetElementCount() const
{
  return _tokens.size();
}

const TfToken &HdDataSourceLocator::GetElement(size_t i) const
{
  return _tokens[i];
}

const TfToken &HdDataSourceLocator::GetLastElement() const
{
  if (_tokens.empty()) {
...
const TfToken &HdDataSourceLocator::GetFirstElement() const
{
  if (_tokens.empty()) {
    static TfToken empty;
    return empty;
...
HdDataSourceLocator HdDataSourceLocator::ReplaceLastElement(const TfToken &name) const
{
  if (_tokens.empty()) {
    return *this;
  }
...
HdDataSourceLocator HdDataSourceLocator::RemoveLastElement() const
{
  if (_tokens.size() < 2) {
...
HdDataSourceLocator HdDataSourceLocator::RemoveFirstElement() const
{
  if (_tokens.size() < 2) {
    return HdDataSourceLocator();
  }
...
HdDataSourceLocator HdDataSourceLocator::Append(const TfToken &name) const
{
  HdDataSourceLocator result(*this);
...
HdDataSourceLocator HdDataSourceLocator::Append(const HdDataSourceLocator &locator) const
{
  HdDataSourceLocator result(*this);
  result._tokens.insert(result._tokens.end(), locator._tokens.begin(), locator._tokens.end());
  return result;
}

HdDataSourceLocator HdDataSourceLocator::Prepend(const TfToken &name) const
{
...
HdDataSourceLocator HdDataSourceLocator::Prepend(const HdDataSourceLocator &locator) const
{
...
bool HdDataSourceLocator::HasPrefix(const HdDataSourceLocator &prefix) const
{
  if (prefix.IsEmpty()) {
    return true;
  }
...
HdDataSourceLocator HdDataSourceLocator::ReplacePrefix(const HdDataSourceLocator &oldPrefix,
                                                       const HdDataSourceLocator &newPrefix) const
{
  ...
  result._tokens.insert(result._tokens.end(),
                        _tokens.begin() + oldPrefix.GetElementCount(),
                        _tokens.end());
...
std::string HdDataSourceLocator::GetString(const char *delimiter) const
{
  if (_tokens.empty()) {
    return std::string();
  }
...
bool HdDataSourceLocator::operator<(const HdDataSourceLocator &rhs) const
{
...
HdDataSourceLocatorSet::HdDataSourceLocatorSet(const HdDataSourceLocator &locator)
{
...
static bool _LessThanNotPrefix(const HdDataSourceLocator &a, const HdDataSourceLocator &b)
{
...
void HdDataSourceLocatorSet::_InsertAndDeleteSuffixes(_Locators::iterator *position,
...
void HdDataSourceLocatorSet::insert(const HdDataSourceLocator &locator)
{
  if (_locators.empty()) {
    _locators.push_back(locator);
    return;
  ...
  auto it = std::lower_bound(_locators.begin(), _locators.end(), locator, _LessThanNotPrefix);
...
void HdDataSourceLocatorSet::insert(const HdDataSourceLocatorSet &locatorSet)
{
  ...
  for (auto otherIt = locatorSet._locators.begin(); otherIt != locatorSet._locators.end();
       ++otherIt) {
    while (thisIt != _locators.end() && _LessThanNotPrefix(*thisIt, *otherIt)) {
...
void HdDataSourceLocatorSet::append(const HdDataSourceLocator &locator)
{
  if (_locators.size() == 0 || _LessThanNotPrefix(_locators.back(), locator)) {
    _locators.push_back(locator);
  } else {
    insert(locator);
  }
...
HdDataSourceLocatorSet::const_iterator HdDataSourceLocatorSet::begin() const
{
  return _locators.begin();
}

HdDataSourceLocatorSet::const_iterator HdDataSourceLocatorSet::end() const
{
  return _locators.end();
}

bool HdDataSourceLocatorSet::Intersects(const HdDataSourceLocator &locator) const
{
  ...
  const auto it = std::lower_bound(_locators.begin(),
                                   _locators.end(),
                                   locator,
                                   _LessThanNotPrefix);
  if (it != _locators.end() && (locator.HasPrefix(*it) || it->HasPrefix(locator))) {
    return true;
  }
  return false;
}

bool HdDataSourceLocatorSet::Intersects(const HdDataSourceLocatorSet &locatorSet) const
{
  // Note: operator< and _LessThanNotPrefix are almost as expensive as
...
bool HdDataSourceLocatorSet::IsEmpty() const
{
