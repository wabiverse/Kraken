...
void HdBufferArray::IncrementVersion()
{
  _version = _uniqueVersion++;  // Atomic
}

bool HdBufferArray::TryAssignRange(HdBufferArrayRangeSharedPtr &range)
{
...
void HdBufferArray::RemoveUnusedRanges()
{
...
void HdBufferArray::_SetRangeList(std::vector<HdBufferArrayRangeSharedPtr> const &ranges)
{
  std::lock_guard<std::mutex> lock(_rangeListLock);
...
/*virtual*/
size_t HdBufferArray::GetMaxNumElements() const
{
