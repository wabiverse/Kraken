...
HdDirtyBits HdRprim::PropagateRprimDirtyBits(HdDirtyBits bits)
{
...
void HdRprim::InitRepr(HdSceneDelegate *delegate, TfToken const &reprToken, HdDirtyBits *dirtyBits)
{
...
// -------------------------------------------------------------------------- //
void HdRprim::Finalize(HdRenderParam *renderParam) {}

// -------------------------------------------------------------------------- //
///                              Rprim Data API
// -------------------------------------------------------------------------- //
void HdRprim::SetPrimId(int32_t primId)
{
  _primId = primId;
  // Don't set DirtyPrimID here, to avoid undesired variability tracking.
}

void HdRprim::SetMaterialId(SdfPath const &materialId)
{
...
VtMatrix4dArray HdRprim::GetInstancerTransforms(HdSceneDelegate *delegate)
{
