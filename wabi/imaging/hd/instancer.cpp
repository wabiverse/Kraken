...
/* static */
int HdInstancer::GetInstancerNumLevels(HdRenderIndex &index, HdRprim const &rprim)
{
  ...
  while (!parent.IsEmpty()) {
    ...
    TF_VERIFY(instancer);
    parent = instancer ? instancer->GetParentId() : SdfPath::EmptyPath();
  }
  return instancerLevels;
}

/* static */
TfTokenVector const &HdInstancer::GetBuiltinPrimvarNames()
{
  static const TfTokenVector primvarNames = {HdInstancerTokens->instanceTransform,
...
HdDirtyBits HdInstancer::GetInitialDirtyBitsMask() const
{
  return HdChangeTracker::DirtyTransform | HdChangeTracker::DirtyPrimvar |
