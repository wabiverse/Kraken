                               ...
                               VtValue const &value)
{
  ...
  if (status != RPR_SUCCESS) {
    auto errMsg = TfStringPrintf("Failed to set material input %d(%s)",
                                 input,
                                 value.GetTypeName().c_str());
...
{
  return std::abs(v1[0] - v2[0]) <= tolerance && std::abs(v1[1] - v2[1]) <= tolerance &&
         std::abs(v1[2] - v2[2]) <= tolerance && std::abs(v1[3] - v2[3]) <= tolerance;
}
