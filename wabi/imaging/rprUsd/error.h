...
#ifndef WABI_IMAGING_RPR_USD_ERROR_H
#define WABI_IMAGING_RPR_USD_ERROR_H
...
#define RPR_GET_ERROR_MESSAGE(status, msg, ...)  \
  RprUsdConstructErrorMessage(status,            \
                              msg,               \
                              __ARCH_FILE__,     \
                              __ARCH_FUNCTION__, \
                              __LINE__,          \
...
inline std::string RprUsdConstructErrorMessage(rpr::Status errorStatus,
                                               ...
                                               rpr::Context *context = nullptr)
{
  auto rprErrorString = [errorStatus, context]() -> std::string {
    if (context) {
      ...
      auto status = context->GetInfo(RPR_CONTEXT_LAST_ERROR_MESSAGE,
                                     0,
                                     nullptr,
                                     &lastErrorMessageSize);
      if (status == RPR_SUCCESS && lastErrorMessageSize > 1) {
        std::string message(lastErrorMessageSize, '\0');
        status = context->GetInfo(RPR_CONTEXT_LAST_ERROR_MESSAGE,
                                  message.size(),
                                  &message[0],
                                  nullptr);
    ...
    switch (errorStatus) {
      case RPR_ERROR_INVALID_API_VERSION:
        return "invalid api version";
      case RPR_ERROR_INVALID_PARAMETER:
        return "invalid parameter";
      case RPR_ERROR_UNSUPPORTED:
        return "unsupported";
      case RPR_ERROR_INTERNAL_ERROR:
        return "internal error";
      case RPR_ERROR_INVALID_CONTEXT:
  ...
  if (errorStatus == RPR_SUCCESS) {
    ...
    return TfStringPrintf("[RPR ERROR] %s -- %s%s",
                          messageOnFail.c_str(),
                          errorStr.c_str(),
                          suffix.c_str());
  }
...
inline bool RprUsdFailed(rpr::Status status,
                         ...
                         rpr::Context *context = nullptr)
{
  ...
  if ((status == RPR_ERROR_UNSUPPORTED || status == RPR_ERROR_UNIMPLEMENTED) &&
      !TfDebug::IsEnabled(RPR_USD_DEBUG_CORE_UNSUPPORTED_ERROR)) {
    return true;
  }

  auto errorMessage =
    RprUsdConstructErrorMessage(status, messageOnFail, file, function, line, context);
  fprintf(stderr, "%s\n", errorMessage.c_str());
  return true;
}
...
class RprUsdError : public std::runtime_error
{
 public:
  ...
  RprUsdError(rpr::Status errorStatus,
              const char *messageOnFail,
              char const *file,
              char const *function,
              size_t line,
...
#endif  // WABI_IMAGING_RPR_USD_ERROR_H
