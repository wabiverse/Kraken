...
TF_DEFINE_ENV_SETTING(RPRUSD_USE_RPRMTLXLOADER, true, "Whether to use RPRMtlxLoader or rprLoadMateriaX");
  ...
  if (m_mtlxDefsDirty) {
    ...
    for (auto &file : materialFiles) {
      ...
      try {
        ...
        TF_RUNTIME_ERROR("Error on parsing of \"%s\": materialX error - %s",
                         file.c_str(),
                         e.what());
      }
    }
...
void RprUsdMaterialRegistry::CommitResources(RprUsdImageCache *imageCache)
{
  ...
  for (size_t i = 0; i < m_textureCommits.size(); ++i) {
    ...
    if (auto rprImage =
          imageCache->GetImage(commit.filepath, commit.colorspace, commit.wrapType, {}, 0)) {
                                      ...
                                      RprUsd_MaterialNetwork *result)
  {
    ...
    for (auto &entry : hdNetworkMap.map) {
      ...
      for (auto &node : hdNetwork.nodes) {
        ...
        auto termIt = std::find(hdNetworkMap.terminals.begin(),
