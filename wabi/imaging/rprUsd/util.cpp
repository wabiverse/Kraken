...
TF_DEFINE_PRIVATE_TOKENS(RprUsdUDIMTags, ((arnoldTag, "<UDIM>"))((sidefxTag, "%(UDIM)d")));
...
std::shared_ptr<RprUsdTextureData> RprUsdTextureData::New(std::string const &filepath)
{
  ...
  size_t dataSize = ret->_hioStorageSpec.width * ret->_hioStorageSpec.height *
                    HioGetDataSizeOfFormat(ret->_hioStorageSpec.format);
