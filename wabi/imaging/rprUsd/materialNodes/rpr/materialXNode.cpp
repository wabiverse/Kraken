...
class RprUsd_RprMaterialXNode : public RprUsd_MaterialNode
{
 public:
  ...
  RprUsd_RprMaterialXNode(RprUsd_MaterialBuilderContext *ctx) : m_ctx(ctx) {}
  ...
  bool UpdateNodeOutput()
  {
    ...
    if (m_ctx->mtlxLoader) {
      ...
      try {
        ...
        fprintf(stderr,
                "Failed to parse %s: no such file - %s\n",
                m_mtlxFilepath.c_str(),
                e.what());
      }
        ...
        for (size_t i = 0; i < mtlxPtr->numImageNodes; ++i) {
          ...
          textureCommit.setTextureCallback =
            [retainedImagesPtr, rprImageNode](std::shared_ptr<RprUsdCoreImage> const &image) {
              ...
              if (!RPR_ERROR_CHECK(rprMaterialNodeSetInputImageDataByKey(rprImageNode,
    ...
    rpr::Status status;
    if (!m_mtlxString.empty()) {
      m_surfaceNode.reset(m_ctx->rprContext->CreateMaterialXNode(m_mtlxString.c_str(),
                                                                 basePath.c_str(),
                                                                 0,
                                                                 nullptr,
                                                                 nullptr,
