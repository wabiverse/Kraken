...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (minscale)(maxscale)(in));

/// \class RprUsd_RprDisplaceNode
///
/// The node that maps RPR displacement functionality one-to-one:
///   - `in` expects rpr::MaterialNode as input and corresponds to
///   `rprShapeSetDisplacementMaterial`
///   - `minscale` and `maxscale` inputs are expected to be of float type and correspond to
