...
struct RprUsd_RprNodeOutput : public RprUsdMaterialNodeElement {
  ...
  const char *GetUIName() const override
  {
    return !uiName.empty() ? uiName.c_str() : nullptr;
  };
  const char *GetDocString() const override
  {
    return !docString.empty() ? docString.c_str() : nullptr;
  };
