...
RprUsd_HoudiniPrincipledNode::RprUsd_HoudiniPrincipledNode(RprUsd_MaterialBuilderContext *ctx,
  ...
  : RprUsd_BaseRuntimeNode(RPR_MATERIAL_NODE_UBERV2, ctx)
{
  ...
  populateParameter(
    HoudiniPrincipledShaderTokens->transmissionColor,
    {RPR_MATERIAL_INPUT_UBER_REFRACTION_COLOR, RPR_MATERIAL_INPUT_UBER_REFRACTION_ABSORPTION_COLOR});
...
bool IsHoudiniPrincipledShaderHydraNode(HdSceneDelegate *delegate, SdfPath const &nodePath, bool *isSurface)
{
  ...
  auto implementationSource = delegate->Get(nodePath,
