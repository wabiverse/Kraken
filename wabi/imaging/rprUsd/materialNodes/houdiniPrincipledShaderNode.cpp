...
#include "wabi/imaging/rprUsd/materialHelpers.h"
#include "wabi/imaging/rprUsd/materialMappings.h"
...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    (basecolor)(albedomult)(ior)((roughness,
                                  "rough"))((anisotropy,
        ...
        "transdist"))((subsurface, "sss"))((subsurfaceDistance, "sssdist"))((
        subsurfaceModel,
        "sssmodel"))((subsurfaceColor, "ssscolor"))((subsurfacePhase,
                                                     "sssphase"))(sheen)((sheenTint,
                                                                          "sheentint"))((
        emissionColor,
        "emitcolor"))((emissionIntensity, "emitint"))((opacity, "opac"))((
        opacityColor,
        "opaccolor"))(baseNormal)((baseNormalScale,
                                   "baseNormal_scale"))(coatNormal)((coatNormalScale,
                                                                     "coatNormal_scale"))((
        baseNormalEnable,
        "baseBumpAndNormal_enable"))((
        baseNormalType,
        "baseBumpAndNormal_type"))(separateCoatNormals)((doubleSided,
                                                         "frontface"))((displacementEnable,
                                                                        "dispTex_enable"))((
        displacementTexture,
        "dispTex_texture"))((displacementOffset, "dispTex_offset"))((displacementScale,
                                                                     "dispTex_scale"))((
        displacementColorSpace,
        "dispTex_colorSpace"))((displacementChannel, "dispTex_channel"))((displacementWrap,
                                                                          "dispTex_wrap"))((
        displacementType,
        "dispTex_type"))((infoSourceAsset,
                          "info:sourceAsset"))((infoImplementationSource,
...
template<typename T>
T GetParameter(TfToken const &name,
    ...
    : RprUsd_BaseRuntimeNode(RPR_MATERIAL_NODE_UBERV2, ctx)
{
  ...
  auto getTextureValue = [&](TfToken const &baseParameter,
                             bool forceLinearSpace = false) -> VtValue {
    ...
    if (baseParameter == HoudiniPrincipledShaderTokens->baseNormal) {
      // baseNormal's texture enabled by baseBumpAndNormal_enable parameter unlike all other
      // textures by *_useTexture
      useTexture = true;
    }
    ...
    return GetTextureOutput(
        fileAsset, wrapMode, scale, 0.0f, forceLinearSpace, textureOutputId, uvTexture);
  };
  ...
  auto setInputs = [this](VtValue const &value,
                          std::vector<rpr::MaterialNodeInput> const &rprInputs) {
    for (auto rprInput : rprInputs) {
      SetRprInput(m_rprNode.get(), rprInput, value);
    }
  };

  auto populateParameter = [&](TfToken const &paramName,
                               std::vector<rpr::MaterialNodeInput> const &rprInputs) {
  ...
  if (!basecolorParam.value.IsEmpty()) {
    ...
    setInputs(albedoMultiplyNode->GetOutput(),
              {RPR_MATERIAL_INPUT_UBER_DIFFUSE_COLOR,
               ...
               RPR_MATERIAL_INPUT_UBER_COATING_COLOR,
               RPR_MATERIAL_INPUT_UBER_COATING_TRANSMISSION_COLOR,
               RPR_MATERIAL_INPUT_UBER_SHEEN});
  }

  populateParameter(HoudiniPrincipledShaderTokens->ior,
                    {RPR_MATERIAL_INPUT_UBER_REFRACTION_IOR, RPR_MATERIAL_INPUT_UBER_COATING_IOR});
  populateParameter(HoudiniPrincipledShaderTokens->roughness,
                    {RPR_MATERIAL_INPUT_UBER_DIFFUSE_ROUGHNESS,
                     RPR_MATERIAL_INPUT_UBER_REFLECTION_ROUGHNESS,
                     RPR_MATERIAL_INPUT_UBER_REFRACTION_ROUGHNESS});

  populateParameter(HoudiniPrincipledShaderTokens->anisotropy,
                    {RPR_MATERIAL_INPUT_UBER_REFLECTION_ANISOTROPY});
  populateParameter(HoudiniPrincipledShaderTokens->anisotropyDirection,
                    {RPR_MATERIAL_INPUT_UBER_REFLECTION_ANISOTROPY_ROTATION});

  populateParameter(HoudiniPrincipledShaderTokens->coatRoughness,
                    {RPR_MATERIAL_INPUT_UBER_COATING_ROUGHNESS});
  auto coatParam = getParameterValue(HoudiniPrincipledShaderTokens->coat);
  if (!coatParam.value.IsEmpty()) {
    auto coatingWeightNode = AddAuxiliaryNode(
        RprUsd_RprArithmeticNode::Create(RPR_MATERIAL_NODE_OP_GREATER, m_ctx));
    coatingWeightNode->SetInput(0, coatParam.value);
    coatingWeightNode->SetInput(1, VtValue(0.0f));

    SetRprInput(
        m_rprNode.get(), RPR_MATERIAL_INPUT_UBER_COATING_WEIGHT, coatingWeightNode->GetOutput());
  ...
  if (!subsurfaceParam.value.IsEmpty()) {
    ...
    SetRprInput(
        m_rprNode.get(), RPR_MATERIAL_INPUT_UBER_BACKSCATTER_WEIGHT, subsurfaceParam.value);
  }
  populateParameter(HoudiniPrincipledShaderTokens->subsurfaceDistance,
                    {RPR_MATERIAL_INPUT_UBER_SSS_SCATTER_DISTANCE});
  populateParameter(
      HoudiniPrincipledShaderTokens->subsurfaceColor,
  ...
  if (subsurfaceModel == "full") {
    ...
    RPR_ERROR_CHECK(m_rprNode->SetInput(RPR_MATERIAL_INPUT_UBER_SSS_MULTISCATTER, 0u),
                    "Failed to set sss multiscatter input");
    populateParameter(HoudiniPrincipledShaderTokens->subsurfacePhase,
                      {RPR_MATERIAL_INPUT_UBER_SSS_SCATTER_DIRECTION});
  }

  populateParameter(HoudiniPrincipledShaderTokens->sheen, {RPR_MATERIAL_INPUT_UBER_SHEEN_WEIGHT});
  populateParameter(HoudiniPrincipledShaderTokens->sheenTint,
                    {RPR_MATERIAL_INPUT_UBER_SHEEN_TINT});

  auto emissionColorParam = getParameterValue(HoudiniPrincipledShaderTokens->emissionColor);
  if (!emissionColorParam.value.IsEmpty()) {
    auto emissiveWeightNode = AddAuxiliaryNode(
        RprUsd_RprArithmeticNode::Create(RPR_MATERIAL_NODE_OP_GREATER, m_ctx));
    emissiveWeightNode->SetInput(0, emissionColorParam.value);
    emissiveWeightNode->SetInput(1, VtValue(0.0f));
    SetRprInput(
        m_rprNode.get(), RPR_MATERIAL_INPUT_UBER_EMISSION_WEIGHT, emissiveWeightNode->GetOutput());

    auto emissiveIntensityNode = AddAuxiliaryNode(
        RprUsd_RprArithmeticNode::Create(RPR_MATERIAL_NODE_OP_MUL, m_ctx));
    ...
    SetRprInput(m_rprNode.get(),
                RPR_MATERIAL_INPUT_UBER_EMISSION_COLOR,
                emissiveIntensityNode->GetOutput());
  }
  ...
  if (GetParameter(HoudiniPrincipledShaderTokens->baseNormalEnable, params, 0)) {
    std::vector<rpr::MaterialNodeInput> rprInputs = {RPR_MATERIAL_INPUT_UBER_DIFFUSE_NORMAL,
                                                     RPR_MATERIAL_INPUT_UBER_REFLECTION_NORMAL,
                                                     RPR_MATERIAL_INPUT_UBER_REFRACTION_NORMAL};
    ...
    if (GetParameter(HoudiniPrincipledShaderTokens->separateCoatNormals, params, 0)) {
      ...
      if (!coatNormal.IsEmpty()) {
        auto normalMapNode = AddAuxiliaryNode(
            std::make_unique<RprUsd_BaseRuntimeNode>(RPR_MATERIAL_NODE_NORMAL_MAP, m_ctx));
    ...
    if (!baseNormal.IsEmpty()) {
      ...
      normalMapNode->SetInput(
          RprUsdMaterialNodeInputTokens->scale,
          VtValue(GetParameter(HoudiniPrincipledShaderTokens->baseNormalScale, params, 1.0f)));
  ...
  if (!transparencyParam.value.IsEmpty()) {
    ...
    RPR_ERROR_CHECK(m_rprNode->SetInput(RPR_MATERIAL_INPUT_UBER_REFRACTION_CAUSTICS, 1),
                    "Failed to set caustics input");
    SetRprInput(
        m_rprNode.get(), RPR_MATERIAL_INPUT_UBER_REFRACTION_WEIGHT, transparencyParam.value);

    auto diffuseWeightNode = AddAuxiliaryNode(
        RprUsd_RprArithmeticNode::Create(RPR_MATERIAL_NODE_OP_SUB, m_ctx));
    diffuseWeightNode->SetInput(0, VtValue(1.0f));
    diffuseWeightNode->SetInput(1, transparencyParam.value);
    SetRprInput(
        m_rprNode.get(), RPR_MATERIAL_INPUT_UBER_DIFFUSE_WEIGHT, diffuseWeightNode->GetOutput());
  }
  populateParameter(HoudiniPrincipledShaderTokens->transmissionColor,
                    {RPR_MATERIAL_INPUT_UBER_REFRACTION_COLOR,
                     RPR_MATERIAL_INPUT_UBER_REFRACTION_ABSORPTION_COLOR});
  populateParameter(HoudiniPrincipledShaderTokens->transmissionDistance,
  ...
  if (useBaseColorTextureAlpha) {
    ...
    if (!opacity.IsEmpty()) {
      auto transparencyNode = AddAuxiliaryNode(
          RprUsd_RprArithmeticNode::Create(RPR_MATERIAL_NODE_OP_MUL, m_ctx));
      ...
      transparencyNode = AddAuxiliaryNode(
          RprUsd_RprArithmeticNode::Create(RPR_MATERIAL_NODE_OP_SUB, m_ctx));
  ...
  if (!hasTransparency) {
    ...
    populateParameter(
        HoudiniPrincipledShaderTokens->metallic,
        {RPR_MATERIAL_INPUT_UBER_REFLECTION_METALNESS, RPR_MATERIAL_INPUT_UBER_COATING_METALNESS});
  }

  RPR_ERROR_CHECK(m_rprNode->SetInput(RPR_MATERIAL_INPUT_UBER_REFLECTION_MODE, iorMode),
                  "Failed to set ior mode input");
  RPR_ERROR_CHECK(m_rprNode->SetInput(RPR_MATERIAL_INPUT_UBER_COATING_MODE, iorMode),
                  "Failed to set ior mode input");

  if (dispParamsPtr) {
    auto &dispParams = *dispParamsPtr;
    if (GetParameter(HoudiniPrincipledShaderTokens->displacementEnable, dispParams, 0)) {
      auto dispType = GetParameter<std::string>(HoudiniPrincipledShaderTokens->displacementType,
                                                dispParams);
      if (dispType == "vectordisp") {
        TF_RUNTIME_ERROR("Vector displacement unsupported");
      }
      else {
        auto dispTexturePath = GetParameter<SdfAssetPath>(
            HoudiniPrincipledShaderTokens->displacementTexture, dispParams);
        if (!dispTexturePath.GetResolvedPath().empty()) {
          m_displacementOutput = GetTextureOutput(
              dispTexturePath,
              GetParameter<std::string>(HoudiniPrincipledShaderTokens->displacementWrap,
                                        dispParams),
              ...
              GetParameter(HoudiniPrincipledShaderTokens->displacementColorSpace,
                           dispParams,
                           std::string("linear")) == "linear",
              ToUsdUVTextureOutputId(GetParameter(
                  HoudiniPrincipledShaderTokens->displacementChannel, dispParams, 0)));
        }
      }
    }
  }
...
VtValue RprUsd_HoudiniPrincipledNode::GetTextureOutput(SdfAssetPath const &path,
...
{
  if (path.GetResolvedPath().empty() && path.GetAssetPath().empty()) {
    return VtValue();
  }
  ...
  if (forceLinearSpace) {
    uvTextureParams.emplace(RprUsd_UsdUVTextureTokens->sourceColorSpace,
                            RprUsd_UsdUVTextureTokens->srgblinear);
  }
  ...
  if (outputId.IsEmpty()) {
    ...
    auto luminanceNode = AddAuxiliaryNode(
        RprUsd_RprArithmeticNode::Create(RPR_MATERIAL_NODE_OP_DOT3, m_ctx));
                                        ...
                                        bool *isSurface)
{
  ...
  if (RprUsdMaterialRegistry::GetInstance().GetMaterialNetworkSelector() !=
      HoudiniPrincipledShaderTokens->karma) {
    return false;
  }

  auto implementationSource = delegate->Get(
      nodePath, HoudiniPrincipledShaderTokens->infoImplementationSource);
  if (implementationSource.IsHolding<TfToken>() &&
    ...
    if (nodeAsset.IsHolding<SdfAssetPath>()) {
      ...
      if (!asset.GetAssetPath().empty()) {
        ...
        if (asset.GetAssetPath().compare(
                0, principledShaderDef.size(), principledShaderDef.c_str()) == 0) {
