...
bool RprUsd_UsdPreviewSurface::SetInput(TfToken const &inputId, VtValue const &value)
{
  if (UsdPreviewSurfaceTokens->diffuseColor == inputId) {
    ...
    TF_CODING_ERROR(
      "Unknown UsdPreviewSurface parameter %s: %s", inputId.GetText(), value.GetTypeName().c_str());
    return false;
  }
...
RprUsd_UsdUVTexture::RprUsd_UsdUVTexture(RprUsd_MaterialBuilderContext *ctx,
                                         std::map<TfToken, VtValue> const &hydraParameters)
  : m_ctx(ctx)
{
  ...
  if (wrapSIt != hydraParameters.end())
    wrapS = GetWrapType(wrapSIt->second);
...
TF_DEFINE_PRIVATE_TOKENS(UsdPrimvarReaderTokens, (varname));

RprUsd_UsdPrimvarReader::RprUsd_UsdPrimvarReader(RprUsd_MaterialBuilderContext *ctx,
                                                 std::map<TfToken, VtValue> const &hydraParameters)
  : RprUsd_BaseRuntimeNode(RPR_MATERIAL_NODE_INPUT_LOOKUP, ctx)
{
  ...
  //
  // TODO: I think that we can use buffer sampler node to fully implement primvar reader but this
  // is only an idea
  //
...
bool RprUsd_UsdPrimvarReader::SetInput(TfToken const &inputId, VtValue const &value)
{
...
TF_DEFINE_PRIVATE_TOKENS(UsdTransform2dTokens, (rotation)(scale)(translation));

RprUsd_UsdTransform2d::RprUsd_UsdTransform2d(RprUsd_MaterialBuilderContext *ctx,
                                             std::map<TfToken, VtValue> const &hydraParameters)
  : m_ctx(ctx)
{
...
template<typename T>
RprUsd_MaterialNode *RprUsd_CreateUsdNode(RprUsd_MaterialBuilderContext *ctx,
