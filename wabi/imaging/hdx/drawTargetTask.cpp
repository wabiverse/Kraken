...
static HdCompareFunction HdxDrawTargetTask_GetResolvedDepthFunc(HdCompareFunction depthFunc,
                                                                HdDepthPriority priority)
{
  static const HdCompareFunction ResolvedDepthFunc[HdDepthPriorityCount][HdCmpFuncLast] = {
    {
     HdCmpFuncNever, HdCmpFuncLess,
     HdCmpFuncEqual, HdCmpFuncLEqual,
     HdCmpFuncGreater, HdCmpFuncNotEqual,
     HdCmpFuncGEqual, HdCmpFuncAlways,
     },
    {
     HdCmpFuncNever,              HdCmpFuncGEqual,
     HdCmpFuncEqual, HdCmpFuncGreater,
     HdCmpFuncLEqual,   HdCmpFuncNotEqual,
     HdCmpFuncLess, HdCmpFuncAlways,
     },
  };
  ...
  const HdPhDrawTarget *const drawTarget)
{
  ...
  const double aspect = resolution[1] != 0.0 ? resolution[0] / resolution[1] : 1.0;

  const HdPhDrawTargetRenderPassState *const state = drawTarget->GetDrawTargetRenderPassState();
...
void HdxDrawTargetTask::Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  HD_TRACE_FUNCTION();
  ...
  for (_RenderPassInfo &renderPassInfo : _renderPassesInfo) {
    ...
    _UpdateRenderPass(&renderPassInfo);
  }

  *dirtyBits = HdChangeTracker::Clean;
}

void HdxDrawTargetTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  // Not calling HdPh_RenderPass::Prepare(...) here since
  // HdxDrawTargetRenderPass::Prepare didn't do so.
}

void HdxDrawTargetTask::Execute(HdTaskContext *ctx)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
const TfTokenVector &HdxDrawTargetTask::GetRenderTags() const
{
  return _renderTags;
}
// --------------------------------------------------------------------------- //
