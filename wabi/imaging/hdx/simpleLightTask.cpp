...
std::vector<GfMatrix4d> HdxSimpleLightTask::_ComputeShadowMatrices(
  ...
  HdxShadowMatrixComputationSharedPtr const &computation) const
{
  ...
  if (_framing.IsValid()) {
    CameraUtilConformWindowPolicy const policy = _overrideWindowPolicy.first ?
...
void HdxSimpleLightTask::Sync(HdSceneDelegate *delegate,
                              HdTaskContext *ctx,
                              HdDirtyBits *dirtyBits)
{
  ...
  (*ctx)[HdxTokens->lightingShader] = std::dynamic_pointer_cast<HdStLightingShader>(
    _lightingShader);
  ...
  _lightIds.clear();
  _numLights = _AppendLightsOfType(renderIndex,
                                   lightTypes,
                                   _lightIncludePaths,
                                   _lightExcludePaths,
                                   &_lightIds);
  ...
  for (TfToken const &lightType : lightTypes) {
    ...
    for (SdfPath const &lightPath : lightPathsIt->second) {
      ...
      const VtValue vLightShadowParams = light->Get(HdLightTokens->shadowParams);
      const HdxShadowParams lightShadowParams = vLightShadowParams.GetWithDefault<HdxShadowParams>(
        HdxShadowParams());
  ...
  if (shadowIndex > -1) {
    for (size_t lightId = 0; lightId < _numLights; ++lightId) {
      if (lightId >= _glfSimpleLights.size()) {
        TF_CODING_ERROR("lightId %zu >= _glfSimpleLights.size() %zu",
                        lightId,
                        _glfSimpleLights.size());
        continue;
      }
...
void HdxSimpleLightTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  GlfSimpleLightingContextRefPtr const &lightingContext = _lightingShader->GetLightingContext();
  if (!TF_VERIFY(lightingContext)) {
    return;
  ...
  if (!_lightingBar) {
    ...
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->useLighting, HdTupleType{HdTypeBool, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->useColorMaterialDiffuse,
    ...
    _lightingShader->AddBufferBinding(HdBindingRequest(HdBinding::UBO,
  ...
  // Add lighting buffer sources
  {
    HdBufferSourceSharedPtrVector sources = {
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->useLighting,
                                         VtValue(lightingContext->GetUseLighting())),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->useColorMaterialDiffuse,
                                         VtValue(lightingContext->GetUseColorMaterialDiffuse()))};

    hdStResourceRegistry->AddSources(_lightingBar, std::move(sources));
  }

  size_t const numLights = static_cast<size_t>(lightingContext->GetNumLightsUsed());
  size_t const numShadows = static_cast<size_t>(lightingContext->ComputeNumShadowsUsed());

  // Allocate light sources BAR
  if (!_lightSourcesBar) {
    HdBufferSpecVector bufferSpecs;

    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->position, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->ambient, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->diffuse, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->specular, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->spotDirection,
                             HdTupleType{HdTypeFloatVec3, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->spotCutoff, HdTupleType{HdTypeFloat, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->spotFalloff, HdTupleType{HdTypeFloat, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->attenuation,
                             HdTupleType{HdTypeFloatVec3, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->worldToLightTransform,
                             HdTupleType{HdTypeFloatMat4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->shadowIndexStart,
                             HdTupleType{HdTypeInt32, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->shadowIndexEnd,
                             HdTupleType{HdTypeInt32, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->hasShadow, HdTupleType{HdTypeBool, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->isIndirectLight,
                             HdTupleType{HdTypeBool, 1});
  ...
  if (!_shadowsBar && useShadows) {
    ...
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->worldToShadowMatrix,
                             HdTupleType{HdTypeFloatMat4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->shadowToWorldMatrix,
                             HdTupleType{HdTypeFloatMat4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->blur, HdTupleType{HdTypeFloat, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->bias, HdTupleType{HdTypeFloat, 1});

    _shadowsBar = hdStResourceRegistry->AllocateUniformBufferArrayRange(
      HdxSimpleLightTaskTokens->lighting,
      bufferSpecs,
      HdBufferArrayUsageHint());
  }
  ...
  if (numShadows != 0) {
    _lightingShader->AddBufferBinding(HdBindingRequest(HdBinding::UBO,
                                                       HdxSimpleLightTaskTokens->shadow,
                                                       _shadowsBar,
                                                       /*interleaved=*/true,
                                                       /*writable*/ false,
                                                       numShadows,
  ...
  // Add light and shadow buffer sources
  {
    ...
    for (size_t i = 0; i < numLights; ++i) {
      ...
      worldToLightTransform[i] = GfMatrix4f(lights[i].GetTransform().GetInverse());
      ...
      if (hasShadow[i]) {
        for (int j = shadowIndexStart[i]; j <= shadowIndexEnd[i]; ++j) {
          worldToShadowMatrix[j] = GfMatrix4f(shadows->GetWorldToShadowMatrix(j));
          shadowToWorldMatrix[j] = GfMatrix4f(worldToShadowMatrix[j].GetInverse());
    ...
    HdBufferSourceSharedPtrVector sources = {
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->position, VtValue(position)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->ambient, VtValue(ambient)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->diffuse, VtValue(diffuse)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->specular, VtValue(specular)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->spotDirection,
                                         VtValue(spotDirection)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->spotCutoff,
                                         VtValue(spotCutoff)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->spotFalloff,
                                         VtValue(spotFalloff)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->attenuation,
                                         VtValue(attenuation)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->worldToLightTransform,
                                         VtValue(worldToLightTransform)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->shadowIndexStart,
                                         VtValue(shadowIndexStart)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->shadowIndexEnd,
                                         VtValue(shadowIndexEnd)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->hasShadow, VtValue(hasShadow)),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->isIndirectLight,
                                         VtValue(isIndirectLight)),
    };

    hdStResourceRegistry->AddSources(_lightSourcesBar, std::move(sources));

    if (useShadows) {
      HdBufferSourceSharedPtrVector shadowSources = {
        std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->worldToShadowMatrix,
                                           VtValue(worldToShadowMatrix)),
        std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->shadowToWorldMatrix,
                                           VtValue(shadowToWorldMatrix)),
        std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->blur, VtValue(blur)),
        std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->bias, VtValue(bias)),
      };

      hdStResourceRegistry->AddSources(_shadowsBar, std::move(shadowSources));
    }
  }
  ...
  if (!_materialBar) {
    ...
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->diffuse, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->specular, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->emission, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->sceneColor,
                             HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdxSimpleLightTaskTokens->shininess, HdTupleType{HdTypeFloat, 1});
  ...
  // Add material buffer sources
  {
    ...
    HdBufferSourceSharedPtrVector sources = {
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->ambient,
                                         VtValue(material.GetAmbient())),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->diffuse,
                                         VtValue(material.GetDiffuse())),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->specular,
                                         VtValue(material.GetSpecular())),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->emission,
                                         VtValue(material.GetEmission())),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->sceneColor,
                                         VtValue(lightingContext->GetSceneAmbient())),
      std::make_shared<HdVtBufferSource>(HdxSimpleLightTaskTokens->shininess,
...
void HdxSimpleLightTask::Execute(HdTaskContext *ctx)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
}

size_t HdxSimpleLightTask::_AppendLightsOfType(HdRenderIndex &renderIndex,
                                               ...
                                               std::map<TfToken, SdfPathVector> *lights)
{
  ...
  TF_FOR_ALL (it, lightTypes) {
    if (renderIndex.IsSprimTypeSupported(*it)) {
      ...
      HdPrimGather gather;
      gather.Filter(sprimPaths, lightIncludePaths, lightExcludePaths, &lightsLocal);
...
{
  return lhs.cameraPath == rhs.cameraPath && lhs.lightIncludePaths == rhs.lightIncludePaths &&
         lhs.lightExcludePaths == rhs.lightExcludePaths && lhs.material == rhs.material &&
         lhs.sceneAmbient == rhs.sceneAmbient && lhs.enableShadows == rhs.enableShadows;
}
...
{
  out << pv.shadowMatrix << " " << pv.resolution << " " << pv.bias << " " << pv.blur << " "
      << pv.enabled;
  return out;
}
...
{
  return lhs.shadowMatrix == rhs.shadowMatrix && lhs.resolution == rhs.resolution &&
         lhs.bias == rhs.bias && lhs.blur == rhs.blur && lhs.enabled == rhs.enabled;
}
