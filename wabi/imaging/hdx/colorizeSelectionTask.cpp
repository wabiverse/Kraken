...
bool HdxColorizeSelectionTask::IsConverged() const
{
  return _converged;
}
                                     ...
                                     HdDirtyBits *dirtyBits)
{
  ...
  if (!_compositor) {
    _compositor = std::make_unique<HdxFullscreenShader>(_GetHgi(), "ColorizeSelection");
  }
...
void HdxColorizeSelectionTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  if (sel && sel->GetVersion() != _lastVersion) {
    ...
    _hasSelection = sel->GetSelectionOffsetBuffer(renderIndex,
                                                  _params.enableSelection,
                                                  &_selectionOffsets);
  }
}

void HdxColorizeSelectionTask::Execute(HdTaskContext *ctx)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  HgiShaderFunctionAddConstantParam(&fragDesc, "texelSize", "vec2");
  HgiShaderFunctionAddConstantParam(&fragDesc, "enableOutline", "int");
  HgiShaderFunctionAddConstantParam(&fragDesc, "radius", "int");
  ...
  _CreateTexture(_primId->GetWidth(), _primId->GetHeight(), HdFormatUNorm8Vec4, _outputBuffer);
...
GfVec4f HdxColorizeSelectionTask::_GetColorForMode(int mode) const
{
  if (mode == 0) {
...
void HdxColorizeSelectionTask::_ColorizeSelection()
{
...
bool HdxColorizeSelectionTask::_UpdateParameterBuffer()
{
  _ParameterBuffer pb;
...
void HdxColorizeSelectionTask::_CreateTexture(int width, int height, HdFormat format, void *data)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
