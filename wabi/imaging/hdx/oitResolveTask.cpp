...
#include "wabi/imaging/hdSt/imageShaderRenderPass.h"
...
void HdxOitResolveTask::Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  ...
  if (!_renderPassState) {
    ...
    _renderPassShader = std::make_shared<HdStRenderPassShader>(HdxPackageOitResolveImageShader());
  ...
  if ((*dirtyBits) & HdChangeTracker::DirtyParams) {
    ...
    _renderPassState->SetUseAovMultiSample(params.useAovMultiSample);
    _renderPassState->SetResolveAovMultiSample(params.resolveAovMultiSample);
  }
...
void HdxOitResolveTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  ...
  if (!_renderPass) {
    ...
    _renderPass = std::make_shared<HdSt_ImageShaderRenderPass>(renderIndex, collection);
...
bool operator==(HdxOitResolveTaskParams const &lhs, HdxOitResolveTaskParams const &rhs)
{
...
std::ostream &operator<<(std::ostream &out, HdxOitResolveTaskParams const &p)
{
  out << "OitResolveTask Params: (...) " << p.useAovMultiSample << " " << p.resolveAovMultiSample;
  return out;
}
