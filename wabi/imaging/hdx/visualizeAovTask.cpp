...
bool HdxVisualizeAovTask::_UpdateVizKernel(TfToken const &aovName)
{
...
bool HdxVisualizeAovTask::_CreateShaderResources(HgiTextureDesc const &inputAovTextureDesc)
{
  ...
  const HioGlslfx glslfx(HdxPackageVisualizeAovShader(), HioGlslfxTokens->defVal);
  ...
  TfToken const &mixin = _GetFragmentMixin();
  {
    ...
    HgiShaderFunctionAddTexture(&fragDesc,
                                _GetTextureIdentifierForShader().GetString(),
                                /*dimensions*/ 2,
                                inputAovTextureDesc.format);
...
bool HdxVisualizeAovTask::_CreateResourceBindings(HgiTextureHandle const &inputAovTexture)
{
...
void HdxVisualizeAovTask::_DestroyShaderProgram()
{
  if (!_shaderProgram)
    return;
...
void HdxVisualizeAovTask::_PrintCompileErrors()
{
  if (!_shaderProgram)
    return;
...
void HdxVisualizeAovTask::_UpdateMinMaxDepth(HgiTextureHandle const &inputAovTexture)
{
...
void HdxVisualizeAovTask::_ApplyVisualizationKernel(HgiTextureHandle const &outputTexture)
{
...
{
  out << "HdxVisualizeAovTaskParams Params: " << pv.aovName;
  return out;
}
