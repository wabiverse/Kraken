 ...
 */
//
#include "wabi/imaging/hdx/unitTestUtils.h"
#include "wabi/wabi.h"
...
namespace
{
  ...
  static size_t _GetPartialHitHash(HdxPickHit const &hit)
  {
    size_t hash = 0;
  ...
  static AggregatedHits _AggregateHits(HdxPickHitVector const &allHits)
  {
  ...
  static void _ProcessHit(AggregatedHit const &aHit,
...
namespace HdxUnitTestUtils
{
  ...
  void Marquee::DestroyGLResources()
  {
    glDeleteProgram(_program);
    glDeleteBuffers(1, &_vbo);
  }

  void Marquee::Draw(float width, float height, GfVec2f const &startPos, GfVec2f const &endPos)
  {
    glDisable(GL_DEPTH_TEST);
    glUseProgram(_program);

    GfVec2f s(2 * startPos[0] / width - 1, 1 - 2 * startPos[1] / height);
    GfVec2f e(2 * endPos[0] / width - 1, 1 - 2 * endPos[1] / height);
    float pos[] = {s[0], s[1], e[0], s[1], e[0], e[1], s[0], e[1], s[0], s[1]};
