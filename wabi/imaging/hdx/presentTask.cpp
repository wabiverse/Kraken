...
/*static*/
bool HdxPresentTask::IsFormatSupported(HgiFormat aovFormat)
{
...
void HdxPresentTask::_Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  HD_TRACE_FUNCTION();
...
{
  out << "PresentTask Params: (...) " << pv.dstApi;
  return out;
}

bool operator==(const HdxPresentTaskParams &lhs, const HdxPresentTaskParams &rhs)
{
  return lhs.dstApi == rhs.dstApi && lhs.dstFramebuffer == rhs.dstFramebuffer &&
         lhs.dstRegion == rhs.dstRegion && lhs.enabled == rhs.enabled;
}
