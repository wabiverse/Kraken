...
HdxColorCorrectionTask::HdxColorCorrectionTask(HdSceneDelegate *delegate, SdfPath const &id)
  : HdxTask(id),
    _indexBuffer(),
    _vertexBuffer(),
    _texture3dLUT(),
...
bool HdxColorCorrectionTask::_GetUseOcio() const
{
...
bool HdxColorCorrectionTask::_CreateShaderResources()
{
  ...
  const HioGlslfx glslfx(HdxPackageColorCorrectionShader(), HioGlslfxTokens->defVal);
...
bool HdxColorCorrectionTask::_CreateResourceBindings(HgiTextureHandle const &aovTexture)
{
...
void HdxColorCorrectionTask::_ApplyColorCorrection(HgiTextureHandle const &aovTexture)
{
...
void HdxColorCorrectionTask::_DestroyShaderProgram()
{
  if (!_shaderProgram)
    return;
...
void HdxColorCorrectionTask::_PrintCompileErrors()
{
  if (!_shaderProgram)
    return;
