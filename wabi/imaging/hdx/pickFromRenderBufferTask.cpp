...
void HdxPickFromRenderBufferTask::_Sync(HdSceneDelegate *delegate,
                                        HdTaskContext *ctx,
...
void HdxPickFromRenderBufferTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
GfMatrix4d HdxPickFromRenderBufferTask::_ComputeProjectionMatrix() const
{
  ...
  if (_params.framing.IsValid()) {
    const CameraUtilConformWindowPolicy policy = _params.overrideWindowPolicy.has_value() ?
                                                     _params.overrideWindowPolicy.value() :
...
void HdxPickFromRenderBufferTask::Execute(HdTaskContext *ctx)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  HdxPickResult result(reinterpret_cast<int32_t *>(primIdPtr),
