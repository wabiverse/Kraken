...
void HdxPickTask::_InitIfNeeded(GfVec2i const &size)
{
  ...
  // so we access it consistently.
  {
    ...
    _drawTarget->AddAttachment("primId", GL_RGBA, GL_UNSIGNED_BYTE, GL_RGBA8);
    _drawTarget->AddAttachment("instanceId", GL_RGBA, GL_UNSIGNED_BYTE, GL_RGBA8);
    _drawTarget->AddAttachment("elementId", GL_RGBA, GL_UNSIGNED_BYTE, GL_RGBA8);
...
void HdxPickTask::Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  ...
  HdRenderPassStateSharedPtr states[] = {_pickableRenderPassState, _occluderRenderPassState};
...
void HdxPickTask::Execute(HdTaskContext *ctx)
{
  ...
  glBindTexture(GL_TEXTURE_2D, drawTarget->GetAttachments().at("elementId")->GetGlTextureName());
  glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, &elementIds[0]);

  glBindTexture(GL_TEXTURE_2D, drawTarget->GetAttachments().at("edgeId")->GetGlTextureName());
  glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, &edgeIds[0]);

  glBindTexture(GL_TEXTURE_2D, drawTarget->GetAttachments().at("pointId")->GetGlTextureName());
  glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, &pointIds[0]);

  glBindTexture(GL_TEXTURE_2D, drawTarget->GetAttachments().at("neye")->GetGlTextureName());
  ...
  glGetTexImage(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, GL_FLOAT, &depths[0]);
