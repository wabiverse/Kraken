...
static HdRenderPassStateSharedPtr _InitIdRenderPassState(HdRenderIndex *index)
{
  ...
  if (HdStRenderPassState *extendedState = dynamic_cast<HdStRenderPassState *>(rps.get())) {
    extendedState->SetRenderPassShader(
      std::make_shared<HdStRenderPassShader>(HdxPackageRenderPassPickingShader()));
  }
...
static SdfPath _GetAovPath(TfToken const &aovName)
{
...
void HdxPickTask::_InitIfNeeded()
{
  ...
  if (_pickableRenderPass == nullptr || _occluderRenderPass == nullptr ||
    ...
    _pickableRenderPass = _index->GetRenderDelegate()->CreateRenderPass(&*_index, col);
    _occluderRenderPass = _index->GetRenderDelegate()->CreateRenderPass(&*_index, col);
    _widgetRenderPass = _index->GetRenderDelegate()->CreateRenderPass(&*_index, col);
...
void HdxPickTask::_CreateAovBindings()
{
  HdStResourceRegistrySharedPtr hdStResourceRegistry =
    std::static_pointer_cast<HdStResourceRegistry>(_index->GetResourceRegistry());

  HdRenderDelegate const *renderDelegate = _index->GetRenderDelegate();

  GfVec3i dimensions(_contextParams.resolution[0], _contextParams.resolution[1], 1);

  bool const stencilReadback = _hgi->GetCapabilities()->IsSet(
    HgiDeviceCapabilitiesBitsStencilReadback);

  _depthToken = stencilReadback ? HdAovTokens->depthStencil : HdAovTokens->depth;

  // Generated renderbuffers
  TfTokenVector const _aovOutputs{HdAovTokens->primId,
  ...
  for (size_t i = 0; i < _aovOutputs.size(); ++i) {
    ...
    HdAovDescriptor aovDesc = renderDelegate->GetDefaultAovDescriptor(aovOutput);
    ...
    if (HdAovHasDepthSemantic(aovOutput) || HdAovHasDepthStencilSemantic(aovOutput)) {
  ...
  // don't clear them at all, so that previously-drawn items are retained.
  {
    ...
    HdAovDescriptor depthDesc = renderDelegate->GetDefaultAovDescriptor(HdAovTokens->depth);
    ...
    widgetDepthBinding.renderBufferId = _GetAovPath(_tokens->widgetDepthStencil);
...
void HdxPickTask::_CleanupAovBindings()
{
  if (_index) {
    HdRenderParam *renderParam = _index->GetRenderDelegate()->GetRenderParam();
...
void HdxPickTask::_ResizeOrCreateBufferForAOV(const HdRenderPassAovBinding &aovBinding)

{
  ...
  HdAovDescriptor aovDesc = renderDelegate->GetDefaultAovDescriptor(aovBinding.aovName);

  aovBinding.renderBuffer->Allocate(dimensions, aovDesc.format, false);

  VtValue newResource = aovBinding.renderBuffer->GetResource(false);

  if (!newResource.IsHolding<HgiTextureHandle>()) {
    TF_CODING_ERROR(
      "No texture on render buffer for AOV "
      "%s",
      aovBinding.aovName.GetText());
  }
}
...
bool HdxPickTask::_UseWidgetPass() const
{
  return _allRenderTags != _nonWidgetRenderTags;
}

template<typename T>
HdStTextureUtils::AlignedBuffer<T> HdxPickTask::_ReadAovBuffer(TfToken const &aovName) const
{
  ...
  if (aov.IsHolding<HgiTextureHandle>()) {
    ...
    if (texture) {
      size_t bufferSize = 0;
      return HdStTextureUtils::HgiTextureReadback<T>(_hgi, texture, &bufferSize);
    }
  }
...
HdRenderBuffer const *HdxPickTask::_FindAovBuffer(TfToken const &aovName) const
{
  HdRenderPassAovBindingVector::const_iterator bindingIt = std::find_if(
    _pickableAovBindings.begin(),
    _pickableAovBindings.end(),
...
void HdxPickTask::Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  ...
  bool needStencilConditioning = (_contextParams.depthMaskCallback != nullptr);

  // Calculate the viewport
  GfVec4i viewport(0, 0, _contextParams.resolution[0], _contextParams.resolution[1]);

  const float stepSize = delegate->GetRenderIndex().GetRenderDelegate()->GetRenderSetting<float>(
    HdStRenderSettingsTokens->volumeRaymarchingStepSize,
    HdStVolume::defaultStepSize);
  const float stepSizeLighting =
  ...
  for (auto &state : states) {
    ...
    if (HdStRenderPassState *extState = dynamic_cast<HdStRenderPassState *>(state.get())) {
...
void HdxPickTask::Execute(HdTaskContext *ctx)
{
  GLF_GROUP_FUNCTION();
  ...
  if (_UseOcclusionPass()) {
    _occluderRenderPass->Execute(_occluderRenderPassState, _nonWidgetRenderTags);
    ...
    _pickableAovBindings[_pickableDepthIndex].clearValue = VtValue(GfVec4f(1.0f));
  }

  // Push the changes to the clearValue into the renderPassState.
  _pickableRenderPassState->SetAovBindings(_pickableAovBindings);
  _pickableRenderPass->Execute(_pickableRenderPassState, _nonWidgetRenderTags);
  ...
  if (_UseWidgetPass()) {
    ...
    _widgetRenderPassState->SetAovBindings(_widgetAovBindings);
    _widgetRenderPass->Execute(_widgetRenderPassState, {HdxRenderTagTokens->widget});
  }

  // Capture the result buffers and cast to the appropriate types.
  HdStTextureUtils::AlignedBuffer<int> primIds = _ReadAovBuffer<int>(HdAovTokens->primId);
  HdStTextureUtils::AlignedBuffer<int> instanceIds = _ReadAovBuffer<int>(HdAovTokens->instanceId);
  HdStTextureUtils::AlignedBuffer<int> elementIds = _ReadAovBuffer<int>(HdAovTokens->elementId);
  HdStTextureUtils::AlignedBuffer<int> edgeIds = _ReadAovBuffer<int>(HdAovTokens->edgeId);
  HdStTextureUtils::AlignedBuffer<int> pointIds = _ReadAovBuffer<int>(HdAovTokens->pointId);
  HdStTextureUtils::AlignedBuffer<int> neyes = _ReadAovBuffer<int>(HdAovTokens->Neye);
  HdStTextureUtils::AlignedBuffer<float> depths = _ReadAovBuffer<float>(_depthToken);
  ...
  HdxPickResult result(primIds.get(),
                       instanceIds.get(),
                       elementIds.get(),
                       edgeIds.get(),
                       pointIds.get(),
...
const TfTokenVector &HdxPickTask::GetRenderTags() const
{
...
// HdxPickResult
// -------------------------------------------------------------------------- //
HdxPickResult::HdxPickResult(int const *primIds,
...
HdxPickResult &HdxPickResult::operator=(HdxPickResult &&) = default;
...
size_t HdxPickResult::_GetHash(int index) const
{
  size_t hash = 0;
...
bool HdxPickResult::_IsValidHit(int index) const
{
...
void HdxPickResult::ResolveNearestToCamera(HdxPickHitVector *allHits) const
{
  TRACE_FUNCTION();
...
void HdxPickResult::ResolveNearestToCenter(HdxPickHitVector *allHits) const
{
  TRACE_FUNCTION();
...
void HdxPickResult::ResolveAll(HdxPickHitVector *allHits) const
{
  TRACE_FUNCTION();
...
void HdxPickResult::ResolveUnique(HdxPickHitVector *allHits) const
{
  TRACE_FUNCTION();
...
// HdxPickHit
// -------------------------------------------------------------------------- //
size_t HdxPickHit::GetHash() const
{
  size_t hash = 0;
...
bool operator==(HdxPickHit const &lhs, HdxPickHit const &rhs)
{
  return lhs.objectId == rhs.objectId && lhs.delegateId == rhs.delegateId &&
...
std::ostream &operator<<(std::ostream &out, HdxPickHit const &h)
{
...
bool operator==(HdxPickTaskParams const &lhs, HdxPickTaskParams const &rhs)
{
  return lhs.cullStyle == rhs.cullStyle && lhs.enableSceneMaterials == rhs.enableSceneMaterials;
}
...
std::ostream &operator<<(std::ostream &out, HdxPickTaskParams const &p)
{
  out << "PickTask Params: (...) " << p.cullStyle << " " << p.enableSceneMaterials;
  return out;
}
