...
bool HdxAovInputTask::IsConverged() const
{
  return _converged;
}

void HdxAovInputTask::_Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  HD_TRACE_FUNCTION();
...
void HdxAovInputTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
...
void HdxAovInputTask::Execute(HdTaskContext *ctx)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void HdxAovInputTask::_UpdateIntermediateTexture(HgiTextureHandle &texture, HdRenderBuffer *buffer)
{
  GfVec3i dim(buffer->GetWidth(), buffer->GetHeight(), buffer->GetDepth());
...
{
  out << "AovInputTask Params: (...) " << pv.aovBufferPath << " " << pv.depthBufferPath;
  return out;
}

bool operator==(const HdxAovInputTaskParams &lhs, const HdxAovInputTaskParams &rhs)
{
  return lhs.aovBufferPath == rhs.aovBufferPath && lhs.depthBufferPath == rhs.depthBufferPath;
}
