...
#include "wabi/imaging/hdSt/textureHandle.h"
...
void HdxSkydomeTask::_Sync(HdSceneDelegate *delegate, HdTaskContext *ctx, HdDirtyBits *dirtyBits)
{
  HD_TRACE_FUNCTION();

  if (!_compositor) {
    _compositor = std::make_unique<HdxFullscreenShader>(_GetHgi(), "Skydome");
  }
...
void HdxSkydomeTask::Prepare(HdTaskContext *ctx, HdRenderIndex *renderIndex)
{
  if (_setupTask) {
...
void HdxSkydomeTask::Execute(HdTaskContext *ctx)
{
  HD_TRACE_FUNCTION();
  ...
  HdStRenderPassState *const hdStRenderPassState = dynamic_cast<HdStRenderPassState *>(
    renderPassState.get());
  if (!hdStRenderPassState) {
    return;
  }
  HgiGraphicsCmdsDesc gfxCmdsDesc = hdStRenderPassState->MakeGraphicsCmdsDesc(_renderIndex);
  ...
  if (framing.IsValid()) {
    ...
    viewport = GfVec4i(int(rect[0]), int(rect[1]), int(rect[2]), int(rect[3]));
  }
  ...
  HgiTextureHandle colorDst = gfxCmdsDesc.colorTextures.empty() ? HgiTextureHandle() :
...
HdRenderPassStateSharedPtr HdxSkydomeTask::_GetRenderPassState(HdTaskContext *ctx) const
{
  if (_setupTask) {
...
bool HdxSkydomeTask::_GetSkydomeTexture(HdTaskContext *ctx)
{
  TRACE_FUNCTION();

  // Get the texture from the Lighting Shader
  HdStLightingShaderSharedPtr lightingShader;
  bool haveLightingShader = _GetTaskContextData(ctx, HdxTokens->lightingShader, &lightingShader);
  if (!haveLightingShader) {
    return false;
  }
  HdStSimpleLightingShader *simpleLightingShader = dynamic_cast<HdStSimpleLightingShader *>(
    lightingShader.get());
  ...
  const HdStUvTextureObject *const domeLightTextureObject = dynamic_cast<HdStUvTextureObject *>(
...
void HdxSkydomeTask::_SetFragmentShader()
{
  ...
  _compositor->SetProgram(HdxPackageSkydomeShader(), _tokens->skydomeFrag, fragDesc);
}

bool HdxSkydomeTask::_UpdateParameterBuffer(const GfMatrix4f &invProjMatrix,
