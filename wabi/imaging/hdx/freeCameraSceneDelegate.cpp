    ...
    _policy(CameraUtilFit)
{
  ...
  GetRenderIndex().InsertSprim(HdPrimTypeTokens->camera, this, GetCameraId());
}
...
HdxFreeCameraSceneDelegate::~HdxFreeCameraSceneDelegate()
{
  ...
  GetRenderIndex().RemoveSprim(HdPrimTypeTokens->camera, GetCameraId());
}
...
void HdxFreeCameraSceneDelegate::_MarkDirty(const HdDirtyBits bits)
{
  ...
  GetRenderIndex().GetChangeTracker().MarkSprimDirty(GetCameraId(), bits);
}
...
GfMatrix4d HdxFreeCameraSceneDelegate::GetTransform(SdfPath const &id)
{
