...
static GLuint _compileShader(GLchar const *const shaderSource, GLenum shaderType)
{
  ...
  if (status != GL_TRUE) {
    ...
    TF_CODING_ERROR("Failed to compile Metal/GL interop shader %s", errorLog);
    free(errorLog);
  }
...
static void _OutputShaderLog(GLuint program)
{
  ...
  if (maxLength) {
    ...
    TF_FATAL_CODING_ERROR("Failed to link GL program for Metal/GL interop:\n%s", errorLog);
    free(errorLog);
  }
}
...
HgiInteropMetal::HgiInteropMetal(Hgi *hgi) : _hgiMetal(nullptr)
{
  ...
  /* NS::OpenGLContext has been deprecated and removed. */
  ...
  _CreateShaderContext(vs, fsColorDepth, _shaderProgramContext[ShaderContextColorDepth]);
  ...
  _computeDepthCopyProgram = _defaultLibrary->newFunction(
    NS::String::string("copyDepth", NS::UTF8StringEncoding));
  _computeColorCopyProgram = _defaultLibrary->newFunction(
    NS::String::string("copyColour", NS::UTF8StringEncoding));
  ...
  MTL::ComputePipelineDescriptor *computePipelineStateDescriptor =
  ...
  _computePipelineStateDepth = _device->newComputePipelineState(computePipelineStateDescriptor,
  ...
  // Create a new Compute pipeline state object
  _computePipelineStateColor = _device->newComputePipelineState(computePipelineStateDescriptor,
                                                                MTL::PipelineOptionNone,
                                                                reflData,
                                                                &error);
  ...
  _cmdBuffer = _device->newCommandQueue()->commandBuffer();

  // Create the render pass.
  MTL::RenderPassDescriptor *renderPassDesc = MTL::RenderPassDescriptor::alloc()->init();
  if (!renderPassDesc) {
    TF_FATAL_CODING_ERROR("Failed to create a Metal render pass for Metal/GL interop");
    _cmdBuffer->commit();
    return;
  }
  ...
  {
    attachment->setClearColor(MTL::ClearColor::Make(0.00, 0.00, 0.00, 1.00));
    renderPassDesc->setRenderTargetWidth(_mtlAliasedColorTexture->width());
    renderPassDesc->setRenderTargetWidth(_mtlAliasedColorTexture->height());
  }

  MTL::Buffer *vertexBuffer = _device->newBuffer((void *)(offsetof(Vertex, position)),
                                                 sizeof(Vertex),
  ...
  /* Draw... */
  MTL::RenderCommandEncoder *renderEncoder = _cmdBuffer->renderCommandEncoder(renderPassDesc);
  {
    renderEncoder->setFragmentTexture(_cvglColorTexture, 0);
    renderEncoder->setVertexBuffer(vertexBuffer, offsetof(Vertex, uv) * sizeof(Vertex), 0);
    renderEncoder->drawPrimitives(MTL::PrimitiveTypeTriangle, vertexBuffer, 4);

    renderEncoder->endEncoding();
  }

  /* Present frame. */
  _cmdBuffer->presentDrawable(_cvglTextureCache);
  _cmdBuffer->commit();
...
void HgiInteropMetal::_ValidateGLContext()
{
  ...
  TF_FATAL_CODING_ERROR("OpenGL is not supported on this platform.");
}
...
void HgiInteropMetal::_SetAttachmentSize(int width, int height)
{
  ...
  MTL::TextureDescriptor *depthTexDescriptor =
  ...
  depthTexDescriptor->setResourceOptions(MTL::ResourceCPUCacheModeDefaultCache |
...
void HgiInteropMetal::_CaptureOpenGlState()
{
  ...
  if (!_restoreVao && _restoreVbo) {
    for (int i = 0; i < 2; i++) {
      ...
      glGetVertexAttribiv(i, GL_VERTEX_ATTRIB_ARRAY_SIZE, &state.size);
      glGetVertexAttribiv(i, GL_VERTEX_ATTRIB_ARRAY_TYPE, &state.type);
      glGetVertexAttribiv(i, GL_VERTEX_ATTRIB_ARRAY_NORMALIZED, &state.normalized);
      glGetVertexAttribiv(i, GL_VERTEX_ATTRIB_ARRAY_STRIDE, &state.stride);
      glGetVertexAttribiv(i, GL_VERTEX_ATTRIB_ARRAY_BUFFER_BINDING, &state.bufferBinding);

      glGetVertexAttribPointerv(i, GL_VERTEX_ATTRIB_ARRAY_POINTER, &state.pointer);
    }
  }
                                         ...
                                         GfVec4i const &compRegion)
{
  ...
  if (color) {
    colorTexture = (MTL::Texture *)color->GetRawResource();
  }
  if (depth) {
    depthTexture = (MTL::Texture *)depth->GetRawResource();
  }
  ...
  if (colorTexture) {
    ...
    MTL::Size threadsPerGrid = MTL::Size::Make(
      ...
      (_mtlAliasedColorTexture->height() + (threadgroupCount.height - 1)) /
        threadgroupCount.height,
      1);
