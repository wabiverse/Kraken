...
static int Garch_GetButton(unsigned int button)
{
  if (button == Button1)
    return GarchGLDebugWindow::MyButton1;
  else if (button == Button2)
    return GarchGLDebugWindow::MyButton2;
  else if (button == Button3)
    return GarchGLDebugWindow::MyButton3;
  return 0;
}

static int Garch_GetModifierKeys(int state)
{
  int keys = 0;
  if (state & ShiftMask)
