...
HgiGLOpsFn HgiGLOps::PopDebugGroup()
{
  return [] {
#if defined(GL_KHR_debug)
    if (GARCH_GLAPI_HAS(KHR_debug)) {
...
HgiGLOpsFn HgiGLOps::CopyTextureGpuToCpu(HgiTextureGpuToCpuOp const &copyOp)
{
  return [copyOp] {
    ...
    glGetTextureSubImage(srcTexture->GetTextureId(),
                         copyOp.mipLevel,
...
HgiGLOpsFn HgiGLOps::CopyTextureCpuToGpu(HgiTextureCpuToGpuOp const &copyOp)
{
  return [copyOp] {
    TRACE_SCOPE("HgiGLOps::CopyTextureCpuToGpu");

    HgiTextureDesc const &desc = copyOp.gpuDestinationTexture->GetDescriptor();
...
HgiGLOpsFn HgiGLOps::CopyBufferGpuToGpu(HgiBufferGpuToGpuOp const &copyOp)
{
  return [copyOp] {
...
HgiGLOpsFn HgiGLOps::CopyBufferCpuToGpu(HgiBufferCpuToGpuOp const &copyOp)
{
  return [copyOp] {
    ...
    HgiGLBuffer *glBuffer = static_cast<HgiGLBuffer *>(copyOp.gpuDestinationBuffer.Get());
    ...
    glNamedBufferSubData(glBuffer->GetBufferId(), dstOffset, copyOp.byteSize, src);

    HGIGL_POST_PENDING_GL_ERRORS();
  };
}

HgiGLOpsFn HgiGLOps::CopyBufferGpuToCpu(HgiBufferGpuToCpuOp const &copyOp)
{
  return [copyOp] {
    ...
    HgiGLBuffer *glBuffer = static_cast<HgiGLBuffer *>(copyOp.gpuSourceBuffer.Get());

    // Offset into the dst buffer
    const char *dst = ((const char *)copyOp.cpuDestinationBuffer) + copyOp.destinationByteOffset;

    // Offset into the src buffer
    GLintptr srcOffset = copyOp.sourceByteOffset;

    glGetNamedBufferSubData(glBuffer->GetBufferId(), srcOffset, copyOp.byteSize, (void *)dst);

    HGIGL_POST_PENDING_GL_ERRORS();
  };
}

HgiGLOpsFn HgiGLOps::CopyTextureToBuffer(HgiTextureToBufferOp const &copyOp)
{
  return [copyOp] {
    ...
    if (HgiIsCompressed(texDesc.format)) {
      glGetCompressedTextureImage(
          srcTexture->GetTextureId(), copyOp.mipLevel, copyOp.byteSize, byteOffset);
    }
    else {
      GLenum format = 0;
      GLenum type   = 0;
...
HgiGLOpsFn HgiGLOps::CopyBufferToTexture(HgiBufferToTextureOp const &copyOp)
{
  return [copyOp] {
    TRACE_SCOPE("HgiGLOps::CopyTextureToBuffer");
    ...
    HgiGLConversions::GetFormat(texDesc.format, &format, &type, &internalFormat);
...
HgiGLOpsFn HgiGLOps::BindPipeline(HgiComputePipelineHandle pipeline)
{
  return [pipeline] {
    TRACE_SCOPE("HgiGLOps::BindPipeline");
...
HgiGLOpsFn HgiGLOps::BindResources(HgiResourceBindingsHandle res)
{
  return [res] {
...
HgiGLOpsFn HgiGLOps::SetConstantValues(HgiGraphicsPipelineHandle pipeline,
...
HgiGLOpsFn HgiGLOps::SetConstantValues(HgiComputePipelineHandle pipeline,
                                       uint32_t bindIndex,
                                       uint32_t byteSize,
                                       const void *data)
{
...
HgiGLOpsFn HgiGLOps::BindVertexBuffers(uint32_t firstBinding,
...
HgiGLOpsFn HgiGLOps::Dispatch(int dimX, int dimY)
{
  return [dimX, dimY] {
...
HgiGLOpsFn HgiGLOps::BindFramebufferOp(HgiGLDevice *device, HgiGraphicsCmdsDesc const &desc)
{
  return [device, desc] {
    ...
    for (size_t i = 0; i < desc.colorAttachmentDescs.size(); i++) {
      ...
      GLenum srcColor = HgiGLConversions::GetBlendFactor(colorAttachment.srcColorBlendFactor);
      GLenum dstColor = HgiGLConversions::GetBlendFactor(colorAttachment.dstColorBlendFactor);

      GLenum srcAlpha = HgiGLConversions::GetBlendFactor(colorAttachment.srcAlphaBlendFactor);
      GLenum dstAlpha = HgiGLConversions::GetBlendFactor(colorAttachment.dstAlphaBlendFactor);
...
HgiGLOpsFn HgiGLOps::GenerateMipMaps(HgiTextureHandle const &texture)
{
  return [texture] {
                                        ...
                                        HgiGraphicsCmdsDesc const &graphicsCmds)
{
  return [device, graphicsCmds] {
    ...
    const uint32_t framebuffer = device->AcquireFramebuffer(graphicsCmds);
