...
HgiGLGraphicsPipeline::HgiGLGraphicsPipeline(HgiGL const *hgi, HgiGraphicsPipelineDesc const &desc)
...
void HgiGLGraphicsPipeline::BindPipeline()
{
    ...
    for (HgiVertexBufferDesc const &vbo : _descriptor.vertexBuffers) {
      ...
      for (size_t loc = 0; loc < vas.size(); loc++) {
        ...
        if (HgiGLConversions::IsVertexAttribIntegerFormat(va.format)) {
          glVertexArrayAttribIFormat(_vao,
                                     idx,
  ...
  if (_descriptor.depthState.stencilTestEnabled) {
    glStencilFuncSeparate(
      GL_FRONT,
      HgiGLConversions::GetCompareFunction(_descriptor.depthState.stencilFront.compareFn),
      _descriptor.depthState.stencilFront.referenceValue,
      _descriptor.depthState.stencilFront.readMask);
    glStencilOpSeparate(
      GL_FRONT,
      HgiGLConversions::GetStencilOp(_descriptor.depthState.stencilFront.stencilFailOp),
      HgiGLConversions::GetStencilOp(_descriptor.depthState.stencilFront.depthFailOp),
      HgiGLConversions::GetStencilOp(_descriptor.depthState.stencilFront.depthStencilPassOp));
    glStencilMaskSeparate(GL_FRONT, _descriptor.depthState.stencilFront.writeMask);

    glStencilFuncSeparate(
      GL_BACK,
      HgiGLConversions::GetCompareFunction(_descriptor.depthState.stencilBack.compareFn),
      _descriptor.depthState.stencilBack.referenceValue,
      _descriptor.depthState.stencilBack.readMask);
    glStencilOpSeparate(
      GL_BACK,
      HgiGLConversions::GetStencilOp(_descriptor.depthState.stencilBack.stencilFailOp),
      HgiGLConversions::GetStencilOp(_descriptor.depthState.stencilBack.depthFailOp),
      HgiGLConversions::GetStencilOp(_descriptor.depthState.stencilBack.depthStencilPassOp));
    glStencilMaskSeparate(GL_BACK, _descriptor.depthState.stencilBack.writeMask);
  }
  ...
  if (_hgi->GetCapabilities()->IsSet(HgiDeviceCapabilitiesBitsConservativeRaster)) {
