...
GLenum HgiGLConversions::GetFormatType(HgiFormat inFormat)
{
  const _FormatDesc &desc = FORMAT_DESC[inFormat];
  return desc.type;
}
...
GLenum HgiGLConversions::GetCullMode(HgiCullMode cm)
{
  return _CullModeTable[cm][1];
}

GLenum HgiGLConversions::GetPolygonMode(HgiPolygonMode pm)
{
  return _PolygonModeTable[pm][1];
}

GLenum HgiGLConversions::GetBlendFactor(HgiBlendFactor bf)
{
  return _blendFactorTable[bf][1];
}

GLenum HgiGLConversions::GetBlendEquation(HgiBlendOp bo)
{
...
GLenum HgiGLConversions::GetSamplerAddressMode(HgiSamplerAddressMode am)
{
  ...
  switch (mipFilter) {
    ...
    case HgiMipFilterNearest:
      switch (minFilter) {
        ...
        default:
          TF_CODING_ERROR("Unsupported typr");
          break;
      }
