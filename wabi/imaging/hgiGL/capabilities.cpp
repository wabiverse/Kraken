...
#include "wabi/imaging/hgi/debugCodes.h"
...
void HgiGLCapabilities::_LoadCapabilities()
{
  GarchGLApiLoad();
  ...
  if (_glVersion >= 430) {
    glGetIntegerv(GL_MAX_SHADER_STORAGE_BLOCK_SIZE, &_maxShaderStorageBlockSize);
  }
  if (_glVersion >= 450) {
...
int HgiGLCapabilities::GetAPIVersion() const
{
  return _glVersion;
}

int HgiGLCapabilities::GetShaderVersion() const
{
  return _glslVersion;
}
