...
void HgiGLResourceBindings::BindResources()
{
  ...
  for (HgiBufferBindDesc const &bufDesc : _descriptor.buffers) {
    ...
    if (!bufDesc.sizes.empty() && bufDesc.buffers.size() != bufDesc.sizes.size()) {
      TF_CODING_ERROR("Invalid number of buffer sizes");
      continue;
    }

    HgiBufferHandle const &bufHandle = bufDesc.buffers.front();
    HgiGLBuffer const *glbuffer = static_cast<HgiGLBuffer *>(bufHandle.Get());
