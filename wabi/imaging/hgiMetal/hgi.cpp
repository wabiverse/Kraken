    ...
    _workToFlush(false)
{
  ...
  MTLArgumentDescriptor *argumentDescBuffer = [[MTLArgumentDescriptor alloc] init];
  argumentDescBuffer.dataType = MTLDataTypePointer;
  _argEncoderBuffer = [_device newArgumentEncoderWithArguments:@[ argumentDescBuffer ]];
  [argumentDescBuffer release];

  MTLArgumentDescriptor *argumentDescSampler = [[MTLArgumentDescriptor alloc] init];
  argumentDescSampler.dataType = MTLDataTypeSampler;
  _argEncoderSampler = [_device newArgumentEncoderWithArguments:@[ argumentDescSampler ]];
  [argumentDescSampler release];

  MTLArgumentDescriptor *argumentDescTexture = [[MTLArgumentDescriptor alloc] init];
  argumentDescTexture.dataType = MTLDataTypeTexture;
  _argEncoderTexture = [_device newArgumentEncoderWithArguments:@[ argumentDescTexture ]];
...
id<MTLDevice> HgiMetal::GetPrimaryDevice() const
{
  return _device;
}
...
void HgiMetal::DestroyTextureView(HgiTextureViewHandle *viewHandle)
{
  ...
  if (_workToFlush) {
    [_commandBuffer addCompletedHandler:[texHandle](id<MTLCommandBuffer> cmdBuffer) {
...
id<MTLCommandQueue> HgiMetal::GetQueue() const
{
  return _commandQueue;
}

id<MTLCommandBuffer> HgiMetal::GetPrimaryCommandBuffer(HgiCmds *requester, bool flush)
{
  if (_workToFlush) {
...
id<MTLCommandBuffer> HgiMetal::GetSecondaryCommandBuffer()
{
...
void HgiMetal::CommitSecondaryCommandBuffer(id<MTLCommandBuffer> commandBuffer,
...
void HgiMetal::ReleaseSecondaryCommandBuffer(id<MTLCommandBuffer> commandBuffer)
{
  [commandBuffer release];
}

id<MTLArgumentEncoder> HgiMetal::GetBufferArgumentEncoder() const
{
  return _argEncoderBuffer;
}

id<MTLArgumentEncoder> HgiMetal::GetSamplerArgumentEncoder() const
{
  return _argEncoderSampler;
}

id<MTLArgumentEncoder> HgiMetal::GetTextureArgumentEncoder() const
{
  return _argEncoderTexture;
}

id<MTLBuffer> HgiMetal::GetArgBuffer()
{
  ...
  [_commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> cmdBuffer) {
    std::lock_guard<std::mutex> lock(_freeArgMutex);
