...
namespace
{
  ...
  class ShaderStageData final
  {
   public:
    ...
    HgiMetalShaderSectionPtrVector AccumulateParams(const HgiShaderFunctionParamDescVector &params,
  ...
  template<typename T>
  T *_BuildStructInstance(const std::string &typeName,
                          ...
                          const std::string templateWrapper = std::string())
  {
    ...
    HgiMetalStructTypeDeclarationShaderSection *const section =
      generator->CreateShaderSection<HgiMetalStructTypeDeclarationShaderSection>(typeName,
                                                                                 members,
                                                                                 templateWrapper);
    ...
    return generator->CreateShaderSection<T>(instanceName,
                                             attributes,
                                             addressSpace,
                                             isPointer,
                                             section);
  ...
  bool _GetBuiltinKeyword(HgiShaderFunctionParamDesc const &param, std::string *keyword = nullptr)
  {
...
class HgiMetalShaderStageEntryPoint final
{
 ...
 private:
  ...
  void _Init(const HgiMetalShaderSectionPtrVector &stageConstantBuffers,
  ...
  HgiMetalShaderStageEntryPoint &operator=(const HgiMetalShaderStageEntryPoint &) = delete;
  HgiMetalShaderStageEntryPoint(const HgiMetalShaderStageEntryPoint &) = delete;
...
namespace
{
  ...
  const char *_GetDeclarationDefinitions()
  {
    return "#define REF(space,type) space type &\n"
           "#define FORWARD_DECL(...)\n"
  ...
  {
    return "struct hgi_ivec3 { int    x, y, z;\n"
  ...
  HgiMetalShaderSectionPtrVector _AccumulateParamsAndBlockParams(
  ...
  bool _IsTessFunction(const HgiShaderFunctionDesc &descriptor)
  {
    return descriptor.shaderStage == HgiShaderStagePostTessellationVertex;
  }

  ShaderStageData::ShaderStageData(const HgiShaderFunctionDesc &descriptor,
                                   HgiMetalShaderGenerator *generator)
    : inputsGenericWrapper(_IsTessFunction(descriptor) ? "patch_control_point" : ""),
      _constantParams(
        AccumulateParams(descriptor.constantParams, generator, descriptor.shaderStage, false)),
      _inputBlocks(
        AccumulateParamBlocks(descriptor.stageInputBlocks, generator, descriptor.shaderStage)),
      _outputBlocks(
        AccumulateParamBlocks(descriptor.stageOutputBlocks, generator, descriptor.shaderStage)),
      _inputs(_AccumulateParamsAndBlockParams(
        _inputBlocks,
        AccumulateParams(descriptor.stageInputs,
                         generator,
      ...
      _outputs(_AccumulateParamsAndBlockParams(
        _outputBlocks,
        AccumulateParams(descriptor.stageOutputs, generator, descriptor.shaderStage, false))),
      _bufferBindings(AccumulateBufferBindings(descriptor.buffers, generator))
  {
    // Also populates _samplerBindings
    _textureBindings = AccumulateTextureBindings(descriptor.textures, generator);
  }

  // Convert ShaderFunctionParamDescs into shader sections
  HgiMetalShaderSectionPtrVector ShaderStageData::AccumulateParams(
    ...
    if (!iterateAttrs) {
      ...
      for (const HgiShaderFunctionParamDesc &p : params) {
        ...
        HgiMetalMemberShaderSection *const section =
          generator->CreateShaderSection<HgiMetalMemberShaderSection>(p.nameInShader,
                                                                      p.type,
                                                                      attributes,
                                                                      p.arraySize);
      ...
      for (size_t i = 0; i < params.size(); i++) {
        ...
        const int location = (p.location != -1) ? p.location : nextLocation;
  ...
  HgiMetalInterstageBlockShaderSectionPtrVector ShaderStageData::AccumulateParamBlocks(
    const HgiShaderFunctionParamBlockDescVector &params,
    HgiMetalShaderGenerator *generator,
    HgiShaderStage stage)
  {
    ...
    for (const HgiShaderFunctionParamBlockDesc &p : params) {
      ...
      for (size_t i = 0; i < p.members.size(); ++i) {
        ...
        const std::string role = "user(slot" + std::to_string(slotIndex) + ")";
        const HgiShaderSectionAttributeVector attributes = {
          {role, ""}
        };

        HgiMetalMemberShaderSection *const memberSection =
          generator->CreateShaderSection<HgiMetalMemberShaderSection>(m.name,
                                                                      m.type,
                                                                      attributes,
                                                                      std::string(),
                                                                      p.instanceName);
      ...
      HgiMetalStructTypeDeclarationShaderSection *const blockStruct =
        generator->CreateShaderSection<HgiMetalStructTypeDeclarationShaderSection>(
          p.blockName + "_" + p.instanceName,
          blockMembers);

      HgiMetalInterstageBlockShaderSection *const blockSection =
        generator->CreateShaderSection<HgiMetalInterstageBlockShaderSection>(p.blockName,
                                                                             p.instanceName,
                                                                             blockStruct);
  ...
  HgiMetalShaderSectionPtrVector ShaderStageData::AccumulateBufferBindings(
    const HgiShaderFunctionBufferDescVector &buffers,
    HgiMetalShaderGenerator *generator)
  {
    HgiMetalShaderSectionPtrVector stageShaderSections;
    ...
    for (int i = 0; i <= maxBindIndex; i++) {
      ...
      if (p) {
        section = generator->CreateShaderSection<HgiMetalBufferShaderSection>(
          p->nameInShader,
        ...
        section = generator->CreateShaderSection<HgiMetalBufferShaderSection>("_unused" +
                                                                                std::to_string(i),
                                                                              attributes);
      }
      stageShaderSections.push_back(section);
  ...
  HgiMetalShaderSectionPtrVector ShaderStageData::AccumulateTextureBindings(
    const HgiShaderFunctionTextureDescVector &textures,
    HgiMetalShaderGenerator *generator)
  {
    HgiMetalShaderSectionPtrVector stageShaderSections;
    ...
    for (size_t i = 0; i < textures.size(); i++) {
      ...
      HgiMetalSamplerShaderSection *const samplerSection =
        generator->CreateShaderSection<HgiMetalSamplerShaderSection>(texName,
  ...
  const HgiMetalShaderSectionPtrVector &ShaderStageData::GetConstantParams() const
  {
    return _constantParams;
  }
  const HgiMetalShaderSectionPtrVector &ShaderStageData::GetInputs() const
  {
    return _inputs;
  }
  const HgiMetalShaderSectionPtrVector &ShaderStageData::GetOutputs() const
  {
    return _outputs;
  }
  const HgiMetalShaderSectionPtrVector &ShaderStageData::GetBufferBindings() const
  {
    return _bufferBindings;
  }
  const HgiMetalShaderSectionPtrVector &ShaderStageData::GetSamplerBindings() const
  {
    return _samplerBindings;
  }
  const HgiMetalShaderSectionPtrVector &ShaderStageData::GetTextureBindings() const
  {
  ...
  std::string _BuildOutputTypeName(const HgiMetalShaderStageEntryPoint &ep)
  {
    ...
    std::stringstream ss;
    ss << "MSL" << char(std::toupper(shortHandPrefix[0])) << shortHandPrefix[1] << "Outputs";
    return ss.str();
  }
...
{
  _Init(stageData.GetConstantParams(),
        stageData.GetInputs(),
...
const std::string &HgiMetalShaderStageEntryPoint::GetInputsInstanceName() const
{
  return _inputInstanceName;
}

const std::string &HgiMetalShaderStageEntryPoint::GetEntryPointFunctionName() const
{
  return _entryPointFunctionName;
}

const std::string &HgiMetalShaderStageEntryPoint::GetOutputTypeName() const
{
  return _outputTypeName;
}

std::string HgiMetalShaderStageEntryPoint::GetOutputInstanceName() const
{
  return GetOutputShortHandPrefix() + "Output";
}

const std::string &HgiMetalShaderStageEntryPoint::GetScopeInstanceName() const
{
  static const std::string result = "scope";
  return result;
}

const std::string &HgiMetalShaderStageEntryPoint::GetScopePostfix() const
{
  return _scopePostfix;
}

const std::string &HgiMetalShaderStageEntryPoint::GetEntryPointStageName() const
{
  return _entryPointStageName;
}

const std::string &HgiMetalShaderStageEntryPoint::GetEntryPointAttributes() const
{
  return _entryPointAttributes;
}

const std::string &HgiMetalShaderStageEntryPoint::GetOutputShortHandPrefix() const
{
  return _outputShortHandPrefix;
}

std::string HgiMetalShaderStageEntryPoint::GetConstantBufferTypeName() const
{
  const std::string &shortHandPrefix = GetOutputShortHandPrefix();

  std::stringstream ss;
  ss << "MSL" << char(std::toupper(shortHandPrefix[0])) << shortHandPrefix[1] << "Uniforms";
  return ss.str();
}

std::string HgiMetalShaderStageEntryPoint::GetConstantBufferInstanceName() const
{
  return GetOutputShortHandPrefix() + "Uniforms";
}

std::string HgiMetalShaderStageEntryPoint::GetScopeTypeName() const
{
...
std::string HgiMetalShaderStageEntryPoint::GetBindingsBufferTypeName() const
{
...
std::string HgiMetalShaderStageEntryPoint::GetBindingsSamplerTypeName() const
{
...
std::string HgiMetalShaderStageEntryPoint::GetBindingsTextureTypeName() const
{
...
HgiMetalParameterInputShaderSection *HgiMetalShaderStageEntryPoint::GetParameters()
{
  return _parameters;
}

HgiMetalParameterInputShaderSection *HgiMetalShaderStageEntryPoint::GetInputs()
{
  return _inputs;
}

HgiMetalStageOutputShaderSection *HgiMetalShaderStageEntryPoint::GetOutputs()
{
  return _outputs;
}

void HgiMetalShaderStageEntryPoint::_Init(
  ...
  HgiMetalShaderGenerator *generator)
{
  static const std::string constIndex = "buffer(" +
                                        std::to_string(HgiMetalArgumentIndexConstants) + ")";
  static const std::string samplerIndex = "buffer(" +
                                          std::to_string(HgiMetalArgumentIndexSamplers) + ")";
  static const std::string textureIndex = "buffer(" +
                                          std::to_string(HgiMetalArgumentIndexTextures) + ")";
  static const std::string bufferIndex = "buffer(" + std::to_string(HgiMetalArgumentIndexBuffers) +
                                         ")";

  _parameters = _BuildStructInstance<HgiMetalParameterInputShaderSection>(
  ...
  _inputs = _BuildStructInstance<HgiMetalParameterInputShaderSection>(
    GetInputsTypeName(),
  ...
  _bufferBindings = _BuildStructInstance<HgiMetalArgumentBufferInputShaderSection>(
  ...
  _samplerBindings = _BuildStructInstance<HgiMetalArgumentBufferInputShaderSection>(
  ...
  _textureBindings = _BuildStructInstance<HgiMetalArgumentBufferInputShaderSection>(
  ...
  const HgiShaderFunctionDesc &descriptor)
{
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    ...
    if (_GetBuiltinKeyword(p, &msl_attrib)) {
      ...
      // Shader section vector on the generator
      //  owns all sections, point to it in the vector
      CreateShaderSection<HgiMetalKeywordInputShaderSection>(keywordName, p.type, attributes);
    }
  }
}

void _BuildTessAttribute(std::stringstream &ss, const HgiShaderFunctionTessellationDesc &tessDesc)
{
  ss << "[[patch(";
...
void _BuildFragmentAttribute(std::stringstream &ss,
...
void _BuildComputeAttribute(std::stringstream &ss, const HgiShaderFunctionComputeDesc &computeDesc)
{
  if (computeDesc.localSize[0] > 0 && computeDesc.localSize[1] > 0 &&
      computeDesc.localSize[2] > 0) {
    ss << "[[max_total_threads_per_threadgroup(" << computeDesc.localSize[0] << " * "
       << computeDesc.localSize[1] << " * " << computeDesc.localSize[2] << ")]]\n";
  }
}

std::unique_ptr<HgiMetalShaderStageEntryPoint> HgiMetalShaderGenerator::
  _BuildShaderStageEntryPoints(const HgiShaderFunctionDesc &descriptor)
{
  ...
  switch (descriptor.shaderStage) {
    case HgiShaderStageVertex: {
      return std::make_unique<HgiMetalShaderStageEntryPoint>(stageData,
                                                             this,
                                                             "vsInput",
                                                             "vsInput",
                                                             "vertex",
    ...
    case HgiShaderStageFragment: {
      _BuildFragmentAttribute(functionAttributesSS, descriptor.fragmentDescriptor);
    ...
    case HgiShaderStageCompute: {
      _BuildComputeAttribute(functionAttributesSS, descriptor.computeDescriptor);
    ...
    case HgiShaderStagePostTessellationVertex: {
      _BuildTessAttribute(functionAttributesSS, descriptor.tessellationDescriptor);
    ...
    _generatorShaderSections(_BuildShaderStageEntryPoints(descriptor))
{
  for (const auto &member : descriptor.stageGlobalMembers) {
    ...
    CreateShaderSection<HgiMetalMemberShaderSection>(member.nameInShader,
                                                     member.type,
                                                     attrs,
                                                     member.arraySize);
  }
...
void HgiMetalShaderGenerator::_Execute(std::ostream &ss)
{
  ...
  ss << " " << returnSS.str() << " " << _generatorShaderSections->GetEntryPointFunctionName()
     << "(\n";
...
HgiMetalShaderSectionUniquePtrVector *HgiMetalShaderGenerator::GetShaderSections()
{
