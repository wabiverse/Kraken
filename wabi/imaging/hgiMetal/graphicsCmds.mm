                                             ...
                                             std::vector<uint32_t> const &byteOffsets)
{
  ...
  for (size_t i = 0; i < vertexBuffers.size(); i++) {
    ...
    [_encoder setVertexBuffer:buf->GetBufferId() offset:byteOffsets[i] atIndex:firstBinding + i];
  }
}

void HgiMetalGraphicsCmds::Draw(uint32_t vertexCount, uint32_t firstVertex, uint32_t instanceCount)
{
...
void HgiMetalGraphicsCmds::DrawIndexedIndirect(HgiBufferHandle const &indexBuffer,
...
void HgiMetalGraphicsCmds::MemoryBarrier(HgiMemoryBarrier barrier)
{
  ...
  [_encoder memoryBarrierWithScope:scope afterStages:srcStages beforeStages:dstStages];
}
