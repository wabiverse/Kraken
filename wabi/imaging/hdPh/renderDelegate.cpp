...
void HdPhRenderDelegate::SetDrivers(HdDriverVector const &drivers)
{
  ...
  for (HdDriver *hdDriver : drivers) {
    if (hdDriver->name == HgiTokens->renderDriver && hdDriver->driver.IsHolding<HgiVulkan *>()) {
      _hgi = hdDriver->driver.UncheckedGet<HgiVulkan *>();
      break;
    }
  }
...
{
  return HdRenderPassSharedPtr(new HdPh_RenderPass(index, collection));
}
...
{
  HioGlslfxSharedPtr glslfx(new HioGlslfx(HdPhPackageFallbackSurfaceShader()));

  HdPhSurfaceShaderSharedPtr fallbackShaderCode(new HdPhGLSLFXShader(glslfx));
