                                                ...
                                                HdPh_MaterialParamVector *params)
{
  ...
  SdrRegistry &shaderReg = SdrRegistry::GetInstance();
  SdrShaderNodeConstPtr sdrNode = shaderReg.GetShaderNodeByIdentifierAndType(node.nodeTypeId,
                                                                             HioGlslfxTokens->glslfx);
  ...
  if (inIt != node.inputConnections.end())
  {
    if (!inIt->second.empty())
    {
      ...
      if (primvarSdr)
      {
        ...
        _MakeMaterialParamsForPrimvarReader(network,
                                            primvarNode,
                                            upstreamNodePath,
                                            inIt->first,
                                            visitedNodes,
                                               ...
                                               SdrShaderNodeConstPtr const &sdrNode)
{
  ...
  const TfToken value = _ResolveParameter(node,
                                          sdrNode,
                                          HdPhTextureTokens->minFilter,
                                          HdPhTextureTokens->linearMipmapLinear);
                                               ...
                                               SdrShaderNodeConstPtr const &sdrNode)
{
  const TfToken value = _ResolveParameter(node,
                                          sdrNode,
                                          HdPhTextureTokens->magFilter,
                                          HdPhTextureTokens->linear);
...
//
static std::unique_ptr<HdPhSubtextureIdentifier> _GetSubtextureIdentifier(const HdTextureType textureType,
                                                                          ...
                                                                          const TfToken &sourceColorSpace)
{
  if (textureType == HdTextureType::Uv)
  {
    ...
    return std::make_unique<HdPhAssetUvSubtextureIdentifier>(flipVertically,
                                                             premultiplyAlpha,
                                                             sourceColorSpace);
  }
  ...
  TfToken const &materialTag)
{
  ...
  const TfToken sourceColorSpace = _ResolveParameter(node,
                                                     sdrNode,
                                                     _tokens->sourceColorSpace,
                                                     HdPhTokens->colorSpaceAuto);
  ...
  if (assetIdentifierPropertyNames.size() == 1)
  {
    ...
    if (it != node.parameters.end())
    {
      ...
      } else if (v.IsHolding<std::string>() || v.IsHolding<SdfAssetPath>())
      {
        ...
        useTexturePrimToFindTexture = false;
        textureId = HdPhTextureIdentifier(TfToken(filePath),
                                          _GetSubtextureIdentifier(texParam.textureType,
                                                                   node.nodeTypeId,
                                                                   premultiplyTexture,
                                                                   sourceColorSpace));
  ...
  texScaleParam.fallbackValue = VtValue(_ResolveParameter(node, sdrNode, HdPhTokens->scale, GfVec4f(1.0f)));
  ...
  texBiasParam.fallbackValue = VtValue(_ResolveParameter(node, sdrNode, HdPhTokens->bias, GfVec4f(0.0f)));
...
static void _MakeParamsForInputParameter(HdMaterialNetwork2 const &network,
                                         HdMaterialNode2 const &node,
                                         TfToken const &paramName,
                                         SdfPathSet *visitedNodes,
                                         HdPh_MaterialParamVector *params,
    ...
    if (!cons.empty())
    {
      ...
      if (upIt != network.nodes.end())
      {
        ...
        if (upstreamSdr)
        {
          ...
          {
            _MakeMaterialParamsForFieldReader(network,
                                              upstreamNode,
                                              upstreamPath,
                                              paramName,
                                              visitedNodes,
          ...
          {
            _MakeMaterialParamsForTransform2d(network,
                                              upstreamNode,
                                              upstreamPath,
                                              paramName,
                                              visitedNodes,
                                  ...
                                  TfToken const &materialTag)
{
  ...
  {
    _MakeParamsForInputParameter(network,
                                 node,
                                 inputName,
                                 &visitedNodes,
                                 params,
                                                 ...
                                                 HdPhResourceRegistry *resourceRegistry)
{
  ...
  if (HdMaterialNode2 const *surfTerminal = _GetTerminalNode(surfaceNetwork,
                                                             terminalName,
                                                             &surfTerminalPath))
  {
    ...
    if (_surfaceGfx)
    {
      ...
      if (_surfaceGfx->IsValid())
      {
        ...
        _GatherMaterialParams(surfaceNetwork,
                              *surfTerminal,
                              &_materialParams,
                              &_textureDescriptors,
                              _materialTag);
...
HdPhMaterialNetwork::TextureDescriptorVector const &HdPhMaterialNetwork::GetTextureDescriptors() const
{
