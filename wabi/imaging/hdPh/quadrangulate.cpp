...
bool HdPh_QuadInfoBuilderComputation::Resolve()
{
  if (!_TryLock())
    return false;
...
bool HdPh_QuadInfoBuilderComputation::_CheckValid() const
{
  return true;
}
...
HdPh_QuadIndexBuilderComputation::HdPh_QuadIndexBuilderComputation(
...
void HdPh_QuadIndexBuilderComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
...
bool HdPh_QuadIndexBuilderComputation::Resolve()
{
  ...
  _SetResult(std::make_shared<HdVtBufferSource>(HdTokens->indices, VtValue(quadsFaceVertexIndices)));

  _primitiveParam.reset(new HdVtBufferSource(HdTokens->primitiveParam, VtValue(primitiveParam)));

  _quadsEdgeIndices.reset(new HdVtBufferSource(HdTokens->edgeIndices, VtValue(quadsEdgeIndices)));

  _SetResolved();
  return true;
}

bool HdPh_QuadIndexBuilderComputation::HasChainedBuffer() const
{
  return true;
}

HdBufferSourceSharedPtrVector HdPh_QuadIndexBuilderComputation::GetChainedBuffers() const
{
  return {_primitiveParam, _quadsEdgeIndices};
}

bool HdPh_QuadIndexBuilderComputation::_CheckValid() const
{
  return true;
}
...
HdPh_QuadrangulateTableComputation::HdPh_QuadrangulateTableComputation(
  HdPh_MeshTopology *topology,
...
bool HdPh_QuadrangulateTableComputation::Resolve()
{
  if (!TF_VERIFY(_quadInfoBuilder))
    return false;
  if (!_quadInfoBuilder->IsResolved())
    return false;
  if (!_TryLock())
    return false;
...
void HdPh_QuadrangulateTableComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // quadinfo computation produces an index buffer for quads.
  specs->emplace_back(HdTokens->quadInfo, HdTupleType{HdTypeInt32, 1});
}

bool HdPh_QuadrangulateTableComputation::_CheckValid() const
{
  return true;
}
...
HdPh_QuadrangulateComputation::HdPh_QuadrangulateComputation(HdPh_MeshTopology *topology,
...
bool HdPh_QuadrangulateComputation::Resolve()
{
...
void HdPh_QuadrangulateComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // produces same spec buffer as source
  _source->GetBufferSpecs(specs);
}

HdTupleType HdPh_QuadrangulateComputation::GetTupleType() const
{
  return _source->GetTupleType();
}

bool HdPh_QuadrangulateComputation::_CheckValid() const
{
  return (_source->IsValid());
}

bool HdPh_QuadrangulateComputation::HasPreChainedBuffer() const
{
  return true;
}

HdBufferSourceSharedPtr HdPh_QuadrangulateComputation::GetPreChainedBuffer() const
{
  return _source;
}
...
HdPh_QuadrangulateFaceVaryingComputation::HdPh_QuadrangulateFaceVaryingComputation(
  HdPh_MeshTopology *topology,
  HdBufferSourceSharedPtr const &source,
  SdfPath const &id)
  : _id(id),
...
bool HdPh_QuadrangulateFaceVaryingComputation::Resolve()
{
  if (!TF_VERIFY(_source))
    return false;
  if (!_source->IsResolved())
  ...
  // HdPh_QuadrangulateComputation::Resolve()...
...
void HdPh_QuadrangulateFaceVaryingComputation::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // produces same spec buffer as source
  _source->GetBufferSpecs(specs);
}

bool HdPh_QuadrangulateFaceVaryingComputation::_CheckValid() const
{
  return (_source->IsValid());
}
...
HdPh_QuadrangulateComputationGPU::HdPh_QuadrangulateComputationGPU(HdPh_MeshTopology *topology,
...
void HdPh_QuadrangulateComputationGPU::Execute(HdBufferArrayRangeSharedPtr const &range,
                                               HdResourceRegistry *resourceRegistry)
{
  ...
  HdBufferArrayRangeSharedPtr const &quadrangulateTableRange = _topology->GetQuadrangulateTableRange();
  if (!quadrangulateTableRange)
    return;
  ...
  TfToken shaderToken = (HdGetComponentType(_dataType) == HdTypeFloat) ?
                          ...
                          HdPhGLSLProgramTokens->quadrangulateDouble;
  ...
  HdPhBufferResourceSharedPtr primvar = std::static_pointer_cast<HdPhBufferResource>(primvar_);

  HdPhBufferArrayRangeSharedPtr quadrangulateTableRange_ = std::static_pointer_cast<HdPhBufferArrayRange>(
    quadrangulateTableRange);
  ...
  const size_t componentSize = HdDataSizeOfType(HdGetComponentType(primvar->GetTupleType().type));
  uniform.primvarOffset = primvar->GetOffset() / componentSize;
  uniform.primvarStride = primvar->GetStride() / componentSize;
  uniform.numComponents = HdGetComponentCount(primvar->GetTupleType().type);
  ...
  uint64_t rbHash = (uint64_t)TfHash::Combine(primvar->GetHandle().Get(),
...
void HdPh_QuadrangulateComputationGPU::GetBufferSpecs(HdBufferSpecVector *specs) const
{
  // nothing
  //
...
int HdPh_QuadrangulateComputationGPU::GetNumOutputElements() const
{
  HdQuadInfo const *quadInfo = _topology->GetQuadInfo();
