...
GlfSimpleLight HdPhLight::_ApproximateAreaLight(SdfPath const &id, HdSceneDelegate *sceneDelegate)
{
  // Get the color of the light
  GfVec3f hdc = sceneDelegate->GetLightParamValue(id, HdPhLightTokens->color).Get<GfVec3f>();
                     ...
                     HdDirtyBits *dirtyBits)
{
  ...
  // HdPhLight communicates to the scene graph and caches all interesting
...
/* virtual */
void HdPhLight::Finalize(HdRenderParam *renderParam)
{
  // HdPhLight doesn't own resources directly, so we don't need to
  // flag garbage collection.
}

VtValue HdPhLight::Get(TfToken const &token) const
{
  VtValue val;
...
/* virtual */
HdDirtyBits HdPhLight::GetInitialDirtyBitsMask() const
{
