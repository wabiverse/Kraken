...
HdPhRenderPassState::~HdPhRenderPassState() = default;

bool HdPhRenderPassState::_UseAlphaMask() const
{
...
void HdPhRenderPassState::Prepare(HdResourceRegistrySharedPtr const &resourceRegistry)
{
  HD_TRACE_FUNCTION();
  ...
  if (!_renderPassStateBar || (_clipPlanesBufferSize != clipPlanes.size()) ||
    ...
    bufferSpecs.emplace_back(HdShaderTokens->worldToViewMatrix, HdTupleType{matType, 1});
    bufferSpecs.emplace_back(HdShaderTokens->worldToViewInverseMatrix, HdTupleType{matType, 1});
    bufferSpecs.emplace_back(HdShaderTokens->projectionMatrix, HdTupleType{matType, 1});
    bufferSpecs.emplace_back(HdShaderTokens->overrideColor, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdShaderTokens->wireframeColor, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdShaderTokens->maskColor, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdShaderTokens->indicatorColor, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdShaderTokens->pointColor, HdTupleType{HdTypeFloatVec4, 1});
    bufferSpecs.emplace_back(HdShaderTokens->pointSize, HdTupleType{HdTypeFloat, 1});
  ...
  sources.push_back(
    std::make_shared<HdVtBufferSource>(HdShaderTokens->viewport,
...
void HdPhRenderPassState::SetResolveAovMultiSample(bool state)
{
  _resolveMultiSampleAov = state;
}

bool HdPhRenderPassState::GetResolveAovMultiSample() const
{
...
void HdPhRenderPassState::SetUseSceneMaterials(bool state)
{
  _useSceneMaterials = state;
}

HdPhShaderCodeSharedPtrVector HdPhRenderPassState::GetShaders() const
{
...
void HdPhRenderPassState::Bind()
{
  ...
  _lightingShader->SetCamera(GetWorldToViewMatrix(), GetProjectionMatrix());
  ...
  if (_blendEnabled) {
    ...
    glBlendFuncSeparate(HdPhGLConversions::GetGlBlendFactor(_blendColorSrcFactor),
...
void HdPhRenderPassState::Unbind()
{
  GLF_GROUP_FUNCTION();
...
void HdPhRenderPassState::SetCameraFramingState(GfMatrix4d const &worldToViewMatrix,
  ...
  const HdRenderIndex *const renderIndex) const
{
  ...
  for (const HdRenderPassAovBinding &aov : aovBindings) {
    ...
    HgiAttachmentLoadOp loadOp = aov.clearValue.IsEmpty() ? HgiAttachmentLoadOpLoad :
