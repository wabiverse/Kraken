...
#include "wabi/imaging/hdPh/vboMemoryManager.h"
...
TF_DEFINE_ENV_SETTING(HD_MAX_VBO_SIZE, (1 * 1024 * 1024 * 1024), "Maximum aggregated VBO size");

// ---------------------------------------------------------------------------
//  HdPhVBOMemoryManager
// ---------------------------------------------------------------------------
HdBufferArraySharedPtr HdPhVBOMemoryManager::CreateBufferArray(TfToken const &role,
                                                               HdBufferSpecVector const &bufferSpecs,
                                                               HdBufferArrayUsageHint usageHint)
{
  return std::make_shared<HdPhVBOMemoryManager::_StripedBufferArray>(_resourceRegistry,
                                                                     role,
                                                                     bufferSpecs,
                                                                     usageHint);
}

HdBufferArrayRangeSharedPtr HdPhVBOMemoryManager::CreateBufferArrayRange()
{
  return std::make_shared<_StripedBufferArrayRange>(_resourceRegistry);
}

HdAggregationStrategy::AggregationId HdPhVBOMemoryManager::ComputeAggregationId(
  HdBufferSpecVector const &bufferSpecs,
...
HdBufferSpecVector HdPhVBOMemoryManager::GetBufferSpecs(HdBufferArraySharedPtr const &bufferArray) const
{
  _StripedBufferArraySharedPtr bufferArray_ = std::static_pointer_cast<_StripedBufferArray>(bufferArray);
  return bufferArray_->GetBufferSpecs();
}

/// Returns the size of the GPU memory used by the passed buffer array
size_t HdPhVBOMemoryManager::GetResourceAllocation(HdBufferArraySharedPtr const &bufferArray,
...
//  _StripedBufferArray
// ---------------------------------------------------------------------------
HdPhVBOMemoryManager::_StripedBufferArray::_StripedBufferArray(HdPhResourceRegistry *resourceRegistry,
                                                               TfToken const &role,
                                                               HdBufferSpecVector const &bufferSpecs,
                                                               HdBufferArrayUsageHint usageHint)
    ...
    _maxBytesPerElement(0)
{
  ...
  */
  ...
  {
    _maxBytesPerElement = std::max(_maxBytesPerElement, HdDataSizeOfTupleType(it->second->GetTupleType()));
  }
...
HdPhBufferResourceSharedPtr HdPhVBOMemoryManager::_StripedBufferArray::_AddResource(TfToken const &name,
...
HdPhVBOMemoryManager::_StripedBufferArray::~_StripedBufferArray()
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
bool HdPhVBOMemoryManager::_StripedBufferArray::GarbageCollect()
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void HdPhVBOMemoryManager::_StripedBufferArray::Reallocate(
  ...
  HdBufferArraySharedPtr const &curRangeOwner)
{
  ...
  _StripedBufferArraySharedPtr curRangeOwner_ = std::static_pointer_cast<_StripedBufferArray>(curRangeOwner);

  if (!TF_VERIFY(GetResources().size() == curRangeOwner_->GetResources().size()))
  {
  ...
  if (TfDebug::IsEnabled(HD_SAFE_MODE))
  {
    HdPhBufferResourceNamedList::size_type bresIdx = 0;
  ...
  {
    HdPhBufferResourceSharedPtr const &bres = resources[bresIdx].second;
    HdPhBufferResourceSharedPtr const &curRes = curRangeOwner_->GetResources()[bresIdx].second;
    ...
    if (curBuf && newBuf)
    {
      ...
      TF_FOR_ALL (it, ranges)
      {
        _StripedBufferArrayRangeSharedPtr range = std::static_pointer_cast<_StripedBufferArrayRange>(*it);
        if (!range)
        {
          TF_CODING_ERROR(
  ...
  {
    _StripedBufferArrayRangeSharedPtr range = std::static_pointer_cast<_StripedBufferArrayRange>(
      ranges[idx]);
    if (!range)
    {
...
void HdPhVBOMemoryManager::_StripedBufferArray::_DeallocateResources()
{
  Hgi *hgi = _resourceRegistry->GetHgi();
  TF_FOR_ALL (it, GetResources())
  {
...
/*virtual*/
size_t HdPhVBOMemoryManager::_StripedBufferArray::GetMaxNumElements() const
{
  static size_t vboMaxSize = TfGetEnvSetting(HD_MAX_VBO_SIZE);
  return vboMaxSize / _maxBytesPerElement;
}

void HdPhVBOMemoryManager::_StripedBufferArray::DebugDump(std::ostream &out) const
{
  out << "  HdPhVBOMemoryManager\n";
...
HdPhBufferResourceSharedPtr HdPhVBOMemoryManager::_StripedBufferArray::GetResource() const
{
  HD_TRACE_FUNCTION();
...
HdPhBufferResourceSharedPtr HdPhVBOMemoryManager::_StripedBufferArray::GetResource(TfToken const &name)
{
  HD_TRACE_FUNCTION();
...
HdBufferSpecVector HdPhVBOMemoryManager::_StripedBufferArray::GetBufferSpecs() const
{
...
//  _StripedBufferArrayRange
// ---------------------------------------------------------------------------
HdPhVBOMemoryManager::_StripedBufferArrayRange::~_StripedBufferArrayRange()
{
...
bool HdPhVBOMemoryManager::_StripedBufferArrayRange::IsAssigned() const
{
  return (_stripedBufferArray != nullptr);
}

bool HdPhVBOMemoryManager::_StripedBufferArrayRange::IsImmutable() const
{
  return (_stripedBufferArray != nullptr) && _stripedBufferArray->IsImmutable();
}

bool HdPhVBOMemoryManager::_StripedBufferArrayRange::Resize(int numElements)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  if (_capacity != numElements)
  {
    ...
    {
      TF_WARN(
        "Attempting to resize the BAR with 0x%x elements when the "
...
void HdPhVBOMemoryManager::_StripedBufferArrayRange::CopyData(HdBufferSourceSharedPtr const &bufferSource)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  HdPhBufferResourceSharedPtr VBO = _stripedBufferArray->GetResource(bufferSource->GetName());

  if (!TF_VERIFY((VBO && VBO->GetHandle()), "VBO doesn't exist for %s", bufferSource->GetName().GetText()))
  {
    return;
  }
...
int HdPhVBOMemoryManager::_StripedBufferArrayRange::GetByteOffset(TfToken const &resourceName) const
{
  if (!TF_VERIFY(_stripedBufferArray))
    return 0;
  HdPhBufferResourceSharedPtr VBO = _stripedBufferArray->GetResource(resourceName);
...
VtValue HdPhVBOMemoryManager::_StripedBufferArrayRange::ReadData(TfToken const &name) const
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  HdPhBufferResourceSharedPtr VBO = _stripedBufferArray->GetResource(name);
  ...
  result = HdPhGLUtils::ReadBuffer(vbo,
                                   VBO->GetTupleType(),
                                   vboOffset,
...
size_t HdPhVBOMemoryManager::_StripedBufferArrayRange::GetMaxNumElements() const
{
  return _stripedBufferArray->GetMaxNumElements();
}

HdBufferArrayUsageHint HdPhVBOMemoryManager::_StripedBufferArrayRange::GetUsageHint() const
{
  if (!TF_VERIFY(_stripedBufferArray))
  {
...
HdPhBufferResourceSharedPtr HdPhVBOMemoryManager::_StripedBufferArrayRange::GetResource() const
{
  if (!TF_VERIFY(_stripedBufferArray))
    return HdPhBufferResourceSharedPtr();
...
HdPhBufferResourceSharedPtr HdPhVBOMemoryManager::_StripedBufferArrayRange::GetResource(TfToken const &name)
{
  if (!TF_VERIFY(_stripedBufferArray))
    return HdPhBufferResourceSharedPtr();
...
HdPhBufferResourceNamedList const &HdPhVBOMemoryManager::_StripedBufferArrayRange::GetResources() const
{
  if (!TF_VERIFY(_stripedBufferArray))
  {
...
void HdPhVBOMemoryManager::_StripedBufferArrayRange::SetBufferArray(HdBufferArray *bufferArray)
{
  _stripedBufferArray = static_cast<_StripedBufferArray *>(bufferArray);
}

void HdPhVBOMemoryManager::_StripedBufferArrayRange::DebugDump(std::ostream &out) const
{
  out << "[StripedBAR] offset = " << _elementOffset << ", numElements = " << _numElements
      << ", capacity = " << _capacity << "\n";
}

const void *HdPhVBOMemoryManager::_StripedBufferArrayRange::_GetAggregation() const
{
  return _stripedBufferArray;
}

size_t HdPhVBOMemoryManager::_StripedBufferArrayRange::_GetByteOffset(
  HdPhBufferResourceSharedPtr const &resource) const
{
