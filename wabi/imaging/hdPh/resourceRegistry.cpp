...
TF_DEFINE_ENV_SETTING(HDPH_ENABLE_RESOURCE_INSTANCING,
                      true,
                      "Enable instance registry deduplication of resource data");

TF_DEFINE_PRIVATE_TOKENS(_perfTokens,
...
{
  static bool isResourceInstancingEnabled = TfGetEnvSetting(HDPH_ENABLE_RESOURCE_INSTANCING);
...
void HdPhResourceRegistry::InvalidateShaderRegistry()
{
...
VtDictionary HdPhResourceRegistry::GetResourceAllocation() const
{
  VtDictionary result;
...
Hgi *HdPhResourceRegistry::GetHgi()
{
  return _hgi;
}
  ...
  HdBufferArrayUsageHint usageHint)
{
  ...
  return _AllocateBufferArrayRange(_nonUniformImmutableAggregationStrategy.get(),
...
HdBufferArrayRangeSharedPtr HdPhResourceRegistry::AllocateSingleBufferArrayRange(
  ...
  HdBufferArrayUsageHint usageHint)
{
  return _AllocateBufferArrayRange(_singleAggregationStrategy.get(),
  ...
  HdBufferArrayUsageHint usageHint)
{
  ...
  return _UpdateBufferArrayRange(_nonUniformImmutableAggregationStrategy.get(),
                                 _nonUniformImmutableBufferArrayRegistry,
                                 role,
                                 curRange,
                                 updatedOrAddedSpecs,
...
/// ------------------------------------------------------------------------
void HdPhResourceRegistry::AddSources(HdBufferArrayRangeSharedPtr const &range,
...
void HdPhResourceRegistry::AddSource(HdBufferArrayRangeSharedPtr const &range,
...
void HdPhResourceRegistry::AddSource(HdBufferSourceSharedPtr const &source)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void HdPhResourceRegistry::AddComputation(HdBufferArrayRangeSharedPtr const &range,
...
/// ------------------------------------------------------------------------
HdPhDispatchBufferSharedPtr HdPhResourceRegistry::RegisterDispatchBuffer(TfToken const &role,
                                                                         int count,
                                                                         int commandNumUints)
{
  HdPhDispatchBufferSharedPtr const result = std::make_shared<HdPhDispatchBuffer>(this,
                                                                                  role,
                                                                                  count,
                                                                                  commandNumUints);
...
HdPhBufferResourceSharedPtr HdPhResourceRegistry::RegisterBufferResource(TfToken const &role,
                                                                         HdTupleType tupleType)
{
  HdPhBufferResourceSharedPtr const result = std::make_shared<HdPhBufferResource>(role,
                                                                                  tupleType,
                                                                                  /*offset*/ 0,
                                                                                  /*stride*/ 0);
...
void HdPhResourceRegistry::GarbageCollectDispatchBuffers()
{
  HD_TRACE_FUNCTION();
...
std::ostream &operator<<(std::ostream &out, const HdPhResourceRegistry &self)
{
  out << "HdPhResourceRegistry " << &self << " :\n";
...
void HdPhResourceRegistry::_CommitTextures()
{
  HdPhShaderCode::ResourceContext ctx(this);

  const std::set<HdPhShaderCodeSharedPtr> shaderCodes = _textureHandleRegistry->Commit();
...
void HdPhResourceRegistry::_Commit()
{
  ...
  // TODO: requests should be sorted by resource, and range.
  {
    ...
    HD_PERF_COUNTER_ADD(HdPerfTokens->bufferSourcesResolved, numBufferSourcesResolved);
  }
  ...
  // release sources
  WorkParallelForEach(_pendingSources.begin(), _pendingSources.end(), [](_PendingSource &ps) {
    ps.range.reset();
    ps.sources.clear();
  });
...
void HdPhResourceRegistry::_GarbageCollect()
{
  ...
  HdBufferArrayUsageHint usageHint)
{
  ...
  HdBufferArrayRangeSharedPtr newRange =
    _AllocateBufferArrayRange(strategy, bufferArrayRegistry, role, newBufferSpecs, usageHint);
...
{
  size_t gpuMemoryUsed = VtDictionaryGet<size_t>(*result,
  ...
  // Texture Memory and other texture information
  {
    ...
    const size_t numTexObjects = textureObjectRegistry->GetNumberOfTextureObjects();
    (*result)[_perfTokens->numberOfTextureObjects] = VtValue(numTexObjects);

    const size_t numTexHandles = _textureHandleRegistry->GetNumberOfTextureHandles();
...
{
  return _textureHandleRegistry->AllocateTextureHandle(textureId,
                                                       textureType,
                                                       samplerParams,
                                                       memoryRequest,
  ...
  const HdTextureType textureType)
{
  ...
  return reg->AllocateTextureObject(textureId, textureType);
}

void HdPhResourceRegistry::SetMemoryRequestForTextureType(const HdTextureType textureType,
                                                          const size_t memoryRequest)
{
  _textureHandleRegistry->SetMemoryRequestForTextureType(textureType, memoryRequest);
}
