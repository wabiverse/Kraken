...
bool HdPh_BasisCurvesIndexBuilderComputation::Resolve()
{
  if (!_TryLock())
    return false;
  ...
  _SetResult(
      HdBufferSourceSharedPtr(new HdVtBufferSource(HdTokens->indices, VtValue(result._indices))));
...
bool HdPh_BasisCurvesIndexBuilderComputation::_CheckValid() const
{
  return true;
}

bool HdPh_BasisCurvesIndexBuilderComputation::HasChainedBuffer() const
{
  return true;
}

HdBufferSourceSharedPtrVector HdPh_BasisCurvesIndexBuilderComputation::GetChainedBuffers() const
{
