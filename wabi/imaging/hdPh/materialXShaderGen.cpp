...
R"(#if NUM_LIGHTS > 0
    for (int i = 0; i < NUM_LIGHTS; ++i) {
        ...
        // Save the direct light data
            ...
            // Color and Intensity 
            // Note: Phoenix supports Simple, Sphere and Rect Direct Lights where
...
mx::ShaderPtr HdPhMaterialXShaderGen::generate(const std::string &shaderName,
...
void HdPhMaterialXShaderGen::_EmitGlslfxShader(const mx::ShaderGraph &mxGraph,
...
void HdPhMaterialXShaderGen::_EmitMxFunctions(const mx::ShaderGraph &mxGraph,
                                              mx::GenContext &mxContext,
                                              mx::ShaderStage &mxStage) const
{
  // Add global constants and type definitions
  emitInclude(
      "pbrlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_defines.glsl", mxContext, mxStage);
  ...
  for (const auto &it : mxStage.getUniformBlocks()) {
    ...
    if (!uniforms.empty() && uniforms.getName() != mx::HW::LIGHT_DATA) {
      ...
      emitVariableDeclarations(
          uniforms, mx::EMPTY_STRING, mx::Syntax::SEMICOLON, mxContext, mxStage);
      emitLineBreak(mxStage);
    }
  }

  const bool lighting = mxGraph.hasClassification(mx::ShaderNode::Classification::SHADER |
  ...
  if (lighting) {
    ...
    emitVariableDeclarations(
        lightData, mx::EMPTY_STRING, mx::Syntax::SEMICOLON, mxContext, mxStage, false);
    emitScopeEnd(mxStage, true);
    emitLineBreak(mxStage);
    emitLine(lightData.getName() + " " + lightData.getInstance() + "[MAX_LIGHT_SOURCES]", mxStage);
    emitLineBreak(mxStage);
    emitLineBreak(mxStage);
  }
  ...
  if (!vertexData.empty()) {
    ...
    emitScopeBegin(mxStage);
    emitVariableDeclarations(
        vertexData, mx::EMPTY_STRING, mx::Syntax::SEMICOLON, mxContext, mxStage, false);
  ...
  emitInclude(
      "pbrlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_math.glsl", mxContext, mxStage);
  emitLineBreak(mxStage);
  ...
  if (shadowing) {
    emitInclude(
        "pbrlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_shadow.glsl", mxContext, mxStage);
  }
  ...
  if (mxContext.getOptions().hwDirectionalAlbedoMethod ==
    ...
    emitInclude(
        "pbrlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_table.glsl", mxContext, mxStage);
    emitLineBreak(mxStage);
  }
  ...
  if (mxContext.getOptions().fileTextureVerticalFlip) {
    _tokenSubstitutions[mx::ShaderGenerator::T_FILE_TRANSFORM_UV] =
        "stdlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_transform_uv_vflip.glsl";
  }
  else {
    _tokenSubstitutions[mx::ShaderGenerator::T_FILE_TRANSFORM_UV] =
        "stdlib/" + mx::GlslShaderGenerator::TARGET + "/lib/mx_transform_uv.glsl";
  }
...
// Similar to GlslShaderGenerator::emitPixelStage() with alterations and
// additions to match Pxr's codeGen
void HdPhMaterialXShaderGen::_EmitMxSurfaceShader(const mx::ShaderGraph &mxGraph,
                                                  mx::GenContext &mxContext,
                                                  mx::ShaderStage &mxStage) const
{
  ...
  emitLine(
      "vec4 surfaceShader(vec4 Peye, vec3 Neye, vec4 color, vec4 patchCoord)", mxStage, false);
  emitScopeBegin(mxStage);
  ...
  if (mxGraph.hasClassification(mx::ShaderNode::Classification::CLOSURE)) {
    ...
    emitLine(outputSocket->getVariable() + " = vec4(mx_compute_depth_moments(), 0.0, 1.0)",
             mxStage);
  }
    ...
    if (outputConnection) {
      ...
      if (!outputSocket->getType()->isFloat4()) {
        ...
        emitLine(_syntax->getTypeName(outputSocket->getType()) + " " + finalOutput + " = " +
                     outputValue,
                 mxStage);
        toVec4(outputSocket->getType(), finalOutput);
...
void HdPhMaterialXShaderGen::_EmitMxInitFunction(mx::VariableBlock const &vertexData,
                                                 mx::ShaderStage &mxStage) const
{
  ...
  emitLine(
      "u_viewPosition = vec3(HdGet_worldToViewInverseMatrix()"
...
void HdPhMaterialXShaderGen::_EmitMxVertexDataDeclarations(mx::VariableBlock const &block,
...
void HdPhMaterialXShaderGen::emitVariableDeclarations(mx::VariableBlock const &block,
                                                      ...
                                                      bool assignValue) const
{
  ...
  for (size_t i = 0; i < block.size(); ++i) {
    ...
    if (MxHdVariables.count(block[i]->getName())) {
      emitVariableDeclaration(block[i], mx::EMPTY_STRING, context, stage, false);
    }
