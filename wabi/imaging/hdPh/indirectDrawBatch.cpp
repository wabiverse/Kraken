...
TF_DEFINE_ENV_SETTING(HD_ENABLE_GPU_INSTANCE_FRUSTUM_CULLING,
                      true,
...
/*virtual*/
void HdPh_IndirectDrawBatch::_Init(HdPhDrawItemInstance *drawItemInstance)
{
  ...
  TF_DEBUG(HDPH_DRAW_BATCH).Msg("   Resetting dispatch buffer.\n");
...
HdPh_IndirectDrawBatch::~HdPh_IndirectDrawBatch() {}
...
/* static */
bool HdPh_IndirectDrawBatch::IsEnabledGPUFrustumCulling()
{
...
/* static */
bool HdPh_IndirectDrawBatch::IsEnabledGPUInstanceFrustumCulling()
{
  GlfContextCaps const &caps = GlfContextCaps::GetInstance();
...
void HdPh_IndirectDrawBatch::_CompileBatch(HdPhResourceRegistrySharedPtr const &resourceRegistry)
{
  HD_TRACE_FUNCTION();
  ...
  for (size_t item = 0; item < numDrawItemInstances; ++item) {
    ...
    // index buffer data
    //
    HdBufferArrayRangeSharedPtr const &indexBar = drawItem->GetTopologyRange();

    //
    // topology visibility buffer data
    //
    HdBufferArrayRangeSharedPtr const &topVisBar = drawItem->GetTopologyVisibilityRange();

    //
    // element (per-face) buffer data
    //
    HdBufferArrayRangeSharedPtr const &elementBar = drawItem->GetElementPrimvarRange();

    //
    // vertex attrib buffer data
    //
    HdBufferArrayRangeSharedPtr const &vertexBar = drawItem->GetVertexPrimvarRange();

    //
    // varying buffer data
    //
    HdBufferArrayRangeSharedPtr const &varyingBar = drawItem->GetVaryingPrimvarRange();

    //
    // constant buffer data
    //
    HdBufferArrayRangeSharedPtr const &constantBar = drawItem->GetConstantPrimvarRange();

    //
    // face varying buffer data
    //
    HdBufferArrayRangeSharedPtr const &fvarBar = drawItem->GetFaceVaryingPrimvarRange();
    ...
    for (size_t i = 0; i < instancerNumLevels; ++i) {
      HdBufferArrayRangeSharedPtr const &ins = drawItem->GetInstancePrimvarRange(i);
    ...
    // instance indices
    //
    HdBufferArrayRangeSharedPtr const &instanceIndexBar = drawItem->GetInstanceIndexRange();

    //
    // shader parameter
    //
    HdBufferArrayRangeSharedPtr const &shaderBar = drawItem->GetMaterialShader()->GetShaderData();

    // 3 for triangles, 4 for quads, n for patches
    uint32_t numIndicesPerPrimitive = drawItem->GetGeometricShader()->GetPrimitiveIndexSize();
...
HdPh_DrawBatch::ValidationResult HdPh_IndirectDrawBatch::Validate(bool deepValidation)
{
  ...
  if (_bufferArraysHash != bufferArraysHash) {
    ...
    TF_DEBUG(HDPH_DRAW_BATCH).Msg("   Buffer arrays hash changed. Need to rebuild batch.\n");
  ...
  TF_DEBUG(HDPH_DRAW_BATCH).Msg("   Validation passed. No need to rebuild batch.\n");
  ...
  std::vector<HdPhBufferArrayRangeSharedPtr> const &instanceBars) const
{
  ...
  for (HdPhDrawItemInstance const *itemInstance : _drawItemInstances) {
    HdPhDrawItem const *itm = itemInstance->GetDrawItem();

    if (constantBar && !TF_VERIFY(constantBar->IsAggregatedWith(itm->GetConstantPrimvarRange()))) {
      failed = itm;
      break;
    }
    if (indexBar && !TF_VERIFY(indexBar->IsAggregatedWith(itm->GetTopologyRange()))) {
      failed = itm;
      break;
    }
    if (topologyVisibilityBar &&
        !TF_VERIFY(topologyVisibilityBar->IsAggregatedWith(itm->GetTopologyVisibilityRange()))) {
      failed = itm;
      break;
    }
    if (elementBar && !TF_VERIFY(elementBar->IsAggregatedWith(itm->GetElementPrimvarRange()))) {
      failed = itm;
      break;
    }
    if (fvarBar && !TF_VERIFY(fvarBar->IsAggregatedWith(itm->GetFaceVaryingPrimvarRange()))) {
      failed = itm;
      break;
    }
    if (varyingBar && !TF_VERIFY(varyingBar->IsAggregatedWith(itm->GetVaryingPrimvarRange()))) {
      failed = itm;
      break;
    }
    if (vertexBar && !TF_VERIFY(vertexBar->IsAggregatedWith(itm->GetVertexPrimvarRange()))) {
      failed = itm;
      break;
    }
    if (!TF_VERIFY(instancerNumLevels == itm->GetInstancePrimvarNumLevels())) {
      failed = itm;
      break;
    }
    if (instanceIndexBar &&
    ...
    std::vector<HdPhBufferArrayRangeSharedPtr> itmInstanceBars(instancerNumLevels);
    if (instanceIndexBar) {
      for (int i = 0; i < instancerNumLevels; ++i) {
        if (itmInstanceBars[i] &&
            !TF_VERIFY(itmInstanceBars[i]->IsAggregatedWith(itm->GetInstancePrimvarRange(i)),
                       "%d",
                       i)) {
          failed = itm;
          break;
...
void HdPh_IndirectDrawBatch::PrepareDraw(HdPhRenderPassStateSharedPtr const &renderPassState,
                                         HdPhResourceRegistrySharedPtr const &resourceRegistry)
{
  HD_TRACE_FUNCTION();
  ...
  if ((_useDrawArrays && _numTotalVertices == 0) || (!_useDrawArrays && _numTotalElements == 0))
    return;

  HdPhDrawItem const *batchItem = _drawItemInstances.front()->GetDrawItem();

  // Bypass freezeCulling if the command buffer is dirty.
  bool freezeCulling = TfDebug::IsEnabled(HD_FREEZE_CULL_FRUSTUM) && !_drawCommandBufferDirty;
  ...
  if (gpuCulling && !_useGpuInstanceCulling) {
    ...
    if (batchItem->GetInstanceIndexRange())
      gpuCulling = false;
  }
  ...
  if (gpuCulling && !freezeCulling) {
    if (_useGpuInstanceCulling) {
      _GPUFrustumInstanceCulling(batchItem,
...
void HdPh_IndirectDrawBatch::ExecuteDraw(HdPhRenderPassStateSharedPtr const &renderPassState,
                                         HdPhResourceRegistrySharedPtr const &resourceRegistry)
{
  HD_TRACE_FUNCTION();
  ...
  if (!TF_VERIFY(batchItem))
    return;

  if (!TF_VERIFY(_dispatchBuffer))
    return;
  ...
  // shader buffer bind
  HdPhBufferArrayRangeSharedPtr shaderBar;
  for (HdPhShaderCodeSharedPtr const &shader : shaders) {
    HdBufferArrayRangeSharedPtr shaderBar_ = shader->GetShaderData();
    shaderBar = std::static_pointer_cast<HdPhBufferArrayRange>(shaderBar_);
    if (shaderBar) {
  ...
  HdPhBufferArrayRangeSharedPtr dispatchBar = _dispatchBuffer->GetBufferArrayRange();
  binder.BindBufferArray(dispatchBar);

  // update geometric shader states
  program.GetGeometricShader()->BindResources(programId, binder, *renderPassState);
  ...
  if (shaderBar) {
    binder.UnbindBuffer(HdTokens->materialParams, shaderBar->GetResource());
  }
  ...
  HdPhResourceRegistrySharedPtr const &resourceRegistry)
{
  ...
  HdPhGLSLProgramSharedPtr const &glslProgram = cullingProgram.GetGLSLProgram();
  ...
  const HdPh_ResourceBinder &binder = cullingProgram.GetBinder();
  ...
  if (validProgram) {
    ...
    // Reset Pass
    cullGfxCmds->SetConstantValues(psoHandle,
  ...
  binder.UnbindBuffer(_tokens->dispatchBuffer, _dispatchBuffer->GetEntireResource());
...
void HdPh_IndirectDrawBatch::DrawItemInstanceChanged(HdPhDrawItemInstance const *instance)
{
...
void HdPh_IndirectDrawBatch::_CullingProgram::Initialize(bool useDrawArrays,
                                                         bool useInstanceCulling,
                                                         size_t bufferArrayHash)
{
  if (useDrawArrays != _useDrawArrays || useInstanceCulling != _useInstanceCulling ||
  ...
  bool *enableInstanceDraw) const
{
  ...
  customBindings->push_back(HdBindingRequest(HdBinding::SSBO, _tokens->drawIndirectResult));
  customBindings->push_back(HdBindingRequest(HdBinding::SSBO, _tokens->dispatchBuffer));
