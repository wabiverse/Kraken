...
HdPhRenderParam::HdPhRenderParam() : _needsGarbageCollection(false)
{
...
void HdPhRenderParam::MarkDrawBatchesDirty()
{
  ++_drawBatchesVersion;  // uses std::memory_order_seq_cst
}

unsigned int HdPhRenderParam::GetDrawBatchesVersion() const
{
...
void HdPhRenderParam::MarkMaterialTagsDirty()
{
  ++_materialTagsVersion;  // uses std::memory_order_seq_cst
}

unsigned int HdPhRenderParam::GetMaterialTagsVersion() const
{
  // Can use relaxed ordering because render passes are expected to
  // only read the value, and that too in a single threaded fashion.
