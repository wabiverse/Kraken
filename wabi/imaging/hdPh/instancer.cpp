...
void HdPhInstancer::_SyncPrimvars(HdSceneDelegate *sceneDelegate, HdDirtyBits *dirtyBits)
{
  SdfPath const &instancerId = GetId();

  HdPrimvarDescriptorVector primvars = HdPhGetInstancerPrimvarDescriptors(this, sceneDelegate);
...
void HdPhInstancer::_GetInstanceIndices(SdfPath const &prototypeId,
                                        std::vector<VtIntArray> *instanceIndicesArray)
{
  SdfPath const &instancerId = GetId();
  VtIntArray instanceIndices = GetDelegate()->GetInstanceIndices(instancerId, prototypeId);
...
VtIntArray HdPhInstancer::GetInstanceIndices(SdfPath const &prototypeId)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
