...
HdPhGLSLProgramSharedPtr HdPh_CodeGen::Compile(HdPhResourceRegistry *const registry)
{
  ...
  // create GLSL program.
  HdPhGLSLProgramSharedPtr glslProgram(new HdPhGLSLProgram(HdTokens->drawingShader, registry));
...
HdPhGLSLProgramSharedPtr HdPh_CodeGen::CompileComputeProgram(HdPhResourceRegistry *const registry)
{
  ...
  // create GLSL program.
  HdPhGLSLProgramSharedPtr glslProgram(new HdPhGLSLProgram(HdTokens->computeShader, registry));
                                ...
                                HdPh_GeometricShader::PrimitiveType const &primType)
{
  ...
  str << _GetUnpackedType(type, false) << " HdGet_" << name << "_Coarse(int localIndex) {\n"
      ...
      << "  return " << _GetPackedTypeAccessor(type, true) << "(" << name << "[fvarIndex]);\n}\n";
  ...
  */
    ...
    if (_geometricShader->IsPrimTypePoints()) {
      ...
      switch (_geometricShader->GetPrimitiveType()) {
        ...
        case HdPh_GeometricShader::PrimitiveType::PRIM_MESH_REFINED_TRIANGLES: {
          ...
          accessors << "int GetEdgeFlag(int localIndex) {\n"
      ...
      if (_geometricShader->IsPrimTypeTriangles() ||
          (_geometricShader->GetPrimitiveType() ==
           HdPh_GeometricShader::PrimitiveType::PRIM_MESH_BOXSPLINETRIANGLE)) {
        // note that triangulated meshes don't have ptexIndex.
        // Here we're passing primitiveID as ptexIndex PatchParam
        // since Hd_TriangulateFaceVaryingComputation unrolls facevaring
        // primvars for each triangles.
  ...
  if (_metaData.edgeIndexBinding.binding.IsValid()) {
    ...
    if (_geometricShader->IsPrimTypePoints()) {
      ...
      TF_VERIFY(false,
                "edgeIndexBinding shouldn't be found on a "
                "basis curve");
    }
    else if (_geometricShader->IsPrimTypeMesh()) {
  ...
  declarations << "int GetAuthoredEdgeId(int primitiveEdgeID);\n"
  ...
  */
  ...
  if (hasGS) {
    ...
    TF_FOR_ALL(it, _metaData.fvarData)
    {
      ...
      _EmitFVarGSAccessor(
          accessorsGS, name, dataType, binding, _geometricShader->GetPrimitiveType());
  ...
  if (!interstageVertexData.str().empty()) {
    ...
    _genTCS << "in Primvars {\n"
            ...
            << "out Primvars {\n"
            << interstageVertexData.str() << "} outPrimvars[HD_NUM_PATCH_VERTS];\n"
            << accessorsTCS.str();
  ...
  TF_FOR_ALL(it, _metaData.shaderParameterBinding)
  {
    ...
    if (bindingType == HdBinding::FALLBACK) {
      ...
      accessors << _GetUnpackedType(it->second.dataType, false) << " HdGet_" << it->second.name
                ...
                << "isampler1DArray(shaderData[shaderCoord]." << it->second.name
      ...
      accessors << _GetUnpackedType(it->second.dataType, false) << " HdGet_" << it->second.name
                ...
                << "(PtexTextureLookup("
                << "sampler2darray_" << it->second.name << ", "
                << "isampler1darray_" << it->second.name
                << HdPh_ResourceBindingSuffixTokens->layout << ", "
                << "GetPatchCoord(localIndex))" << swizzle << ");\n"
                << "}\n"
                << _GetUnpackedType(it->second.dataType, false) << " HdGet_" << it->second.name
      ...
      declarations << LayoutQualifier(HdBinding(it->first.GetType(),
                   ...
                   << "uniform isampler1DArray isampler1darray_" << it->second.name << ";\n";
    }
