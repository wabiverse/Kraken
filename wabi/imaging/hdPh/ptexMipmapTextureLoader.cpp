...
void HdPhPtexMipmapTextureLoader::Block::guttering(HdPhPtexMipmapTextureLoader *loader,
                                                   PtexTexture *ptex,
                                                   int level,
                                                   int wid,
                                                   int hei,
                                                   ...
                                                   int stride)
{
  ...
  for (int edge = 0; edge < numEdges; edge++) {
    ...
    if (loader->getCornerPixel(
            accumPixel, numchannels, this->index, edge, (int8_t)(this->ulog2 - level))) {
      // case1, case 2
      if (edge == 1 || edge == 2)
        du += wid;
      if (edge == 2 || edge == 3)
...
void HdPhPtexMipmapTextureLoader::Block::Generate(HdPhPtexMipmapTextureLoader *loader,
                                                  PtexTexture *ptex,
...
struct HdPhPtexMipmapTextureLoader::Page {
  struct SlotLimit {
  ...
  void Generate(HdPhPtexMipmapTextureLoader *loader,
                PtexTexture *ptex,
                unsigned char *destination,
                int bpp,
                int width,
...
HdPhPtexMipmapTextureLoader::HdPhPtexMipmapTextureLoader(PtexTexture *ptex,
                                                         int maxNumPages,
                                                         int maxLevels,
                                                         size_t targetMemory,
                                                         bool seamlessMipmap)
...
//
int HdPhPtexMipmapTextureLoader::resampleBorder(int face,
                                                int edgeId,
                                                ...
                                                float srcEnd)
{
  ...
  if (dstLength >= srcLength) {
    ...
    if (!data) {
      ...
      TF_WARN("Ptex missing texture face for face %d at res (%d x %d)", face, res.u(), res.v());
      return srcLength;
    }
...
void HdPhPtexMipmapTextureLoader::sampleNeighbor(unsigned char *border,
                                                 int face,
                                                 int edge,
                                                 int length,
                                                 int bpp)
...
//
bool HdPhPtexMipmapTextureLoader::getCornerPixel(float *resultPixel,
                                                 int numchannels,
                                                 int face,
                                                 int edge,
                                                 int8_t reslog2)
  ...
  if (fi.isSubface() && edge == 3) {
    ...
    */
    ...
    if (adjface != -1 && !_ptex->getFaceInfo(adjface).isSubface()) {
      ...
      _ptex->getPixel(adjface, uv[0], uv[1], resultPixel, 0, numchannels, res);
      return true;
    }
  }
...
int HdPhPtexMipmapTextureLoader::getLevelDiff(int face, int edge)
{
...
void HdPhPtexMipmapTextureLoader::optimizePacking(int maxNumPages, size_t targetMemory)
{
  size_t numTexels = 0;
  ...
  if (targetMemory != 0 && _bpp * numTexels > targetMemory) {
    ...
    while (numTexels > numTargetTexels && candidateBlocks.size() > 0) {
      ...
      block->SetSize(
          (unsigned char)(block->ulog2 - 1), (unsigned char)(block->vlog2 - 1), _maxLevels != 0);
      numTexels += block->GetNumTexels();
    }
  }
  ...
  Page::SlotLimit limit(smallestBlockTexels, smallestBlockWidth, smallestBlockHeight);
...
void HdPhPtexMipmapTextureLoader::generateBuffers()
{
  // ptex layout struct
  // struct Layout {
  //     uint16_t page;
