  ...
  for (const std::string &fileName : dirFileNames) {
    ...
    if (extIter != allowedExtensions.end()) {
      ...
      const bool parsed = parseIdentifierFn ?
                            ...
                            NdrFsHelpersSplitShaderIdentifier(identifier,
                                                              &family,
                                                              &name,
                                                              &version);
      if (!parsed) {
        TF_WARN(
          "Could not parse the family, name, and version "
...
static void _WalkDirs(const NdrStringVec &searchPaths,
                                                    ...
                                                    const NdrParseIdentifierFn &parseIdentifierFn)
{
  ...
  auto discoverNodesFn =
    [&](const std::string &dirPath, NdrStringVec *unused, const NdrStringVec &dirFileNames) {
      return _FsHelpersExamineFiles(&foundNodes,
                                    &foundNodesWithTypes,
                                    allowedExtensions,
                                    context,
                                    dirPath,
