...
TF_DEFINE_ENV_SETTING(WABI_NDR_SKIP_DISCOVERY_PLUGIN_DISCOVERY,
                      0,
                      "The auto-discovery of discovery plugins in ndr can be skipped. "
                      "This is used mostly for testing purposes.");

TF_DEFINE_ENV_SETTING(WABI_NDR_SKIP_PARSER_PLUGIN_DISCOVERY,
                      0,
...
template<typename T>
static bool _MatchesFamilyAndFilter(const T &object,
                                            ...
                                            const TfToken &sourceType)
{
  ...
  return NdrIdentifier(TfStringPrintf(
      "%s<%s><%s>", std::to_string(h).c_str(), subIdentifier.GetText(), sourceType.GetText()));
}
...
static bool _ValidateNode(const NdrNodeUniquePtr &newNode, const NdrNodeDiscoveryResult &dr)
{
  ...
  if (newNode->IsValid() &&
      !(newNode->GetIdentifier() == dr.identifier && newNode->GetName() == dr.name &&
                                              ...
                                              const TfToken &sourceType)
{
  ...
  std::string resolvedUri = asset.GetResolvedPath().empty() ? asset.GetAssetPath() :
  ...
  NodeMap::const_iterator result = _nodeMap.emplace(std::move(key), std::move(newNode));
                                                   ...
                                                   const NdrTokenMap &metadata)
{
  ...
  if (!parserForSourceType) {
    ...
    TF_DEBUG(NDR_PARSING)
        .Msg(
            "Encountered source code of type [%s], but "
            "a parser for the type could not be found; "
            "ignoring.\n",
            sourceType.GetText());
    return nullptr;
...
NdrIdentifierVec NdrRegistry::GetNodeIdentifiers(const TfToken &family,
...
NdrNodeConstPtr NdrRegistry::GetNodeByIdentifierAndType(const NdrIdentifier &identifier,
                                                        const TfToken &sourceType)
{
  TRACE_FUNCTION();
  std::lock_guard<std::mutex> drLock(_discoveryResultMutex);
...
NdrNodeConstPtr NdrRegistry::_GetNodeByIdentifierAndTypeImpl(const NdrIdentifier &identifier,
                                                             const TfToken &sourceType)
{
...
NdrNodeConstPtr NdrRegistry::_GetNodeByNameAndTypeImpl(const std::string &name,
                                                       const TfToken &sourceType,
                                                       NdrVersionFilter filter)
{
  // Note that this impl function doesn't lock the discovery result mutex as
...
NdrNodeConstPtr NdrRegistry::_ParseNodeMatchingNameAndFilter(const NdrNodeDiscoveryResult &dr,
...
NdrNodeConstPtr NdrRegistry::_InsertNodeIntoCache(const NdrNodeDiscoveryResult &dr)
{
...
NdrNodeConstPtrVec NdrRegistry::_GetNodeMapAsNodePtrVec(const TfToken &family,
                                                        NdrVersionFilter filter) const
{
