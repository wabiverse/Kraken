...
NdrNodeConstPtr NdrRegistry::_ParseNodeFromAssetOrSourceCode(NdrParserPlugin &parser,
                                                 ...
                                                 const NdrTokenVec &sourceTypePriority)
{
  ...
  const _DiscoveryResultsByIdentifierRange range = _discoveryResultsByIdentifier.equal_range(
    identifier);
...
NdrNodeConstPtr NdrRegistry::_GetNodeInIdentifierRangeWithSourceType(
...
NdrNodeConstPtr NdrRegistry::_GetNodeInNameRangeWithSourceType(
  _DiscoveryResultPtrsByNameRange range,
  const TfToken &sourceType,
  NdrVersionFilter filter)
{
  for (auto it = range.first; it != range.second; ++it) {
