...
///
/// type    | wrap                        | curve segment count                    | batch segment
/// count
/// ------- | --------------------------- | -------------------------------------- |
/// -------------------------- linear  | nonperiodic                 | curveVertexCounts[i] - 1 |
/// sum(curveVertexCounts[:]) - len(curveVertexCounts) linear  | periodic                    |
/// curveVertexCounts[i]                   | sum(curveVertexCounts[:]) cubic   | nonperiodic |
/// (curveVertexCounts[i] - 4) / vstep + 1 | sum(curveVertexCounts[:] - 4) / vstep +
/// len(curveVertexCounts) cubic   | periodic                    | curveVertexCounts[i] / vstep |
/// sum(curveVertexCounts[:]) / vstep cubic   | pinned (catmullRom/bspline) | (curveVertexCounts[i]
/// - 2) + 1         | sum(curveVertexCounts[:] - 2) + len(curveVertexCounts)
///
...
/// ------------------- | ---------------------------- |
/// ------------------------------------------------ nonperiodic/pinned  | segmentCounts[i] + 1 |
/// sum(segmentCounts[:]) + len(curveVertexCounts) periodic            | segmentCounts[i] |
/// sum(segmentCounts[:])
///
...
/// ------------- | ------- | ------- | ----------------- | ------------------ |
/// ------------------------- nonperiodic   | linear  | N/A     | [2 3 2 5]         | [1 2 1 4] |
/// [2 3 2 5] nonperiodic   | cubic   | bezier  | [4 7 10 4 7]      | [1 2 3 1 2]        | [2 3 4 2
/// 3] nonperiodic   | cubic   | bspline | [5 4 6 7]         | [2 1 3 4]          | [3 2 4 5]
...
/// wabi/extras/examples/usdGeomExamples/basisCurves.usda.  It's provided
...
/// releases of HdPh to fix the discontinuity seen with broken tangents to
