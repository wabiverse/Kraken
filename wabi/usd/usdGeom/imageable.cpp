...
inline static void _IssueAPIWarningIfDeprecationFlagEnabled()
{
...
UsdGeomPrimvar UsdGeomImageable::CreatePrimvar(const TfToken &attrName,
                                               const SdfValueTypeName &typeName,
                                               const TfToken &interpolation,
                                               int elementSize) const
{
  _IssueAPIWarningIfDeprecationFlagEnabled();
  return UsdGeomPrimvarsAPI(GetPrim()).CreatePrimvar(attrName,
                                                     typeName,
                                                     interpolation,
                                                     elementSize);
}
...
std::vector<UsdGeomPrimvar> UsdGeomImageable::GetPrimvars() const
{
  _IssueAPIWarningIfDeprecationFlagEnabled();
  return UsdGeomPrimvarsAPI(GetPrim()).GetPrimvars();
}

std::vector<UsdGeomPrimvar> UsdGeomImageable::GetAuthoredPrimvars() const
{
  _IssueAPIWarningIfDeprecationFlagEnabled();
  return UsdGeomPrimvarsAPI(GetPrim()).GetAuthoredPrimvars();
}

bool UsdGeomImageable::HasPrimvar(const TfToken &name) const
{
  _IssueAPIWarningIfDeprecationFlagEnabled();
...
static TfToken _ComputePurposeVisibility(const UsdPrim &prim,
                                         ...
                                         const UsdTimeCode &time)
{
  ...
  TF_CODING_ERROR("Unexpected purpose '%s' computing purpose visibility for <%s>.",
                  purpose.GetText(),
                  prim.GetPath().GetText());
  return UsdGeomTokens->invisible;
}

TfToken UsdGeomImageable::ComputeEffectiveVisibility(const TfToken &purpose,
...
static void _SetVisibility(const UsdGeomImageable &imageable,
                           const TfToken &visState,
                           const UsdTimeCode &time)
{
...
static TfToken _ComputePurpose(UsdPrim const &prim, UsdPrim *root = NULL)
{
  if (UsdPrim parent = prim.GetParent()) {
                                        ...
                                        TfToken const &purpose4)
{
  ...
  if (!purpose1.IsEmpty())
    purposes.push_back(purpose1);
  if (!purpose2.IsEmpty())
    purposes.push_back(purpose2);
  if (!purpose3.IsEmpty())
    purposes.push_back(purpose3);
  if (!purpose4.IsEmpty())
    purposes.push_back(purpose4);
