...
UsdAttribute UsdGeomPointInstancer::CreateProtoIndicesAttr(VtValue const &defaultValue,
...
UsdAttribute UsdGeomPointInstancer::CreateIdsAttr(VtValue const &defaultValue,
                                                  bool writeSparsely) const
{
...
UsdAttribute UsdGeomPointInstancer::CreatePositionsAttr(VtValue const &defaultValue,
                                                        bool writeSparsely) const
{
...
UsdAttribute UsdGeomPointInstancer::CreateOrientationsAttr(VtValue const &defaultValue,
                                                           bool writeSparsely) const
{
...
UsdAttribute UsdGeomPointInstancer::CreateScalesAttr(VtValue const &defaultValue,
                                                     bool writeSparsely) const
{
...
UsdAttribute UsdGeomPointInstancer::CreateVelocitiesAttr(VtValue const &defaultValue,
                                                         bool writeSparsely) const
{
...
UsdAttribute UsdGeomPointInstancer::CreateAccelerationsAttr(VtValue const &defaultValue,
                                                            bool writeSparsely) const
{
...
UsdAttribute UsdGeomPointInstancer::CreateAngularVelocitiesAttr(VtValue const &defaultValue,
                                                                bool writeSparsely) const
{
...
UsdAttribute UsdGeomPointInstancer::CreateInvisibleIdsAttr(VtValue const &defaultValue,
                                                           bool writeSparsely) const
{
...
template<typename T> static SdfListOp<T> _CanonicalizeListOp(const SdfListOp<T> &op)
{
  if (op.IsExplicit()) {
    return op;
  }
...
bool UsdGeomPointInstancer::DeactivateId(int64_t id) const
{
  ...
  return UsdGeomPointInstancerSetOrMergeOverOp(
      toAdd,
      TfGetEnvSetting(USDGEOM_POINTINSTANCER_NEW_APPLYOPS) ? SdfListOpTypeAppended :
                                                             SdfListOpTypeAdded,
      GetPrim(),
...
std::vector<bool> UsdGeomPointInstancer::ComputeMaskAtTime(UsdTimeCode time,
...
bool UsdGeomPointInstancer::_GetPrototypePathsForInstanceTransforms(
...
bool UsdGeomPointInstancer::_ComputePointInstancerAttributesPreamble(
  ...
  if (applyMask == ApplyMask) {
    *mask = ComputeMaskAtTime(baseTime);
    if (!(mask->empty() || mask->size() == numInstances)) {
      TF_WARN("%s -- found mask of size [%zu], but expected size [%zu]",
              GetPrim().GetPath().GetText(),
              mask->size(),
              numInstances);
      return false;
...
bool UsdGeomPointInstancer::ComputeInstanceTransformsAtTime(
...
bool UsdGeomPointInstancer::ComputeInstanceTransformsAtTimes(
    std::vector<VtArray<GfMatrix4d>> *xformsArray,
    ...
    const MaskApplication applyMask) const
{
  ...
  if (!UsdGeom_GetPositionsVelocitiesAndAccelerations(GetPositionsAttr(),
                                                      GetVelocitiesAttr(),
  ...
  UsdGeom_GetOrientationsAndAngularVelocities(GetOrientationsAttr(),
  ...
  for (size_t i = 0; i < numSamples; i++) {
    ...
    if (!UsdGeomPointInstancer::ComputeInstanceTransformsAtTime(xforms,
                                                                stage,
                                                                time,
                                                                protoIndices,
                                                                positions,
    ...
    float velocityScale)
{
  ...
  const auto computeInstanceXforms = [&mask,
                                      &velocityTimeDelta,
                                                         ...
                                                         const GfMatrix4d *transform) const
{
  ...
  if (protoIndices.size() <= protoPaths.size()) {
    TF_DEBUG(USDGEOM_BBOX)
        .Msg(
            "Number of prototypes (%zu) is >= number"
  ...
  const auto computeInstanceAlignedRange = [&mask,
                                            &protoIndices,
                                            &transform,
                                            &protoUntransformedBounds,
                                            &instanceTransforms,
                                            &instanceAlignedRanges](size_t start, size_t end) {
    for (size_t instanceId = start; instanceId < end; ++instanceId) {
      if (!mask.empty() && !mask[instanceId]) {
        continue;
      }
                                                 ...
                                                 const GfMatrix4d *transform) const
{
  ...
  if (!ComputeInstanceTransformsAtTime(
          &instanceTransforms, time, baseTime, IncludeProtoXform, IgnoreMask)) {
                                                  ...
                                                  const GfMatrix4d *transform) const
{
  ...
  for (size_t i = 0; i < times.size(); i++) {
    ...
    if (!_ComputeExtentFromTransforms(&(computedExtents[i]),
                                      protoIndices,
                                      mask,
                                      prototypes,
                                      protoPaths,
