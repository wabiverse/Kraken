...
////////////////////////////////////////////////////////////////////////
//
// UsdGeomConnectableAPIBehavior base implementation
//

UsdGeomConnectableAPIBehavior::~UsdGeomConnectableAPIBehavior()
{}

bool UsdGeomConnectableAPIBehavior::CanConnectInputToSource(const UsdGeomInput &input,
...
bool UsdGeomConnectableAPIBehavior::_CanConnectInputToSource(const UsdGeomInput &input,
                                                             const UsdAttribute &source,
                                                             std::string *reason,
                                                             ConnectableNodeTypes nodeType)
{
  ...
  auto encapsulationCheckForInputSources = [&input, &source](std::string *reason) {
    ...
    if (!UsdGeomConnectableAPI(source.GetPrim()).IsContainer()) {
      if (reason) {
  ...
  auto encapsulationCheckForOutputSources = [&input, &source, &nodeType](std::string *reason) {
    ...
    switch (nodeType) {
      case ConnectableNodeTypes::DerivedContainerNodes:
        if (!UsdGeomConnectableAPI(input.GetPrim()).IsContainer()) {
          if (reason) {
            *reason = TfStringPrintf(
                "Encapsulation check failed - "
      ...
      default:
        if (!UsdGeomConnectableAPI(input.GetPrim().GetParent()).IsContainer()) {
          if (reason) {
            *reason = TfStringPrintf(
                "Encapsulation check failed - "
  ...
  if (inputConnectability == UsdGeomTokens->full) {
    if (UsdGeomInput::IsInput(source)) {
  ...
  else if (inputConnectability == UsdGeomTokens->interfaceOnly) {
    if (UsdGeomInput::IsInput(source)) {
      TfToken sourceConnectability = UsdGeomInput(source).GetConnectability();
      if (sourceConnectability == UsdGeomTokens->interfaceOnly) {
        if (encapsulationCheckForInputSources(reason)) {
          return true;
        }
        return false;
...
bool UsdGeomConnectableAPIBehavior::_CanConnectOutputToSource(const UsdGeomOutput &output,
                                                              const UsdAttribute &source,
                                                              std::string *reason,
                                                              ConnectableNodeTypes nodeType)
{
...
bool UsdGeomConnectableAPIBehavior::CanConnectOutputToSource(const UsdGeomOutput &,
                                                             const UsdAttribute &,
                                                             std::string *reason)
{
...
bool UsdGeomConnectableAPIBehavior::IsContainer() const
{
  return false;
}
...
////////////////////////////////////////////////////////////////////////
//
// UsdGeomConnectableAPIBehavior registry
//
...
class _BehaviorRegistry : public TfWeakBase {
 public:
  ...
  _BehaviorRegistry() : _initialized(false)
  {
    ...
    TfRegistryManager::GetInstance().SubscribeTo<UsdGeomConnectableAPI>();
                        ...
                        const std::shared_ptr<UsdGeomConnectableAPIBehavior> &behavior)
  {
    ...
    if (!didInsert) {
      TF_CODING_ERROR(
          "UsdGeom Connectable behavior already registered for "
          "prim type '%s'",
  ...
  UsdGeomConnectableAPIBehavior *GetBehavior(const UsdPrim &prim)
  {
    _WaitUntilInitialized();
    ...
    std::shared_ptr<UsdGeomConnectableAPIBehavior> behavior;
 ...
 private:
  ...
  bool _LoadPluginForType(const TfType &type) const
  {
    ...
    const JsValue implementsUsdGeomConnectableAPIBehavior = plugReg.GetDataFromPluginMetaData(
        type, "implementsUsdGeomConnectableAPIBehavior");
    if (!implementsUsdGeomConnectableAPIBehavior.Is<bool>() ||
        !implementsUsdGeomConnectableAPIBehavior.Get<bool>()) {
      return false;
    }
  ...
  bool _FindBehaviorForType(const TfType &type,
                            std::shared_ptr<UsdGeomConnectableAPIBehavior> *behavior) const
  {
    _RWMutex::scoped_lock lock(_mutex, /* write = */ false);
 ...
 private:
  ...
  using _Registry =
      std::unordered_map<TfType, std::shared_ptr<UsdGeomConnectableAPIBehavior>, TfHash>;
  _Registry _registry;
...
////////////////////////////////////////////////////////////////////////
//
// UsdGeomConnectableAPI implementations using registered behavior
//

/* virtual */
bool UsdGeomConnectableAPI::_IsCompatible() const
{
...
bool UsdGeomConnectableAPI::CanConnect(const UsdGeomInput &input, const UsdAttribute &source)
{
  ...
  // validation in USD.
  std::string reason;
  if (UsdGeomConnectableAPIBehavior *behavior = _BehaviorRegistry::GetInstance().GetBehavior(
          input.GetPrim())) {
...
bool UsdGeomConnectableAPI::CanConnect(const UsdGeomOutput &output, const UsdAttribute &source)
{
  // The reason why a connection can't be made isn't exposed currently.
  // We may want to expose it in the future, especially when we have
  // validation in USD.
...
/* static */
bool UsdGeomConnectableAPI::HasConnectableAPI(const TfType &schemaType)
{
  return _BehaviorRegistry::GetInstance().HasBehaviorForType(schemaType);
}

bool UsdGeomConnectableAPI::IsContainer() const
{
  if (UsdGeomConnectableAPIBehavior *behavior = _BehaviorRegistry::GetInstance().GetBehavior(
          GetPrim())) {
