...
#ifndef USDGEOM_GENERATED_NODEDEFAPI_H
#define USDGEOM_GENERATED_NODEDEFAPI_H

/**
 *  @file usdGeom/nodeDefAPI.h */
...
/**
 * @class UsdGeomNodeDefAPI
 *
 * UsdGeomNodeDefAPI is an API schema that provides attributes for a prim
 * to select a corresponding Geom Node Definition ("Geom Node"), as well
 * as to look up a runtime entry for that geom node in the form of an
 * GdrGeomNode.
 *
 * UsdGeomNodeDefAPI is intended to be a pre-applied API schema for any prim
 * type that wants to refer to the GdrRegistry for further implementation details
 * about the behavior of that prim. The primary use in UsdGeom itself as UsdGeomGprim,
 * which is a basis for all geometric primitives (UsdGeomMaterial), but this is intended
 * to be used in other domains that also use the Gdr node mechanism.
 *
 * This schema provides properties that allow a prim to identify an external
 * node definition, either by a direct identifier key into the GdrRegistry
 ...
 * as the value. */
class UsdGeomNodeDefAPI : public UsdAPISchemaBase {
 public:
  /**
  ...
  /**
   * Construct a UsdGeomNodeDefAPI on UsdPrim @p prim .
   * Equivalent to UsdGeomNodeDefAPI::Get(prim.GetStage(), prim.GetPath())
   * for a @em valid @p prim, but will not immediately throw an error for
   * an invalid @p prim. */
  explicit UsdGeomNodeDefAPI(const UsdPrim &prim = UsdPrim()) : UsdAPISchemaBase(prim)
  {}

  /**
   * Construct a UsdGeomNodeDefAPI on the prim held by @p schemaObj.
   * Should be preferred over UsdGeomNodeDefAPI(schemaObj.GetPrim()),
   * as it preserves SchemaBase state. */
  explicit UsdGeomNodeDefAPI(const UsdSchemaBase &schemaObj) : UsdAPISchemaBase(schemaObj)
  {}

  /**
   * Destructor. */
  USDGEOM_API
  virtual ~UsdGeomNodeDefAPI();
  ...
  /**
   * Return a UsdGeomNodeDefAPI holding the prim adhering to this
   * schema at @p path on @p stage.  If no prim exists at @p path on
   * @p stage, or if the prim at that path does not adhere to this schema,
   * return an invalid schema object.  This is shorthand for the following:
   *
   * @code
   * UsdGeomNodeDefAPI(stage->GetPrimAtPath(path));
   * @endcode */
  USDGEOM_API
  static UsdGeomNodeDefAPI Get(const UsdStagePtr &stage, const SdfPath &path);

  /**
   * Applies this <b>single-apply</b> API schema to the given @p prim.
   * This information is stored by adding "GeomNodeDefAPI" to the token
   * valued, listOp metadata @em apiSchemas on the prim.
   *
   * @return A valid UsdGeomNodeDefAPI object is returned upon success.
   * An invalid (or empty) UsdGeomNodeDefAPI object is returned upon
   * failure. See @ref UsdPrim::ApplyAPI() for conditions resulting in
   * failure.
   *
  ...
  USDGEOM_API
  static UsdGeomNodeDefAPI Apply(const UsdPrim &prim);
 ...
 protected:
   ...
   * compatibility with older generated schemas. */
  USDGEOM_API UsdSchemaKind _GetSchemaType() const override;
 ...
 public:
   ...
   * ---------------------------------------------------------------------
   *//** 
   * Specifies the attribute that should be consulted to get the geom's
   * implementation or its source code.
   *
   * * If set to "id", the "info:id" attribute's value is used to
   * determine the geom source from the geom registry.
   ...
   * is used to locate the geom source.  A source asset file may also
   * specify multiple geom definitions, so there is an optional attribute
   * "info:sourceAsset:subIdentifier" whose value should be used to indicate
   * a particular geom definition from a source asset file.
   ...
   * | @ref UsdGeomTokens "Allowed Values" | id, sourceAsset, sourceCode | */
  USDGEOM_API
 ...
 public:
   ...
   * geom. E.g.: Mesh, Fluid, Fur, Physics, and so fourth.
   * The use of this id will depend on the primitive target: some will turn
   * it into an actual geom path, some will use it to generate geometric
 ...
 public:
  ...
  /**
   * @anchor UsdGeomNodeDefAPI_ImplementationSource
   * @name Geom Source API
   *
   * This section provides API for identifying the source of a geom's
   * implementation.
   *
   * @{
   *
   ...
   * <b>sourceType</b> should be read to get geom's source.
   *
   ...
   * the type of geom source or its implementation. For example: osl,
   * glslfx, riCpp etc. This allows a geom to specify different sourceAsset
   ...
   * used to parse the geom source (\ref NdrParserPlugin::SourceType).
   *
   ...
   * empty-valued token UsdGeomTokens->universalSourceType. When the
  ...
  /**
   * Sets the geom's ID value. This also sets the
   * <i>info:implementationSource</i> attribute on the geom to
   * <b>UsdGeomTokens->id</b>, if the existing value is different. */
  USDGEOM_API
  bool SetGeomId(const TfToken &id) const;

  /**
   * Fetches the geom's ID value from the <i>info:id</i> attribute, if the
   * geom's <i>info:implementationSource</i> is <b>id</b>.
   *
   * Returns <b>true</b> if the geom's implementation source is <b>id</b>
   * and the value was fetched properly into @p id. Returns false otherwise.
   *
   * @sa GetImplementationSource() */
  USDGEOM_API
  bool GetGeomId(TfToken *id) const;

  /**
   * Sets the geom's source-asset path value to @p sourceAsset for the
   * given source type, @p sourceType.
   *
   * This also sets the <i>info:implementationSource</i> attribute on the
   * geom to <b>UsdGeomTokens->sourceAsset</b>. */
  USDGEOM_API
  bool SetSourceAsset(const SdfAssetPath &sourceAsset,
                      const TfToken &sourceType = UsdGeomTokens->universalSourceType) const;

  /**
   * Fetches the geom's source asset value for the specified
   * @p sourceType value from the <b>info:<i>sourceType:</i>sourceAsset</b>
   * attribute, if the geom's <i>info:implementationSource</i> is
   * <b>sourceAsset</b>.
   *
   * If the <i>sourceAsset</i> attribute corresponding to the requested
   * <i>sourceType</i> isn't present on the geom, then the <i>universal</i>
   * <i>fallback</i> sourceAsset attribute, i.e. <i>info:sourceAsset</i> is
   * consulted, if present, to get the source asset path.
   *
   * Returns <b>true</b> if the geom's implementation source is
  ...
  /**
   * Fetches the geom's sub-identifier for the source asset with the
   * specified @p sourceType value from the <b>info:<i>sourceType:</i>
   * sourceAsset:subIdentifier</b> attribute, if the geom's <i>info:
  ...
  /**
   * Sets the geom's source-code value to @p sourceCode for the given
   * source type, @p sourceType.
   *
   * This also sets the <i>info:implementationSource</i> attribute on the
   * geom to <b>UsdGeomTokens->sourceCode</b>. */
  USDGEOM_API
  bool SetSourceCode(const std::string &sourceCode,
                     const TfToken &sourceType = UsdGeomTokens->universalSourceType) const;

  /**
   * Fetches the geom's source code for the specified @p sourceType value
   * by reading the <b>info:<i>sourceType:</i>sourceCode</b> attribute, if
   * the geom's <i>info:implementationSource</i> is <b>sourceCode</b>.
   *
   * If the <i>sourceCode</i> attribute corresponding to the
   * requested <i>sourceType</i> isn't present on the geom, then the
  ...
  /**
   * This method attempts to ensure that there is a GeomNode in the geom
   * registry (i.e. @ref GdrRegistry) representing this geom for the
   * given @p sourceType. It may return a null pointer if none could be
   * found or created. */
  USDGEOM_API
  GdrGeomNodeConstPtr GetGeomNodeForSourceType(const TfToken &sourceType) const;
};

WABI_NAMESPACE_END

#endif /* USDGEOM_GENERATED_NODEDEFAPI_H */
