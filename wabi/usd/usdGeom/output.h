 ...
 */
#ifndef WABI_USD_USD_GEOM_OUTPUT_H
#define WABI_USD_USD_GEOM_OUTPUT_H
...
/// \class UsdGeomOutput
///
/// This class encapsulates a geom or node-graph output, which is a
...
class UsdGeomOutput {
 public:
  ...
  /// on its GetTypeName()
  ///
  /// For example, we set the primitiveType to "struct" for outputs that
  ...
  /// Return this output's specialized primitiveType, or an empty
  ...
  /// Return true if a primitiveType has been specified for this
  /// output.
  ///
  /// \sa SetPrimitiveType()
  USDGEOM_API
  ...
  /// \name API to author and query an Output's gdrMetadata
  ///
  /// This section provides API for authoring and querying geom registry
  /// metadata on an Output. When the owning geom prim is providing a geom
  /// definition, the authored "gdrMetadata" dictionary value provides
  /// metadata needed to populate the Output correctly in the geom registry.
  ///
  ...
  /// Returns this Output's composed "gdrMetadata" dictionary as a
  /// NdrTokenMap.
  USDGEOM_API
  ...
  /// Authors the given \p gdrMetadata value on this Output at the current
  /// EditTarget.
  USDGEOM_API
  ...
  /// the Output's "gdrMetadata" dictionary at the current EditTarget.
  USDGEOM_API
  void SetGdrMetadataByKey(const TfToken &key, const std::string &value) const;

  /// Returns true if the Output has a non-empty composed "gdrMetadata"
  /// dictionary value.
  USDGEOM_API
  ...
  /// Clears any "gdrMetadata" value authored on the Output in the current
  /// EditTarget.
  USDGEOM_API
  ...
  /// Speculative constructor that will produce a valid UsdGeomOutput when
  /// \p attr already represents a geom Output, and produces an \em invalid
  /// UsdGeomOutput otherwise (i.e. the explicit bool conversion operator
  /// will return false).
  USDGEOM_API
  explicit UsdGeomOutput(const UsdAttribute &attr);

  /// Test whether a given UsdAttribute represents a valid Output, which
  /// implies that creating a UsdGeomOutput from the attribute will succeed.
  ///
  ...
  /// Allow UsdGeomOutput to auto-convert to UsdAttribute, so you can
  /// pass a UsdGeomOutput to any function that accepts a UsdAttribute or
  /// const-ref thereto.
  ...
  /// same UsdGeomOutput, false otherwise.
  friend bool operator==(const UsdGeomOutput &lhs, const UsdGeomOutput &rhs)
  {
    return lhs.GetAttr() == rhs.GetAttr();
  }

  /// Inequality comparison. Return false if \a lhs and \a rhs represent the
  /// same UsdGeomOutput, true otherwise.
  friend bool operator!=(const UsdGeomOutput &lhs, const UsdGeomOutput &rhs)
  {
    return !(lhs == rhs);
  }
 ...
 private:
  ...
  // Constructor that creates a UsdGeomOutput with the given name on the
  // given prim.
  // \p name here is the unnamespaced name of the output.
  UsdGeomOutput(UsdPrim prim, TfToken const &name, SdfValueTypeName const &typeName);
...
#endif  // WABI_USD_USD_GEOM_OUTPUT_H
