...
{
  const NdrTokenMap::const_iterator widgetSearch = metadata.find(GdrPropertyMetadata->Widget);
...
{
  const NdrTokenMap::const_iterator roleSearch = metadata.find(GdrPropertyMetadata->Role);
  ...
  if (roleSearch != metadata.end()) {
    ...
    if (std::find(GdrPropertyRole->allTokens.begin(), GdrPropertyRole->allTokens.end(), role) !=
        GdrPropertyRole->allTokens.end()) {
      return role;
    }
  }
  /**
