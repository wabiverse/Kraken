...
struct GdrGeomPropertyConstPtrToPythonConverter {
  static PyObject *convert(GdrGeomPropertyConstPtr geomProperty)
  {
    object geomPropertyObject(ptr(geomProperty));

    return incref(geomPropertyObject.ptr());
  }
};

void wrapGeomProperty()
{
  typedef GdrGeomProperty This;
  typedef GdrGeomPropertyPtr ThisPtr;

  TF_PY_WRAP_PUBLIC_TOKENS("PropertyTypes", GdrPropertyTypes, GDR_PROPERTY_TYPE_TOKENS);
  TF_PY_WRAP_PUBLIC_TOKENS("PropertyMetadata", GdrPropertyMetadata, GDR_PROPERTY_METADATA_TOKENS);
  TF_PY_WRAP_PUBLIC_TOKENS("PropertyRole", GdrPropertyRole, GDR_PROPERTY_ROLE_TOKENS);

  to_python_converter<NdrOption, TfTokenPairToPythonConverter>();
  return_value_policy<copy_const_reference> copyRefPolicy;

  to_python_converter<GdrGeomPropertyConstPtr, GdrGeomPropertyConstPtrToPythonConverter>();

  class_<This, ThisPtr, bases<NdrProperty>, boost::noncopyable>("GeomProperty", no_init)
