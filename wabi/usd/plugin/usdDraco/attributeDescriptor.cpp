...
{
  static const std::set<TfToken> supportedInterpolations = {UsdGeomTokens->vertex,
...
{
  return Create(draco::GeometryAttribute::GENERIC,
                GetHoleFacesName(),
                draco::DT_UINT8,
                false,
                1,
    ...
    bool isPrimvar)
{
  ...
  // Create a descriptor.
  return UsdDracoAttributeDescriptor::Create(attribute.attribute_type(),
                                             TfToken(name.c_str()),
                                             attribute.data_type(),
                                             isPrimvar,
    ...
    const TfToken &interpolation)
{
  ...
  if (dimensionOneSize < 1 || dimensionOneSize > 4 || dimensionTwoSize < 1 || dimensionTwoSize > 4)
    return Invalid();
...
{
  // Create metadata.
  std::unique_ptr<draco::AttributeMetadata> metadata = std::unique_ptr<draco::AttributeMetadata>(
  ...
  if (_shape != GetDefaultShape())
    metadata->AddEntryString(METADATA_SHAPE_KEY.c_str(), GetShapeText());
  if (_isHalf != GetDefaultHalf())
    metadata->AddEntryInt(METADATA_HALF_KEY.c_str(), static_cast<int>(_isHalf));
