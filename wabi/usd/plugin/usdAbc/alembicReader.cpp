...
namespace
{
  ...
  static const char *_WarningNames[] = {"visibility",
                                        "subdivision scheme",
                                           ...
                                           const std::string &replacementValue)
  {
    ...
    if (issueWarning) {
      TF_WARN("Unsupported %s detected in archive '%s'. Using '%s' instead.",
              _WarningNames[warning],
              archiveName.c_str(),
              replacementValue.c_str());
    }
  ...
  static void _GetStringMetadata(const MetaData &alembicMetadata,
                                 MetadataMap &usdMetadata,
                                 const TfToken &field)
  {
    std::string value = alembicMetadata.get(_AmdName(field));
  ...
  static void _GetTokenMetadata(const MetaData &alembicMetadata,
                                MetadataMap &usdMetadata,
                                const TfToken &field)
  {
    std::string value = alembicMetadata.get(_AmdName(field));
  ...
  static void _GetDoubleMetadata(const MetaData &alembicMetadata,
                                 MetadataMap &usdMetadata,
                                 const TfToken &field)
  {
    std::string value = alembicMetadata.get(_AmdName(field));
  ...
  template<class T> struct _AlembicPropertyHelper
  {
    //  T operator()(const ICompoundProperty& parent, const std::string& name)const;
  };
  template<> struct _AlembicPropertyHelper<ICompoundProperty>
  {
  ...
  template<> struct _AlembicPropertyHelper<IScalarProperty>
  {
  ...
  template<class T> struct _AlembicPropertyHelper<ITypedScalarProperty<T>>
  {
  ...
  template<> struct _AlembicPropertyHelper<IArrayProperty>
  {
  ...
  template<class T> struct _AlembicPropertyHelper<ITypedArrayProperty<T>>
  {
  ...
  template<class T> struct _AlembicPropertyHelper<ITypedGeomParam<T>>
  {
  ...
  class AlembicProperty
  {
   public:
    ...
    template<class T> T Cast(SchemaInterpMatching matching = kStrictMatching) const
    {
      if (_parent.valid()) {
  ...
  class _ReaderContext
  {
   public:
    ...
    void SetSchema(const _ReaderSchema *schema)
    {
      _schema = schema;
    }

    /// Returns the reader schema.
    const _ReaderSchema &GetSchema() const
    {
      return *_schema;
    }
                            ...
                            const SdfFileFormat::FileFormatArguments &args)
  {
    ...
    for (size_t i = 0, n = root.getNumChildren(); i != n; ++i) {
      ...
      std::string name = _CleanName(child.getName(),
                                    " _",
                                    usedRootNames,
                                    _AlembicFixName(),
  ...
  bool _ReaderContext::HasField(const SdfPath &path,
                                const TfToken &fieldName,
                                const UsdAbc_AlembicDataAny &value) const
  {
    TRACE_FUNCTION();
                                        ...
                                        std::set<std::string> *usedNames)
  {
    ...
    for (const auto &value : instances) {
      ...
      const SdfPath prototypePath = SdfPath::AbsoluteRootPath().AppendChild(
        TfToken(_CleanName(prototypeName,
                           " _",
                           *usedNames,
                           _AlembicFixName(),
                           &SdfPath::IsValidIdentifier)));
  ...
  bool _ReaderContext::_HasField(const Prim *prim,
                                 const TfToken &fieldName,
                                 const UsdAbc_AlembicDataAny &value) const
  {
  ...
  template<typename T> size_t _GetNumSamples(const T &object)
  {
  ...
  template<class T> static _AlembicTimeSamples _GetSampleTimes(const T &object)
  {
  ...
  template<class T> static _AlembicTimeSamples _GetSampleTimes(const ISchemaObject<T> &object)
  {
  ...
  {
    std::vector<std::string>::iterator i = std::find(_unextracted.begin(),
                                                     _unextracted.end(),
                                                     name);
  ...
  {
    std::vector<std::string>::iterator i = std::find(_unextractedSchema.begin(),
                                                     _unextractedSchema.end(),
                                                     name);
  ...
  template<class T>
  void _PrimReaderContext::AddProperty(const TfToken &name,
                                       const SdfValueTypeName &typeName,
                                       const T &converter)
  {
  ...
  template<class T>
  void _PrimReaderContext::AddUniformProperty(const TfToken &name,
                                              const SdfValueTypeName &typeName,
                                              const T &converter)
  {
    if (converter(IsValidTag())) {
                                                  ...
                                                  const AlembicProperty &property)
  {
    ...
    const SdfValueTypeName usdTypeName = _context.GetSchema().GetConversions().FindConverter(
      alembicType);
    if (usdTypeName) {
      _PrimReaderContext::Property &prop = (TfGetEnvSetting(
                                              USD_ABC_WRITE_UV_AS_ST_TEXCOORD2FARRAY) &&
    ...
    bool isOutOfSchemaProperty)
  {
    ...
    fprintf(stdout,
            ...
            property.typeName.GetAsToken().GetText(),
            name.GetText());
#endif
                                                ...
                                                bool isOutOfSchemaProperty) const
  {
    ...
    property->typeName = _GetInterpretation(property->typeName,
  ...
  struct _CopySynthetic
  {
    VtValue value;
    MetaData metadata;
    template<class T> _CopySynthetic(const T &value_) : value(value_) {}
  ...
  template<class T, class UsdValueType = void, bool expand = true> struct _CopyGeneric
  {
  ...
  template<class GeomParamType> struct _CopyIndices
  {
    typedef typename GeomParamType::prop_type PropertyType;
    typedef typename PropertyType::traits_type AlembicTraits;
    ...
    bool operator()(const UsdAbc_AlembicDataAny &dst, const ISampleSelector &iss) const
    {
      ...
      switch (vis) {
        ...
        case kVisibilityVisible: {
          const std::string authoredValue = (vis == kVisibilityVisible ? "kVisibilityVisible" :
                                                                         TfStringify(vis));
  ...
  {
    // Add properties.
    context->AddProperty(UsdGeomTokens->extent,
  ...
  {
    // Add userProperties.
    context->AddOutOfSchemaProperty(UsdAbcPropertyNames->userProperties,
  ...
  {
    // Add properties.
    context->AddProperty(UsdGeomTokens->visibility,
  ...
  static void _ReadMayaColor(_PrimReaderContext *context)
  {
    ...
    // Add properties.
    context->AddProperty(displayColor,
  ...
  void _ReadProperty(_PrimReaderContext *context,
                     const char *name,
                     TfToken propName,
                     SdfValueTypeName typeName)
  {
  ...
  static void _ReadOrientation(_PrimReaderContext *context)
  {
    ...
    if (orientation.Cast<IStringProperty>().valid()) {
      context->AddProperty(UsdGeomTokens->orientation,
                           SdfValueTypeNames->Token,
  ...
  static void _ReadXform(_PrimReaderContext *context)
  {
    ...
    if (nSamples > 0) {
      ...
      //
      context->AddProperty(_tokens->xformOpTransform,
                           SdfValueTypeNames->Matrix4d,
                           _CopyXform(object));

      VtTokenArray opOrderVec(1);
      opOrderVec[0] = _tokens->xformOpTransform;
      context->AddUniformProperty(UsdGeomTokens->xformOpOrder,
  ...
  static void _ReadFaceSet(_PrimReaderContext *context)
  {
    ...
    context->AddProperty(UsdGeomTokens->indices,
                         SdfValueTypeNames->IntArray,
                         _CopyGeneric<IInt32ArrayProperty, int>(context->ExtractSchema(".faces")));
    context->AddUniformProperty(UsdGeomTokens->elementType,
                                SdfValueTypeNames->Token,
    ...
    parentPrimContext.AddUniformProperty(UsdAbcPropertyNames->defaultFamilyTypeAttributeName,
                                         SdfValueTypeNames->Token,
  ...
  static void _ReadCurves(_PrimReaderContext *context)
  {
    ...
    if (sample.getType() == kVariableOrder) {
      ...
      context->AddProperty(UsdGeomTokens->tangents,
                           SdfValueTypeNames->Vector3fArray,
  ...
  static void _ReadCameraParameters(_PrimReaderContext *context)
  {
    ...
    // camera frustum.
    context->AddProperty(UsdGeomTokens->focalLength,
    ...
    context->AddProperty(UsdGeomTokens->verticalApertureOffset,
                         SdfValueTypeNames->Float,
                         _CopyCameraVerticalApertureOffset(object));
    context->AddProperty(UsdGeomTokens->clippingRange,
  ...
  {
    // Choose the name.
    std::string name = _CleanName(object.getName(),
                                  " _",
                                  *usedSiblingNames,
                                  _AlembicFixName(),
                                  &SdfPath::IsValidIdentifier);
...
template<class T>
bool UsdAbc_AlembicDataReader::TimeSamples::Bracket(const T &samples,
                                                    double usdTime,
                                                    double *tLower,
                                                    double *tUpper)
{
...
bool UsdAbc_AlembicDataReader::Open(const std::string &filePath,
                                    const SdfFileFormat::FileFormatArguments &args)
{
  TRACE_FUNCTION();
...
void UsdAbc_AlembicDataReader::VisitSpecs(const SdfAbstractData &owner,
                                          SdfAbstractDataSpecVisitor *visitor) const
{
