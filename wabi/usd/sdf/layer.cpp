...
#include "wabi/usd/sdf/layerRegistry.h"
    ...
    _hints{/*.mightHaveRelocates =*/false}
{
  ...
  string layerIdentifier = Sdf_IsAnonLayerIdentifier(identifier) ?
                             Sdf_ComputeAnonLayerIdentifier(identifier, this) :
                             identifier;
...
#if AR_VERSION == 1
static bool _ModificationTimesEqual(const VtValue &t1, const VtValue &t2)
{
  return t1 == t2;
}
                                                    ...
                                                    const FileFormatArguments &args)
{
  ...
  SdfLayerRefPtr layer = _CreateNewWithFormat(fileFormat,
                                              Sdf_GetAnonLayerIdentifierTemplate(tag),
                                              string(),
                                              ArAssetInfo(),
                                              args);
                                    ...
                                    const FileFormatArguments &args)
{
  ...
  const string absIdentifier = isRelativePath ? TfAbsPath(identifier) : identifier;
  ...
  SdfLayerRefPtr layer;
  {
    ...
    layer = _CreateNewWithFormat(fileFormat, absIdentifier, localPath, ArAssetInfo(), args);
                             ...
                             const FileFormatArguments &args)
{
  ...
  // When creating a new layer, assume that relative identifiers are
  // relative to the current working directory.
  const string absIdentifier = ArGetResolver().IsRelativePath(identifier) ? TfAbsPath(identifier) :
                                                                            identifier;
#else
  ...
  SdfLayerRefPtr layer =
    _CreateNewWithFormat(fileFormat, absIdentifier, std::string(), ArAssetInfo(), args);
...
static SdfLayer::FileFormatArguments &_CanonicalizeFileFormatArguments(
                                             ...
                                             bool computeAssetInfo)
{
  ...
  // real path for it.
  ArAssetInfo assetInfo;
  string resolvedLayerPath = isAnonymous ?
                               layerPath :
  ...
  info->fileFormat = SdfFileFormat::FindByExtension(resolvedLayerPath.empty() ? layerPath :
                                                                                resolvedLayerPath,
                                                    layerArgs);
  info->fileFormatArgs.swap(
    _CanonicalizeFileFormatArguments(layerPath, info->fileFormat, layerArgs));
...
SdfLayerRefPtr SdfLayer::FindOrOpen(const string &identifier, const FileFormatArguments &args)
{
  ...
  WorkWithScopedParallelism([&]() -> SdfLayerRefPtr {
    ...
    if (SdfLayerRefPtr layer = _TryToFindLayer(layerInfo.identifier,
                                               layerInfo.resolvedLayerPath,
                                               lock,
                                               /*retryAsWriter=*/true)) {
...
/* static */
SdfLayerRefPtr SdfLayer::OpenAsAnonymous(const std::string &layerPath,
                                         bool metadataOnly,
                                         const std::string &tag)
{
  _FindOrOpenLayerInfo layerInfo;
  ...
  {
    tbb::queuing_rw_mutex::scoped_lock lock(_GetLayerRegistryMutex());
    layer = _CreateNewWithFormat(layerInfo.fileFormat,
                                 Sdf_GetAnonLayerIdentifierTemplate(tag),
                                 string());
...
SdfLayer::_ReloadResult SdfLayer::_Reload(bool force)
{
  ...
  if (isAnonymous && GetFileFormat()->ShouldSkipAnonymousReload()) {
    ...
    if (resolvedPath.empty()) {
      TF_RUNTIME_ERROR("Cannot determine resolved path for '%s', skipping reload.",
                       identifier.c_str());
      return _ReloadFailed;
    }
    ...
    VtValue timestamp(ArGetResolver().GetModificationTimestamp(layerPath, resolvedPath));
#if AR_VERSION == 1
    if (timestamp.IsEmpty()) {
      TF_CODING_ERROR("Unable to get modification time for '%s (%s)'",
                      GetIdentifier().c_str(),
...
bool SdfLayer::_Read(const string &identifier, const string &resolvedPath, bool metadataOnly)
{
  TRACE_FUNCTION();
  ...
  if (format->LayersAreFileBased()) {
    ...
    TF_DEBUG(SDF_LAYER).Msg("SdfLayer::_Read - fetched '%s' to local path '%s'\n",
                            identifier.c_str(),
                            resolvedPath.c_str());
  }
#endif
...
bool SdfLayer::GetBracketingTimeSamplesForPath(const SdfPath &path,
                                               double time,
                                               double *tLower,
                                               double *tUpper)
{
...
static TfType _GetExpectedTimeSampleValueType(const SdfLayer &layer, const SdfPath &path)
{
  ...
  if (specType == SdfSpecTypeUnknown) {
    TF_CODING_ERROR(
      ...
      path.GetText());
    return TfType();
  } else if (specType != SdfSpecTypeAttribute && specType != SdfSpecTypeRelationship) {
    TF_CODING_ERROR(
...
void SdfLayer::SetTimeSample(const SdfPath &path,
                             double time,
...
template<class T>
void SdfLayer::_PrimSetTimeSample(const SdfPath &path,
                                  double time,
                                  const T &value,
                                  bool useDelegate)
{
...
template void SdfLayer::_PrimSetTimeSample(const SdfPath &,
                                           double,
...
template<class T> inline void SdfLayer::_SetValue(const TfToken &key, T value)
{
  SetField(SdfPath::AbsoluteRootPath(), key, VtValue(value));
}

template<class T> inline T SdfLayer::_GetValue(const TfToken &key) const
{
  VtValue value;
...
SdfSubLayerProxy SdfLayer::GetSubLayerPaths() const
{
  boost::shared_ptr<Sdf_ListEditor<SdfSubLayerTypePolicy>> editor(
    new Sdf_SubLayerListEditor(_self));
...
{
  return GetFieldAs<SdfLayerOffsetVector>(SdfPath::AbsoluteRootPath(),
...
{
  SdfLayerOffsetVector offsets = GetFieldAs<SdfLayerOffsetVector>(SdfPath::AbsoluteRootPath(),
                                                                  SdfFieldKeys->SubLayerOffsets);
  if (index < 0 || static_cast<size_t>(index) >= offsets.size()) {
    TF_CODING_ERROR("Invalid sublayer index");
    return;
...
bool SdfLayer::_CanGetSpecAtPath(const SdfPath &path,
...
template<class Spec> SdfHandle<Spec> SdfLayer::_GetSpecAtPath(const SdfPath &path)
{
  SdfPath canonicalPath;
...
{
  return _permissionToSave && !IsAnonymous() && !IsMuted() &&
...
static bool _CanEdit(const SdfLayerHandle &layer,
  ...
  if (edit.currentPath.IsPrimPath()) {
    if (edit.newPath.IsEmpty()) {
      ...
      return Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::CanRemoveChildForBatchNamespaceEdit(
        layer,
        edit.currentPath.GetParentPath(),
...
void SdfLayer::_RemoveIfInert(const SdfSpec &spec)
{
  if (!spec.IsDormant()) {
    ...
    } else if (SdfPropertySpecHandle property = TfDynamic_cast<SdfPropertySpecHandle>(
                 specHandle)) {
...
void SdfLayer::RemovePropertyIfHasOnlyRequiredFields(SdfPropertySpecHandle prop)
{
  ...
  if (SdfPrimSpecHandle owner = TfDynamic_cast<SdfPrimSpecHandle>(prop->GetOwner())) {
    ...
    Sdf_ChildrenUtils<Sdf_AttributeChildPolicy>::RemoveChild(_self,
                                                             attr->GetPath().GetParentPath(),
                                                             attr->GetNameToken());
  } else if (SdfRelationshipSpecHandle rel = TfDynamic_cast<SdfRelationshipSpecHandle>(prop)) {
    Sdf_ChildrenUtils<Sdf_RelationshipChildPolicy>::RemoveChild(_self,
                                                                rel->GetPath().GetParentPath(),
                                                                rel->GetNameToken());
  }
...
bool SdfLayer::SplitIdentifier(const string &identifier,
                               string *layerPath,
...
std::string SdfLayer::CreateIdentifier(const string &layerPath,
...
void SdfLayer::SetIdentifier(const string &identifier)
{
...

  {
    ...
    SdfLayerRefPtr existingLayer = _Find(absIdentifier,
                                         FileFormatArguments(),
                                         lock,
                                         retryAsWriter);
    if (existingLayer) {
...
#if AR_VERSION == 1
    _assetModificationTime = ArGetResolver().GetModificationTimestamp(GetIdentifier(),
                                                                      newResolvedPath);
#else
    const ArTimestamp timestamp = ArGetResolver().GetModificationTimestamp(newLayerPath,
                                                                           newResolvedPath);
    _assetModificationTime = (timestamp.IsValid() || Sdf_ResolvePath(newLayerPath)) ?
                               VtValue(timestamp) :
                               VtValue();
#endif
  }
...
void SdfLayer::UpdateAssetInfo(const string &fileVersion)
{
  TRACE_FUNCTION();
...
string SdfLayer::ComputeAbsolutePath(const string &assetPath) const
{
  ...
  const string relativeToPath = GetRepositoryPath().empty() ? GetRealPath() : GetRepositoryPath();
  ...
  if (didChange) {
    if (SdfLayerHandle layer = Find(path)) {
      if (layer->IsDirty()) {
        ...
        SdfAbstractDataRefPtr initializedData = format->InitData(layer->GetFileFormatArguments());
        if (layer->_data->StreamsData()) {
          ...
          SdfAbstractDataRefPtr mutedData = format->InitData(layer->GetFileFormatArguments());
...
bool SdfLayer::IsEmpty() const
{
  ...
  return GetRootPrims().empty() && GetRootPrimOrder().empty() && GetSubLayerPaths().empty();
}
...
static void _GatherPrimCompositionDependencies(const SdfPrimSpecHandle &prim,
...
bool SdfLayer::UpdateCompositionAssetDependency(const string &oldLayerPath,
...
static boost::optional<RefOrPayloadType> _UpdateRefOrPayloadPath(
                                                     ...
                                                     const string &newLayerPath)
{
  ...
  for (const auto &setNameAndSpec : variantSetMap) {
    ...
    for (const auto &variantSpec : variants) {
      _UpdatePrimCompositionDependencyPaths(variantSpec->GetPrimSpec(),
                                            oldLayerPath,
                                            newLayerPath);
    }
  }
...
/* static */
template<class Lock>
SdfLayerRefPtr SdfLayer::_OpenLayerAndUnlockRegistry(Lock &lock,
                                                     ...
                                                     bool metadataOnly)
{
  ...
  SdfLayerRefPtr layer = _CreateNewWithFormat(info.fileFormat,
                                              info.identifier,
                                              info.resolvedLayerPath,
                                              info.assetInfo,
                                              info.fileFormatArgs);
  ...
  // other layers.
  TF_VERIFY(_layerRegistry->FindByIdentifier(layer->GetIdentifier()) == layer,
            "Could not find %s",
  ...
  const string readFilePath = info.isAnonymous ? info.layerPath : info.resolvedLayerPath;
  ...
  if (!layer->IsMuted()) {
    ...
    layer->_initDispatcher.Run([&readSuccess, &layer, &info, &readFilePath, metadataOnly]() {
      readSuccess = layer->_Read(info.identifier, readFilePath, metadataOnly);
    });
  ...
  if (!info.isAnonymous) {
    ...
    VtValue timestamp(
      ArGetResolver().GetModificationTimestamp(info.layerPath, ArResolvedPath(readFilePath)));
#if AR_VERSION == 1
    if (timestamp.IsEmpty()) {
      TF_CODING_ERROR("Unable to get modification timestamp for '%s (%s)'",
                      info.identifier.c_str(),
                      readFilePath.c_str());
      layer->_FinishInitialization(/* success = */ false);
      return TfNullPtr;
  ...
  layer->_externalAssetModificationTimes = _GetExternalAssetModificationTimes(*layer);
...
vector<TfToken> SdfLayer::_ListFields(SdfSchemaBase const &schema,
...
bool SdfLayer::HasField(const SdfPath &path,
                        const TfToken &fieldName,
...
VtValue SdfLayer::GetFieldDictValueByKey(const SdfPath &path,
                                         const TfToken &fieldName,
...
static bool _IsValidFieldForLayer(const SdfLayer &layer,
                                  const SdfPath &path,
                                  const TfToken &fieldName)
{
...
void SdfLayer::SetField(const SdfPath &path,
                        const TfToken &fieldName,
                        const SdfAbstractDataConstValue &value)
{
...
void SdfLayer::EraseFieldDictValueByKey(const SdfPath &path,
                                        const TfToken &fieldName,
                                        const TfToken &keyPath)
{
  if (!PermissionToEdit()) {
...
void SdfLayer::_SetData(const SdfAbstractDataPtr &newData, const SdfSchemaBase *newDataSchema)
{
  ...
  // Update spec fields.
  {
    ...
    {
      _SpecUpdater(SdfLayer *layer_, const SdfSchemaBase &newDataSchema_)
        : layer(layer_),
    ...
    // this layer's schema.
    _SpecUpdater updater(this, newDataSchema ? *newDataSchema : GetSchema());
...
template<class T>
void SdfLayer::_PrimSetField(const SdfPath &path,
                             const TfToken &fieldName,
                             const T &value,
                             ...
                             bool useDelegate)
{
  ...
  const VtValue &oldValue = oldValuePtr ? *oldValuePtr : GetField(path, fieldName);
  const VtValue &newValue = _GetVtValue(value);

  // Send notification when leaving the change block.
  SdfChangeBlock block;

  Sdf_ChangeManager::Get().DidChangeField(_self, path, fieldName, oldValue, newValue);

  _data->Set(path, fieldName, value);
}

template void SdfLayer::_PrimSetField(const SdfPath &,
                                      const TfToken &,
                                      const VtValue &,
                                      const VtValue *,
                                      bool);
...
template<class T>
void SdfLayer::_PrimPushChild(const SdfPath &parentPath,
                              const TfToken &fieldName,
                              const T &value,
                              bool useDelegate)
{
...
template void SdfLayer::_PrimSetFieldDictValueByKey(const SdfPath &,
                                                    const TfToken &,
                                                    const TfToken &,
                                                    const VtValue &,
                                                    const VtValue *,
...
{
  const SdfPath newSpecPath = oldSpecPath.ReplacePrefix(oldRootPath,
                                                        newRootPath,
                                                        /* fixTargets = */ false);
...
bool SdfLayer::_CreateSpec(const SdfPath &path, SdfSpecType specType, bool inert)
{
  ...
  if (_data->HasSpec(path)) {
    TF_CODING_ERROR("Cannot create spec <%s> because it already exists in @%s@",
                    path.GetText(),
                    GetIdentifier().c_str());
    return false;
  }
  ...
  if (_IsInertSubtree(path, &inertSpecs)) {
    ...
    for (const SdfPath &inertSpecPath : inertSpecs) {
      if (inertSpecPath.IsPrimPath()) {
        ...
        // from the bottom up).
        VtValue val;
        if (HasField(inertSpecPath, SdfChildrenKeys->PrimChildren, &val)) {
          _PrimSetField(inertSpecPath, SdfChildrenKeys->PrimChildren, VtValue(), &val);
        }

        if (HasField(inertSpecPath, SdfChildrenKeys->PropertyChildren, &val)) {
          _PrimSetField(inertSpecPath, SdfChildrenKeys->PropertyChildren, VtValue(), &val);
        }
      }
...
void SdfLayer::_TraverseChildren(const SdfPath &path, const TraversalFunction &func)
{
  std::vector<typename ChildPolicy::FieldType> children =
    GetFieldAs<std::vector<typename ChildPolicy::FieldType>>(path,
                                                             ChildPolicy::GetChildrenToken(path));
...
void SdfLayer::_PrimDeleteSpec(const SdfPath &path, bool inert, bool useDelegate)
{
  ...
  TraversalFunction eraseFunc = std::bind(&_EraseSpecAtPath, boost::get_pointer(_data), ph::_1);
  Traverse(path, eraseFunc);
}

void SdfLayer::_PrimCreateSpec(const SdfPath &path,
                               SdfSpecType specType,
                               bool inert,
                               bool useDelegate)
{
...
bool SdfLayer::_IsInert(const SdfPath &path,
                        bool ignoreChildren,
                        bool requiredFieldOnlyPropertiesAreInert) const
{
  ...
  if (specType == SdfSpecTypePrim) {
    const SdfSpecifier specifier = GetFieldAs<SdfSpecifier>(path,
  ...
  if (specType == SdfSpecTypePrim || specType == SdfSpecTypeVariant ||
      specType == SdfSpecTypeVariantSet || specType == SdfSpecTypeAttribute ||
    ...
    TF_FOR_ALL (field, fields) {
      ...
      if (ignoreChildren &&
          ((specType == SdfSpecTypePrim && (*field == SdfChildrenKeys->PrimChildren ||
...
bool SdfLayer::_IsInertSubtree(const SdfPath &path, std::vector<SdfPath> *inertSpecs)
{
  ...
  if (path.IsPrimVariantSelectionPath() && path.GetVariantSelection().second.empty()) {
    ...
    for (auto const &childrenField :
         {SdfChildrenKeys->PrimChildren, SdfChildrenKeys->VariantSetChildren}) {
                            ...
                            const FileFormatArguments &args) const
{
  ...
  if (Sdf_IsPackageOrPackagedLayer(fileFormat, newFileName)) {
    TF_CODING_ERROR(
      "Cannot save layer @%s@: writing %s %s layer "
...
bool SdfLayer::Export(const string &newFileName,
                      const string &comment,
                      const FileFormatArguments &args) const
{
  return _WriteToFile(newFileName,
                      comment,
                      ...
                      GetFileFormat()->IsSupportedExtension(newFileName) ? GetFileFormat() :
                                                                           TfNullPtr,
                      args);
}
...
bool SdfLayer::_Save(bool force) const
{
  ...
  if (!_WriteToFile(path, std::string(), GetFileFormat(), GetFileFormatArguments()))
    return false;
  ...
  VtValue timestamp(ArGetResolver().GetModificationTimestamp(GetIdentifier(), path));
#if AR_VERSION == 1
  if (timestamp.IsEmpty()) {
    TF_CODING_ERROR("Unable to get modification timestamp for '%s (%s)'",
                    GetIdentifier().c_str(),
