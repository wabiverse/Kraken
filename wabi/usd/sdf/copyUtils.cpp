...
static void _GetFieldNames(const SdfLayerHandle &layer,
                           const SdfPath &path,
                               ...
                               _CopyStack *copyStack)
{
  ...
  if (!shouldCopyChildren(childField,
                          srcLayer,
                          srcPath,
                          childrenInSrc,
                          dstLayer,
  ...
  if (childField == SdfChildrenKeys->ConnectionChildren) {
    _ProcessChildren<Sdf_AttributeConnectionChildPolicy>(childField,
                                                         srcChildren,
                                                         dstChildren,
                                                         srcLayer,
                                                         srcPath,
  ...
  if (childField == SdfChildrenKeys->MapperChildren) {
    _ProcessChildren<Sdf_MapperChildPolicy>(childField,
                                            srcChildren,
                                            dstChildren,
                                            srcLayer,
                                            srcPath,
  ...
  if (childField == SdfChildrenKeys->MapperArgChildren) {
    _ProcessChildren<Sdf_MapperArgChildPolicy>(childField,
                                               srcChildren,
                                               dstChildren,
                                               srcLayer,
                                               srcPath,
  ...
  if (childField == SdfChildrenKeys->ExpressionChildren) {
    _ProcessChildren<Sdf_ExpressionChildPolicy>(childField,
                                                srcChildren,
                                                dstChildren,
                                                srcLayer,
                                                srcPath,
  ...
  if (childField == SdfChildrenKeys->RelationshipTargetChildren) {
    _ProcessChildren<Sdf_RelationshipTargetChildPolicy>(childField,
                                                        srcChildren,
                                                        dstChildren,
                                                        srcLayer,
                                                        srcPath,
  ...
  if (childField == SdfChildrenKeys->VariantChildren) {
    _ProcessChildren<Sdf_VariantChildPolicy>(childField,
                                             srcChildren,
                                             dstChildren,
                                             srcLayer,
                                             srcPath,
  ...
  if (childField == SdfChildrenKeys->VariantSetChildren) {
    _ProcessChildren<Sdf_VariantSetChildPolicy>(childField,
                                                srcChildren,
                                                dstChildren,
                                                srcLayer,
                                                srcPath,
  ...
  if (childField == SdfChildrenKeys->PropertyChildren) {
    _ProcessChildren<Sdf_PropertyChildPolicy>(childField,
                                              srcChildren,
                                              dstChildren,
                                              srcLayer,
                                              srcPath,
  ...
  if (childField == SdfChildrenKeys->PrimChildren) {
    _ProcessChildren<Sdf_PrimChildPolicy>(childField,
                                          srcChildren,
                                          dstChildren,
                                          srcLayer,
                                          srcPath,
...
static void _DoAddNewPrimSpec(const SdfLayerHandle &destLayer, const _SpecDataEntry &specData)
{
  ...
  Sdf_ChildrenUtils<Sdf_PrimChildPolicy>::CreateSpec(destLayer,
                                                     specData.dstPath,
                                                     SdfSpecTypePrim,
                                                     /* inert = */ inert);
}
...
static void _DoAddNewPropertySpec(const SdfLayerHandle &destLayer, const _SpecDataEntry &specData)
{
  ...
  Sdf_ChildrenUtils<ChildPolicy>::CreateSpec(destLayer,
                                             specData.dstPath,
                                             specData.specType,
...
{
  Sdf_ChildrenUtils<ChildPolicy>::RemoveChild(dstLayer,
                 ...
                 const SdfShouldCopyChildrenFn &shouldCopyChildrenFn)
{
  ...
  while (!copyStack.empty()) {
    ...
    _GetFieldNames(dstLayer, toCopy.dstPath, &dstValueFields, &dstChildrenFields);

    std::vector<TfToken> srcValueFields;
    std::vector<TfToken> srcChildrenFields;
    _GetFieldNames(srcLayer, toCopy.srcPath, &srcValueFields, &srcChildrenFields);
    ...
    if (copyingPrimToVariant || copyingVariantToPrim) {
      ...
      copyEntry.dataToCopy.erase(std::remove_if(copyEntry.dataToCopy.begin(),
  ...
  if (fieldInSrc) {
    if (field == SdfFieldKeys->ConnectionPaths || field == SdfFieldKeys->TargetPaths ||
      ...
      if (srcLayer->HasField(srcPath, field, &refListOp)) {
        ...
        refListOp.ModifyOperations(std::bind(&_FixInternalSubrootPaths<SdfReference>,
                                             std::placeholders::_1,
                                             std::cref(srcPrefix),
                                             std::cref(dstPrefix)));
      ...
      if (srcLayer->HasField(srcPath, field, &payloadListOp)) {
        ...
        payloadListOp.ModifyOperations(std::bind(&_FixInternalSubrootPaths<SdfPayload>,
                                                 std::placeholders::_1,
                                                 std::cref(srcPrefix),
                                                 std::cref(dstPrefix)));
