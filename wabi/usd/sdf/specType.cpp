...
static inline size_t _GetBitmaskForSpecType(SdfSpecType specType)
{
...
struct Sdf_SpecTypeInfo
{
  ...
  SpecTypeToBitmask::iterator FindOrCreateSpecTypeEntry(const std::type_info &specCPPType)
  {
 ...
 private:
  ...
  Sdf_SpecTypeInfo() : specTypeToBitmask(0), registrationsCompleted(false)
  {
...
void SdfSpecTypeRegistration::_RegisterAbstractSpecType(const std::type_info &specCPPType,
                                                        const std::type_info &schemaType)
{
  Sdf_SpecTypeInfo &specTypeInfo = Sdf_SpecTypeInfo::GetInstance();
...
static bool _CanCast(SdfSpecType fromType, const TfType &toType)
{
  if (toType.IsUnknown()) {
    return TfType();
  }
  ...
  const size_t allowedBitmask = TfMapLookupByValue(specTypeInfo.specTypeToBitmask,
                                                   toType,
                                                   size_t(0));
...
bool Sdf_SpecType::CanCast(const SdfSpec &from, const std::type_info &to)
{
  ...
  const Sdf_SpecTypeInfo::SchemaTypes *toSchemaTypes = TfMapLookupPtr(
    specTypeInfo.specTypeToSchemaTypes,
    toType);
  if (!toSchemaTypes) {
    return false;
  }
