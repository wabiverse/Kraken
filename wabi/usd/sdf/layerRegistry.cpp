...
void Sdf_LayerRegistry::Erase(const SdfLayerHandle &layer)
{
...
#if AR_VERSION == 1
    const string layerPath = resolver.ComputeNormalizedPath(inputLayerPath);
...
SdfLayerHandle Sdf_LayerRegistry::FindByIdentifier(const string &layerPath) const
{
  TRACE_FUNCTION();
  ...
  TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::FindByIdentifier('%s') => %s\n",
                          layerPath.c_str(),
...
SdfLayerHandle Sdf_LayerRegistry::FindByRepositoryPath(const string &layerPath) const
{
  TRACE_FUNCTION();
  ...
  TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::FindByRepositoryPath('%s') => %s\n",
                          layerPath.c_str(),
                          foundLayer ? "Found" : "Not Found");
                                                 ...
                                                 const string &resolvedPath) const
{
  ...
  TF_DEBUG(SDF_LAYER).Msg("Sdf_LayerRegistry::FindByRealPath('%s') => %s\n",
                          searchPath.c_str(),
                          foundLayer ? "Found" : "Not Found");
