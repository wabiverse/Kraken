        ...
        with open(installedFiles[i], 'r') as installedFile,\
            ...
            if installedContent != generatedContent:
                diff = '\n'.join(unified_diff(installedContent.split('\n'),
                                              ...
                                              'Source ' + installedFile.name.replace("\\", "/"),
                                              'Generated ' + generatedFile.name.replace("\\", "/")))
                diffs[basename(installedFile.name.replace("\\", "/"))] = diff
...
def _runBisonAndFlexCommands(configuration):
    ...
    bisonFiles      = [join(srcDir.replace("\\", "/"), base.replace("\\", "/") + '.yy') for base in bases]
    flexFiles       = [join(srcDir.replace("\\", "/"), base.replace("\\", "/") + '.ll') for base in bases]
    bisonGenSources = [join(destDir.replace("\\", "/"), base.replace("\\", "/") + '.tab.cpp') for base in bases]
    bisonGenHeaders = [join(destDir.replace("\\", "/"), base.replace("\\", "/") + '.tab.hpp') for base in bases]
    flexGenSources  = [join(destDir.replace("\\", "/"), base.replace("\\", "/") + '.lex.cpp') for base in bases]
    ...
    bisonCommand = lambda index: ([bisonExecutable]
                                  + bisonFlags(base)
                                  + [bisonGenSources[index].replace("\\", "/")]
                                  + [bisonFiles[index].replace("\\", "/")])

    flexCommand  = lambda index: ([flexExecutable]
                                  + flexFlags(base)
                                  + [flexFiles[index].replace("\\", "/")])
    ...
    for index, base in enumerate(bases):
        ...
        print('%s' % (base + '.ll'))
        with open(flexGenSources[index].replace("\\", "/"), 'w') as outputFile:
            print(flexCommand(index))
...
def _canonicalizeFiles(sourceFiles, generatedFiles):
    ...
    for index, fileName in enumerate(list(renamed + sourceFiles)):
        ...
        if '/' in fileName:
            identifiers[index] = join(WABI_PREFIX_PATH, basename(fileName))
...
def _getConfiguration():
    ...
    arguments.srcDir = arguments.srcDir.replace("\\", "/")
    arguments.bison = arguments.bison.replace("\\", "/")
    arguments.flex = arguments.flex.replace("\\", "/")
    ...
    if not arguments.bases:
        allFiles = listdir(arguments.srcDir).replace("\\", "/")
        validExts = ['.yy', '.ll']
