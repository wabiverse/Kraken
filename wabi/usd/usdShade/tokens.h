...
struct UsdShadeTokensType {
  ...
  /// UsdShadeMaterialBindingAPI. Its value is empty and its purpose is to represent a general
  /// purpose material-binding that applies in the absence of a  specific-purpose binding.
  const TfToken allPurpose;
  /// \brief "bindMaterialAs"
  ///
  /// Token valued metadata key authored on a material  binding relationship to indicate the
  /// strength of the binding  relative to bindings authored on descendants.
  ...
  /// \brief "displacement"
  ///
  /// Describes the <i>displacement</i> output  terminal on a UsdShadeMaterial. It is used to
  /// define the  terminal UsdShadeShader describing the displacement of a  UsdShadeMaterial.
  const TfToken displacement;
  /// \brief "fallbackStrength"
  ///
  /// Sentinal value to be used for 'bindMaterialAs'  metadata's default value. Clients should pass
  /// this in for the  'bindingStrength' argument to UsdShadeMaterialBindingAPI::Bind(), if they
  /// want to author the default value (weakerThanDescendants) sparsely. The value
  /// "fallbackStrength" never gets authored  into scene description.
  ...
  /// \brief "id"
  ///
  /// Possible value for UsdShadeNodeDefAPI::GetImplementationSourceAttr(), Default value for
  /// UsdShadeNodeDefAPI::GetImplementationSourceAttr()
  const TfToken id;
  /// \brief "info:id"
  ///
  /// UsdShadeNodeDefAPI
  ...
  /// \brief "materialBind"
  ///
  /// The name of the GeomSubset family used to  identify face subsets defined for the purpose of
  /// binding  materials to facesets.
  ...
  ///
  ///  The relationship name on non-shading prims to denote a collection-based binding to a
  ///  UsdShadeMaterial.
  ...
  ///
  /// Possible value for the 'materialPurpose'  parameter in UsdShadeMaterialBindingAPI, to be used
  /// when the  render is in service of a goal other than a high fidelity "full" render (such as
  /// scene manipulation, modeling, or realtime  playback). Latency and speed are generally of
  /// greater concern  for preview renders, therefore preview materials are generally  designed to
  /// be "lighterweight" compared to full materials.
  const TfToken preview;
  /// \brief "sdrMetadata"
  ///
  /// Dictionary valued metadata key authored on Shader prims with implementationSource value of
  /// sourceAsset or  sourceCode to pass along metadata to the shader parser or  compiler. It is
  /// also used to author metadata on shader  properties in a UsdShade-based shader definition
  /// file.
  ...
  ///
  /// Possible value for 'bindMaterialAs' metadata on the  collection-based material binding
  /// relationship. Indicates  that the binding represented by the relationship is stronger than
  ...
  /// definition within the source asset, if the source asset specifies more than one shader node
  /// definition.
  const TfToken subIdentifier;
  /// \brief "surface"
  ///
  /// Describes the <i>surface</i> output  terminal on a UsdShadeMaterial. It is used to define the
  /// terminal UsdShadeShader describing the surface of a  UsdShadeMaterial.
  const TfToken surface;
  /// \brief ""
  ///
  /// Possible value for the "renderContext" parameter in \ref UsdShadeMaterial_Outputs API.
  /// Represents the universal renderContext. An output with a universal renderContext is
  /// applicable to all possible rendering contexts.
  const TfToken universalRenderContext;
  /// \brief ""
  ///
  /// Possible value for the "sourceType" parameter  in \ref
  /// UsdShadeNodeDefAPI_ImplementationSource API. Represents  the universal or fallback source
  /// type.
  const TfToken universalSourceType;
  /// \brief "volume"
  ///
  /// Describes the <i>volume</i> output  terminal on a UsdShadeMaterial. It is used to define the
  /// terminal UsdShadeShader describing the volume of a  UsdShadeMaterial.
  const TfToken volume;
  /// \brief "weakerThanDescendants"
  ///
  /// Possible value for 'bindMaterialAs' metadata on the  collection-based material binding
  /// relationship. Indicates  that the binding represented by the relationship is weaker than  any
  /// bindings authored on the descendants.
