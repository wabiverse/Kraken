                                            ...
                                            const TfToken &materialPurpose)
{
  ...
  return TfToken(
    SdfPath::JoinIdentifier(std::vector<TfToken>{UsdShadeTokens->materialBindingCollection,
                                                 materialPurpose,
                                                 bindingName}));
}
...
bool UsdShadeMaterialBindingAPI::UnbindDirectBinding(const TfToken &materialPurpose) const
{
  UsdRelationship bindingRel = GetPrim().CreateRelationship(
    _GetDirectBindingRelName(materialPurpose),
    /*custom*/ false);
    ...
    for (UsdPrim p = GetPrim(); !p.IsPseudoRoot(); p = p.GetParent()) {
      ...
      for (auto &collBinding : collBindings) {
        ...
        if (collIt == collectionQueryCache->end()) {
          ...
          // intel-threading-building-blocks/topic/671548
          collIt =
            collectionQueryCache->insert(std::make_pair(collectionPath, std::move(mQuery))).first;
        }
                                                                   ...
                                                                   const TfToken &elementType)
{
  ...
  UsdGeomSubset result = UsdGeomSubset::CreateGeomSubset(geom,
                                                         subsetName,
                                                         elementType,
                                                         indices,
...
{
  UsdGeomImageable geom(GetPrim());
  return UsdGeomSubset::GetGeomSubsets(geom,
                                       /* elementType */ TfToken(),
                                       UsdShadeTokens->materialBind);
}
