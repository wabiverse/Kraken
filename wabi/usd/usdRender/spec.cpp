...
//
#include "wabi/usd/usdRender/spec.h"
...
static void _ReadExtraSettings(UsdPrim const &prim,
                              ...
                              bool sparse)
{
  ...
  _Get(base.GetAspectRatioConformPolicyAttr(), &pd->aspectRatioConformPolicy, sparse);
  {
  ...
  _Get(base.GetDisableMotionBlurAttr(), &pd->disableMotionBlur, sparse);

  {
    // For backwards-compatibility:
    // instantaneousShutter disables motion blur
    bool instantaneousShutter = false;
    _Get(base.GetDisableMotionBlurAttr(), &instantaneousShutter, sparse);
                                   ...
                                   std::vector<std::string> const &extraNamespaces)
{
  ...
  for (SdfPath const &target : targets) {
    if (UsdRenderProduct product = UsdRenderProduct(stage->GetPrimAtPath(target))) {
      ...
      for (SdfPath const &renderVarPath : renderVarPaths) {
        ...
        if (!foundExisting) {
          ...
          if (prim.IsA<UsdRenderVar>()) {
            ...
            _ReadExtraSettings(prim, &rvd.extraSettings, extraNamespaces);
      ...
      // Store any other custom attributes in extraSettings.
      _ReadExtraSettings(product.GetPrim(), &pd.extraSettings, extraNamespaces);
