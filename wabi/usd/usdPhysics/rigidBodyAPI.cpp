...
#include "wabi/usd/usdPhysics/massProperties.h"
#include "wabi/usd/usdPhysics/massAPI.h"
#include "wabi/usd/usdPhysics/collisionAPI.h"
#include "wabi/usd/usdPhysics/materialAPI.h"
#include "wabi/usd/usdPhysics/metrics.h"
  ...
  if (accumulateMass || !rigidBodyMassInfo.hasInertia || !hasCoM) {
    ...
    std::vector<UsdShadeMaterial> physicsMaterials =
      UsdShadeMaterialBindingAPI::ComputeBoundMaterials(
        collisionPrims,
        _physicsPurposeTokens->materialPurposePhysics);
    ...
    for (size_t i = 0; i < collisionPrims.size(); i++) {
      ...
      _MassApiData _MassApiData = _GetCollisionShapeMassAPIData(collisionAPI,
                                                                rigidBodyMassInfo.density,
                                                                &shapeDensity,
                                                                physicsMaterials[i]);

      GfMatrix4f matrix;
      massProps.push_back(_ParseCollisionShapeForMass(collisionPrim,
                                                      _MassApiData,
                                                      shapeDensity,
                                                      &matrix,
                                                      &xfCache,
    ...
    if (!massProps.empty()) {
      ...
      const GfVec3f accInertia = UsdPhysicsMassProperties::GetMassSpaceInertia(
        accumulatedMassProps.GetInertiaTensor(),
        accPa);
      ...
      if (!rigidBodyMassInfo.hasInertia) {
        ...
        if (outMass > 0.0f) {
          ...
          TF_DIAGNOSTIC_WARNING(
            "The rigid body at %s has a possibly invalid inertia tensor of {1.0, 1.0, 1.0}%s. %s "
            "%s",
            usdPrim.GetPrimPath().GetString().c_str(),
