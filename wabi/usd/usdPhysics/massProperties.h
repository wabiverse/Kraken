...
//
#ifndef WABI_USD_USD_MASS_PROPERTIES_H
#define WABI_USD_USD_MASS_PROPERTIES_H
...
GfVec3f UsdPhysicsDiagonalize(const GfMatrix3f &m, GfQuatf &massFrame)
{
  ...
  for (uint32_t i = 0; i < MAX_ITERS; i++) {
    ...
    if (absw > 1000)
      ...
      r = UsdPhysicsIndexedRotation(a,
                                    sqrt((1 - h) / 2) * ((w >= 0.0f) ? 1.0f : -1.0f),
                                    sqrt((1 + h) / 2));
    }
