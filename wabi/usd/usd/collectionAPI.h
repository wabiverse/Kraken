 ...
 * and defined in @ref UsdTokens. So to set an attribute
 * to the value "rightHanded", use UsdTokens->rightHanded
 * as the value.
 */
...
class UsdCollectionAPI : public UsdAPISchemaBase
{
 public:
   ...
   * CollectionAPI. If so, it stores the instance name
   ...
   * to the token-valued, listOp metadata @em apiSchemas on the prim. For
   * example, if @p name is 'instance1', the token 'CollectionAPI:instance1'
   * is added to 'apiSchemas'.
   *
 ...
 public:
  ...
  /**
   * See GetExpansionRuleAttr(), and also @ref
 ...
 public:
  ...
  /**
   * See GetIncludeRootAttr(), and also @ref
   * Usd_Create_Or_Get_Property for when to use Get vs Create.
   * If specified, author @p defaultValue as the attribute's
   * default, sparsely (when it makes sense to do so) if @p
   * writeSparsely is @c true, the default for @p writeSparsely
 ...
 public:
  ...
  /**
   * Returns the canonical path to the collection named,
   * @p name on the given prim, @p prim.
   *
  ...
  /**
   * Returns all the usd objects that satisfy the predicate, @p pred
   * in the collection represented by the UsdCollectionMembershipQuery
   * object, @p query.
   *
   ...
   * @name Collection Authoring API
   *
   * Convenience API for adding or removing prims
   * and properties to (or from) a collection... */
  ...
  /**
   * Resets the collection by clearing both the
   * includes and excludes targets of the collection
   * in the current UsdEditTarget.
   *
   * @note This does not modify the "includeRoot" attribute which is used
   * to include or exclude everything (i.e. the pseudoRoot) in the USD stage. */
  USD_API
  bool ResetCollection() const;

  /**
   * Blocks the targets of the includes and excludes
   * relationships of the collection, making the collection
 ...
 private:
  ...
  /**
   * Helper method for computing the UsdCollectionMembershipQuery
   ...
   *
   * If @p foundCircularDependency is not nullptr, it is set to true if
   * a circular dependency is detected amongst the included collections.
