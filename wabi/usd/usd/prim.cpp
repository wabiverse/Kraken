...
bool UsdPrim::ApplyAPI(const TfType &schemaType, const TfToken &instanceName) const
{
  if (!UsdSchemaRegistry::GetInstance().IsAppliedAPISchema(schemaType)) {
    TF_CODING_ERROR(
        "ApplyAPI: provided schema type ( %s ) is not an "
        "applied API schema type.",
        schemaType.GetTypeName().c_str());
    return false;
  }
  ...
  if (UsdSchemaRegistry::GetInstance().IsMultipleApplyAPISchema(schemaType)) {
    if (instanceName.IsEmpty()) {
      TF_CODING_ERROR(
          "ApplyAPI: Multiple application API schemas like "
    ...
    if (!instanceName.IsEmpty()) {
      TF_CODING_ERROR(
          "ApplyAPI: Single application API schemas like "
...
bool UsdPrim::RemoveAPI(const TfType &schemaType, const TfToken &instanceName) const
{
  if (!UsdSchemaRegistry::GetInstance().IsAppliedAPISchema(schemaType)) {
    TF_CODING_ERROR(
        "RemoveAPI: provided schema type ( %s ) is not an "
        "applied API schema type.",
        schemaType.GetTypeName().c_str());
    return false;
  }
  ...
  if (UsdSchemaRegistry::GetInstance().IsMultipleApplyAPISchema(schemaType)) {
    if (instanceName.IsEmpty()) {
      TF_CODING_ERROR(
          "RemoveAPI: Multiple application API schemas like "
          "%s must have an application instanceName.",
          schemaType.GetTypeName().c_str());
      return false;
    }
    ...
    if (!instanceName.IsEmpty()) {
      TF_CODING_ERROR(
          "RemoveAPI: Single application API schemas like "
          "%s cannot have an application instanceName.",
          schemaType.GetTypeName().c_str());
      return false;
    }
...
template<class PropertyType, class Derived> struct UsdPrim_TargetFinder {
 ...
 private:
  ...
  void _Find()
  {
    ...
    // deadlock issue).
    _dispatcher.Run(
        [this]() { tbb::parallel_sort(_result.begin(), _result.end(), SdfPath::FastLessThan()); });
    _dispatcher.Wait();
...
bool UsdPrim::IsMasterPath(const SdfPath &path)
{
  return IsPrototypePath(path);
}

bool UsdPrim::IsPathInMaster(const SdfPath &path)
{
  return IsPathInPrototype(path);
}

UsdPrim UsdPrim::GetMaster() const
{
  return GetPrototype();
}
