...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    (appliedAPISchemas)(multipleApplyAPISchemas)(multipleApplyAPISchemaPrefixes)(autoApplyAPISchemas)
        ...
        (apiSchemaAutoApplyTo)(schemaKind)(nonAppliedAPI)(singleApplyAPI)(multipleApplyAPI)(concreteTyped)(abstractTyped)(abstractBase)
...
static TfTokenVector _GetApiSchemaAutoApplyToNamesFromMetadata(const JsObject &dict)
{
  const JsValue *autoApplyToValue = TfMapLookupPtr(dict, _tokens->apiSchemaAutoApplyTo);
  if (!autoApplyToValue) {
    ...
    std::map<TfToken, TfTokenVector> *autoApplyAPISchemas)
{
  ...
  for (const PlugPluginPtr &plug : plugins) {
    ...
    for (const auto &entry : autoApplyMetadata) {
      ...
      TfTokenVector apiSchemaAutoApplyToNames = _GetApiSchemaAutoApplyToNamesFromMetadata(
...
static std::map<TfToken, TfTokenVector> _GetAutoApplyAPISchemas()
{
  TRACE_FUNCTION();
  ...
  for (const auto &valuePair : typeCache.typeToName) {
    const TfType &type   = valuePair.first;
    PlugPluginPtr plugin = PlugRegistry::GetInstance().GetPluginForType(type);
    if (!plugin) {
      TF_CODING_ERROR("Failed to find plugin for schema type '%s'", type.GetTypeName().c_str());
      continue;
    }
    ...
    TfTokenVector apiSchemaAutoApplyToNames = _GetApiSchemaAutoApplyToNamesFromMetadata(dict);
    ...
    if (!apiSchemaAutoApplyToNames.empty()) {
      ...
      result.emplace(valuePair.second.name, std::move(apiSchemaAutoApplyToNames));
    }
  }
...
std::pair<TfToken, TfToken> UsdSchemaRegistry::GetTypeAndInstance(const TfToken &apiSchemaName)
{
...
{
  static const std::map<TfToken, TfTokenVector> result = _GetAutoApplyAPISchemas();
  return result;
}
    ...
    const TfTokenVector &appliedAPISchemas) const
{
  ...
  for (auto it = appliedAPISchemas.crbegin(); it != appliedAPISchemas.crend(); ++it) {
    ...
    if (typeAndInstance.second.IsEmpty()) {
      ...
      auto it = _multipleApplyAPISchemaNamespaces.find(typeAndInstance.first);
      ...
      if (TF_VERIFY(!prefix.IsEmpty())) {
        ...
        const std::string propPrefix = SdfPath::JoinIdentifier(prefix, typeAndInstance.second);
