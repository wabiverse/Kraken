...
#undef LOGIC_OP
  ...
  struct _ListOpHeader
  {
    ...
    template<class T>
    explicit _ListOpHeader(SdfListOp<T> const &op)
      : bits(0)
    {
  ...
  class CrateFile::_BufferedOutput
  {
   ...
   private:
    ...
    struct _WriteOp
    {
      ...
      _WriteOp(_Buffer &&buf, int64_t pos)
        : buf(std::move(buf)),
          pos(pos)
      {}
      _Buffer buf;
  ...
  {
   public:
    _ReaderBase(CrateFile const *crate)
      : crate(crate)
    {}
  ...
  class CrateFile::_Reader : public _ReaderBase
  {
   ...
   public:
    ...
    // read bitwise.
    template<class T>
    typename std::enable_if<_IsBitwiseReadWrite<T>::value, T>::type Read(T *)
    {
  ...
  template<class T>
  struct CrateFile::_ArrayValueHandlerBase<T,
                                           typename std::enable_if<ValueTypeTraits<T>::supportsArray>::type>
  ...
  CrateFile::Packer::Packer(Packer &&other)
    : _crate(other._crate)
  {
  ...
  void CrateFile::_WriteFields(_Writer &w)
  {
    ...
    } else
    {
      ...
      uint64_t repsSize = TfFastCompression::CompressToBuffer(reinterpret_cast<char *>(reps.data()),
                                                              compBuffer2.get(),
                                                    ...
                                                    vector<int32_t> &jumps)
  {
    ...
    for (Iter next = cur; cur != end; cur = next)
    {
      ...
      {
        next =
          _BuildCompressedPathDataRecursive(curIndex, next, end, pathIndexes, elementTokenIndexes, jumps);
      }
...
#undef xx
  ...
  void CrateFile::_DoTypeRegistration()
  {
    ...
    _unpackValueFunctionsMmap[typeEnumIndex] = [this, valueHandler](ValueRep rep, VtValue *out) {
      valueHandler->UnpackVtValue(_MakeReader(_MakeMmapStream(_mmapSrc.get(), _debugPageMap.get())),
                                  rep,
                                  out);
    };
