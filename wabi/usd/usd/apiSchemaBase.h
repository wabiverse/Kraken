...
class UsdAPISchemaBase : public UsdSchemaBase {
 public:
  ...
  /**
   * Construct a UsdAPISchemaBase on UsdPrim @p prim . Equivalent to
 ...
 protected:
  ...
  /**
   * Returns the instance name of the API schema object
   * belonging to a multiple-apply API schema.
   *
 ...
 protected:
   ...
   *
   * This information is stored by adding @p apiSchemaName value to
   * the token-valued, listOp metadata @em apiSchemas on the prim.
   *
   ...
   * within an instance prototype OR</li> <li>@p apiSchemaName cannot be
   * added to the apiSchemas listOp metadata.</li></ul>
   *
 ...
 private:
  /**
   * The instance name associated with this schema object,
   * if it is a multiple-apply API schema. For example, in
   * the case of UsdCollectionAPI, this will hold the name
   * of the collection. */
