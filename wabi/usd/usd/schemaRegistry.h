...
class UsdSchemaRegistry : public TfWeakBase, boost::noncopyable
{
 public:
  ...
  USD_API
  static bool IsAllowedAPISchemaInstanceName(const TfToken &apiSchemaName,
  ...
  ///
  USD_API
  static TfToken MakeMultipleApplyNameInstance(const std::string &nameTemplate,
  ...
  const UsdPrimDefinition *FindAppliedAPIPrimDefinition(const TfToken &typeName) const
  {
    ...
    return multiIt != _multiApplyAPIPrimDefinitions.end() ? multiIt->second : nullptr;
  }
 ...
 private:
  ...
  const UsdPrimDefinition *_FindAPIPrimDefinitionByFullName(const TfToken &apiSchemaName,
  ...
  using _TypeNameToPrimDefinitionMap =
    std::unordered_map<TfToken, const std::unique_ptr<UsdPrimDefinition>, TfToken::HashFunctor>;
...
#endif  // WABI_USD_USD_SCHEMA_REGISTRY_H
