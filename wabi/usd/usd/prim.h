 ...
 */
#ifndef WABI_USD_USD_PRIM_H
#define WABI_USD_USD_PRIM_H
...
class UsdPrim : public UsdObject {
 public:
  ...
  /// For details of namespaced properties, see \ref Usd_Ordering
  USD_API
  std::vector<UsdProperty> GetAuthoredPropertiesInNamespace(
      const std::vector<std::string> &namespaces) const;
 ...
 private:
  ...
  USD_API
  bool _ApplyAPI(const TfType &schemaType, const TfToken &instanceName = TfToken()) const;

  // Private implementation for all RemoveAPI methods. This does no input
  // validation as the public methods are expected to have already done either
  // compile time or runtime validation already.
  USD_API
  bool _RemoveAPI(const TfType &schemaType, const TfToken &instanceName = TfToken()) const;
 ...
 public:
  ...
  template<typename T> bool HasAPI(const TfToken &instanceName = TfToken()) const
  {
    ...
    static_assert((T::schemaType == UsdSchemaKind::SingleApplyAPI ||
  ...
  USD_API
  bool ApplyAPI(const TfType &schemaType, const TfToken &instanceName = TfToken()) const;
  ...
  USD_API
  bool RemoveAPI(const TfType &schemaType, const TfToken &instanceName = TfToken()) const;
 ...
 public:
  ...
  USD_API
  SdfPathVector FindAllAttributeConnectionPaths(
  ...
  /// union.
  USD_API
  SdfPathVector FindAllRelationshipTargetPaths(
 ...
 private:
  ...
  // Prim constructor.
  UsdPrim(const Usd_PrimDataHandle &primData, const SdfPath &proxyPrimPath)
                                     ...
                                     UsdPrim>                       // reference type.
{
 ...
 private:
  ...
  {
    return base() == other.base() && _proxyPrimPath == other._proxyPrimPath &&
...
UsdPrim::SiblingRange UsdPrim::_MakeSiblingRange(const Usd_PrimFlagsPredicate &pred) const
{
  ...
  return SiblingRange(SiblingIterator(firstChild, firstChildPath, pred),
