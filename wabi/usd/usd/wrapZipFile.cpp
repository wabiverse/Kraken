...
static object _Open(const std::string &filePath)
{
...
static std::vector<std::string> _GetFileNames(const UsdZipFile &zipFile)
{
...
static UsdZipFileWriter *_CreateNew(const std::string &filePath)
{
  return new UsdZipFileWriter(UsdZipFileWriter::CreateNew(filePath));
}

static void _Enter(const UsdZipFileWriter &)
{
  // Nothing to do
}
