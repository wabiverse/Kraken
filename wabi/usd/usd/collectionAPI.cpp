...
TF_DEFINE_PRIVATE_TOKENS(_schemaTokens, (CollectionAPI)(collection));
...
UsdCollectionAPI UsdCollectionAPI::Apply(const UsdPrim &prim, const TfToken &name)
{
  ...
  if (tokens.empty())
  {
    TF_CODING_ERROR("Invalid CollectionAPI name '%s'.", name.GetText());
    return UsdCollectionAPI();
  }
...
{
  return GetPrim().GetRelationship(_GetNamespacedPropertyName(GetName(), UsdTokens->includes));
}

UsdRelationship UsdCollectionAPI::CreateIncludesRel() const
{
  return GetPrim().CreateRelationship(_GetNamespacedPropertyName(GetName(), UsdTokens->includes),
                                      /* custom = */ false);
}

UsdRelationship UsdCollectionAPI::GetExcludesRel() const
{
  return GetPrim().GetRelationship(_GetNamespacedPropertyName(GetName(), UsdTokens->excludes));
}

UsdRelationship UsdCollectionAPI::CreateExcludesRel() const
{
  return GetPrim().CreateRelationship(_GetNamespacedPropertyName(GetName(), UsdTokens->excludes),
                                      /* custom = */ false);
}
...
static std::vector<std::string> _GetCollectionAPIAliases(const TfType &collSchemaType)
{
  ...
  {
    collectionAPIAliases.reserve(collectionAPIAliases.size() + derivedTypes.size());
...
std::vector<UsdCollectionAPI> UsdCollectionAPI::GetAllCollections(const UsdPrim &prim)
{
  ...
  for (const auto &appliedSchema : appliedSchemas)
  {
    ...
    {
      const std::string collAPIPrefix = alias + UsdObject::GetNamespaceDelimiter();
