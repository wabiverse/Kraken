...
static inline bool _GetBracketingTimes(const vector<double> &times,
                                       const double time,
                                       double *tLower,
                                       double *tUpper)
{
...
class Usd_CrateDataImpl {
 ...
 public:
  ...
  {
    return _crateFile->GetAssetPath();
  }
  ...
  bool Save(string const &fileName)
  {
    ...
    tbb::parallel_sort(
        sortedPaths.begin(), sortedPaths.end(), [](SdfPath const &p1, SdfPath const &p2) {
  ...
  inline VtValue _GetTargetOrConnectionListOpValue(SdfPath const &path,
                                                   SdfSpecType *specTypeOut = nullptr) const
  {
    VtValue targetPaths;
  ...
  inline bool HasSpec(const SdfPath &path) const
  {
    ...
    return _hashData ? _hashData->find(path) != _hashData->end() :
  ...
  inline SdfSpecType GetSpecType(const SdfPath &path) const
  {
    ...
    return path.IsPrimPath() ? SdfSpecTypePrim : _flatTypes[i - _flatData.begin()].type;
  }
  ...
  inline void _VisitSpecs(SdfAbstractData const &data, SdfAbstractDataSpecVisitor *visitor) const
  {
    ...
    // them.
    auto doTargetAndConnectionSpecs = [this, &data, visitor](SdfPath const &path,
      ...
      if (specType == SdfSpecTypeAttribute || specType == SdfSpecTypeRelationship) {
        ...
        if (!listOpVal.IsEmpty()) {
          ...
          if (listOp.IsExplicit()) {
            ...
            copy(appended.begin(),
                 appended.end(),
                 copy(prepended.begin(),
                      prepended.end(),
                      copy(added.begin(), added.end(), specs.begin())));
  ...
  inline bool Has(const SdfPath &path, const TfToken &field, VtValue *value) const
  {
  ...
  inline vector<TfToken> List(const SdfPath &path) const
  {
    ...
    _hashData ? _ListHelper(*_hashData, path, names) : _ListHelper(_flatData, path, names);
    return names;
  }

  template<class Data>
  inline void _SetHelper(Data &d,
                         SdfPath const &path,
  ...
  inline void Set(const SdfPath &path, const TfToken &fieldName, const VtValue &value)
  {
    ...
    _hashData ? _SetHelper(*_hashData, path, _hashLastSet, fieldName, value) :
  ...
  {
    _hashData ? _EraseHelper(*_hashData, path, field) : _EraseHelper(_flatData, path, field);
  }
  ...
  inline size_t GetNumTimeSamplesForPath(const SdfPath &path) const
  {
 ...
 private:
  ...
  {
    return _hashData ? _ListAllTimeSamplesHelper(*_hashData) :
  ...
  template<class Data>
  inline VtValue const *_GetFieldValueHelper(Data const &d,
                                             SdfPath const &path,
  ...
  inline VtValue const *_GetFieldValue(SdfPath const &path, TfToken const &field) const
  {
    return _hashData ? _GetFieldValueHelper(*_hashData, path, field) :
  ...
  {
    return _hashData ? _GetMutableFieldValueHelper(*_hashData, path, field) :
  ...
  typedef boost::container::flat_map<SdfPath, _FlatSpecData, SdfPath::FastLessThan> _FlatMap;

  typedef std::unordered_map<SdfPath, _MapSpecData, SdfPath::Hash> _HashMap;
...
bool Usd_CrateData::Open(const std::string &assetPath)
{
...
bool Usd_CrateData::Has(const SdfPath &path,
                        const TfToken &field,
                        SdfAbstractDataValue *value) const
{
...
void Usd_CrateData::Set(const SdfPath &path,
                        const TfToken &field,
                        const SdfAbstractDataConstValue &value)
{
...
bool Usd_CrateData::QueryTimeSample(const SdfPath &path,
                                    double time,
                                    SdfAbstractDataValue *value) const
{
  return _impl->QueryTimeSample(path, time, value);
