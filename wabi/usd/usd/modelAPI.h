...
class UsdModelAPI : public UsdAPISchemaBase
{
 ...
 public:
   ...
   *
   * To test whether the returned @p kind matches
   * a particular known "clientKind":
   *
   * @code
   * TfToken kind;
   *
  ...
  /**
   * Author a @p kind for this prim, at the current
   * UsdEditTarget. @return true if @p kind was
   * successully authored, otherwise false. */
  USD_API
  bool SetKind(const TfToken &kind) const;

  /**
   * Return true if the prim's kind metadata is or inherits
   * from @p baseKind as defined by the Kind Registry.
   *
   * If @p validation is KindValidationModelHierarchy (the default),
   * then this also ensures that if baseKind is a model, the prim
   * conforms to the rules of model hierarchy, as defined by IsModel.
   * If set to KindValidationNone, no additional validation is done.
   *
   * IsModel and IsGroup are preferrable to IsKind("model") as they
   * are optimized for fast traversal.
   *
   * @note If a prim's model hierarchy is not valid, it is possible that
   * prim.IsModel() and prim.IsKind("model", Usd.ModelAPI.KindValidationNone)
   ...
   * @name Model Asset Info API
   * @{ */

  /**
   * Returns the model's asset identifier as
   * authored in the composed assetInfo dictionary.
   *
   ...
   * record the resolved version <em>at the time at which a reference to
   * the asset was added to an aggregate</em>, at the referencing site. In
  ...
  /**
   * Sets the list of external asset dependencies
   * referenced inside the payload of a model.
   *
