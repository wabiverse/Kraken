...
namespace
{
  ...
  {
    const TfToken &sdrUsdDefinitionType = TfToken(
      StringVal(SdrPropertyMetadata->SdrUsdDefinitionType, metadata));
    ...
    if (aliasesTokensToSdfValueTypeNames.find(sdrUsdDefinitionType) ==
      ...
      TF_WARN(
        "Invalid SdfValueTypeName or alias provided for "
  ...
  // a hint.
  const NdrSdfTypeIndicator _GetTypeIndicatorFromDefaultMapping(const TfToken &type, bool isArray)
  {
    const TokenToSdfTypeMap &tokenTypeToSdfType = isArray ? _GetTokenTypeToSdfArrayType() :
                                                            _GetTokenTypeToSdfType();
  ...
  {
    const NdrSdfTypeIndicator GetTypeAsSdfType(const TfToken &type,
                                               size_t arraySize,
                                               const NdrTokenMap &metadata)
    {
      ...
      if (_IsAssetIdentifier(metadata)) {
        return std::make_pair(isArray ? SdfValueTypeNames->StringArray : SdfValueTypeNames->String,
                              TfToken());
      }
  ...
  namespace _Encoding_1
  {
                                               ...
                                               const NdrTokenMap &metadata)
    {
      ...
      // mapping because it is determined dynamically
      if (_IsAssetIdentifier(metadata)) {
        return std::make_pair(isArray ? SdfValueTypeNames->AssetArray : SdfValueTypeNames->Asset,
                              TfToken());
      }
  ...
  const NdrSdfTypeIndicator _GetTypeAsSdfType(const TfToken &type,
                                              size_t arraySize,
                                              const NdrTokenMap &metadata,
                                      ...
                                      int usdEncodingVersion)
  {
    ...
    const NdrSdfTypeIndicator sdfTypeIndicator = _GetTypeAsSdfType(sdrType,
                                                                   arraySize,
                                                                   metadata,
                                                                   usdEncodingVersion);
...
bool SdrShaderProperty::CanConnectTo(const NdrProperty &other) const
{
  ...
  const NdrSdfTypeIndicator &sdfInputTypeInd = _GetTypeAsSdfType(inputType,
                                                                 inputArraySize,
                                                                 inputMetadata,
                                                                 _usdEncodingVersion);
  const NdrSdfTypeIndicator &sdfOutputTypeInd = _GetTypeAsSdfType(outputType,
                                                                  outputArraySize,
                                                                  outputMetadata,
                                                                  _usdEncodingVersion);
...
const NdrSdfTypeIndicator SdrShaderProperty::GetTypeAsSdfType() const
{
