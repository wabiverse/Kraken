...
bool ArDefaultResolver::IsRepositoryPath(const std::string &path)
{
  return false;
}
...
std::string ArDefaultResolver::GetExtension(const std::string &path)
{
  return TfGetExtension(path);
}

std::string ArDefaultResolver::ComputeNormalizedPath(const std::string &path)
{
  return TfNormPath(path);
}

std::string ArDefaultResolver::ComputeRepositoryPath(const std::string &path)
{
  return std::string();
}

static std::string _Resolve(const std::string &anchorPath, const std::string &path)
{
...
std::string ArDefaultResolver::_ResolveNoCache(const std::string &path)
{
  if (path.empty()) {
    return path;
  }
  ...
  if (IsRelativePath(path)) {
    ...
    if (IsSearchPath(path)) {
      const ArDefaultResolverContext *contexts[2] = {_GetCurrentContext(), &_fallbackContext};
...
std::string ArDefaultResolver::Resolve(const std::string &path)
{
...
std::string ArDefaultResolver::ComputeLocalPath(const std::string &path)
{
  return path.empty() ? path : TfAbsPath(path);
}

void ArDefaultResolver::UpdateAssetInfo(const std::string &identifier,
...
std::shared_ptr<ArAsset> ArDefaultResolver::OpenAsset(const std::string &resolvedPath)
{
...
bool ArDefaultResolver::CanWriteLayerToPath(const std::string &path, std::string *whyNot)
{
  return true;
}
...
ArResolverContext ArDefaultResolver::CreateDefaultContext()
{
  return _defaultContext;
}

ArResolverContext ArDefaultResolver::CreateDefaultContextForAsset(const std::string &filePath)
{
  if (filePath.empty()) {
...
void ArDefaultResolver::RefreshContext(const ArResolverContext &context)
{}

ArResolverContext ArDefaultResolver::GetCurrentContext()
{
  const ArDefaultResolverContext *ctx = _GetCurrentContext();
  return ctx ? ArResolverContext(*ctx) : ArResolverContext();
}

void ArDefaultResolver::BeginCacheScope(VtValue *cacheScopeData)
{
  _threadCache.BeginCacheScope(cacheScopeData);
}

void ArDefaultResolver::EndCacheScope(VtValue *cacheScopeData)
{
...
void ArDefaultResolver::BindContext(const ArResolverContext &context, VtValue *bindingData)
{
...
void ArDefaultResolver::UnbindContext(const ArResolverContext &context, VtValue *bindingData)
{
  _ContextStack &contextStack = _threadContextStack.local();
...
const ArDefaultResolverContext *ArDefaultResolver::_GetCurrentContext()
{
  _ContextStack &contextStack = _threadContextStack.local();
