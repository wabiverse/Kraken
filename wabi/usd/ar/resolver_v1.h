...
#ifndef WABI_USD_AR_RESOLVER_V1_H
#  define WABI_USD_AR_RESOLVER_V1_H

/// \file ar/resolver_v1.h
...
class ArResolver {
 public:
  ...
  AR_API
  virtual void BindContext(const ArResolverContext &context, VtValue *bindingData) = 0;
  ...
  /// \see ArResolverContextBinder
  AR_API
  virtual void UnbindContext(const ArResolverContext &context, VtValue *bindingData) = 0;
  ...
  /// create one that may be used later.
  AR_API
  virtual ArResolverContext CreateDefaultContextForAsset(const std::string &filePath) = 0;
  ...
  AR_API
  virtual std::string ResolveWithAssetInfo(const std::string &path, ArAssetInfo *assetInfo) = 0;
  ...
  AR_API
  virtual std::shared_ptr<ArAsset> OpenAsset(const std::string &resolvedPath) = 0;
  ...
  AR_API
  virtual bool CanWriteLayerToPath(const std::string &path, std::string *whyNot) = 0;
