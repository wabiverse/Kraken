...
TF_DEFINE_ENV_SETTING(
    WABI_AR_DISABLE_PLUGIN_URI_RESOLVERS, false,
  ...
  class _DispatchingResolver final : public ArResolver
  {
   ...
   private:
    ...
    void _InitializeURIResolvers(const std::vector<_ResolverInfo> &availableResolvers)
    {
      ...
      for (const _ResolverInfo &resolverInfo : availableResolvers) {
        ...
        for (std::string uriScheme : resolverInfo.uriSchemes) {
          ...
          if (const _ResolverSharedPtr *existingResolver = TfMapLookupPtr(uriResolvers,
                                                                          uriScheme)) {
            TF_WARN(
                                         ...
                                         const _ResolverInfo **info = nullptr) const
    {
      ...
      const _ResolverSharedPtr *uriResolver = TfMapLookupPtr(_uriResolvers,
