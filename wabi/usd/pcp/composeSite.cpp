      ...
      curListOp.ApplyOperations(
        ...
        [&layer, layerOffset, &infoMap](SdfListOpType opType,
          ...
          const std::string assetPath = authoredAssetPath.empty() ?
                                          authoredAssetPath :
                                          SdfComputeAssetPathRelativeToLayer(layer,
                                                                             authoredAssetPath);
...
void PcpComposeSiteReferences(PcpLayerStackRefPtr const &layerStack,
                              ...
                              PcpSourceArcInfoVector *info)
{
  _PcpComposeSiteReferencesOrPayloads(SdfFieldKeys->References, layerStack, path, result, info);
}
...
void PcpComposeSitePayloads(PcpLayerStackRefPtr const &layerStack,
                            ...
                            PcpSourceArcInfoVector *info)
{
  _PcpComposeSiteReferencesOrPayloads(SdfFieldKeys->Payload, layerStack, path, result, info);
}
                              ...
                              const TfToken *orderField)
{
  TF_REVERSE_FOR_ALL(layer, layers)
  {
    ...
    if (namesVal.IsHolding<TfTokenVector>()) {
      const TfTokenVector &names = namesVal.UncheckedGet<TfTokenVector>();
