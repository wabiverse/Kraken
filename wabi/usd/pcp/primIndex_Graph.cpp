...
PcpPrimIndex_GraphRefPtr PcpPrimIndex_Graph::New(const PcpPrimIndex_GraphPtr &copy)
{
  TfAutoMallocTag2 tag("Pcp", "PcpPrimIndex_Graph");
...
bool PcpPrimIndex_Graph::IsUsd() const
{
  return _data->usd;
}
...
PcpNodeRef PcpPrimIndex_Graph::GetNodeUsingSite(const PcpLayerStackSite &site) const
{
  ...
  for (size_t i = 0, numNodes = _data->nodes.size(); i != numNodes; ++i) {
    ...
    if (!(node.smallInts.inert || node.smallInts.culled) && node.layerStack == site.layerStack &&
...
std::pair<size_t, size_t> PcpPrimIndex_Graph::_FindRootChildRange(const Predicate &pred) const
{
  ...
  for (size_t startIdx = rootNode.smallInts.firstChildIndex; startIdx != _Node::_invalidNodeIndex;
...
void PcpPrimIndex_Graph::Finalize()
{
  ...
  const bool nodeOrderMatchesStrengthOrder = _ComputeStrengthOrderIndexMapping(
    &nodeIndexToStrengthOrder);
...
void PcpPrimIndex_Graph::_ApplyNodeIndexMapping(const std::vector<size_t> &nodeIndexMap)
{
  ...
  TF_VERIFY(oldNodes.size() == oldSitePaths.size() && oldNodes.size() == oldHasSpecs.size());
  TF_VERIFY(nodeIndexMap.size() == oldNodes.size());

  const size_t numNodesToErase = std::count(nodeIndexMap.begin(),
                                            nodeIndexMap.end(),
...
bool PcpPrimIndex_Graph::IsFinalized() const
{
                                               ...
                                               PcpErrorBasePtr *error)
{
  ...
  if (arc.siblingNumAtOrigin >= 1 << _Node::_childrenSize) {
    if (error) {
      *error = PcpErrorCapacityExceeded::New(PcpErrorType_ArcCapacityExceeded);
    }
    return PcpNodeRef();
  }
...
PcpNodeRef PcpPrimIndex_Graph::InsertChildSubgraph(const PcpNodeRef &parent,
                                                   const PcpPrimIndex_GraphPtr &subgraph,
                                                   const PcpArc &arc,
                                                   PcpErrorBasePtr *error)
{
  TfAutoMallocTag2 tag("Pcp", "PcpPrimIndex_Graph");
  ...
  const size_t parentNodeIdx = parent._GetNodeIndex();
  const size_t childNodeIdx = _CreateNodesForSubgraph(*get_pointer(subgraph), arc);

  return _InsertChildInStrengthOrder(parentNodeIdx, childNodeIdx);
}

PcpNodeRef PcpPrimIndex_Graph::_InsertChildInStrengthOrder(size_t parentNodeIdx,
                                                           size_t childNodeIdx)
{
...
size_t PcpPrimIndex_Graph::_CreateNodesForSubgraph(const PcpPrimIndex_Graph &subgraph,
                                                   const PcpArc &arc)
{
  ...
  _data->finalized = false;
  _data->nodes.insert(_data->nodes.end(),
                      subgraph._data->nodes.begin(),
                      subgraph._data->nodes.end());
  _nodeSitePaths.insert(_nodeSitePaths.end(),
                        subgraph._nodeSitePaths.begin(),
                        subgraph._nodeSitePaths.end());
  _nodeHasSpecs.insert(_nodeHasSpecs.end(),
                       subgraph._nodeHasSpecs.begin(),
                       subgraph._nodeHasSpecs.end());
...
bool PcpPrimIndex_Graph::_ComputeStrengthOrderIndexMapping(
  std::vector<size_t> *nodeIndexToStrengthOrder) const
{
  ...
  return _ComputeStrengthOrderIndexMappingRecursively(rootNodeIdx,
                                                      &strengthIdx,
                                                      nodeIndexToStrengthOrder);
}
...
bool PcpPrimIndex_Graph::_ComputeEraseCulledNodeIndexMapping(
