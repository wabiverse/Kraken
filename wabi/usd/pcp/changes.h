...
//
#ifndef WABI_USD_PCP_CHANGES_H
#define WABI_USD_PCP_CHANGES_H
...
class PcpChanges {
 public:
  ...
  /// \c GetCacheChanges().
  PCP_API
  void DidChange(const TfSpan<const PcpCache *> &caches, const SdfLayerChangeListVec &changes);
 ...
 private:
  ...
  // needs to invalidate.
  void _DidChangeLayerStackRelocations(const TfSpan<const PcpCache *> &caches,
                                       const PcpLayerStackPtr &layerStack,
                                       std::string *debugSummary);
