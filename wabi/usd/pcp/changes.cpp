                                                        ...
                                                        std::string *debugSummary)
{
  ...
  {
    const bool isRelevantChange = depData.CanFieldChangeAffectFileFormatArguments(change.first,
                                                                                  change.second.first,
...
void PcpChanges::DidChange(const TfSpan<const PcpCache *> &caches, const SdfLayerChangeListVec &changes)
{
  // LayerStack changes
  ...
  const bool allCachesInUsdMode = std::all_of(caches.begin(), caches.end(), [](const PcpCache *cache) {
    return cache->IsUsd();
  });
      ...
      if (path == SdfPath::AbsoluteRootPath())
      {
        ...
        switch (Pcp_EntryRequiresLayerStackChange(entry))
        {
          ...
          case Pcp_ChangesLayerStackChangeNone:
            ...
            if (Pcp_EntryRequiresLayerStackOffsetsChange(layer, entry, &rootLayerStacksMayNeedTcpsRecompute))
            {
    ...
    for (const auto &path : pathsWithSignificantChanges)
    {
      ...
      for (auto cache : caches)
      {
        ...
        Pcp_DidChangeDependents(
          ...
          [this, &cache](const PcpDependency &dep) { DidChangeSignificantly(cache, dep.indexPath); },
          debugSummary);
      }
    }
