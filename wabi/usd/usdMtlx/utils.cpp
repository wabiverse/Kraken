...
_ComputeStdlibSearchPaths()
{
    ...
    // if moved/changed from the path initialized in WABI_MATERIALX_STDLIB_DIR.
    NdrStringVec stdlibSearchPaths =
        _GetSearchPathsFromEnvVar("WABI_MTLX_STDLIB_SEARCH_PATHS");

    // Add path to the MaterialX standard library discovered at build time.
#ifdef WABI_MATERIALX_STDLIB_DIR
    stdlibSearchPaths =
        _MergeSearchPaths(stdlibSearchPaths, { WABI_MATERIALX_STDLIB_DIR });
#endif
...
UsdMtlxCustomSearchPaths()
{
    ...
    static const auto materialxCustomSearchPaths =
        _GetSearchPathsFromEnvVar("WABI_MTLX_PLUGIN_SEARCH_PATHS");
