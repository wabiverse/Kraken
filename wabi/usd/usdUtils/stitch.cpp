...
namespace
{
                            ...
                            const UsdUtilsStitchValueFn &stitchFn)
  {
    ...
    if (stitchFn) {
      ...
      const Status status =
        stitchFn(field, srcPath, dstLayer, fieldInDst, srcLayer, fieldInSrc, &value);
    ...
    if (field == SdfFieldKeys->FramesPerSecond) {
      ...
      if (srcFPS != dstFPS) {
        TF_WARN("Mismatched framesPerSecond values in @%s@ and @%s@",
      ...
      if (srcTPS != dstTPS) {
        TF_WARN("Mismatched timeCodesPerSecond values in @%s@ and @%s@",
                srcLayer->GetIdentifier().c_str(),
                dstLayer->GetIdentifier().c_str());
      }
      return false;
      ...
      if (srcPrecision != dstPrecision) {
        TF_WARN("Mismatched framePrecision values in @%s@ and @%s@",
                srcLayer->GetIdentifier().c_str(),
                dstLayer->GetIdentifier().c_str());
      }
      return false;
    ...
    return _MergeValue<VtDictionary>(field,
                                     fallback,
                                     dstLayer,
                                     dstPath,
                                     srcLayer,
                                     srcPath,
                                     valueToCopy) ||
           _MergeValue<SdfVariantSelectionMap>(field,
                                               fallback,
                                               dstLayer,
                                               dstPath,
                                               srcLayer,
                                               srcPath,
                                               valueToCopy) ||
           _MergeValue<SdfIntListOp>(field,
                                     fallback,
                                     dstLayer,
                                     dstPath,
                                     srcLayer,
                                     srcPath,
                                     valueToCopy) ||
           _MergeValue<SdfUIntListOp>(field,
                                      fallback,
                                      dstLayer,
                                      dstPath,
                                      srcLayer,
                                      srcPath,
                                      valueToCopy) ||
           _MergeValue<SdfUInt64ListOp>(field,
                                        fallback,
                                        dstLayer,
                                        dstPath,
                                        srcLayer,
                                        srcPath,
                                        valueToCopy) ||
           _MergeValue<SdfTokenListOp>(field,
                                       fallback,
                                       dstLayer,
                                       dstPath,
                                       srcLayer,
                                       srcPath,
                                       valueToCopy) ||
           _MergeValue<SdfStringListOp>(field,
                                        fallback,
                                        dstLayer,
                                        dstPath,
                                        srcLayer,
                                        srcPath,
                                        valueToCopy) ||
           _MergeValue<SdfPathListOp>(field,
                                      fallback,
                                      dstLayer,
                                      dstPath,
                                      srcLayer,
                                      srcPath,
                                      valueToCopy) ||
           _MergeValue<SdfReferenceListOp>(field,
                                           fallback,
                                           dstLayer,
                                           dstPath,
                                           srcLayer,
                                           srcPath,
                                           valueToCopy) ||
           _MergeValue<SdfPayloadListOp>(field,
                                         fallback,
                                         dstLayer,
                                         dstPath,
                                         srcLayer,
                                         srcPath,
                                         valueToCopy) ||
           _MergeValue<SdfUnregisteredValueListOp>(field,
                                                   fallback,
                                                   dstLayer,
                                                   dstPath,
                                                   srcLayer,
                               ...
                               boost::optional<VtValue> *finalDstChildren)
  {
    ...
    TF_CODING_ERROR("Children field '%s' holding unexpected type '%s'",
                        ...
                        const UsdUtilsStitchValueFn &stitchValueFn)
{
  ...
  SdfCopySpec(weakObj->GetLayer(),
              weakObj->GetPath(),
              strongObj->GetLayer(),
              strongObj->GetPath(),
              /* shouldCopyValueFn = */
              std::bind(_MergeValueFn,
                        ph::_1,
                        ph::_2,
                        ph::_3,
                        ph::_4,
