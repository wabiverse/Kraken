...
#ifndef WABI_USD_IMAGING_USD_APOLLO_RENDER_PARAMS_H
#define WABI_USD_IMAGING_USD_APOLLO_RENDER_PARAMS_H
...
class UsdApolloRenderParams
{
 public:
  ...
  UsdTimeCode frame;
  float complexity;
  UsdApolloDrawMode drawMode;
  bool showGuides;
  bool showProxy;
  bool showRender;
  bool forceRefresh;
  bool flipFrontFacing;
  UsdApolloCullStyle cullStyle;
  bool enableIdRender;
  bool enableLighting;
  ...
  bool enableUsdDrawModes; /** Respect USD's model:drawMode attribute... */
  GfVec4f clearColor;
  TfToken colorCorrectionMode;
  int lut3dSizeOCIO;

  inline UsdApolloRenderParams();

  inline bool operator==(const UsdApolloRenderParams &other) const;
  inline bool operator!=(const UsdApolloRenderParams &other) const
  {
    return !(*this == other);
  }
...
UsdApolloRenderParams::UsdApolloRenderParams()
  : frame(UsdTimeCode::EarliestTime()),
    complexity(1.0),
    drawMode(UsdApolloDrawMode::DRAW_SHADED_SMOOTH),
    showGuides(false),
    showProxy(true),
    showRender(false),
    forceRefresh(false),
    flipFrontFacing(false),
    cullStyle(UsdApolloCullStyle::CULL_STYLE_NOTHING),
...
#endif /* WABI_USD_IMAGING_USD_APOLLO_RENDER_PARAMS_H */
