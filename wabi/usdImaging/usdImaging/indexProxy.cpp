...
UsdImagingDelegate::_HdPrimInfo *UsdImagingIndexProxy::_AddHdPrimInfo(
  ...
  UsdImagingPrimAdapterSharedPtr const &adapter)
{
  ...
  std::tie(it, inserted) = _delegate->_hdPrimInfoMap.insert(
    UsdImagingDelegate::_HdPrimInfoMap::value_type(cachePath, UsdImagingDelegate::_HdPrimInfo()));
...
void UsdImagingIndexProxy::_AddTask(SdfPath const &usdPath)
{
...
void UsdImagingIndexProxy::RemovePrimInfoDependency(SdfPath const &cachePath)
{
...
void UsdImagingIndexProxy::InsertRprim(TfToken const &primType,
                                       SdfPath const &cachePath,
                                       UsdPrim const &usdPrim,
                                       UsdImagingPrimAdapterSharedPtr adapter)
{
  UsdImagingDelegate::_HdPrimInfo *primInfo = _AddHdPrimInfo(cachePath, usdPrim, adapter);
...
void UsdImagingIndexProxy::InsertSprim(TfToken const &primType,
                                       SdfPath const &cachePath,
                                       UsdPrim const &usdPrim,
                                       UsdImagingPrimAdapterSharedPtr adapter)
{
...
void UsdImagingIndexProxy::InsertBprim(TfToken const &primType,
                                       SdfPath const &cachePath,
                                       UsdPrim const &usdPrim,
                                       UsdImagingPrimAdapterSharedPtr adapter)
{
...
void UsdImagingIndexProxy::InsertInstancer(SdfPath const &cachePath,
                                           UsdPrim const &usdPrim,
                                           UsdImagingPrimAdapterSharedPtr adapter)
{
  UsdImagingDelegate::_HdPrimInfo *primInfo = _AddHdPrimInfo(cachePath, usdPrim, adapter);
...
void UsdImagingIndexProxy::Repopulate(SdfPath const &usdPath)
{
  // Repopulation is deferred to enable batch processing in parallel.
  _usdPathsToRepopulate.push_back(usdPath);
}

void UsdImagingIndexProxy::Refresh(SdfPath const &cachePath)
{
  _AddTask(cachePath);
}
...
void UsdImagingIndexProxy::MarkRprimDirty(SdfPath const &cachePath, HdDirtyBits dirtyBits)
{
  UsdImagingDelegate::_HdPrimInfo *primInfo = _delegate->_GetHdPrimInfo(cachePath);
...
void UsdImagingIndexProxy::MarkSprimDirty(SdfPath const &cachePath, HdDirtyBits dirtyBits)
{
  UsdImagingDelegate::_HdPrimInfo *primInfo = _delegate->_GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo, "%s", cachePath.GetText())) {
    primInfo->dirtyBits |= dirtyBits;
...
void UsdImagingIndexProxy::MarkBprimDirty(SdfPath const &cachePath, HdDirtyBits dirtyBits)
{
  UsdImagingDelegate::_HdPrimInfo *primInfo = _delegate->_GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo, "%s", cachePath.GetText())) {
    primInfo->dirtyBits |= dirtyBits;
...
void UsdImagingIndexProxy::MarkInstancerDirty(SdfPath const &cachePath, HdDirtyBits dirtyBits)
{
  UsdImagingDelegate::_HdPrimInfo *primInfo = _delegate->_GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo, "%s", cachePath.GetText())) {
    primInfo->dirtyBits |= dirtyBits;
  ...
  UsdPrim const &materialPrim)
{
  ...
  return materialAdapter && materialAdapter->IsSupported(this) ? materialAdapter : nullptr;
}

bool UsdImagingIndexProxy::IsPopulated(SdfPath const &cachePath) const
{
  return _delegate->_hdPrimInfoMap.find(cachePath) != _delegate->_hdPrimInfoMap.end();
}

bool UsdImagingIndexProxy::IsRprimTypeSupported(TfToken const &typeId) const
{
  return _delegate->GetRenderIndex().IsRprimTypeSupported(typeId);
}

bool UsdImagingIndexProxy::IsSprimTypeSupported(TfToken const &typeId) const
{
  return _delegate->GetRenderIndex().IsSprimTypeSupported(typeId);
}

bool UsdImagingIndexProxy::IsBprimTypeSupported(TfToken const &typeId) const
{
  return _delegate->GetRenderIndex().IsBprimTypeSupported(typeId);
}

void UsdImagingIndexProxy::_ProcessRemovals()
{
  TRACE_FUNCTION();
...

  {
    ...
    TF_FOR_ALL (it, _sprimsToRemove) {
      ...
      index.RemoveSprim(primType, _delegate->ConvertCachePathToIndexPath(cachePath));
    }
...

  {
    ...
    TF_FOR_ALL (it, _bprimsToRemove) {
      ...
      index.RemoveBprim(primType, _delegate->ConvertCachePathToIndexPath(cachePath));
    }
