...
#include "wabi/usdImaging/usdImaging/dataSourcePrimvars.h"
...
bool UsdImagingDataSourcePrimvars::Has(const TfToken &name)
{
  TRACE_FUNCTION();
...
TfTokenVector UsdImagingDataSourcePrimvars::GetNames()
{
  TRACE_FUNCTION();
...
HdDataSourceBaseHandle UsdImagingDataSourcePrimvars::Get(const TfToken &name)
{
  TRACE_FUNCTION();
  ...
  if (nsIt != _namespacedPrimvars.end()) {
    return UsdImagingDataSourcePrimvar::New(
      ...
      HdPrimvarSchema::BuildInterpolationDataSource(nsIt->second.GetInterpolation()),
      HdPrimvarSchema::BuildRoleDataSource(nsIt->second.GetAttr().GetRoleName()));
  }
  ...
  if (cIt != _customPrimvars.end()) {
    return UsdImagingDataSourcePrimvar::New(
      ...
      _stageGlobals,
      cIt->second /* value */,
      UsdAttributeQuery() /* indices */,
      HdPrimvarSchema::BuildInterpolationDataSource(_GetCustomPrimvarInterpolation(cIt->second)),
      HdPrimvarSchema::BuildRoleDataSource(cIt->second.GetAttribute().GetRoleName()));
  }

  return nullptr;
}

TfToken UsdImagingDataSourcePrimvars::_GetCustomPrimvarInterpolation(
  const UsdAttributeQuery &attrQuery)
{
  ...
  if (attrQuery.GetAttribute().GetMetadata(UsdGeomTokens->interpolation, &interpolation)) {
...
bool UsdImagingDataSourcePrimvar::Has(const TfToken &name)
{
  const bool indexed = _indicesQuery.IsValid();

  if (indexed) {
    return name == HdPrimvarSchemaTokens->indexedPrimvarValue ||
           name == HdPrimvarSchemaTokens->indices ||
           name == HdPrimvarSchemaTokens->interpolation || name == HdPrimvarSchemaTokens->role;
  } else {
...
TfTokenVector UsdImagingDataSourcePrimvar::GetNames()
{
  const bool indexed = _indicesQuery.IsValid();
...
HdDataSourceBaseHandle UsdImagingDataSourcePrimvar::Get(const TfToken &name)
{
  TRACE_FUNCTION();
