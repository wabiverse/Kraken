    ...
    _primvars(nullptr)
{
  if (usdPrim.IsA<UsdGeomPointBased>()) {
    ...
    _AddCustomPrimvar(HdTokens->nonlinearSampleCount, UsdGeomTokens->motionNonlinearSampleCount);
    _AddCustomPrimvar(HdTokens->blurScale, UsdGeomTokens->motionBlurScale);
    _AddCustomPrimvar(HdTokens->normals, UsdGeomTokens->normals);
  }
}

void UsdImagingDataSourceGprim::_AddCustomPrimvar(const TfToken &primvarName,
                                                  const TfToken &attrName)
{
  _customPrimvarMapping.emplace_back(primvarName, attrName);
}

bool UsdImagingDataSourceGprim::Has(const TfToken &name)
{
...
TfTokenVector UsdImagingDataSourceGprim::GetNames()
{
...
HdDataSourceBaseHandle UsdImagingDataSourceGprim::Get(const TfToken &name)
{
  if (name == HdPrimvarsSchemaTokens->primvars) {
    auto primvars = UsdImagingDataSourcePrimvars::AtomicLoad(_primvars);
    if (!primvars) {
      primvars = UsdImagingDataSourcePrimvars::New(_GetSceneIndexPath(),
