...
//
#include "wabi/usdImaging/usdImaging/meshAdapter.h"
...
TfTokenVector UsdImagingMeshAdapter::GetImagingSubprims()
{
  return {TfToken()};
}

TfToken UsdImagingMeshAdapter::GetImagingSubprimType(TfToken const &subprim)
{
  if (subprim.IsEmpty()) {
...
HdContainerDataSourceHandle UsdImagingMeshAdapter::GetImagingSubprimData(
  TfToken const &subprim,
  UsdPrim const &prim,
  const UsdImagingDataSourceStageGlobals &stageGlobals)
{
  if (subprim.IsEmpty()) {
    return UsdImagingDataSourceMeshPrim::New(prim.GetPath(), prim, stageGlobals);
  }
  return nullptr;
}

bool UsdImagingMeshAdapter::IsSupported(UsdImagingIndexProxy const *index) const
{
  return index->IsRprimTypeSupported(HdPrimTypeTokens->mesh);
}

SdfPath UsdImagingMeshAdapter::Populate(UsdPrim const &prim,
...
void UsdImagingMeshAdapter::TrackVariability(
  UsdPrim const &prim,
...
bool UsdImagingMeshAdapter::_IsBuiltinPrimvar(TfToken const &primvarName) const
{
  return (primvarName == HdTokens->normals) ||
         UsdImagingGprimAdapter::_IsBuiltinPrimvar(primvarName);
}

void UsdImagingMeshAdapter::UpdateForTime(UsdPrim const &prim,
                                          SdfPath const &cachePath,
                                          UsdTimeCode time,
...
HdDirtyBits UsdImagingMeshAdapter::ProcessPropertyChange(UsdPrim const &prim,
                                                         SdfPath const &cachePath,
...
/*virtual*/
VtValue UsdImagingMeshAdapter::GetTopology(UsdPrim const &prim,
                                           SdfPath const &cachePath,
                                           UsdTimeCode time) const
{
  ...
  HdMeshTopology meshTopo(schemeToken,
...
PxOsdSubdivTags UsdImagingMeshAdapter::GetSubdivTags(UsdPrim const &prim,
                                                     SdfPath const &cachePath,
                                                     UsdTimeCode time) const
{
  HD_TRACE_FUNCTION();
  ...
  TfToken faceVaryingRule = _Get<TfToken>(prim,
  ...
  VtIntArray creaseIndices = _Get<VtIntArray>(prim, UsdGeomTokens->creaseIndices, time);
  tags.SetCreaseIndices(creaseIndices);

  VtIntArray creaseLengths = _Get<VtIntArray>(prim, UsdGeomTokens->creaseLengths, time);
  tags.SetCreaseLengths(creaseLengths);

  VtFloatArray creaseSharpnesses = _Get<VtFloatArray>(prim,
                                                      UsdGeomTokens->creaseSharpnesses,
                                                      time);
  tags.SetCreaseWeights(creaseSharpnesses);

  VtIntArray cornerIndices = _Get<VtIntArray>(prim, UsdGeomTokens->cornerIndices, time);
  tags.SetCornerIndices(cornerIndices);

  VtFloatArray cornerSharpnesses = _Get<VtFloatArray>(prim,
...
/*virtual*/
VtValue UsdImagingMeshAdapter::Get(UsdPrim const &prim,
                                   SdfPath const &cachePath,
                                   TfToken const &key,
                                   UsdTimeCode time,
