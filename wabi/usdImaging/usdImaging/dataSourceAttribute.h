...
//
#ifndef WABI_USD_IMAGING_USD_IMAGING_DATA_SOURCE_ATTRIBUTE_H
#define WABI_USD_IMAGING_USD_IMAGING_DATA_SOURCE_ATTRIBUTE_H
...
///
template<typename T> class UsdImagingDataSourceAttribute : public HdTypedSampledDataSource<T>
{
 public:
  ...
  {
    UsdTimeCode time = _stageGlobals.GetTime();
    if (!_usdAttrQuery.ValueMightBeTimeVarying() || !time.IsNumeric()) {
      return false;
    }
...
///
USDIMAGING_API
HdSampledDataSourceHandle UsdImagingDataSourceAttributeNew(
  const UsdAttribute &usdAttr,
  const UsdImagingDataSourceStageGlobals &stageGlobals,
  const SdfPath &sceneIndexPath = SdfPath::EmptyPath(),
  const HdDataSourceLocator &timeVaryingFlagLocator = HdDataSourceLocator::EmptyLocator());
    ...
    _stageGlobals(stageGlobals)
{
  if (!timeVaryingFlagLocator.IsEmpty()) {
    if (_usdAttrQuery.ValueMightBeTimeVarying()) {
      _stageGlobals.FlagAsTimeVarying(sceneIndexPath, timeVaryingFlagLocator);
    }
  }
}
...
#endif  // WABI_USD_IMAGING_USD_IMAGING_DATA_SOURCE_ATTRIBUTE_H
