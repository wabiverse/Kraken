...
namespace
{
  ...
  using _FactoryMap =
    std::unordered_map<SdfValueTypeName, _DataSourceFactory, SdfValueTypeNameHash>;

  template<typename T>
  HdSampledDataSourceHandle _FactoryImpl(const UsdAttributeQuery &usdAttrQuery,
                                         const UsdImagingDataSourceStageGlobals &stageGlobals,
                                         const SdfPath &sceneIndexPath,
                                         const HdDataSourceLocator &timeVaryingFlagLocator)
  {
    return UsdImagingDataSourceAttribute<T>::New(usdAttrQuery,
                                                 stageGlobals,
                                                 sceneIndexPath,
  ...
  const HdDataSourceLocator &timeVaryingFlagLocator)
{
  ...
  _FactoryMap::const_iterator i = _factoryMap.find(usdAttrQuery.GetAttribute().GetTypeName());
  if (i != _factoryMap.end()) {
    _DataSourceFactory factory = i->second;
    return factory(usdAttrQuery, stageGlobals, sceneIndexPath, timeVaryingFlagLocator);
  } else {
...
HdSampledDataSourceHandle UsdImagingDataSourceAttributeNew(
  ...
  const HdDataSourceLocator &timeVaryingFlagLocator)
{
  return UsdImagingDataSourceAttributeNew(UsdAttributeQuery(usdAttr),
                                          stageGlobals,
                                          sceneIndexPath,
                                          timeVaryingFlagLocator);
}
