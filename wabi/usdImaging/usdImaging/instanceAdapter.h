...
//
#ifndef WABI_USD_IMAGING_USD_IMAGING_INSTANCE_ADAPTER_H
#define WABI_USD_IMAGING_USD_IMAGING_INSTANCE_ADAPTER_H
...
class UsdImagingInstanceAdapter : public UsdImagingPrimAdapter {
 public:
  ...
  TfToken GetPurpose(UsdPrim const &usdPrim,
                     SdfPath const &cachePath,
  ...
  virtual GfMatrix4d GetRelativeInstancerTransform(SdfPath const &parentInstancerPath,
                                                   SdfPath const &instancerPath,
                                                   UsdTimeCode time) const override;
 ...
 private:
  ...
  bool _IsChildPrim(UsdPrim const &prim, SdfPath const &cachePath) const;
  ...
  SdfPath _InsertProtoPrim(UsdPrimRange::iterator *iter,
  ...
  void _ResyncPath(SdfPath const &cachePath, UsdImagingIndexProxy *index, bool reload);
  // Removes and optionally reloads all instancer data, both locally and
  // from the render index.
  void _ResyncInstancer(SdfPath const &instancerPath, UsdImagingIndexProxy *index, bool reload);
  ...
  bool _GetProtoPrimForChild(UsdPrim const &usdPrim,
                             SdfPath const &cachePath,
  ...
  // locked.
  typedef std::unordered_map<SdfPath, _InstancerData, SdfPath::Hash> _InstancerDataMap;
  ...
  typedef TfHashMap<SdfPath, SdfPath, SdfPath::Hash> _InstanceToInstancerMap;
...
#endif  // WABI_USD_IMAGING_USD_IMAGING_INSTANCE_ADAPTER_H
