...
template<typename Strategy, typename ImplData = bool> class UsdImaging_ResolvedAttributeCache {
 ...
 private:
  ...
  static unsigned _GetInitialCacheVersion()
  {
    return 1;
  }
  ...
  // new value is set.
  void _SetCacheEntryForPrim(const UsdPrim &prim, value_type const &value, _Entry *entry) const;
...
UsdImaging_ResolvedAttributeCache<Strategy, ImplData>::_GetValue(const UsdPrim &prim) const
{
...
struct UsdImaging_XfStrategy {
  ...
  static value_type Compute(UsdImaging_XformCache const *owner, UsdPrim const &prim, query_type const *query)
  {
    ...
    return !query->GetResetXformStack() ? (xform * (*owner->_GetValue(prim.GetParent()))) : xform;
  }
...
typedef UsdImaging_ResolvedAttributeCache<UsdImaging_PurposeStrategy> UsdImaging_PurposeCache;
...
typedef UsdImaging_ResolvedAttributeCache<UsdImaging_DrawModeStrategy> UsdImaging_DrawModeCache;
