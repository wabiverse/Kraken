...
//
#include "wabi/usdImaging/usdImaging/cameraAdapter.h"

#include "wabi/usdImaging/usdImaging/dataSourceCamera.h"
...
TfTokenVector UsdImagingCameraAdapter::GetImagingSubprims()
{
  return {TfToken()};
}

TfToken UsdImagingCameraAdapter::GetImagingSubprimType(TfToken const &subprim)
{
  if (subprim.IsEmpty()) {
...
HdContainerDataSourceHandle UsdImagingCameraAdapter::GetImagingSubprimData(
  TfToken const &subprim,
  UsdPrim const &prim,
  const UsdImagingDataSourceStageGlobals &stageGlobals)
{
  if (subprim.IsEmpty()) {
    return UsdImagingDataSourceCameraPrim::New(prim.GetPath(), prim, stageGlobals);
  }
  return nullptr;
}

bool UsdImagingCameraAdapter::IsSupported(UsdImagingIndexProxy const *index) const
{
  return index->IsSprimTypeSupported(HdPrimTypeTokens->camera);
}

SdfPath UsdImagingCameraAdapter::Populate(UsdPrim const &prim,
...
static std::vector<GfVec4d> _ToGfVec4dVector(const VtArray<GfVec4f> &vec)
{
...
VtValue UsdImagingCameraAdapter::Get(UsdPrim const &prim,
                                     SdfPath const &cachePath,
                                     TfToken const &key,
                                     UsdTimeCode time,
...
HdDirtyBits UsdImagingCameraAdapter::ProcessPropertyChange(UsdPrim const &prim,
                                                           SdfPath const &cachePath,
                                                           TfToken const &propertyName)
{
  ...
  if (propertyName == UsdGeomTokens->projection ||
      ...
      propertyName == UsdGeomTokens->focalLength || propertyName == UsdGeomTokens->clippingRange ||
      propertyName == UsdGeomTokens->fStop || propertyName == UsdGeomTokens->focusDistance ||
...
void UsdImagingCameraAdapter::MarkDirty(UsdPrim const &prim,
                                        SdfPath const &cachePath,
                                        HdDirtyBits dirty,
                                        UsdImagingIndexProxy *index)
{
...
void UsdImagingCameraAdapter::MarkTransformDirty(UsdPrim const &prim,
                                                 SdfPath const &cachePath,
                                                 UsdImagingIndexProxy *index)
{
...
void UsdImagingCameraAdapter::MarkWindowPolicyDirty(UsdPrim const &prim,
                                                    SdfPath const &cachePath,
                                                    UsdImagingIndexProxy *index)
{
...
void UsdImagingCameraAdapter::_RemovePrim(SdfPath const &cachePath, UsdImagingIndexProxy *index)
{
