...
SdfPath UsdImagingBasisCurvesAdapter::Populate(UsdPrim const &prim,
...
void UsdImagingBasisCurvesAdapter::TrackVariability(UsdPrim const &prim,
...
bool UsdImagingBasisCurvesAdapter::_IsBuiltinPrimvar(TfToken const &primvarName) const
{
  return (primvarName == HdTokens->normals || primvarName == HdTokens->widths) ||
         UsdImagingGprimAdapter::_IsBuiltinPrimvar(primvarName);
}

void UsdImagingBasisCurvesAdapter::UpdateForTime(UsdPrim const &prim,
                                                 SdfPath const &cachePath,
                                                 UsdTimeCode time,
...
HdDirtyBits UsdImagingBasisCurvesAdapter::ProcessPropertyChange(UsdPrim const &prim,
                                                                SdfPath const &cachePath,
                                                                TfToken const &propertyName)
{
  ...
  else if (propertyName == UsdGeomTokens->curveVertexCounts || propertyName == UsdGeomTokens->basis ||
           propertyName == UsdGeomTokens->type || propertyName == UsdGeomTokens->wrap)
  {
...
/*virtual*/
VtValue UsdImagingBasisCurvesAdapter::GetTopology(UsdPrim const &prim,
                                                  SdfPath const &cachePath,
                                                  UsdTimeCode time) const
{
  ...
  else
  {
    ...
    {
      TF_WARN("Unknown curve basis '%s', using '%s'", curveBasis.GetText(), topoCurveBasis.GetText());
    }
  }
  ...
  else
  {
    ...
    {
      TF_WARN("Unknown curve type '%s', using '%s'", curveType.GetText(), topoCurveType.GetText());
    }
  }
  ...
  else
  {
    ...
    {
      TF_WARN("Unknown curve wrap '%s', using '%s'", curveWrap.GetText(), topoCurveWrap.GetText());
    }
  }
...
/*virtual*/
VtValue UsdImagingBasisCurvesAdapter::Get(UsdPrim const &prim,
                                          SdfPath const &cachePath,
                                          TfToken const &key,
                                          UsdTimeCode time,
