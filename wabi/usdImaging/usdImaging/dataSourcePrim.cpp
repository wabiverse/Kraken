...
//
#include "wabi/usdImaging/usdImaging/dataSourcePrim.h"
#include "wabi/usdImaging/usdImaging/dataSourceAttribute.h"
...
bool UsdImagingDataSourceVisibility::Has(const TfToken &name)
{
...
TfTokenVector UsdImagingDataSourceVisibility::GetNames()
{
  return {
...
HdDataSourceBaseHandle UsdImagingDataSourceVisibility::Get(const TfToken &name)
{
...
bool UsdImagingDataSourcePurpose::Has(const TfToken &name)
{
...
TfTokenVector UsdImagingDataSourcePurpose::GetNames()
{
  return {
...
HdDataSourceBaseHandle UsdImagingDataSourcePurpose::Get(const TfToken &name)
{
  if (name == HdPurposeSchemaTokens->purpose) {
...
VtValue UsdImagingDataSourceExtentCoordinate::GetValue(HdSampledDataSource::Time shutterOffset)
{
  return VtValue(GetTypedValue(shutterOffset));
}

GfVec3d UsdImagingDataSourceExtentCoordinate::GetTypedValue(
  HdSampledDataSource::Time shutterOffset)
{
  ...
  if (_index >= raw.size()) {
    TF_WARN("<%s> Attribute does not have expected index entry %d", _attrPath.GetText(), _index);
    return GfVec3d(0);
  }
...
bool UsdImagingDataSourceExtentCoordinate::GetContributingSampleTimesForInterval(
  HdSampledDataSource::Time startTime,
  HdSampledDataSource::Time endTime,
  std::vector<HdSampledDataSource::Time> *outSampleTimes)
{
  return _extentAttr->GetContributingSampleTimesForInterval(startTime, endTime, outSampleTimes);
}
...
bool UsdImagingDataSourceExtent::Has(const TfToken &name)
{
  if (!_extentAttr) {
    return false;
  }

  return (name == HdExtentSchemaTokens->min || name == HdExtentSchemaTokens->max);
}

TfTokenVector UsdImagingDataSourceExtent::GetNames()
{
  if (!_extentAttr) {
    return {};
  }
...
HdDataSourceBaseHandle UsdImagingDataSourceExtent::Get(const TfToken &name)
{
  ...
  if (name == HdExtentSchemaTokens->min) {
    return UsdImagingDataSourceExtentCoordinate::New(_extentAttr, _attrPath, 0);
  } else if (name == HdExtentSchemaTokens->max) {
    return UsdImagingDataSourceExtentCoordinate::New(_extentAttr, _attrPath, 1);
  }
...
VtValue UsdImagingDataSourceXformResetXformStack::GetValue(HdSampledDataSource::Time shutterOffset)
{
  return VtValue(GetTypedValue(shutterOffset));
}

bool UsdImagingDataSourceXformResetXformStack::GetTypedValue(
  HdSampledDataSource::Time shutterOffset)
{
...
bool UsdImagingDataSourceXformMatrix::GetContributingSampleTimesForInterval(
  HdSampledDataSource::Time startTime,
  HdSampledDataSource::Time endTime,
  std::vector<HdSampledDataSource::Time> *outSampleTimes)
{
  ...
  // a single attribute.
  UsdTimeCode time = _stageGlobals.GetTime();
  if (!_xformQuery.TransformMightBeTimeVarying() || !time.IsNumeric()) {
    return false;
  }

  GfInterval interval(time.GetValue() + startTime, time.GetValue() + endTime);
...
bool UsdImagingDataSourceXform::Has(const TfToken &name)
{
...
TfTokenVector UsdImagingDataSourceXform::GetNames()
{
  return {
...
HdDataSourceBaseHandle UsdImagingDataSourceXform::Get(const TfToken &name)
{
  if (name == HdXformSchemaTokens->matrix) {
    return UsdImagingDataSourceXformMatrix::New(_xformQuery, _stageGlobals);
  } else if (name == HdXformSchemaTokens->resetXformStack) {
    return UsdImagingDataSourceXformResetXformStack::New(_xformQuery, _stageGlobals);
  }
...
bool UsdImagingDataSourcePrim::Has(const TfToken &name)
{
  if (!_sceneIndexPath.IsPrimPath()) {
    return false;
  }

  if (name == HdVisibilitySchemaTokens->visibility || name == HdPurposeSchemaTokens->purpose) {
...
TfTokenVector UsdImagingDataSourcePrim::GetNames()
{
  TfTokenVector vec;
...
HdDataSourceBaseHandle UsdImagingDataSourcePrim::Get(const TfToken &name)
{
  TRACE_FUNCTION();
  ...
  if (name == HdXformSchemaTokens->xform) {
    ...
    if (xformQuery.HasNonEmptyXformOpOrder()) {
      return UsdImagingDataSourceXform::New(xformQuery, _sceneIndexPath, _GetStageGlobals());
    } else {
      return nullptr;
    }
    ...
    if (visibilityQuery.HasAuthoredValue()) {
      return UsdImagingDataSourceVisibility::New(visibilityQuery,
                                                 _sceneIndexPath,
                                                 _GetStageGlobals());
    } else {
      return nullptr;
    ...
    if (purposeQuery.HasAuthoredValue()) {
      return UsdImagingDataSourcePurpose::New(purposeQuery, _GetStageGlobals());
    } else {
      return nullptr;
    }
  } else if (name == HdExtentSchemaTokens->extent) {
    ...
    if (extentQuery.HasAuthoredValue()) {
      return UsdImagingDataSourceExtent::New(extentQuery, _sceneIndexPath, _GetStageGlobals());
    } else {
      return nullptr;
    }
  }
