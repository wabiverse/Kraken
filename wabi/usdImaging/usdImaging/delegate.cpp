...
#include "wabi/usdImaging/usdImaging/coordSysAdapter.h"
...
TF_DEFINE_PRIVATE_TOKENS(_tokens, (lightFilterType));

// This environment variable matches a set of similar ones in
// primAdapter.cpp, controlling other attribute caches.
TF_DEFINE_ENV_SETTING(USDIMAGING_ENABLE_DRAWMODE_CACHE, 1, "Enable a cache for model:drawMode.");
...
UsdImagingDelegate::UsdImagingDelegate(HdRenderIndex *parentIndex, SdfPath const &delegateID)
  : HdSceneDelegate(parentIndex, delegateID),
    ...
    _xformCache(GetTime()),
    _materialBindingImplData(parentIndex->GetRenderDelegate()->GetMaterialBindingPurpose()),
    ...
    _hasDrawModeAdapter(
      UsdImagingAdapterRegistry::GetInstance().HasAdapter(UsdImagingAdapterKeyTokens->drawModeAdapterKey)),
    _sceneMaterialsEnabled(true),
    _sceneLightsEnabled(true),
    _appWindowPolicy(CameraUtilMatchVertically),
    _coordSysEnabled(parentIndex->IsSprimTypeSupported(HdPrimTypeTokens->coordSys)),
...
bool UsdImagingDelegate::_IsDrawModeApplied(UsdPrim const &prim)
{
...
TfToken UsdImagingDelegate::_GetModelDrawMode(UsdPrim const &prim)
{
  HD_TRACE_FUNCTION();
...
UsdImagingPrimAdapterSharedPtr const &UsdImagingDelegate::_AdapterLookup(UsdPrim const &prim,
...
UsdImagingPrimAdapterSharedPtr const &UsdImagingDelegate::_AdapterLookup(TfToken const &adapterKey)
{
  ...
  return _adapterMap.insert(std::make_pair(adapterKey, adapter)).first->second;
}

UsdImagingDelegate::_HdPrimInfo *UsdImagingDelegate::_GetHdPrimInfo(const SdfPath &cachePath)
{
...
Usd_PrimFlagsConjunction UsdImagingDelegate::_GetDisplayPredicate() const
{
  return _displayUnloadedPrimsWithBounds ? UsdPrimIsActive && UsdPrimIsDefined && !UsdPrimIsAbstract :
...
Usd_PrimFlagsConjunction UsdImagingDelegate::_GetDisplayPredicateForPrototypes() const
{
  return _displayUnloadedPrimsWithBounds ?
           ...
           UsdPrimIsActive && UsdPrimHasDefiningSpecifier && !UsdPrimIsAbstract && UsdPrimIsLoaded;
}
...
class UsdImagingDelegate::_Worker {
 ...
 public:
  _Worker(UsdImagingDelegate *delegate) : _delegate(delegate)
  {}
    ...
    for (size_t i = start; i < end; i++) {
      ...
      if (TF_VERIFY(primInfo, "%s\n", cachePath.GetText())) {
        ...
        if (TF_VERIFY(adapter, "%s\n", cachePath.GetText())) {
          adapter->TrackVariability(primInfo->usdPrim, cachePath, &primInfo->timeVaryingBits);
          if (primInfo->timeVaryingBits != HdChangeTracker::Clean) {
            adapter->MarkDirty(primInfo->usdPrim, cachePath, primInfo->timeVaryingBits, &indexProxy);
          }
        }
      }
    }
  ...
  void UpdateForTime(size_t start, size_t end)
  {
    for (size_t i = start; i < end; i++) {
      ...
      if (TF_VERIFY(primInfo, "%s\n", cachePath.GetText())) {
        UsdImagingPrimAdapterSharedPtr const &adapter = primInfo->adapter;
        if (TF_VERIFY(adapter, "%s\n", cachePath.GetText())) {
          adapter->UpdateForTime(primInfo->usdPrim, cachePath, time, primInfo->dirtyBits);
...
void UsdImagingDelegate::_AddTask(UsdImagingDelegate::_Worker *worker, SdfPath const &cachePath)
{
...
void UsdImagingDelegate::SyncAll(bool includeUnvarying)
{
  ...
  TF_FOR_ALL(it, _hdPrimInfoMap)
  {
    ...
    if (TF_VERIFY(adapter, "%s\n", cachePath.GetText())) {
      TF_DEBUG(USDIMAGING_UPDATES)
        .Msg("[Sync] PREP: <%s> dirtyFlags: 0x%x [%s]\n",
             cachePath.GetText(),
             primInfo.dirtyBits,
...
void UsdImagingDelegate::Sync(HdSyncRequestVector *request)
{
  UsdImagingDelegate::_Worker worker(this);
  ...
  for (SdfPath const &cachePath : _instancerPrimCachePaths) {
    ...
    if (TF_VERIFY(adapter, "%s\n", cachePath.GetText())) {
      TF_DEBUG(USDIMAGING_UPDATES)
        .Msg("[Sync] PREP Instancer: <%s> dirtyFlags: 0x%x [%s]\n",
             cachePath.GetText(),
             primInfo->dirtyBits,
             HdChangeTracker::StringifyDirtyBits(primInfo->dirtyBits).c_str());
      worker.AddTask(cachePath);
...
void UsdImagingDelegate::PostSyncCleanup()
{
  _primvarDescCache.GarbageCollect();
}

void UsdImagingDelegate::Populate(UsdPrim const &rootPrim)
{
  SdfPathVector empty;
...
bool UsdImagingDelegate::_CanPopulate(UsdPrim const &rootPrim) const
{
...
void UsdImagingDelegate::_SetStateForPopulation(UsdPrim const &rootPrim,
                                                SdfPathVector const &excludedPrimPaths,
                                                SdfPathVector const &invisedPrimPaths)
{
  if (_stage)
  ...
  _objectsChangedNoticeKey = TfNotice::Register(self, &This::_OnUsdObjectsChanged, _stage);
}
...
void UsdImagingDelegate::_Populate(UsdImagingIndexProxy *proxy)
{
  ...
  SdfPathVector const &usdPathsToRepopulate = proxy->_GetUsdPathsToRepopulate();
  ...
  for (SdfPath const &usdPath : usdPathsToRepopulate) {
    ...
    for (auto iter = range.begin(); iter != range.end(); ++iter) {
      ...
      if (UsdImagingPrimAdapterSharedPtr adapter = _AdapterLookup(*iter)) {
        ...
        // bindings.
        _PopulateMaterialBindingCache wu = {*iter, &_materialBindingCache};
  ...
  for (const UsdPrim &prim : primsToPopulateWithoutInstancing) {
    TF_DEBUG(USDIMAGING_CHANGES)
      .Msg("[Repopulate] Root path (no instancing): <%s>\n", prim.GetPath().GetText());

    const UsdPrimRange range(prim, UsdTraverseInstanceProxies(_GetDisplayPredicate()));
...
void UsdImagingDelegate::_ExecuteWorkForVariabilityUpdate(_Worker *worker)
{
  HD_TRACE_FUNCTION();

  TF_DEBUG(USDIMAGING_CHANGES).Msg("[Repopulate] %zu variability tasks in worker\n", worker->GetTaskCount());
...
void UsdImagingDelegate::_ExecuteWorkForTimeUpdate(_Worker *worker)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
void UsdImagingDelegate::SetTime(UsdTimeCode time)
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  if (_timeVaryingPrimCacheValid) {
    ...
    for (auto const &pair : _hdPrimInfoMap) {
      ...
      if (primInfo.timeVaryingBits != HdChangeTracker::Clean) {
        ...
        primInfo.adapter->MarkDirty(primInfo.usdPrim, cachePath, primInfo.timeVaryingBits, &indexProxy);
      }
    }
...
void UsdImagingDelegate::SetTimes(const std::vector<UsdImagingDelegate *> &delegates,
...
void UsdImagingDelegate::_GatherDependencies(SdfPath const &subtree, SdfPathVector *affectedCachePaths)
{
  HD_TRACE_FUNCTION();
  ...
  _DependencyMap::const_iterator start = _dependencyInfo.lower_bound(subtree);
  ...
  std::unique_copy(affectedPaths.begin(), affectedPaths.end(), std::back_inserter(*affectedCachePaths));
}

void UsdImagingDelegate::ApplyPendingUpdates()
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  if (!_usdPathsToResync.empty()) {
    ...
    std::unique_copy(_usdPathsToResync.begin(),
                     _usdPathsToResync.end(),
...
void UsdImagingDelegate::_OnUsdObjectsChanged(UsdNotice::ObjectsChanged const &notice,
                                              UsdStageWeakPtr const &sender)
{
  ...
  TF_DEBUG(USDIMAGING_CHANGES)
    .Msg(
      "[Objects Changed] Notice recieved "
  ...
  _usdPathsToResync.insert(_usdPathsToResync.end(), pathsToResync.begin(), pathsToResync.end());
                                        ...
                                        bool repopulateFromRoot)
{
  ...
  for (SdfPath currentPath = usdPath; !foundPruningPrim && currentPath != SdfPath::AbsoluteRootPath();
  ...
  if (affectedCachePaths.size() > 0) {
    ...
    if (repopulateFromRoot) {
      ...
      for (auto iter = range.begin(); iter != range.end(); ++iter) {
        ...
        auto const &depRange = _dependencyInfo.equal_range(iter->GetPath());
        ...
        if (adapter != nullptr) {
          TF_DEBUG(USDIMAGING_CHANGES).Msg("[Resync Prim]: Populating <%s>\n", iter->GetPath().GetText());
...
void UsdImagingDelegate::_RefreshUsdObject(SdfPath const &usdPath,
                                           ...
                                           UsdImagingIndexProxy *proxy)
{
  ...
  if (usdPath.IsAbsoluteRootOrPrimPath()) {
    ...
    if (attrName == UsdGeomTokens->modelDrawMode || attrName == UsdGeomTokens->modelApplyDrawMode ||
    ...
    if (attrName == UsdGeomTokens->visibility || attrName == UsdGeomTokens->purpose ||
  ...
  for (SdfPath const &affectedCachePath : affectedCachePaths) {
    ...
    if (primInfo != nullptr && primInfo->usdPrim.IsValid() &&
      ...
      if (dirtyBits == HdChangeTracker::Clean) {
        ...
        adapter->TrackVariability(primInfo->usdPrim, affectedCachePath, &primInfo->timeVaryingBits);

        // Propagate the dirty bits back out to the change tracker.
        HdDirtyBits combinedBits = dirtyBits | primInfo->timeVaryingBits;
...
VtValue UsdImagingDelegate::_GetUsdPrimAttribute(SdfPath const &cachePath, TfToken const &attrName)
{
  VtValue value;
...
void UsdImagingDelegate::_UpdateSingleValue(SdfPath const &cachePath, int requestBits)
{
  ...
  if (TF_VERIFY(primInfo, "%s\n", cachePath.GetText()) &&
    ...
    UsdImagingPrimAdapterSharedPtr &adapter = primInfo->adapter;
    adapter->UpdateForTime(primInfo->usdPrim, cachePath, _time, requestBits);
  }
}

void UsdImagingDelegate::ClearPickabilityMap()
{
  _pickablesMap.clear();
}

void UsdImagingDelegate::SetPickability(SdfPath const &path, bool pickable)
{
...
PickabilityMap UsdImagingDelegate::GetPickabilityMap() const
{
  return _pickablesMap;
}

void UsdImagingDelegate::_MarkRenderTagsDirty()
{
  UsdImagingIndexProxy indexProxy(this, nullptr);

  for (_HdPrimInfoMap::iterator it = _hdPrimInfoMap.begin(); it != _hdPrimInfoMap.end(); ++it) {
    const SdfPath &cachePath = it->first;
    _HdPrimInfo &primInfo = it->second;

    if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
      primInfo.adapter->MarkRenderTagDirty(primInfo.usdPrim, cachePath, &indexProxy);
    }
  }
}

void UsdImagingDelegate::SetDisplayRender(const bool displayRender)
{
...
void UsdImagingDelegate::SetDisplayProxy(const bool displayProxy)
{
...
void UsdImagingDelegate::SetDisplayGuides(const bool displayGuides)
{
...
void UsdImagingDelegate::SetUsdDrawModesEnabled(bool enableUsdDrawModes)
{
...
void UsdImagingDelegate::SetSceneMaterialsEnabled(bool enable)
{
  if (_sceneMaterialsEnabled != enable) {
    ...
    for (auto &pair : _hdPrimInfoMap) {
      ...
      if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
        primInfo.adapter->MarkMaterialDirty(primInfo.usdPrim, cachePath, &indexProxy);
      }
    }
  }
}

void UsdImagingDelegate::SetSceneLightsEnabled(bool enable)
{
  if (_sceneLightsEnabled != enable) {
    ...
    for (auto &pair : _hdPrimInfoMap) {
      ...
      _HdPrimInfo &primInfo = pair.second;
      if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
        primInfo.adapter->MarkLightParamsDirty(primInfo.usdPrim, cachePath, &indexProxy);
      }
    }
  }
}

void UsdImagingDelegate::SetWindowPolicy(CameraUtilConformWindowPolicy policy)
{
  if (_appWindowPolicy != policy) {
    ...
    for (auto &pair : _hdPrimInfoMap) {
      ...
      _HdPrimInfo &primInfo = pair.second;
      if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
        primInfo.adapter->MarkWindowPolicyDirty(primInfo.usdPrim, cachePath, &indexProxy);
      }
    }
  }
}

void UsdImagingDelegate::SetDisplayUnloadedPrimsWithBounds(bool displayUnloaded)
{
  if (_hdPrimInfoMap.size() > 0) {
    TF_CODING_ERROR(
...
GfInterval UsdImagingDelegate::GetCurrentTimeSamplingInterval()
{
  TRACE_FUNCTION();
  ...
  if (!_cameraPathForSampling.IsEmpty()) {
    const VtValue vShutterOpen = GetCameraParamValue(_cameraPathForSampling, HdCameraTokens->shutterOpen);
    if (vShutterOpen.IsHolding<double>()) {
      shutterOpen = vShutterOpen.Get<double>();
    }

    const VtValue vShutterClose = GetCameraParamValue(_cameraPathForSampling, HdCameraTokens->shutterClose);
    if (vShutterClose.IsHolding<double>()) {
      shutterClose = vShutterClose.Get<double>();
    }
  }

  return GfInterval(GetTimeWithOffset(shutterOpen).GetValue(), GetTimeWithOffset(shutterClose).GetValue());
}

void UsdImagingDelegate::SetCameraForSampling(SdfPath const &usdPath)
{
  _cameraPathForSampling = usdPath;
}

/*virtual*/
TfToken UsdImagingDelegate::GetRenderTag(SdfPath const &id)
{
  TRACE_FUNCTION();
  ...
  if (TF_VERIFY(primInfo)) {
    purpose = primInfo->adapter->GetPurpose(primInfo->usdPrim, cachePath, TfToken());
  }
  ...
  TF_DEBUG(USDIMAGING_COLLECTIONS).Msg("GetRenderTag %s -> %s \n", cachePath.GetText(), purpose.GetText());
  return purpose;
}

/*virtual*/
HdBasisCurvesTopology UsdImagingDelegate::GetBasisCurvesTopology(SdfPath const &id)
{
  TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    VtValue topology = primInfo->adapter->GetTopology(primInfo->usdPrim, cachePath, _time);
...
/*virtual*/
HdMeshTopology UsdImagingDelegate::GetMeshTopology(SdfPath const &id)
{
  TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
...
/*virtual*/
UsdImagingDelegate::SubdivTags UsdImagingDelegate::GetSubdivTags(SdfPath const &id)
{
  TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->GetSubdivTags(primInfo->usdPrim, cachePath, _time);
  }
  return PxOsdSubdivTags();
}

/*virtual*/
GfRange3d UsdImagingDelegate::GetExtent(SdfPath const &id)
{
  TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->GetExtent(primInfo->usdPrim, cachePath, _time);
  }
  return GfRange3d();
}

/*virtual*/
bool UsdImagingDelegate::GetDoubleSided(SdfPath const &id)
{
  ...
  if (TF_VERIFY(primInfo)) {
    doubleSided = primInfo->adapter->GetDoubleSided(primInfo->usdPrim, cachePath, _time);
  }
...
/*virtual*/
HdCullStyle UsdImagingDelegate::GetCullStyle(SdfPath const &id)
{
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    cullStyle = primInfo->adapter->GetCullStyle(primInfo->usdPrim, cachePath, _time);
  }
...
/*virtual*/
HdDisplayStyle UsdImagingDelegate::GetDisplayStyle(SdfPath const &id)
{
  SdfPath cachePath = ConvertIndexPathToCachePath(id);
  int level = 0;
...
void UsdImagingDelegate::SetRefineLevelFallback(int level)
{
  ...
  TF_FOR_ALL(it, _hdPrimInfoMap)
  {
    ...
    if (_refineLevelMap.find(cachePath) == _refineLevelMap.end()) {
      if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
        primInfo.adapter->MarkRefineLevelDirty(primInfo.usdPrim, cachePath, &indexProxy);
      }
    }
  }
}

void UsdImagingDelegate::SetRefineLevel(SdfPath const &usdPath, int level)
{
...
void UsdImagingDelegate::ClearRefineLevel(SdfPath const &usdPath)
{
  _RefineLevelMap::iterator it = _refineLevelMap.find(usdPath);
  if (it == _refineLevelMap.end())
    return;
...
void UsdImagingDelegate::SetReprFallback(HdReprSelector const &repr)
{
  HD_TRACE_FUNCTION();
  ...
  TF_FOR_ALL(it, _hdPrimInfoMap)
  {
    ...
    if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
      primInfo.adapter->MarkReprDirty(primInfo.usdPrim, cachePath, &indexProxy);
    }
  }
}

void UsdImagingDelegate::SetCullStyleFallback(HdCullStyle cullStyle)
{
  HD_TRACE_FUNCTION();
  ...
  TF_FOR_ALL(it, _hdPrimInfoMap)
  {
    ...
    if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
      primInfo.adapter->MarkCullStyleDirty(primInfo.usdPrim, cachePath, &indexProxy);
    }
  }
}

void UsdImagingDelegate::SetRootTransform(GfMatrix4d const &xf)
{
  HD_TRACE_FUNCTION();
  ...
  TF_FOR_ALL(it, _hdPrimInfoMap)
  {
    ...
    _HdPrimInfo &primInfo = it->second;
    if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
      primInfo.adapter->MarkTransformDirty(primInfo.usdPrim, cachePath, &indexProxy);
    }
  }
}

void UsdImagingDelegate::SetInvisedPrimPaths(SdfPathVector const &invisedPaths)
{
  HD_TRACE_FUNCTION();
  ...
  SdfPathSet sortedExistingInvisedPaths(_invisedPrimPaths.begin(), _invisedPrimPaths.end());
  ...
  TF_FOR_ALL(changingInvisPathIt, changingInvisPaths)
  {
    ...
    if (!usdPrim) {
      TF_CODING_ERROR("Could not find prim at path <%s>.", usdSubtreeRoot.GetText());
      continue;
    }

    TF_DEBUG(USDIMAGING_CHANGES).Msg("[Vis/Invis Prim] <%s>\n", usdSubtreeRoot.GetText());
    SdfPath visAttr = usdSubtreeRoot.AppendProperty(UsdGeomTokens->visibility);
...
void UsdImagingDelegate::SetRigidXformOverrides(RigidXformOverridesMap const &rigidXformOverrides)
{
  HD_TRACE_FUNCTION();
  ...
  TF_FOR_ALL(it, rigidXformOverrides)
  {
    ...
    RigidXformOverridesMap::const_iterator existingIt = _rigidXformOverrides.find(overridePath);
  ...
  _xformCache.UpdateValueOverrides(overridesToUpdate, overridesToRemove, &dirtySubtreeRoots);
  ...
  TF_FOR_ALL(it, dirtySubtreeRoots)
  {
    ...
    if (!usdPrim) {
      TF_CODING_ERROR("Could not find prim at path <%s>.", subtreeRoot.GetText());
      continue;
    }

    TF_DEBUG(USDIMAGING_CHANGES).Msg("[RigidXform override] <%s>\n", subtreeRoot.GetText());

    SdfPath xformAttr = subtreeRoot.AppendProperty(UsdGeomTokens->xformOpOrder);
...
void UsdImagingDelegate::SetRootVisibility(bool isVisible)
{
  ...
  TF_FOR_ALL(it, _hdPrimInfoMap)
  {
    ...
    if (TF_VERIFY(primInfo.adapter, "%s", cachePath.GetText())) {
      primInfo.adapter->MarkVisibilityDirty(primInfo.usdPrim, cachePath, &indexProxy);
    }
  }
}

SdfPath UsdImagingDelegate::GetScenePrimPath(SdfPath const &rprimId,
                                             int instanceIndex,
                                             HdInstancerContext *instancerContext)
{
  ...
  if (!primInfo || !primInfo->adapter) {
    TF_WARN("GetScenePrimPath: Couldn't find rprim <%s>", rprimId.GetText());
    return cachePath;
  }

  SdfPath protoPath = primInfo->adapter->GetScenePrimPath(cachePath, instanceIndex, instancerContext);
...
bool UsdImagingDelegate::PopulateSelection(HdSelection::HighlightMode const &highlightMode,
                                           ...
                                           HdSelectionSharedPtr const &result)
{
  ...
  for (size_t i = 0; i < affectedCachePaths.size(); ++i) {
    ...
    _HdPrimInfo *primInfo = _GetHdPrimInfo(affectedCachePath);
    if (primInfo == nullptr) {
      TF_CODING_ERROR("Couldn't find primInfo for cache path %s", affectedCachePath.GetText());
      continue;
    }
...
/*virtual*/
GfMatrix4d UsdImagingDelegate::GetTransform(SdfPath const &id)
{
  TRACE_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->GetTransform(primInfo->usdPrim, cachePath, _time);
  }
  return ctm;
}

/*virtual*/
size_t UsdImagingDelegate::SampleTransform(SdfPath const &id,
...
bool UsdImagingDelegate::IsInInvisedPaths(SdfPath const &usdPath) const
{
...
/*virtual*/
bool UsdImagingDelegate::GetVisible(SdfPath const &id)
{
  TRACE_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->GetVisible(primInfo->usdPrim, cachePath, _time);
  }
  return false;
}

/*virtual*/
VtValue UsdImagingDelegate::Get(SdfPath const &id, TfToken const &key)
{
  return _Get(id, key, nullptr);
}

/*virtual*/
VtValue UsdImagingDelegate::GetIndexedPrimvar(SdfPath const &id, TfToken const &key, VtIntArray *outIndices)
{
  return _Get(id, key, outIndices);
}

VtValue UsdImagingDelegate::_Get(SdfPath const &id, TfToken const &key, VtIntArray *outIndices)
{
  HD_TRACE_FUNCTION();
...
HdIdVectorSharedPtr UsdImagingDelegate::GetCoordSysBindings(SdfPath const &id)
{
  if (!_coordSysEnabled) {
    return nullptr;
  }
...
/*virtual*/
size_t UsdImagingDelegate::SamplePrimvar(SdfPath const &id,
                                         TfToken const &key,
                                         size_t maxNumSamples,
                                         float *sampleTimes,
                                         VtValue *sampleValues)
{
  return _SamplePrimvar(id, key, maxNumSamples, sampleTimes, sampleValues, nullptr);
}

/*virtual*/
size_t UsdImagingDelegate::SampleIndexedPrimvar(SdfPath const &id,
                                                TfToken const &key,
                                                size_t maxNumSamples,
                                                float *sampleTimes,
                                                VtValue *sampleValues,
...
{
  return _SamplePrimvar(id, key, maxNumSamples, sampleTimes, sampleValues, sampleIndices);
}

size_t UsdImagingDelegate::_SamplePrimvar(SdfPath const &id,
                                          TfToken const &key,
                                          size_t maxNumSamples,
                                          float *sampleTimes,
                                          VtValue *sampleValues,
...
/*virtual*/
HdReprSelector UsdImagingDelegate::GetReprSelector(SdfPath const &id)
{
  return _reprFallback;
}

/*virtual*/
VtArray<TfToken> UsdImagingDelegate::GetCategories(SdfPath const &id)
{
  SdfPath cachePath = ConvertIndexPathToCachePath(id);
...
/*virtual*/
std::vector<VtArray<TfToken>> UsdImagingDelegate::GetInstanceCategories(SdfPath const &instancerId)
{
  SdfPath cachePath = ConvertIndexPathToCachePath(instancerId);
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->GetInstanceCategories(primInfo->usdPrim);
  }
...
HdPrimvarDescriptorVector UsdImagingDelegate::GetPrimvarDescriptors(SdfPath const &id,
...
/*virtual*/
VtIntArray UsdImagingDelegate::GetInstanceIndices(SdfPath const &instancerId, SdfPath const &prototypeId)
{
  HD_TRACE_FUNCTION();
  ...
  if (TF_VERIFY(primInfo)) {
    indices = primInfo->adapter->GetInstanceIndices(primInfo->usdPrim, cachePath, prototypeCachePath, _time);
  }
...
/*virtual*/
GfMatrix4d UsdImagingDelegate::GetInstancerTransform(SdfPath const &instancerId)
{
  TRACE_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    ctm = primInfo->adapter->GetInstancerTransform(primInfo->usdPrim, cachePath, _time);
  }
  return ctm;
}

/*virtual*/
size_t UsdImagingDelegate::SampleInstancerTransform(SdfPath const &instancerId,
...
/*virtual*/
SdfPath UsdImagingDelegate::GetInstancerId(SdfPath const &primId)
{
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    pathValue = primInfo->adapter->GetInstancerId(primInfo->usdPrim, cachePath);
  }
...
/*virtual*/
SdfPathVector UsdImagingDelegate::GetInstancerPrototypes(SdfPath const &instancerId)
{
  SdfPath cachePath = ConvertIndexPathToCachePath(instancerId);
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    protos = primInfo->adapter->GetInstancerPrototypes(primInfo->usdPrim, cachePath);
  }
...
/*virtual*/
SdfPath UsdImagingDelegate::GetMaterialId(SdfPath const &rprimId)
{
  SdfPath cachePath = ConvertIndexPathToCachePath(rprimId);
  SdfPath pathValue;
  ...
  if (TF_VERIFY(primInfo)) {
    pathValue = primInfo->adapter->GetMaterialId(primInfo->usdPrim, cachePath, _time);
  }

  return ConvertCachePathToIndexPath(pathValue);
}

VtValue UsdImagingDelegate::GetMaterialResource(SdfPath const &materialId)
{
  ...
  if (TF_VERIFY(primInfo)) {
    vtMatResource = primInfo->adapter->GetMaterialResource(primInfo->usdPrim, cachePath, _time);
  }

  return vtMatResource;
}

VtValue UsdImagingDelegate::GetLightParamValue(SdfPath const &id, TfToken const &paramName)
{
  ...
  if (!light) {
    ...
    if (paramName == HdTokens->lightFilterLink) {
      ...
      UsdCollectionAPI lightFilterLink = lightFilter.GetFilterLinkCollectionAPI();
      return VtValue(_collectionCache.GetIdForCollection(lightFilterLink));
    }
...
VtValue UsdImagingDelegate::GetCameraParamValue(SdfPath const &id, TfToken const &paramName)
{
  TRACE_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->Get(primInfo->usdPrim, cachePath, paramName, _time, nullptr);
  }
  return VtValue();
}

HdVolumeFieldDescriptorVector UsdImagingDelegate::GetVolumeFieldDescriptors(SdfPath const &volumeId)
{
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->GetVolumeFieldDescriptors(primInfo->usdPrim, cachePath, _time);
  }

  return HdVolumeFieldDescriptorVector();
}

TfTokenVector UsdImagingDelegate::GetExtComputationSceneInputNames(SdfPath const &computationId)
{
  HD_TRACE_FUNCTION();
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->GetExtComputationSceneInputNames(cachePath);
  }
  return TfTokenVector();
}

HdExtComputationInputDescriptorVector UsdImagingDelegate::GetExtComputationInputDescriptors(
...
HdExtComputationOutputDescriptorVector UsdImagingDelegate::GetExtComputationOutputDescriptors(
  SdfPath const &computationId)
{
  HD_TRACE_FUNCTION();
...
HdExtComputationPrimvarDescriptorVector UsdImagingDelegate::GetExtComputationPrimvarDescriptors(
  SdfPath const &computationId,
  HdInterpolation interpolation)
{
  HD_TRACE_FUNCTION();
...
VtValue UsdImagingDelegate::GetExtComputationInput(SdfPath const &computationId, TfToken const &input)
{
  TRACE_FUNCTION();
...
size_t UsdImagingDelegate::SampleExtComputationInput(SdfPath const &computationId,
                                                     TfToken const &input,
                                                     size_t maxSampleCount,
                                                     float *sampleTimes,
                                                     VtValue *sampleValues)
{
  ...
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->SampleExtComputationInput(primInfo->usdPrim,
                                                        cachePath,
                                                        input,
                                                        GetTime(),
...
std::string UsdImagingDelegate::GetExtComputationKernel(SdfPath const &computationId)
{
  TRACE_FUNCTION();
...
void UsdImagingDelegate::InvokeExtComputation(SdfPath const &computationId, HdExtComputationContext *context)
{
  SdfPath cachePath = ConvertIndexPathToCachePath(computationId);
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
