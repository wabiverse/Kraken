...
UsdImagingDelegate::UsdImagingDelegate(HdRenderIndex *parentIndex, SdfPath const &delegateID)
  : HdSceneDelegate(parentIndex, delegateID),
    ...
    _hasDrawModeAdapter(UsdImagingAdapterRegistry::GetInstance().HasAdapter(
      UsdImagingAdapterKeyTokens->drawModeAdapterKey)),
    ...
    _displayUnloadedPrimsWithBounds(false)
{
  ...
  _coordSysBindingImplData.usdToHydraPath =
    std::bind(&UsdImagingDelegate::ConvertCachePathToIndexPath, this, std::placeholders::_1);
}
...
class UsdImagingDelegate::_Worker
{
 ...
 public:
  ...
  typename std::enable_if<!std::is_integral<ForwardIter>::value>::type AddTasks(ForwardIter begin,
                                                                                ForwardIter end)
  {
...
namespace
{
  ...
  class _NoticeBatchingContext
  {
   public:
    ...
    _NoticeBatchingContext(HdRenderIndex *renderIndex) : _renderIndex(renderIndex)
    {
...
void UsdImagingDelegate::_GatherDependencies(SdfPath const &subtree,
                                             SdfPathVector *affectedCachePaths)
{
  ...
  std::unique_copy(affectedPaths.begin(),
                   affectedPaths.end(),
  ...
  if (!_usdPathsToResync.empty()) {
    ...
    for (SdfPath const &usdPath : usdPathsToResync) {
      if (usdPath.IsPropertyPath()) {
        _RefreshUsdObject(usdPath, TfTokenVector(), &indexProxy, &allTrackedVariabilityPaths);
        ...
        _RefreshUsdObject(usdPath.GetParentPath(),
                          TfTokenVector(),
                          &indexProxy,
                          &allTrackedVariabilityPaths);
  ...
  if (!_usdPathsToUpdate.empty()) {
    ...
    TF_FOR_ALL (pathIt, usdPathsToUpdate) {
      ...
      if (usdPath.IsPropertyPath() || usdPath.IsAbsoluteRootOrPrimPath()) {
        ...
        _RefreshUsdObject(usdPath,
                          changedPrimInfoFields,
                          &indexProxy,
                          &allTrackedVariabilityPaths);
                                           ...
                                           SdfPathSet *allTrackedVariabilityPaths)
{
  ...
  for (SdfPath const &affectedCachePath : affectedCachePaths) {
    ...
    if (primInfo != nullptr && primInfo->usdPrim.IsValid() &&
      ...
      if (usdPath.IsAbsoluteRootOrPrimPath()) {
        dirtyBits = adapter->ProcessPrimChange(primInfo->usdPrim,
                                               affectedCachePath,
                                               changedInfoFields);
      } else if (usdPath.IsPropertyPath()) {
...
GfInterval UsdImagingDelegate::GetCurrentTimeSamplingInterval()
{
  ...
  if (!_cameraPathForSampling.IsEmpty()) {
    const VtValue vShutterOpen = GetCameraParamValue(_cameraPathForSampling,
                                                     HdCameraTokens->shutterOpen);
    if (vShutterOpen.IsHolding<double>()) {
      shutterOpen = vShutterOpen.Get<double>();
    }

    const VtValue vShutterClose = GetCameraParamValue(_cameraPathForSampling,
  ...
  return GfInterval(GetTimeWithOffset(shutterOpen).GetValue(),
                                             ...
                                             HdInstancerContext *instancerContext)
{
  ...
  SdfPath protoPath = primInfo->adapter->GetScenePrimPath(cachePath,
                                                          instanceIndex,
                                                          instancerContext);
...
SdfPathVector UsdImagingDelegate::GetScenePrimPaths(
  ...
  std::vector<HdInstancerContext> *instancerContexts)
{
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (!primInfo || !primInfo->adapter) {
    TF_WARN("GetScenePrimPaths: Couldn't find rprim <%s>", rprimId.GetText());
    return SdfPathVector(instanceIndices.size(), cachePath);
  }

  return primInfo->adapter->GetScenePrimPaths(cachePath, instanceIndices, instancerContexts);
}
                                           ...
                                           GfMatrix4d *sampleValues)
{
  ...
  _HdPrimInfo *primInfo = _GetHdPrimInfo(cachePath);
  if (TF_VERIFY(primInfo)) {
    return primInfo->adapter->SampleTransform(primInfo->usdPrim,
                                              cachePath,
                                              _time,
                                              maxNumSamples,
                                              sampleTimes,
...
/*virtual*/
VtValue UsdImagingDelegate::GetIndexedPrimvar(SdfPath const &id,
                                              TfToken const &key,
...
/*virtual*/
VtIntArray UsdImagingDelegate::GetInstanceIndices(SdfPath const &instancerId,
                                                  SdfPath const &prototypeId)
{
  ...
  if (TF_VERIFY(primInfo)) {
    indices = primInfo->adapter->GetInstanceIndices(primInfo->usdPrim,
                                                    cachePath,
                                                    prototypeCachePath,
                                                    _time);
  }
...
VtValue UsdImagingDelegate::GetMaterialResource(SdfPath const &materialId)
{
  ...
  return primInfo->adapter->GetMaterialResource(primInfo->usdPrim, cachePath, _time);
}
...
HdVolumeFieldDescriptorVector UsdImagingDelegate::GetVolumeFieldDescriptors(
...
VtValue UsdImagingDelegate::GetExtComputationInput(SdfPath const &computationId,
                                                   TfToken const &input)
{
  TRACE_FUNCTION();
...
void UsdImagingDelegate::InvokeExtComputation(SdfPath const &computationId,
