  ...
  while (!treeStack.empty()) {
    ...
    if (adapter) {
      ...
      if (adapter->IsInstancerAdapter()) {
        ...
        TfToken protoName(TfStringPrintf("proto%d_%s_id%d",
                                         protoIndex,
                                         iter->GetPath().GetName().c_str(),
                                         protoID++));
                                                                   ...
                                                                   TfToken const &propertyName)
{
  if (IsChildPath(cachePath)) {
    ...
    // a re-sync.
    HdDirtyBits dirtyBits = proto.adapter->ProcessPropertyChange(protoUsdPrim,
                                                                 cachePath,
                                                                 propertyName);
                                                              ...
                                                              UsdTimeCode time) const
{
  ...
  if (parent) {
    GfMatrix4d parentToWorld = GetTransform(parent,
                                            parent.GetPath(),
                                            time,
                                            /*ignoreRootTransform=*/true);
  ...
  HdInstancerContext *instancerContext) const
{
  ...
  if (parentInstancerUsdPrim) {
    ...
    fqInstancerPath = parentAdapter->GetScenePrimPath(instancerPath,
                                                      parentIndex,
                                                      instancerContext);
  }
                                                                 ...
                                                                 GfMatrix4d *sampleValues)
{
  ...
  if (!parentInstancerCachePath.IsEmpty()) {
    ...
    numSamples = _GatherAuthoredTransformTimeSamples(parentInstancerUsdPrim,
                                                     interval,
                                                     &timeSamples);
    ...
    numSamples = _GatherAuthoredTransformTimeSamples(_GetPrim(instancerPath),
                                                     interval,
                                                     &timeSamples);
                                                 ...
                                                 UsdTimeCode time) const
{
  ...
  if (IsChildPath(cachePath)) {
    ...
    if (protoHasFixedVis) {
      ...
      _ComputeProtoVisibility(_GetPrim(proto.protoRootPath),
  ...
  const UsdImagingInstancerContext * /*unused*/) const
{
  ...
  if (_GetProtoPrimForChild(usdPrim, cachePath, &proto, &ctx)) {
    ...
    return proto->adapter->GetExtComputationInput(_GetProtoUsdPrim(*proto),
                                                  cachePath,
                                                  name,
                                                  time,
                                                  &ctx);
  ...
  if (!parentInstancerCachePath.IsEmpty()) {
    ...
    if (proto.protoRootPath == cachePath) {
      ...
      if (inPrototype) {
        ...
        transformRoot = BaseAdapter::GetTransform(_GetPrim(proto.protoRootPath),
                                                  proto.protoRootPath,
                                                  time);
      }
    }
