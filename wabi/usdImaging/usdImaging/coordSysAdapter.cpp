...
bool UsdImagingCoordSysAdapter::IsSupported(UsdImagingIndexProxy const *index) const
{
  return index->IsSprimTypeSupported(HdPrimTypeTokens->coordSys);
}

SdfPath UsdImagingCoordSysAdapter::Populate(UsdPrim const &usdPrim,
    ...
    for (size_t i = 0, n = idVec.size(); i < n; ++i) {
      ...
      if (!index->IsPopulated(idVec[i])) {
        index->InsertSprim(HdPrimTypeTokens->coordSys,
                           ...
                           shared_from_this());
        index->AddDependency(idVec[i], _GetPrim(bindingVec[i].bindingRelPath.GetPrimPath()));
      }
    }
  }
  return SdfPath();
...
void UsdImagingCoordSysAdapter::_RemovePrim(SdfPath const &cachePath, UsdImagingIndexProxy *index)
{
...
HdDirtyBits UsdImagingCoordSysAdapter::ProcessPropertyChange(UsdPrim const &prim,
                                                             SdfPath const &cachePath,
...
void UsdImagingCoordSysAdapter::MarkDirty(UsdPrim const &prim,
                                          SdfPath const &cachePath,
                                          HdDirtyBits dirty,
                                          UsdImagingIndexProxy *index)
{
...
void UsdImagingCoordSysAdapter::MarkTransformDirty(UsdPrim const &prim,
                                                   SdfPath const &cachePath,
                                                   UsdImagingIndexProxy *index)
{
