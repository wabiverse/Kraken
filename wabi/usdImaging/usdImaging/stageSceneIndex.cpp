...
//
#include "wabi/usdImaging/usdImaging/stageSceneIndex.h"
...
TfTokenVector UsdImagingStageSceneIndex::_GetImagingSubprims(
  const UsdImagingPrimAdapterSharedPtr &adapter) const
{
  if (adapter) {
    ...
    if (std::find(subprims.begin(), subprims.end(), TfToken()) == subprims.end()) {
...
TfToken UsdImagingStageSceneIndex::_GetImagingSubprimType(
...
HdContainerDataSourceHandle UsdImagingStageSceneIndex::_GetImagingSubprimData(
  const UsdImagingPrimAdapterSharedPtr &adapter,
  UsdPrim prim,
  TfToken const &subprim) const
{
  if (adapter) {
    HdContainerDataSourceHandle ds = adapter->GetImagingSubprimData(subprim, prim, _stageGlobals);
    if (ds) {
      return ds;
    }
  }
  ...
  if (subprim.IsEmpty()) {
    return UsdImagingDataSourcePrim::New(prim.GetPath(), prim, _stageGlobals);
  }

  return nullptr;
}

void UsdImagingStageSceneIndex::_PopulateAdapterMap()
{
  TRACE_FUNCTION();
...
UsdImagingPrimAdapterSharedPtr UsdImagingStageSceneIndex::_AdapterLookup(UsdPrim prim) const
{
  ...
  _AdapterMap::const_iterator it = _adapterMap.find(typeInfo.GetSchemaTypeName());
...
HdSceneIndexPrim UsdImagingStageSceneIndex::GetPrim(const SdfPath &path) const
{
  TRACE_FUNCTION();
  ...
  const TfToken subprim = path.IsPropertyPath() ? path.GetNameToken() : TfToken();
  ...
  const HdContainerDataSourceHandle dataSource = _GetImagingSubprimData(adapter, prim, subprim);
...
SdfPathVector UsdImagingStageSceneIndex::GetChildPrimPaths(const SdfPath &path) const
{
  TRACE_FUNCTION();
  ...
  UsdPrimSiblingRange range = prim.GetFilteredChildren(_GetTraversalPredicate());
...
void UsdImagingStageSceneIndex::_Populate(UsdPrim subtreeRoot)
{
  ...
  for (UsdPrim prim : range) {
    ...
    const TfTokenVector subprims = _GetImagingSubprims(adapter);
    for (TfToken const &subprim : subprims) {
      const SdfPath subpath = subprim.IsEmpty() ? primPath : primPath.AppendChild(subprim);
      addedPrims.emplace_back(subpath, _GetImagingSubprimType(adapter, subprim));
    }

    if (TfDebug::IsEnabled(USDIMAGING_POPULATION)) {
      TF_DEBUG(USDIMAGING_POPULATION)
        .Msg("[Population] Populating <%s> (type = %s) ->\n",
             primPath.GetText(),
...
Usd_PrimFlagsConjunction UsdImagingStageSceneIndex::_GetTraversalPredicate() const
{
...
void UsdImagingStageSceneIndex::_StageGlobals::SetTime(
  UsdTimeCode time,
  HdSceneIndexObserver::DirtiedPrimEntries *dirtied)
{
  _time = time;
