...
//
#ifndef WABI_USD_IMAGING_USD_IMAGING_STAGE_SCENE_INDEX_H
#define WABI_USD_IMAGING_USD_IMAGING_STAGE_SCENE_INDEX_H
...
class UsdImagingStageSceneIndex : public HdSceneIndexBase
{
 ...
 private:
  ...
  TfTokenVector _GetImagingSubprims(const UsdImagingPrimAdapterSharedPtr &adapter) const;
  TfToken _GetImagingSubprimType(const UsdImagingPrimAdapterSharedPtr &adapter,
                                 TfToken const &subprim) const;
  HdContainerDataSourceHandle _GetImagingSubprimData(const UsdImagingPrimAdapterSharedPtr &adapter,
                                                     UsdPrim prim,
                                                     TfToken const &subprim) const;
  ...
  class _StageGlobals : public UsdImagingDataSourceStageGlobals
  {
   public:
    ...
    void FlagAsTimeVarying(const SdfPath &primPath,
                           const HdDataSourceLocator &locator) const override;

    UsdTimeCode GetTime() const override;

    // Scene index-facing API
    void SetTime(UsdTimeCode time, HdSceneIndexObserver::DirtiedPrimEntries *dirtied);
    void Clear();
   ...
   private:
    ...
    using _VariabilityMap =
      tbb::concurrent_hash_map<SdfPath, HdDataSourceLocatorSet, _PathHashCompare>;
  ...
  using _AdapterMap = TfHashMap<TfToken, UsdImagingPrimAdapterSharedPtr, TfToken::HashFunctor>;
