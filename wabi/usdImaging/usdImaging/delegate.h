 ...
 */
#ifndef WABI_USD_IMAGING_USD_IMAGING_DELEGATE_H
#define WABI_USD_IMAGING_USD_IMAGING_DELEGATE_H
...
class UsdImagingDelegate : public HdSceneDelegate, public TfWeakBase {
 ...
 public:
  ...
  USDIMAGING_API
  UsdImagingDelegate(HdRenderIndex *parentIndex, SdfPath const &delegateID);
  ...
  USDIMAGING_API
  virtual std::vector<VtArray<TfToken>> GetInstanceCategories(SdfPath const &instancerId) override;
  USDIMAGING_API
  ...
  // Motion samples
  USDIMAGING_API
  virtual size_t SampleTransform(SdfPath const &id,
                                 size_t maxNumSamples,
                                 float *times,
                                 GfMatrix4d *samples) override;
  USDIMAGING_API
  virtual size_t SampleInstancerTransform(SdfPath const &instancerId,
                                          size_t maxSampleCount,
                                          float *times,
                                          GfMatrix4d *samples) override;
  USDIMAGING_API
  virtual size_t SamplePrimvar(SdfPath const &id,
                               TfToken const &key,
                               size_t maxNumSamples,
                               float *times,
  ...
  USDIMAGING_API
  virtual size_t SampleIndexedPrimvar(SdfPath const &id,
                                      TfToken const &key,
                                      size_t maxNumSamples,
                                      float *times,
                                      VtValue *samples,
  ...
  // Light Support
  USDIMAGING_API
  virtual VtValue GetLightParamValue(SdfPath const &id, TfToken const &paramName) override;
  // Camera Support
  USDIMAGING_API
  virtual VtValue GetCameraParamValue(SdfPath const &id, TfToken const &paramName) override;
 ...
 private:
  ...
  size_t _SamplePrimvar(SdfPath const &id,
                        TfToken const &key,
                        size_t maxNumSamples,
                        float *times,
                        VtValue *samples,
  ...
  // ---------------------------------------------------------------------- //
  void _OnUsdObjectsChanged(UsdNotice::ObjectsChanged const &, UsdStageWeakPtr const &sender);
  ...
  {
    UsdPrim const &p = _stage->GetPrimAtPath(usdPath.GetAbsoluteRootOrPrimPath());
    TF_VERIFY(p, "No prim found for id: %s", usdPath.GetAbsoluteRootOrPrimPath().GetText());
    return p;
  }

  VtValue _GetUsdPrimAttribute(SdfPath const &cachePath, TfToken const &attrName);
  ...
  TfTokenVector _GetPrimvarNames(SdfPath const &usdPath, TfToken const &interpolation);
  ...
  typedef TfHashMap<TfToken, UsdImagingPrimAdapterSharedPtr, TfToken::HashFunctor> _AdapterMap;
  ...
  struct _HdPrimInfo {
    ...
    TfDenseHashSet<SdfPath, SdfPath::Hash> extraDependencies;  // Dependencies that aren't usdPrim.
  };
  ...
  void _GatherDependencies(SdfPath const &subtree, SdfPathVector *affectedCachePaths);
  ...
  typedef std::unordered_map<SdfPath, TfTokenVector, SdfPath::Hash> _PathsToUpdateMap;
...
#endif  // WABI_USD_IMAGING_USD_IMAGING_DELEGATE_H
