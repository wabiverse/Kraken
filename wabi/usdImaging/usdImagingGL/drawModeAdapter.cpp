 ...
 */
#include "wabi/usdImaging/usdImagingGL/drawModeAdapter.h"
#include "wabi/usdImaging/usdImagingGL/package.h"
...
TF_DEFINE_PRIVATE_TOKENS(
  ...
  (material)
    ...
    (cardsUv)(cardsTexAssign)

      (textureXPosColor)(textureYPosColor)(textureZPosColor)(textureXNegColor)(textureYNegColor)(textureZNegColor)(textureXPosOpacity)(textureYPosOpacity)(textureZPosOpacity)(textureXNegOpacity)(textureYNegOpacity)(textureZNegOpacity)

        (worldtoscreen)

          (displayRoughness)

            (file)(st)(rgb)(a)(fallback)(minFilter)(magFilter)(linear)(linearMipmapLinear)

              (varname)(result)(activeTexCard));
...
static SdfPath _GetMaterialPath(UsdPrim const &prim)
{
  const SdfPath matPath = SdfPath(_tokens->material.GetString());
  return prim.GetPath().AppendPath(matPath);
}

UsdImagingGLDrawModeAdapter::UsdImagingGLDrawModeAdapter() : UsdImagingPrimAdapter(), _schemaColor(0)
{
  ...
  if (primDef) {
    primDef->GetAttributeFallbackValue(UsdGeomTokens->modelDrawModeColor, &_schemaColor);
  }
}
...
bool UsdImagingGLDrawModeAdapter::ShouldCullChildren() const
{
  return true;
}

bool UsdImagingGLDrawModeAdapter::CanPopulateUsdInstance() const
{
  return true;
}

bool UsdImagingGLDrawModeAdapter::IsSupported(UsdImagingIndexProxy const *index) const
{
  return true;
}

SdfPath UsdImagingGLDrawModeAdapter::Populate(UsdPrim const &prim,
                                              UsdImagingIndexProxy *index,
                                              UsdImagingInstancerContext const *instancerContext)
{
  SdfPath cachePath = UsdImagingGprimAdapter::_ResolveCachePath(prim.GetPath(), instancerContext);
  ...
  if (drawMode == UsdGeomTokens->origin || drawMode == UsdGeomTokens->bounds) {
    ...
    if (!index->IsRprimTypeSupported(HdPrimTypeTokens->mesh)) {
      TF_WARN(
        "Unable to display cards draw mode for model %s, "
    ...
    TF_CODING_ERROR(
      "Model <%s> has unsupported drawMode '%s'", prim.GetPath().GetText(), drawMode.GetText());
    return SdfPath();
  }
...
bool UsdImagingGLDrawModeAdapter::_IsMaterialPath(SdfPath const &path) const
{
  return path.GetNameToken() == _tokens->material;
}

void UsdImagingGLDrawModeAdapter::_RemovePrim(SdfPath const &cachePath, UsdImagingIndexProxy *index)
{
...
void UsdImagingGLDrawModeAdapter::MarkDirty(UsdPrim const &prim,
...
void UsdImagingGLDrawModeAdapter::MarkTransformDirty(UsdPrim const &prim,
                                                     SdfPath const &cachePath,
                                                     UsdImagingIndexProxy *index)
{
  if (!_IsMaterialPath(cachePath)) {
...
void UsdImagingGLDrawModeAdapter::MarkVisibilityDirty(UsdPrim const &prim,
                                                      SdfPath const &cachePath,
                                                      UsdImagingIndexProxy *index)
{
  if (!_IsMaterialPath(cachePath)) {
...
void UsdImagingGLDrawModeAdapter::MarkMaterialDirty(UsdPrim const &prim,
                                                    SdfPath const &cachePath,
                                                    UsdImagingIndexProxy *index)
{
  if (_IsMaterialPath(cachePath)) {
...
bool UsdImagingGLDrawModeAdapter::_HasVaryingExtent(UsdPrim const &prim) const
{
  UsdAttribute attr;
...
void UsdImagingGLDrawModeAdapter::_ComputeGeometryData(UsdPrim const &prim,
                                                       SdfPath const &cachePath,
                                                       UsdTimeCode time,
  ...
  if (drawMode == UsdGeomTokens->origin) {
    ...
    *extent = _ComputeExtent(prim, _HasVaryingExtent(prim) ? time : UsdTimeCode::EarliestTime());
    ...
    if (!model) {
      ...
      TF_CODING_ERROR(
        "Prim has draw mode 'cards' but geom model "
    ...
    if (cardGeometry == UsdGeomTokens->fromTexture) {
      ...
      *extent = _ComputeExtent(prim, _HasVaryingExtent(prim) ? time : UsdTimeCode::EarliestTime());
      ...
      if (cardGeometry == UsdGeomTokens->cross) {
        ...
        TF_CODING_ERROR(
          "<%s> Unexpected card geometry mode %s", cachePath.GetText(), cardGeometry.GetText());
      }
...
/*virtual*/
VtValue UsdImagingGLDrawModeAdapter::GetTopology(UsdPrim const &prim,
                                                 SdfPath const &cachePath,
...
/*virtual*/
GfRange3d UsdImagingGLDrawModeAdapter::GetExtent(UsdPrim const &prim,
                                                 SdfPath const &cachePath,
                                                 UsdTimeCode time) const
{
  TRACE_FUNCTION();
...
/*virtual*/
bool UsdImagingGLDrawModeAdapter::GetDoubleSided(UsdPrim const &prim,
                                                 SdfPath const &cachePath,
                                                 UsdTimeCode time) const
{
  return false;
...
/*virtual*/
VtValue UsdImagingGLDrawModeAdapter::Get(UsdPrim const &prim,
                                         SdfPath const &cachePath,
                                         TfToken const &key,
                                         UsdTimeCode time,
...
/*virtual*/
SdfPath UsdImagingGLDrawModeAdapter::GetMaterialId(UsdPrim const &prim,
                                                   SdfPath const &cachePath,
                                                   UsdTimeCode time) const
{
...
/*virtual*/
VtValue UsdImagingGLDrawModeAdapter::GetMaterialResource(UsdPrim const &prim,
                                                         SdfPath const &cachePath,
                                                         UsdTimeCode time) const
{
  if (!_IsMaterialPath(cachePath)) {
  ...
  SdrShaderNodeConstPtr sdrNode = shaderReg.GetShaderNodeFromAsset(
    path, NdrTokenMap(), TfToken(), HioGlslfxTokens->glslfx);
  ...
  const TfToken textureNames[12] = {_tokens->textureXPosColor,
  ...
  if (model) {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      if (!textureFile.GetAssetPath().empty()) {
        ...
        textureNode.parameters[_tokens->minFilter] = _tokens->linearMipmapLinear;
        textureNode.parameters[_tokens->magFilter] = _tokens->linear;
  ...
  SdfPath primvarNodePath = _GetMaterialPath(prim).AppendProperty(_tokens->cardsTexAssign);
...
void UsdImagingGLDrawModeAdapter::_CheckForTextureVariability(UsdPrim const &prim,
...
void UsdImagingGLDrawModeAdapter::TrackVariability(UsdPrim const &prim,
                                                   SdfPath const &cachePath,
...
void UsdImagingGLDrawModeAdapter::UpdateForTime(UsdPrim const &prim,
                                                SdfPath const &cachePath,
                                                UsdTimeCode time,
                                                ...
                                                UsdImagingInstancerContext const *instancerContext) const
{
  ...
  if (requestedBits & geometryBits) {
    ...
    if (drawMode == UsdGeomTokens->cards) {
      ...
      _MergePrimvar(&primvars, _tokens->cardsTexAssign, HdInterpolationUniform);

      // XXX: backdoor into the material system.
      _MergePrimvar(&primvars, _tokens->displayRoughness, HdInterpolationConstant);
    }

    // Merge "points" primvar
    _MergePrimvar(&primvars, HdTokens->points, HdInterpolationVertex, HdPrimvarRoleTokens->point);
  }
}

HdDirtyBits UsdImagingGLDrawModeAdapter::ProcessPropertyChange(UsdPrim const &prim,
                                                               SdfPath const &cachePath,
                                                               TfToken const &propertyName)
{
  ...
  HdDirtyBits dirtyGeo = HdChangeTracker::DirtyTopology | HdChangeTracker::DirtyPoints |
                         HdChangeTracker::DirtyPrimvar | HdChangeTracker::DirtyExtent;
...
void UsdImagingGLDrawModeAdapter::_GenerateOriginGeometry(VtValue *topo,
                                                          VtValue *points,
                                                          GfRange3d const &extents) const
{
  ...
  *points = VtValue(pt);
  ...
  HdBasisCurvesTopology topology(
    HdTokens->linear, HdTokens->bezier, HdTokens->segmented, curveVertexCounts, curveIndices);
  *topo = VtValue(topology);
}

void UsdImagingGLDrawModeAdapter::_GenerateBoundsGeometry(VtValue *topo,
                                                          VtValue *points,
                                                          GfRange3d const &extents) const
{
...
void UsdImagingGLDrawModeAdapter::_GenerateCardsCrossGeometry(VtValue *topo,
                                                              VtValue *points,
                                                              GfRange3d const &extents,
                                                              uint8_t axes_mask) const
{
  // Generate one face per axis direction, for included axes.
  const int numFaces = ((axes_mask & xAxis) ? 2 : 0) + ((axes_mask & yAxis) ? 2 : 0) +
                       ((axes_mask & zAxis) ? 2 : 0);
  ...
  HdMeshTopology topology(
    PxOsdOpenSubdivTokens->none, PxOsdOpenSubdivTokens->rightHanded, faceCounts, faceIndices, holeIndices);

  *points = VtValue(pt);
  *topo = VtValue(topology);
}

void UsdImagingGLDrawModeAdapter::_SanityCheckFaceSizes(SdfPath const &cachePath,
                                                        GfRange3d const &extents,
                                                        uint8_t axes_mask) const
{
  GfVec3d min = extents.GetMin(), max = extents.GetMax();
...
void UsdImagingGLDrawModeAdapter::_GenerateCardsBoxGeometry(VtValue *topo,
                                                            VtValue *points,
                                                            GfRange3d const &extents,
                                                            uint8_t axes_mask) const
{
  ...
  *points = VtValue(pt);
  ...
  HdMeshTopology topology(
    UsdGeomTokens->none, UsdGeomTokens->rightHanded, faceCounts, faceIndices, holeIndices);

  *points = VtValue(pt);
  *topo = VtValue(topology);
}

void UsdImagingGLDrawModeAdapter::_GenerateCardsFromTextureGeometry(VtValue *topo,
                                                                    VtValue *points,
                                                                    VtValue *uv,
                                                                    VtValue *assign,
                                                                    GfRange3d *extents,
                                                                    UsdPrim const &prim) const
{
  ...
  GfMatrix4d mat;
  if (_GetMatrixFromImageMetadata(model.GetModelCardTextureXPosAttr(), &mat))
    faces.push_back(std::make_pair(mat, xPos));
  if (_GetMatrixFromImageMetadata(model.GetModelCardTextureYPosAttr(), &mat))
    faces.push_back(std::make_pair(mat, yPos));
  if (_GetMatrixFromImageMetadata(model.GetModelCardTextureZPosAttr(), &mat))
    faces.push_back(std::make_pair(mat, zPos));
  if (_GetMatrixFromImageMetadata(model.GetModelCardTextureXNegAttr(), &mat))
    faces.push_back(std::make_pair(mat, xNeg));
  if (_GetMatrixFromImageMetadata(model.GetModelCardTextureYNegAttr(), &mat))
    faces.push_back(std::make_pair(mat, yNeg));
  if (_GetMatrixFromImageMetadata(model.GetModelCardTextureZNegAttr(), &mat))
...
template<class Vec> bool _ConvertToMatrix(const Vec &mvec, GfMatrix4d *mat)
{
  ...
  TF_WARN("worldtoscreen metadata expected 16 values, got %zu", mvec.size());
  return false;
};

}  // namespace

bool UsdImagingGLDrawModeAdapter::_GetMatrixFromImageMetadata(UsdAttribute const &attr,
...
static std::array<GfVec2f, 4> _GetUVsForQuad(const bool flipU, bool flipV)
{
  return {GfVec2f(flipU ? 0.0 : 1.0, flipV ? 0.0 : 1.0),
          GfVec2f(flipU ? 1.0 : 0.0, flipV ? 0.0 : 1.0),
          GfVec2f(flipU ? 1.0 : 0.0, flipV ? 1.0 : 0.0),
          GfVec2f(flipU ? 0.0 : 1.0, flipV ? 1.0 : 0.0)};
}

void UsdImagingGLDrawModeAdapter::_GenerateTextureCoordinates(VtValue *uv,
                                                              VtValue *assign,
                                                              uint8_t axes_mask) const
{
  ...
  static const std::array<GfVec2f, 4> uv_normal = _GetUVsForQuad(false, false);
  static const std::array<GfVec2f, 4> uv_flipped_s = _GetUVsForQuad(true, false);
  static const std::array<GfVec2f, 4> uv_flipped_t = _GetUVsForQuad(false, true);
  static const std::array<GfVec2f, 4> uv_flipped_st = _GetUVsForQuad(true, true);
  ...
  if (axes_mask & zAxis) {
    ...
    uv_faces.push_back((axes_mask & zPos) ? uv_normal.data() : uv_flipped_t.data());
    face_assign.push_back((axes_mask & zPos) ? zPos : zNeg);
    uv_faces.push_back((axes_mask & zNeg) ? uv_flipped_st.data() : uv_flipped_s.data());
...
GfRange3d UsdImagingGLDrawModeAdapter::_ComputeExtent(UsdPrim const &prim, const UsdTimeCode &timecode) const
{
  HD_TRACE_FUNCTION();
  HF_MALLOC_TAG_FUNCTION();
  ...
  if (prim.IsLoaded()) {
    ...
    if (prim.IsA<UsdGeomBoundable>() && (attr = UsdGeomBoundable(prim).GetExtentAttr()) &&
      ...
      for (size_t i = 0; i < purposeTokens.size(); ++i) {
        ...
        if (std::find(purposes.begin(), purposes.end(), purposeTokens[i]) == purposes.end())
          continue;

        GfRange3d purposeExtent = GfRange3d(extentsHint[idx], extentsHint[idx + 1]);
...
/*virtual*/
HdCullStyle UsdImagingGLDrawModeAdapter::GetCullStyle(UsdPrim const &prim,
                                                      SdfPath const &cachePath,
                                                      UsdTimeCode time) const
{
...
/*virtual*/
GfMatrix4d UsdImagingGLDrawModeAdapter::GetTransform(UsdPrim const &prim,
                                                     SdfPath const &cachePath,
                                                     UsdTimeCode time,
