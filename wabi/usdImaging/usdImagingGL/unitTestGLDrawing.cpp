...
#include "wabi/usdImaging/usdImagingGL/unitTestGLDrawing.h"

#include "wabi/base/arch/attributes.h"
#include "wabi/base/gf/vec2i.h"
#include "wabi/base/trace/collector.h"
...
class UsdImagingGL_UnitTestWindow : public GarchGLDebugWindow
{
 ...
 public:
  ...
  UsdImagingGL_UnitTestWindow(UsdImagingGL_UnitTestGLDrawing *unitTest, int w, int h);
...
void UsdImagingGL_UnitTestWindow::OnInitializeGL()
{
  ...
  _drawTarget->AddAttachment("depth", GL_DEPTH_COMPONENT, GL_FLOAT, GL_DEPTH_COMPONENT);
...
/* virtual */
void UsdImagingGL_UnitTestWindow::OnMousePress(int button, int x, int y, int modKeys)
{
  _unitTest->MousePress(button, x, y, modKeys);
}

/* virtual */
void UsdImagingGL_UnitTestWindow::OnMouseRelease(int button, int x, int y, int modKeys)
{
...
static void ParseError(const char *pname, const char *fmt, ...) ARCH_PRINTF_FUNCTION(2, 3);
static void ParseError(const char *pname, const char *fmt, ...)
{
