 ...
 */
#ifndef WABI_USD_IMAGING_USD_IMAGING_GL_LEGACY_ENGINE_H
#define WABI_USD_IMAGING_USD_IMAGING_GL_LEGACY_ENGINE_H

#include "wabi/usdImaging/usdImagingGL/api.h"
#include "wabi/wabi.h"

#include "wabi/usdImaging/usdImagingGL/renderParams.h"
...
class UsdImagingGLLegacyEngine : public TfWeakBase {
 ...
 public:
  ...
  /// -- Free Camera API
  USDIMAGINGGL_API
  void SetFreeCameraMatrices(const GfMatrix4d &viewMatrix, const GfMatrix4d &projectionMatrix);
 ...
 private:
  ...
  void _RenderPrimitive(const UsdPrim &prim,
 ...
 private:
  ...
  struct _PrimID {
    ...
    {
      ValueType primID = {((primIdColor[0] & 0xff) << 0) | ((primIdColor[1] & 0xff) << 8) |
  ...
  using _DrawTargetPerContextMap = std::unordered_map<GlfGLContextSharedPtr, GlfDrawTargetRefPtr>;
  _DrawTargetPerContextMap _drawTargets;
};

WABI_NAMESPACE_END

#endif  // WABI_USD_IMAGING_USD_IMAGING_GL_LEGACY_ENGINE_H
