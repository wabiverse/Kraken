...
#include "wabi/usdImaging/usdImaging/cubeAdapter.h"
#include "wabi/usdImaging/usdImaging/cylinderAdapter.h"
#include "wabi/usdImaging/usdImaging/nurbsPatchAdapter.h"
...
#include "wabi/usd/usdGeom/nurbsPatch.h"
#include "wabi/usd/usdGeom/points.h"
#include "wabi/usd/usdGeom/scope.h"
...
bool UsdImagingGLLegacyEngine::_SupportsPrimitiveRestartIndex()
{
  static bool supported = GlfHasExtensions("GL_NV_primitive_restart");
  return supported;
}

void UsdImagingGLLegacyEngine::InvalidateBuffers()
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_PopulateBuffers()
{
...
void UsdImagingGLLegacyEngine::_DrawLines(bool drawID)
{
  ...
  // lines.
  size_t offset = sizeof(GLfloat) * (_points.size() + _normals.size() + _colors.size());
...
void UsdImagingGLLegacyEngine::Render(const UsdPrim &root, const UsdImagingGLRenderParams &params)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::SetFreeCameraMatrices(const GfMatrix4d &viewMatrix,
...
void UsdImagingGLLegacyEngine::SetLightingState(GlfSimpleLightVector const &lights,
...
void UsdImagingGLLegacyEngine::_OnObjectsChanged(UsdNotice::ObjectsChanged const &notice,
                                                 UsdStageWeakPtr const &sender)
{
  InvalidateBuffers();
}

static void UsdImagingGLLegacyEngine_ComputeSmoothNormals(const VtVec3fArray &points,
...
static bool _ShouldCullDueToOpacity(const UsdGeomGprim *gprimSchema, const UsdTimeCode &frame)
{
  // XXX:
...
void UsdImagingGLLegacyEngine::_TraverseStage(const UsdPrim &root)
{
...
void UsdImagingGLLegacyEngine::_ResolveCamera()
{
  ...
  else
  {
    ...
    _UpdateGLCameraFramingState(_freeCamViewMatrix, _freeCamProjMatrix, _viewport);
  }
}

void UsdImagingGLLegacyEngine::_UpdateGLCameraFramingState(const GfMatrix4d &viewMatrix,
                                                           const GfMatrix4d &projectionMatrix,
...
void UsdImagingGLLegacyEngine::_ProcessGprimColor(const UsdGeomGprim *gprimSchema,
                                                  const UsdPrim &prim,
                                                  bool *doubleSided,
...
void UsdImagingGLLegacyEngine::_HandleXform(const UsdPrim &prim)
{
...
GfVec4f UsdImagingGLLegacyEngine::_IssueID(SdfPath const &path)
{
...
void UsdImagingGLLegacyEngine::_HandleMesh(const UsdPrim &prim)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_HandleCurves(const UsdPrim &prim)
{
  TRACE_FUNCTION();
  ...
  _ProcessGprimColor(&curvesSchema, prim, &doubleSided, &color, &colorInterpolation);
...
void UsdImagingGLLegacyEngine::_HandleCube(const UsdPrim &prim)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_HandleSphere(const UsdPrim &prim)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_HandleCone(const UsdPrim &prim)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_HandleCylinder(const UsdPrim &prim)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_HandleCapsule(const UsdPrim &prim)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_HandlePoints(const UsdPrim &prim)
{
  TF_WARN("Point primitives are not yet supported.");
}

void UsdImagingGLLegacyEngine::_HandleNurbsPatch(const UsdPrim &prim)
{
  TRACE_FUNCTION();
...
void UsdImagingGLLegacyEngine::_RenderPrimitive(const UsdPrim &prim,
...
static GfVec3d _UnProject(GfVec3d Pwin,
...
bool UsdImagingGLLegacyEngine::TestIntersection(const GfMatrix4d &viewMatrix,
                                                const GfMatrix4d &projectionMatrix,
                                                const UsdPrim &root,
                                                ...
                                                int *outHitInstanceIndex)
{
  ...
  if (!TfMapLookup(_drawTargets, context, &drawTarget))
  {
    ...
    else
    {
      ...
      drawTarget->AddAttachment("instanceId", GL_RGBA, GL_UNSIGNED_BYTE, GL_RGBA8);
      drawTarget->AddAttachment("depth", GL_DEPTH_COMPONENT, GL_FLOAT, GL_DEPTH_COMPONENT32F);
      drawTarget->Unbind();
    }
  ...
  glBindTexture(GL_TEXTURE_2D, drawTarget->GetAttachments().at("primId")->GetGlTextureName());
  glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, primId);

  GLubyte instanceId[width * height * 4];
  glBindTexture(GL_TEXTURE_2D, drawTarget->GetAttachments().at("instanceId")->GetGlTextureName());
  glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, instanceId);

  GLfloat depths[width * height];
  glBindTexture(GL_TEXTURE_2D, drawTarget->GetAttachments().at("depth")->GetGlTextureName());
