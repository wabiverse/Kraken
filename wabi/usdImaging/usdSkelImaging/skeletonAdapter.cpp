                                                ...
                                                const UsdImagingInstancerContext *instancerContext)
{
  ...
  if (bindingIt != _skelBindingMap.end()) {
    ...
    for (UsdSkelSkinningQuery const &query : binding.GetSkinningTargets()) {
      ...
      index->InsertSprim(HdPrimTypeTokens->extComputation,
                         compPath,
                         skinnedPrim,
                         shared_from_this());
                               ...
                               VtFloatArray *subShapeWeights)
  {
    ...
    if (const UsdSkelAnimQuery &animQuery = skelQuery.GetAnimQuery()) {
      if (animQuery.ComputeBlendShapeWeights(&weights, time)) {
        ...
        return blendShapeQuery.ComputeFlattenedSubShapeWeights(weightsInPrimOrder,
                                                               subShapeWeights);
      }
    }
    return false;
  ...
  const UsdImagingInstancerContext *instancerContext) const
{
  ...
  if (_IsSkinningComputationPath(cachePath)) {
    return _GetExtComputationInputForSkinningComputation(prim,
                                                         cachePath,
                                                         name,
                                                         time,
                                                         instancerContext);
  ...
  if (_IsSkinningInputAggregatorComputationPath(cachePath)) {
    return _GetExtComputationInputForInputAggregator(prim,
                                                     cachePath,
                                                     name,
                                                     time,
                                                     instancerContext);
