  ...
  _ParallelForN(topology.GetNumJoints(), [&](size_t start, size_t end) {
    for (size_t i = start; i < end; ++i) {
      ...
      if (boneIndex >= 0) {
        ...
        _ComputePointsForSingleBone(points + offset, jointSkelXforms[i], jointSkelXforms[parent]);
      }
    }
  });
                                           ...
                                           size_t numPoints)
{
  ...
  return UsdSkelImagingComputeBoneJointIndices(topology, jointIndices->data(), numPoints);
}
