...
ifeq ($(OS), Darwin)
	...
	BUILD_COMMAND:=xcodebuild docbuild -derivedDataPath "$(BUILD_DIR)/docbuild" -arch "arm64" -sdk "macosx14.0" -project "$(BUILD_DIR)/Kraken.xcodeproj" -configuration Release -scheme install | pwsh -Command xcprettyfix -r json-compilation-database --output "$(KRAKEN_DIR)/compile_commands.json"
