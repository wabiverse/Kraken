...
if [ ! -d "${SRC_DIR}/Kraken.app" ]; then
    echo "use --source parameter to set source directory where Kraken.app can be found"
    exit 1
fi
...
if [ -d "${_mount_dir}" ]; then
    echo -n "Ejecting existing kraken volume.."
...
# Copy dmg contents.
echo -n "Copying Kraken.app..."
cp -r "${SRC_DIR}/Kraken.app" "${_tmp_dir}/" || exit 1
echo
...
if [ ! -z "${C_CERT}" ]; then
    ...
    for f in $(find "${_mount_dir}/Kraken.app/Contents/Resources" -name "python*"); do
        if [ -x ${f} ] && [ ! -d ${f} ]; then
    ...
    for f in $(find "${_mount_dir}/Kraken.app" -name "*.dylib" -o -name "*.so"); do
        codesign --remove-signature "${f}"
        codesign --timestamp --options runtime --entitlements="${_entitlements}" --sign "${C_CERT}" "${f}"
    done
    echo ; echo -n "Codesigning Kraken.app"
    codesign --remove-signature "${_mount_dir}/Kraken.app"
    codesign --timestamp --options runtime --entitlements="${_entitlements}" --sign "${C_CERT}" "${_mount_dir}/Kraken.app"
    echo
else
