...
functions for dealing with paths in Kraken.
"""
...
def abspath(path, *, start=None, library=None):
    ...
    :type library: :class:`kpy.types.Library`
    ...
    if isinstance(path, bytes):
        if path.startswith(b"//"):
            ...
            return _os.path.join(
                _os.path.dirname(_getattr_bytes(_kpy.data, "filepath"))
        ...
        if path.startswith("//"):
            ...
            return _os.path.join(
                _os.path.dirname(_kpy.data.filepath)
                if start is None else start,
                path[2:],
            )
...
def relpath(path, *, start=None):
    ...
    if isinstance(path, bytes):
        if not path.startswith(b"//"):
            if start is None:
                start = _os.path.dirname(_getattr_bytes(_kpy.data, "filepath"))
            return b"//" + _os.path.relpath(path, start)
    else:
        if not path.startswith("//"):
            if start is None:
                start = _os.path.dirname(_kpy.data.filepath)
