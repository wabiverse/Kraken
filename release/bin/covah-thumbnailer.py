...
covah.thumbnailer file in ${HOME}/.local/share/thumbnailers/ directory
with the following contents:

[Thumbnailer Entry]
TryExec=covah-thumbnailer.py
Exec=covah-thumbnailer.py %u %o
MimeType=application/x-covah;
"""
...
def usd_extract_thumb(path):
    ...
    if not head.startswith(b'COVAH'):
        usdfile.close()
        return None, 0, 0
    ...
    while True:
        bhead = usdfile.read(sizeof_bhead)
        ...
        if code == REND:
            usdfile.seek(length, os.SEEK_CUR)
        else:
            break
    ...
    try:
        x, y = struct.unpack(int_endian_pair, usdfile.read(8))  # 8 == sizeof(int) * 2
    except struct.error:
        return None, 0, 0
    ...
    image_buffer = usdfile.read(length)
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Expected 2 arguments <input.usda> <output.png>")
    else:
        file_in = sys.argv[-2]

        buf, width, height = usd_extract_thumb(file_in)
