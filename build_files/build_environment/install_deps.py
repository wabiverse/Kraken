...
def InstallCycles(context, force, buildArgs):
    ...
    with CurrentWorkingDirectory(DownloadURL(CYCLES_URL, context, force)):
        ...
        if Linux():
            ...
            buildArgs.append("-DUSE_OPENGL=ON")
            buildArgs.append('-DBOOST_ROOT={boost}'.format(boost=context.libInstDir))
            buildArgs.append('-DBoost_INCLUDE_DIR={boost}'.format(boost=context.libInstDir + "/boost/include/boost-1_78"))
            buildArgs.append('-DTBB_ROOT={tbb}'.format(tbb=context.libInstDir + "/tbb"))
            ...
            buildArgs.append('-DOPENCOLORIO_ROOT_DIR={color}'.format(color=context.libInstDir + "/opencolorio"))
            ...
            buildArgs.append("-DCLANG_ROOT_DIR={}".format(context.libInstDir + "/llvm"))

        RunCMake(context, force, buildArgs)

        if Linux():
            copy_tree(context.buildDir + "/cycles/bin", context.libInstDir + "/bin")
            copy_tree(context.buildDir + "/cycles/lib", context.libInstDir + "/lib")
            CopyFilesHeaders(context.srcDir + "/cycles/src/app",                 context.libInstDir + "/include/app")
            CopyFilesHeaders(context.srcDir + "/cycles/src/blender",             context.libInstDir + "/include/blender")
            CopyFilesHeaders(context.srcDir + "/cycles/src/bvh",                 context.libInstDir + "/include/bvh")
            CopyFilesHeaders(context.srcDir + "/cycles/src/device",              context.libInstDir + "/include/device")
            CopyFilesHeaders(context.srcDir + "/cycles/src/graph",               context.libInstDir + "/include/graph")
            CopyFilesHeaders(context.srcDir + "/cycles/src/kernel",              context.libInstDir + "/include/kernel")
            CopyFilesHeaders(context.srcDir + "/cycles/src/render",              context.libInstDir + "/include/render")
            CopyFilesHeaders(context.srcDir + "/cycles/src/subd",                context.libInstDir + "/include/subd")
            CopyFilesHeaders(context.srcDir + "/cycles/src/util",                context.libInstDir + "/include/util")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/atomic",      context.libInstDir + "/include/atomic")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/clew",        context.libInstDir + "/include/clew")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/cuew",        context.libInstDir + "/include/cuew")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/libc_compat", context.libInstDir + "/include/libc_compat")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/numaapi",     context.libInstDir + "/include/numaapi")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/sky",         context.libInstDir + "/include/sky")
        elif Windows():
            copy_tree(context.buildDir + "/cycles/bin", context.libInstDir + "/Cycles/bin")
            copy_tree(context.buildDir + "/cycles/lib", context.libInstDir + "/Cycles/lib")
            CopyFilesHeaders(context.srcDir + "/cycles/src/app",                 context.libInstDir + "/Cycles/include/app")
            CopyFilesHeaders(context.srcDir + "/cycles/src/blender",             context.libInstDir + "/Cycles/include/blender")
            CopyFilesHeaders(context.srcDir + "/cycles/src/bvh",                 context.libInstDir + "/Cycles/include/bvh")
            CopyFilesHeaders(context.srcDir + "/cycles/src/device",              context.libInstDir + "/Cycles/include/device")
            CopyFilesHeaders(context.srcDir + "/cycles/src/graph",               context.libInstDir + "/Cycles/include/graph")
            CopyFilesHeaders(context.srcDir + "/cycles/src/kernel",              context.libInstDir + "/Cycles/include/kernel")
            CopyFilesHeaders(context.srcDir + "/cycles/src/render",              context.libInstDir + "/Cycles/include/render")
            CopyFilesHeaders(context.srcDir + "/cycles/src/subd",                context.libInstDir + "/Cycles/include/subd")
            CopyFilesHeaders(context.srcDir + "/cycles/src/util",                context.libInstDir + "/Cycles/include/util")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/atomic",      context.libInstDir + "/Cycles/include/atomic")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/clew",        context.libInstDir + "/Cycles/include/clew")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/cuew",        context.libInstDir + "/Cycles/include/cuew")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/libc_compat", context.libInstDir + "/Cycles/include/libc_compat")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/numaapi",     context.libInstDir + "/Cycles/include/numaapi")
            CopyFilesHeaders(context.srcDir + "/cycles/third_party/sky",         context.libInstDir + "/Cycles/include/sky")
...
def InstallOSL(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(OSL_URL, context, force)):
        ...
        if Linux():
            ...
            # For now...
            llvmpath = Path(context.libInstDir) / "llvm"
            if not llvmpath.exists():
                # LLVM (🡱)
                llvm = subprocess.call("svn checkout https://svn.blender.org/svnroot/bf-blender/tags/blender-2.93-release/lib/win64_vc15/llvm/bin/", stdout=subprocess.PIPE, shell=True)
                copy_tree(context.srcDir + "/OpenShadingLanguage-1.11.13.0/llvm", context.libInstDir + "/llvm")
                # And PugiXML
                pugixml = subprocess.call("svn checkout https://svn.blender.org/svnroot/bf-blender/tags/blender-2.93-release/lib/win64_vc15/pugixml/", stdout=subprocess.PIPE, shell=True)
