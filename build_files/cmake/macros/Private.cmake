...
function(_copy_headers LIBRARY_NAME)
    ...
    if ("${CMAKE_CURRENT_SOURCE_DIR}" MATCHES ".*/wabi/.*")
        # Include paths under wabi/ match the source path.
        file(RELATIVE_PATH hpath "${CMAKE_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
    endif()
    set(header_dest_dir "${CMAKE_BINARY_DIR}/${WABI_INSTALL_SUBDIR}/include/${hpath}")
    if( NOT "${_args_FILES}" STREQUAL "")
        ...
        foreach (f ${_args_FILES})
            ...
            add_custom_command(
                ...
                COMMAND ${CMAKE_COMMAND} -Dinfile="${infile}" -Doutfile="${outfile}" -P "${CMAKE_SOURCE_DIR}/build_files/cmake/macros/copyHeaderForBuild.cmake"
...
function(_install_python LIBRARY_NAME)
    ...
    foreach(file ${ip_FILES})
        set(filesToInstall "")
        set(installDest
            "${libPythonPrefix}/wabi/${LIBRARY_INSTALLNAME}")
...
function(_install_resource_files NAME pluginInstallPrefix pluginToLibraryPath)
    ...
    foreach(resourceFile ${ARGN})
        ...
        # Because we moved plugin resources into a resources/ folder
        # (to keep it out of source code), we need to ensure we are
        # not installing plugins to resources/resources/, this will
        # strip the extra path out.
        string(REPLACE "resources" "" correctedDirPath "${dirPath}")
        install(
            FILES ${resourceFile}
            DESTINATION ${resourcesPath}/${correctedDirPath}
            RENAME ${destFileName}
        )
    endforeach()
...
function(_install_pyside_ui_files LIBRARY_NAME)
    ...
    install(
        FILES ${uiFiles}
        DESTINATION "${libPythonPrefix}/wabi/${LIBRARY_INSTALLNAME}"
    )
...
function(_get_install_dir path out)
    if (WABI_INSTALL_SUBDIR)
        set(${out} ${WABI_INSTALL_SUBDIR}/${path} PARENT_SCOPE)
    else()
...
function(_get_folder suffix result)
    # XXX -- Shouldn't we set WABI_PREFIX everywhere?
    if(WABI_PREFIX)
        set(folder "${WABI_PREFIX}")
    elseif(WABI_INSTALL_SUBDIR)
        set(folder "${WABI_INSTALL_SUBDIR}")
    else()
        set(folder "misc")
    endif()
    if(suffix)
...
function(_wabi_enable_precompiled_header TARGET_NAME)
    # Ignore if disabled.
    if(NOT WABI_ENABLE_PRECOMPILED_HEADERS)
        return()
    endif()
    ...
    # ${WABI_PRECOMPILED_HEADER_NAME}.
    if("${pch_SOURCE_NAME}" STREQUAL "")
        set(pch_SOURCE_NAME "${WABI_PRECOMPILED_HEADER_NAME}")
    endif()
    if("${pch_SOURCE_NAME}" STREQUAL "")
    ...
    set(rel_output_header_path "${WABI_PREFIX}/${TARGET_NAME}/${output_header_name}")
    set(abs_output_header_path "${CMAKE_BINARY_DIR}/include/${rel_output_header_path}")
    set(abs_precompiled_path ${CMAKE_BINARY_DIR}/include/${WABI_PREFIX}/${TARGET_NAME}/${precompiled_name})
...
function(_wabi_init_rpath rpathRef origin)
...
function(_wabi_add_rpath rpathRef target)
    if(IS_ABSOLUTE "${target}")
        # Make target relative to $ORIGIN (which is the first element in
        # rpath when initialized with _wabi_init_rpath()).
...
function(_wabi_install_rpath rpathRef NAME)
...
function(_wabi_split_libraries libs internal_result external_result)
    set(internal "")
    set(external "")
    foreach(lib ${libs})
        if(";${WABI_CORE_LIBS};" MATCHES ";${lib};")
...
# Helper functions for _wabi_transitive_internal_libraries.
function(_wabi_transitive_internal_libraries_r libs transitive_libs)
    ...
    foreach(lib ${libs})
        ...
        if(NOT ";${result};" MATCHES ";${lib};")
            ...
            _wabi_split_libraries("${implicit}" internal external)

            # Recurse on the internal libraries.
            _wabi_transitive_internal_libraries_r("${internal}" result)
        endif()
    endforeach()
    set(${transitive_libs} "${result}" PARENT_SCOPE)
endfunction()

function(_wabi_transitive_internal_libraries libs transitive_libs)
    # Get the transitive libs in some order.
    set(transitive "")
    _wabi_transitive_internal_libraries_r("${libs}" transitive)

    # Get the transitive libs in build order.
    set(result "")
    foreach(lib ${WABI_ALL_LIBS})
...
#
#   4) We link against WABI_MALLOC_LIBRARY and WABI_THREAD_LIBS because we
#      always want those.
#
function(_wabi_target_link_libraries NAME)
    # Split core libraries from non-core libraries.
    _wabi_split_libraries("${ARGN}" internal external)
    ...
    if("${type}" STREQUAL "OBJECT_LIBRARY")
        ...
        set(finalIncs "")
        set(finalSystemIncs "")
        _wabi_transitive_internal_libraries("${internal}" internal)
        ...
        if(WITH_KRAKEN_MONOLITHIC)
            if(internal)
                if(TARGET maelstrom)
                    ...
                    # target_link_options(maelstrom PUBLIC "LINKER:/WINMDFILE:maelstrom")
                else()
        ...
        elseif(NOT BUILD_SHARED_LIBS)
            ...
            foreach(lib ${internal})
                ...
                elseif(WIN32)
                    ...
                    set_property(TARGET ${lib} PROPERTY VS_PACKAGE_REFERENCES "Microsoft.NETCore.Platforms_6.0.0-preview.7.21377.19;Microsoft.Windows.CppWinRT_2.0.210806.1;Microsoft.WindowsAppSDK.DWrite_1.0.0-experimental1;Microsoft.WindowsAppSDK_1.0.0-experimental1;Microsoft.WindowsAppSDK.Foundation_1.0.0-experimental1;Microsoft.WindowsAppSDK.WinUI_1.0.0-experimental1")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_UseWindowsSdkPreview "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_WindowsSdkPackageVersion "10.0.22000.160-preview")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_PlatformToolset "v143")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_CppWinRTOptimized "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_CppWinRTRootNamespaceAutoMerge "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_MinimalCoreWin "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_DefaultLanguage "en-US")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_MinimumVisualStudioVersion "16.0")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_WindowsAppContainer "false")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_AppContainerApplication "false")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_ApplicationType "Windows Store")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_ApplicationTypeRevision "10.0")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_WindowsTargetPlatformVersion "10.0.22000.0")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_WindowsTargetPlatformMinVersion "10.0.22000.0")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_UseWinUI "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_CppWinRTProjectLanguage "C++/WinRT")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_CompileAsWinRT "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_PrecompiledHeader "Use")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_CanReferenceWinRT "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_DesktopCompatible "true")
                    set_property(TARGET ${lib} PROPERTY VS_GLOBAL_TargetFramework "net6.0-windows")
                elseif(CMAKE_COMPILER_IS_GNUCXX)
        ...
        if(WIN32)
            ...
            set_property(TARGET ${NAME} PROPERTY VS_PACKAGE_REFERENCES "Microsoft.NETCore.Platforms_6.0.0-preview.7.21377.19;Microsoft.Windows.CppWinRT_2.0.210806.1;Microsoft.WindowsAppSDK.DWrite_1.0.0-experimental1;Microsoft.WindowsAppSDK_1.0.0-experimental1;Microsoft.WindowsAppSDK.Foundation_1.0.0-experimental1;Microsoft.WindowsAppSDK.WinUI_1.0.0-experimental1")
            ...
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_CppWinRTOptimized "true")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_CppWinRTRootNamespaceAutoMerge "true")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_MinimalCoreWin "true")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_DefaultLanguage "en-US")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_MinimumVisualStudioVersion "16.0")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_WindowsAppContainer "false")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_AppContainerApplication "false")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_ApplicationType "Windows Store")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_ApplicationTypeRevision "10.0")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_WindowsTargetPlatformVersion "10.0.22000.0")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_WindowsTargetPlatformMinVersion "10.0.22000.0")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_UseWinUI "true")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_CppWinRTProjectLanguage "C++/WinRT")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_CompileAsWinRT "true")
            set_property(TARGET ${NAME} PROPERTY VS_GLOBAL_PrecompiledHeader "Use")
            ...
            if(TARGET maelstrom)
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_CompileAsWinRT "true")
                set_property(TARGET maelstrom APPEND_STRING PROPERTY LINK_FLAGS " /WINMD")
                set_property(TARGET maelstrom PROPERTY VS_WINRT_COMPONENT TRUE)
                set_property(TARGET maelstrom PROPERTY VS_PACKAGE_REFERENCES "Microsoft.NETCore.Platforms_6.0.0-preview.7.21377.19;Microsoft.Windows.CppWinRT_2.0.210806.1;Microsoft.WindowsAppSDK.DWrite_1.0.0-experimental1;Microsoft.WindowsAppSDK_1.0.0-experimental1;Microsoft.WindowsAppSDK.Foundation_1.0.0-experimental1;Microsoft.WindowsAppSDK.WinUI_1.0.0-experimental1")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_UseWindowsSdkPreview "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_WindowsSdkPackageVersion "10.0.22000.160-preview")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_PlatformToolset "v143")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_CppWinRTOptimized "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_CppWinRTRootNamespaceAutoMerge "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_MinimalCoreWin "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_DefaultLanguage "en-US")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_MinimumVisualStudioVersion "16.0")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_WindowsAppContainer "false")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_AppContainerApplication "false")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_ApplicationType "Windows Store")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_ApplicationTypeRevision "10.0")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_XamlLanguage "CppWinRT")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_WindowsTargetPlatformVersion "10.0.22000.0")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_WindowsTargetPlatformMinVersion "10.0.22000.0")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_UseWinUI "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_CppWinRTProjectLanguage "C++/WinRT")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_CompileAsWinRT "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_PrecompiledHeader "Use")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_CanReferenceWinRT "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_DesktopCompatible "true")
                set_property(TARGET maelstrom PROPERTY VS_GLOBAL_TargetFramework "net6.0-windows")
            elseif(TARGET maelstrom_static)
                set_property(TARGET maelstrom_static PROPERTY VS_GLOBAL_CompileAsWinRT "true")
            endif()
        endif()
    endif()
endfunction()
...
function(_wabi_python_module NAME)
    ...
    set_property(GLOBAL
        APPEND PROPERTY WABI_PYTHON_MODULES ${pyModuleName}
    )

    # Always install under the 'wabi' module, rather than base on the
    # project name. This makes importing consistent, e.g.
    # 'from wabi import X'. Additionally, python libraries always install
    ...
    if (WABI_INSTALL_SUBDIR)
        get_filename_component(
            SUBDIR_INC_DIR
            "${CMAKE_BINARY_DIR}/${WABI_INSTALL_SUBDIR}/include"
            ABSOLUTE
        )
    endif()
    ...
    # WABI_STRICT_BUILD_MODE as the redefinition warnings would cause build
    # failures.
    target_include_directories(${LIBRARY_NAME}
        SYSTEM
        PUBLIC
    ...
    if(NOT "${WABI_PREFIX}" STREQUAL "")
        if(args_PRECOMPILED_HEADERS)
            _wabi_enable_precompiled_header(${LIBRARY_NAME}
                OUTPUT_NAME_PREFIX "py"
                SOURCE_NAME "${args_PRECOMPILED_HEADER_NAME}"
            )
        endif()
    endif()
endfunction() # wabi_python_module

# Add a library target named NAME.
function(_wabi_library NAME)
    # Argument parsing.
    set(options
    )
    set(oneValueArgs
    ...
    if(_building_core)
        ...
        get_property(help CACHE WABI_CORE_LIBS PROPERTY HELPSTRING)
        list(APPEND WABI_CORE_LIBS ${NAME})
        set(WABI_CORE_LIBS "${WABI_CORE_LIBS}" CACHE INTERNAL "${help}")

        # Keep track of core OBJECT libraries.
        if(isObject)
            get_property(help CACHE WABI_OBJECT_LIBS PROPERTY HELPSTRING)
            list(APPEND WABI_OBJECT_LIBS ${NAME})
            set(WABI_OBJECT_LIBS "${WABI_OBJECT_LIBS}" CACHE INTERNAL "${help}")
        endif()
    endif()
    ...
    if(isObject)
        ...
        # manually.  See wabi_monolithic_epilogue().
        add_library(${NAME}
            OBJECT
            ${args_CPPFILES}
            ${args_PUBLIC_HEADERS}
    ...
    elseif(args_TYPE STREQUAL "STATIC")
        ...
        if(WABI_PY_UNDEFINED_DYNAMIC_LOOKUP)
    ...
    if(WABI_INSTALL_LOCATION)
        file(TO_CMAKE_PATH "${WABI_INSTALL_LOCATION}" wabiInstallLocation)
        set(wabiInstallLocation "WABI_INSTALL_LOCATION=${wabiInstallLocation}")
    endif()
    ...
    if(NOT args_TYPE STREQUAL "STATIC")
   	if(NOT (";${WABI_CORE_LIBS};" MATCHES ";${NAME};" AND _building_monolithic))
            file(RELATIVE_PATH
                pluginToLibraryPath
    ...
    _wabi_target_link_libraries(${NAME} ${args_LIBRARIES})
    ...
    _wabi_add_rpath(rpath "${CMAKE_INSTALL_PREFIX}/lib")
    _wabi_install_rpath(rpath ${NAME})
    ...
    if(isObject)
        ...
        if(NOT isPlugin)
            export(TARGETS ${NAME}
                APPEND
                FILE "${PROJECT_BINARY_DIR}/wabiTargets.cmake"
            )
        endif()
    ...
    if(NOT "${WABI_PREFIX}" STREQUAL "")
        if(args_PRECOMPILED_HEADERS)
            _wabi_enable_precompiled_header(${NAME}
                SOURCE_NAME "${args_PRECOMPILED_HEADER_NAME}"
            )
        endif()
    endif()
endfunction() # _wabi_library
