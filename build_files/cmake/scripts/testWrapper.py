...
def _parseArgs():
    parser = argparse.ArgumentParser(description='MAELSTROM test wrapper')
    ...
    parser.add_argument('--env-var', dest='envVars', default=[], type=str,
            ...
            help=('Variable to set in the test environment, in KEY=VALUE form. '
                  'If "<WABI_TEST_DIR>" is in the value, it is replaced with the '
    ...
    for varStr in args.envVars:
        try:
            ...
            if k == 'PATH':
                ...
                sys.exit(1)
            v = v.replace('<WABI_TEST_DIR>', testDir)
            env[k] = v
        except IndexError:
