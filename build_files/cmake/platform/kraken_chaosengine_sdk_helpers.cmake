...
# get windows 11 install path from registry,
# and confusingly enough KitsRoot10 is the
# proper name for windows 11 as well as 10.
function(get_installed_sdk
  sdk_folder         # the current sdk folder
  output_sdk_version # the current sdk version
)
  # return the kit path
  get_filename_component(win11_sdk_root "[HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows Kits\\Installed Roots;KitsRoot10]" ABSOLUTE CACHE)
  set(${sdk_folder} ${win11_sdk_root} PARENT_SCOPE)
  ...
  if(CMAKE_VS_WINDOWS_TARGET_PLATFORM_VERSION)
    ...
    file(GLOB win11_sdks RELATIVE "${win11_sdk_root}/UnionMetadata" "${win11_sdk_root}/UnionMetadata/*.*.*.*")
    list(GET win11_sdks 0 latest_sdk)
    foreach(sdk IN LISTS win11_sdks)
...
function(get_sdk_cppwinrt_exe
  ...
  if (NOT DEFINED kraken_chaosengine_CPPWINRT_EXE_PATH_OVERRIDE)
    ...
    set(${output_cppwinrt_exe_path} ${kraken_chaosengine_CPPWINRT_EXE_PATH_OVERRIDE} PARENT_SCOPE)
    endif()
endfunction()
...
function(get_sdk
  ...
  if ((NOT DEFINED kraken_chaosengine_WINDOWS_SDK_DIR_OVERRIDE) AND (NOT DEFINED kraken_chaosengine_WINDOWS_SDK_VERSION_OVERRIDE))
  ...
  elseif ((DEFINED kraken_chaosengine_WINDOWS_SDK_DIR_OVERRIDE) AND (DEFINED kraken_chaosengine_WINDOWS_SDK_VERSION_OVERRIDE))
    set(${output_sdk_folder} ${kraken_chaosengine_WINDOWS_SDK_DIR_OVERRIDE} PARENT_SCOPE)
    set(${output_sdk_version} ${kraken_chaosengine_WINDOWS_SDK_VERSION_OVERRIDE} PARENT_SCOPE)
  else()
    message(
      FATAL_ERROR
      "Options kraken_chaosengine_WINDOWS_SDK_DIR_OVERRIDE and kraken_chaosengine_WINDOWS_SDK_VERSION_OVERRIDE must be defined together, or not at all.")
    endif()
endfunction()