#!/usr/bin/env python3
#
# "make update" for all platforms, updating svn libraries and tests and COVAH
...
def parse_arguments():
    ...
    parser.add_argument("--no-covah", action="store_true")
...
def svn_update(args, release_version):
    ...
    lib_dirpath = os.path.join(get_covah_git_root(), '..', 'lib')
...
def submodules_update(args, release_version, branch):
    ...
    submodules = [
        ("release/scripts/addons", "covah-v2.90-staging"),
        ("release/scripts/addons_contrib", "covah-v2.90-staging"),
    ...
    for submodule_path, submodule_branch in submodules:
        ...
        try:
            ...
            if msg:
                ...
                if submodule_branch in ["master"]:
                    call([args.git_command, "pull", "--rebase", "origin", submodule_branch])
                else:
                    # I know this is a duplicate, testing things for buildbot...
                    call([args.git_command, "pull", "--rebase", "origin", submodule_branch])
        finally:
            os.chdir(cwd)
...
if __name__ == "__main__":
    ...
    if not args.no_covah:
        covah_skip_msg = git_update_skip(args)
        if covah_skip_msg:
            covah_skip_msg = "Blender repository skipped: " + covah_skip_msg + "\n"
        else:
            covah_update(args)
    ...
    skip_msg = covah_skip_msg + submodules_skip_msg
    if skip_msg:
