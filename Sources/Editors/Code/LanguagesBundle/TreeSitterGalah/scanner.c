#include <TreeSitterGalah/TreeSitterGalah.h>
#include <string.h>
#include <wctype.h>
...
const char* GALAH_OPERATORS[OPERATOR_COUNT] = {
    "->",
    ".",
    "&&",
    "||",
...
const enum TokenType OP_GALAH_SYMBOLS[OPERATOR_COUNT] = {
    ARROW_OPERATOR,
    DOT_OPERATOR,
    CONJUNCTION_OPERATOR,
    DISJUNCTION_OPERATOR,
...
const uint64_t GALAH_OP_SYMBOL_SUPPRESSOR[OPERATOR_COUNT] = {
    0, // ARROW_OPERATOR,
    0, // DOT_OPERATOR,
...
const char* RESERVED_GALAH_OPS[RESERVED_OP_COUNT] = {
    "/",
    "=",
    "-",
    "+",
...
void *tree_sitter_galah_external_scanner_create() {
    return calloc(0, sizeof(struct ScannerState));
}

void tree_sitter_galah_external_scanner_destroy(void *payload) {
    free(payload);
}

void tree_sitter_galah_external_scanner_reset(void *payload) {
    struct ScannerState *state = (struct ScannerState *)payload;
    state->ongoing_raw_str_hash_count = 0;
}

unsigned tree_sitter_galah_external_scanner_serialize(void *payload, char *buffer) {
    struct ScannerState *state = (struct ScannerState *)payload;
...
void tree_sitter_galah_external_scanner_deserialize(
    void *payload,
    const char *buffer,
    unsigned length
) {
...
static bool eat_operators(
    ...
    for (int op_idx = 0; op_idx < OPERATOR_COUNT; op_idx++) {
        possible_operators[op_idx] = valid_symbols[OP_GALAH_SYMBOLS[op_idx]] && (!prior_char || GALAH_OPERATORS[op_idx][0] == prior_char);
    }
    for (int op_idx = 0; op_idx < RESERVED_OP_COUNT; op_idx++) {
        reserved_operators[op_idx] = !prior_char || RESERVED_GALAH_OPS[op_idx][0] == prior_char;
    }
    ...
    while(true) {
        for (int op_idx = 0; op_idx < OPERATOR_COUNT; op_idx++) {
            ...
            if (GALAH_OPERATORS[op_idx][str_idx] != lexer->lookahead) {
                possible_operators[op_idx] = false;
                continue;
            }
        }
        ...
        for (int op_idx = 0; op_idx < RESERVED_OP_COUNT; op_idx++) {
            ...
            if (RESERVED_GALAH_OPS[op_idx][str_idx] == '\0') {
                reserved_operators[op_idx] = 0;
                continue;
            }

            if (RESERVED_GALAH_OPS[op_idx][str_idx] != lexer->lookahead) {
                reserved_operators[op_idx] = 0;
                continue;
            }

            if (RESERVED_GALAH_OPS[op_idx][str_idx + 1] == '\0') {
    ...
    if (full_match != -1) {
        ...
        // an immediate token.
        uint64_t suppressing_symbols = GALAH_OP_SYMBOL_SUPPRESSOR[full_match];
        ...
        *symbol_result = OP_GALAH_SYMBOLS[full_match];
        return true;
    }
...
bool tree_sitter_galah_external_scanner_scan(
    void *payload,
    TSLexer *lexer,
    const bool *valid_symbols
) {
