...
public extension TextLayoutManager
{
  ...
  func textLineForPosition(_ posY: CGFloat) -> TextLineStorage<TextLine>.TextLinePosition?
  {
    lineStorage.getLine(atPosition: posY)
  }

  func textLineForOffset(_ offset: Int) -> TextLineStorage<TextLine>.TextLinePosition?
  {
    ...
    {
      lineStorage.getLine(atOffset: offset)
    }
  }
  ...
  func textLineForIndex(_ index: Int) -> TextLineStorage<TextLine>.TextLinePosition?
  {
    guard index >= 0, index < lineStorage.count else { return nil }
    return lineStorage.getLine(atIndex: index)
  }

  func textOffsetAtPoint(_ point: CGPoint) -> Int?
  {
    guard point.y <= estimatedHeight()
    else
    { // End position is a special case.
  ...
  func rectForOffset(_ offset: Int) -> CGRect?
  {
    guard offset != lineStorage.length
    else
    {
  ...
  func ensureLayoutUntil(_ offset: Int)
  {
    guard let linePosition = lineStorage.getLine(atOffset: offset),
