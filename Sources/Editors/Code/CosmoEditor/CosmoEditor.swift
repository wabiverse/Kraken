...
public struct CosmoEditor: NSViewControllerRepresentable
{
  ...
  public class Coordinator: NSObject
  {
    ...
    @objc func textControllerCursorsDidUpdate(_: Notification)
    {
      guard !isUpdatingFromRepresentable else { return }
      isUpdateFromTextView = true
      parent._cursorPositions.wrappedValue = controller?.cursorPositions ?? []
      if controller != nil
      {
        parent.coordinators.forEach
        {
...
@available(*, unavailable, renamed: "CosmoEditor", message: "CosmoTextView has been renamed to CosmoEditor.")
public struct CosmoTextView: View
{
  public init(
    _: Binding<String>,
    language _: CodeLanguage,
    theme _: EditorTheme,
    font _: NSFont,
    tabWidth _: Int,
    indentOption _: IndentOption = .spaces(count: 4),
    lineHeight _: Double,
    wrapLines _: Bool,
    editorOverscroll _: CGFloat = 0,
    cursorPositions _: Binding<[CursorPosition]>,
    useThemeBackground _: Bool = true,
    highlightProvider _: HighlightProviding? = nil,
    contentInsets _: NSEdgeInsets? = nil,
    isEditable _: Bool = true,
    isSelectable _: Bool = true,
    letterSpacing _: Double = 1.0,
    bracketPairHighlight _: BracketPairHighlight? = nil,
    undoManager _: CEUndoManager? = nil,
    coordinators _: [any TextViewCoordinator] = []
  )
  {}
