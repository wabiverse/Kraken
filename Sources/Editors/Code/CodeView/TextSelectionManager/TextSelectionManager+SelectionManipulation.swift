...
public extension TextSelectionManager
{
  ...
  {
    guard let line = layoutManager?.textLineForOffset(offset),
          let lineText = textStorage?.substring(from: line.range)
    else
    {
      return NSRange(location: offset, length: 0)
    }
  ...
  private func findBeginningOfLineText(string: NSString, initialRange: NSRange) -> NSRange
  {
    ...
    string.enumerateSubstrings(in: foundRange, options: .byCaretPositions)
    { substring, _, _, stop in
  ...
  ) -> NSRange
  {
    ...
    {
      case .character:
        extendSelectionVerticalCharacter(from: offset, up: up, suggestedXPos: suggestedXPos)
      case .word, .line, .visualLine:
        extendSelectionVerticalLine(from: offset, up: up)
      case .container:
        extendSelectionContainer(from: offset, delta: up ? 1 : -1)
      case .document:
        if up
        {
        ...
        {
          NSRange(location: offset, length: (textStorage?.length ?? 0) - offset)
        }
    }
  }
  ...
  ) -> NSRange
  {
    ...
    else
    {
      ...
      guard let nextLine = layoutManager?.textLineForOffset(nextQueryIndex)
      else
      {
        return NSRange(location: offset, length: 0)
      }
