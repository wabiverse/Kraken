...
extension TextViewController
{
  ...
  func highlightSelectionPairs()
  {
    guard bracketPairHighlight != nil else { return }
    removeHighlightLayers()
    for range in textView.selectionManager.textSelections.map(\.range)
    {
      if range.isEmpty,
  ...
  func findClosingPair(_ close: String, _ open: String, from: Int, limit: Int, reverse: Bool) -> Int?
  {
  ...
  private func highlightCharacter(_ location: Int, scrollToRange: Bool = false)
  {
    guard let bracketPairHighlight,
          var rectToHighlight = textView.layoutManager.rectForOffset(location)
    else
    {
      return
    }
